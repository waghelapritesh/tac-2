...
Run TAC auto mode inside an isolated Docker sandbox so it cannot touch your host filesystem, SSH keys, or other projects.
...
docker sandbox create --template ./docker --name tac-sandbox

# Shell into the sandbox
docker sandbox exec -it tac-sandbox bash

# Inside the sandbox, run TAC
...
docker exec -it tac-sandbox bash

# 4. Run TAC inside the container
tac auto "implement the feature described in issue #42"
```

## UID/GID Remapping

The entrypoint handles UID/GID remapping via `PUID` and `PGID` environment variables. This avoids permission issues on bind-mounted volumes by matching the container's `tac` user to your host UID/GID.
...
1. **UID/GID remapping** — adjusts the `tac` user to match `PUID`/`PGID`
...
4. **Drop privileges** — `exec gosu tac` for proper PID 1 signal forwarding

No hardcoded `user:` directive in compose — the entrypoint starts as root, remaps, then drops to `tac`.

## Two-Terminal Workflow

TAC's recommended workflow uses two terminals — one for auto mode, one for interactive discussion:

```bash
# Terminal 1: auto mode
docker sandbox exec -it tac-sandbox bash
tac auto "your task description"
...
If you restrict outbound network access in your sandbox, TAC needs these endpoints:
...
Build with a specific TAC version:

```bash
docker compose -f docker/docker-compose.yaml build --build-arg TAC_VERSION=2.51.0
```
...
- **Named agent registration**: Docker Desktop's built-in named agents (claude, codex, etc.) are registered by Docker itself. Third-party tools cannot register new named agents. TAC uses the generic shell sandbox type with a custom template instead.
