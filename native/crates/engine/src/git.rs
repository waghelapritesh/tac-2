...
//!
//! Provides high-performance git operations for TAC, eliminating the need
...
/// Replaces: `git diff --name-status main...branch -- .tac/`
#[napi]
...
/// `exclude`: optional pathspec to exclude (e.g., ".tac/").
/// Replaces: `git diff main...branch -- .tac/` and `-- . :(exclude).tac/`
#[napi]
...
/// Replaces: `git branch --list milestone/*`, `git branch --list tac/*`
#[napi]
...
/// Simple branch pattern matching for patterns like "milestone/*", "tac/*/*"
fn matches_branch_pattern(name: &str, pattern: &str) -> bool {
    // Handle simple prefix/* patterns
    if let Some(prefix) = pattern.strip_suffix("/*") {
        // For "tac/*/*", this becomes "tac/*" after first strip
        if prefix.contains('*') {
            // Recursive: "tac/*/*" → name must start with "tac/" and have at least 2 segments after
...
/// Replaces: `git branch --merged main --list tac/*`
#[napi]
...
/// Replaces: `git for-each-ref refs/tac/snapshots/ --format=%(refname)`
#[napi]
