//! TAC `.tac/` directory file parser.
//!
//! Parses markdown files containing YAML-like frontmatter, section headings,
//! and structured content used by TAC's planning system (roadmaps, plans,
...
//! - `batchParseTacFiles`: walk a `.tac/` tree and parse all `.md` files in parallel
...
/// A single parsed TAC file from batch parsing.
#[napi(object)]
pub struct ParsedTacFile {
...
/// Batch-parse all `.md` files in a `.tac/` directory tree.
///
...
#[napi(js_name = "batchParseTacFiles")]
pub fn batch_parse_tac_files(directory: String) -> Result<BatchParseResult> {
    ...
    for (path, content) in &file_contents {
        ...
        parsed_files.push(ParsedTacFile {
            path: path.clone(),
            metadata,
            body: body.to_string(),
...
#[napi(js_name = "scanTacTree")]
pub fn scan_tac_tree(directory: String) -> Result<Vec<TacTreeEntry>> {
...
fn collect_tree_entries(base: &Path, dir: &Path, entries: &mut Vec<TacTreeEntry>) -> Result<()> {
