...
export class DiscordBot {
  ...
  /**
   * Set the EventBridge reference so the bot can dispatch /tac-verbose commands.
    ...
    switch (commandName) {
      case 'tac-status': {
        ...
        interaction.reply({ content, ephemeral: true }).catch((err) => {
          this.logger.warn('tac-status reply failed', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      ...
      case 'tac-start':
        this.handleTacStart(interaction).catch((err) => {
          this.logger.warn('tac-start handler error', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      case 'tac-stop':
        this.handleTacStop(interaction).catch((err) => {
          this.logger.warn('tac-stop handler error', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      case 'tac-verbose': {
        if (!this.eventBridge) {
          interaction.reply({ content: 'Event bridge not available.', ephemeral: true }).catch((err) => {
            this.logger.warn('tac-verbose reply failed', {
              error: err instanceof Error ? err.message : String(err),
            });
          });
          break;
  ...
  // ---------------------------------------------------------------------------
  // Private: /tac-start handler
  // ---------------------------------------------------------------------------

  private async handleTacStart(interaction: import('discord.js').ChatInputCommandInteraction): Promise<void> {
    await interaction.deferReply({ ephemeral: true });
    this.logger.info('tac-start: scanning projects');
    ...
    try {
      ...
      this.logger.error('tac-start: scan failed', {
        error: err instanceof Error ? err.message : String(err),
      });
    ...
    try {
      ...
      this.logger.info('tac-start: project selected', { projectPath });

      // Defer the update immediately — startSession can take 10-30s to spawn the TAC process,
      ...
      try {
        ...
        this.logger.error('tac-start: startSession failed', { error: errMsg, projectPath });
        await interaction.editReply({
      ...
      this.logger.info('tac-start: selection timed out');
      await interaction.editReply({ content: 'Selection timed out.', components: [] });
    }
  }

  // ---------------------------------------------------------------------------
  // Private: /tac-stop handler
  // ---------------------------------------------------------------------------

  private async handleTacStop(interaction: import('discord.js').ChatInputCommandInteraction): Promise<void> {
    await interaction.deferReply({ ephemeral: true });
    this.logger.info('tac-stop: listing sessions');
    ...
    try {
      ...
      this.logger.info('tac-stop: session selected', { sessionId });
      ...
      try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        this.logger.error('tac-stop: cancelSession failed', { error: errMsg, sessionId });
        await collected.update({
      ...
      // Timeout or other collector error
      this.logger.info('tac-stop: selection timed out');
      await interaction.editReply({ content: 'Selection timed out.', components: [] });
    }
  }
}
