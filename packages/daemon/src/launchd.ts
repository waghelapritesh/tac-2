...
/** Generate valid launchd plist XML for the TAC daemon. */
export function generatePlist(opts: PlistOptions): string {
  const home = homedir();
  const workDir = opts.workingDirectory ?? home;
  const stdoutPath = opts.stdoutPath ?? resolve(home, '.tac', 'daemon-stdout.log');
  const stderrPath = opts.stderrPath ?? resolve(home, '.tac', 'daemon-stderr.log');
