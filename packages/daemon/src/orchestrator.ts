/**
 * Orchestrator — LLM-powered agent for the #tac-control Discord channel.
 *
...
function resolveAnthropicApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error(
      'ANTHROPIC_API_KEY is required. Set it in your environment or run `tac config`.',
    );
  }
  return apiKey;
}
...
const SYSTEM_PROMPT = `You are TAC Control — a concise, capable orchestrator for managing TAC (Think. Architect. Code.) coding agent sessions via Discord.
...
const TOOLS: Tool[] = [
  {
    name: 'list_projects',
    description: 'List all detected projects across configured scan roots. Returns project names, paths, and detected markers (git, node, tac, etc.).',
    input_schema: {
  ...
  {
    name: 'start_session',
    description: 'Start a new TAC auto-mode session for a project. Provide the absolute project path. Optionally provide a command to run instead of the default "/tac auto".',
    input_schema: {
      type: 'object' as const,
      properties: {
        projectPath: { type: 'string', description: 'Absolute path to the project directory' },
        command: { type: 'string', description: 'Optional command to send instead of "/tac auto"' },
      },
  ...
  {
    name: 'get_status',
    description: 'Get the current status of all active TAC sessions. Shows project name, status, duration, and cost for each.',
    input_schema: {
      type: 'object' as const,
      properties: {},
      required: [],
  ...
  {
    name: 'stop_session',
    description: 'Stop a running TAC session. Provide a session ID or project name — fuzzy matching is used to find the session.',
    input_schema: {
      type: 'object' as const,
      properties: {
