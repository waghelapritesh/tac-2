...
function defaults(): DaemonConfig {
  return {
    ...
    projects: { scan_roots: [] },
    log: {
      file: resolve(homedir(), '.tac', 'daemon.log'),
      level: 'info',
      max_size_mb: 50,
    },
  };
 ...
 * Priority: explicit CLI arg → TAC_DAEMON_CONFIG env → ~/.tac/daemon.yaml
 */
export function resolveConfigPath(cliPath?: string): string {
  if (cliPath) return expandTilde(cliPath);
  const envPath = process.env['TAC_DAEMON_CONFIG'];
  if (envPath) return expandTilde(envPath);
  return resolve(homedir(), '.tac', 'daemon.yaml');
}
