...
describe('sanitizeChannelName', () => {
  it('converts basic path to tac-prefixed name', () => {
    assert.equal(sanitizeChannelName('/home/user/my-project'), 'tac-my-project');
  });

  it('converts path with special characters to hyphens', () => {
    assert.equal(sanitizeChannelName('/home/user/My_Cool.Project!v2'), 'tac-my-cool-project-v2');
  });
  ...
  it('truncates very long names to 100 chars', () => {
    ...
    assert.ok(result.startsWith('tac-'));
  });

  it('cleans leading/trailing dots and underscores', () => {
    assert.equal(sanitizeChannelName('/home/...___project___...'), 'tac-project');
  });

  it('returns tac-unnamed for empty basename', () => {
    assert.equal(sanitizeChannelName(''), 'tac-unnamed');
    assert.equal(sanitizeChannelName('/'), 'tac-unnamed');
  });

  it('returns tac-unnamed for basename with only special chars', () => {
    assert.equal(sanitizeChannelName('/home/!!!'), 'tac-unnamed');
  });

  it('collapses consecutive hyphens', () => {
    assert.equal(sanitizeChannelName('/home/a---b---c'), 'tac-a-b-c');
  });

  it('handles Windows-style backslash paths', () => {
    assert.equal(sanitizeChannelName('C:\\Users\\lex\\my-project'), 'tac-my-project');
  });

  it('handles name at exact prefix + 96 chars = 100 char limit', () => {
    // tac- is 4 chars, so a 96-char basename should produce exactly 100
    const name96 = 'a'.repeat(96);
    const result = sanitizeChannelName(`/home/${name96}`);
    assert.equal(result.length, 100);
    assert.equal(result, `tac-${'a'.repeat(96)}`);
  });
...
describe('ChannelManager', () => {
  ...
  it('resolveCategory creates category when not found', async () => {
    ...
    assert.equal(cat.name, 'TAC Projects');
  ...
  it('createProjectChannel creates text channel under category', async () => {
    ...
    assert.equal(channel.name, 'tac-my-project');
...
describe('buildCommands', () => {
  it('returns array with correct command names', () => {
    ...
    assert.ok(names.includes('tac-status'), 'should include tac-status');
    assert.ok(names.includes('tac-start'), 'should include tac-start');
    assert.ok(names.includes('tac-stop'), 'should include tac-stop');
    assert.ok(names.includes('tac-verbose'), 'should include tac-verbose');
  });
...
describe('command dispatch', () => {
  ...
  it('tac-status with no sessions produces empty message', () => {
    // Tests the formatSessionStatus path that /tac-status calls
  ...
  it('unknown command name is not in buildCommands list', () => {
    const commands = buildCommands();
    const names = commands.map((c) => c.name);
    assert.ok(!names.includes('tac-unknown'), 'unknown should not be in command list');
  });
...
// ---------- /tac-start and /tac-stop logic paths ----------

describe('/tac-start and /tac-stop logic', () => {
  // These test the observable logic paths exercised by the handlers.
  // Since handleTacStart/handleTacStop are private, we test the data layer
  // they depend on — project scanning, session listing, and edge cases.

  it('/tac-start: scanForProjects returning 0 projects', async () => {
  ...
  it('/tac-stop: getAllSessions returns empty when no sessions active', async () => {
  ...
  it('/tac-stop: filters to active sessions only', () => {
    // Simulate the filter logic used in handleTacStop
  ...
  it('/tac-start: >25 projects are truncated for select menu', () => {
