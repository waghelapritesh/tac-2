 ...
 * and cleanup without spawning real TAC processes.
 */
...
describe('SessionManager', () => {
  ...
  it('sends custom command when provided', async () => {
    ...
    await manager.startSession({ projectDir: '/tmp/custom-cmd', command: '/tac quick fix-typo' });
    const client = manager.lastClient!;

    assert.ok(client.prompted.includes('/tac quick fix-typo'));
    assert.ok(!client.prompted.includes('/tac auto'));
  });
