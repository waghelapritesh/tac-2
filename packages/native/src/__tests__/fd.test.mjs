...
const candidates = [
  path.join(addonDir, `tac_engine.${platformTag}.node`),
  path.join(addonDir, "tac_engine.dev.node"),
];
...
if (!native) {
  console.error("Native addon not found. Run `npm run build:native -w @tac/native` first.");
  process.exit(1);
}

describe("native fd: fuzzyFind()", () => {
  test("finds files matching a query", (t) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "tac-fd-test-"));
