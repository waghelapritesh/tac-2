/**
 * Tests that the @tac/native package.json is correctly configured
...
describe("@tac/native module compatibility (#2861)", () => {
  ...
  test("package.json should explicitly declare type: commonjs", () => {
    ...
    // (e.g. tac-2), an absent "type" field would inherit the parent's
  ...
  test(
    ...
    () => {
      ...
      const parentDir = mkdtempSync(path.join(os.tmpdir(), "tac-native-modcompat-"));
