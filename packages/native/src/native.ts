 ...
 * Resolution order:
 *   1. @waghelapritesh/engine-{platform} npm optional dependency (production install)
 *   2. native/addon/tac_engine.{platform}.node (local release build)
 *   3. native/addon/tac_engine.dev.node (local debug build)
 */
...
function loadNative(): Record<string, unknown> {
  ...
  if (packageSuffix) {
    try {
      _loadedSuccessfully = true; return _require(`@waghelapritesh/engine-${packageSuffix}`) as Record<string, unknown>;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      errors.push(`@waghelapritesh/engine-${packageSuffix}: ${message}`);
    }
  }

  // 2. Try local release build (native/addon/tac_engine.{platform}.node)
  const releasePath = path.join(addonDir, `tac_engine.${platformTag}.node`);
  try {
  ...
  // 3. Try local dev build (native/addon/tac_engine.dev.node)
  const devPath = path.join(addonDir, "tac_engine.dev.node");
  try {
  ...
  process.stderr.write(
    `[tac] Native addon not available for ${platformTag}. Falling back to JS implementations (slower).\n` +
...
export const native = loadNative() as {
  ...
  batchParseTacFiles: (directory: string) => unknown;
