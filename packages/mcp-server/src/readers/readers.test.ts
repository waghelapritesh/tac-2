...
function tmpProject(): string {
  const dir = join(tmpdir(), `tac-mcp-test-${randomBytes(4).toString('hex')}`);
...
describe('readProgress', () => {
  ...
  before(() => {
    ...
    writeFixture(projectDir, '.tac/STATE.md', `# TAC State
...
`);
    ...
    mkdirSync(join(projectDir, '.tac/milestones/M003'), { recursive: true });
  });
  ...
  it('returns defaults for missing .tac/', () => {
...
describe('readRoadmap', () => {
  ...
  before(() => {
    ...
    writeFixture(projectDir, '.tac/milestones/M001/M001-CONTEXT.md', '# M001: Core Setup\n');
    writeFixture(projectDir, '.tac/milestones/M001/M001-ROADMAP.md', `# M001: Core Setup
...
`);
    ...
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/S01-PLAN.md', `# S01: Database schema
...
`);
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T01-PLAN.md', '# T01');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T01-SUMMARY.md', '# T01 done');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T02-PLAN.md', '# T02');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T02-SUMMARY.md', '# T02 done');

    writeFixture(projectDir, '.tac/milestones/M001/slices/S02/S02-PLAN.md', `# S02: API endpoints
...
`);
    writeFixture(projectDir, '.tac/milestones/M001/slices/S02/tasks/T01-PLAN.md', '# T01');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S02/tasks/T02-PLAN.md', '# T02');
  });
...
describe('readHistory', () => {
  ...
  before(() => {
    projectDir = tmpProject();
    writeFixture(projectDir, '.tac/metrics.json', JSON.stringify({
      version: 1,
  ...
  it('returns empty for missing metrics', () => {
    const empty = tmpProject();
    mkdirSync(join(empty, '.tac'), { recursive: true });
...
describe('readCaptures', () => {
  ...
  before(() => {
    projectDir = tmpProject();
    writeFixture(projectDir, '.tac/CAPTURES.md', `# Captures
...
describe('readKnowledge', () => {
  ...
  before(() => {
    projectDir = tmpProject();
    writeFixture(projectDir, '.tac/KNOWLEDGE.md', `# Project Knowledge
...
describe('runDoctorLite', () => {
  ...
  before(() => {
    ...
    writeFixture(projectDir, '.tac/PROJECT.md', '# Test Project');
    writeFixture(projectDir, '.tac/STATE.md', '# TAC State');
    writeFixture(projectDir, '.tac/milestones/M001/M001-CONTEXT.md', '# M001');
    writeFixture(projectDir, '.tac/milestones/M001/M001-ROADMAP.md', '# Roadmap');
    writeFixture(projectDir, '.tac/milestones/M001/M001-SUMMARY.md', '# Done');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/S01-PLAN.md', '# Plan');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T01-PLAN.md', '# T01');
    writeFixture(projectDir, '.tac/milestones/M001/slices/S01/tasks/T01-SUMMARY.md', '# T01 done');

    // M002: incomplete — has all tasks done but no SUMMARY
    writeFixture(projectDir, '.tac/milestones/M002/M002-CONTEXT.md', '# M002');
    writeFixture(projectDir, '.tac/milestones/M002/M002-ROADMAP.md', '# Roadmap');
    writeFixture(projectDir, '.tac/milestones/M002/slices/S01/S01-PLAN.md', '# Plan');
    writeFixture(projectDir, '.tac/milestones/M002/slices/S01/tasks/T01-PLAN.md', '# T01');
    writeFixture(projectDir, '.tac/milestones/M002/slices/S01/tasks/T01-SUMMARY.md', '# T01 done');
  ...
  it('returns ok:true for healthy project', () => {
    ...
    writeFixture(healthy, '.tac/PROJECT.md', '# Project');
    writeFixture(healthy, '.tac/STATE.md', '# State');
    const result = runDoctorLite(healthy);
    assert.equal(result.ok, true);
    rmSync(healthy, { recursive: true, force: true });
  });

  it('handles missing .tac/ gracefully', () => {
    const empty = tmpProject();
    const result = runDoctorLite(empty);
    assert.equal(result.ok, true);
    assert.ok(
      result.issues.some(
        (issue) => issue.code === 'no_tac_directory' || issue.code === 'missing_project_md',
      ),
    );
    rmSync(empty, { recursive: true, force: true });
  });
