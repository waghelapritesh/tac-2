// TAC MCP Server — roadmap structure reader
...
function readMilestoneTitle(tacRoot: string, mid: string): string {
...
function readVision(tacRoot: string, mid: string): string {
  const roadmapPath = resolveMilestoneFile(tacRoot, mid, 'ROADMAP');
...
export function readRoadmap(projectDir: string, filterMilestoneId?: string): RoadmapResult {
  const tac = resolveTacRoot(projectDir);
  let milestoneIds = findMilestoneIds(tac);
  ...
  for (const mid of milestoneIds) {
    const title = readMilestoneTitle(tac, mid);
    const vision = readVision(tac, mid);

    const summaryPath = resolveMilestoneFile(tac, mid, 'SUMMARY');
    const hasSummary = summaryPath !== null && existsSync(summaryPath);

    const roadmapPath = resolveMilestoneFile(tac, mid, 'ROADMAP');
    let roadmapSlices: ReturnType<typeof parseRoadmapTable> = [];
    if (roadmapPath && existsSync(roadmapPath)) {
      roadmapSlices = parseRoadmapTable(readFileSync(roadmapPath, 'utf-8'));
    }

    const fsSliceIds = findSliceIds(tac, mid);
    ...
    for (const sid of Array.from(sliceIdSet).sort()) {
      ...
      const taskFiles = findTaskFiles(tac, mid, sid);

      const planPath = resolveSliceFile(tac, mid, sid, 'PLAN');
