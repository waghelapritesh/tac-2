// TAC MCP Server — knowledge graph reader tests
...
function tmpProject(): string {
  const dir = join(tmpdir(), `tac-graph-test-${randomBytes(4).toString('hex')}`);
...
function makeProjectWithArtifacts(projectDir: string): void {
  writeFixture(projectDir, '.tac/STATE.md', [
    '# TAC State',
    '',
  ...
  writeFixture(projectDir, '.tac/KNOWLEDGE.md', [
    '# Project Knowledge',
    '',
    '## Rules',
    '',
  ...
  writeFixture(projectDir, '.tac/milestones/M001/M001-ROADMAP.md', [
    '# M001: Auth System',
    '',
    '## Vision',
    '',
  ...
  writeFixture(projectDir, '.tac/milestones/M001/slices/S01/S01-PLAN.md', [
    '# S01: Login flow',
    '',
    '## Tasks',
    '',
...
function writeLearningsFixture(projectDir: string, milestoneId: string, content: string): void {
  writeFixture(projectDir, `.tac/milestones/${milestoneId}/${milestoneId}-LEARNINGS.md`, content);
}
...
describe('buildGraph', () => {
  ...
  it('skips unparseable artifact and does not throw', async () => {
    ...
    writeFixture(badProject, '.tac/STATE.md', 'not valid tac state at all \0\0\0');
  ...
  it('returns empty graph for project with no .tac/ directory', async () => {
    ...
    // contract for a .tac-less project: truly empty graph.
    assert.deepEqual(graph.nodes, [], "nodes must be empty for .tac-less project");
    assert.deepEqual(graph.edges, [], "edges must be empty for .tac-less project");
...
describe('buildGraph — LEARNINGS.md parsing', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(projectDir, '.tac', 'milestones', 'M001'), { recursive: true });
  ...
  it('skips LEARNINGS.md gracefully when file is malformed', async () => {
    const badProject = tmpProject();
    mkdirSync(join(badProject, '.tac', 'milestones', 'M002'), { recursive: true });
  ...
  it('produces no learning nodes when all sections are empty', async () => {
    const emptyProject = tmpProject();
    mkdirSync(join(emptyProject, '.tac', 'milestones', 'M003'), { recursive: true });
...
`);
  ...
  it('does not crash when LEARNINGS.md is missing entirely', async () => {
    ...
    mkdirSync(join(noLearningsProject, '.tac', 'milestones', 'M004'), { recursive: true });
...
describe('writeGraph', () => {
  ...
  it('creates graph.json in .tac/graphs/ after writeGraph()', async () => {
    const tacRoot = join(projectDir, '.tac');
    await writeGraph(tacRoot, graph);
    const graphPath = join(tacRoot, 'graphs', 'graph.json');
  ...
  it('write is atomic — no temp file remains after writeGraph()', async () => {
    const tacRoot = join(projectDir, '.tac');
    await writeGraph(tacRoot, graph);
    const tmpPath = join(tacRoot, 'graphs', 'graph.tmp.json');
  ...
  it('written graph.json is valid JSON with nodes and edges', async () => {
    const tacRoot = join(projectDir, '.tac');
    await writeGraph(tacRoot, graph);
    const raw = readFileSync(join(tacRoot, 'graphs', 'graph.json'), 'utf-8');
...
describe('graphStatus', () => {
  ...
  it('stale = true for a graph older than 24h (builtAt backdated)', async () => {
    makeProjectWithArtifacts(projectDir);
    const tacRoot = join(projectDir, '.tac');
    mkdirSync(join(tacRoot, 'graphs'), { recursive: true });
    ...
    writeFileSync(
      join(tacRoot, 'graphs', 'graph.json'),
...
describe('graphQuery', () => {
  ...
  it('budget trims AMBIGUOUS edges first — keeps INFERRED edge when budget only forces one drop', async () => {
    ...
    await writeGraph(tacRoot, mixedGraph);
    ...
    await writeGraph(tacRoot, originalGraph);
  });
});
...
describe('graphDiff', () => {
  ...
  it('returns added nodes when a new node appears after snapshot', async () => {
    ...
    await writeGraph(tacRoot, extraGraph);
  ...
  it('returns removed nodes when a node disappears after snapshot', async () => {
    ...
    writeFileSync(
      join(tacRoot, 'graphs', '.last-build-snapshot.json'),
  ...
  it('writeSnapshot creates .last-build-snapshot.json with snapshotAt', async () => {
    const tacRoot = join(projectDir, '.tac');
    await writeSnapshot(tacRoot);
    const snapshotPath = join(tacRoot, 'graphs', '.last-build-snapshot.json');
