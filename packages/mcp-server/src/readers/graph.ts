// TAC MCP Server — knowledge graph reader
// Copyright (c) 2026 Jeremy McSpadden <jeremy@fluxlabs.net>

/**
 * Knowledge Graph for TAC projects.
 *
 * Parses .tac/ artifacts (STATE.md, milestone ROADMAPs, slice PLANs,
...
import { resolveTacRoot, findMilestoneIds, resolveMilestoneDir, findSliceIds, resolveSliceDir } from './paths.js';
...
function graphsDir(tacRoot: string): string {
  return join(tacRoot, 'graphs');
}

function graphJsonPath(tacRoot: string): string {
  return join(graphsDir(tacRoot), 'graph.json');
}

function graphTmpPath(tacRoot: string): string {
  return join(graphsDir(tacRoot), 'graph.tmp.json');
}

function snapshotPath(tacRoot: string): string {
  return join(graphsDir(tacRoot), '.last-build-snapshot.json');
}
 ...
 */
function parseStateFile(tacRoot: string, nodes: GraphNode[], _edges: GraphEdge[]): void {
  const statePath = join(tacRoot, 'STATE.md');
 ...
 */
function parseKnowledgeFile(tacRoot: string, nodes: GraphNode[], _edges: GraphEdge[]): void {
  const knowledgePath = join(tacRoot, 'KNOWLEDGE.md');
...
function parseMilestoneFiles(
  ...
  const milestoneIds = findMilestoneIds(tacRoot);

  for (const milestoneId of milestoneIds) {
    try {
      parseSingleMilestone(tacRoot, milestoneId, nodes, edges);
    } catch {
...
function parseSingleMilestone(
  ...
  const mDir = resolveMilestoneDir(tacRoot, milestoneId);
  if (!mDir) return;
  ...
  const sliceIds = findSliceIds(tacRoot, milestoneId);
  for (const sliceId of sliceIds) {
    try {
      parseSingleSlice(tacRoot, milestoneId, sliceId, milestoneNodeId, nodes, edges);
    } catch {
...
function parseSingleSlice(
  ...
  const sDir = resolveSliceDir(tacRoot, milestoneId, sliceId);
  if (!sDir) return;
 ...
 */
function parseLearningsFiles(tacRoot: string, nodes: GraphNode[], edges: GraphEdge[]): void {
  const milestoneIds = findMilestoneIds(tacRoot);

  for (const milestoneId of milestoneIds) {
    try {
      parseSingleLearningsFile(tacRoot, milestoneId, nodes, edges);
    } catch {
...
/**
 * Build a KnowledgeGraph by parsing all .tac/ artifacts.
 *
...
export async function buildGraph(projectDir: string): Promise<KnowledgeGraph> {
  const tacRoot = resolveTacRoot(resolve(projectDir));
...
/**
 * Write the graph to .tac/graphs/graph.json atomically.
 *
 * Writes to graph.tmp.json first, then renames to graph.json.
 * Creates the graphs/ directory if it does not exist.
 */
export async function writeGraph(tacRoot: string, graph: KnowledgeGraph): Promise<void> {
  const dir = graphsDir(tacRoot);
  mkdirSync(dir, { recursive: true });

  const tmp = graphTmpPath(tacRoot);
  const final = graphJsonPath(tacRoot);
 ...
 */
export async function writeSnapshot(tacRoot: string): Promise<void> {
  const src = graphJsonPath(tacRoot);
  ...
  writeFileSync(snapshotPath(tacRoot), JSON.stringify(snapshot, null, 2), 'utf-8');
}
...
export async function graphStatus(projectDir: string): Promise<GraphStatusResult> {
  const tacRoot = resolveTacRoot(resolve(projectDir));
  const graphPath = graphJsonPath(tacRoot);
...
export async function graphDiff(projectDir: string): Promise<GraphDiffResult> {
  ...
  const graphPath = graphJsonPath(tacRoot);
  const snap = snapshotPath(tacRoot);
