// TAC MCP Server — .tac/ directory resolution
...
/**
 * Resolve the .tac/ root directory for a project.
 *
 * Probes in order:
 *   1. projectDir/.tac (fast path)
 *   2. git repo root/.tac
 *   3. Walk up from projectDir
 *   4. Fallback: projectDir/.tac (even if missing — for init)
 */
export function resolveTacRoot(projectDir: string): string {
  const resolved = resolve(projectDir);

  // Fast path: .tac/ in the given directory
  const direct = join(resolved, '.tac');
  ...
  try {
    ...
    const gitTac = join(gitRoot, '.tac');
    if (existsSync(gitTac) && statSync(gitTac).isDirectory()) {
      return gitTac;
    }
  } catch {
...
/** Resolve path to a .tac/ root file (STATE.md, KNOWLEDGE.md, etc.) */
export function resolveRootFile(tacRoot: string, name: string): string {
  return join(tacRoot, name);
}

/** Resolve path to milestones directory */
export function milestonesDir(tacRoot: string): string {
  return join(tacRoot, 'milestones');
}
 ...
 */
export function findMilestoneIds(tacRoot: string): string[] {
  const dir = milestonesDir(tacRoot);
 ...
 */
export function resolveMilestoneDir(tacRoot: string, milestoneId: string): string | null {
  const dir = milestonesDir(tacRoot);
 ...
 */
export function resolveMilestoneFile(tacRoot: string, milestoneId: string, suffix: string): string | null {
...
export function findSliceIds(tacRoot: string, milestoneId: string): string[] {
  const mDir = resolveMilestoneDir(tacRoot, milestoneId);
  if (!mDir) return [];
...
export function resolveSliceDir(tacRoot: string, milestoneId: string, sliceId: string): string | null {
  const mDir = resolveMilestoneDir(tacRoot, milestoneId);
  if (!mDir) return null;
...
export function resolveSliceFile(
  tacRoot: string, milestoneId: string, sliceId: string, suffix: string,
): string | null {
...
export function findTaskFiles(
  tacRoot: string, milestoneId: string, sliceId: string,
