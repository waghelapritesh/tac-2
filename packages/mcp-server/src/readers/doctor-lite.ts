// TAC MCP Server — lightweight structural health checks
...
function checkProjectLevel(tacRoot: string, issues: DoctorIssue[]): void {
  // PROJECT.md should exist
  const projectPath = resolveRootFile(tacRoot, 'PROJECT.md');
  ...
  const milestones = findMilestoneIds(tacRoot);
  if (milestones.length > 0) {
    const statePath = resolveRootFile(tacRoot, 'STATE.md');
    if (!existsSync(statePath)) {
      issues.push({
        ...
        scope: 'project',
        unitId: '',
        message: 'STATE.md is missing — run /tac status to regenerate',
        file: statePath,
      });
    }
  }
...
function checkMilestoneLevel(tacRoot: string, mid: string, issues: DoctorIssue[]): void {
  const mDir = resolveMilestoneDir(tacRoot, mid);
  if (!mDir) {
    issues.push({
      severity: 'error',
  ...
  const ctxPath = resolveMilestoneFile(tacRoot, mid, 'CONTEXT');
  if (!ctxPath || !existsSync(ctxPath)) {
    // Check for draft
    const draftPath = resolveMilestoneFile(tacRoot, mid, 'CONTEXT-DRAFT');
  ...
  const sliceIds = findSliceIds(tacRoot, mid);
  if (sliceIds.length > 0) {
    const roadmapPath = resolveMilestoneFile(tacRoot, mid, 'ROADMAP');
  ...
  if (sliceIds.length > 0) {
    const allDone = sliceIds.every((sid) => {
      const tasks = findTaskFiles(tacRoot, mid, sid);
      return tasks.length > 0 && tasks.every((t) => t.hasSummary);
    });
    const summaryPath = resolveMilestoneFile(tacRoot, mid, 'SUMMARY');
...
function checkSliceLevel(
  tacRoot: string, mid: string, sid: string, issues: DoctorIssue[],
): void {
  const unitId = `${mid}/${sid}`;

  // PLAN.md should exist
  const planPath = resolveSliceFile(tacRoot, mid, sid, 'PLAN');
...
export function runDoctorLite(projectDir: string, scope?: string): DoctorResult {
  ...
  if (!existsSync(tacRoot)) {
    return {
      ok: true,
      issues: [{
        severity: 'info',
        code: 'no_tac_directory',
        scope: 'project',
        unitId: '',
        message: 'No .tac/ directory found — project not initialized',
      }],
  ...
  checkProjectLevel(tacRoot, issues);

  // Milestone + slice checks
  const milestoneIds = scope
    ? findMilestoneIds(tacRoot).filter((id) => id === scope)
    : findMilestoneIds(tacRoot);

  for (const mid of milestoneIds) {
    checkMilestoneLevel(tacRoot, mid, issues);

    const sliceIds = findSliceIds(tacRoot, mid);
    for (const sid of sliceIds) {
      checkSliceLevel(tacRoot, mid, sid, issues);
    }
  }
