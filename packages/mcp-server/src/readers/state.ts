// TAC MCP Server — project state reader
...
function countSlicesAndTasks(tacRoot: string, milestoneIds: string[]): {
...
export function readProgress(projectDir: string): ProgressResult {
  ...
  const statePath = resolveRootFile(tac, 'STATE.md');
  ...
  if (!existsSync(statePath)) {
    ...
    const milestoneIds = findMilestoneIds(tac);
    result.milestones.total = milestoneIds.length;
    result.milestones.pending = milestoneIds.length;
    const counts = countSlicesAndTasks(tac, milestoneIds);
