...
import { _getAdapter, closeDatabase } from "../../../src/resources/extensions/tac/tac-db.ts";
import { registerWorkflowTools, WORKFLOW_TOOL_NAMES, validateProjectDir } from "./workflow-tools.ts";

function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-mcp-workflow-${randomUUID()}`);
...
function writeWriteGateSnapshot(
  ...
  mkdirSync(join(base, ".tac", "runtime"), { recursive: true });
  writeFileSync(
    join(base, ".tac", "runtime", "write-gate-state.json"),
    JSON.stringify(
      {
...
describe("workflow MCP tools", () => {
  ...
  it("tac_summary_save writes artifact through the shared executor", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const tool = server.tools.find((t) => t.name === "tac_summary_save");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md")),
  ...
  it("rejects workflow tool calls outside the configured project root", async () => {
    ...
    const prevRoot = process.env.TAC_WORKFLOW_PROJECT_ROOT;
    try {
      process.env.TAC_WORKFLOW_PROJECT_ROOT = base;
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const tool = server.tools.find((t) => t.name === "tac_summary_save");
      assert.ok(tool, "summary tool should be registered");
      ...
      if (prevRoot === undefined) {
        delete process.env.TAC_WORKFLOW_PROJECT_ROOT;
      } else {
        process.env.TAC_WORKFLOW_PROJECT_ROOT = prevRoot;
      }
      cleanup(base);
      cleanup(otherBase);
    }
  ...
  it("rejects non-file executor module URLs", async () => {
    const base = makeTmpBase();
    const prevModule = process.env.TAC_WORKFLOW_EXECUTORS_MODULE;
    const prevRoot = process.env.TAC_WORKFLOW_PROJECT_ROOT;
    try {
      process.env.TAC_WORKFLOW_PROJECT_ROOT = base;
      process.env.TAC_WORKFLOW_EXECUTORS_MODULE = "data:text/javascript,export default {}";
      ...
      if (prevModule === undefined) {
        delete process.env.TAC_WORKFLOW_EXECUTORS_MODULE;
      } else {
        process.env.TAC_WORKFLOW_EXECUTORS_MODULE = prevModule;
      }
      if (prevRoot === undefined) {
        delete process.env.TAC_WORKFLOW_PROJECT_ROOT;
      } else {
  ...
  it("blocks workflow mutation tools while a discussion gate is pending", async () => {
    const base = makeTmpBase();
    try {
      mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S01"), { recursive: true });
      writeFileSync(
        join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md"),
        "# S01\n\n- [ ] **T01: Demo** `est:5m`\n",
      );
      ...
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
  ...
  it("blocks workflow mutation tools during queue mode", async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        /planning tool .* not executes work|Cannot tac_task_complete|Unknown tools are not permitted during queue mode/,
      );
    } finally {
      cleanup(base);
    }
  ...
  it("tac_task_complete and tac_milestone_status work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S01"), { recursive: true });
      writeFileSync(
      ...
      registerWorkflowTools(server as any);
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
      const statusTool = server.tools.find((t) => t.name === "tac_milestone_status");
      assert.ok(taskTool, "task tool should be registered");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks", "T01-SUMMARY.md")),
  ...
  it("#4477 tac_task_complete forwards every schema field to the executor (regression for destructure-rebuild bug class)", async () => {
    ...
    // module that captures the args, calling tac_task_complete with an
    ...
    const prevModule = process.env.TAC_WORKFLOW_EXECUTORS_MODULE;
    const prevCapture = process.env.TAC_TEST_TASK_COMPLETE_CAPTURE_PATH;
    try {
...
export const executeTaskComplete = async (params, projectDir) => {
  const capturePath = process.env.TAC_TEST_TASK_COMPLETE_CAPTURE_PATH;
  if (capturePath) {
...
`;
      ...
      process.env.TAC_WORKFLOW_EXECUTORS_MODULE = mockModulePath;
      process.env.TAC_TEST_TASK_COMPLETE_CAPTURE_PATH = capturePath;
      ...
      if (prevCapture === undefined) {
        delete process.env.TAC_TEST_TASK_COMPLETE_CAPTURE_PATH;
      } else {
        process.env.TAC_TEST_TASK_COMPLETE_CAPTURE_PATH = prevCapture;
      }
      cleanup(base);
    }
  });

  it("tac_complete_task alias delegates to tac_task_complete behavior", async () => {
    const base = makeTmpBase();
    try {
      mkdirSync(join(base, ".tac", "milestones", "M002", "slices", "S02"), { recursive: true });
      writeFileSync(
        join(base, ".tac", "milestones", "M002", "slices", "S02", "S02-PLAN.md"),
        "# S02\n\n- [ ] **T02: Demo** `est:5m`\n",
      );

      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const aliasTool = server.tools.find((t) => t.name === "tac_complete_task");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M002", "slices", "S02", "tasks", "T02-SUMMARY.md")),
  ...
  it("tac_plan_milestone and tac_plan_slice work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const milestoneTool = server.tools.find((t) => t.name === "tac_plan_milestone");
      const sliceTool = server.tools.find((t) => t.name === "tac_plan_slice");
        ...
        tasks: [
          {
            ...
            estimate: "15m",
            files: ["src/resources/extensions/tac/tools/workflow-tool-executors.ts"],
            verify: "node --test",
            inputs: ["ROADMAP.md"],
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md")),
        "slice plan should exist on disk",
      );
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks", "T01-PLAN.md")),
  ...
  it("other workflow tools reject empty required strings at the schema layer", async () => {
    ...
    try {
      ...
      await expectRejection("tac_plan_slice", {
        projectDir: base,
        milestoneId: "M001",
        sliceId: "",
      ...
      // Empty milestoneId on tac_plan_task
      await expectRejection("tac_plan_task", {
        projectDir: base,
        milestoneId: "",
        sliceId: "S01",
        taskId: "T01",
      ...
      // Empty assessment on tac_reassess_roadmap
      await expectRejection("tac_reassess_roadmap", {
        projectDir: base,
        milestoneId: "M001",
      ...
      // Empty keyRisks[i].risk on tac_plan_milestone top-level arrays
      await expectRejection("tac_plan_milestone", {
        projectDir: base,
        milestoneId: "M001",
        title: "T",
        vision: "V",
      ...
      // Empty blockerDescription on tac_replan_slice
      await expectRejection("tac_replan_slice", {
        projectDir: base,
        milestoneId: "M001",
        sliceId: "S01",
        blockerTaskId: "T01",
      ...
      // Empty milestoneId on tac_task_complete
      await expectRejection("tac_task_complete", {
        projectDir: base,
        taskId: "T01",
        sliceId: "S01",
        milestoneId: "",
  ...
  it("tac_plan_milestone rejects empty slice fields up front with all violations", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
  ...
  it("tac_plan_milestone rejects a full slice with missing heavy fields via a behavioral round-trip", async () => {
  ...
  it("tac_plan_milestone requires sketchScope when isSketch=true and skips heavy fields", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
  ...
  it("tac_requirement_save opens the DB before inline requirement writes", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const requirementTool = server.tools.find((t) => t.name === "tac_requirement_save");
      ...
      assert.ok(existsSync(join(base, ".tac", "REQUIREMENTS.md")), "REQUIREMENTS.md should be written to disk");
  ...
  it("tac_plan_task reopens the DB before inline task planning writes", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      const milestoneTool = server.tools.find((t) => t.name === "tac_plan_milestone");
      const sliceTool = server.tools.find((t) => t.name === "tac_plan_slice");
      const taskTool = server.tools.find((t) => t.name === "tac_plan_task");
      assert.ok(milestoneTool, "milestone planning tool should be registered");
      assert.ok(sliceTool, "slice planning tool should be registered");
        ...
        slices: [
          {
            ...
            risk: "medium",
            depends: [],
            demo: "Inline tac_plan_task reopens the DB after it was closed.",
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M010", "slices", "S10", "tasks", "T11-PLAN.md")),
  ...
  it("tac_replan_slice and tac_slice_replan work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      ...
      const sliceTool = server.tools.find((t) => t.name === "tac_plan_slice");
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
      const canonicalTool = server.tools.find((t) => t.name === "tac_replan_slice");
      const aliasTool = server.tools.find((t) => t.name === "tac_slice_replan");
      assert.ok(milestoneTool, "milestone planning tool should be registered");
      assert.ok(sliceTool, "slice planning tool should be registered");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M099", "slices", "S09", "S09-REPLAN.md")),
        "replan artifact should exist on disk",
      );
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M099", "slices", "S09", "S09-PLAN.md")),
  ...
  it("tac_slice_complete and tac_complete_slice work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      ...
      const sliceTool = server.tools.find((t) => t.name === "tac_plan_slice");
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
      const canonicalTool = server.tools.find((t) => t.name === "tac_slice_complete");
      const aliasTool = server.tools.find((t) => t.name === "tac_complete_slice");
      assert.ok(milestoneTool, "milestone planning tool should be registered");
      assert.ok(sliceTool, "slice planning tool should be registered");
      assert.ok(taskTool, "task completion tool should be registered");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M004", "slices", "S04", "S04-SUMMARY.md")),
        "alias should write slice summary to disk",
      );
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M004", "slices", "S04", "S04-UAT.md")),
  ...
  it("tac_validate_milestone and tac_milestone_complete work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      ...
      const sliceTool = server.tools.find((t) => t.name === "tac_plan_slice");
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
      const completeSliceTool = server.tools.find((t) => t.name === "tac_slice_complete");
      const validateTool = server.tools.find((t) => t.name === "tac_validate_milestone");
      const completeMilestoneAlias = server.tools.find((t) => t.name === "tac_milestone_complete");
      assert.ok(milestoneTool, "milestone planning tool should be registered");
      assert.ok(sliceTool, "slice planning tool should be registered");
      assert.ok(taskTool, "task completion tool should be registered");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M005", "M005-VALIDATION.md")),
        "validation artifact should exist on disk",
      );
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M005", "M005-SUMMARY.md")),
  ...
  it("tac_reassess_roadmap, tac_roadmap_reassess, and tac_save_gate_result work end-to-end", async () => {
    const base = makeTmpBase();
    try {
      const server = makeMockServer();
      registerWorkflowTools(server as any);
      ...
      const taskTool = server.tools.find((t) => t.name === "tac_task_complete");
      const completeSliceTool = server.tools.find((t) => t.name === "tac_slice_complete");
      const reassessTool = server.tools.find((t) => t.name === "tac_reassess_roadmap");
      const reassessAlias = server.tools.find((t) => t.name === "tac_roadmap_reassess");
      const gateTool = server.tools.find((t) => t.name === "tac_save_gate_result");
      assert.ok(milestoneTool, "milestone planning tool should be registered");
      assert.ok(sliceTool, "slice planning tool should be registered");
      assert.ok(taskTool, "task completion tool should be registered");
      assert.ok(completeSliceTool, "slice completion tool should be registered");
      ...
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M006", "slices", "S06", "S06-ASSESSMENT.md")),
        "assessment artifact should exist on disk",
      );
      assert.ok(
        existsSync(join(base, ".tac", "milestones", "M006", "M006-ROADMAP.md")),
...
describe("validateProjectDir", () => {
  it("rejects a symlink inside the allowed root that points outside it", () => {
    ...
    const prevRoot = process.env.TAC_WORKFLOW_PROJECT_ROOT;
    try {
      process.env.TAC_WORKFLOW_PROJECT_ROOT = allowedRoot;
      assert.throws(
  ...
  it("accepts a non-existent path inside the allowed root (new worktree case)", () => {
    ...
    const prevRoot = process.env.TAC_WORKFLOW_PROJECT_ROOT;
    try {
      process.env.TAC_WORKFLOW_PROJECT_ROOT = canonicalRoot;
