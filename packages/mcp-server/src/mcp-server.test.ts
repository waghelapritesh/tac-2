/**
 * @waghelapritesh/mcp-server — Integration and unit tests.
 *
 * Strategy: We cannot mock @waghelapritesh/rpc-client at the module level without
...
class TestableSessionManager extends SessionManager {
  ...
  override async startSession(projectDir: string, options: { cliPath?: string; command?: string; model?: string; bare?: boolean } = {}): Promise<string> {
    ...
    try {
      ...
      // Kick off auto-mode
      const command = options.command ?? '/tac auto';
...
describe('SessionManager', () => {
  ...
  it('startSession creates session and returns sessionId', async () => {
    const sessionId = await sm.startSession('/tmp/test-project', { cliPath: '/usr/bin/tac' });
  ...
  it('startSession sends /tac auto by default', async () => {
    await sm.startSession('/tmp/test-prompt', { cliPath: '/usr/bin/tac' });
    assert.ok(sm.lastClient);
    assert.deepEqual(sm.lastClient.prompted, ['/tac auto']);
  });

  it('startSession sends custom command when provided', async () => {
    await sm.startSession('/tmp/test-cmd', { cliPath: '/usr/bin/tac', command: '/tac auto --resume' });
    assert.ok(sm.lastClient);
    assert.deepEqual(sm.lastClient.prompted, ['/tac auto --resume']);
  });

  it('startSession rejects duplicate projectDir', async () => {
    await sm.startSession('/tmp/dup-test', { cliPath: '/usr/bin/tac' });
    await assert.rejects(
      () => sm.startSession('/tmp/dup-test', { cliPath: '/usr/bin/tac' }),
      (err: Error) => {
  ...
  for (const terminalStatus of ['completed', 'error', 'cancelled'] as const) {
    it(`startSession evicts a prior '${terminalStatus}' session for the same projectDir`, async () => {
      ...
      const firstSessionId = await sm.startSession(dir, { cliPath: '/usr/bin/tac' });
      ...
      const secondSessionId = await sm.startSession(dir, { cliPath: '/usr/bin/tac' });
  ...
  for (const activeStatus of ['starting', 'running', 'blocked'] as const) {
    it(`startSession still rejects a prior '${activeStatus}' session`, async () => {
      ...
      const sid = await sm.startSession(dir, { cliPath: '/usr/bin/tac' });
      sm.getSession(sid)!.status = activeStatus;
      await assert.rejects(
        () => sm.startSession(dir, { cliPath: '/usr/bin/tac' }),
  ...
  it('startSession rejects empty projectDir', async () => {
    await assert.rejects(
      () => sm.startSession('', { cliPath: '/usr/bin/tac' }),
      (err: Error) => {
  ...
  it('startSession sets error status on start() failure', async () => {
    ...
    await assert.rejects(
      () => sm.startSession('/tmp/fail-start', { cliPath: '/usr/bin/tac' }),
      (err: Error) => {
  ...
  it('startSession sets error status on init() failure', async () => {
    ...
    await assert.rejects(
      () => sm.startSession('/tmp/fail-init', { cliPath: '/usr/bin/tac' }),
      (err: Error) => {
        assert.ok(err.message.includes('Failed to start session'));
  ...
  it('getSessionByDir returns session for known dir', async () => {
    await sm.startSession('/tmp/by-dir', { cliPath: '/usr/bin/tac' });
  ...
  it('resolveBlocker errors when no pending blocker', async () => {
    const sessionId = await sm.startSession('/tmp/no-blocker', { cliPath: '/usr/bin/tac' });
    await assert.rejects(
  ...
  it('resolveBlocker clears pendingBlocker and sends UI response', async () => {
    const sessionId = await sm.startSession('/tmp/blocker-resolve', { cliPath: '/usr/bin/tac' });
  ...
  it('cancelSession calls abort + stop on client', async () => {
    const sessionId = await sm.startSession('/tmp/cancel-test', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('cleanup stops all active sessions', async () => {
    await sm.startSession('/tmp/cleanup-1', { cliPath: '/usr/bin/tac' });
    await sm.startSession('/tmp/cleanup-2', { cliPath: '/usr/bin/tac' });
  ...
  it('event ring buffer caps at MAX_EVENTS', async () => {
    const sessionId = await sm.startSession('/tmp/ring-buffer', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('blocker detection: non-fire-and-forget extension_ui_request sets pendingBlocker', async () => {
    const sessionId = await sm.startSession('/tmp/blocker-detect', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('fire-and-forget methods do not set pendingBlocker', async () => {
    const sessionId = await sm.startSession('/tmp/fire-forget', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('terminal detection: auto-mode stopped sets status to completed', async () => {
    const sessionId = await sm.startSession('/tmp/terminal', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('terminal detection with blocked: message sets status to blocked', async () => {
    const sessionId = await sm.startSession('/tmp/terminal-blocked', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('cost tracking: cumulative-max from cost_update events', async () => {
    const sessionId = await sm.startSession('/tmp/cost-track', { cliPath: '/usr/bin/tac' });
    const client = sm.lastClient!;
  ...
  it('getResult returns HeadlessJsonResult-shaped object', async () => {
    const sessionId = await sm.startSession('/tmp/result-shape', { cliPath: '/usr/bin/tac' });
...
describe('SessionManager.resolveCLIPath', () => {
  const originalTacPath = process.env['TAC_CLI_PATH'];
  const originalPath = process.env['PATH'];

  afterEach(() => {
    if (originalTacPath !== undefined) {
      process.env['TAC_CLI_PATH'] = originalTacPath;
    } else {
      delete process.env['TAC_CLI_PATH'];
    }
  ...
  it('TAC_CLI_PATH env var takes precedence', () => {
    process.env['TAC_CLI_PATH'] = '/custom/path/to/tac';
    const result = SessionManager.resolveCLIPath();
    assert.equal(result, resolve('/custom/path/to/tac'));
  });

  it('throws when TAC_CLI_PATH not set and which fails', () => {
    delete process.env['TAC_CLI_PATH'];
    process.env['PATH'] = '/nonexistent';
    assert.throws(
      () => SessionManager.resolveCLIPath(),
      (err: Error) => {
        assert.ok(err.message.includes('Cannot find TAC CLI'));
        return true;
      },
    );
  });
...
describe('createMcpServer tool registration', () => {
  ...
  it('tac_execute flow returns sessionId on success', async () => {
    const sessionId = await sm.startSession('/tmp/tool-exec', { cliPath: '/usr/bin/tac' });
    assert.equal(typeof sessionId, 'string');
    assert.ok(sessionId.length > 0);
  });

  it('tac_status flow returns correct shape', async () => {
    const sessionId = await sm.startSession('/tmp/tool-status', { cliPath: '/usr/bin/tac' });
    const session = sm.getSession(sessionId)!;
  ...
  it('tac_resolve_blocker flow returns error when no blocker', async () => {
    const sessionId = await sm.startSession('/tmp/tool-resolve', { cliPath: '/usr/bin/tac' });
    await assert.rejects(
  ...
  it('tac_result flow returns HeadlessJsonResult shape', async () => {
    const sessionId = await sm.startSession('/tmp/tool-result', { cliPath: '/usr/bin/tac' });
    const result = sm.getResult(sessionId);
  ...
  it('tac_cancel flow marks session as cancelled', async () => {
    const sessionId = await sm.startSession('/tmp/tool-cancel', { cliPath: '/usr/bin/tac' });
    await sm.cancelSession(sessionId);
    const session = sm.getSession(sessionId)!;
    assert.equal(session.status, 'cancelled');
  });

  it('tac_cancel can cancel an interactive session (no sessionId) via projectDir fallback', async () => {
    // Simulate an interactive session: registered by projectDir but with an empty sessionId
    // (e.g. started via `/tac auto` in terminal or from a restarted MCP server that lost its session registry)
  ...
  it('tac_cancel via projectDir works even when sessionId lookup returns undefined', async () => {
    // Start a normal session to get its projectDir
    const sessionId = await sm.startSession('/tmp/cancel-by-dir', { cliPath: '/usr/bin/tac' });
    const session = sm.getSession(sessionId)!;
