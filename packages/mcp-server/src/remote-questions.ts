 ...
 * but without any dependency on @tac/pi-coding-agent or the main src/ tree.
...
// ---------------------------------------------------------------------------
// Config resolution — reads ~/.tac/PREFERENCES.md YAML frontmatter
// ---------------------------------------------------------------------------
...
function resolveRemoteConfig(): ResolvedConfig | null {
  ...
  const globalPath = join(tacHome, 'PREFERENCES.md');
...
function formatForSlack(prompt: RemotePrompt): unknown[] {
  const blocks: unknown[] = [
    { type: 'header', text: { type: 'plain_text', text: 'TAC needs your input' } },
  ];
...
function formatForTelegram(prompt: RemotePrompt): { text: string; parse_mode: 'HTML'; reply_markup?: unknown } {
  ...
  const lines: string[] = ['<b>TAC needs your input</b>', ''];
...
async function discordSend(prompt: RemotePrompt, token: string, channelId: string, guildId: string | null): Promise<DispatchResult> {
  ...
  const res = await apiRequest(
    ...
    'POST',
    { content: '**TAC needs your input** — reply to this message with your answer', embeds },
...
async function slackSend(prompt: RemotePrompt, token: string, channelId: string): Promise<DispatchResult> {
  const res = await apiRequest(
    ...
    'POST',
    { channel: channelId, text: 'TAC needs your input', blocks: formatForSlack(prompt) },
