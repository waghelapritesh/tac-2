/**
 * SessionManager — manages RpcClient lifecycle for background TAC execution.
 *
...
import { RpcClient } from '@waghelapritesh/rpc-client';
import type { SdkAgentEvent, RpcInitResult, RpcCostUpdateEvent, RpcExtensionUIRequest } from '@waghelapritesh/rpc-client';
import type {
  ManagedSession,
  ExecuteOptions,
  PendingBlocker,
...
export class SessionManager {
  ...
  /**
   * Start a new TAC auto-mode session for the given project directory.
   *
   ...
   * wires event tracking, and sends '/tac auto' to begin execution.
   */
   ...
   *
   * This is the fallback path for interactive sessions (started via `/tac auto`
  ...
  private async stopDetachedAutoProcess(projectDir: string): Promise<boolean> {
    const lockPath = join(projectDir, '.tac', 'auto.lock');
  ...
  /**
   * Resolve the TAC CLI path.
   *
   * 1. TAC_CLI_PATH env var (highest priority)
   * 2. `which tac` → resolve to the actual dist/cli.js
   */
  static resolveCLIPath(): string {
    // Check env var first
    const envPath = process.env['TAC_CLI_PATH'];
    if (envPath) return resolve(envPath);

    // Fallback: locate `tac` via which
    try {
      const tacBin = execSync('which tac', { encoding: 'utf-8' }).trim();
      if (tacBin) {
        // tac bin is typically a symlink to dist/loader.js — return the resolved path
        return resolve(tacBin);
      }
    } catch {
      // which failed
    }
    ...
    throw new Error(
      'Cannot find TAC CLI. Set TAC_CLI_PATH environment variable or ensure `tac` is in PATH.'
    );
  }
