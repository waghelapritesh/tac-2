...
describe("tool credentials", () => {
  it("hydrates supported model and tool keys from auth.json", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "tac-mcp-auth-"));
  ...
  it("resolves auth.json from TAC_CODING_AGENT_DIR", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "tac-mcp-agent-dir-"));
    const agentDir = join(tempRoot, "agent");
    mkdirSync(agentDir, { recursive: true });

    try {
      assert.equal(
        resolveAuthPath({ TAC_CODING_AGENT_DIR: agentDir }),
