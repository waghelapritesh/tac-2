/**
 * MCP Server — registers TAC orchestration, project-state, and workflow tools.
 *
 * Session tools (6): tac_execute, tac_status, tac_result, tac_cancel, tac_query, tac_resolve_blocker
 * Interactive tools (2): ask_user_questions, secure_env_collect via MCP form elicitation
 * Read-only tools (6): tac_progress, tac_roadmap, tac_history, tac_doctor, tac_captures, tac_knowledge
...
import { resolveTacRoot } from './readers/paths.js';
...
async function readProjectState(projectDir: string, query: string | undefined): Promise<Record<string, unknown>> {
  const tacDir = join(resolve(projectDir), '.tac');
  ...
  if (wanted.has('state')) {
    try {
      result.state = await readFile(join(tacDir, 'STATE.md'), 'utf-8');
    } catch {
      result.state = null;
    }
  }
  ...
  if (wanted.has('project')) {
    try {
      result.project = await readFile(join(tacDir, 'PROJECT.md'), 'utf-8');
    } catch {
      result.project = null;
    }
  }
  ...
  if (wanted.has('requirements')) {
    try {
      result.requirements = await readFile(join(tacDir, 'REQUIREMENTS.md'), 'utf-8');
    } catch {
  ...
  if (wanted.has('milestones')) {
    const milestonesDir = join(tacDir, 'milestones');
    try {
...
export async function createMcpServer(
  ...
  // -----------------------------------------------------------------------
  // tac_execute — start a new TAC auto-mode session.
  //
  ...
  // `tac_cancel` to stop it via sessionId.
  // -----------------------------------------------------------------------
  server.tool(
    'tac_execute',
    'Start a TAC auto-mode session for a project directory. Returns a sessionId for tracking.',
    {
      projectDir: z.string().describe('Absolute path to the project directory'),
      command: z.string().optional().describe('Command to send (default: "/tac auto")'),
    ...
    async (args: Record<string, unknown>, extra?: McpToolExtra) => {
      ...
      try {
        ...
        if (extra?.signal?.aborted) {
          ...
          return errorContent('tac_execute aborted by client before returning');
        }
  ...
  // -----------------------------------------------------------------------
  // tac_status — poll session status
  // -----------------------------------------------------------------------
  server.tool(
    'tac_status',
    'Get the current status of a TAC session including progress, recent events, and pending blockers.',
    {
      sessionId: z.string().describe('Session ID returned from tac_execute'),
    },
  ...
  // -----------------------------------------------------------------------
  // tac_result — get accumulated session result
  // -----------------------------------------------------------------------
  server.tool(
    'tac_result',
    'Get the result of a TAC session. Returns partial results if the session is still running.',
    {
      sessionId: z.string().describe('Session ID returned from tac_execute'),
    },
    async (args: Record<string, unknown>) => {
  ...
  // -----------------------------------------------------------------------
  // tac_cancel — cancel a running session
  //
  // Supports two lookup strategies:
  //   1. sessionId  — the ID returned from tac_execute (primary)
  ...
  // `/tac auto` in the terminal) and post-restart MCP sessions that were
  // never registered with a sessionId in this server instance.
  // -----------------------------------------------------------------------
  server.tool(
    'tac_cancel',
    'Cancel a running TAC session. Aborts the current operation and stops the process. Provide sessionId (from tac_execute) or projectDir as a fallback for interactive/restarted sessions.',
    {
      sessionId: z.string().optional().describe('Session ID returned from tac_execute'),
  ...
  // -----------------------------------------------------------------------
  // tac_query — read project state from filesystem (no session needed).
  //
  ...
  server.tool(
    'tac_query',
    'Query TAC project state from the filesystem. By default returns STATE.md, PROJECT.md, requirements, and milestone listing. Pass `query` to narrow the response (accepted: "state"/"status", "project", "requirements", "milestones", "all"). Does not require an active session.',
    {
      projectDir: z.string().describe('Absolute path to the project directory'),
      query: z
  ...
  // -----------------------------------------------------------------------
  // tac_resolve_blocker — resolve a pending blocker
  // -----------------------------------------------------------------------
  server.tool(
    'tac_resolve_blocker',
    'Resolve a pending blocker in a TAC session by sending a response to the UI request.',
    {
      sessionId: z.string().describe('Session ID returned from tac_execute'),
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>, extra?: McpToolExtra) => {
      ...
      try {
        ...
        // Claude Code-under-tac sessions, which have no local TUI.
  ...
  // -----------------------------------------------------------------------
  // tac_progress — structured project progress metrics
  // -----------------------------------------------------------------------
  server.tool(
    'tac_progress',
    'Get structured project progress: active milestone/slice/task, phase, completion counts, blockers, and next action. No session required — reads directly from .tac/ on disk.',
    {
      projectDir: z.string().describe('Absolute path to the project directory'),
    },
    async (args: Record<string, unknown>) => {
  ...
  // -----------------------------------------------------------------------
  // tac_roadmap — milestone/slice/task structure with status
  // -----------------------------------------------------------------------
  server.tool(
    'tac_roadmap',
  ...
  // -----------------------------------------------------------------------
  // tac_history — execution history with cost/token metrics
  // -----------------------------------------------------------------------
  server.tool(
    'tac_history',
  ...
  // -----------------------------------------------------------------------
  // tac_doctor — lightweight structural health check
  // -----------------------------------------------------------------------
  server.tool(
    'tac_doctor',
    'Run a lightweight structural health check on the .tac/ directory. Checks for missing files, status inconsistencies, and orphaned state. No session required.',
    {
      projectDir: z.string().describe('Absolute path to the project directory'),
  ...
  // -----------------------------------------------------------------------
  // tac_captures — pending captures and ideas
  // -----------------------------------------------------------------------
  server.tool(
    'tac_captures',
  ...
  // -----------------------------------------------------------------------
  // tac_knowledge — project knowledge base
  // -----------------------------------------------------------------------
  server.tool(
    'tac_knowledge',
  ...
  // -----------------------------------------------------------------------
  // tac_graph — knowledge graph for TAC projects
  //
  // Modes:
  //   build   Parse .tac/ artifacts and write graph.json atomically.
  ...
  server.tool(
    'tac_graph',
    [
      'Manage the TAC project knowledge graph. No session required.',
      '',
      'Modes:',
    ...
    async (args: Record<string, unknown>) => {
      ...
      try {
        ...
        switch (mode) {
          case 'build': {
            if (snapshot) {
              await writeSnapshot(tacRoot).catch(() => { /* best-effort */ });
            }
