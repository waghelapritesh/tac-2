/**
 * Workflow MCP tools — exposes the core TAC mutation/read handlers over MCP.
 */
...
function getAllowedProjectRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const configuredRoot = env.TAC_WORKFLOW_PROJECT_ROOT?.trim();
...
/**
 * Resolve the symlink target of `<allowedRoot>/.tac` when it points into the
 * external state layout (`~/.tac/projects/<hash>/`). Returns the realpath of
 * that target so callers can accept worktree paths that live under
 * `<external-state>/worktrees/<MID>/`. Returns null when `.tac` is absent or
...
function resolveExternalStateRoot(allowedRoot: string): string | null {
  try {
    return realpathSync(join(allowedRoot, ".tac"));
  } catch {
    return null;
  }
}
...
export function validateProjectDir(projectDir: string, env: NodeJS.ProcessEnv = process.env): string {
  ...
  // External state layout: `<allowedRoot>/.tac` may be a symlink into
  // `~/.tac/projects/<hash>/`, and auto-worktrees live under
  // `~/.tac/projects/<hash>/worktrees/<MID>/`. Accept candidates that are
  // under the realpath of `<allowedRoot>/.tac` — they belong to this project
 ...
 * `<projectRoot>/.tac/worktrees/<milestoneId>/`, return that path as the
 ...
 * the shared project `.tac/` and auto-mode's verifyExpectedArtifact (which
 * uses the worktree `.tac/`) fails, triggering a guaranteed retry per unit.
 */
function resolveActiveWorktreeBasePath(
  ...
  const wtPath = join(projectRoot, ".tac", "worktrees", milestoneId);
...
function parseWorkflowArgs<T extends { projectDir?: string }>(
  ...
  // auto-worktree, re-route writes to the worktree's .tac rather than the
  // project's shared .tac. auto-mode's verifyExpectedArtifact runs against
...
function getWriteGateModuleCandidates(): string[] {
  ...
  const explicitModule = process.env.TAC_WORKFLOW_WRITE_GATE_MODULE?.trim();
  if (explicitModule) {
    if (/^[a-z]{2,}:/i.test(explicitModule) && !explicitModule.startsWith("file:")) {
      throw new Error("TAC_WORKFLOW_WRITE_GATE_MODULE only supports file: URLs or filesystem paths.");
    }
    candidates.push(explicitModule.startsWith("file:") ? explicitModule : toFileUrl(explicitModule));
  }

  candidates.push(
    new URL("../../../src/resources/extensions/tac/bootstrap/write-gate.js", import.meta.url).href,
    new URL("../../../dist/resources/extensions/tac/bootstrap/write-gate.js", import.meta.url).href,
    new URL("../../../src/resources/extensions/tac/bootstrap/write-gate.ts", import.meta.url).href,
  );
...
function getWorkflowExecutorModuleCandidates(env: NodeJS.ProcessEnv = process.env): string[] {
  const candidates: string[] = [];
  const explicitModule = env.TAC_WORKFLOW_EXECUTORS_MODULE?.trim();
  if (explicitModule) {
    if (/^[a-z]{2,}:/i.test(explicitModule) && !explicitModule.startsWith("file:")) {
      throw new Error("TAC_WORKFLOW_EXECUTORS_MODULE only supports file: URLs or filesystem paths.");
    }
    candidates.push(explicitModule.startsWith("file:") ? explicitModule : toFileUrl(explicitModule));
  }

  candidates.push(
    new URL("../../../src/resources/extensions/tac/tools/workflow-tool-executors.js", import.meta.url).href,
    new URL("../../../dist/resources/extensions/tac/tools/workflow-tool-executors.js", import.meta.url).href,
    new URL("../../../src/resources/extensions/tac/tools/workflow-tool-executors.ts", import.meta.url).href,
  );
...
async function getWorkflowToolExecutors(): Promise<WorkflowToolExecutors> {
  if (!workflowToolExecutorsPromise) {
    workflowToolExecutorsPromise = (async () => {
      ...
      throw new Error(
        "Unable to load TAC workflow executor bridge for MCP mutation tools. " +
        "Set TAC_WORKFLOW_EXECUTORS_MODULE to an importable workflow-tool-executors module, " +
        "or run the MCP server from a TAC checkout that includes src/resources/extensions/tac/tools/workflow-tool-executors.(js|ts). " +
...
async function getWorkflowWriteGateModule(): Promise<WorkflowWriteGateModule> {
  if (!workflowWriteGatePromise) {
    workflowWriteGatePromise = (async () => {
      ...
      throw new Error(
        "Unable to load TAC write-gate bridge for workflow MCP tools. " +
        `Attempts: ${attempts.join("; ")}`,
      );
    })();
  }
...
export const WORKFLOW_TOOL_NAMES = [
  ...
  // tac_memory_graph is namespaced to avoid collision with the existing
  // tac_graph tool (project knowledge graph from .tac/ artifacts).
  "tac_capture_thought",
  "tac_memory_query",
  "tac_memory_graph",
] as const;
...
function getWorkflowOpTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.TAC_MCP_WORKFLOW_TIMEOUT_MS?.trim();
...
async function runSerializedWorkflowOperation<T>(fn: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timer = setTimeout(() => {
        reject(new Error(`Workflow operation exceeded ${timeoutMs}ms deadline (TAC_MCP_WORKFLOW_TIMEOUT_MS)`));
      }, timeoutMs);
    });
    try {
...
async function runSerializedWorkflowDbOperation<T>(
  ...
  return runSerializedWorkflowOperation(async () => {
    const { ensureDbOpen } = await importLocalModule<WorkflowDbBootstrapModule>(
      "../../../src/resources/extensions/tac/bootstrap/dynamic-tools.js",
    );
    const dbAvailable = await ensureDbOpen(projectDir);
    if (!dbAvailable) {
      throw new Error("TAC database is not available");
    }
    return fn();
  });
}
...
async function handleTaskComplete(
  ...
  await enforceWorkflowWriteGate("tac_task_complete", projectDir, args.milestoneId);
...
async function handleSliceComplete(
  ...
  await enforceWorkflowWriteGate("tac_slice_complete", projectDir, args.milestoneId);
...
async function handleReplanSlice(
  ...
  await enforceWorkflowWriteGate("tac_replan_slice", projectDir, args.milestoneId);
...
async function handleCompleteMilestone(
  ...
  await enforceWorkflowWriteGate("tac_complete_milestone", projectDir, args.milestoneId);
...
async function handleValidateMilestone(
  ...
  await enforceWorkflowWriteGate("tac_validate_milestone", projectDir, args.milestoneId);
...
async function handleReassessRoadmap(
  ...
  await enforceWorkflowWriteGate("tac_reassess_roadmap", projectDir, args.milestoneId);
...
async function handleSaveGateResult(
  ...
  await enforceWorkflowWriteGate("tac_save_gate_result", projectDir, args.milestoneId);
...
async function ensureMilestoneDbRow(milestoneId: string): Promise<void> {
  try {
    const { insertMilestone } = await importLocalModule<any>("../../../src/resources/extensions/tac/tac-db.js");
...
const taskCompleteParams = {
  ...
  escalation: z.object({
    ...
    options: z.array(z.object({
      id: z.string().describe("Short id (e.g. 'A', 'B') used by /tac escalate resolve."),
    ...
    continueWithDefault: z.boolean().describe(
      "When true, loop continues (artifact logged for later review). When false, auto-mode pauses until the user resolves via /tac escalate resolve.",
    ),
...
export function registerWorkflowTools(server: McpToolServer): void {
  server.tool(
    "tac_decision_save",
    "Record a project decision to the TAC database and regenerate DECISIONS.md.",
    decisionSaveParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(decisionSaveSchema, args);
      const { projectDir, ...params } = parsed;
      await enforceWorkflowWriteGate("tac_decision_save", projectDir);
      const result = await runSerializedWorkflowDbOperation(projectDir, async () => {
        const { saveDecisionToDb } = await importLocalModule<any>("../../../src/resources/extensions/tac/db-writer.js");
  ...
  server.tool(
    "tac_save_decision",
    "Alias for tac_decision_save. Record a project decision to the TAC database and regenerate DECISIONS.md.",
    decisionSaveParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(decisionSaveSchema, args);
      const { projectDir, ...params } = parsed;
  ...
  server.tool(
    "tac_requirement_update",
    "Update an existing requirement in the TAC database and regenerate REQUIREMENTS.md.",
    requirementUpdateParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(requirementUpdateSchema, args);
      const { projectDir, id, ...updates } = parsed;
      await enforceWorkflowWriteGate("tac_requirement_update", projectDir);
      await runSerializedWorkflowDbOperation(projectDir, async () => {
        const { updateRequirementInDb } = await importLocalModule<any>("../../../src/resources/extensions/tac/db-writer.js");
  ...
  server.tool(
    "tac_update_requirement",
    "Alias for tac_requirement_update. Update an existing requirement in the TAC database and regenerate REQUIREMENTS.md.",
    requirementUpdateParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(requirementUpdateSchema, args);
      const { projectDir, id, ...updates } = parsed;
  ...
  server.tool(
    "tac_requirement_save",
    "Record a new requirement to the TAC database and regenerate REQUIREMENTS.md.",
    requirementSaveParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(requirementSaveSchema, args);
      const { projectDir, ...params } = parsed;
      await enforceWorkflowWriteGate("tac_requirement_save", projectDir);
      const result = await runSerializedWorkflowDbOperation(projectDir, async () => {
        const { saveRequirementToDb } = await importLocalModule<any>("../../../src/resources/extensions/tac/db-writer.js");
  ...
  server.tool(
    "tac_save_requirement",
    "Alias for tac_requirement_save. Record a new requirement to the TAC database and regenerate REQUIREMENTS.md.",
    requirementSaveParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(requirementSaveSchema, args);
      const { projectDir, ...params } = parsed;
  ...
  server.tool(
    "tac_milestone_generate_id",
    "Generate the next milestone ID for a new TAC milestone.",
    milestoneGenerateIdParams,
    async (args: Record<string, unknown>) => {
      const { projectDir } = parseWorkflowArgs(milestoneGenerateIdSchema, args);
      await enforceWorkflowWriteGate("tac_milestone_generate_id", projectDir);
      const id = await runSerializedWorkflowDbOperation(projectDir, async () => {
        const {
          ...
          nextMilestoneId,
        } = await importLocalModule<any>("../../../src/resources/extensions/tac/milestone-ids.js");
        ...
        const prefsMod = await importLocalModule<any>(
          "../../../src/resources/extensions/tac/preferences.js",
        ).catch(() => null);
        ...
        try {
          uniqueEnabled = !!prefsMod?.loadEffectiveTACPreferences?.()?.preferences?.unique_milestone_ids;
        } catch {
          uniqueEnabled = false;
        }
  ...
  server.tool(
    "tac_generate_milestone_id",
    "Alias for tac_milestone_generate_id. Generate the next milestone ID for a new TAC milestone.",
    milestoneGenerateIdParams,
    async (args: Record<string, unknown>) => {
      const { projectDir } = parseWorkflowArgs(milestoneGenerateIdSchema, args);
      await enforceWorkflowWriteGate("tac_milestone_generate_id", projectDir);
  ...
  server.tool(
    "tac_plan_milestone",
    "Write milestone planning state to the TAC database and render ROADMAP.md from DB.",
    planMilestoneParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(planMilestoneSchema, args);
      const { projectDir, ...params } = parsed;
      await enforceWorkflowWriteGate("tac_plan_milestone", projectDir, params.milestoneId);
  ...
  server.tool(
    "tac_plan_slice",
    "Write slice/task planning state to the TAC database and render plan artifacts from DB.",
    planSliceParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(planSliceSchema, args);
      const { projectDir, ...params } = parsed;
      await enforceWorkflowWriteGate("tac_plan_slice", projectDir, params.milestoneId);
  ...
  server.tool(
    "tac_plan_task",
    "Write task planning state to the TAC database and render tasks/T##-PLAN.md from DB.",
    planTaskParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(planTaskSchema, args);
      const { projectDir, ...params } = parsed;
      await enforceWorkflowWriteGate("tac_plan_task", projectDir, params.milestoneId);
      const result = await runSerializedWorkflowDbOperation(projectDir, async () => {
        const { handlePlanTask } = await importLocalModule<any>("../../../src/resources/extensions/tac/tools/plan-task.js");
  ...
  server.tool(
    "tac_task_plan",
    "Alias for tac_plan_task. Write task planning state to the TAC database and render tasks/T##-PLAN.md from DB.",
    planTaskParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(planTaskSchema, args);
      const { projectDir, ...params } = parsed;
  ...
  server.tool(
    "tac_slice_replan",
    "Alias for tac_replan_slice. Replan a slice after a blocker is discovered.",
    replanSliceParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(replanSliceSchema, args);
      return handleReplanSlice(parsed.projectDir, parsed);
  ...
  server.tool(
    "tac_slice_complete",
    "Record a completed slice to the TAC database, render SUMMARY.md + UAT.md, and update roadmap projection.",
    sliceCompleteParams,
    async (args: Record<string, unknown>) => {
  ...
  server.tool(
    "tac_complete_slice",
    "Alias for tac_slice_complete. Record a completed slice to the TAC database and render summary/UAT artifacts.",
    sliceCompleteParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(sliceCompleteSchema, args);
      return handleSliceComplete(parsed.projectDir, parsed);
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      await enforceWorkflowWriteGate("tac_skip_slice", projectDir, milestoneId);
      await runSerializedWorkflowDbOperation(projectDir, async () => {
        const { getSlice, updateSliceStatus } = await importLocalModule<any>("../../../src/resources/extensions/tac/tac-db.js");
        const { invalidateStateCache } = await importLocalModule<any>("../../../src/resources/extensions/tac/state.js");
        const { rebuildState } = await importLocalModule<any>("../../../src/resources/extensions/tac/doctor.js");
  ...
  server.tool(
    "tac_complete_milestone",
    "Record a completed milestone to the TAC database and render its SUMMARY.md.",
  ...
  server.tool(
    "tac_milestone_complete",
    "Alias for tac_complete_milestone. Record a completed milestone to the TAC database and render its SUMMARY.md.",
    completeMilestoneParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(completeMilestoneSchema, args);
      return handleCompleteMilestone(parsed.projectDir, parsed);
  ...
  server.tool(
    "tac_validate_milestone",
    "Validate a milestone, persist validation results to the TAC database, and render VALIDATION.md.",
  ...
  server.tool(
    "tac_milestone_validate",
    "Alias for tac_validate_milestone. Validate a milestone and render VALIDATION.md.",
    validateMilestoneParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(validateMilestoneSchema, args);
      return handleValidateMilestone(parsed.projectDir, parsed);
  ...
  server.tool(
    "tac_roadmap_reassess",
    "Alias for tac_reassess_roadmap. Reassess a roadmap after slice completion.",
    reassessRoadmapParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(reassessRoadmapSchema, args);
      return handleReassessRoadmap(parsed.projectDir, parsed);
  ...
  server.tool(
    "tac_save_gate_result",
    "Save a quality gate result to the TAC database.",
  ...
  server.tool(
    "tac_summary_save",
    "Save a TAC summary/research/context/assessment artifact to the database and disk.",
    summarySaveParams,
    async (args: Record<string, unknown>) => {
      ...
      await enforceWorkflowWriteGate("tac_summary_save", projectDir, milestone_id);
  ...
  server.tool(
    "tac_task_complete",
    "Record a completed task to the TAC database and render its SUMMARY.md.",
    taskCompleteParams,
    async (args: Record<string, unknown>) => {
  ...
  server.tool(
    "tac_complete_task",
    "Alias for tac_task_complete. Record a completed task to the TAC database and render its SUMMARY.md.",
    taskCompleteParams,
    async (args: Record<string, unknown>) => {
      const parsed = parseWorkflowArgs(taskCompleteSchema, args);
      const { projectDir, ...taskArgs } = parsed;
  ...
  server.tool(
    "tac_milestone_status",
    "Read the current status of a milestone and all its slices from the TAC database.",
    milestoneStatusParams,
    async (args: Record<string, unknown>) => {
      // tac_milestone_status is a read-only query. In-process (query-tools.ts)
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      const { queryJournal } = await importLocalModule<any>("../../../src/resources/extensions/tac/journal.js");
  ...
  // `memory_query`, and `tac_graph` (the memory variant). MCP exposes them
  // under the tac_* prefix and renames the memory graph to tac_memory_graph
  // to avoid collision with the project knowledge graph tool registered as
  // `tac_graph` in server.ts.
  ...
  server.tool(
    "tac_capture_thought",
    "Record a durable project insight into the TAC memory store. Categories: architecture, convention, gotcha, preference, environment, pattern. Mirrors the in-process capture_thought tool for external MCP clients.",
    captureThoughtParams,
    async (args: Record<string, unknown>) => {
      const { projectDir, ...params } = parseWorkflowArgs(captureThoughtSchema, args);
      await enforceWorkflowWriteGate("tac_capture_thought", projectDir);
      return runSerializedWorkflowDbOperation(projectDir, async () => {
        const { executeMemoryCapture } = await importLocalModule<any>(
          "../../../src/resources/extensions/tac/tools/memory-tools.js",
        );
  ...
  server.tool(
    "tac_memory_query",
    "Search the TAC memory store by keyword. Returns ranked memories with id, category, content, confidence, scope, and tags. Mirrors the in-process memory_query tool for external MCP clients.",
    memoryQueryParams,
    async (args: Record<string, unknown>) => {
  ...
  server.tool(
    "tac_memory_graph",
    "Inspect the relationship graph between memories. mode=query walks edges from a given memoryId. mode=build is a placeholder reserved for future graph rebuilds. Distinct from tac_graph (project knowledge graph) — see ADR-013.",
    memoryGraphParams,
    async (args: Record<string, unknown>) => {
      const { projectDir, ...params } = parseWorkflowArgs(memoryGraphSchema, args);
      return runSerializedWorkflowDbOperation(projectDir, async () => {
        const { executeTacGraph } = await importLocalModule<any>(
          "../../../src/resources/extensions/tac/tools/memory-tools.js",
        );
        return executeTacGraph(params);
      });
    },
  );
}
