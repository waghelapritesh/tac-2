 ...
 * straight to elicitInput (which is unreachable in Claude Code-under-tac
 * sessions).
 *
...
describe('isRemoteConfigured', () => {
  let tmpDir: string;
  const origTacHome = process.env['TAC_HOME'];
  const origToken = process.env['DISCORD_BOT_TOKEN'];

  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'tac-rq-test-'));
  });

  after(() => {
    rmSync(tmpDir, { recursive: true, force: true });
    if (origTacHome !== undefined) {
      process.env['TAC_HOME'] = origTacHome;
    } else {
...
describe('tryRemoteQuestions returns null when remote is not configured', () => {
  ...
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'tac-rq-null-test-'));
    process.env['TAC_HOME'] = tmpDir;
...
describe('tryRemoteQuestions returns error result on auth failure', () => {
  ...
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'tac-rq-auth-test-'));
...
describe('remote-questions YAML frontmatter parsing (via isRemoteConfigured)', () => {
  ...
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'tac-rq-yaml-test-'));
  });
