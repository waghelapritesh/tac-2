# @waghelapritesh/mcp-server

MCP server exposing TAC orchestration tools for Claude Code, Cursor, and other MCP-compatible clients.

Start TAC auto-mode sessions, poll progress, resolve blockers, and retrieve results — all through the [Model Context Protocol](https://modelcontextprotocol.io/).

This package now exposes two tool surfaces:

- session/read tools for starting and inspecting TAC sessions
- MCP-native interactive tools for structured user input
- headless-safe workflow tools for planning, completion, validation, reassessment, metadata persistence, and journal reads

## Installation

```bash
npm install @waghelapritesh/mcp-server
```

Or with the monorepo workspace:

```bash
# Already available as a workspace package
npx tac-mcp-server
```

## Configuration

### Claude Code

Add to your project's `.mcp.json`:

```json
{
  "mcpServers": {
    "tac": {
      "command": "npx",
      "args": ["tac-mcp-server"],
      "env": {
        "TAC_CLI_PATH": "/path/to/tac"
      }
    }
  }
}
```

Or if installed globally:

```json
{
  "mcpServers": {
    "tac": {
      "command": "tac-mcp-server"
    }
  }
}
```

### Cursor

Add to `.cursor/mcp.json`:

```json
{
  "mcpServers": {
    "tac": {
      "command": "npx",
      "args": ["tac-mcp-server"],
      "env": {
        "TAC_CLI_PATH": "/path/to/tac"
      }
    }
  }
}
```

## Tools

### Workflow tools

The workflow MCP surface includes:

- `tac_decision_save`
- `tac_save_decision`
- `tac_requirement_update`
- `tac_update_requirement`
- `tac_requirement_save`
- `tac_save_requirement`
- `tac_milestone_generate_id`
- `tac_generate_milestone_id`
- `tac_plan_milestone`
- `tac_plan_slice`
- `tac_plan_task`
- `tac_task_plan`
- `tac_replan_slice`
- `tac_slice_replan`
- `tac_task_complete`
- `tac_complete_task`
- `tac_slice_complete`
- `tac_complete_slice`
- `tac_skip_slice`
- `tac_validate_milestone`
- `tac_milestone_validate`
- `tac_complete_milestone`
- `tac_milestone_complete`
- `tac_reassess_roadmap`
- `tac_roadmap_reassess`
- `tac_save_gate_result`
- `tac_summary_save`
- `tac_milestone_status`
- `tac_journal_query`

These tools use the same TAC workflow handlers as the native in-process tool path wherever a shared handler exists.

### Interactive tools

The packaged server exposes `ask_user_questions` through MCP form elicitation. This keeps the existing TAC answer payload shape while allowing Claude Code CLI and other elicitation-capable clients to surface structured user choices.

The packaged server also exposes `secure_env_collect` through MCP form elicitation. Secret values are written directly to the selected destination and are not included in tool output. For dotenv writes, `envFilePath` must resolve inside the validated project directory; parent traversal and symlink escapes are rejected.

Current support boundary:

- when running inside the TAC monorepo checkout, the MCP server auto-discovers the shared workflow executor module
- outside the monorepo, set `TAC_WORKFLOW_EXECUTORS_MODULE` to an importable `workflow-tool-executors` module path if you want the mutation tools enabled
- `ask_user_questions` and `secure_env_collect` require an MCP client that supports form elicitation
- session/read tools do not depend on this bridge

If the executor bridge cannot be loaded, workflow mutation calls will fail with a precise configuration error instead of silently degrading.

### `tac_execute`

Start a TAC auto-mode session for a project directory.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `projectDir` | `string` | ✅ | Absolute path to the project directory |
| `command` | `string` | | Command to send (default: `"/tac auto"`) |
| `model` | `string` | | Model ID override |
| `bare` | `boolean` | | Run in bare mode (skip user config) |

**Returns:** `{ sessionId, status: "started" }`

### `tac_status`

Poll the current status of a running TAC session.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `sessionId` | `string` | ✅ | Session ID from `tac_execute` |

**Returns:**

```json
{
  "status": "running",
  "progress": { "eventCount": 42, "toolCalls": 15 },
  "recentEvents": [ ... ],
  "pendingBlocker": null,
  "cost": { "totalCost": 0.12, "tokens": { "input": 5000, "output": 2000, "cacheRead": 1000, "cacheWrite": 500 } },
  "durationMs": 45000
}
```

### `tac_result`

Get the accumulated result of a session. Works for both running (partial) and completed sessions.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `sessionId` | `string` | ✅ | Session ID from `tac_execute` |

**Returns:**

```json
{
  "sessionId": "abc-123",
  "projectDir": "/path/to/project",
  "status": "completed",
  "durationMs": 120000,
  "cost": { ... },
  "recentEvents": [ ... ],
  "pendingBlocker": null,
  "error": null
}
```

### `tac_cancel`

Cancel a running session. Aborts the current operation and stops the agent process.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `sessionId` | `string` | ✅ | Session ID from `tac_execute` |

**Returns:** `{ cancelled: true }`

### `tac_query`

Query TAC project state from the filesystem without an active session. Returns STATE.md, PROJECT.md, requirements, and milestone listing.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `projectDir` | `string` | ✅ | Absolute path to the project directory |
| `query` | `string` | ✅ | What to query (e.g. `"status"`, `"milestones"`) |

**Returns:**

```json
{
  "projectDir": "/path/to/project",
  "state": "...",
  "project": "...",
  "requirements": "...",
  "milestones": [
    { "id": "M001", "hasRoadmap": true, "hasSummary": false }
  ]
}
```

### `tac_resolve_blocker`

Resolve a pending blocker in a session by sending a response to the blocked UI request.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `sessionId` | `string` | ✅ | Session ID from `tac_execute` |
| `response` | `string` | ✅ | Response to send for the pending blocker |

**Returns:** `{ resolved: true }`

## Environment Variables

| Variable | Description |
|----------|-------------|
| `TAC_CLI_PATH` | Absolute path to the TAC CLI binary. If not set, the server resolves `tac` via `which`. |
| `TAC_WORKFLOW_EXECUTORS_MODULE` | Optional absolute path or `file:` URL for the shared TAC workflow executor module used by workflow mutation tools. |

The server also hydrates supported model-provider and tool credentials from `~/.tac/agent/auth.json` on startup. Keys saved through `/tac config` or `/tac keys` become available to the MCP server process automatically, and any explicitly-set environment variable still wins.

## Architecture

```
┌─────────────────┐     stdio      ┌──────────────────┐
│  MCP Client     │ ◄────────────► │  @waghelapritesh/mcp-server │
│  (Claude Code,  │    JSON-RPC    │                  │
│   Cursor, etc.) │                │  SessionManager  │
└─────────────────┘                │       │          │
                                   │       ▼          │
                                   │  @waghelapritesh/rpc-client │
                                   │       │          │
                                   │       ▼          │
                                   │  TAC CLI (child  │
                                   │  process via RPC)│
                                   └──────────────────┘
```

- **@waghelapritesh/mcp-server** — MCP protocol adapter. Translates MCP tool calls into SessionManager operations.
- **SessionManager** — Manages RpcClient lifecycle. One session per project directory. Tracks events in a ring buffer (last 50), detects blockers, accumulates cost.
- **@waghelapritesh/rpc-client** — Low-level RPC client that spawns and communicates with the TAC CLI process via JSON-RPC over stdio.

## License

MIT
