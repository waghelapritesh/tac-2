 ...
 *
 * Fixes: waghelapritesh/tac-2#2854
 */
...
const SPAWN_FILES_NEEDING_SHELL_GUARD = [
	// Extension's TAC client — spawns the `tac` binary which is a .cmd on Windows
	join(coreDir, "..", "..", "..", "vscode-extension", "src", "tac-client.ts"),
...
test("all spawn sites that invoke user-facing binaries include shell: process.platform === 'win32'", () => {
	...
	assert.deepEqual(
		...
		`\nOn Windows, .cmd wrapper scripts (npm, npx, tsc, tac) require shell ` +
