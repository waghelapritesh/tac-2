 ...
 * Mirrors the check in src/resources/extensions/tac/doctor-providers.ts so the
...
import { Agent, type AgentMessage, type ThinkingLevel } from "@tac/pi-agent-core";
import type { Message, Model } from "@tac/pi-ai";
 ...
 *
 * // With explicit model
 * import { getModel } from '@tac/pi-ai';
 * const { session } = await createAgentSession({
