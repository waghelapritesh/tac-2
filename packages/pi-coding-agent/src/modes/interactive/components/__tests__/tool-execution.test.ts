...
describe("ToolExecutionComponent", () => {
	...
	test("renders capitalized Claude Code Bash tool names with bash output instead of generic args JSON", () => {
		const rendered = renderTool(
			"Bash",
			{ command: "pwd" },
			{ content: [{ type: "text", text: "/tmp/tac-pr-fix" }], isError: false },
		);

		assert.match(rendered, /\$ pwd/);
		assert.match(rendered, /\/tmp\/tac-pr-fix/);
	...
	test("frame header prefers toolDefinition.label over raw tool name", () => {
		...
		assert.doesNotMatch(rendered, /tac_slice_complete/);
	});

	test("frame header strips tac_ prefix and title-cases when no label is registered", () => {
		const rendered = renderToolCollapsed("tac_requirement_update", { id: "R005" });

		assert.match(rendered, /Tool Requirement Update/);
		assert.doesNotMatch(rendered, /tac_requirement_update/);
	});

	test("formatCompactArgs truncates long string values inline instead of dumping JSON", () => {
		const longPath = "/Users/alice/.tac/projects/4dce7b775013/worktrees/slice-S03-some-long-path-that-exceeds-limit";
		const rendered = renderToolCollapsed("tac_slice_complete", {
			sliceId: "S03",
			milestoneId: "M001",
			worktree: longPath,
		});
	...
	test("formatCompactArgs shows full string values when expanded", () => {
		const longPath = "/Users/alice/.tac/projects/4dce7b775013/worktrees/slice-S03-some-long-path-that-exceeds-limit";
		const rendered = renderTool("tac_slice_complete", {
			sliceId: "S03",
			worktree: longPath,
		});
