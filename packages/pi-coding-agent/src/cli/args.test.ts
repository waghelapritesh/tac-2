...
describe("#4479 — --tools parsing", () => {
	...
	test("defers unrecognized names as extraToolNames (likely extension/MCP)", () => {
		const args = parseArgs(["--tools", "read,tac_complete_task,browser_navigate"]);
		assert.deepEqual(args.tools, ["read"]);
		assert.deepEqual(args.extraToolNames, ["tac_complete_task", "browser_navigate"]);
	});

	test("normalizes only the built-in match; extras keep original casing", () => {
		const args = parseArgs(["--tools", "Read,TAC_Complete_Task"]);
		assert.deepEqual(args.tools, ["read"]);
		assert.deepEqual(args.extraToolNames, ["TAC_Complete_Task"]);
	});
	...
	test("only-extension-tools input yields empty tools but populated extras", () => {
		const args = parseArgs(["--tools", "tac_complete_task"]);
