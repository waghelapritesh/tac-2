...
- `tac` CLI installed (`npm install -g tac-2`)
...
mkdir -p "$PROJECT_DIR"
cd "$PROJECT_DIR"
git init 2>/dev/null  # TAC needs a git repo
```
...
**Fire-and-forget (simplest — TAC does everything):**
```bash
cd "$PROJECT_DIR"
RESULT=$(tac headless --output-format json --timeout 0 --context spec.md new-milestone --auto 2>/dev/null)
EXIT=$?
```
...
```bash
RESULT=$(tac headless --bare --output-format json --timeout 0 --context spec.md new-milestone --auto 2>/dev/null)
EXIT=$?
```
...
case $EXIT in
  0)
    ...
    tac headless query | jq '.state.progress'
  ...
  10)
    ...
    tac headless query | jq '{phase: .state.phase, blockers: .state.blockers}'
...
if [ $EXIT -eq 0 ]; then
  ...
  echo "Files created:"
  find . -not -path './.tac/*' -not -path './.git/*' -type f
else
