...
/**
 * TAC Interactive Installer
 *
 * Entry point for `npx tac-2` or `npx tac-2@latest`.
 ...
 *
 * If TAC is already installed and the user runs `tac`, this script
 * is NOT invoked — the normal loader.js handles that via the "tac" bin.
 * This script only fires for `npx tac-2` (the package name bin).
 */
...
// packageRoot is always relative to this script — it's the tac-2 package directory.
// This is correct whether running as postinstall (inside node_modules/tac-2) or
...
if (HAS_HELP) {
  ...
  ${c.bold}TAC Installer${c.reset} ${c.dim}v${tacVersion}${c.reset}

  ${c.yellow}Usage:${c.reset}
    npx tac-2@latest          Install TAC globally (recommended)
    npx tac-2@latest --local  Install TAC to current project
  ...
  ${c.yellow}Environment:${c.reset}
    ...
    TAC_SKIP_RTK_INSTALL=1              Skip RTK
    TAC_RTK_DISABLED=1                  Disable RTK integration
...
${c.cyan}   ██████╗ ███████╗██████╗
  ...
  ${c.bold}Think. Architect. Code.${c.reset} ${c.dim}v${tacVersion}${c.reset}
`)
}
...
const RTK_SKIP =
  process.env.TAC_SKIP_RTK_INSTALL === '1' ||
  process.env.TAC_SKIP_RTK_INSTALL === 'true' ||
  process.env.TAC_RTK_DISABLED === '1' ||
  process.env.TAC_RTK_DISABLED === 'true' ||
...
const managedBinDir = join(process.env.TAC_HOME || join(homedir(), '.tac'), 'agent', 'bin')
const managedBinaryPath = join(managedBinDir, platform() === 'win32' ? 'rtk.exe' : 'rtk')

// ── Step: npm install -g ───────────────────────────────────────────────────

async function installGlobally() {
  startSpinner('Installing tac-2 globally...             ')
  try {
    const result = await new Promise((res) => {
      execCb(
        `npm install -g tac-2@${tacVersion}`,
        { timeout: 300_000 },
        (error, stdout, stderr) => {
    ...
    if (!result.ok) {
      const meaningful = (result.stderr || '')
        ...
        .slice(-3)
        .join('; ')
      printFail('Global install failed', meaningful || 'run npm install -g tac-2 manually')
      return false
    }

    printStep('Installed globally', 'npm install -g tac-2')
    return true
  } catch (err) {
    stopSpinner()
...
async function installLocally() {
  startSpinner('Installing tac-2 locally...              ')
  try {
    const result = await new Promise((res) => {
      execCb(
        `npm install tac-2@${tacVersion}`,
    ...
    if (!result.ok) {
      const meaningful = (result.stderr || '')
        ...
        .slice(-3)
        .join('; ')
      printFail('Local install failed', meaningful || 'run npm install tac-2 manually')
      return false
    }

    printStep('Installed locally', 'npm install tac-2')
    return true
  } catch (err) {
    stopSpinner()
...
async function downloadToFile(url, destination) {
  const response = await fetch(url, { headers: { 'User-Agent': 'tac-2-installer' } })
...
async function installRtk() {
  ...
  try {
    const checksumsResponse = await fetch(`${releaseBase}/checksums.txt`, {
      headers: { 'User-Agent': 'tac-2-installer' },
    })
...
function verifyInstall(local) {
  let bin = 'tac'
  if (local) {
    const localBin = resolve(process.cwd(), 'node_modules', '.bin', 'tac')
...
if (IS_POSTINSTALL) {
  ...
  if (version) {
    printStep('Verified', `tac v${version}`)
  }
}

process.stdout.write(`\n  ${c.green}Ready.${c.reset} Run: ${c.bold}tac${c.reset}\n\n`)
