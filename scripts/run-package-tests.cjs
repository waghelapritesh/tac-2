// TAC-2 + scripts/run-package-tests.cjs — run `node --test` across every linkable workspace package
'use strict'
...
function hasNativeAddon() {
	...
	return (
		existsSync(join(REPO_ROOT, 'native', 'addon', `tac_engine.${platformTag}.node`)) ||
		existsSync(join(REPO_ROOT, 'native', 'addon', 'tac_engine.dev.node'))
	)
}
