#!/usr/bin/env node
/**
 * TAC Parallel Worker Monitor
 * 
 * Real-time TUI dashboard for monitoring parallel TAC auto-mode workers.
 ...
 *   --dir <path>          Status file directory (default: .tac/parallel)
 *   --root <path>         Project root (default: cwd)
 * 
 * Data sources:
 *   .tac/parallel/M0xx.status.json  — heartbeat, cost, state (written by orchestrator)
 *   .tac/worktrees/M0xx/.tac/auto.lock — current unit type + ID (written by worker)
 *   .tac/worktrees/M0xx/.tac/tac.db — task/slice completion (SQLite, queried via cli)
 *   .tac/parallel/M0xx.stdout.log — NDJSON events (cost extraction, notify messages)
 *   .tac/parallel/M0xx.stderr.log — error surfacing
 * 
 * Health indicators:
...
const PARALLEL_DIR = getArg('--dir', '.tac/parallel');
...
function discoverWorkers() {
  ...
  const worktreeDir = path.resolve(PROJECT_ROOT, '.tac/worktrees');
  const mids = new Set();
  ...
  if (fs.existsSync(worktreeDir)) {
    for (const d of fs.readdirSync(worktreeDir)) {
      if (d.startsWith('M') && fs.existsSync(path.join(worktreeDir, d, '.tac', 'auto.lock'))) {
        mids.add(d);
      }
    }
  }
...
function readAutoLock(mid) {
  const lockPath = path.resolve(PROJECT_ROOT, `.tac/worktrees/${mid}/.tac/auto.lock`);
  return readJsonSafe(lockPath);
}

function querySliceProgress(mid) {
  const dbPath = path.resolve(PROJECT_ROOT, `.tac/worktrees/${mid}/.tac/tac.db`);
...
// Auto-detect the TAC loader path — works across npm global, homebrew, and local installs
function findTacLoader() {
  // 1. Check if we're running from inside the tac-2 repo itself
  ...
  try {
    ...
    const candidates = [
      path.join(globalRoot, 'tac-2', 'dist', 'loader.js'),
      path.join(globalRoot, '@tac', 'pi', 'dist', 'loader.js'),
    ];
  ...
  // 3. Try `which tac` and resolve symlink
  try {
    const pathLookup = process.platform === 'win32' ? 'where.exe' : 'which';
    const lookupArgs = ['tac'];
...
const TAC_LOADER = findTacLoader();
...
function respawnWorker(mid) {
  const worktreeDir = path.resolve(PROJECT_ROOT, `.tac/worktrees/${mid}`);
  if (!fs.existsSync(worktreeDir)) return null;
  if (!fs.existsSync(TAC_LOADER)) return null;
  ...
  try {
    ...
    const child = spawn(process.execPath, [TAC_LOADER, 'headless', '--json', 'auto'], {
      cwd: worktreeDir,
      detached: true,
      env: {
        ...process.env,
        TAC_MILESTONE_LOCK: mid,
        TAC_PROJECT_ROOT: PROJECT_ROOT,
...
function render(workers) {
  ...
  // ── Header ──
  buf.push('');
  const title = ' TAC Parallel Monitor ';
  ...
  if (workers.length === 0) {
    ...
    buf.push(`  ${DIM}Waiting for .tac/parallel/*.status.json files...${RESET}`);
  } else {
    for (const wk of workers) {
      buf.push('');
