#!/usr/bin/env node
// TAC-2 + scripts/verify-workspace-coverage.cjs — CI gate: every linkable workspace package must have test coverage
'use strict'
...
function main() {
	...
	for (const pkg of packages) {
		...
		// Rule 2: package.json must have "name" that matches tac.scope/tac.name (already enforced by
		...
		if (pkg.packageName !== expectedName) {
			reasons.push(`package.json "name" (${pkg.packageName}) must equal tac.scope/tac.name (${expectedName})`)
		}
	...
	process.stderr.write(
		'\nEvery package marked "tac.linkable: true" must ship with tests.\n' +
		'This gate exists because PR #4668 shipped three new @tac/* packages with zero test\n' +
		'coverage (the test globs silently excluded them), which hid 14 CRITICAL regressions.\n' +
		'See PR #4673 revert notes.\n\n' +
		'To fix: add at least one *.test.ts file to the package, or remove "tac.linkable" if\n' +
