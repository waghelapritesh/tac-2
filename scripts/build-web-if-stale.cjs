 ...
 *
 * Also self-heals a missing/incomplete web dependency install so `npm run tac:web`
...
if (process.platform === 'win32') {
  console.log('[tac] Web build skipped on Windows.')
  process.exit(0)
}
...
function ensureWebBuildDependencies() {
  ...
  console.log('[tac] Web build dependencies are missing or incomplete — running npm --prefix web ci...')
...
if (builtMtime > 0 && builtMtime >= sourceMtime) {
  console.log('[tac] Web build is up-to-date, skipping rebuild.')
  process.exit(0)
}

if (builtMtime === 0) {
  console.log('[tac] No staged web build found — building now...')
} else {
  console.log('[tac] Web/src source has changed since last build — rebuilding...')
}
...
try {
  ...
  console.error('[tac] Web build failed:', err.message)
  process.exit(1)
}
