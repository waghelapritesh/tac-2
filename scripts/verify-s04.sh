...
# Checks: dist integrity, TAC_BUNDLED_EXTENSION_PATHS, prepublishOnly,
#         npm pack dry-run, tarball install, binary exists, launch (no extension
#         errors, "tac" branding), ~/.tac/ untouched, non-TTY warning/no exit 1.
...
SMOKE_PREFIX=/tmp/tac-smoke-prefix
TARBALL=""

# Capture ~/.tac/agent/sessions/ count before any smoke runs (for Check 9)
PI_SESSIONS_BEFORE=$(ls ~/.tac/agent/sessions/ 2>/dev/null | wc -l | tr -d ' ')
...
# ----------------------------------------------------------------
# Check 2 — TAC_BUNDLED_EXTENSION_PATHS does NOT reference src/resources
# ----------------------------------------------------------------
# The variable must be present and must use agentDir-based paths only.
paths_line=$(grep "TAC_BUNDLED_EXTENSION_PATHS" dist/loader.js | grep -v "src/resources" | head -1)
if [ -n "$paths_line" ]; then
  ...
  if grep -A 15 "TAC_BUNDLED_EXTENSION_PATHS" dist/loader.js | grep "join(" | grep -q "src/resources"; then
    fail "2 — TAC_BUNDLED_EXTENSION_PATHS still references src/resources path(s)"
  else
    pass "2 — TAC_BUNDLED_EXTENSION_PATHS uses agentDir-based paths (no src/resources)"
  fi
else
  fail "2 — TAC_BUNDLED_EXTENSION_PATHS line not found or still references src/resources"
fi
...
TARBALL=$(ls glittercowboy-tac-*.tgz 2>/dev/null | head -1 || true)
if [ -n "$TARBALL" ] && [ -f "$TARBALL" ]; then
...
# ----------------------------------------------------------------
if [ -f "$SMOKE_PREFIX/bin/tac" ] || [ -L "$SMOKE_PREFIX/bin/tac" ]; then
  pass "7 — $SMOKE_PREFIX/bin/tac exists after install"
else
  fail "7 — $SMOKE_PREFIX/bin/tac not found after install"
...
# ----------------------------------------------------------------
# Check 8 — launch: "tac" branding + zero extension load errors
...
(
  env -i HOME="$HOME" PATH="$PATH" \
    "$SMOKE_PREFIX/bin/tac" < /dev/null > "$smoke_out" 2>&1
) &
smoke_pid=$!
sleep 8
...
has_tac=$(echo "$plain_out" | grep -qi "tac\|get shit done" && echo "yes" || echo "no")
...
if [ "$has_tac" = "yes" ]; then
  pass "8b — \"tac\" / \"get shit done\" branding found in launch output"
else
  ...
  if [ "$has_pi_only" = "no" ]; then
    pass "8b — output does not show \"pi\" branding (tac branding likely in ANSI sequences)"
  else
    fail "8b — output shows \"pi\" branding instead of \"tac\""
...
# ----------------------------------------------------------------
# Check 9 — ~/.tac/ session count unchanged before/after smoke run
# PI_SESSIONS_BEFORE captured at script start (before any binary invocation).
# ----------------------------------------------------------------
pi_after=$(ls ~/.tac/agent/sessions/ 2>/dev/null | wc -l | tr -d ' ')
if [ "$PI_SESSIONS_BEFORE" = "$pi_after" ]; then
  pass "9 — ~/.tac/agent/sessions/ count unchanged (${pi_after} sessions before and after)"
else
  fail "9 — ~/.tac/agent/sessions/ count changed: was ${PI_SESSIONS_BEFORE}, now ${pi_after}"
fi
...
(
  env -i HOME="$HOME" PATH="$PATH" \
    "$SMOKE_PREFIX/bin/tac" < /dev/null > "$tmp10" 2>&1
  echo "$?" > "$exit10_tmp"
) &
pid10=$!
sleep 5
