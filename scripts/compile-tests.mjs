...
async function main() {
  ...
  const testDirsToClean = [
    ...
    [join(ROOT, 'dist-test', 'src', 'resources', 'extensions', 'tac', 'tests'),
     join(ROOT, 'src', 'resources', 'extensions', 'tac', 'tests')],
  ];
  let staleCleaned = 0;
  ...
  // packageRoot from import.meta.url) resolves tacNodeModules to a real path.
