...
# --- Guard: workspace packages must not have @tac/* cross-deps ---
echo "==> Checking workspace packages for @tac/* cross-deps..."
CROSS_FAILED=0
...
if [ "$CROSS_FAILED" = "1" ]; then
  echo "ERROR: Workspace packages have @tac/* cross-dependencies."
  echo "    These cause 404s when npm resolves them from the registry."
  exit 1
fi
echo "    No @tac/* cross-dependencies."
