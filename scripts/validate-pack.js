...
try {
  ...
  // --- Guard: @tac/* workspace packages must not have @tac/* cross-deps ---
  // (@waghelapritesh/* packages CAN depend on each other — e.g., mcp-server depends
  // on rpc-client — because they are both published to the registry.)
  console.log('==> Checking workspace packages for @tac/* cross-deps...');
  let crossFailed = false;

  for (const ws of getCorePackages()) {
    const pkg = JSON.parse(readFileSync(ws.packageJsonPath, 'utf8'));
    const deps = Object.keys(pkg.dependencies || {}).filter(d => d.startsWith('@tac/'));
    if (deps.length) {
  ...
  if (crossFailed) {
    console.log('ERROR: Workspace packages have @tac/* cross-dependencies.');
    console.log('    These cause 404s when npm resolves them from the registry.');
    process.exit(1);
  }
  console.log('    No @tac/* cross-dependencies.');
  ...
  // node_modules/@tac/ is never populated, causing ERR_MODULE_NOT_FOUND at runtime.
  // Checks every package with `tac.linkable: true` — not just a hand-picked subset —
  // so any future addition is automatically covered.
  console.log('==> Verifying workspace package resolution (every linkable package)...');
  const installedRoot = join(installDir, 'node_modules', 'tac-2');
  ...
  console.log('==> Running installed binary (tac -v)...');
  ...
  try {
    ...
    console.log(`    tac -v => ${versionOutput}`);
    if (!versionOutput.match(/^\d+\.\d+\.\d+/)) {
      console.log('ERROR: tac -v returned unexpected output (expected a version string).');
      process.exit(1);
    }
  } catch (err) {
    console.log('ERROR: Running tac -v failed after install.');
    if (err.stdout) console.log(err.stdout);
    if (err.stderr) console.log(err.stderr);
    process.exit(1);
  }
