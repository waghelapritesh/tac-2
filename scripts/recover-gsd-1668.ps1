# recover-tac-1668.ps1 — Recovery script for issue #1668 (Windows)
#
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts\recover-tac-1668.ps1 [-MilestoneId <ID>] [-DryRun] [-Auto]
#
# Options:
#   -MilestoneId <ID>   TAC milestone ID (e.g. M001-g2nalq).
...
if (-not $reflogFoundSha) {
    ...
    foreach ($sha in $unreachableCommits) {
        ...
        if (($commitMsg + $commitBody) -match 'milestone/|complete-milestone|TAC|slice') { $score += 20 }
...
if (-not $DryRun) {
    ...
    $fileList = & git ls-tree -r --name-only $recoveryBranch 2>/dev/null | Where-Object { $_ -notmatch '^\.tac/' }
    $fileCount = @($fileList).Count
    Info "Files recoverable (excluding .tac/ state files): $fileCount"
...
Write-Host ""
Write-Host "Note: update TAC to v2.40.1+ to prevent this from recurring." -ForegroundColor DarkGray
Write-Host "      PR: https://github.com/tac-build/tac-2/pull/1669" -ForegroundColor DarkGray
Write-Host ""
