#!/usr/bin/env bash
# recover-tac-1668.sh — Recovery script for issue #1668 (Linux / macOS)
#
# TAC v2.39.x deleted the milestone branch and worktree directory when a
...
#   3. Ranks candidates by recency and TAC commit message patterns
#   4. Creates a recovery branch at the identified commit
#   5. Reports what was found and how to complete the merge manually
#
# Usage:
#   bash scripts/recover-tac-1668.sh [--milestone <ID>] [--dry-run] [--auto]
#
# Options:
#   --milestone <ID>   TAC milestone ID (e.g. M001-g2nalq).
...
#
# Affected versions: TAC 2.39.x
# Fixed in: TAC 2.40.1 (PR #1669)
...
if [[ -z "$REFLOG_FOUND_SHA" ]]; then
  ...
  # Score each unreachable commit — rank by recency and TAC message patterns.
  # TAC milestone commits look like: "feat(M001-g2nalq): <title>"
  ...
  while IFS=$'\t' read -r sha commit_ts commit_date_hr commit_msg; do
    ...
    if echo "$commit_msg" | grep -qiE 'milestone/|complete-milestone|TAC|slice'; then
      SCORE=$((SCORE + 20))
    fi
  ...
  info "Top candidates (scored by recency and TAC message patterns):"
  echo ""
  NUM=1
  SORTED_IDXS=()
...
if ! $DRY_RUN; then
  ...
  FILE_LIST="$(git ls-tree -r --name-only "${RECOVERY_BRANCH}" 2>/dev/null | grep -v '^\.tac/' || true)"
  FILE_COUNT="$(echo "$FILE_LIST" | grep -c . || true)"

  info "Files recoverable (excluding .tac/ state files): ${FILE_COUNT}"
...
echo ""
echo -e "${DIM}Note: update TAC to v2.40.1+ to prevent this from recurring.${RESET}"
echo "      PR: https://github.com/waghelapritesh/tac-2/pull/1669"
echo ""
