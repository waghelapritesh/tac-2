...
- **SCM provider** — "TAC Agent" appears in Source Control panel with accept/discard per-file diffs
...
- **Slash completion** filtered to `/tac` commands only
- **Checkpoint labels** show timestamp + first action (e.g., "10:32 — Edit sidebar.ts")
- **Session tree** supports ISO timestamp filenames (TAC's actual format)
...
- **Sidebar reduced** from 6 panels to 3 (TAC Agent, Sessions, Activity)
...
- **Enhanced code lens** — Refactor, Find Bugs, and Generate Tests alongside Ask TAC
...
- Full RPC client — spawns `tac --mode rpc`, JSON line framing, all RPC commands
- Sidebar dashboard — connection status, model info, thinking level, token usage, cost, quick actions
- Chat participant — `@tac` in VS Code Chat with streaming responses
...
- Slash command completion — auto-complete for `/tac` commands
- Code lens — "Ask TAC" above functions and classes in TS/JS/Python/Go/Rust
