...
/**
 * Mirrors the RPC command/response protocol from the TAC agent.
...
/**
 * Client that spawns `tac --mode rpc` and communicates via JSON lines
 * over stdin/stdout. Emits VS Code events for streaming responses.
 */
export class TacClient implements vscode.Disposable {
	...
	/**
	 * Spawn the TAC agent in RPC mode.
	 */
	async start(): Promise<void> {
		...
		const startupResult = new Promise<void>((resolve, reject) => {
			...
			const handleStartupError = (err: NodeJS.ErrnoException) => {
				...
				const hint = err.code === "ENOENT"
					? ` Make sure TAC is installed ("npm install -g tac-2") and set "tac.binaryPath" to the absolute path if it is not on PATH.`
					: "";
				const message = `Failed to start TAC process: ${err.message}.${hint}`;
		...
		proc.on("error", (err: NodeJS.ErrnoException) => {
			...
			const hint = err.code === "ENOENT"
				? ` Make sure TAC is installed ("npm install -g tac-2") and set "tac.binaryPath" to the absolute path if it is not on PATH.`
				: "";
			this._onError.fire(`TAC process error: ${err.message}.${hint}`);
		});
		...
		proc.on("exit", (code, signal) => {
			...
			this.rejectAllPending(`TAC process exited (code=${code}, signal=${signal})`);
			this._onConnectionChange.fire(false);
			...
			if (code !== 0 && signal !== "SIGTERM") {
				...
				if (this.restartTimestamps.length > 3) {
					...
					this._onError.fire(
						`TAC process crashed ${this.restartTimestamps.length} times within 60s. Not restarting. Use "TAC: Start Agent" to retry manually.`,
					);
		...
		try {
			switch (method) {
				...
				case "notify": {
					...
					if (notifyType === "error") {
						vscode.window.showErrorMessage(`TAC: ${message}`);
					} else if (notifyType === "warning") {
						vscode.window.showWarningMessage(`TAC: ${message}`);
					} else {
						vscode.window.showInformationMessage(`TAC: ${message}`);
					}
	...
	private send(command: Record<string, unknown>): Promise<RpcResponse> {
		if (!this.process?.stdin) {
			return Promise.reject(new Error("TAC client not started"));
		}
