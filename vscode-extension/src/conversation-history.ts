...
import type { TacClient } from "./tac-client.js";
 ...
 * current TAC session using the get_messages RPC call. Shows tool calls,
 * thinking blocks, search/filter, and fork-from-here actions.
 */
export class TacConversationHistoryPanel implements vscode.Disposable {
	private static currentPanel: TacConversationHistoryPanel | undefined;
	...
	static createOrShow(
		...
		client: TacClient,
	): TacConversationHistoryPanel {
		const column = vscode.window.activeTextEditor?.viewColumn ?? vscode.ViewColumn.One;

		if (TacConversationHistoryPanel.currentPanel) {
			TacConversationHistoryPanel.currentPanel.panel.reveal(column);
			void TacConversationHistoryPanel.currentPanel.refresh();
			return TacConversationHistoryPanel.currentPanel;
		}

		const panel = vscode.window.createWebviewPanel(
			"tac-history",
			"TAC Conversation History",
			column,
			{
				enableScripts: true,
		...
		TacConversationHistoryPanel.currentPanel = new TacConversationHistoryPanel(
			panel,
			extensionUri,
			client,
		);
	...
	async refresh(): Promise<void> {
		if (!this.client.isConnected) {
			this.panel.webview.html = this.getHtml([], "Not connected to TAC agent.");
			return;
		}
	...
	dispose(): void {
		TacConversationHistoryPanel.currentPanel = undefined;
		this.panel.dispose();
	...
	private getHtml(messages: ConversationMessage[], errorMessage?: string): string {
		...
		const renderedMessages = visibleMessages
			.map((msg, idx) => {
				...
				<div class="role-row">
					<span class="role">${isUser ? "You" : "TAC"}</span>
					${forkBtn}
				</div>
