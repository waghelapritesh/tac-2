 ...
 */
export class TacDiagnosticBridge implements vscode.Disposable {
	private readonly collection: vscode.DiagnosticCollection;
	private disposables: vscode.Disposable[] = [];

	constructor(private readonly client: TacClient) {
		this.collection = vscode.languages.createDiagnosticCollection("tac");
	...
	/**
	 * Add a TAC diagnostic (agent finding) to a file.
	...
	addFinding(
		...
		diagnostic.source = "TAC Agent";
		this.collection.set(uri, [...existing, diagnostic]);
	}

	/** Clear all TAC diagnostics */
	clearFindings(): void {
