...
import type { TacClient, SessionStats, ThinkingLevel } from "./tac-client.js";

/**
 * Send a message through VS Code's Chat panel so the user sees the response.
 * Opens the Chat panel and pre-fills the @tac participant with the message.
 */
 ...
 */
export class TacSidebarProvider implements vscode.WebviewViewProvider {
	public static readonly viewId = "tac-sidebar";
	...
	constructor(
		...
		private readonly client: TacClient,
	) {
	...
	resolveWebviewView(
		...
		webviewView.webview.onDidReceiveMessage(async (msg: { command: string; value?: string }) => {
			switch (msg.command) {
				case "start":
					await vscode.commands.executeCommand("tac.start");
					break;
				case "stop":
					await vscode.commands.executeCommand("tac.stop");
					break;
				case "newSession":
					await vscode.commands.executeCommand("tac.newSession");
					break;
				case "cycleModel":
					await vscode.commands.executeCommand("tac.cycleModel");
					break;
				case "cycleThinking":
					await vscode.commands.executeCommand("tac.cycleThinking");
					break;
				case "switchModel":
					await vscode.commands.executeCommand("tac.switchModel");
					break;
				case "setThinking":
					await vscode.commands.executeCommand("tac.setThinking");
					break;
				case "compact":
					await vscode.commands.executeCommand("tac.compact");
					break;
				case "abort":
					await vscode.commands.executeCommand("tac.abort");
					break;
				case "exportHtml":
					await vscode.commands.executeCommand("tac.exportHtml");
					break;
				case "sessionStats":
					await vscode.commands.executeCommand("tac.sessionStats");
					break;
				case "listCommands":
					await vscode.commands.executeCommand("tac.listCommands");
					break;
				...
				case "toggleAutoRetry":
					...
					break;
				case "setSessionName":
					await vscode.commands.executeCommand("tac.setSessionName");
					break;
				case "copyLastResponse":
					await vscode.commands.executeCommand("tac.copyLastResponse");
					break;
				case "autoMode":
					await sendViaChat("@tac /tac auto");
					break;
				case "nextUnit":
					await sendViaChat("@tac /tac next");
					break;
				case "quickTask": {
					...
					if (quickInput) {
						await sendViaChat(`@tac /tac quick ${quickInput}`);
					}
					break;
				}
				case "capture": {
					...
					if (thought) {
						await sendViaChat(`@tac /tac capture ${thought}`);
					}
					break;
				}
				case "status":
					await sendViaChat("@tac /tac status");
					break;
				case "forkSession":
					await vscode.commands.executeCommand("tac.forkSession");
					break;
				case "toggleSteeringMode":
					await vscode.commands.executeCommand("tac.toggleSteeringMode");
					break;
				case "toggleFollowUpMode":
					await vscode.commands.executeCommand("tac.toggleFollowUpMode");
					break;
					case "showHistory":
						await vscode.commands.executeCommand("tac.showHistory");
						break;
					case "fixProblemsInFile":
						await vscode.commands.executeCommand("tac.fixProblemsInFile");
						break;
					case "selectApprovalMode":
						await vscode.commands.executeCommand("tac.selectApprovalMode");
						break;
					default:
						vscode.window.showWarningMessage(`Unknown TAC sidebar command: ${msg.command}`);
						break;
				}
			});
