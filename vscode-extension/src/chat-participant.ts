...
import type { AgentEvent, TacClient } from "./tac-client.js";

/**
 * Registers the @tac chat participant that forwards messages to the
 * TAC RPC client and streams tool execution events back to the chat.
 */
export function registerChatParticipant(
	context: vscode.ExtensionContext,
	client: TacClient,
): vscode.Disposable {
	const participant = vscode.chat.createChatParticipant("tac.agent", async (
		...
		if (!client.isConnected) {
			response.progress("Starting TAC agent...");
			try {
				await client.start();
			} catch (err) {
				const msg = err instanceof Error ? err.message : String(err);
				response.markdown(`**Failed to start TAC agent:** ${msg}\n\nMake sure \`tac\` is installed (\`npm install -g tac-2\`) and try again.`);
				return;
			}
		}
		...
		const eventHandler = (event: AgentEvent) => {
			switch (event.type) {
				case "agent_start":
					response.progress("TAC is working...");
					break;
