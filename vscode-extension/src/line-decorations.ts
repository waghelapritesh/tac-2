...
/**
 * Provides line-level editor decorations for files modified by the TAC agent.
 * Shows subtle background highlights on changed lines and gutter icons.
 */
export class TacLineDecorationManager implements vscode.Disposable {
	...
	private refreshEditor(editor: vscode.TextEditor): void {
		...
		const addedRanges = added.map((line) => {
			...
			return { range, hoverMessage: new vscode.MarkdownString("$(hubot) *Added by TAC Agent*") };
		});

		const modifiedRanges = modified.map((line) => {
			const range = new vscode.Range(line, 0, line, currentLines[line]?.length ?? 0);
			return { range, hoverMessage: new vscode.MarkdownString("$(hubot) *Modified by TAC Agent*") };
		});
