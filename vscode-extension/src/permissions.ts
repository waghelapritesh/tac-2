 ...
 */
export class TacPermissionManager implements vscode.Disposable {
	...
	constructor(private readonly client: TacClient) {
		...
		this._mode = vscode.workspace.getConfiguration("tac").get<ApprovalMode>("approvalMode", "auto-approve");

		this.disposables.push(
			this._onModeChange,
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("tac.approvalMode")) {
					this._mode = vscode.workspace.getConfiguration("tac").get<ApprovalMode>("approvalMode", "auto-approve");
	...
	async cycleMode(): Promise<void> {
		...
		await vscode.workspace.getConfiguration("tac").update("approvalMode", this._mode, vscode.ConfigurationTarget.Workspace);
		this._onModeChange.fire(this._mode);
