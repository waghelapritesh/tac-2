...
import type { TacClient, SlashCommand } from "./tac-client.js";

/**
 * CompletionItemProvider that surfaces TAC slash commands when the user
 ...
 */
export class TacSlashCompletionProvider
	implements vscode.CompletionItemProvider, vscode.Disposable
{
	...
	private async refreshCache(): Promise<void> {
		try {
			...
			// Only show /tac commands — filter out unrelated extension/skill commands
			this.cachedCommands = all.filter((cmd) => cmd.name.startsWith("tac"));
		} catch {
