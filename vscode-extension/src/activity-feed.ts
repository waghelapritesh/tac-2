...
import type { TacClient, AgentEvent } from "./tac-client.js";
...
/**
 * TreeDataProvider that shows real-time tool executions from the TAC agent.
 * Listens to tool_execution_start/end and agent_start/end events.
 */
export class TacActivityFeedProvider implements vscode.TreeDataProvider<ActivityItem>, vscode.Disposable {
	public static readonly viewId = "tac-activity";
	...
	constructor(private readonly client: TacClient) {
		this.maxItems = vscode.workspace.getConfiguration("tac").get<number>("activityFeedMaxItems", 100);
		...
		this.disposables.push(
			...
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("tac.activityFeedMaxItems")) {
					this.maxItems = vscode.workspace.getConfiguration("tac").get<number>("activityFeedMaxItems", 100);
				}
			}),
		);
