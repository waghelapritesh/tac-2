...
import { TacClient, ThinkingLevel } from "./tac-client.js";
import { registerChatParticipant } from "./chat-participant.js";
import { TacSidebarProvider } from "./sidebar.js";
import { TacFileDecorationProvider } from "./file-decorations.js";
import { TacBashTerminal } from "./bash-terminal.js";
import { TacSessionTreeProvider } from "./session-tree.js";
import { TacConversationHistoryPanel } from "./conversation-history.js";
import { TacSlashCompletionProvider } from "./slash-completion.js";
import { TacCodeLensProvider } from "./code-lens.js";
import { TacActivityFeedProvider } from "./activity-feed.js";
import { TacChangeTracker } from "./change-tracker.js";
import { TacScmProvider } from "./scm-provider.js";
import { TacDiagnosticBridge } from "./diagnostics.js";
import { TacLineDecorationManager } from "./line-decorations.js";
import { TacGitIntegration } from "./git-integration.js";
import { TacPermissionManager } from "./permissions.js";

let client: TacClient | undefined;
let sidebarProvider: TacSidebarProvider | undefined;
let fileDecorations: TacFileDecorationProvider | undefined;
let sessionTreeProvider: TacSessionTreeProvider | undefined;
let activityFeedProvider: TacActivityFeedProvider | undefined;
let changeTracker: TacChangeTracker | undefined;
let scmProvider: TacScmProvider | undefined;
let diagnosticBridge: TacDiagnosticBridge | undefined;
let lineDecorations: TacLineDecorationManager | undefined;
let gitIntegration: TacGitIntegration | undefined;
let permissionManager: TacPermissionManager | undefined;
...
export function resolveTrustedTacStartupConfig(): { binaryPath: string; autoStart: boolean } {
	return {
		binaryPath: getTrustedConfigurationValue("tac", "binaryPath", "tac"),
		autoStart: getTrustedConfigurationValue("tac", "autoStart", false),
	};
}

function requireConnected(): boolean {
	if (!client?.isConnected) {
		vscode.window.showWarningMessage("TAC agent is not running.");
		return false;
	}
	return true;
}
...
export function activate(context: vscode.ExtensionContext): void {
	const startupConfig = resolveTrustedTacStartupConfig();
	const config = vscode.workspace.getConfiguration("tac");
	const binaryPath = startupConfig.binaryPath;
	const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? process.cwd();

	client = new TacClient(binaryPath, cwd);
	context.subscriptions.push(client);

	// Log stderr to an output channel
	const outputChannel = vscode.window.createOutputChannel("TAC-2 Agent");
	...
	statusBarItem.command = "workbench.view.extension.tac";
	statusBarItem.text = "$(hubot) TAC";
	statusBarItem.tooltip = "TAC Agent — click to open";
	statusBarItem.show();
	...
	async function refreshStatusBar(): Promise<void> {
		if (!client?.isConnected) {
			statusBarItem.text = "$(hubot) TAC";
			statusBarItem.tooltip = "TAC: Disconnected";
			return;
		}
		try {
			...
			statusBarItem.text = `$(hubot) TAC${modelId ? ` | ${modelId}` : ""}${costPart}${streamPart}`;
			statusBarItem.tooltip = state?.model
				? `TAC: Connected — ${state.model.provider}/${state.model.id}`
				: "TAC: Connected";
		} catch {
			// ignore fetch errors
		}
	...
	client.onConnectionChange(async (connected) => {
		...
		if (connected) {
			vscode.window.setStatusBarMessage("$(hubot) TAC connected", 3000);
		} else {
			vscode.window.setStatusBarMessage("$(hubot) TAC disconnected", 3000);
		}
	});

	// -- Sidebar -----------------------------------------------------------

	sidebarProvider = new TacSidebarProvider(context.extensionUri, client);
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(
			TacSidebarProvider.viewId,
			sidebarProvider,
		),
	);
	...
	fileDecorations = new TacFileDecorationProvider(client);
	context.subscriptions.push(
		fileDecorations,
	...
	const bashTerminal = new TacBashTerminal(client);
	context.subscriptions.push(bashTerminal);

	// -- Session tree view -------------------------------------------------

	sessionTreeProvider = new TacSessionTreeProvider(client);
	context.subscriptions.push(
		sessionTreeProvider,
		vscode.window.registerTreeDataProvider(TacSessionTreeProvider.viewId, sessionTreeProvider),
	);

	// -- Activity feed -----------------------------------------------------

	activityFeedProvider = new TacActivityFeedProvider(client);
	context.subscriptions.push(
		activityFeedProvider,
		vscode.window.registerTreeDataProvider(TacActivityFeedProvider.viewId, activityFeedProvider),
	);

	// -- Change tracker & SCM provider -------------------------------------

	changeTracker = new TacChangeTracker(client);
	context.subscriptions.push(changeTracker);

	scmProvider = new TacScmProvider(changeTracker, cwd);
	context.subscriptions.push(scmProvider);

	// -- Diagnostics -------------------------------------------------------

	diagnosticBridge = new TacDiagnosticBridge(client);
	context.subscriptions.push(diagnosticBridge);

	// -- Line-level decorations --------------------------------------------

	lineDecorations = new TacLineDecorationManager(changeTracker!);
	context.subscriptions.push(lineDecorations);

	// -- Git integration ---------------------------------------------------

	gitIntegration = new TacGitIntegration(changeTracker!, cwd);
	context.subscriptions.push(gitIntegration);

	// -- Permissions -------------------------------------------------------

	permissionManager = new TacPermissionManager(client);
	...
	client.onEvent((evt) => {
		const showProgress = vscode.workspace.getConfiguration("tac").get<boolean>("showProgressNotifications", true);
	...
	client.onEvent(async (evt) => {
		...
		const showWarning = vscode.workspace.getConfiguration("tac").get<boolean>("showContextWarning", true);
		...
		try {
			...
			const threshold = vscode.workspace.getConfiguration("tac").get<number>("contextWarningThreshold", 80);
			...
			if (pct >= threshold) {
				...
				if (action === "Compact Now") {
					await vscode.commands.executeCommand("tac.compact");
				}
			}
		} catch {
			// ignore
	...
	// (panel is created on demand via tac.showHistory command)

	// -- Slash command completion ------------------------------------------

	const slashCompletion = new TacSlashCompletionProvider(client);
	context.subscriptions.push(
		slashCompletion,
	...
	const codeLensProvider = new TacCodeLensProvider(client);
	context.subscriptions.push(
		codeLensProvider,
	...
	// Start
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.start", async () => {
			try {
				await client!.start();
				// Apply auto-compaction setting
				const autoCompaction = vscode.workspace.getConfiguration("tac").get<boolean>("autoCompaction", true);
				await client!.setAutoCompaction(autoCompaction).catch(() => {});
				sidebarProvider?.refresh();
				refreshStatusBar();
				vscode.window.showInformationMessage("TAC agent started.");
			} catch (err) {
				handleError(err, "Failed to start TAC");
			}
		}),
	);
	...
	// Stop
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.stop", async () => {
			await client!.stop();
			sidebarProvider?.refresh();
			vscode.window.showInformationMessage("TAC agent stopped.");
		}),
	);

	// New Session
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.newSession", async () => {
			...
			try {
				...
				vscode.window.showInformationMessage("New TAC session started.");
			} catch (err) {
	...
	// Send Message
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.sendMessage", async () => {
			if (!requireConnected()) return;
			const message = await vscode.window.showInputBox({
				prompt: "Enter message for TAC",
	...
	// Abort
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.abort", async () => {
			if (!requireConnected()) return;
			try {
				await client!.abort();
	...
	// Cycle Model
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.cycleModel", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.switchModel", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.cycleThinking", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.setThinking", async () => {
			if (!requireConnected()) return;
	...
	// Compact Context
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.compact", async () => {
			if (!requireConnected()) return;
			try {
	...
	// Export HTML
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.exportHtml", async () => {
			if (!requireConnected()) return;
			try {
				const saveUri = await vscode.window.showSaveDialog({
					defaultUri: vscode.Uri.file("tac-conversation.html"),
	...
	// Session Stats
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.sessionStats", async () => {
			if (!requireConnected()) return;
			try {
	...
	// Run Bash Command
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.runBash", async () => {
			if (!requireConnected()) return;
	...
	// Steer Agent
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.steer", async () => {
			if (!requireConnected()) return;
			const message = await vscode.window.showInputBox({
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.listCommands", async () => {
			if (!requireConnected()) return;
			try {
	...
	// Switch Session
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.switchSession", async (sessionFile?: string) => {
			if (!requireConnected()) return;
	...
	// Refresh Sessions
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.refreshSessions", () => {
			sessionTreeProvider?.refresh();
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.showHistory", () => {
			if (!requireConnected()) return;
			TacConversationHistoryPanel.createOrShow(context.extensionUri, client!);
		}),
	);
		...
		vscode.commands.registerCommand(
			...
			async (symbolName: string, fileName: string, lineNumber: number) => {
				...
				try {
					...
					handleError(err, "Failed to send Ask TAC request");
				}
			},
		),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.clearFileDecorations", () => {
			fileDecorations?.clear();
		}),
	);
	...
	// Clear Activity Feed
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.clearActivity", () => {
	...
	// Fork Session
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.forkSession", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.toggleSteeringMode", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.toggleFollowUpMode", async () => {
			if (!requireConnected()) return;
			try {
				const state = await client!.getState();
	...
	// Toggle Auto-Retry
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.toggleAutoRetry", async () => {
			if (!requireConnected()) return;
			try {
	...
	// Abort Retry
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.abortRetry", async () => {
			if (!requireConnected()) return;
			try {
	...
	// Set Session Name
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.setSessionName", async () => {
			if (!requireConnected()) return;
	...
	// Copy Last Response
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.copyLastResponse", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.acceptAllChanges", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.discardAllChanges", async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.discardFileChanges", async (resourceState: vscode.SourceControlResourceState) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.acceptFileChanges", (resourceState: vscode.SourceControlResourceState) => {
			if (!changeTracker || !resourceState?.resourceUri) return;
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.restoreCheckpoint", async (checkpointId: number) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.fixProblemsInFile", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.fixAllProblems", async () => {
			if (!requireConnected()) return;
			try {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.clearDiagnostics", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.cycleApprovalMode", () => {
			permissionManager?.cycleMode();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("tac.selectApprovalMode", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("tac.commitAgentChanges", () => {
			gitIntegration?.commitAgentChanges();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("tac.createAgentBranch", () => {
			gitIntegration?.createAgentBranch();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("tac.showAgentDiff", () => {
	...
	if (startupConfig.autoStart) {
		vscode.commands.executeCommand("tac.start");
	}
}
