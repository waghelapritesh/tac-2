...
/**
 * CodeLensProvider that adds an "Ask TAC" lens above named function and class
 * declarations. Clicking the lens sends a brief explanation request to the TAC
 * agent for that specific symbol.
 */
export class TacCodeLensProvider implements vscode.CodeLensProvider, vscode.Disposable {
	private readonly _onDidChangeCodeLenses = new vscode.EventEmitter<void>();
	readonly onDidChangeCodeLenses = this._onDidChangeCodeLenses.event;

	private disposables: vscode.Disposable[] = [];

	constructor(private readonly client: TacClient) {
		this.disposables.push(
			this._onDidChangeCodeLenses,
			client.onConnectionChange(() => this._onDidChangeCodeLenses.fire()),
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("tac.codeLens")) {
	...
	provideCodeLenses(
		...
		if (!vscode.workspace.getConfiguration("tac").get<boolean>("codeLens", true)) {
			return lenses;
		}
				...
				if (match && match[1] && !seen.has(i)) {
					...
					lenses.push(
						new vscode.CodeLens(range, {
							title: "$(hubot) Ask TAC",
							tooltip: `Ask TAC to explain ${symbolName}`,
							command: "tac.askAboutSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(pencil) Refactor",
							tooltip: `Refactor ${symbolName}`,
							command: "tac.refactorSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(bug) Find Bugs",
							tooltip: `Review ${symbolName} for bugs`,
							command: "tac.findBugsSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(beaker) Tests",
							tooltip: `Generate tests for ${symbolName}`,
							command: "tac.generateTestsSymbol",
							arguments: args,
						}),
					);
				}
