...
**Related ADR:** [ADR-008-tac-tools-over-mcp-for-provider-parity.md](/Users/jeremymcspadden/Github/tac-2/docs/ADR-008-tac-tools-over-mcp-for-provider-parity.md)
**Status:** Draft
**Date:** 2026-04-09

## Objective

Implement the ADR-008 decision by exposing the core TAC workflow tool contract over MCP, then wiring MCP-backed access into provider paths that cannot use the native in-process TAC tool registry directly.

The first usable outcome is:

- a Claude Code-backed execution session can complete a task using canonical TAC tools
- no manual summary-writing fallback is needed
- native provider behavior remains unchanged

## Non-Goals

- Replacing native in-process TAC tools with MCP
...
- Canonical TAC state transitions must remain DB-backed
...
- `src/resources/extensions/tac/bootstrap/db-tools.ts`
- `src/resources/extensions/tac/bootstrap/query-tools.ts`
- `src/resources/extensions/tac/tools/complete-task.ts`
...
Goal: add an MCP server surface for real TAC workflow tools, distinct from the current session/read API.
...
- extend existing MCP package vs create `packages/mcp-tac-tools-server`
...
Goal: attach the TAC workflow-tool MCP surface to Claude Code sessions.
...
- build a TAC-managed `mcpServers` config for the Claude SDK session
- attach the workflow MCP server only when the session requires TAC tools
- keep current Claude Code streaming behavior intact

Exit criteria:

- Claude Code session can discover the TAC workflow MCP tools
- task execution path can call `tac_task_complete` successfully
...
- TAC dispatch / auto-mode preflight
- provider selection and routing checks
- user-facing compatibility errors

Required behavior:

- if native TAC tools are available, proceed
- else if TAC workflow MCP tools are available, proceed
...
- `src/resources/extensions/tac/prompts/execute-task.md`
- related planning/discuss prompts that reference tool availability
- provider and MCP docs

Rules:

- prompts should keep requiring canonical TAC completion/planning tools
...
- extract shared logic for `tac_summary_save`, `tac_task_complete`, and `tac_milestone_status`
...
- Claude Code can call `tac_task_complete`
...
- plan or execute a small fixture task and complete it through canonical TAC tools
...
1. Claude Code-backed execution can use canonical TAC workflow tools over MCP.
...
1. Extract shared handlers for `tac_summary_save`, `tac_task_complete`, and `tac_milestone_status`.
