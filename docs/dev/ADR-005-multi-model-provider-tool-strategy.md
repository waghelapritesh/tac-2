...
**Related:** ADR-004 (capability-aware model routing), ADR-003 (pipeline simplification), [Issue #2790](https://github.com/waghelapritesh/tac-2/issues/2790)

## Context

PR #2755 lands capability-aware model routing (ADR-004), extending the router from a one-dimensional complexity-tier system to a two-dimensional system that scores models across 7 capability dimensions. TAC can now intelligently pick the best model for a task from a heterogeneous pool.
...
| `src/resources/extensions/tac/model-router.ts` | Capability scoring (ADR-004) |
| `src/resources/extensions/tac/auto-model-selection.ts` | Model selection orchestration |
