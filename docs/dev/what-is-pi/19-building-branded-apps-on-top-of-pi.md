...
- **No, you do not have to rely on `~/.tac`** if you embed pi with custom paths and storage
...
```bash
npm install -g my-tac
# or a standalone binary / packaged desktop app

tac
```

And inside `tac`, you import pi packages and create your own session, UI, storage, and resource loading behavior.

---

## 19.3 The Second Biggest Misconception: `~/.tac` Is a Default, Not a Requirement

Pi CLI defaults to `~/.tac/agent`, but embedded applications are not forced to use it.
...
- `~/.tac/agent`
- `~/Library/Application Support/TAC`
- `%APPDATA%/TAC`
...
This is the most natural fit for tools like `tac`.
...
- easiest way to remove dependency on `~/.tac`
...
|------|---------------------|
| Branded CLI like `tac` | `@mariozechner/pi-coding-agent` SDK |
...
## 19.6 The Recommended Path for a Branded CLI Like `tac`
...
const appRoot = path.join(os.homedir(), ".tac");
...
This is another place where people often assume they must rely on discovery from `~/.tac` or `.tac/`.
  ...
  extensionFactories: [
    (pi) => {
      pi.registerCommand("hello", {
        ...
        handler: async (_args, ctx) => ctx.ui.notify("Hello from TAC", "info"),
      });
    },
  ],
});
...
const loader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () => "You are TAC, a specialized software delivery agent.",
});
```
...
### Recommendation
For a branded tool like `tac`, default to **bundled-app product** behavior.
...
```typescript
const authStorage = AuthStorage.create("/path/to/tac/auth.json");
```
...
```typescript
const modelRegistry = new ModelRegistry(authStorage, "/path/to/tac/models.json");
```
...
## 19.18 Building a Branded `tac` CLI: Recommended Shape
...
const resourceLoader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () =>
    "You are TAC, a branded software delivery agent. Prefer project-specific workflows and terminology.",
  additionalExtensionPaths: [
...
If your app silently loads from a user's `~/.tac`, you may get:
...
If your app is called `tac`, but state lives in `~/.tac`, users will notice.
...
- Good for productized tools like `tac`
...
> “I want users to download and run `tac`, and have it use pi internally without requiring a separate pi install or `~/.tac` setup.”
