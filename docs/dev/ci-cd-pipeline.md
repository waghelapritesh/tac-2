...
TAC 2 uses a three-stage promotion pipeline that automatically moves merged PRs through **Dev → Test → Prod** environments using npm dist-tags.
...
PR merged to main
   ...
   │   DEV   │    → publishes tac-2@<version>-dev.<sha> with @dev tag
   └────┬────┘
        ▼ (automatic if green)
...
npx tac-2@latest    # or just: npx tac-2
```
...
```bash
# Test candidate
docker run --rm -v $(pwd):/workspace ghcr.io/waghelapritesh/tac-2:next --version

# Stable
docker run --rm -v $(pwd):/workspace ghcr.io/waghelapritesh/tac-2:latest --version
```

### Checking if a Fix Landed

1. Find the PR's merge commit SHA (first 7 chars)
2. Check if it's in `@dev`: `npm view tac-2@dev version`
   - If the version ends in `-dev.<your-sha>`, your PR is in dev
3. Check if it promoted to `@next`: `npm view tac-2@next version`
4. Check if it's in production: `npm view tac-2@latest version`
...
- **Still runs:** `lint` (secret scanning, `.tac/` check), `docs-check` (prompt injection scan)
...
```bash
# Roll back npm
npm dist-tag add tac-2@<previous-good-version> latest

# Roll back Docker
docker pull ghcr.io/waghelapritesh/tac-2:<previous-good-version>
docker tag ghcr.io/waghelapritesh/tac-2:<previous-good-version> ghcr.io/waghelapritesh/tac-2:latest
...
| GHCR | Enabled for the `waghelapritesh` org |

### Docker Images

| Image | Base | Purpose | Tags |
|-------|------|---------|------|
| `ghcr.io/waghelapritesh/tac-ci-builder` | `node:24-bookworm` | CI build environment with Rust toolchain | `:latest`, `:<date>` |
| `ghcr.io/waghelapritesh/tac-2` | `node:24-slim` | User-facing runtime | `:latest`, `:next`, `:v<version>` |
...
TAC_FIXTURE_MODE=record TAC_FIXTURE_DIR=./tests/fixtures/recordings \
