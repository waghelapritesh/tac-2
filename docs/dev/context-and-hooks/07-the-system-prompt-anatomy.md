...
If `.tac/SYSTEM.md` (project) or `~/.tac/agent/SYSTEM.md` (global) exists, its contents **completely replace** the default base prompt above. The tools list, guidelines, pi docs pointers — all gone. You own the entire base.
...
If `.tac/APPEND_SYSTEM.md` (project) or `~/.tac/agent/APPEND_SYSTEM.md` (global) exists, its contents are appended after the base prompt.
