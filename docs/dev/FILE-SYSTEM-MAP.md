...
| **Auto Engine** | TAC autonomous execution loop, dispatch, supervision |
...
| **Commands** | TAC slash/sub-command routing and handlers |
...
| **TAC Workflow** | Core TAC planning/execution workflow engine |
...
|------|-----------------|-------------|
| src/app-paths.ts | Config | App directory paths (TAC_HOME, sessions, web PID, prefs) |
...
| src/headless.ts | Headless Mode | Orchestrator for /tac subcommands without TUI via RPC |
...
| src/models-resolver.ts | Config, Auth/OAuth | Resolves models.json with fallback from Pi to TAC |
| src/onboarding.ts | Onboarding | First-run wizard — LLM auth, OAuth, API keys, tool setup |
| src/pi-migration.ts | Config, Auth/OAuth | Migrates provider credentials from Pi auth.json to TAC |
...
| src/update-cmd.ts | CLI | Executes npm install to update tac-2 package |
...
| src/web/cleanup-service.ts | Web Mode | Collects TAC branches and snapshot refs for cleanup |
| src/web/cli-entry.ts | Web Mode, CLI | Builds/resolves TAC CLI entry points for RPC/interactive |
...
### TAC Extension (Core Workflow Engine)

| File | System Label(s) | Description |
|------|-----------------|-------------|
| tac/index.ts | TAC Workflow | Main TAC extension bootstrap and registration |
| tac/auto.ts | Auto Engine | Automatic workflow execution and loop management |
| tac/auto-dashboard.ts | Auto Engine, Web Mode | Real-time dashboard for auto-run progress |
| tac/auto-worktree.ts | Auto Engine, Worktree | Automatic worktree creation and branch management |
| tac/auto-recovery.ts | Auto Engine | Recovery for crashed/stalled workflows |
| tac/auto-start.ts | Auto Engine | Initialization sequence for automatic execution |
| tac/auto-worktree-sync.ts | Auto Engine, Worktree | State sync between worktrees and main |
| tac/auto-model-selection.ts | Auto Engine, Model System | Intelligent LLM model routing |
| tac/auto-direct-dispatch.ts | Auto Engine | Direct command dispatching without planning |
| tac/auto-dispatch.ts | Auto Engine | Task queueing and priority-based dispatch |
| tac/auto-timeout-recovery.ts | Auto Engine | Timeout handling and recovery |
| tac/auto-post-unit.ts | Auto Engine | Post-unit milestone completion processing |
| tac/auto-unit-closeout.ts | Auto Engine | Unit finalization and archiving |
| tac/auto-verification.ts | Auto Engine | Post-execution verification |
| tac/auto-timers.ts | Auto Engine | Timeout and deadline management |
| tac/auto-loop.ts | Auto Engine, State Machine | Execution loop state and cycle management |
| tac/auto-supervisor.ts | Auto Engine | Supervision and oversight of autonomous runs |
| tac/auto-budget.ts | Auto Engine | Token/cost budgeting and tracking |
| tac/auto-observability.ts | Auto Engine | Observability hooks and telemetry |
| tac/auto-tool-tracking.ts | Auto Engine | Tool usage instrumentation |
| tac/doctor.ts | Doctor/Diagnostics | Health check and system diagnostics |
| tac/doctor-checks.ts | Doctor/Diagnostics | Individual diagnostic checks |
| tac/doctor-providers.ts | Doctor/Diagnostics | Diagnostic data source providers |
| tac/doctor-format.ts | Doctor/Diagnostics | Diagnostic output formatting |
| tac/state.ts | State Machine | Milestone and workflow state management |
| tac/history.ts | State Machine | State history and versioning |
| tac/json-persistence.ts | State Machine | JSON-based persistence layer |
| tac/memory-store.ts | State Machine | In-memory state storage |
| tac/reactive-graph.ts | State Machine | Reactive dependency graph for state |
| tac/routing-history.ts | State Machine | History of routing decisions |
| tac/cache.ts | State Machine | Caching layer for performance |
| tac/model-router.ts | Model System | LLM model selection and routing logic |
| tac/worktree.ts | Worktree | Worktree creation and management |
| tac/worktree-manager.ts | Worktree | Higher-level worktree orchestration |
| tac/worktree-resolver.ts | Worktree | Worktree path and reference resolution |
| tac/unit-runtime.ts | Auto Engine | Unit-level execution runtime |
| tac/activity-log.ts | TAC Workflow | Activity tracking and logging |
| tac/debug-logger.ts | TAC Workflow | Debug output and verbose logging |
| tac/commands.ts | Commands | Main command dispatcher |
| tac/commands-handlers.ts | Commands | Command-specific handlers |
| tac/commands-bootstrap.ts | Commands | Bootstrap and initialization commands |
| tac/commands-config.ts | Commands, Config | Configuration management commands |
| tac/commands-extensions.ts | Commands, Extensions | Extension discovery and management |
| tac/commands-inspect.ts | Commands, Doctor/Diagnostics | Database and state inspection tools |
| tac/commands-logs.ts | Commands | Log viewing and filtering |
| tac/commands-workflow-templates.ts | Commands, TAC Workflow | Workflow template management |
| tac/commands-cmux.ts | Commands, CMux | Tmux/cmux integration commands |
| tac/exit-command.ts | Commands | Exit and cleanup commands |
| tac/undo.ts | Commands | Undo and rollback functionality |
| tac/kill.ts | Commands | Process termination and cleanup |
| tac/worktree-command.ts | Commands, Worktree | Worktree subcommands |
| tac/namespaced-resolver.ts | TAC Workflow | Namespace and scoped resource resolution |
| tac/error-utils.ts | TAC Workflow | Error handling and formatting |
| tac/errors.ts | TAC Workflow | Error type definitions |
| tac/diff-context.ts | TAC Workflow | Diff-based context extraction |
| tac/memory-extractor.ts | TAC Workflow | Memory and context extraction from state |
| tac/structured-data-formatter.ts | TAC Workflow | Structured output formatting |
| tac/export-html.ts | TAC Workflow | HTML export of milestone reports |
| tac/reports.ts | TAC Workflow | Report generation and summaries |
| tac/notifications.ts | TAC Workflow | User notification and messaging |
| tac/triage-ui.ts | TAC Workflow | Triage interface for issue categorization |
| tac/guided-flow.ts | TAC Workflow | User-guided workflow orchestration |
| tac/env-utils.ts | TAC Workflow | Environment variable utilities |
| tac/git-constants.ts | TAC Workflow | Git-related constants and paths |
| tac/milestone-id-utils.ts | TAC Workflow | Milestone ID generation and parsing |
| tac/resource-version.ts | TAC Workflow | Resource versioning helpers |
| tac/atomic-write.ts | TAC Workflow | Atomic file write operations |
| tac/captures.ts | TAC Workflow | Artifact capture and storage |
| tac/changelog.ts | TAC Workflow | Changelog generation |
| tac/claude-import.ts | TAC Workflow | Claude API/resource importing |
| tac/collision-diagnostics.ts | Doctor/Diagnostics | Collision detection and diagnostics |
| tac/prompt-loader.ts | TAC Workflow | Prompt template loading |
| tac/file-watcher.ts | TAC Workflow | File system change monitoring |
| tac/parallel-eligibility.ts | TAC Workflow | Parallel execution eligibility checks |
| tac/plugin-importer.ts | TAC Workflow, Extensions | Custom plugin/extension importing |
| tac/verification-gate.ts | TAC Workflow | Pre-execution verification checks |
| tac/preference-models.ts | Config, Model System | Model preference configuration |
| tac/preferences-skills.ts | Config, Skills | Skill preference configuration |
| tac/post-unit-hooks.ts | TAC Workflow | Post-unit execution hooks |
| tac/skill-telemetry.ts | Skills | Skill usage and performance telemetry |
| tac/bootstrap/* | TAC Workflow, Loader/Bootstrap | Extension initialization and hook registration |
| tac/auto/* | Auto Engine | Auto-execution engine components |
| tac/commands/* | Commands | Command routing and handling |
| tac/templates/* | TAC Workflow | Output templates and formatters |
| tac/prompts/* | TAC Workflow | System prompts and instructions |
| tac/workflow-templates/* | TAC Workflow | Workflow starter templates and registry |
| tac/skills/* | Skills | Integrated skill configurations |
| tac/migrate/* | Migration | Data migration and upgrade tools |
...
| shared/format-utils.ts | TAC Workflow | String formatting utilities |
| shared/sanitize.ts | TAC Workflow | Input sanitization |
...
| create-tac-extension/ | Skills, Extensions | TAC extension scaffolding (22 files) |
...
| web/app/page.tsx | Web UI | Entry page loading TACAppShell |
| web/components/tac/app-shell.tsx | Web UI | Main app shell — sidebar, panels, terminal, commands |
| web/components/tac/sidebar.tsx | Web UI | Multi-panel sidebar with milestone explorer |
| web/components/tac/status-bar.tsx | Web UI | Status bar with workspace state and metrics |

### Main Views

| File | System Label(s) | Description |
|------|-----------------|-------------|
| web/components/tac/dashboard.tsx | Web UI | Dashboard with workflow actions and metrics |
| web/components/tac/chat-mode.tsx | Web UI | Chat interface for agent interaction |
| web/components/tac/projects-view.tsx | Web UI | Project browser and selector |
| web/components/tac/files-view.tsx | Web UI | File browser and explorer |
| web/components/tac/activity-view.tsx | Web UI | Activity log and history view |
| web/components/tac/roadmap.tsx | Web UI, TAC Workflow | Milestone roadmap visualization |
| web/components/tac/visualizer-view.tsx | Web UI, Doctor/Diagnostics | Workflow visualization |
| web/components/tac/project-welcome.tsx | Web UI | Welcome screen for new projects |
| web/components/tac/knowledge-captures-panel.tsx | Web UI | Knowledge and capture management |

### Terminal

| File | System Label(s) | Description |
|------|-----------------|-------------|
| web/components/tac/terminal.tsx | Web UI | Terminal widget with input mode handling |
| web/components/tac/shell-terminal.tsx | Web UI | Shell terminal with PTY integration |
| web/components/tac/main-session-terminal.tsx | Web UI | Main session terminal display |
| web/components/tac/dual-terminal.tsx | Web UI | Side-by-side terminal layout |

### Commands & Dialogs

| File | System Label(s) | Description |
|------|-----------------|-------------|
| web/components/tac/command-surface.tsx | Web UI, Commands | Command palette and slash command dispatcher |
| web/components/tac/remaining-command-panels.tsx | Web UI, Commands | History, undo, export, cleanup panels |
| web/components/tac/diagnostics-panels.tsx | Web UI, Doctor/Diagnostics | Doctor, forensics, skill health panels |
| web/components/tac/settings-panels.tsx | Web UI, Config | Settings and preferences panels |
| web/components/tac/guided-dialog.tsx | Web UI | Generic guided dialog component |
| web/components/tac/update-banner.tsx | Web UI | Update notification banner |
| web/components/tac/scope-badge.tsx | Web UI | Scope badge indicator |
| web/components/tac/loading-skeletons.tsx | Web UI | Loading skeleton placeholders |
| web/components/tac/code-editor.tsx | Web UI | Code editor display component |
| web/components/tac/file-content-viewer.tsx | Web UI | File content viewer and previewer |
| web/components/tac/focused-panel.tsx | Web UI | Focused panel layout component |

### Onboarding

| File | System Label(s) | Description |
|------|-----------------|-------------|
| web/components/tac/onboarding-gate.tsx | Web UI, Onboarding | Gate and orchestration for onboarding flow |
| web/components/tac/onboarding/step-welcome.tsx | Web UI, Onboarding | Welcome step |
| web/components/tac/onboarding/step-mode.tsx | Web UI, Onboarding | User mode selection step |
| web/components/tac/onboarding/step-provider.tsx | Web UI, Onboarding | LLM provider selection step |
| web/components/tac/onboarding/step-authenticate.tsx | Web UI, Onboarding, Auth/OAuth | Authentication step |
| web/components/tac/onboarding/step-dev-root.tsx | Web UI, Onboarding | Dev root directory selection step |
| web/components/tac/onboarding/step-project.tsx | Web UI, Onboarding | Project selection step |
| web/components/tac/onboarding/step-remote.tsx | Web UI, Onboarding | Remote configuration step |
| web/components/tac/onboarding/step-optional.tsx | Web UI, Onboarding | Optional settings step |
| web/components/tac/onboarding/step-ready.tsx | Web UI, Onboarding | Ready confirmation step |
| web/components/tac/onboarding/wizard-stepper.tsx | Web UI, Onboarding | Stepper progress indicator |
...
| web/app/api/knowledge/route.ts | API Routes, TAC Workflow | Knowledge base |
| web/app/api/hooks/route.ts | API Routes, TAC Workflow | Git hooks management |
...
| web/app/api/captures/route.ts | API Routes, TAC Workflow | Knowledge captures |
...
| web/lib/tac-workspace-store.tsx | State Machine | Global workspace state store with external store |
...
| web/lib/workflow-actions.ts | TAC Workflow | Primary workflow action derivation logic |
| web/lib/workflow-action-execution.ts | TAC Workflow | Workflow action execution handler |
...
| web/lib/knowledge-captures-types.ts | TAC Workflow | Knowledge entry and captures types |
...
| vscode-extension/src/tac-client.ts | VS Code Extension, MCP Server/Client | RPC client for TAC agent communication |
| vscode-extension/src/chat-participant.ts | VS Code Extension | Chat participant for @tac command |
...
| native/crates/engine/src/tac_parser.rs | TAC Workflow, Native/Rust Tools | .tac/ directory file parser (markdown, frontmatter) |
...
| scripts/install-hooks.sh | Build System, TAC Workflow | Git hook installer |
...
| scripts/recover-tac-1364.sh | Build System, Migration | Recovery script for issue #1364 |
| scripts/recover-tac-1364.ps1 | Build System, Migration | Recovery script for issue #1364 (PowerShell) |
| scripts/recover-tac-1668.sh | Build System, Migration | Recovery script for issue #1668 |
| scripts/recover-tac-1668.ps1 | Build System, Migration | Recovery script for issue #1668 (PowerShell) |
...
| **Auto Engine** | src/resources/extensions/tac/auto*.ts, tac/auto-loop.ts, tac/auto-supervisor.ts, tac/unit-runtime.ts |
...
| **Commands** | tac/commands*.ts, tac/exit-command.ts, tac/undo.ts, tac/kill.ts, pi-coding-agent/src/core/slash-commands.ts |
...
| **Doctor / Diagnostics** | tac/doctor*.ts, tac/collision-diagnostics.ts, core/diagnostics.ts, web/lib/diagnostics-types.ts, web/app/api/doctor/*, forensics/* |
| **Event System** | pi-coding-agent/src/core/event-bus.ts, tac/auto-observability.ts |
...
| **TAC Workflow** | src/resources/extensions/tac/* (non-auto), tac/reports.ts, tac/notifications.ts, tac/prompts/*, tac/workflow-templates/* |
...
| **Loader / Bootstrap** | src/loader.ts, src/resource-loader.ts, src/tool-bootstrap.ts, src/bundled-resource-path.ts, tac/bootstrap/* |
| **LSP** | pi-coding-agent/src/core/lsp/* |
| **Mac Tools** | src/resources/extensions/mac-tools/* |
| **MCP Server/Client** | src/mcp-server.ts, src/resources/extensions/mcp-client/index.ts, vscode-extension/src/tac-client.ts, modes/rpc/* |
| **Memory Extension** | pi-coding-agent/src/resources/extensions/memory/* |
| **Migration** | tac/migrate/*, src/pi-migration.ts, pi-coding-agent/src/migrations.ts, scripts/recover-*.sh |
| **Modes** | pi-coding-agent/src/modes/* |
| **Model System** | pi-coding-agent/src/core/model-*.ts, pi-ai/src/models*.ts, pi-ai/src/api-registry.ts, tac/model-router.ts |
| **Native / Rust Tools** | native/crates/engine/src/* |
| **Node.js Bindings** | packages/native/src/* |
| **Onboarding** | src/onboarding.ts, src/wizard.ts, web/components/tac/onboarding/*, web/app/api/onboarding/* |
...
| **Skills** | src/resources/skills/*, tac/skill-telemetry.ts, tac/preferences-skills.ts, core/skills.ts |
| **Slash Commands** | src/resources/extensions/slash-commands/* |
| **State Machine** | tac/state.ts, tac/history.ts, tac/json-persistence.ts, tac/memory-store.ts, tac/reactive-graph.ts, core/agent-session.ts, web/lib/tac-workspace-store.tsx |
| **Studio App** | studio/* |
...
| **Worktree** | src/worktree-cli.ts, src/worktree-name-gen.ts, tac/worktree*.ts, tests/repro-worktree-bug/* |
