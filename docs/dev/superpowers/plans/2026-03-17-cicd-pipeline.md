...
| `tests/smoke/test-version.ts` | Verify `tac --version` outputs valid semver |
| `tests/smoke/test-help.ts` | Verify `tac --help` exits 0 and contains expected output |
| `tests/smoke/test-init.ts` | Verify `tac init` creates expected files in a temp dir |
...
| `tests/fixtures/record.ts` | Recording helper — runs a session with `TAC_FIXTURE_MODE=record` |
...
# ──────────────────────────────────────────────
# Stage 1: CI Builder
# Image: ghcr.io/waghelapritesh/tac-ci-builder
...
Run: `docker build --target builder -t tac-ci-builder-test .`
Expected: Completes successfully (may take 5-10 min first time)

- [ ] **Step 3: Verify runtime stage builds**

Run: `docker build --target runtime -t tac-2-test .`
Expected: Completes successfully

- [ ] **Step 4: Verify runtime image works**

Run: `docker run --rm tac-2-test --version`
...
// Verifies that `tac --version` outputs valid semver-like string.
// When TAC_SMOKE_BINARY is set (CI), uses that binary directly.
// Otherwise falls back to npx tac-2.

import { execFileSync } from "child_process";

const bin = process.env.TAC_SMOKE_BINARY;
const output = bin
  ? execFileSync(bin, ["--version"], { encoding: "utf8", timeout: 30_000 }).trim()
  : execFileSync("npx", ["tac-2", "--version"], { encoding: "utf8", timeout: 30_000 }).trim();
...
// Verifies that `tac --help` exits 0 and contains expected keywords.
...
const output = bin
  ...
  : execFileSync("npx", ["tac-2", "--help"], { encoding: "utf8", timeout: 30_000 });

const requiredKeywords = ["tac", "usage"];
...
// Verifies that `tac init` creates expected files in a temp directory.
...
const tmp = mkdtempSync(join(tmpdir(), "tac-smoke-init-"));

try {
  const bin = process.env.TAC_SMOKE_BINARY;
  const args = bin ? [bin, "init"] : ["npx", "tac-2", "init"];
  execFileSync(args[0], args.slice(1), {
    encoding: "utf8",
    cwd: tmp,
    timeout: 30_000,
    env: { ...process.env, TAC_NON_INTERACTIVE: "1" },
  });

  // Check that .tac directory was created
  if (!existsSync(join(tmp, ".tac"))) {
    console.error("Expected .tac/ directory not found after init");
    process.exit(1);
  }
...
//   TAC_FIXTURE_MODE=record|replay
//   TAC_FIXTURE_DIR=./tests/fixtures/recordings
...
export function getFixtureMode(): FixtureMode {
  const mode = process.env.TAC_FIXTURE_MODE;
  if (mode === "record" || mode === "replay") return mode;
  return "off";
}

export function getFixtureDir(): string {
  return process.env.TAC_FIXTURE_DIR || join(process.cwd(), "tests/fixtures/recordings");
}
...
Note: This provider implements the core recording/replay data structures and utilities. Wiring it into the `pi-ai` registry as a drop-in `ApiProvider` (via `registerApiProvider()` from `packages/pi-ai/src/api-registry.ts`) requires importing `@tac/pi-ai` internals, which couples tests to the build output. This integration is deferred to a follow-up task after the pipeline is operational. The current implementation validates fixture format, turn sequencing, and replay correctness independently.
...
```typescript
// tests/live/run.ts
// Runs real LLM integration tests. Only executes when TAC_LIVE_TESTS=1.
// These tests cost real money — used in the Prod gate only.
//
// Usage: TAC_LIVE_TESTS=1 node --experimental-strip-types tests/live/run.ts

if (process.env.TAC_LIVE_TESTS !== "1") {
  console.log("Skipping live tests (set TAC_LIVE_TESTS=1 to enable)");
  process.exit(0);
}
...
Add to `package.json` `scripts`:
```json
"test:fixtures:record": "TAC_FIXTURE_MODE=record node --experimental-strip-types tests/fixtures/record.ts",
"test:live": "TAC_LIVE_TESTS=1 node --experimental-strip-types tests/live/run.ts",
"pipeline:version-stamp": "node scripts/version-stamp.mjs",
"docker:build-runtime": "docker build --target runtime -t ghcr.io/waghelapritesh/tac-2 .",
"docker:build-builder": "docker build --target builder -t ghcr.io/waghelapritesh/tac-ci-builder ."
```

- [ ] **Step 5: Verify live tests skip without env var**

Run: `npm run test:live`
Expected: `Skipping live tests (set TAC_LIVE_TESTS=1 to enable)` and exit 0
...
jobs:
  ...
  dev-publish:
    ...
    container:
      image: ghcr.io/waghelapritesh/tac-ci-builder:latest  # Pin to date tag after first build
    environment: dev
    outputs:
  ...
  test-verify:
    ...
    steps:
      ...
      - name: Run CLI smoke tests
        run: npm run test:smoke
        env:
          TAC_SMOKE_BINARY: tac  # Use globally installed binary, not npx

      - name: Run fixture replay tests
        run: npm run test:fixtures

      - name: Promote to @next
        run: npm dist-tag add tac-2@${{ needs.dev-publish.outputs.dev-version }} next
        env:
          NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}
      ...
      - name: Build and push runtime Docker image
        run: |
          ...
          docker build --target runtime \
            --build-arg TAC_VERSION=${{ needs.dev-publish.outputs.dev-version }} \
            -t ghcr.io/waghelapritesh/tac-2:next \
            -t ghcr.io/waghelapritesh/tac-2:${{ needs.dev-publish.outputs.dev-version }} \
            .
          docker push ghcr.io/waghelapritesh/tac-2:next
          docker push ghcr.io/waghelapritesh/tac-2:${{ needs.dev-publish.outputs.dev-version }}
    ...
    steps:
      ...
      - name: Run live LLM tests (optional)
        ...
        run: |
          npm ci
          npm run build
          TAC_LIVE_TESTS=1 npm run test:live
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}

      - name: Promote to @latest
        run: npm dist-tag add tac-2@${{ needs.dev-publish.outputs.dev-version }} latest
        env:
          NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}
      ...
      - name: Tag and push Docker images
        run: |
          echo "${{ secrets.GITHUB_TOKEN }}" | docker login ghcr.io -u ${{ github.actor }} --password-stdin
          docker pull ghcr.io/waghelapritesh/tac-2:${{ needs.dev-publish.outputs.dev-version }}
          docker tag ghcr.io/waghelapritesh/tac-2:${{ needs.dev-publish.outputs.dev-version }} ghcr.io/waghelapritesh/tac-2:latest
          docker push ghcr.io/waghelapritesh/tac-2:latest
      ...
      - name: Build and push CI builder image
        run: |
          ...
          docker build --target builder \
            -t ghcr.io/waghelapritesh/tac-ci-builder:latest \
            -t ghcr.io/waghelapritesh/tac-ci-builder:${{ steps.tag.outputs.date }} \
            .
          docker push ghcr.io/waghelapritesh/tac-ci-builder:latest
          docker push ghcr.io/waghelapritesh/tac-ci-builder:${{ steps.tag.outputs.date }}

      - name: Verify builder image
        run: |
          docker run --rm ghcr.io/waghelapritesh/tac-ci-builder:latest node --version
          docker run --rm ghcr.io/waghelapritesh/tac-ci-builder:latest rustc --version
```
  ...
  cleanup:
    ...
    steps:
      ...
      - name: Remove old dev versions
        run: |
          VERSIONS=$(npm view tac-2 versions --json 2>/dev/null || echo "[]")
          ...
          for VERSION in $DEV_VERSIONS; do
            PUBLISH_TIME=$(npm view "tac-2@$VERSION" time --json 2>/dev/null || echo "")
            ...
            if [ -n "$PUBLISH_TIME" ]; then
              ...
              if [ "$AGE_MS" -gt "$THIRTY_DAYS_MS" ]; then
                echo "Unpublishing tac-2@$VERSION"
                npm unpublish "tac-2@$VERSION" || echo "Failed to unpublish $VERSION"
              else
                echo "Keeping tac-2@$VERSION (within 30 days)"
              fi
            fi
          done
        env:
...
if (mode !== "record") {
  console.error("Recording requires TAC_FIXTURE_MODE=record");
  console.error("");
  console.error("Usage:");
  console.error("  TAC_FIXTURE_MODE=record TAC_FIXTURE_DIR=./tests/fixtures/recordings \\");
...
console.log("1. Set TAC_FIXTURE_MODE=record in your environment");
console.log("2. Run your TAC session normally");
...
- Live tests: Skipped (no `TAC_LIVE_TESTS=1`)
...
4. **Enable GHCR:**
   - Ensure GitHub Container Registry is enabled for the `waghelapritesh` org
