# CI/CD Pipeline Design — TAC 2

## Overview

A three-stage promotion pipeline for TAC 2 that moves merged PRs through Dev → Test → Prod using npm dist-tags as environment markers, GitHub Environments for approval gates, and Docker images for both CI acceleration and end-user distribution.

## Goals

1. Every merged PR is immediately installable via `npx tac-2@dev`
...
5. End users can run TAC via Docker as an alternative to npm
...
│  2. npm publish tac-2@<version>-dev.<sha> --tag dev         │
│  3. Smoke test: npx tac-2@dev --version                    │
│                                                              │
...
│                                                              │
│  1. Install tac-2@dev from registry                         │
...
│  4. npm dist-tag add tac-2@<version> next                   │
│                                                              │
...
│  2. npm dist-tag add tac-2@<version> latest                 │
...
Dev versions (`@dev` tag) use the native binaries from the most recent stable `build-native.yml` release. The `optionalDependencies` in `package.json` use `>=` ranges, so a `-dev.` version of `tac-2` resolves the latest stable `@waghelapritesh/engine-*` packages from the registry.
...
|---------|--------|----------|
| Dev publish succeeds, smoke test fails | Broken version on `@dev` tag | Next successful merge overwrites `@dev`. Manual fix: `npm dist-tag add tac-2@<last-good> dev` |
| Test stage fails after promoting to `@next` | Broken version on `@next` tag | Manual: `npm dist-tag add tac-2@<last-good> next`. `@latest` is never affected. |
| Prod promotion publishes `@latest` then found broken | Broken production release | Manual: `npm dist-tag add tac-2@<previous-stable> latest` and `docker tag ghcr.io/waghelapritesh/tac-2:<previous> latest && docker push`. Post-mortem required. |
...
- **Name:** `ghcr.io/waghelapritesh/tac-ci-builder`
...
- **Name:** `ghcr.io/waghelapritesh/tac-2`
- **Base:** `node:22-slim`
- **Contains:** Node 22, git, `tac-2` installed globally
- **Size:** ~250 MB
- **Tags:** `:latest`, `:next`, `:v2.27.0`
- **Published:** On every Prod promotion
- **Purpose:** `docker run ghcr.io/waghelapritesh/tac-2` as alternative to `npx`
...
The `FixtureProvider` implements the `Provider` interface from `@tac/pi-ai` (the same interface all 20+ built-in providers implement). It registers itself via environment variable detection at provider initialization:

```typescript
// Pseudocode — actual implementation will follow pi-ai patterns
import type { Provider, StreamingResponse } from "@tac/pi-ai";
...
- **Registration:** When `TAC_FIXTURE_MODE` is set, the fixture provider wraps the configured real provider. No changes to provider selection logic needed.
...
|------|---------|----------|
| **Record** | `TAC_FIXTURE_MODE=record TAC_FIXTURE_DIR=./fixtures` | Wraps real provider, saves request/response pairs |
| **Replay** | `TAC_FIXTURE_MODE=replay TAC_FIXTURE_DIR=./fixtures` | Returns saved responses, zero API calls |
...
Old `-dev.` versions accumulate on npm with every merged PR. A scheduled workflow (`cleanup-dev-versions.yml`) runs weekly and unpublishes dev versions older than 30 days via `npm unpublish tac-2@<old-dev-version>`. This prevents registry bloat while keeping recent dev versions available.
...
1. A merged PR is installable via `npx tac-2@dev` within 15 minutes (assumes warm CI builder image cache)
...
5. `docker run ghcr.io/waghelapritesh/tac-2 --version` returns the correct version
