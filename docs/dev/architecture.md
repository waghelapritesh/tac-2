...
TAC is a TypeScript application built on the [Pi SDK](https://github.com/badlogic/pi-mono). It embeds the Pi coding agent and extends it with the TAC workflow engine, auto mode state machine, and project management primitives.

## System Structure

```
tac (CLI binary)
  └─ loader.ts          Sets PI_PACKAGE_DIR, TAC env vars, dynamic-imports cli.ts
      └─ cli.ts         Wires SDK managers, loads extensions, starts InteractiveMode
          ├─ onboarding.ts   First-run setup wizard (LLM provider + tool keys)
          ├─ wizard.ts       Env hydration from stored auth.json credentials
          ├─ app-paths.ts    ~/.tac/agent/, ~/.tac/sessions/, auth.json
          ├─ resource-loader.ts  Syncs bundled extensions + agents to ~/.tac/agent/
          └─ src/resources/
              ├─ extensions/tac/    Core TAC extension
              ├─ extensions/...     23 supporting extensions
              ├─ agents/            scout, researcher, worker
              ├─ AGENTS.md          Agent routing instructions
              └─ TAC-WORKFLOW.md    Manual bootstrap protocol

tac headless              Headless mode — CI/cron orchestration via RPC child process
tac --mode mcp            MCP server mode — exposes tools over stdin/stdout

vscode-extension/         VS Code extension — chat participant (@tac), sidebar dashboard, RPC integration
```
...
`.tac/` is the sole source of truth. Auto mode reads it, writes it, and advances based on what it finds. No in-memory state survives across sessions. This enables crash recovery, multi-terminal steering, and session resumption.
...
`PI_PACKAGE_DIR` points to `pkg/` (not project root) to avoid Pi's theme resolution colliding with TAC's `src/` directory. Contains only `piConfig` and theme assets.

### Always-Overwrite Sync

Bundled extensions and agents are synced to `~/.tac/agent/` on every launch, not just first run. This means `npm update -g` takes effect immediately.
...
|-----------|-----------------|
| **TAC** | Core workflow engine — auto mode, state machine, commands, dashboard |
...
- **parser** — TAC file parsing and frontmatter extraction
