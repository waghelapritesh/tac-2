...
**Related ADR:** [ADR-009-orchestration-kernel-refactor.md](/Users/jeremymcspadden/Github/tac-2/docs/dev/ADR-009-orchestration-kernel-refactor.md)  
...
Implement ADR-009 by migrating TAC orchestration internals to a Unified Orchestration Kernel (UOK) with six control planes:
...
- `src/resources/extensions/tac/auto.ts`
- `src/resources/extensions/tac/auto/loop.ts`
- `src/resources/extensions/tac/auto/types.ts`
- `src/resources/extensions/tac/auto/session.ts`
...
- `src/resources/extensions/tac/verification-gate.ts`
- `src/resources/extensions/tac/auto-verification.ts`
- `src/resources/extensions/tac/pre-execution-checks.ts`
- `src/resources/extensions/tac/post-execution-checks.ts`
- `src/resources/extensions/tac/milestone-validation-gates.ts`
...
- `src/resources/extensions/tac/model-router.ts`
- `src/resources/extensions/tac/auto-model-selection.ts`
- `src/resources/extensions/tac/preferences-models.ts`
- `src/resources/extensions/tac/model-cost-table.ts`
- `src/resources/extensions/tac/custom-execution-policy.ts`
...
- `src/resources/extensions/tac/reactive-graph.ts`
- `src/resources/extensions/tac/slice-parallel-orchestrator.ts`
- `src/resources/extensions/tac/parallel-orchestrator.ts`
- `src/resources/extensions/tac/graph.ts`
- `src/resources/extensions/tac/unit-runtime.ts`
...
- `src/resources/extensions/tac/git-service.ts`
- `src/resources/extensions/tac/auto-post-unit.ts`
- `src/resources/extensions/tac/auto-unit-closeout.ts`
- `src/resources/extensions/tac/auto-worktree.ts`
...
- `src/resources/extensions/tac/journal.ts`
- `src/resources/extensions/tac/activity-log.ts`
- `src/resources/extensions/tac/metrics.ts`
- `src/resources/extensions/tac/workflow-logger.ts`
- `src/resources/extensions/tac/tac-db.ts`
...
- `src/resources/extensions/tac/guided-flow.ts`
- `src/resources/extensions/tac/preparation.ts`
- `src/resources/extensions/tac/auto/phases.ts`
- `src/resources/extensions/tac/auto-prompts.ts`
- prompt templates under `src/resources/extensions/tac/prompts/`
...
- targeted tests in `src/resources/extensions/tac/tests/*auto*`
- `npm run test:unit`
...
- integration: `src/resources/extensions/tac/tests/integration/*.test.ts`
...
- audit projection tables in `tac.db`
...
| --- | --- | --- |
| 1. UOK path is default and stable. | Complete | `resolveUokFlags()` defaults UOK planes to enabled; kernel path dispatch by default in `runAutoLoopWithUok()` and `startAuto()`. (`src/resources/extensions/tac/uok/flags.ts`, `src/resources/extensions/tac/uok/kernel.ts`, `src/resources/extensions/tac/auto.ts`) |
...
| 3. Model selection supports any eligible model in any phase with policy enforcement. | Complete | ADR-009 model plane behavior is active in runtime selection/policy integration. (`src/resources/extensions/tac/auto-model-selection.ts`, `src/resources/extensions/tac/custom-execution-policy.ts`) |
| 4. Hooks/agents/subagents/parallel/team execution runs through one scheduler contract. | Complete | Dispatch kind mapping + scheduler facade for `unit|hook|subagent|team-worker|verification|reprocess` in `runUnitPhaseViaContract()`. (`src/resources/extensions/tac/auto/loop.ts`) |
| 5. Turn-level git transaction record exists for every executed turn. | Complete | Turn observer emits git transaction stages + closeout records via UOK gitops adapter. (`src/resources/extensions/tac/uok/loop-adapter.ts`, `src/resources/extensions/tac/uok/gitops.ts`) |
| 6. Unified audit events provide causal traceability across orchestration, model, tool, git, and test actions. | Complete | Unified audit envelope + emission path wired through UOK observer/kernel entry. (`src/resources/extensions/tac/uok/audit.ts`, `src/resources/extensions/tac/uok/kernel.ts`) |
| 7. Plan v2 can produce a complete unit graph with fail-closed plan gate. | Complete | Plan v2 control plane remains default-on with bounded flow + gate coverage in current planner pipeline. (`src/resources/extensions/tac/uok/flags.ts`, planning/gate suites) |
| 8. `burn-max` profile is available and policy-safe. | Complete | Profile and policy bounds are enforced via preferences + model policy filtering. (`src/resources/extensions/tac/auto-model-selection.ts`, `src/resources/extensions/tac/preferences-models.ts`) |
| 9. Legacy orchestration branches are retired or behind emergency-only fallback. | Complete | Legacy path remains only under explicit emergency controls (`uok.legacy_fallback.enabled`, `TAC_UOK_FORCE_LEGACY`, `TAC_UOK_LEGACY_FALLBACK`). (`src/resources/extensions/tac/uok/flags.ts`) |
| 10. CLI/web/headless behavior remains user-compatible. | Complete | Existing startup/dispatch surfaces preserved; integration coverage remains green in CI. (`src/resources/extensions/tac/auto.ts`, `src/tests/integration/*.test.ts`) |

Targeted closure/parity tests:

- `src/resources/extensions/tac/tests/uok-flags.test.ts`
- `src/resources/extensions/tac/tests/uok-execution-graph.test.ts`
- `src/resources/extensions/tac/tests/uok-kernel-path.test.ts`
