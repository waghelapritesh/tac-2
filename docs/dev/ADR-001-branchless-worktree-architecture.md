...
TAC uses git for isolation during autonomous coding sessions. The current architecture (shipped in M003, v2.13.0) creates a **worktree per milestone** with **slice branches inside each worktree**. Each slice (`S01`, `S02`, ...) gets its own branch (`tac/M001/S01`) within the worktree, which merges back to the milestone branch (`milestone/M001`) via `--no-ff` when the slice completes. The milestone branch squash-merges to `main` when the milestone completes.

This architecture replaced a previous "branch-per-slice" model that had severe `.tac/` merge conflicts. M003 solved the merge conflicts but retained slice branches inside worktrees, inheriting complexity that has produced persistent, user-facing failures.
...
**2. `.tac/` state clobbering across branches**

`.tac/` is gitignored (line 52 of `.gitignore`: `.tac/`). Planning artifacts (roadmaps, plans, summaries, decisions, requirements) live in `.tac/milestones/` but are invisible to git. When multiple branches or worktrees operate from the same repo, they share a single `.tac/` directory on disk. Branch A's M001 roadmap overwrites Branch B's M001 roadmap. TAC reads corrupted state, shows wrong milestone as complete, or enters infinite dispatch loops.

The codebase has a contradictory workaround: `smartStage()` (git-service.ts:304-352) force-adds `TAC_DURABLE_PATHS` (milestones/, DECISIONS.md, PROJECT.md, REQUIREMENTS.md, QUEUE.md) despite the `.gitignore`. This means `.tac/milestones/` IS partially tracked on some branches but the gitignore claims otherwise. The code fights the configuration.
...
- Conflict categorization — 80 lines classifying `.tac/` vs runtime vs code conflicts
...
- 15+ separate bug fixes for `.tac/` merge conflicts in the pre-M003 era
...
Track `.tac/` planning artifacts in git. Gitignore only runtime/ephemeral state.
...
```
.tac/milestones/         — roadmaps, plans, summaries, research, contexts, task plans/summaries
.tac/PROJECT.md          — project overview
.tac/DECISIONS.md        — architectural decision register
.tac/REQUIREMENTS.md     — requirements register
.tac/QUEUE.md            — work queue
```

**Gitignored (ephemeral, runtime, infrastructure):**
```
.tac/runtime/            — dispatch records, timeout tracking
.tac/activity/           — JSONL session dumps
.tac/worktrees/          — git worktree working directories
.tac/auto.lock           — crash detection sentinel
.tac/metrics.json        — token/cost accumulator
.tac/completed-units.json — dispatch idempotency tracker
.tac/STATE.md            — derived state cache (rebuilt by deriveState())
.tac/tac.db              — SQLite cache (rebuilt from tracked markdown by importers)
.tac/DISCUSSION-MANIFEST.json — discussion phase tracking
.tac/milestones/**/*-CONTINUE.md — interrupted-work markers
.tac/milestones/**/continue.md   — legacy continue markers
```

### `.gitignore` Update

Replace the current blanket `.tac/` ignore with explicit runtime-only ignores:

```gitignore
# ── TAC: Runtime / Ephemeral ─────────────────────────────────
.tac/auto.lock
...
No conflict categorization. No runtime file stripping. No `.tac/` special handling. Planning artifacts merge cleanly because they're in `.tac/milestones/M001/` which doesn't exist on `main` until this merge.

### What `smartStage()` Becomes

The force-add of `TAC_DURABLE_PATHS` is no longer needed — planning artifacts are not gitignored, so `git add -A` picks them up naturally. The function reduces to:
...
Projects with existing `tac/M001/S01` slice branches can still be read by state derivation, but new work never creates slice branches.
...
If tracked markdown rolls back via `git reset --hard`, the gitignored `tac.db` doesn't. Mitigation: the importer layer (M001/S02) rebuilds the DB from markdown on startup. The DB is a cache, markdown is truth.
...
### B. Keep `.tac/` gitignored, bootstrap from git history for manual worktrees

When TAC detects an empty `.tac/` in a worktree, reconstruct state from the branch's git history using `git show <commit>:.tac/...`.
...
### C. Branch-scoped `.tac/` directories (`.tac/branches/<branch-name>/milestones/...`)

Each branch writes to a namespaced subdirectory within `.tac/`.
...
**Codebase analysis** confirmed `.tac/milestones/` is already partially tracked on `main` despite the `.gitignore`, that `TAC_DURABLE_PATHS` exists as a code-level acknowledgment that planning artifacts should be tracked, and that the README already documents the correct runtime-only gitignore pattern.
...
Response: Accepted in spirit. Commits with conventional tags (`feat(M001/S01):`, `feat(M001/S01/T01):`) serve as the slice boundary primitive. `git log --grep="M001/S01"` isolates a slice's history. `git revert` targets specific commits. Git tags (`tac/M001/S01-complete`) can mark slice completion if needed. The boundary primitive is commit metadata, not branches.

## Action Items

1. Close PR #487 (`resolveMainWorktreeRoot`) — contradicts this architecture
2. Implement as a TAC milestone with phases:
