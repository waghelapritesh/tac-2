...
TAC's auto-mode is unreliable. Users experience:
...
2. **State corruption across branches** — `.tac/` planning artifacts (roadmaps, plans, decisions) are gitignored but branch-specific. Multiple branches sharing a single `.tac/` directory clobber each other's state. Users see wrong milestones marked complete, wrong roadmaps loaded, and auto-mode starting from the wrong phase.
...
| Zero `.tac/` state corruption | Manual worktrees created via `git worktree add` have correct `.tac/` state without any TAC-specific initialization |
...
| Backwards compatibility | Existing projects with `tac/M001/S01` slice branches continue to work (read-only; new work uses new model) |
...
```
main
  └─ milestone/M001 (worktree at .tac/worktrees/M001/)
       ├─ tac/M001/S01 (slice branch — code + .tac/ artifacts)
...
### `.tac/` Tracking (Current — Contradictory)

- `.gitignore` line 52: `.tac/` — ignores everything
- `smartStage()` lines 338-349: force-adds `TAC_DURABLE_PATHS` — tracks milestones/, DECISIONS.md, PROJECT.md, REQUIREMENTS.md, QUEUE.md
- Result: `.tac/milestones/` is partially tracked on some branches, fully ignored on others. The code fights the config.
...
### `.tac/` Tracking (Proposed — Coherent)

**Tracked (travels with branch):**
```
.tac/milestones/**/*.md    (except CONTINUE markers)
.tac/milestones/**/*.json  (META.json integration records)
.tac/PROJECT.md
.tac/DECISIONS.md
.tac/REQUIREMENTS.md
.tac/QUEUE.md
...
| `.tac/` state clobbering | Artifacts tracked in git. Each branch carries its own `.tac/`. `git worktree add` and `git checkout` give correct state. |
| Merge conflict complexity | No merges within a worktree. Only merge is milestone→main (squash). |
| Manual worktree initialization | Tracked artifacts are checked out with the branch. No TAC-specific bootstrap needed. |
...
1. Update `.gitignore`:
   - Remove blanket `.tac/` ignore
   - Add explicit runtime-only ignores (see proposed list above)

2. Force-add existing planning artifacts on current branch:
   ```
   git add --force .tac/milestones/ .tac/PROJECT.md .tac/DECISIONS.md .tac/REQUIREMENTS.md .tac/QUEUE.md
   ```

3. Ensure runtime files are NOT tracked:
   ```
   git rm --cached -r .tac/runtime/ .tac/activity/ .tac/STATE.md .tac/metrics.json .tac/completed-units.json .tac/auto.lock
   ```

4. Update README suggested `.gitignore` section

5. Remove `smartStage()` force-add of `TAC_DURABLE_PATHS` — no longer needed since `.gitignore` doesn't block them

**Verification:** `git status` shows planning artifacts tracked, runtime files untracked. `git worktree add` on a new worktree has correct `.tac/milestones/` state.
...
No conflict categorization. No runtime file stripping (runtime files are gitignored, not in the merge). No `.tac/` special handling.

If squash-merge conflicts (parallel milestone edge case): stop auto-mode with clear error, user resolves manually or TAC dispatches a one-time resolution session.
...
2. Add new tests:
   ...
   - `.tac/` tracking: planning artifacts tracked, runtime files ignored
   - Manual worktree: `git worktree add` has correct `.tac/` state
...
1. State derivation (`deriveState()`) continues to read `tac/M001/S01` branch naming for legacy detection
2. On first run after upgrade:
   - Detect existing slice branches
   - Notify user: "TAC no longer creates slice branches. Existing branches are preserved but new work commits directly to the milestone branch."
...
**Verification:** Open a project with existing `tac/M001/S01` branches. TAC reads state correctly, new work commits on milestone branch without slice branches.
...
- Confirmed `.tac/milestones/` partially tracked on `main` despite `.gitignore`
...
| Need a replacement slice-boundary primitive | Accepted: conventional commit tags (`feat(M001/S01):`) + optional git tags (`tac/M001/S01-complete`) serve as boundaries. |
...
```
cd .tac/worktrees/M002
git fetch origin main
git rebase main
# Resolve any conflicts (code-only, never .tac/)
# Then squash-merge
```

This is standard git workflow. TAC can automate the rebase step as a pre-merge check.
...
| Branch types | 4 (main, milestone/*, tac/*/*, worktree/*) |
...
- **M001 (Memory Database):** The SQLite database (`tac.db`) must remain gitignored. The M001/S02 importer layer rebuilds it from tracked markdown. This PRD's `.gitignore` update explicitly ignores `tac.db`.

- **PR #487:** Must be closed. The `resolveMainWorktreeRoot` approach (sharing `.tac/` across worktrees) contradicts tracked-artifact architecture.
...
2. **Should `worktrees/` move outside `.tac/`?** Having worktrees inside `.tac/` creates a nesting-doll pattern (worktree contains `.tac/` which is inside `.tac/worktrees/`). Relocating to `.tac-worktrees/` or `~/.tac/worktrees/<repo-hash>/` is cleaner but changes the filesystem layout. Recommendation: defer, address separately if it causes issues.

3. **Pre-flight rebase automation?** Before milestone→main squash-merge, should TAC automatically `git rebase main`? Gemini recommends yes. Risk: rebase can fail with conflicts, adding a code path. Recommendation: implement as a doctor check ("milestone branch is behind main by N commits") with manual resolution, automate later if needed.
