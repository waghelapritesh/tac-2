...
**Related:** ADR-004 (capability-aware model routing), [ADR-005](https://github.com/waghelapritesh/tac-2/issues/2790), [ADR-006](https://github.com/waghelapritesh/tac-2/issues/2995), `packages/pi-ai/src/providers/`, `packages/pi-ai/src/models.ts`
...
```typescript
// models/index.ts
// TAC-2 — Model registry (split by provider for maintainability)
...
Move each provider to its own package (`@tac/provider-anthropic`, etc.). Maximum isolation but dramatically more complex build/release/versioning. Overkill for a monorepo where all providers ship together.
