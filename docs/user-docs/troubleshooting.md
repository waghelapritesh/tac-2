...
The built-in diagnostic tool validates `.tac/` integrity:
...
**Fix:** Run `/tac doctor` to repair state, then resume with `/tac auto`. If the issue persists, check that the expected artifact file exists on disk.

### Auto mode stops with "Loop detected"

**Cause:** A unit failed to produce its expected artifact twice in a row.

**Fix:** Check the task plan for clarity. If the plan is ambiguous, refine it manually, then `/tac auto` to resume.
...
**Symptoms:** `npm install -g tac-2` succeeds but `tac` isn't found.
...
**Workaround:** Run `npx tac-2` or `$(npm prefix -g)/bin/tac` directly.
...
- **oh-my-zsh** — the `gitfast` plugin aliases `tac` to `git svn dcommit`. Check with `alias tac` and unalias if needed

### `npm install -g tac-2` fails
...
For transient errors, TAC pauses briefly and resumes automatically. For permanent errors, configure fallback models:
...
**Headless mode:** `tac headless auto` auto-restarts the entire process on crash (default 3 attempts with exponential backoff). Combined with provider error auto-resume, this enables true overnight unattended execution.
...
**Fix:** Increase `budget_ceiling` in preferences, or switch to `budget` token profile to reduce per-unit cost, then resume with `/tac auto`.

### Stale lock file

**Symptoms:** Auto mode won't start, says another session is running.

**Fix:** TAC automatically detects stale locks — if the owning PID is dead, the lock is cleaned up and re-acquired on the next `/tac auto`. This includes stranded `.tac.lock/` directories left by `proper-lockfile` after crashes. If automatic recovery fails, delete `.tac/auto.lock` and the `.tac.lock/` directory manually:

```bash
rm -f .tac/auto.lock
rm -rf "$(dirname .tac)/.tac.lock"
```

### Git merge conflicts

**Symptoms:** Worktree merge fails on `.tac/` files.

**Fix:** TAC auto-resolves conflicts on `.tac/` runtime files. For content conflicts in code files, the LLM is given an opportunity to resolve them via a fix-merge session. If that fails, manual resolution is needed.

### Pre-dispatch says the milestone integration branch no longer exists

**Symptoms:** Auto mode or `/tac doctor` reports that a milestone recorded an integration branch that no longer exists in git.

**What it means:** The milestone's `.tac/milestones/<MID>/<MID>-META.json` still points at the branch that was active when the milestone started, but that branch has since been renamed or deleted.

**Current behavior:**
- If TAC can deterministically recover to a safe branch, it no longer hard-stops auto mode.
- Safe fallbacks are:
...
- In that case `/tac doctor` reports a warning and `/tac doctor fix` rewrites the stale metadata to the effective branch.
- TAC still blocks when no safe fallback branch can be determined.

**Fix:**
- Run `/tac doctor fix` to rewrite the stale milestone metadata automatically when the fallback is obvious.
- If TAC still blocks, recreate the missing branch or update your git preferences so `git.main_branch` points at a real branch.

### Transient `EBUSY` / `EPERM` / `EACCES` while writing `.tac/` files

**Symptoms:** On Windows, auto mode or doctor occasionally fails while updating `.tac/` files with errors like `EBUSY`, `EPERM`, or `EACCES`.

**Cause:** Antivirus, indexers, editors, or filesystem watchers can briefly lock the destination or temp file just as TAC performs the atomic rename.

**Current behavior:** TAC now retries those transient rename failures with a short bounded backoff before surfacing an error. The retry is intentionally limited so genuine filesystem problems still fail loudly instead of hanging forever.
...
- If repeated failures continue, run `/tac doctor` to confirm the repo state is still healthy and report the exact path + error code.

### Node v24 web boot failure

**Symptoms:** `tac --web` fails with `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING` on Node v24.
...
**Symptoms:** `tac --web` fails because port 3000 is already in use, even though no TAC session is running.

**Cause:** A previous web server process was not cleaned up on exit.

**Fix:** Fixed in v2.42.0+. TAC now cleans up stale web server processes automatically. If you're on an older version, kill the orphan process manually: `lsof -ti:3000 | xargs kill`.
...
**Cause:** TAC parsed git output assuming English locale strings.
...
**Common causes:**
- No `.mcp.json` or `.tac/mcp.json` file exists in the current project
- The config file is malformed JSON
- The server is configured in a different project directory than the one where you launched TAC

**Fix:**
- Add the server to `.mcp.json` or `.tac/mcp.json`
...
**Fix:**
- Run the configured command directly outside TAC and confirm the server actually starts
...
- If you're developing TAC itself, rebuild after schema changes with `npm run build`

### Local stdio server works manually but not in TAC

**Symptoms:** Running the server command manually seems fine, but TAC can't connect.

**Common causes:**
- The server depends on shell state that TAC doesn't inherit
...
### Session lock stolen by `/tac` in another terminal

**Symptoms:** Running `/tac` (step mode) in a second terminal causes a running auto-mode session to lose its lock.

**Fix:** Fixed in v2.36.0. Bare `/tac` no longer steals the session lock from a running auto-mode session. Upgrade to the latest version.
...
```bash
rm .tac/auto.lock
rm .tac/completed-units.json
```

Then `/tac auto` to restart from current disk state.
...
- **GitHub Issues:** [github.com/waghelapritesh/tac-2/issues](https://github.com/waghelapritesh/tac-2/issues)
- **Dashboard:** `Ctrl+Alt+G` or `/tac status` for real-time diagnostics
- **Forensics:** `/tac forensics` for structured post-mortem analysis of auto-mode failures
- **Session logs:** `.tac/activity/` contains JSONL session dumps for crash forensics

## iTerm2-Specific Issues

### Ctrl+Alt shortcuts trigger the wrong action (e.g., Ctrl+Alt+G opens external editor instead of TAC dashboard)

**Symptoms:** Pressing Ctrl+Alt+G opens the external editor prompt (Ctrl+G) instead of the TAC dashboard. Other Ctrl+Alt shortcuts behave as their Ctrl-only counterparts.
...
### "TAC database is not available"

**Symptoms:** `tac_decision_save` (or its alias `tac_save_decision`), `tac_requirement_update` (or `tac_update_requirement`), or `tac_summary_save` (or `tac_save_summary`) fail with this error.

**Cause:** The SQLite database wasn't initialized. This happens in manual `/tac` sessions (non-auto mode) on versions before v2.29.
...
TAC auto-detects language servers based on project files (e.g. `package.json` → TypeScript, `Cargo.toml` → Rust, `go.mod` → Go). If no servers are detected, the agent skips LSP features.
...
After installing, run `lsp reload` to restart detection without restarting TAC.
...
**Cause:** TAC uses `osascript display notification` as a fallback on macOS. This command is attributed to your terminal app (Ghostty, iTerm2, Alacritty, Kitty, Warp, etc.). If that app doesn't have notification permissions in System Settings → Notifications, macOS silently drops the notification — `osascript` exits 0 with no error.
...
TAC automatically prefers `terminal-notifier` when available. On first use, macOS will prompt you to allow notifications — this is the expected behavior.
...
```bash
osascript -e 'display notification "test" with title "TAC"'
```

**Verify:** After applying either fix, test with:

```bash
terminal-notifier -title "TAC" -message "working!" -sound Glass
```
...
- **Bot token is incorrect or expired** — run `/tac remote status` to confirm the configuration is saved, then `/tac remote telegram` to re-run setup and re-validate the token.
...
- **Wrong `channel_id`** — verify the chat ID in `~/.tac/PREFERENCES.md` matches the chat where you expect notifications. For group chats, the ID is typically a negative number (e.g., `-1001234567890`).
- **Network or firewall issue** — TAC must be able to reach `api.telegram.org`. Test with `curl https://api.telegram.org` from the machine running TAC.
...
- **Auto-mode is not running** — background polling only operates while auto-mode is active. Start auto-mode with `/tac auto` and then retry the command.
...
- **Bot token mismatch** — the `TELEGRAM_BOT_TOKEN` environment variable or the token in `~/.tac/PREFERENCES.md` may not match the bot you are messaging. Run `/tac remote status` to confirm which bot token is active.
- **Polling not started** — if TAC was already running when the Telegram configuration was added, restart auto-mode (`/tac stop`, then `/tac auto`) so polling initializes with the new configuration.
