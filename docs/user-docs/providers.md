...
Step-by-step setup instructions for every LLM provider TAC supports. If you ran the onboarding wizard (`tac config`) and picked a provider, you may already be configured — check with `/model` inside a session.
...
Built-in providers have models pre-registered in TAC. You only need to supply credentials.
...
Or run `tac config` and paste your key when prompted.

**Get a key:** [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys)

**Option B — Claude Code CLI:**

If you have a Claude Pro or Max subscription, you can authenticate through Anthropic's official Claude Code CLI. Install it, sign in with `claude`, then TAC will detect and route through it automatically:

```bash
# Install Claude Code CLI (see https://docs.anthropic.com/en/docs/claude-code)
claude
# Sign in when prompted, then start TAC
tac
```

TAC detects your local Claude Code installation and uses it as the authenticated Anthropic surface. This is the TOS-compliant path for subscription users — TAC never handles your subscription credentials directly.

> **Note:** TAC does not support browser-based OAuth sign-in for Anthropic. Use an API key or the Claude Code CLI instead.

**Option C — Use your Claude Pro/Max plan with TAC inside Claude Code:**

If you already have a Claude Pro or Max subscription and want to use TAC's planning, execution, and milestone orchestration directly from Claude Code — without switching to a separate terminal — you can connect TAC as an MCP server. This gives Claude Code access to TAC's full workflow toolset via the [Model Context Protocol](https://modelcontextprotocol.io), so you get TAC's structured project management powered by your existing Claude plan.

**Automatic setup (recommended):**

When TAC detects a Claude Code model during startup, it automatically writes a `.mcp.json` file in your project root with the TAC workflow MCP server configured. No manual steps needed — just start TAC once with Claude Code as the provider and the config is created for you.

You can also trigger this manually from inside a TAC session:

```bash
/tac mcp init
```

This writes (or updates) the `tac-workflow` entry in your project's `.mcp.json`. Claude Code discovers this file automatically on its next session start.

**Manual setup:**

If you prefer to configure it yourself, add TAC to your project's `.mcp.json`:
...
Or if `tac-mcp-server` is installed globally:

```json
{
  "mcpServers": {
    "tac": {
      "command": "tac-mcp-server"
    }
  }
}
```

You can also add this to `~/.claude/settings.json` under `mcpServers` to make TAC available across all projects.

**What's exposed:**

The MCP server provides TAC's full workflow tool surface — milestone planning, task completion, slice management, roadmap reassessment, journal queries, and more. Session management tools (`tac_execute`, `tac_status`, `tac_result`, `tac_cancel`) let Claude Code start and monitor TAC auto-mode sessions. See [Commands → MCP Server Mode](./commands.md#mcp-server-mode) for the full tool list.

**Verify the connection:**

From inside a TAC session, check that the MCP server is reachable:
...
Or run `tac config` and choose "Paste an API key" then "OpenAI".
...
Or run `tac config`, choose "Paste an API key", then "OpenRouter".

**Step 3 — Switch to an OpenRouter model:**

Inside a TAC session, type `/model` and select an OpenRouter model. Models are prefixed with `openrouter/` (e.g., `openrouter/anthropic/claude-sonnet-4`).

**Optional — Add custom OpenRouter models via `models.json`:**

If you want models not in the built-in list, add them to `~/.tac/agent/models.json`:
...
Note: the `apiKey` field here is the *name* of the environment variable, not the literal key. TAC resolves it automatically. You can also use a literal value or a shell command (see [Value Resolution](./custom-models.md#value-resolution)).
...
Local providers run on your machine. They require a `models.json` configuration file because TAC needs to know the endpoint URL and which models are available.

**Config file location:** `~/.tac/agent/models.json`
...
**Step 3 — Create `~/.tac/agent/models.json`:**
...
Inside TAC, type `/model` and pick your Ollama model.
...
Any server that implements the OpenAI Chat Completions API can work with TAC. This covers proxies (LiteLLM, Portkey, Helicone), self-hosted inference, and new providers.
...
This writes `~/.tac/agent/models.json` for you automatically.
...
**Cause:** The key is set in your shell but not visible to TAC.

**Fix:** Make sure the environment variable is exported in the same terminal where you run `tac`. Or use `tac config` to save the key to `~/.tac/agent/auth.json` so it persists across sessions.

### OpenRouter models not appearing in `/model`

**Cause:** No `OPENROUTER_API_KEY` set, so TAC hides OpenRouter models.

**Fix:** Set the key and restart TAC:
...
**Fix:** Add `compat.supportsDeveloperRole: false` to the provider config. This makes TAC send `system` messages instead:
...
**Expected behavior.** TAC defaults cost to zero for custom models. Override with the `cost` field if you want accurate cost tracking:
...
- `models.json` is valid JSON (use `cat ~/.tac/agent/models.json | python3 -m json.tool`)
- The server is running (for local providers)

For additional help, see [Troubleshooting](./troubleshooting.md) or run `/tac doctor` inside a session.
