...
Auto mode is TAC's autonomous execution engine. Run `/tac auto`, walk away, come back to built software with clean git history.

## How It Works

Auto mode is a **state machine driven by files on disk**. It reads `.tac/STATE.md`, determines the next unit of work, creates a fresh agent session, injects a focused prompt with all relevant context pre-inlined, and lets the LLM execute. When the LLM finishes, auto mode reads disk state again and dispatches the next unit.
...
TAC isolates milestone work using one of three modes (configured via `git.isolation` in preferences):

- **`worktree`** (default): Each milestone runs in its own git worktree at `.tac/worktrees/<MID>/` on a `milestone/<MID>` branch. All slice work commits sequentially — no branch switching, no merge conflicts mid-milestone. When the milestone completes, it's squash-merged to main as one clean commit.
...
A lock file tracks the current unit. If the session dies, the next `/tac auto` reads the surviving session file, synthesizes a recovery briefing from every tool call that made it to disk, and resumes with full context.

**Headless auto-restart (v2.26):** When running `tac headless auto`, crashes trigger automatic restart with exponential backoff (5s → 10s → 30s cap, default 3 attempts). Configure with `--max-restarts N`. SIGINT/SIGTERM bypasses restart. Combined with crash recovery, this enables true overnight "run until done" execution.

### Provider Error Recovery

TAC classifies provider errors and auto-resumes when safe:
...
TAC uses a sliding-window analysis to detect stuck loops. Instead of a simple "same unit dispatched twice" counter, the detector examines recent dispatch history for repeated patterns — catching cycles like A→B→A→B as well as single-unit repeats. On detection, TAC retries once with a deep diagnostic prompt. If it fails again, auto mode stops with the exact file it expected, so you can intervene.
...
`/tac forensics` is a full-access TAC debugger for post-mortem analysis of auto-mode failures. It provides:
...
- **Doctor integration** — includes structural health issues from `/tac doctor`
- **LLM-guided investigation** — an agent session with full tool access to investigate root causes

```
/tac forensics [optional problem description]
```
...
After a milestone completes, TAC auto-generates a self-contained HTML report in `.tac/reports/`. Reports include project summary, progress tree, slice dependency graph (SVG DAG), cost/token metrics with bar charts, execution timeline, changelog, and knowledge base. No external dependencies — all CSS and JS are inlined.

```yaml
auto_report: true    # enabled by default
```

Generate manually anytime with `/tac export --html`, or generate reports for all milestones at once with `/tac export --html --all` (v2.28).
...
Doctor issues (from `/tac doctor`) now surface in real time across three places:
...
```
/tac capture "add rate limiting to API endpoints"
```
...
TAC polls for incoming Telegram commands every ~5 seconds while auto-mode is active. Commands are only available during active auto-mode sessions.
...
`Ctrl+Alt+G` or `/tac status` shows real-time progress:
...
When `reactive_execution: true` is set in preferences, TAC derives a dependency graph from IO annotations in task plans. Tasks that don't conflict (no shared file reads/writes) are dispatched in parallel via subagents, while dependent tasks wait for their predecessors to complete.
