...
Hooks let you run shell commands at specific points in TAC's lifecycle without
...
```json
{
  "hooks": {
    ...
    "Stop": [
      { "command": "notify-send 'TAC is done'" }
    ]
  }
}
```

Each hook is a shell command. TAC pipes the event payload to the command's
...
The child process also receives `TAC_HOOK_EVENT` and `TAC_HOOK_SCOPE` env vars.
...
```json
{
  "hooks": {
    "Stop": [
      { "command": "terminal-notifier -title TAC -message 'Agent stopped'" }
    ]
  }
}
```
...
into TAC with no modifications.
