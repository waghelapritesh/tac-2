...
6. Saves the configuration to `~/.tac/PREFERENCES.md`
...
Remote questions are configured in `~/.tac/PREFERENCES.md`:
...
5. TAC picks up the response and continues execution
...
If no response is received within `timeout_minutes`, the prompt times out and TAC continues with a timeout result. The LLM handles timeouts according to the task context — typically by making a conservative default choice or pausing auto-mode.

## Informational Notifications

In addition to interactive questions, TAC sends informational notifications to your remote channel for significant auto-mode events. These do not require a response — they are delivered alongside desktop notifications.
...
When Telegram is configured as your remote channel, TAC runs a background polling loop (every ~5 seconds) while auto-mode is active. This allows you to send commands directly to your bot and receive live project status updates in return.
...
While auto-mode is running, TAC polls the Telegram Bot API every ~5 seconds for new messages sent to the bot. When a command is received, TAC processes it and replies in the same chat.

Polling is active only while auto-mode is running. When auto-mode stops (normally, or via `/pause`), polling stops as well. The next `/tac auto` resumes polling automatically.

The `/pause` command sets a stop directive that TAC checks at each unit boundary — the current unit always completes before auto-mode halts. `/resume` clears that directive so auto-mode continues without requiring a terminal interaction.

## Commands

| Command | Description |
|---------|-------------|
| `/tac remote` | Show remote questions menu and current status |
| `/tac remote slack` | Set up Slack integration |
| `/tac remote discord` | Set up Discord integration |
| `/tac remote telegram` | Set up Telegram integration |
| `/tac remote status` | Show current configuration and last prompt status |
| `/tac remote disconnect` | Remove remote questions configuration |
...
- Verify the bot token in `~/.tac/PREFERENCES.md` matches the token from [@BotFather](https://t.me/BotFather)
...
- Run `/tac remote status` from your terminal to confirm the Telegram configuration is saved correctly
