...
For TAC2, the safe path is:

1. Treat local Claude Code as an external authenticated runtime.
2. Never ask TAC users to sign into Claude subscriptions through TAC-managed Anthropic OAuth.
3. Never exchange Claude.ai subscription OAuth into a bearer token and call Anthropic APIs as if TAC were Claude Code.
4. If TAC needs direct Anthropic API access, require a Claude Console API key, Bedrock, Vertex, or another explicitly supported provider path.
...
- If TAC2 shells out to or embeds Claude Code, it should respect Claude Code's own credential selection instead of inventing a parallel Anthropic OAuth flow.
...
- If TAC2 is acting as a product for users, direct Anthropic access should be through commercial auth paths, not subscription-token reuse.
...
- A TAC-managed Anthropic OAuth flow for subscription accounts is high risk.
- Reusing user Claude subscription credentials inside TAC's own API client is high risk.
- Any flow that makes Anthropic believe requests come from Claude Code when they actually come from TAC infrastructure is out of bounds.

## Current TAC2 Findings
...
These are directionally correct because TAC is using the user's own local Claude Code installation as the authenticated Anthropic surface.
...
- TAC2 must not implement its own Anthropic subscription OAuth flow for end users.
- TAC2 must not persist Anthropic subscription OAuth tokens for later API use.
- TAC2 must not send Anthropic API traffic using subscription OAuth tokens obtained by TAC.
- TAC2 may support Anthropic direct access only via:
  - `ANTHROPIC_API_KEY`
...
4. Add explicit UI copy:
   - "Claude subscription users: sign into the local Claude Code app/CLI, not TAC."
5. Block migrations or code paths that convert Anthropic OAuth credentials into API auth for TAC-managed requests.
...
If a proposed TAC2 feature needs Anthropic access, ask one question:

"Is TAC calling Anthropic as TAC, or is TAC delegating to the user's already-authenticated local Claude Code runtime?"

- If TAC is calling Anthropic as TAC: require API key or supported cloud auth.
- If TAC is delegating to local Claude Code: acceptable, as long as TAC does not intercept, mint, or replay subscription credentials itself.
