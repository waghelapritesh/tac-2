...
TAC is an AI coding agent that handles planning, execution, verification, and shipping so you can focus on what to build. This guide walks you through installation on macOS, Windows, and Linux, then gets you running your first session.
...
```bash
tac --version     # prints the installed version
```

Inside the session, type `/model` to confirm your LLM is connected.

> **Apple Silicon PATH fix:** If `tac` isn't found after install, npm's global bin may not be in your PATH:
> ```bash
> echo 'export PATH="$(npm prefix -g)/bin:$PATH"' >> ~/.zshrc
> source ~/.zshrc
> ```

> **oh-my-zsh conflict:** The oh-my-zsh git plugin defines `alias tac='git svn dcommit'`. Fix with `unalias tac 2>/dev/null` in `~/.zshrc`, or use `tac-cli` instead.
...
> - If `tac` isn't recognized, restart your terminal. Windows needs a fresh terminal to pick up new PATH entries.
...
Run TAC in an isolated sandbox without installing Node.js on your host.

**Step 1 — Install Docker Desktop** (4.58+ required).

**Step 2 — Clone the TAC repo:**

```bash
git clone https://github.com/waghelapritesh/tac-2.git
cd tac-2/docker
```
...
```bash
docker sandbox create --template . --name tac-sandbox
docker sandbox exec -it tac-sandbox bash
```

**Step 4 — Set your API key and run TAC:**

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
tac auto "implement the feature described in issue #42"
```
...
TAC auto-selects a default model after provider setup. Switch anytime inside a session:
...
Type `/tac` inside a session. TAC executes one unit of work at a time, pausing between each with a wizard showing what completed and what's next.
...
Type `/tac auto` and walk away. TAC autonomously researches, plans, executes, verifies, commits, and advances through every slice until the milestone is complete.
...
All state lives on disk in `.tac/`:
...
TAC has a browser-based interface for visual project management:
...
TAC checks for updates every 24 hours and prompts at startup. You can also update manually:
...
| Problem | Fix |
|---------|-----|
| `command not found: tac` | Add npm global bin to PATH (see OS-specific notes above) |
| `tac` runs `git svn dcommit` | oh-my-zsh conflict — `unalias tac` or use `tac-cli` |
| Permission errors on `npm install -g` | Fix npm prefix (see Linux notes) or use nvm |
| Can't connect to LLM | Check API key with `tac config`, verify network access |
| `tac` hangs on start | Check Node.js version: `node --version` (need 22+) |
