...
TAC uses git for milestone isolation and sequential commits within each milestone. You choose an **isolation mode** that controls where work happens. The strategy is fully automated — you don't need to manage branches manually.

## Isolation Modes

TAC supports three isolation modes, configured via the `git.isolation` preference:

| Mode | Working Directory | Branch | Best For |
|------|-------------------|--------|----------|
| `worktree` (default) | `.tac/worktrees/<MID>/` | `milestone/<MID>` | Most projects — full file isolation between milestones |
...
Each milestone gets its own git worktree at `.tac/worktrees/<MID>/` on a `milestone/<MID>` branch. All execution happens inside the worktree. On completion, the worktree is squash-merged to main as one clean commit. The worktree and branch are then cleaned up.
...
Work happens directly on your current branch. No worktree, no milestone branch. TAC still commits sequentially with conventional commit messages, but there's no branch isolation.
...
Commits use conventional commit format with TAC metadata in trailers:

```
feat: core type definitions

TAC-Task: M001/S01/T01

feat: markdown parser for plan files

TAC-Task: M001/S01/T02
```
...
1. When a milestone starts, a worktree is created at `.tac/worktrees/<MID>/` on branch `milestone/<MID>`
2. Planning artifacts from `.tac/milestones/` are copied into the worktree
...
git:
  ...
  commit_docs: true           # commit .tac/ to git
...
For teams using Gitflow or branch-based workflows, TAC can automatically create a pull request when a milestone completes:
...
When set to `false`, TAC adds `.tac/` to `.gitignore` and keeps all planning artifacts local-only. Useful for teams where only some members use TAC, or when company policy requires a clean repository.

## Self-Healing

TAC includes automatic recovery for common git issues:
...
Run `/tac doctor` to check git health manually.

## Native Git Operations

Since v2.16, TAC uses libgit2 via native bindings for read-heavy operations in the dispatch hot path. This eliminates ~70 process spawns per dispatch cycle, improving auto-mode throughput.
