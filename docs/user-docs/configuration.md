...
TAC preferences live in `~/.tac/PREFERENCES.md` (global) or `.tac/PREFERENCES.md` (project-local). Manage interactively with `/tac prefs`.

## `/tac prefs` Commands

| Command | Description |
|---------|-------------|
| `/tac prefs` | Open the global preferences wizard (default) |
| `/tac prefs global` | Interactive wizard for global preferences (`~/.tac/PREFERENCES.md`) |
| `/tac prefs project` | Interactive wizard for project preferences (`.tac/PREFERENCES.md`) |
| `/tac prefs status` | Show current preference files, merged values, and skill resolution status |
| `/tac prefs wizard` | Alias for `/tac prefs global` |
| `/tac prefs setup` | Alias for `/tac prefs wizard` — creates preferences file if missing |
| `/tac prefs import-claude` | Import Claude marketplace plugins and skills as namespaced TAC components |
| `/tac prefs import-claude global` | Import to global scope |
| `/tac prefs import-claude project` | Import to project scope |
...
|-------|------|-----------|
| Global | `~/.tac/PREFERENCES.md` | All projects |
| Project | `.tac/PREFERENCES.md` | Current project only |
...
## Global API Keys (`/tac config`)

Tool API keys are stored globally in `~/.tac/agent/auth.json` and apply to all projects automatically. Set them once with `/tac config` — no need to configure per-project `.env` files.
...
1. `/tac config` saves keys to `~/.tac/agent/auth.json`
...
TAC can connect to external MCP servers configured in project files. This is useful for local tools, internal APIs, self-hosted services, or integrations that aren't built in as native TAC extensions.

### Config file locations

TAC reads MCP client configuration from these project-local paths:
...
- `.tac/mcp.json` for local-only MCP configuration you do **not** want to share
...
After adding config, verify it from a TAC session:
...
1. `mcp_servers` — confirms TAC can see the config file and parse the server entry
...
- TAC and `tac-mcp-server` both hydrate supported model and tool keys saved in `~/.tac/agent/auth.json`, so MCP configs can safely reference them through `${ENV_VAR}` placeholders without committing raw credentials.
- If a server is team-shared and safe to commit, `.mcp.json` is usually the better home.
- If a server depends on machine-local paths, personal services, or local-only secrets, prefer `.tac/mcp.json`.

## Environment Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `TAC_HOME` | `~/.tac` | Global TAC directory. All paths derive from this unless individually overridden. Affects preferences, skills, sessions, and per-project state. (v2.39) |
| `TAC_PROJECT_ID` | (auto-hash) | Override the automatic project identity hash. Per-project state goes to `$TAC_HOME/projects/<TAC_PROJECT_ID>/` instead of the computed hash. Useful for CI/CD or sharing state across clones of the same repo. (v2.39) |
| `TAC_STATE_DIR` | `$TAC_HOME` | Per-project state root. Controls where `projects/<repo-hash>/` directories are created. Takes precedence over `TAC_HOME` for project state. |
| `TAC_CODING_AGENT_DIR` | `$TAC_HOME/agent` | Agent directory containing managed resources, extensions, and auth. Takes precedence over `TAC_HOME` for agent paths. |
| `TAC_ALLOWED_COMMAND_PREFIXES` | (built-in list) | Comma-separated command prefixes allowed for `!command` value resolution. Overrides `allowedCommandPrefixes` in settings.json. See [Custom Models — Command Allowlist](custom-models.md#command-allowlist). |
| `TAC_FETCH_ALLOWED_URLS` | (none) | Comma-separated hostnames exempted from `fetch_page` URL blocking. Overrides `fetchAllowedUrls` in settings.json. See [URL Blocking](#url-blocking-fetch_page). |
...
Define custom models and providers in `~/.tac/agent/models.json`. This lets you add models not included in the default registry — useful for self-hosted endpoints (Ollama, vLLM, LM Studio), fine-tuned models, proxies, or new provider releases.

TAC resolves models.json with fallback logic:
1. `~/.tac/agent/models.json` — primary (TAC)
2. `~/.pi/agent/models.json` — fallback (Pi)
3. If neither exists, creates `~/.tac/agent/models.json`
...
When a model fails to switch (provider unavailable, rate limited, credits exhausted), TAC automatically tries the next model in the `fallbacks` list.

### Community Provider Extensions

For providers not built into TAC, community extensions can add full provider support with proper model definitions, thinking format configuration, and interactive API key setup.

| Extension | Provider | Models | Install |
|-----------|----------|--------|---------|
| [`pi-dashscope`](https://www.npmjs.com/package/pi-dashscope) | Alibaba DashScope (ModelStudio) | Qwen3, GLM-5, MiniMax M2.5, Kimi K2.5 | `tac install npm:pi-dashscope` |
...
Controls how TAC finds and applies skills during auto mode.
...
If you need the agent to fetch from internal URLs (self-hosted docs, internal APIs behind a VPN), add their hostnames to `fetchAllowedUrls` in global settings (`~/.tac/agent/settings.json`):
...
Alternatively, set the `TAC_FETCH_ALLOWED_URLS` environment variable (comma-separated). The env var takes precedence over settings.json:
...
Reports are written to `.tac/reports/` as self-contained HTML files with embedded CSS/JS.
...
git:
  ...
  commit_docs: true           # commit .tac/ artifacts to git (set false to keep local)
  manage_gitignore: true      # set false to prevent TAC from modifying .gitignore
  worktree_post_create: .tac/hooks/post-worktree-create  # script to run after worktree creation
...
| `commit_docs` | boolean | `true` | Commit `.tac/` planning artifacts to git. Set `false` to keep local-only |
| `manage_gitignore` | boolean | `true` | When `false`, TAC will not modify `.gitignore` at all — no baseline patterns, no self-healing. Use if you manage your own `.gitignore` |
...
```yaml
git:
  worktree_post_create: .tac/hooks/post-worktree-create
```
...
Example hook script (`.tac/hooks/post-worktree-create`):
...
The path can be absolute or relative to the project root. The script runs with a 30-second timeout. Failure is non-fatal — TAC logs a warning and continues.
...
**How it works:**
1. Milestone completes → TAC squash-merges the worktree to the main branch
...
If `pr_target_branch` is not set, the PR targets the `main_branch` (or auto-detected main branch). PR creation failure is non-fatal — TAC logs and continues.

### `github` (v2.39)

GitHub sync configuration. When enabled, TAC auto-syncs milestones, slices, and tasks to GitHub Issues, PRs, and Milestones.

```yaml
github:
  enabled: true
  repo: "owner/repo"              # auto-detected from git remote if omitted
  labels: [tac, auto-generated]   # labels applied to created issues/PRs
...
- Sync mapping is persisted in `.tac/.github-sync.json`
...
Control what notifications TAC sends during auto mode:
...
**macOS delivery:** TAC uses [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) when available, falling back to `osascript`. We recommend installing `terminal-notifier` for reliable notification delivery:
...
For project-specific knowledge (patterns, gotchas, lessons learned), use `.tac/KNOWLEDGE.md` instead — it's injected into every agent prompt automatically. Add entries with `/tac knowledge rule|pattern|lesson <description>`.

### `RUNTIME.md` — Runtime Context (v2.39)

Declare project-level runtime context in `.tac/RUNTIME.md`. This file is inlined into task execution prompts, giving the agent accurate information about your runtime environment without relying on hallucinated paths or URLs.

**Location:** `.tac/RUNTIME.md`
...
OpenAI service tier preference for supported models. Toggle with `/tac fast`.
...
Opt-in: search existing issues and PRs before filing from `/tac forensics`. Uses additional AI tokens.
...
# Service tier
service_tier: priority         # "priority" or "flex" (for /tac fast)
