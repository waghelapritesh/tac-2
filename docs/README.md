...
Welcome to the TAC documentation. This covers everything from getting started to advanced configuration, auto-mode internals, and extending TAC with the Pi SDK.

## User Documentation

Guides for installing, configuring, and using TAC day-to-day. Located in [`user-docs/`](./user-docs/).
...
| [Migration from v1](./user-docs/migration.md) | Migrating `.planning` directories from the original TAC |
| [Troubleshooting](./user-docs/troubleshooting.md) | Common issues, `/tac doctor` (real-time visibility v2.40), `/tac forensics` (full debugger v2.40), and recovery procedures |
| [Web Interface](./user-docs/web-interface.md) | Browser-based project management with `tac --web` (v2.41) |
...
| [ADR-008: TAC Tools over MCP](./dev/ADR-008-tac-tools-over-mcp-for-provider-parity.md) | Native tools over MCP for provider parity |
...
Guides for the underlying Pi SDK that TAC is built on. Located in [`dev/`](./dev/).
