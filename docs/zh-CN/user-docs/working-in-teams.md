...
TAC 支持多人并行工作流，让多个开发者可以同时在同一个仓库中工作。

## 设置

### 1. 启用 Team Mode

为团队使用配置 TAC 的最简单方法，是在项目偏好中设置 `mode: team`。这会一次性开启唯一 milestone ID、推送分支和预合并检查：

```yaml
# .tac/PREFERENCES.md（项目级，提交到 git）
---
version: 1
mode: team
---
...
- `.tac/PREFERENCES.md`：项目偏好
- `.tac/PROJECT.md`：持续维护的项目描述
- `.tac/REQUIREMENTS.md`：需求契约
- `.tac/DECISIONS.md`：架构决策
- `.tac/milestones/`：roadmaps、plans、summaries 和 research
...
如果团队里只有部分成员使用 TAC，或者公司策略要求仓库保持干净：
...
这会把整个 `.tac/` 加入 `.gitignore`，让所有产物都保留在本地。这样使用 TAC 的开发者仍然能获得结构化规划的好处，而不会影响不使用 TAC 的同事。

## 迁移现有项目

如果你当前项目里对 `.tac/` 做了整目录忽略：

1. 确保当前没有进行中的 milestones（工作区状态干净）
2. 按上面的选择性规则更新 `.gitignore`
3. 在 `.tac/PREFERENCES.md` 中添加 `unique_milestone_ids: true`
...
- 获得自己的 worktree（`.tac/worktrees/<MID>/`，已加入 gitignore）
...
TAC 会强制要求上游依赖 milestone 先完成，之后才会启动下游工作。
