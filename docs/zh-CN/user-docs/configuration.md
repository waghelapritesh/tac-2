...
TAC 偏好设置保存在 `~/.tac/PREFERENCES.md`（全局）或 `.tac/PREFERENCES.md`（项目级）中。可以通过 `/tac prefs` 进行交互式管理。

## `/tac prefs` 命令

| 命令 | 说明 |
|------|------|
| `/tac prefs` | 打开全局偏好设置向导（默认） |
| `/tac prefs global` | 全局偏好设置交互向导（`~/.tac/PREFERENCES.md`） |
| `/tac prefs project` | 项目偏好设置交互向导（`.tac/PREFERENCES.md`） |
| `/tac prefs status` | 显示当前偏好文件、合并后的值以及 skill 解析状态 |
| `/tac prefs wizard` | `/tac prefs global` 的别名 |
| `/tac prefs setup` | `/tac prefs wizard` 的别名；若偏好文件不存在会自动创建 |
| `/tac prefs import-claude` | 将 Claude marketplace plugins 和 skills 以命名空间化的 TAC 组件形式导入 |
| `/tac prefs import-claude global` | 导入到全局作用域 |
| `/tac prefs import-claude project` | 导入到项目作用域 |
...
| 作用域 | 路径 | 适用范围 |
|--------|------|----------|
| 全局 | `~/.tac/PREFERENCES.md` | 所有项目 |
| 项目 | `.tac/PREFERENCES.md` | 仅当前项目 |
...
<a id="global-api-keys-tac-config"></a>
## 全局 API Keys（`/tac config`）

工具 API keys 会全局保存在 `~/.tac/agent/auth.json` 中，并自动应用到所有项目。只需用 `/tac config` 配置一次，无需在每个项目里维护 `.env`。
...
1. `/tac config` 会把 keys 保存到 `~/.tac/agent/auth.json`
...
TAC 可以连接配置在项目文件中的外部 MCP servers。这适合接入本地工具、内部 API、自托管服务，或者那些未作为 TAC 原生扩展内置的集成。

### 配置文件位置

TAC 会从以下项目本地路径读取 MCP client 配置：
...
- 把仅本机使用、不希望共享的 MCP 配置放在 `.tac/mcp.json`
...
1. `mcp_servers`：确认 TAC 能看到配置文件并正确解析 server 条目
...
- TAC 和 `tac-mcp-server` 都会自动加载保存在 `~/.tac/agent/auth.json` 中的 model / tool keys，因此 MCP 配置可以安全地通过 `${ENV_VAR}` 占位符引用这些值，而不必提交原始凭据
- 如果某个 server 是团队共享且适合提交到仓库，通常更适合放在 `.mcp.json`
- 如果某个 server 依赖本机路径、个人服务或本地 secrets，更适合放在 `.tac/mcp.json`

## 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `TAC_HOME` | `~/.tac` | 全局 TAC 目录。除非单独覆盖，否则其它路径都从这里派生。影响偏好、skills、sessions 以及项目状态。（v2.39） |
| `TAC_PROJECT_ID` | （自动哈希） | 覆盖自动生成的项目身份哈希。这样项目状态会写入 `$TAC_HOME/projects/<TAC_PROJECT_ID>/`，而不是计算出的哈希目录。适用于 CI/CD 或多个克隆共享状态。（v2.39） |
| `TAC_STATE_DIR` | `$TAC_HOME` | 项目状态根目录。控制 `projects/<repo-hash>/` 的创建位置。对项目状态的优先级高于 `TAC_HOME`。 |
| `TAC_CODING_AGENT_DIR` | `$TAC_HOME/agent` | agent 目录，包含托管资源、扩展和 auth。对 agent 相关路径的优先级高于 `TAC_HOME`。 |
| `TAC_ALLOWED_COMMAND_PREFIXES` | （内置列表） | 允许用于 `!command` 值解析的命令前缀，逗号分隔。会覆盖 settings.json 中的 `allowedCommandPrefixes`。见 [自定义模型：命令允许列表](custom-models.md#command-allowlist)。 |
| `TAC_FETCH_ALLOWED_URLS` | （无） | 对 `fetch_page` URL block 免检的 hostnames，逗号分隔。会覆盖 settings.json 中的 `fetchAllowedUrls`。见 [URL Blocking](#url-blocking-fetch_page)。 |
...
你可以在 `~/.tac/agent/models.json` 里定义自定义 models 和 providers。这允许你添加默认注册表里没有的 models，适合自托管 endpoints（Ollama、vLLM、LM Studio）、微调模型、代理，或者刚发布的新 provider。

TAC 读取 `models.json` 的顺序如下：

1. `~/.tac/agent/models.json`：主位置（TAC）
2. `~/.pi/agent/models.json`：回退位置（Pi）
3. 如果两者都不存在，则创建 `~/.tac/agent/models.json`
...
当某个 model 切换失败（provider 不可用、被限流、额度耗尽）时，TAC 会自动尝试 `fallbacks` 列表中的下一个 model。

### Community Provider Extensions

对于 TAC 未内置的 providers，社区扩展可以添加完整 provider 支持，包括正确的 model 定义、thinking format 配置以及交互式 API key 设置。
...
如果你确实需要 agent 访问内网 URL（例如自托管文档、VPN 后的内部 API），可以在全局设置 `~/.tac/agent/settings.json` 中添加 `fetchAllowedUrls`：
...
或者设置 `TAC_FETCH_ALLOWED_URLS` 环境变量（逗号分隔）。环境变量优先级高于 settings.json：
...
报告会以自包含 HTML 文件的形式写入 `.tac/reports/`，所有 CSS / JS 都内嵌。
...
git:
  ...
  commit_docs: true           # 是否把 .tac/ 产物提交到 git（设为 false 时仅保留本地）
  manage_gitignore: true      # 设为 false 时，TAC 不再修改 .gitignore
  worktree_post_create: .tac/hooks/post-worktree-create  # worktree 创建后执行的脚本
...
| `commit_docs` | boolean | `true` | 是否把 `.tac/` planning 产物提交到 git。设为 `false` 则仅保留本地 |
| `manage_gitignore` | boolean | `true` | 设为 `false` 后，TAC 将完全不修改 `.gitignore`，不会添加基础规则，也不会做自愈 |
...
示例 hook（`.tac/hooks/post-worktree-create`）：
...
路径既可以是绝对路径，也可以相对项目根目录。脚本有 30 秒超时限制。失败不会中断流程，TAC 会记录告警后继续。
...
1. milestone 完成后，TAC 先把 worktree squash merge 回主分支
2. 如果 `auto_push: true`，把主分支推送到远程
3. 把 milestone 分支推送到远程
4. 通过 `gh pr create` 从 milestone 分支向 `pr_target_branch` 创建 PR

如果没有设置 `pr_target_branch`，PR 会默认指向 `main_branch`（或者自动检测出的主分支）。PR 创建失败不会中断流程，TAC 会记录日志后继续。

### `github`（v2.39）

GitHub 同步配置。启用后，TAC 会自动把 milestones、slices 和 tasks 同步到 GitHub Issues、PRs 和 Milestones。

```yaml
github:
  enabled: true
  repo: "owner/repo"              # 省略时从 git remote 自动检测
  labels: [tac, auto-generated]   # 应用到创建出的 issues / PRs 的标签
...
**macOS 通知方式：** TAC 会优先使用 [`terminal-notifier`](https://github.com/julienXX/terminal-notifier)，不可用时回退到 `osascript`。建议安装 `terminal-notifier`，获得更稳定的通知体验：
...
如果是项目特有知识（模式、坑点、经验），请优先放到 `.tac/KNOWLEDGE.md` 中，因为它会自动注入每个 agent prompt。你也可以通过 `/tac knowledge rule|pattern|lesson <description>` 添加。

### `RUNTIME.md`：运行时上下文（v2.39）

你可以在 `.tac/RUNTIME.md` 中声明项目级运行时上下文。这个文件会内联进 task execution prompt，让 agent 能准确知道运行环境，而不必靠猜测路径或 URL。
...
OpenAI 支持模型的 service tier 偏好。可通过 `/tac fast` 切换。
...
可选启用：在 `/tac forensics` 提交 issue 之前，先搜索现有 issues 和 PRs。会额外消耗一些 AI tokens。
