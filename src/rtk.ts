...
export const TAC_SKIP_RTK_INSTALL_ENV = "TAC_SKIP_RTK_INSTALL";
export {
  TAC_RTK_DISABLED_ENV,
  TAC_RTK_PATH_ENV,
  RTK_TELEMETRY_DISABLED_ENV,
...
export function resolveRtkBinaryPath(options: ResolveRtkBinaryPathOptions = {}): string | null {
  ...
  const explicitPath = env[TAC_RTK_PATH_ENV];
...
export async function ensureRtkAvailable(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  const env = options.env ?? process.env;
  if (!isRtkEnabled(env)) {
    return { enabled: false, supported: true, available: false, source: "disabled", reason: `${TAC_RTK_DISABLED_ENV} is set` };
  }
  if (isTruthy(env[TAC_SKIP_RTK_INSTALL_ENV])) {
    const configuredPath = env[TAC_RTK_PATH_ENV];
    if (configuredPath && existsSync(configuredPath)) {
      return { enabled: true, supported: true, available: true, source: "managed", binaryPath: configuredPath };
    }
    return { enabled: true, supported: true, available: false, source: "missing", reason: `${TAC_SKIP_RTK_INSTALL_ENV} is set` };
  }
  ...
  try {
    ...
    const checksumsResponse = await fetch(checksumsUrl, { headers: { "User-Agent": "tac-2-rtk" } });
...
export async function bootstrapRtk(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  ...
  if (result.binaryPath) {
    process.env[TAC_RTK_PATH_ENV] = result.binaryPath;
  }
  return result;
}
