 ...
 * into TAC's auth storage. Runs when TAC has no LLM providers configured,
...
import type { AuthStorage, AuthCredential } from '@tac/pi-coding-agent'
...
/**
 * Migrate provider credentials from Pi's auth.json into TAC's AuthStorage.
 *
 * Only runs when TAC has no LLM provider configured and Pi's auth.json exists.
 * Copies any credentials TAC doesn't already have. Returns true if an LLM
...
export function migratePiCredentials(authStorage: AuthStorage): boolean {
  try {
    ...
    for (const [providerId, credential] of Object.entries(piData)) {
      ...
      process.stderr.write(`[tac] Migrated ${isLlm ? 'LLM provider' : 'credential'}: ${providerId} (from Pi)\n`)
    }
