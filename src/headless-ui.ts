...
import { RpcClient, attachJsonlLineReader } from '@tac/pi-coding-agent'
...
export function summarizeToolArgs(toolName: unknown, toolInput: unknown): string {
  ...
  switch (name) {
    ...
    default: {
      // TAC tools: show milestone/slice/task IDs when present
      if (name.startsWith('tac_')) {
        return summarizeTacTool(name, input)
      }
...
/** Summarize TAC extension tool args into a compact identifier string. */
function summarizeTacTool(name: string, input: Record<string, unknown>): string {
  const parts: string[] = []
  ...
  // Fallback for TAC tools without IDs (e.g. tac_decision_save)
  if (input.decision) {
