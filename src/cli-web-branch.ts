...
export interface CliFlags {
  ...
  messages: string[]
  web?: boolean
  /** Optional project path for web mode: `tac --web <path>` or `tac web start <path>` */
  webPath?: string
  ...
  /** Set by `tac sessions` when the user picks a specific session to resume */
...
function emitWebModeFailure(stderr: WritableLike, status: WebModeLaunchStatus): void {
  if (status.ok) return
  stderr.write(`[tac] Web mode launch failed: ${status.failureReason}\n`)
}
...
export async function runWebCliBranch(
  ...
  // Handle `tac web stop [path|--all]` subcommand
  ...
  // `tac web [start] [path]` is an alias for `tac --web [path]`
  // Matches: `tac web`, `tac web start`, `tac web start <path>`, `tac web <path>`
  ...
  //   tac --web <path>           → flags.webPath
  //   tac web start <path>       → messages[2]
  //   tac web <path>             → messages[1] (when not "start")
  ...
  if (webPath) {
    ...
    if (!checkExists(currentCwd)) {
      stderr.write(`[tac] Project path does not exist: ${currentCwd}\n`)
      return {
        handled: true,
        exitCode: 1,
        action: 'start',
    ...
    stderr.write(`[tac] Using project path: ${currentCwd}\n`)
  } else {
