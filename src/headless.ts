/**
 * Headless Orchestrator — `tac headless`
 *
 * Runs any /tac subcommand without a TUI by spawning a child process in
...
import { RpcClient, SessionManager } from '@tac/pi-coding-agent'
import type { SessionInfo } from '@tac/pi-coding-agent'
...
async function runHeadlessOnce(options: HeadlessOptions, restartCount: number): Promise<{ exitCode: number; interrupted: boolean }> {
  ...
  // For new-milestone, load context and bootstrap .tac/ before spawning RPC child
  if (isNewMilestone) {
    ...
    // Bootstrap .tac/ if needed
    const tacDir = join(process.cwd(), '.tac')
    if (!existsSync(tacDir)) {
      if (!options.json) {
        process.stderr.write('[headless] Bootstrapping .tac/ project structure...\n')
      }
      bootstrapTacProject(process.cwd())
    }

    // Write context to temp file for the RPC child to read
    const runtimeDir = join(tacDir, 'runtime')
    mkdirSync(runtimeDir, { recursive: true })
    writeFileSync(join(runtimeDir, 'headless-context.md'), contextContent, 'utf-8')
  }

  // Validate .tac/ directory (skip for new-milestone since we just bootstrapped it)
  const tacDir = join(process.cwd(), '.tac')
  if (!isNewMilestone && !existsSync(tacDir)) {
    process.stderr.write('[headless] Error: No .tac/ directory found in current directory.\n')
    process.stderr.write("[headless] Run 'tac' interactively first to initialize a project.\n")
    process.exit(1)
  }
  ...
  // The interactive `/tac doctor` command lives in the TAC extension; this CLI
  ...
  if (options.command === 'doctor') {
    ...
    const { runTACDoctor } = await import('./resources/extensions/tac/doctor.js')
    const { formatDoctorReport, formatDoctorReportJson } = await import('./resources/extensions/tac/doctor-format.js')
    let exitCode = 1
    try {
      const report = await runTACDoctor(process.cwd())
  ...
  const cliPath = process.env.TAC_BIN_PATH || process.argv[1]
  if (!cliPath) {
    process.stderr.write('[headless] Error: Cannot determine CLI path. Set TAC_BIN_PATH or run via tac.\n')
    process.exit(1)
  }
  ...
  // Signal headless mode to the TAC extension (skips UAT human pause, etc.)
  clientOptions.env = { ...(clientOptions.env as Record<string, string> || {}), TAC_HEADLESS: '1' }
  ...
  let v2Enabled = false
  try {
    await client.init({ clientId: 'tac-headless' })
    v2Enabled = true
  } catch {
  ...
  if (!options.json) {
    process.stderr.write(`[headless] Running /tac ${options.command}${options.commandArgs.length > 0 ? ' ' + options.commandArgs.join(' ') : ''}...\n`)
  }

  // Send the command
  const command = `/tac ${options.command}${options.commandArgs.length > 0 ? ' ' + options.commandArgs.join(' ') : ''}`
  try {
  ...
  // Auto-mode chaining: if --auto and milestone creation succeeded, send /tac auto
