...
test("resolveModulePaths returns tsLoaderPath and validates it exists", () => {
  ...
  assert.equal(
    result.tsLoaderPath,
    "/fake/package/src/resources/extensions/tac/tests/resolve-ts.mjs",
  )
})
...
test("resolveModulePaths returns env entries for each module", () => {
  ...
  const result = resolveModulePaths(packageRoot, {
    modules: [
      { envKey: "TAC_MOD_A", relativePath: "src/a.ts" },
      { envKey: "TAC_MOD_B", relativePath: "src/b.ts" },
    ],
    existsSync: () => true,
  })
  assert.deepEqual(result.env, {
    TAC_MOD_A: "/fake/package/src/a.ts",
    TAC_MOD_B: "/fake/package/src/b.ts",
  })
})
