// Project/App Name: tac-2 · headless doctor wiring (#4929)
//
// Regression test for the live-regression assertion added in #4904
// ("tac doctor surfaces actionable guidance about the stale lock").
//
// Before this fix, the headless dispatcher had no `doctor` case — the
// only path the live-regression test could reach (`tac headless doctor`)
// fell through to RPC dispatch and tried to launch a TUI subprocess.
// Now the dispatcher invokes runTACDoctor directly (mirrors the existing
...
//
// This test exercises the runTACDoctor + formatDoctorReport pipeline
...
import { runTACDoctor } from "../resources/extensions/tac/doctor.ts";
import {
  formatDoctorReport,
  formatDoctorReportJson,
} from "../resources/extensions/tac/doctor-format.ts";

function makeStaleLockFixture(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-headless-doctor-"));
...
test("#4929: runTACDoctor + formatDoctorReport surface 'lock' + stale PID for stale auto.lock", async (t) => {
  const base = makeStaleLockFixture();
  t.after(() => rmSync(base, { recursive: true, force: true }));

  const report = await runTACDoctor(base);
