// TAC2 — Regression test: auto-mode resume resolves resource-loader.js from deployed path (#3949)
...
const autoTsPath = join(__dirname, "..", "resources", "extensions", "tac", "auto.ts");
const loaderTsPath = join(__dirname, "..", "loader.ts");

test("loader.ts sets TAC_PKG_ROOT env var", () => {
  const loaderSrc = readFileSync(loaderTsPath, "utf-8");
  assert.ok(
    loaderSrc.includes("process.env.TAC_PKG_ROOT"),
    "loader.ts must set TAC_PKG_ROOT so deployed extensions can locate package-root modules",
  );
});

test("auto.ts resume uses TAC_PKG_ROOT for resource-loader import, not bare relative path", () => {
  const autoSrc = readFileSync(autoTsPath, "utf-8");

  // Must reference TAC_PKG_ROOT to build an absolute path
  assert.ok(
    autoSrc.includes("process.env.TAC_PKG_ROOT"),
    "auto.ts must use TAC_PKG_ROOT to resolve resource-loader.js from deployed extension path",
  );
  ...
  // The resourceLoaderPath must be constructed from TAC_PKG_ROOT via pathToFileURL
...
test("TAC_PKG_ROOT resolves resource-loader.js correctly from package root", () => {
  // Simulate what auto.ts does: given TAC_PKG_ROOT, construct the path
