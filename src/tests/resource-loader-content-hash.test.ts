...
/**
 * Regression test for waghelapritesh/tac-2 #4787.
 *
 ...
 * `~/.tac/agent/extensions/tac/prompts/`.
 *
...
test("computeResourceFingerprint detects same-size content edits (#4787)", async (t) => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-fingerprint-content-"));
...
test("syncResourceDir overwrites same-size stale content on refresh (#4787)", async (t) => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-sync-samesize-"));
  t.after(() => { rmSync(tmp, { recursive: true, force: true }); });
