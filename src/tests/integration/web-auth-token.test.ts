...
const appShellSource = readFileSync(join(projectRoot, 'web', 'components', 'tac', 'app-shell.tsx'), 'utf-8')
...
test('middleware.ts skips auth when TAC_WEB_AUTH_TOKEN is not set', () => {
  assert.match(middlewareSource, /TAC_WEB_AUTH_TOKEN/, 'middleware should read TAC_WEB_AUTH_TOKEN from env')
