/**
 * E2E smoke tests for the TAC CLI binary (dist/loader.js).
 *
 ...
 *
 * Run with:
 *   node --import ./src/resources/extensions/tac/tests/resolve-ts.mjs \
...
// ---------------------------------------------------------------------------
// 1. tac --version outputs a semver string and exits 0
// ---------------------------------------------------------------------------

test("tac --version outputs a semver version string and exits 0", async () => {
  const result = await runTac(["--version"]);
...
// ---------------------------------------------------------------------------
// 2. tac --help outputs usage information and exits 0
// ---------------------------------------------------------------------------

test("tac --help outputs usage information and exits 0", async () => {
  const result = await runTac(["--help"]);
...
// ---------------------------------------------------------------------------
// 3. tac config --help outputs config-specific or general help and exits 0
// ---------------------------------------------------------------------------

test("tac config --help outputs help and exits 0", async () => {
  const result = await runTac(["config", "--help"]);
...
// ---------------------------------------------------------------------------
// 4. tac update --help outputs update-specific or general help and exits 0
// ---------------------------------------------------------------------------

test("tac update --help outputs help and exits 0", async () => {
  const result = await runTac(["update", "--help"]);
...
// ---------------------------------------------------------------------------
// 5. tac --list-models runs without crashing
// ---------------------------------------------------------------------------

test("tac --list-models runs without crashing", async () => {
  const result = await runTac(["--list-models"]);

  assert.ok(!result.timedOut, "tac --list-models should exit within the timeout");
  assert.strictEqual(result.code, 0, `expected exit 0, got ${result.code}`);
...
// ---------------------------------------------------------------------------
// 6. tac --print in text mode does not segfault or throw unhandled errors
//    (may fail with "No model selected" when no API keys are configured)
// ---------------------------------------------------------------------------

test("tac --mode text --print does not segfault or throw unhandled errors", { skip: !process.env.ANTHROPIC_API_KEY && !process.env.OPENAI_API_KEY ? "no API key available — print mode requires a configured provider" : undefined }, async () => {
  const result = await runTac(
    ["--mode", "text", "--print", "echo hello"],
    15_000,
  );

  assert.ok(!result.timedOut, "tac --print should not hang indefinitely");
...
// ---------------------------------------------------------------------------
// 7. tac headless --help outputs headless-specific help and exits 0
// ---------------------------------------------------------------------------

test("tac headless --help outputs help and exits 0", async () => {
  const result = await runTac(["headless", "--help"]);
...
// ---------------------------------------------------------------------------
// 8. tac sessions --help outputs sessions-specific help and exits 0
// ---------------------------------------------------------------------------

test("tac sessions --help outputs sessions-specific help and exits 0", async () => {
  const result = await runTac(["sessions", "--help"]);
...
// ---------------------------------------------------------------------------
// 9. tac (no TTY) exits with clean error about requiring a terminal
// ---------------------------------------------------------------------------

test("tac with no TTY exits 1 with clean terminal-required error", async () => {
  // Running with piped stdin (non-TTY) and no subcommand/flags triggers
  // interactive mode which requires a TTY
  const result = await runTac([], 15_000);
...
// ---------------------------------------------------------------------------
// 10. tac with unknown flags does not crash
// ---------------------------------------------------------------------------

test("tac with unknown flags does not crash", async () => {
  // Unknown flags are silently ignored by the arg parser.
  // With --help appended, we get a clean exit path to test.
  const result = await runTac(["--some-unknown-flag", "--help"]);
...
// ---------------------------------------------------------------------------
// 11. tac -v is equivalent to --version
// ---------------------------------------------------------------------------

test("tac -v is equivalent to --version", async () => {
  const result = await runTac(["-v"]);
...
// ---------------------------------------------------------------------------
// 12. tac -h is equivalent to --help
// ---------------------------------------------------------------------------

test("tac -h is equivalent to --help", async () => {
  const result = await runTac(["-h"]);
...
// ---------------------------------------------------------------------------
// 13. tac headless without .tac/ directory exits 1 with clean error
// ---------------------------------------------------------------------------

test("tac headless without .tac/ directory exits 1 with clean error", async (t) => {
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-e2e-no-tac-"));

  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });

  const result = await runTac(["headless"], 10_000, {}, tmpDir);
  ...
  const combined = stripAnsi(result.stdout + result.stderr);
  assert.ok(
    combined.includes(".tac/") || combined.includes("No .tac"),
    `expected .tac/ missing error, got:\n${combined.slice(0, 500)}`,
  );
...
// ---------------------------------------------------------------------------
// 14. tac headless new-milestone without --context exits 1
// ---------------------------------------------------------------------------

test("tac headless new-milestone without --context exits 1", async (t) => {
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-e2e-no-ctx-"));

  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });

  const result = await runTac(["headless", "new-milestone"], 10_000, {}, tmpDir);
...
// ---------------------------------------------------------------------------
// 15. tac headless --timeout with invalid value exits 1
// ---------------------------------------------------------------------------

test("tac headless --timeout with invalid value exits 1", async (t) => {
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-e2e-bad-timeout-"));
...
// ---------------------------------------------------------------------------
// 16. tac headless --timeout with negative value exits 1
// ---------------------------------------------------------------------------

test("tac headless --timeout with negative value exits 1", async (t) => {
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-e2e-neg-timeout-"));
...
test("tac headless query returns JSON from the built CLI", async (t) => {
  const tmpDir = createTempGitRepo("tac-e2e-query-");

  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });

  mkdirSync(join(tmpDir, ".tac", "milestones"), { recursive: true });
  ...
  const result = await runTac(["headless", "query"], 30_000, {}, tmpDir);
...
test("tac worktree list loads the built worktree CLI without module errors", async (t) => {
  const tmpDir = createTempGitRepo("tac-e2e-worktree-");

  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });

  // Cold packaged startup in a fresh temp repo is now regularly >10s because
  // the built CLI loads bundled TS resources through jiti before listing.
  const result = await runTac(["worktree", "list"], 30_000, {}, tmpDir);
...
test("tac --help lists all documented subcommands", async () => {
  const result = await runTac(["--help"]);
...
test("tac --help lists all key flags", async () => {
  const result = await runTac(["--help"]);
...
// ---------------------------------------------------------------------------
// 19. tac --version followed by other flags still just prints version
// ---------------------------------------------------------------------------

test("tac --version ignores trailing arguments", async () => {
  const result = await runTac(["--version", "--help", "--list-models"]);
...
// ---------------------------------------------------------------------------
// 20. tac headless help (positional, not flag) exits 0
// ---------------------------------------------------------------------------

test("tac headless help (positional) exits cleanly", async () => {
  // "help" as a positional is treated as a quick command by headless mode.
  // Without .tac/ it should fail, but with --help flag it should succeed.
  const result = await runTac(["headless", "--help"]);
