...
function makeWorkspaceFixture(label: string): { projectCwd: string; sessionsDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), `tac-multi-project-${label}-`));
            ...
            tasks: [
              {
                ...
                planPath: ".tac/milestones/M001/slices/S01/tasks/T01-PLAN.md",
              },
            ],
          },
        ],
...
test("multi-project: getProjectBridgeServiceForCwd returns distinct instances for different project paths", async (t) => {
  ...
  bridge.configureBridgeServiceForTests({
    env: {
      ...process.env,
      TAC_WEB_PROJECT_CWD: fixtureA.projectCwd,
      TAC_WEB_PROJECT_SESSIONS_DIR: fixtureA.sessionsDir,
...
test("multi-project: resolveProjectCwd falls back to TAC_WEB_PROJECT_CWD when no ?project= present", (t) => {
  bridge.configureBridgeServiceForTests({
    env: {
      ...process.env,
      TAC_WEB_PROJECT_CWD: "/fallback/path",
      TAC_WEB_PACKAGE_ROOT: repoRoot,
    },
    spawn: createHarness("unused").spawn,
    indexWorkspace: async () => fakeWorkspaceIndex(),
