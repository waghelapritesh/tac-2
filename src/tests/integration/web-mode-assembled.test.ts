...
function makeWorkspaceFixture(): { projectCwd: string; sessionsDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "tac-web-assembled-"));
  const projectCwd = join(root, "project");
  const sessionsDir = join(root, "sessions");
  const milestoneDir = join(projectCwd, ".tac", "milestones", "M001");
        ...
        slices: [
          {
            ...
            tasksDir: ".tac/milestones/M001/slices/S01/tasks",
            tasks: [{ id: "T01", title: "Work", done: false, planPath: ".tac/milestones/M001/slices/S01/tasks/T01-PLAN.md" }],
          },
        ],
      },
    ],
...
test("assembled slash-command behavior keeps built-ins safe while preserving TAC prompt commands", async (t) => {
  const fixture = makeWorkspaceFixture();
  ...
  // /tac status is now a browser surface (S02), verify that
  const tacSurface = await submitBrowserInput("/tac status");
  assert.equal(tacSurface.outcome.kind, "surface");
  assert.equal(tacSurface.outcome.surface, "tac-status");
  assert.equal(tacSurface.status, null);

  // /tac auto is a passthrough subcommand — reaches the bridge as a prompt
  const tacPrompt = await submitBrowserInput("/tac auto");
  assert.equal(tacPrompt.outcome.kind, "prompt");
  assert.equal(tacPrompt.status, 200);
  assert.equal(tacPrompt.body.command, "prompt");
  ...
  assert.equal(promptCommand?.message, "/tac auto", "TAC passthrough commands must stay on the extension prompt path");
});
