...
test("resolveTypeStrippingFlag returns --experimental-strip-types for paths outside node_modules", () => {
  const flag = resolveTypeStrippingFlag("/home/user/projects/tac")
...
test("resolveTypeStrippingFlag returns --experimental-strip-types for path with node_modules substring not as directory", () => {
  // e.g. /home/user/my_node_modules_backup/tac — not actually under node_modules/
  const flag = resolveTypeStrippingFlag("/home/user/my_node_modules_backup/tac")
  assert.equal(flag, "--experimental-strip-types")
})
...
test(
  ...
  () => {
    const flag = resolveTypeStrippingFlag("/usr/lib/node_modules/tac-2")
