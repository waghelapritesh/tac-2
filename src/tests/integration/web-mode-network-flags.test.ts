...
test('launchWebMode forwards custom host, port, and allowed origins to subprocess env', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-web-net-'))
  ...
  const status = await webMode.launchWebMode(
    {
      cwd: '/tmp/project',
      projectSessionsDir: '/tmp/.tac/sessions',
  ...
  assert.equal(spawnEnv!.TAC_WEB_HOST, '0.0.0.0')
  assert.equal(spawnEnv!.TAC_WEB_PORT, '8080')
  assert.equal(spawnEnv!.TAC_WEB_ALLOWED_ORIGINS, 'http://192.168.1.10:8080,http://tailscale-host:8080')
})

test('launchWebMode omits TAC_WEB_ALLOWED_ORIGINS when none provided', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-web-no-origins-'))
  const standaloneRoot = join(tmp, 'dist', 'web', 'standalone')
  const serverPath = join(standaloneRoot, 'server.js')
  mkdirSync(standaloneRoot, { recursive: true })
  writeFileSync(serverPath, 'console.log("stub")\n')
