...
test("launchWebMode passes windowsHide: true in spawn options", async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-web-winhide-"));
  ...
  const status = await webMode.launchWebMode(
    {
      ...
      projectSessionsDir: "/tmp/.tac/sessions/winhide",
...
test("launchWebMode source-dev host also passes windowsHide: true", async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-web-winhide-src-"));
  ...
  const status = await webMode.launchWebMode(
    {
      ...
      projectSessionsDir: "/tmp/.tac/sessions/winhide-src",
      agentDir: "/tmp/.tac/agent",
      packageRoot: tmp,
    },
    {
