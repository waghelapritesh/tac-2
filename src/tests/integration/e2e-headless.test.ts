/**
 * E2E integration tests for `tac headless` runtime behavior.
 *
 * Spawns real `tac headless` child processes and asserts on
...
/** Bootstrap a temp directory with .tac/ structure (milestones + runtime). */
function createTempWithTac(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), prefix));
  mkdirSync(join(dir, ".tac", "milestones"), { recursive: true });
  mkdirSync(join(dir, ".tac", "runtime"), { recursive: true });
  return dir;
}
...
test("headless --output-format json emits a single HeadlessJsonResult on stdout", async (t) => {
  const tmpDir = createTempWithTac("tac-e2e-json-batch-");
...
test("headless exits with code 11 after SIGINT", async (t) => {
  ...
  // (e.g. an existing tac session forced an immediate auto-mode conflict exit).
...
test("headless --output-format stream-json emits NDJSON on stdout", async (t) => {
  const tmpDir = createTempWithTac("tac-e2e-stream-json-");
  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });
...
test("headless --resume with nonexistent ID exits 1 with descriptive error", async (t) => {
  const tmpDir = createTempWithTac("tac-e2e-resume-bad-");
  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });
...
test("headless --output-format with invalid value exits 1", async (t) => {
  const tmpDir = createTempWithTac("tac-e2e-bad-format-");
  t.after(() => { rmSync(tmpDir, { recursive: true, force: true }); });
