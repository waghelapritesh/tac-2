...
test("session ID derivation: command sessions are also project-scoped", () => {
  const idA = deriveSessionId("/projects/alpha", undefined, "tac");
  const idB = deriveSessionId("/projects/beta", undefined, "tac");
  assert.notEqual(idA, idB);
  assert.ok(idA.includes("tac-default"), "Uses tac-default base for command sessions");
});
