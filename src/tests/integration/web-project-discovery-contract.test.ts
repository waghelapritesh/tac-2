...
const tempRoot = mkdtempSync(join(tmpdir(), "tac-project-discovery-"));
...
// project-b: empty-tac (.tac folder, no milestones)
const projectB = join(tempRoot, "project-b");
mkdirSync(projectB);
mkdirSync(join(projectB, ".tac"));
...
const monorepoPnpm = mkdtempSync(join(tmpdir(), "tac-mono-pnpm-"));
...
const monorepoLerna = mkdtempSync(join(tmpdir(), "tac-mono-lerna-"));
...
const monorepoWorkspaces = mkdtempSync(join(tmpdir(), "tac-mono-ws-"));
...
const monorepoTurbo = mkdtempSync(join(tmpdir(), "tac-mono-turbo-"));
...
const monorepoNx = mkdtempSync(join(tmpdir(), "tac-mono-nx-"));
...
const plainProject = mkdtempSync(join(tmpdir(), "tac-plain-project-"));
...
describe("project-discovery", () => {
  ...
  test("project-b is detected as empty-tac", () => {
    const results = discoverProjects(tempRoot);
    const b = results.find(r => r.name === "project-b");
    assert.ok(b, "project-b not found");
    assert.equal(b.kind, "empty-tac");
    assert.equal(b.signals.hasTacFolder, true);
  });
...
describe("project-discovery with monorepo root as devRoot", () => {
  ...
  test("monorepo entry has correct kind (brownfield when no .tac)", () => {
    const results = discoverProjects(monorepoPnpm);
