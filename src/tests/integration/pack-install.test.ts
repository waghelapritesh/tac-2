...
test("npm pack produces tarball with required files", async (t) => {
  const sandbox = createNpmSandbox("tac-pack-test-");
  ...
  assert.ok(files.some(f => f.includes("src/resources/extensions/tac/index.ts")), "tarball contains bundled tac extension");
  ...
  assert.equal(pkg.piConfig?.name, "tac", "pkg/package.json piConfig.name is tac");
  assert.equal(pkg.piConfig?.configDir, ".tac", "pkg/package.json piConfig.configDir is .tac");
});

// ═══════════════════════════════════════════════════════════════════════════
// 2. npm pack → install → tac binary resolves
// ═══════════════════════════════════════════════════════════════════════════

test("tarball installs and tac binary resolves", async (t) => {
  const sandbox = createNpmSandbox("tac-install-test-");
  const tarballPath = packTarball(sandbox);
  ...
  // Verify the tac bin exists in the installed package
  const binName = process.platform === "win32" ? "tac.cmd" : "tac";
  const installedBin = join(sandbox.installPrefix, "node_modules", ".bin", binName);
  assert.ok(existsSync(installedBin), `tac binary exists in node_modules/.bin/ (${binName})`);

  // Verify loader.js is executable (has shebang)
  const installedLoader = join(sandbox.installPrefix, "node_modules", "tac-2", "dist", "loader.js");
  ...
  assert.ok(existsSync(installedTacExt), "bundled tac extension present in installed package");
});
...
test("tac launches and loads extensions without errors", async () => {
  // Launch tac with all optional keys set (skip wizard) and capture stderr.
  ...
  assert.ok(
    !output.includes("[tac] Extension load error"),
...
test("tac exits early with a clear message when synced resources are newer than the binary", async (t) => {
  const fakeHome = mkdtempSync(join(tmpdir(), "tac-version-skew-"));
  const fakeAgentDir = join(fakeHome, ".tac", "agent");
  mkdirSync(fakeAgentDir, { recursive: true });
  writeFileSync(
    join(fakeAgentDir, "managed-resources.json"),
    JSON.stringify({ tacVersion: "999.0.0" }),
  );
  ...
  assert.match(result.stderr, /npm install -g tac-2@latest|tac update/, "prints upgrade guidance");
  assert.doesNotMatch(result.stderr, /\[tac\] Extension load error/, "fails before extension loading");
});
