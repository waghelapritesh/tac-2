...
const tacExtension = await import("../../resources/extensions/tac/index.ts")
...
async function collectRegisteredTacCommandRoots(): Promise<string[]> {
  const commands = new Map<string, unknown>()

  await tacExtension.default({
...
test("registered TAC command roots stay on the prompt/extension path", async () => {
  const registeredRoots = await collectRegisteredTacCommandRoots()
  assert.deepEqual(
    registeredRoots,
    ["exit", "tac", "kill", "worktree", "wt"],
    "browser parity contract only expects the current TAC command roots",
  )

  // Non-tac roots are extension commands that pass through to the bridge.
  // Derived dynamically so adding a new registration fails this assertion loudly.
  const nonTacRoots = registeredRoots.filter((r) => r !== "tac")
  assert.equal(nonTacRoots.length, 4, "expected exactly 4 non-tac passthrough roots; update this count when adding registrations")
  for (const root of nonTacRoots) {
    assertPromptPassthrough(`/${root}`)
  }

  // Bare /tac passes through to bridge (equivalent to /tac next)
  const bareTac = dispatchBrowserSlashCommand("/tac")
  assert.equal(bareTac.kind, "prompt", "bare /tac should pass through to bridge")
  assert.equal(bareTac.command.message, "/tac", "bare /tac should preserve exact input")
})

test("current TAC command family samples dispatch to correct outcomes after S02", async (t) => {
  await t.test("/tac (bare) still passes through to bridge", () => {
    assertPromptPassthrough("/tac")
  })

  await t.test("/tac status now dispatches to surface", () => {
    const outcome = dispatchBrowserSlashCommand("/tac status")
    assert.equal(outcome.kind, "surface", "/tac status should dispatch to surface after T01")
    assert.equal(outcome.surface, "tac-status")
  })
  ...
  await t.test("/tac status dispatches to surface regardless of streaming state", () => {
    const streaming = dispatchBrowserSlashCommand("/tac status", { isStreaming: true })
    assert.equal(streaming.kind, "surface", "/tac status should be surface even when streaming")
    assert.equal(streaming.surface, "tac-status")

    const idle = dispatchBrowserSlashCommand("/tac status", { isStreaming: false })
    assert.equal(idle.kind, "surface")
    assert.equal(idle.surface, "tac-status")
  })
})

const EXPECTED_TAC_OUTCOMES = new Map<string, "surface" | "prompt" | "local" | "view-navigate">([
  // Surface commands (19)
  ["status", "surface"],
...
test("every registered /tac subcommand has an explicit browser dispatch outcome", async (t) => {
  assert.equal(
    EXPECTED_TAC_OUTCOMES.size,
    30,
    "EXPECTED_TAC_OUTCOMES must cover all 30 TAC subcommands (19 surface + 1 view-navigate + 9 passthrough + 1 help)",
  )

  for (const [subcommand, expectedKind] of EXPECTED_TAC_OUTCOMES) {
    await t.test(`/tac ${subcommand} -> ${expectedKind}`, () => {
      const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`)
      assert.equal(
        outcome.kind,
        expectedKind,
        `/tac ${subcommand} should dispatch to ${expectedKind}, got ${outcome.kind}`,
      )
    })

    if (expectedKind === "surface") {
      await t.test(`/tac ${subcommand} opens tac-${subcommand} surface`, () => {
        const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`) as any
        assert.equal(outcome.surface, `tac-${subcommand}`, `/tac ${subcommand} should open the tac-${subcommand} surface`)
      })
    }

    if (expectedKind === "prompt") {
      await t.test(`/tac ${subcommand} preserves exact input text`, () => {
        const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`) as any
        assert.equal(outcome.command.message, `/tac ${subcommand}`, `/tac ${subcommand} should preserve exact input text for bridge delivery`)
      })
    }

    if (expectedKind === "local") {
      await t.test(`/tac ${subcommand} dispatches to tac_help action`, () => {
        const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`) as any
        assert.equal(outcome.action, "tac_help", `/tac ${subcommand} should dispatch to tac_help action`)
      })
    }

    if (expectedKind === "view-navigate") {
      await t.test(`/tac ${subcommand} navigates to the ${subcommand} view`, () => {
        const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`) as any
        assert.equal(outcome.view, subcommand, `/tac ${subcommand} should navigate to the ${subcommand} view`)
      })
    }
  }
})

test("TAC dispatch edge cases", async (t) => {
  await t.test("/tac (bare, no subcommand) passes through to bridge", () => {
    const outcome = dispatchBrowserSlashCommand("/tac")
    assert.equal(outcome.kind, "prompt")
    assert.equal(outcome.command.message, "/tac")
  })

  await t.test("/tac help dispatches to local tac_help action", () => {
    const outcome = dispatchBrowserSlashCommand("/tac help")
    assert.equal(outcome.kind, "local")
    assert.equal(outcome.action, "tac_help")
  })

  await t.test("/tac unknown-xyz passes through to bridge", () => {
    const outcome = dispatchBrowserSlashCommand("/tac unknown-xyz")
    assert.equal(outcome.kind, "prompt", "unknown subcommand should pass through to bridge")
    assert.equal(outcome.command.message, "/tac unknown-xyz", "unknown subcommand should preserve exact input")
    assert.equal(outcome.slashCommandName, "tac", "unknown subcommand should identify as tac command")
  })

  await t.test("/export is built-in session export, not tac-export", () => {
  ...
  await t.test("/tac export is TAC milestone export, distinct from built-in /export", () => {
    const outcome = dispatchBrowserSlashCommand("/tac export")
    assert.equal(outcome.kind, "surface")
    assert.equal(outcome.surface, "tac-export", "/tac export should be the TAC milestone export surface")
  })

  await t.test("/tac forensics detailed preserves sub-args", () => {
    const outcome = dispatchBrowserSlashCommand("/tac forensics detailed")
    assert.equal(outcome.kind, "surface")
    assert.equal(outcome.surface, "tac-forensics")
    assert.equal(outcome.args, "detailed", "sub-args after subcommand should be preserved")
  })

  await t.test("TAC surface commands produce system terminal notice", () => {
    const outcome = dispatchBrowserSlashCommand("/tac status")
    const notice = getBrowserSlashCommandTerminalNotice(outcome)
  ...
  await t.test("TAC passthrough commands produce no terminal notice", () => {
    const outcome = dispatchBrowserSlashCommand("/tac auto")
    const notice = getBrowserSlashCommandTerminalNotice(outcome)
    assert.equal(notice, null, "passthrough outcome should produce no terminal notice")
  })
})

test("every TAC surface dispatches through the contract wiring end-to-end", async (t) => {
  const tacSurfaces = [...EXPECTED_TAC_OUTCOMES.entries()].filter(([, kind]) => kind === "surface")

  assert.equal(tacSurfaces.length, 19, "should have exactly 19 TAC surface subcommands")

  for (const [subcommand] of tacSurfaces) {
    await t.test(`/tac ${subcommand} -> dispatch -> open request -> surface state`, () => {
      const outcome = dispatchBrowserSlashCommand(`/tac ${subcommand}`)
      assert.equal(outcome.kind, "surface")
      ...
      assert.equal(state.open, true, `surface state should be open for tac-${subcommand}`)
      assert.ok(state.section, `surface state should have a non-null section for tac-${subcommand}`)
      assert.equal(state.section, `tac-${subcommand}`, `section should match tac-${subcommand}`)
      assert.ok(state.selectedTarget, `surface state should have a non-null selectedTarget for tac-${subcommand}`)
      assert.equal(state.selectedTarget.kind, "tac", `target kind should be "tac" for tac-${subcommand}`)
      assert.equal(state.selectedTarget.subcommand, subcommand, `target subcommand should be "${subcommand}"`)
    })
  }
})

test("/tac visualize dispatches as view-navigate to the visualizer view", () => {
  const outcome = dispatchBrowserSlashCommand("/tac visualize")
...
test("command-surface session affordances use the shared store action path", () => {
  const commandSurfacePath = resolve(import.meta.dirname, "../../../web/components/tac/command-surface.tsx")
