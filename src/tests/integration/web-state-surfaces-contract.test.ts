...
// ─── Helpers ──────────────────────────────────────────────────────────
function makeTacFixture(): { root: string; tacDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "tac-state-surfaces-"));
  const tacDir = join(root, ".tac");
...
test("indexWorkspace extracts risk, depends, and demo from roadmap", async (t) => {
  const { root, tacDir, cleanup } = makeTacFixture();

  t.after(() => { cleanup(); });

  const milestoneDir = join(tacDir, "milestones", "M001");
...
test("files API returns tree listing of .tac/ directory", async (t) => {
  const { root, tacDir, cleanup } = makeTacFixture();
  const origEnv = process.env.TAC_WEB_PROJECT_CWD;

  t.after(() => {
    process.env.TAC_WEB_PROJECT_CWD = origEnv;
    cleanup();
  });

  process.env.TAC_WEB_PROJECT_CWD = root;

  // Create some files
  writeFileSync(join(tacDir, "STATE.md"), "# State\nactive");
  writeFileSync(join(tacDir, "PROJECT.md"), "# Project");
  const msDir = join(tacDir, "milestones", "M001");
...
test("files API returns file content for valid path", async (t) => {
  ...
  writeFileSync(join(tacDir, "STATE.md"), fileContent);
...
test("files API rejects path traversal with ../", async (t) => {
  const { root, cleanup } = makeTacFixture();
  const origEnv = process.env.TAC_WEB_PROJECT_CWD;
...
test("files API returns empty tree when .tac/ does not exist", async (t) => {
  const root = mkdtempSync(join(tmpdir(), "tac-state-surfaces-empty-"));
  const origEnv = process.env.TAC_WEB_PROJECT_CWD;
...
const VIEW_FILES = [
  "web/components/tac/dashboard.tsx",
  "web/components/tac/roadmap.tsx",
  "web/components/tac/activity-view.tsx",
  "web/components/tac/files-view.tsx",
  "web/components/tac/dual-terminal.tsx",
];
...
const MOCK_DATA_PATTERNS = [
  ...
  /const\s+tacFiles\s*=\s*\[/,            // const tacFiles = [...]
...
test("view components read from real data sources (store or API)", () => {
  ...
  const STORE_VIEWS = [
    ...
    "web/components/tac/roadmap.tsx",
    "web/components/tac/activity-view.tsx",
    "web/components/tac/terminal.tsx",
  ];

  // FilesView fetches from /api/files (real endpoint), not the workspace store — that's correct
  const API_VIEWS = [
    { path: "web/components/tac/files-view.tsx", apiPattern: "/api/files" },
  ];
  ...
  for (const filePath of STORE_VIEWS) {
    ...
    const source = readFileSync(fullPath, "utf-8");
    assert.ok(
      source.includes("tac-workspace-store"),
      `${filePath} does not import from tac-workspace-store — store-backed views must read real store state`,
    );
  }
...
test("status bar consumes statusTexts from store", () => {
  const statusBarPath = resolve(import.meta.dirname, "../../../web/components/tac/status-bar.tsx");
...
test("browser shell renders title overrides, widgets, and editor prefills from store-backed state", () => {
  const storePath = resolve(import.meta.dirname, "../../../web/lib/tac-workspace-store.tsx");
  const appShellPath = resolve(import.meta.dirname, "../../../web/components/tac/app-shell.tsx");
  const statusBarPath = resolve(import.meta.dirname, "../../../web/components/tac/status-bar.tsx");
  const terminalPath = resolve(import.meta.dirname, "../../../web/components/tac/terminal.tsx");
  ...
  assert.match(storeSource, /consumeEditorTextBuffer = \(\): string \| null =>/, "tac-workspace-store.tsx must expose a consume-once editor prefill action");
...
test("chat tool blocks normalize Claude Code tool names before choosing built-in render treatment", () => {
  const chatPath = resolve(import.meta.dirname, "../../../web/components/tac/chat-mode.tsx");
...
test("live browser panels consume live selectors and expose inspectable freshness markers", () => {
  ...
  const storePath = resolve(import.meta.dirname, "../../../web/lib/tac-workspace-store.tsx")
  const dashboardPath = resolve(import.meta.dirname, "../../../web/components/tac/dashboard.tsx")
  const sidebarPath = resolve(import.meta.dirname, "../../../web/components/tac/sidebar.tsx")
  const roadmapPath = resolve(import.meta.dirname, "../../../web/components/tac/roadmap.tsx")
  const statusBarPath = resolve(import.meta.dirname, "../../../web/components/tac/status-bar.tsx")
  ...
  assert.match(storeSource, /live_state_invalidation/, "tac-workspace-store.tsx must handle typed live_state_invalidation events")
  assert.match(storeSource, /\/api\/live-state/, "tac-workspace-store.tsx must use the narrow live-state route for targeted refreshes")
  assert.match(storeSource, /softBootRefreshCount/, "tac-workspace-store.tsx must expose a soft boot refresh counter for observability")
  assert.match(storeSource, /targetedRefreshCount/, "tac-workspace-store.tsx must expose a targeted refresh counter for observability")
  assert.match(storeSource, /getLiveWorkspaceIndex/, "tac-workspace-store.tsx must expose a live workspace selector")
  assert.match(storeSource, /getLiveAutoDashboard/, "tac-workspace-store.tsx must expose a live auto selector")
  assert.match(storeSource, /getLiveResumableSessions/, "tac-workspace-store.tsx must expose a live resumable-sessions selector")
...
test("workflow action surfaces route new-milestone CTAs through the shared command path", () => {
  ...
  assert.doesNotMatch(chatSource, /buildPromptCommand\("\/tac auto", bridge\)/, "chat-mode.tsx must not hardcode a special /tac auto path for new-milestone CTA dispatch")
})

test("sidebar Git affordance opens a real git-summary surface with visible repo/not-repo/error states", () => {
  const contractPath = resolve(import.meta.dirname, "../../../web/lib/command-surface-contract.ts");
  const storePath = resolve(import.meta.dirname, "../../../web/lib/tac-workspace-store.tsx");
  const surfacePath = resolve(import.meta.dirname, "../../../web/components/tac/command-surface.tsx");
  const sidebarPath = resolve(import.meta.dirname, "../../../web/components/tac/sidebar.tsx");
  ...
  assert.match(storeSource, /loadGitSummary/, "tac-workspace-store.tsx must expose loadGitSummary so the Git surface is not inert");
  assert.match(storeSource, /\/api\/git/, "tac-workspace-store.tsx must fetch the current-project git route for the Git surface");
...
test("recovery diagnostics surface stays on a dedicated route with explicit stale and action state", () => {
  ...
  assert.match(storeSource, /loadRecoveryDiagnostics = async/, "tac-workspace-store.tsx must expose a recovery diagnostics loader");
  assert.match(storeSource, /\/api\/recovery/, "tac-workspace-store.tsx must call the dedicated recovery route");
  assert.match(storeSource, /markRecoveryStateInvalidated/, "tac-workspace-store.tsx must keep recovery diagnostics stale state inspectable after invalidation");
