...
const { TACWorkspaceStore } = await import("../../../web/lib/tac-workspace-store.tsx")
...
describe("diagnostics type exports", () => {
  ...
  it("ForensicReport has all required fields", () => {
    ...
    assert.equal(typeof report.tacVersion, "string")
...
describe("diagnostics dispatch→surface pipeline", () => {
  it("/tac forensics dispatches to tac-forensics surface", () => {
    const outcome = dispatchBrowserSlashCommand("/tac forensics", {})
  ...
  it("/tac doctor dispatches to tac-doctor surface", () => {
    const outcome = dispatchBrowserSlashCommand("/tac doctor", {})
    assert.equal(outcome.kind, "surface")
    if (outcome.kind === "surface") {
      assert.equal(outcome.surface, "tac-doctor")
    }
  })

  it("/tac skill-health dispatches to tac-skill-health surface", () => {
    const outcome = dispatchBrowserSlashCommand("/tac skill-health", {})
    assert.equal(outcome.kind, "surface")
    if (outcome.kind === "surface") {
      assert.equal(outcome.surface, "tac-skill-health")
    }
  })

  it("/tac doctor fix dispatches to tac-doctor surface with args", () => {
    const outcome = dispatchBrowserSlashCommand("/tac doctor fix", {})
    assert.equal(outcome.kind, "surface")
    if (outcome.kind === "surface") {
      assert.equal(outcome.surface, "tac-doctor")
    }
...
describe("diagnostics surface→section mapping", () => {
  it("tac-forensics surface maps to tac-forensics section", () => {
    const section = commandSurfaceSectionForRequest({ surface: "tac-forensics" as any } as any)
    assert.equal(section, "tac-forensics")
  })

  it("tac-doctor surface maps to tac-doctor section", () => {
    const section = commandSurfaceSectionForRequest({ surface: "tac-doctor" as any } as any)
    assert.equal(section, "tac-doctor")
  })

  it("tac-skill-health surface maps to tac-skill-health section", () => {
    const section = commandSurfaceSectionForRequest({ surface: "tac-skill-health" as any } as any)
    assert.equal(section, "tac-skill-health")
  })
})
...
// names exist on TACWorkspaceStore, then do a runtime check that the class
...
type _AssertLoadForensics = InstanceType<typeof TACWorkspaceStore>["loadForensicsDiagnostics"]
type _AssertLoadDoctor = InstanceType<typeof TACWorkspaceStore>["loadDoctorDiagnostics"]
type _AssertApplyFixes = InstanceType<typeof TACWorkspaceStore>["applyDoctorFixes"]
type _AssertLoadSkillHealth = InstanceType<typeof TACWorkspaceStore>["loadSkillHealthDiagnostics"]

describe("diagnostics store methods", () => {
  it("TACWorkspaceStore is a constructable class export", () => {
    assert.equal(typeof TACWorkspaceStore, "function", "TACWorkspaceStore should be a class/function export")
  })
  ...
  it("loadForensicsDiagnostics is a recognized method name on the store type", () => {
    ...
    // checking the useTACWorkspaceActions hook references it in the exports.
    const methodName: keyof Pick<InstanceType<typeof TACWorkspaceStore>, "loadForensicsDiagnostics"> = "loadForensicsDiagnostics"
    assert.equal(methodName, "loadForensicsDiagnostics")
  })

  it("loadDoctorDiagnostics is a recognized method name on the store type", () => {
    const methodName: keyof Pick<InstanceType<typeof TACWorkspaceStore>, "loadDoctorDiagnostics"> = "loadDoctorDiagnostics"
    assert.equal(methodName, "loadDoctorDiagnostics")
  })

  it("applyDoctorFixes is a recognized method name on the store type", () => {
    const methodName: keyof Pick<InstanceType<typeof TACWorkspaceStore>, "applyDoctorFixes"> = "applyDoctorFixes"
    assert.equal(methodName, "applyDoctorFixes")
  })

  it("loadSkillHealthDiagnostics is a recognized method name on the store type", () => {
    const methodName: keyof Pick<InstanceType<typeof TACWorkspaceStore>, "loadSkillHealthDiagnostics"> = "loadSkillHealthDiagnostics"
