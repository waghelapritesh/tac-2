...
function makeWorkspaceFixture(): { projectCwd: string; sessionsDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "tac-web-onboarding-integration-"));
        ...
        slices: [
          {
            ...
            planPath: ".tac/milestones/M001/slices/S02/S02-PLAN.md",
            tasksDir: ".tac/milestones/M001/slices/S02/tasks",
            tasks: [
              {
                id: "T02",
                title: "Enforce the gate and refresh bridge auth after successful setup",
                done: false,
                planPath: ".tac/milestones/M001/slices/S02/tasks/T02-PLAN.md",
              },
            ],
          },
        ],
...
test("fresh tac --web browser onboarding stays locked on failed validation and unlocks after a successful retry", async (t) => {
  if (process.platform === "win32") {
    t.skip("runtime launch test uses POSIX browser-open stubs")
    return
  }

  const tempRoot = mkdtempSync(join(tmpdir(), "tac-web-onboarding-runtime-"))
  ...
  const launch = await launchPackagedWebHost({
    ...
    browserLogPath,
    env: {
      TAC_WEB_TEST_FAKE_API_KEY_VALIDATION: "1",
      TAC_WEB_TEST_DISABLE_EXTERNAL_CLI: "1",
      ANTHROPIC_API_KEY: "",
      OPENAI_API_KEY: "",
      GOOGLE_API_KEY: "",
    },
