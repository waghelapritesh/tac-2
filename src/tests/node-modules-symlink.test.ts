...
test("initResources creates node_modules symlink in agent dir", async (t) => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-symlink-"));
...
test("initResources replaces a real directory blocking node_modules with a symlink", async (t) => {
  const { initResources } = await import("../resource-loader.ts");
  const tmp = mkdtempSync(join(tmpdir(), "tac-symlink-realdir-"));
  const fakeAgentDir = join(tmp, "agent");
...
test("pnpm layout: merged node_modules contains entries from both hoisted and internal", (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-pnpm-merge-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));
  ...
  mkdirSync(join(internal, "@tac", "pi-ai"), { recursive: true });
  mkdirSync(join(internal, "@tac", "pi-coding-agent"), { recursive: true });
  mkdirSync(join(internal, "@waghelapritesh", "core"), { recursive: true });
  ...
  assert.ok(existsSync(join(agentNodeModules, "@tac")), "@tac should resolve");
  assert.ok(existsSync(join(agentNodeModules, "@tac", "pi-ai")), "@tac/pi-ai should resolve");
  assert.ok(existsSync(join(agentNodeModules, "@waghelapritesh")), "@waghelapritesh should resolve");
  ...
  // @tac points to internal, not hoisted (internal overlay precedence)
  const tacTarget = readlinkSync(join(agentNodeModules, "@tac"));
  assert.equal(tacTarget, join(internal, "@tac"), "@tac should point to internal node_modules");
});

test("pnpm layout: non-@tac internal deps (e.g. @anthropic-ai) are included in merged dir", (t) => {
  // Regression: PR #3564 narrowed the internal overlay to @tac* only,
  // dropping optionalDependencies like @anthropic-ai/claude-agent-sdk
  // that npm installs internally rather than hoisting.
  const tmp = mkdtempSync(join(tmpdir(), "tac-pnpm-internal-optional-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));
...
test("hasMissingWorkspaceScopes detects pnpm layout", (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-pnpm-detect-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));

  const hoisted = join(tmp, "hoisted");
  const internal = join(tmp, "internal");

  // npm-style: @tac exists in both hoisted and internal
  mkdirSync(join(hoisted, "@tac"), { recursive: true });
  mkdirSync(join(internal, "@tac"), { recursive: true });
  ...
  // pnpm-style: @waghelapritesh only in internal
  mkdirSync(join(internal, "@waghelapritesh"), { recursive: true });
  assert.equal(
    hasMissingWorkspaceScopes(hoisted, internal),
    true,
    "pnpm-style: @waghelapritesh missing from hoisted should be detected",
  );

  // Non-@tac scope missing from hoisted does NOT trigger detection —
  // only @tac* scopes are the contract signal.
  const tmp2 = mkdtempSync(join(tmpdir(), "tac-pnpm-detect-non-tac-"));
  ...
  assert.equal(
    ...
    false,
    "non-@tac scope should not trigger missing-workspace detection",
  );

  // Missing internal directory returns false (no layout to reason about)
  const tmp3 = mkdtempSync(join(tmpdir(), "tac-pnpm-detect-missing-"));
...
test("merged node_modules marker uses fingerprint including directory entries", (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-pnpm-marker-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));
  ...
  assert.ok(fingerprint.includes("@tac"), "fingerprint should include internal entries");
  ...
  const agentNodeModules = join(tmp, "agent", "node_modules");
  reconcileMergedNodeModules(agentNodeModules, hoisted, internal);
  const markerPath = join(agentNodeModules, ".tac-merged");
...
test("reconcileMergedNodeModules creates symlinks (OS-appropriate type) to the source entries", (t) => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-pnpm-symtype-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));
  ...
  const internalLink = join(agentNodeModules, "@tac");
