...
import { CURSOR_MARKER, TUI, type Component, type Terminal } from "@tac/pi-tui";
...
describe("TUI cursor tracking regression (#3764)", () => {
  ...
  it("handles editor-to-selector swap without cursor corruption", () => {
    // Simulates /tac prefs: editor with CURSOR_MARKER is replaced by
  ...
  it("handles input component swap (prefs wizard text input)", () => {
    // Simulates /tac prefs input step: selector replaced by text input with cursor
    const terminal = new MockTTYTerminal();
    const tui = new TUI(terminal, false);
