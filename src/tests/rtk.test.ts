...
import {
  buildRtkEnv,
  ensureRtkAvailable,
  TAC_RTK_DISABLED_ENV,
  TAC_RTK_PATH_ENV,
  TAC_SKIP_RTK_INSTALL_ENV,
  getManagedRtkDir,
  prependPathEntry,
  resolveRtkAssetName,
...
test("prependPathEntry preserves the original PATH key casing and avoids duplicates", () => {
  ...
  prependPathEntry(env, "/tmp/tac-bin");
  assert.equal(env.Path, `/tmp/tac-bin${delimiter}${"/usr/bin"}`);
  prependPathEntry(env, "/tmp/tac-bin");
  assert.equal(env.Path, `/tmp/tac-bin${delimiter}${"/usr/bin"}`);
});
...
test("rewriteCommandWithRtk respects the disable flag", () => {
  ...
  assert.equal(
    rewriteCommandWithRtk("git status", {
      binaryPath: "/tmp/rtk",
      spawnSyncImpl,
      env: { [TAC_RTK_DISABLED_ENV]: "1" },
    }),
    "git status",
  );
});

test("rewriteCommandWithRtk falls back to the managed RTK path when TAC_RTK_PATH is unset", () => {
...
test("ensureRtkAvailable respects explicit disable and skip flags without downloading", async () => {
  ...
  const skipped = await ensureRtkAvailable({
    env: {
      [TAC_SKIP_RTK_INSTALL_ENV]: "1",
      [TAC_RTK_PATH_ENV]: "/tmp/nonexistent-rtk",
    },
  });
