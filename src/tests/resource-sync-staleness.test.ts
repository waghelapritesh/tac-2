 ...
 * with a broken import to persist at ~/.tac/agent/extensions/).
 */
...
test("resource manifest includes contentHash", async (t) => {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-resource-test-"));
  ...
  assert.equal(read.tacVersion, "2.28.0");
...
test("missing contentHash in manifest triggers re-sync (upgrade path)", () => {
  ...
  const shouldSkip = oldManifest.tacVersion === "2.28.0"
...
test("matching contentHash skips re-sync", () => {
  ...
  const shouldSkip = manifest.tacVersion === "2.28.0"
