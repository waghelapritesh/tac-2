...
describe('sortExtensionPaths', () => {
  ...
  test('Test 4: missing dep — warns with correct format, extension still in output', (t) => {
    ...
    const pathA = makeExtension(dir, 'test.a', ['tac.nonexistent'])
    ...
    assert.equal(w.missingId, 'tac.nonexistent')
    assert.equal(w.message, "Extension 'test.a' declares dependency 'tac.nonexistent' which is not installed — loading anyway")
  })
