 ...
 *
 * When `tac auto` is run with piped stdout (e.g. `tac auto | cat`),
...
// ─── End-to-end scenario: tac auto | cat ──────────────────────────────────

test("scenario: 'tac auto 2>&1 | cat' — should redirect to headless", () => {
...
test("scenario: 'tac auto > /tmp/output.txt' — should redirect to headless", () => {
  const subcommand = "auto";
  const stdinIsTTY = true;
  const stdoutIsTTY = false;
...
test("scenario: 'tac auto' in terminal — normal interactive mode", () => {
  const subcommand = "auto";
  const stdinIsTTY = true;
...
test("scenario: 'echo msg | tac auto' — stdin piped, should redirect", () => {
  const subcommand = "auto";
...
test("scenario: 'echo msg | tac auto | cat' — both piped", () => {
  const subcommand = "auto";
  const stdinIsTTY = false;
  const stdoutIsTTY = false;
