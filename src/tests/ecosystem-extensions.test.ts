// TAC2 — Tests for the ecosystem extension wrapper (#3338)
...
import {
  ...
  mapAutoLoopPhase,
  updateSnapshot,
} from "../resources/extensions/tac/ecosystem/tac-extension-api.js";
...
function buildState(overrides: Partial<TACState> = {}): TACState {
  return {
...
test("createTACExtensionAPI intercepts before_agent_start", () => {
  const { pi, onCalls } = buildPiStub();
  const shared: TACEcosystemBeforeAgentStartHandler[] = [];
  const api = createTACExtensionAPI(pi, shared);

  const handler: TACEcosystemBeforeAgentStartHandler = async () => undefined;
...
test("createTACExtensionAPI delegates non-intercepted events to pi.on", () => {
  const { pi, onCalls } = buildPiStub();
  const shared: TACEcosystemBeforeAgentStartHandler[] = [];
  const api = createTACExtensionAPI(pi, shared);
...
test("getPhase / getActiveUnit read from module snapshot", () => {
  ...
  const api = createTACExtensionAPI(pi, []);
...
test("wrapper key-drift guard: every ExtensionAPI method is delegated", () => {
  ...
  // the `satisfies TACExtensionAPI` check will fail at compile time. This
