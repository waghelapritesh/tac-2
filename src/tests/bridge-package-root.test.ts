 ...
 * the CI runner's absolute Linux path (file:///home/runner/work/tac-2/tac-2/…).
 ...
 * falls through to the TAC_WEB_PACKAGE_ROOT env var.
 */
...
test('bridge-service resolveBridgeRuntimeConfig falls back to lazy default', () => {
  const source = readFileSync(join(process.cwd(), 'src', 'web', 'bridge-service.ts'), 'utf-8')
  assert.ok(
    source.includes('env.TAC_WEB_PACKAGE_ROOT || getDefaultPackageRoot()'),
