...
function makeTempDir(prefix: string): string {
	const dir = path.join(
		os.tmpdir(),
		`tac-windows-portability-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
...
test("Windows launch points use shell-safe shims", () => {
	const tacClient = readFileSync(
		path.join(process.cwd(), "vscode-extension", "src", "tac-client.ts"),
		"utf8",
	);
	...
	const preExecution = readFileSync(
		path.join(process.cwd(), "src", "resources", "extensions", "tac", "pre-execution-checks.ts"),
		"utf8",
	);
	...
	assert.match(tacClient, /shell:\s*process\.platform === "win32"/);
