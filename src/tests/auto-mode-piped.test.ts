/**
 * Tests for `tac auto` routing — verifies that `auto` is recognized as a
...
// ═══════════════════════════════════════════════════════════════════════════
// Core regression test: `tac auto` must be handled before TUI (#2732)
// ═══════════════════════════════════════════════════════════════════════════
...
test('cli.ts handles `auto` subcommand before interactive TUI (#2732)', () => {
  assert.ok(
    ...
    'reaching `new InteractiveMode()`. Without this, `tac auto` with ' +
...
test('TTY error message mentions `tac auto` as a non-interactive alternative', () => {
  const cliSource = readFileSync(join(projectRoot, 'src', 'cli.ts'), 'utf-8')

  // The TTY error message should mention auto as an alternative
  assert.ok(
    cliSource.includes('tac auto') || cliSource.includes('tac headless'),
    'TTY error hints should mention headless/auto mode as alternatives',
  )
})

// ═══════════════════════════════════════════════════════════════════════════
// `tac headless` still works (no regression)
// ═══════════════════════════════════════════════════════════════════════════
