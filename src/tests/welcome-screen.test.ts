...
test('renders version', () => {
  ...
  assert.ok(out.includes('Think. Architect. Code.'), 'brand name missing')
})

test('renders TAC project state or fallback hint', (t) => {
  // Model/provider intentionally removed from the welcome screen — they live
  // in the persistent footer. Without .tac/STATE.md present the welcome
  // should surface the "No active TAC project" fallback instead.
  // chdir into an empty tmp dir so the fallback path is actually exercised
  // regardless of what the repo we're running from has in .tac/.
  const tmp = mkdtempSync(join(tmpdir(), 'tac-welcome-fallback-'))
  ...
  assert.ok(
    out.includes('No active TAC project') || /Active\s+M\d+/.test(out),
    'welcome should show TAC state lines or the no-project fallback',
  )
})

test('renders cwd hint', () => {
  const out = strip(capture({ version: '1.0.0' }))
  assert.ok(out.includes('/tac to begin'), 'hint line missing')
})
...
test('Active row truncates with ellipsis when milestone text overflows panel width', (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-welcome-test-'))
  mkdirSync(join(tmp, '.tac'))
  writeFileSync(
    join(tmp, '.tac', 'STATE.md'),
    [
...
test('Active row does not truncate short milestone text', (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-welcome-test-'))
  mkdirSync(join(tmp, '.tac'))
  writeFileSync(join(tmp, '.tac', 'STATE.md'), '**Active Milestone:** M001: Short title\n')
  const origCwd = process.cwd()
  process.chdir(tmp)
  const origColumns = (process.stderr as any).columns
  ;(process.stderr as any).columns = 120
