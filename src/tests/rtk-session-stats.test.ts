...
beforeEach(() => {
  // Save and clear TAC_RTK_DISABLED so tests can use fake RTK binaries
  originalRtkDisabled = process.env.TAC_RTK_DISABLED;
  delete process.env.TAC_RTK_DISABLED;
});

afterEach(() => {
  // Restore original env
  if (originalRtkDisabled !== undefined) {
    process.env.TAC_RTK_DISABLED = originalRtkDisabled;
  } else {
    delete process.env.TAC_RTK_DISABLED;
  }
});
...
test("RTK session savings diff from a persisted baseline", () => {
  const basePath = mkdtempSync(join(tmpdir(), "tac-rtk-session-stats-"));
  mkdirSync(join(basePath, ".tac", "runtime"), { recursive: true });
  ...
  const previous = process.env.TAC_RTK_PATH;
  try {
    process.env.TAC_RTK_PATH = first.path;
    ensureRtkSessionBaseline(basePath, "sess-1");

    process.env.TAC_RTK_PATH = second.path;
    ...
    if (previous === undefined) delete process.env.TAC_RTK_PATH;
    else process.env.TAC_RTK_PATH = previous;
    first.cleanup();
    second.cleanup();
...
test("RTK session savings baseline resets cleanly when tracking totals go backwards", () => {
  const basePath = mkdtempSync(join(tmpdir(), "tac-rtk-session-reset-"));
  mkdirSync(join(basePath, ".tac", "runtime"), { recursive: true });
...
test("RTK session stats fall back to the managed RTK path when TAC_RTK_PATH is unset", () => {
  const basePath = mkdtempSync(join(tmpdir(), "tac-rtk-session-managed-"));
  mkdirSync(join(basePath, ".tac", "runtime"), { recursive: true });

  const fake = createFakeRtk({
    "gain --all --format json": { stdout: summary(6, 900, 500, 400) },
  });
  const managedHome = mkdtempSync(join(tmpdir(), "tac-rtk-home-"));
  ...
  const previousHome = process.env.TAC_HOME;
  const previousPath = process.env.TAC_RTK_PATH;

  try {
    process.env.TAC_HOME = managedHome;
    delete process.env.TAC_RTK_PATH;

    const env: NodeJS.ProcessEnv = {
      ...process.env,
      TAC_HOME: managedHome,
    };
    delete env.TAC_RTK_PATH;
    ...
    if (previousHome === undefined) delete process.env.TAC_HOME;
    else process.env.TAC_HOME = previousHome;
    if (previousPath === undefined) delete process.env.TAC_RTK_PATH;
    else process.env.TAC_RTK_PATH = previousPath;
    fake.cleanup();
...
test("clearRtkSessionBaseline removes a stored session entry", () => {
  const basePath = mkdtempSync(join(tmpdir(), "tac-rtk-session-clear-"));
  mkdirSync(join(basePath, ".tac", "runtime"), { recursive: true });
  const fake = createFakeRtk({
  ...
  try {
    process.env.TAC_RTK_PATH = fake.path;
