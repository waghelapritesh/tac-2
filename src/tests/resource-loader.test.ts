...
test("getExtensionKey normalizes top-level .ts and .js entry names to the same key", async () => {
  ...
  assert.equal(
    getExtensionKey("/tmp/extensions/tac/index.js", extensionsDir),
    "tac",
  );
});
...
test("hasStaleCompiledExtensionSiblings only flags top-level .ts/.js sibling pairs", async (t) => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-resource-loader-"));
  ...
  mkdirSync(join(extensionsDir, "tac"), { recursive: true });
  writeFileSync(join(extensionsDir, "tac", "index.ts"), "export {};\n");
...
test("buildResourceLoader excludes duplicate top-level pi extensions when bundled resources use .js", async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-resource-loader-home-"));
  const piExtensionsDir = join(tmp, ".pi", "agent", "extensions");
  const fakeAgentDir = join(tmp, ".tac", "agent");
...
test("initResources manifest tracks all bundled extension subdirectories including remote-questions (#2367)", async () => {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "tac-resource-loader-manifest-"));
...
test("initResources prunes stale top-level extension siblings next to bundled compiled extensions", async (t) => {
  const { initResources } = await import("../resource-loader.ts");
  const tmp = mkdtempSync(join(tmpdir(), "tac-resource-loader-sync-"));
  const fakeAgentDir = join(tmp, "agent");
...
test("pruneRemovedBundledExtensions removes stale subdirectory extensions not in current bundle", async () => {
  const { initResources } = await import("../resource-loader.ts");
  const tmp = mkdtempSync(join(tmpdir(), "tac-resource-loader-prune-dirs-"));
  const fakeAgentDir = join(tmp, "agent");
  ...
  try {
    ...
    // Simulate a stale subdirectory extension left from a previous TAC version.
    // This mirrors the mcporter scenario: it was bundled before, synced to
    // ~/.tac/agent/extensions/, then removed from the bundle in a newer version.
    ...
    // Bump the manifest version to force a re-sync (simulates upgrading TAC).
    manifest.tacVersion = "0.0.0-force-resync";
