...
function makeTmpProject(): { cwd: string; globalDir: string; projectDir: string; cleanup: () => void } {
	...
	const globalDir = join(cwd, '.tac-global', 'agent', 'rules')
	const projectDir = join(cwd, '.tac', 'rules')
...
// a cwd that has .tac/rules/.

// ═══════════════════════════════════════════════════════════════════════════
// Project-local rule loading
// ═══════════════════════════════════════════════════════════════════════════

test('loads rule from project .tac/rules/', (t) => {
...
test('returns empty array when .tac/rules/ does not exist', (t) => {
	const { cwd, cleanup } = makeTmpProject()
 t.after(() => { cleanup() });

		// cwd exists but no .tac/rules/ dir
		const rules = loadRules(cwd)
