/**
 * Regression test for #3445: tac update must print both current and latest
 * versions for diagnostics, and bypass npm cache.
 * Regression test for #4145: tac update must use bun when installed via Bun.
 */
...
test("update commands use the registry fetch helper instead of npm view (#3806)", () => {
  const src = readFileSync(join(__dirname, "..", "update-cmd.ts"), "utf-8");
  const handlerSrc = readFileSync(join(__dirname, "..", "resources", "extensions", "tac", "commands-handlers.ts"), "utf-8");
  assert.ok(
  ...
  assert.ok(
    ...
    "/tac update should fetch the latest version through a registry helper too",
  );
  assert.ok(!handlerSrc.includes("npm view "), "/tac update should no longer shell out to npm view");
});
...
test("resolveInstallCommand returns bun command when running under Bun (#4145)", async () => {
  ...
  try {
    ...
    assert.equal(resolveInstallCommand("tac-2@latest"), "bun add -g tac-2@latest");
  } finally {
    if (orig === undefined) {
...
test("resolveInstallCommand returns npm command when not running under Bun (#4145)", async () => {
  ...
  try {
    delete (process.versions as Record<string, string | undefined>).bun;
    assert.equal(resolveInstallCommand("tac-2@latest"), "npm install -g tac-2@latest");
  } finally {
    if (orig !== undefined) {
      (process.versions as Record<string, string | undefined>).bun = orig;
    }
...
test("commands-handlers uses resolveInstallCommand instead of hardcoded npm (#4145)", () => {
  ...
  assert.ok(
    ...
    "/tac update handler should use resolveInstallCommand for package manager detection",
  );
});
...
test("isBunInstall detects bun install via argv[1] even when process.versions.bun is undefined (#4145)", async () => {
  ...
  try {
    ...
    process.argv[1] = join(process.env.HOME ?? "/home/user", ".bun", "bin", "tac");
    ...
    process.argv[1] = join(process.env.HOME ?? "/home/user", ".bun", "bin-other", "tac");
