...
export class CmuxClient {
  ...
  log(message: string, level: CmuxLogLevel = "info", source = "tac"): void {
    if (!this.config.sidebar) return;
    this.runSync(this.appendWorkspace([
      "log",
      "--level",
   ...
   *
   * Layout strategy (tac stays in the original surface):
   *   1 agent:  [tac | A]
   *   2 agents: [tac | A]
   *             [    | B]
   *   3 agents: [tac | A]
  ...
  async createGridLayout(count: number): Promise<string[]> {
    ...
    // First split: create right column from the tac surface
    ...
    // Third split: split tac surface down → bottom-left
...
/**
 * Wire event subscriptions so cmux reacts to tac events.
 * Called by the tac extension during registration, passing pi.events.
 */
