 ...
 * (.mcp.json, .tac/mcp.json) using the @modelcontextprotocol/sdk Client
...
async function assertTrustedStdioServer(
	...
	if (!ctx?.hasUI) {
		throw new Error(
			...
			"Run this from an interactive TAC session and approve the server before use.",
		);
	}
...
async function connectServer(config: McpServerConfig, signal?: AbortSignal, ctx?: ExtensionContext): Promise<Client> {
	const client = new Client({ name: "tac", version: "1.0.0" });
...
function formatServerList(servers: McpServerConfig[]): string {
	if (servers.length === 0) return "No MCP servers configured. Add servers to .mcp.json or .tac/mcp.json.";
...
// ─── Status helper (consumed by /tac mcp) ─────────────────────────────────────
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "MCP Servers",
		description:
			"List all available MCP servers configured in project files (.mcp.json, .tac/mcp.json). " +
