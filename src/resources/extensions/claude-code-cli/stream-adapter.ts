/**
 * Stream adapter: bridges the Claude Agent SDK into TAC's streamSimple contract.
 *
...
import type { ExtensionUIContext } from "@tac/pi-coding-agent";
import { EventStream } from "@tac/pi-ai";
...
import { buildWorkflowMcpServers } from "../tac/workflow-mcp.js";
 ...
 * (The class itself is only re-exported as a type from the @tac/pi-ai barrel.)
 */
...
/**
 * Build a full conversational prompt from TAC's context messages.
 *
 ...
 * that always approves — replacing the old TAC_AUTO_MODE → bypassPermissions
 * workaround.
 */
...
/**
 * Aborted by the caller's AbortSignal — distinct from exhaustion. TAC's
 ...
 *
 * Set `TAC_CLAUDE_CODE_PERMISSION_MODE` to `bypassPermissions` to restore
 * the old always-approve behaviour, or to `default` / `plan` for stricter
 * modes.
 *
 * When `TAC_HEADLESS=1` is set (auto-mode / non-interactive runs), the
...
export async function resolveClaudePermissionMode(
	...
	const override = env.TAC_CLAUDE_CODE_PERMISSION_MODE?.trim();
	if (override === "bypassPermissions" || override === "acceptEdits" || override === "default" || override === "plan") {
		return override;
	}
	if (env.TAC_HEADLESS === "1") {
		console.warn(
			"[claude-code-cli] Headless mode detected (TAC_HEADLESS=1): defaulting permissionMode to 'bypassPermissions' so verification Bash commands can run. Set TAC_CLAUDE_CODE_PERMISSION_MODE=acceptEdits to opt out.",
		);
...
// NOTE: These helpers intentionally mirror @tac/pi-ai anthropic-shared
// behavior so this extension remains typecheck-stable even when the published
// @tac/pi-ai barrel lags behind monorepo source exports.
...
/** Map a TAC thinking level to the Anthropic effort value, clamping xhigh to max for models that lack native xhigh support. */
...
export function buildSdkOptions(
	...
	// Opt back into gated mode with TAC_CLAUDE_CODE_PERMISSION_MODE=acceptEdits.
...
/**
 * TAC streamSimple function that delegates to the Claude Agent SDK.
 *
...
async function pumpSdkMessages(
	...
	try {
		...
		// Bridge TAC's AbortSignal to SDK's AbortController
		...
		// not a successful completion. Emitting an error lets TAC classify it as a
