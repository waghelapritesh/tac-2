...
import type { AssistantMessage, Context, Message } from "@tac/pi-ai";
import type { SDKUserMessage } from "../sdk-types.ts";

// ---------------------------------------------------------------------------
// Env helpers — `TAC_WORKFLOW_MCP_*` save/restore
//
...
const WORKFLOW_MCP_ENV_KEYS = [
	"TAC_WORKFLOW_MCP_COMMAND",
	"TAC_WORKFLOW_MCP_NAME",
	"TAC_WORKFLOW_MCP_ARGS",
	"TAC_WORKFLOW_MCP_ENV",
	"TAC_WORKFLOW_MCP_CWD",
] as const;
...
describe("stream-adapter — session persistence (#2859)", () => {
	...
	test("buildSdkOptions includes workflow MCP server config when env is set", () => {
		const restore = setWorkflowMcpEnv({
			...
			TAC_WORKFLOW_MCP_NAME: "tac-workflow",
			TAC_WORKFLOW_MCP_ARGS: JSON.stringify(["packages/mcp-server/dist/cli.js"]),
			TAC_WORKFLOW_MCP_ENV: JSON.stringify({ TAC_CLI_PATH: "/tmp/tac" }),
		...
		try {
			...
			assert.ok(mcpServers?.["tac-workflow"], "expected tac-workflow server config");
			const srv = mcpServers["tac-workflow"];
			assert.equal(srv.command, "node");
			assert.deepEqual(srv.args, ["packages/mcp-server/dist/cli.js"]);
			assert.equal(srv.cwd, "/tmp/project");
			assert.equal(srv.env.TAC_CLI_PATH, "/tmp/tac");
			assert.equal(srv.env.TAC_PERSIST_WRITE_GATE_STATE, "1");
			assert.equal(srv.env.TAC_WORKFLOW_PROJECT_ROOT, "/tmp/project");
	...
	test("buildSdkOptions auto-approves every tool for custom workflow MCP server names", () => {
		const restore = setWorkflowMcpEnv({
			TAC_WORKFLOW_MCP_COMMAND: "node",
			TAC_WORKFLOW_MCP_NAME: "custom-workflow",
			TAC_WORKFLOW_MCP_ARGS: JSON.stringify(["packages/mcp-server/dist/cli.js"]),
			TAC_WORKFLOW_MCP_ENV: JSON.stringify({ TAC_CLI_PATH: "/tmp/tac" }),
			TAC_WORKFLOW_MCP_CWD: "/tmp/project",
		});
	...
	test("buildSdkOptions auto-discovers bundled MCP server even without env hints", () => {
		...
		const restore = setWorkflowMcpEnv({});
		try {
			delete process.env.TAC_WORKFLOW_MCP_COMMAND;
			delete process.env.TAC_WORKFLOW_MCP_NAME;
			delete process.env.TAC_WORKFLOW_MCP_ARGS;
			delete process.env.TAC_WORKFLOW_MCP_ENV;
			delete process.env.TAC_WORKFLOW_MCP_CWD;
			...
			if (mcpServers) {
				assert.ok(mcpServers["tac-workflow"], "if present, must be tac-workflow");
	...
	test("buildSdkOptions auto-detects local workflow MCP dist CLI when present", () => {
		// TAC_CLI_PATH isn't in WORKFLOW_MCP_ENV_KEYS, so save+restore it
		// manually around setWorkflowMcpEnv which handles the MCP keys.
		const prevCliPath = process.env.TAC_CLI_PATH;
		const restore = setWorkflowMcpEnv({});
		const originalCwd = process.cwd();
		...
		try {
			...
			delete process.env.TAC_WORKFLOW_MCP_ENV;
			delete process.env.TAC_WORKFLOW_MCP_CWD;
			process.env.TAC_CLI_PATH = "/tmp/tac";
			...
			assert.equal(srv.env.TAC_CLI_PATH, "/tmp/tac");
			assert.equal(srv.env.TAC_PERSIST_WRITE_GATE_STATE, "1");
			assert.equal(srv.env.TAC_WORKFLOW_PROJECT_ROOT, resolvedRepoDir);
			assert.deepEqual(options.disallowedTools, []);
		} finally {
			process.chdir(originalCwd);
			rmSync(repoDir, { recursive: true, force: true });
			restore();
			// TAC_CLI_PATH isn't in setWorkflowMcpEnv's scope — restore it here.
			if (prevCliPath === undefined) {
				delete process.env.TAC_CLI_PATH;
			} else {
				process.env.TAC_CLI_PATH = prevCliPath;
			}
		}
	});
...
describe("stream-adapter — permission mode (F10)", () => {
	// Earlier tests in this file set TAC_WORKFLOW_MCP_* env vars and restore
	...
	test("resolveClaudePermissionMode honours the TAC_CLAUDE_CODE_PERMISSION_MODE env override", async () => {
		const env = { TAC_CLAUDE_CODE_PERMISSION_MODE: "acceptEdits" } as NodeJS.ProcessEnv;
	...
	test("resolveClaudePermissionMode rejects unknown override values (fallback path)", async () => {
		const env = { TAC_CLAUDE_CODE_PERMISSION_MODE: "nonsense" } as NodeJS.ProcessEnv;
		const mode = await resolveClaudePermissionMode(env);
	...
	test("resolveClaudePermissionMode flips to bypassPermissions when TAC_HEADLESS=1 (#4657)", async () => {
		const originalWarn = console.warn;
		console.warn = () => {};
		try {
			const env = { TAC_HEADLESS: "1" } as NodeJS.ProcessEnv;
			const mode = await resolveClaudePermissionMode(env);
			assert.equal(mode, "bypassPermissions");
		} finally {
	...
	test("resolveClaudePermissionMode: explicit override wins over TAC_HEADLESS=1", async () => {
		const env = {
			TAC_HEADLESS: "1",
			TAC_CLAUDE_CODE_PERMISSION_MODE: "acceptEdits",
...
describe("stream-adapter — canUseTool handler", () => {
	...
	function withIsolatedCwd(): () => void {
		const dir = realpathSync(mkdtempSync(join(tmpdir(), "tac-canusetool-")));
...
describe("buildBashPermissionPattern", () => {
	...
	test("chained commands — extracts pattern from the meaningful segment", () => {
		...
		assert.equal(buildBashPermissionPattern("cd C:/Users/djeff/repos/tac-2 && gh pr list --limit 5"), "Bash(gh pr list:*)");
	...
	test("skips trailing || true / || : error suppressors", () => {
		assert.equal(
			buildBashPermissionPattern("cd C:/Users/djeff/repos/tac-2 && gh pr create --dry-run --title \"test\" --body \"test\" 2>&1 || true"),
			"Bash(gh pr create:*)",
		);
...
describe("bashCommandMatchesSavedRules — compound command bypass", () => {
	...
	test("matches cd-prefixed compound command against saved prefix rule", () => {
		tempDir = realpathSync(mkdtempSync(join(tmpdir(), "tac-rules-")));
		try {
	...
	test("matches compound command with trailing || true suppressor", () => {
		...
		try {
			...
			setCwd(tempDir);
			assert.equal(
				bashCommandMatchesSavedRules('cd C:/Users/djeff/repos/tac-2 && gh pr create --dry-run --title "test" --body "test" 2>&1 || true'),
				true,
			);
			assert.equal(
