...
function handleList(ctx: ExtensionCommandContext): void {
  if (!isDbAvailable()) {
    ctx.ui.notify("No TAC database available.", "warning");
    return;
  }
...
function handleShow(ctx: ExtensionCommandContext, id: string | undefined): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac memory show <MEM###>", "warning");
    return;
  }
...
function handleForget(ctx: ExtensionCommandContext, id: string | undefined): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac memory forget <MEM###>", "warning");
    return;
  }
...
function handleExport(ctx: ExtensionCommandContext, target: string | undefined): void {
  if (!target) {
    ctx.ui.notify("Usage: /tac memory export <path.json>", "warning");
    return;
  }
  try {
...
function handleImport(ctx: ExtensionCommandContext, target: string | undefined): void {
  if (!target) {
    ctx.ui.notify("Usage: /tac memory import <path.json>", "warning");
    return;
  }
  try {
    const abs = resolvePath(process.cwd(), target);
...
function handleCap(ctx: ExtensionCommandContext, arg: string | undefined): void {
  ...
  if (!Number.isFinite(max) || max < 1) {
    ctx.ui.notify("Usage: /tac memory cap <max>  (default 50)", "warning");
    return;
  }
...
function handleSources(ctx: ExtensionCommandContext): void {
  ...
  if (sources.length === 0) {
    ctx.ui.notify("No memory sources yet. Use `/tac memory ingest <path|url>` to add one.", "info");
    return;
  }
...
async function handleNote(ctx: ExtensionCommandContext, args: MemoryCmdArgs): Promise<void> {
  ...
  if (!text) {
    ctx.ui.notify('Usage: /tac memory note "your note"', "warning");
    return;
  }
  try {
...
async function handleIngest(ctx: ExtensionCommandContext, args: MemoryCmdArgs): Promise<void> {
  ...
  if (!target) {
    ctx.ui.notify("Usage: /tac memory ingest <path|url> [--tag a,b] [--scope project|global]", "warning");
    return;
  }
  try {
    ...
    if (args.extract && result.sourceId) {
      ...
      ctx.ui.notify(
        `(Dispatching extraction turn — use \`/tac memory extract ${result.sourceId}\` to trigger manually.)`,
        "info",
      );
    }
  } catch (err) {
...
function handleExtractSource(
  ...
  if (!id) {
    ctx.ui.notify("Usage: /tac memory extract <SRC-xxx>", "warning");
    return;
  }
  ...
  pi.sendMessage(
    { customType: "tac-memory-extract", content: prompt, display: false },
    { triggerTurn: true },
  );
}
