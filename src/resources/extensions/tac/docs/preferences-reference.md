# TAC Preferences Reference

Full documentation for `~/.tac/PREFERENCES.md` (global) and `.tac/PREFERENCES.md` (project).
...
- These preferences guide which skills TAC should load and follow; they do not override higher-priority instructions in the current conversation.
- For Claude marketplace/plugin import behavior, see `~/.tac/agent/extensions/tac/docs/claude-marketplace-import.md`.
...
**Empty arrays (`[]`) are equivalent to omitting the field entirely.** During validation, TAC deletes empty arrays from the preferences object (see `validatePreferences()` in `preferences.ts`):
...
1. **Global:** `~/.tac/PREFERENCES.md` — applies to all projects
2. **Project:** `.tac/PREFERENCES.md` — applies to the current project only
...
| ---------------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------- |
| `skill_discovery`                                    | **Whether** TAC looks for relevant skills during research | `resolveSkillDiscoveryMode()` in `preferences.ts`        |
...
- `mode`: workflow mode — `"solo"` or `"team"`. Sets sensible defaults for git and project settings based on your workflow. Mode defaults are the lowest priority layer — any explicit preference overrides them. Omit to configure everything manually.
  ...
  Quick setup: `/tac mode` (global) or `/tac mode project` (project-level).

- `always_use_skills`: skills TAC should use whenever they are relevant.

- `prefer_skills`: soft defaults TAC should prefer when relevant.

- `avoid_skills`: skills TAC should avoid unless clearly needed.

- `skill_rules`: situational rules with a human-readable `when` trigger and one or more of `use`, `prefer`, or `avoid`.

- `custom_instructions`: extra durable instructions related to skill use. For operational project knowledge (recurring rules, gotchas, patterns), use `.tac/KNOWLEDGE.md` instead — it's injected into every agent prompt automatically and agents can append to it during execution.

- `language`: preferred response language for all TAC interactions. Accepts any language name or code — `"Chinese"`, `"zh"`, `"German"`, `"de"`, `"日本語"`, etc. When set, TAC injects "Always respond in \<language\>" into every agent's system prompt, including after `/clear`. Quickest way to set it: `/tac language <name>`. To clear: `/tac language off`.
...
- `skill_discovery`: controls how TAC discovers and applies skills during auto-mode. Valid values:
...
- `git`: configures TAC's git behavior. All fields are optional — omit any to use defaults. Keys:
  ...
  - `snapshots`: boolean — create WIP snapshot commits (e.g. pre-dispatch and stale-uncommitted-changes safety commits emitted by the doctor). Set to `false` to suppress all doctor-initiated `tac snapshot:` commits. Default: `true`.
  ...
  - `manage_gitignore`: boolean — when `false`, TAC will not touch `.gitignore` at all. Useful when your project has a strictly managed `.gitignore` and you don't want TAC adding entries. Default: `true`.
  ...
  - **Deprecated:** `commit_docs` — no longer valid; `.tac/` is always gitignored. Remove this setting.
...
- `cmux`: configures cmux terminal integration when TAC is running inside a cmux workspace. Keys:
...
- `uok`: Unified Orchestration Kernel controls. Keys:
  ...
  - `legacy_fallback.enabled`: boolean — emergency release fallback that forces legacy orchestration behavior even when `uok.enabled` is `true`. Default: `false`.
    - Runtime override: set `TAC_UOK_FORCE_LEGACY=1` (or `TAC_UOK_LEGACY_FALLBACK=1`) to force legacy behavior for the current process.
...
- `context_management`: configures context hygiene for auto-mode sessions. Keys:
  ...
  - `tool_result_max_chars`: number — max chars per tool result in TAC sessions (200-10000). Default: `800`.
...
- `experimental`: opt-in experimental features. All features here are **off by default** — you must explicitly set each one to `true` to enable it. Features in this block may change or be removed without a deprecation cycle while in experimental status. Keys:
  - `rtk`: boolean — enable RTK (Real-Time Kompression) shell-command compression. When enabled, TAC wraps shell commands through the RTK binary to reduce token usage during command execution. RTK is downloaded automatically on first use if not already installed. **Default: `false`** (opt-in required). Set `TAC_RTK_DISABLED=1` in the environment to force-disable regardless of this preference.
...
When a model fails to switch (provider unavailable, rate limited, credits exhausted), TAC automatically tries the next model in the `fallbacks` list. This ensures auto-mode continues even when your preferred provider hits limits.
...
If you use a bare model ID (no provider prefix) and it exists in multiple providers, TAC will warn you and resolve to the first available match. Use `provider/model` format to avoid ambiguity.
...
Enables cmux-aware notifications, sidebar metadata, and visible subagent splits when TAC is running inside a cmux terminal.
...
Opts in to RTK shell-command compression. RTK is downloaded automatically on first use. Set `TAC_RTK_DISABLED=1` to force-disable at the environment level regardless of this setting.
