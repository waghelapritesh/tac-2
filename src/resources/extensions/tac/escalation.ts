// TAC Extension — ADR-011 Phase 2 Mid-Execution Escalation
//
 ...
 *   .tac/milestones/{M}/slices/{S}/tasks/{T}-ESCALATION.json
 */
...
/** Build an EscalationArtifact from a tac_complete_task escalation payload. */
...
export function resolveEscalation(
  ...
  if (choice === "reject-blocker") {
    ...
    return {
      ...
      message: `Escalation rejected. Task ${taskId} now flagged as a blocker — next /tac auto will replan slice ${sliceId}.`,
...
/** Human-readable summary of an artifact for `/tac escalate show`. */
export function formatEscalationForDisplay(art: EscalationArtifact): string {
  ...
  return [
    ...
    resolved,
    "",
    `Resolve with: /tac escalate resolve ${art.taskId} <${art.options.map((o) => o.id).join("|")}|accept|reject-blocker> [rationale...]`,
  ].join("\n");
}

/** List actionable (unresolved) escalations for `/tac escalate list`. */
...
/** List every escalation (including resolved) for `/tac escalate list --all`. */
