...
import { isDbAvailable, getMilestoneSlices, getSliceTasks } from "./tac-db.js";
import { resolveMilestoneFile, resolveMilestonePath, resolveSliceFile, resolveSlicePath, resolveTaskFile, resolveTasksDir, milestonesDir, tacRoot, relMilestoneFile, relSliceFile, relTaskFile, relSlicePath, relTacRootFile, resolveTacRootFile, relMilestonePath } from "./paths.js";
import { deriveState, isMilestoneComplete } from "./state.js";
import { invalidateAllCaches } from "./cache.js";
import { loadEffectiveTACPreferences, type TACPreferences } from "./preferences.js";
...
/**
 * Characters that are used as delimiters in TAC state management documents
 ...
 *   and git branch names (tac/M001/S01). A slash in a title can break path resolution.
 */
 ...
 * with TAC's state document delimiter conventions.
...
export function validateTitle(title: string): string | null {
  if (TITLE_DELIMITER_RE.test(title)) {
    ...
    return `title contains ${found.join(" and ")}, which conflict with TAC state document delimiters`;
  }
  return null;
}

function validatePreferenceShape(preferences: TACPreferences): string[] {
  const issues: string[] = [];
...
async function updateStateFile(basePath: string, fixesApplied: string[]): Promise<void> {
  ...
  const path = resolveTacRootFile(basePath, "STATE");
...
function auditRequirements(content: string | null): DoctorIssue[] {
  ...
  for (const block of blocks) {
    ...
    if (status === "active" && (!owner || owner === "none" || owner === "none yet")) {
      ...
      // a slice via tac_requirement_update. Flagging this as an error
      ...
      issues.push({
        ...
        file: relTacRootFile("REQUIREMENTS"),
        fixable: false,
      });
    }
...
async function appendDoctorHistory(basePath: string, report: DoctorReport): Promise<void> {
  try {
    const historyPath = join(tacRoot(basePath), "doctor-history.jsonl");
...
export async function runTACDoctor(basePath: string, options?: { fix?: boolean; dryRun?: boolean; scope?: string; fixLevel?: "task" | "all"; isolationMode?: "none" | "worktree" | "branch"; includeBuild?: boolean; includeTests?: boolean }): Promise<DoctorReport> {
  const issues: DoctorIssue[] = [];
  ...
  if (prefs) {
    ...
    for (const issue of prefIssues) {
      issues.push({
        ...
        scope: "project",
        unitId: "project",
        message: `TAC preferences invalid: ${issue}`,
        file: prefs.path,
        fixable: false,
      });
    }
  ...
  if (!existsSync(milestonesPath)) {
    const report: DoctorReport = { ok: issues.every(i => i.severity !== "error"), basePath, issues, fixesApplied, timing: { git: gitMs, runtime: runtimeMs, environment: envMs, tacState: 0 } };
    await appendDoctorHistory(basePath, report);
    return report;
  }

  const requirementsPath = resolveTacRootFile(basePath, "REQUIREMENTS");
  ...
  const report: DoctorReport = {
    ...
    issues,
    fixesApplied,
    timing: { git: gitMs, runtime: runtimeMs, environment: envMs, tacState: Math.max(0, Date.now() - t0env - envMs) },
  };
  await appendDoctorHistory(basePath, report);
  return report;
}
