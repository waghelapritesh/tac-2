/**
 * TAC Parallel Orchestrator — Core engine for parallel milestone orchestration.
 *
 ...
 * worktree with TAC_MILESTONE_LOCK env var set. The coordinator monitors
...
import { autoWorktreeBranch, runWorktreePostCreateHook, syncTacStateToWorktree } from "./auto-worktree.js";
...
function stateFilePath(basePath: string): string {
  return join(tacRoot(basePath), ORCHESTRATOR_STATE_FILE);
}

/**
 * Persist the current orchestrator state to .tac/orchestrator.json.
 * Uses atomic write (tmp + rename) to prevent partial reads.
 */
export function persistState(basePath: string): void {
  if (!state) return;
  try {
    const dir = tacRoot(basePath);
...
/**
 * Restore orchestrator state from .tac/orchestrator.json.
...
function workerLogPath(basePath: string, milestoneId: string): string {
  return join(tacRoot(basePath), "parallel", `${milestoneId}.stderr.log`);
}

function appendWorkerLog(basePath: string, milestoneId: string, chunk: string): void {
  try {
    const dir = join(tacRoot(basePath), "parallel");
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
...
function restoreRuntimeState(basePath: string): boolean {
  ...
  // still running and writing heartbeats under .tac/parallel/.
...
export async function prepareParallelStart(
  basePath: string,
  _prefs: TACPreferences | undefined,
...
export async function startParallel(
  ...
  if (process.env.TAC_PARALLEL_WORKER) {
...
function createMilestoneWorktree(basePath: string, milestoneId: string): string {
  ...
  // Copy .tac/ planning artifacts (milestones, CONTEXT, ROADMAP, etc.) from the
  ...
  syncTacStateToWorktree(basePath, info.path);
 ...
 * The worker runs `tac headless --json auto` in the milestone's worktree
 * with TAC_MILESTONE_LOCK set to isolate state derivation.
 *
 * IMPORTANT: We use `headless --json auto` instead of `--print "/tac auto"`.
...
export function spawnWorker(
  ...
  // Resolve the TAC CLI binary path
  const binPath = resolveTacBin();
  ...
  try {
    const workerEnv: Record<string, string | undefined> = {
      ...process.env,
      TAC_MILESTONE_LOCK: milestoneId,
      ...
      // path heuristic can match the user-level ~/.tac instead of the
      // project .tac, causing writes to ~ and corrupting user config.
      TAC_PROJECT_ROOT: basePath,
    ...
    if (state.config.worker_model) {
      workerEnv.TAC_WORKER_MODEL = state.config.worker_model;
    }
...
/**
 * Resolve the TAC CLI binary path.
 * Uses TAC_BIN_PATH env var (set by loader.ts) or falls back to
 * finding the binary relative to the current module.
 */
function resolveTacBin(): string | null {
  // TAC_BIN_PATH is set by loader.ts to the absolute path of dist/loader.js
  if (process.env.TAC_BIN_PATH && existsSync(process.env.TAC_BIN_PATH)) {
    return process.env.TAC_BIN_PATH;
  }

  // Fallback: try to find loader.js relative to this file
  // This file is at dist/resources/extensions/tac/parallel-orchestrator.js
...
function processWorkerLine(basePath: string, milestoneId: string, line: string): void {
  ...
  if (type === "extension_ui_request" && event.method === "notify") {
    // TAC auto-mode sends notifications about current unit
    const worker = state.workers.get(milestoneId);
    if (worker) {
      writeSessionStatus(basePath, {
        milestoneId,
