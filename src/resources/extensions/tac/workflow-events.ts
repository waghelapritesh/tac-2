 ...
 *
 * @param basePath - Project root (parent of .tac/)
...
export function compactMilestoneEvents(
  ...
  const logPath = join(basePath, ".tac", "event-log.jsonl");
  const archivePath = join(basePath, ".tac", `event-log-${milestoneId}.jsonl.archived`);
