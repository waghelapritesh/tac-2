...
import { loadEffectiveTACPreferences, getGlobalTACPreferencesPath } from "./preferences.js";

// Re-export types so existing consumers of ./preferences-models.js keep working
export type { TACPhaseModelConfig, TACModelConfig, TACModelConfigV2, ResolvedModelConfig } from "./preferences-types.js";
...
export function resolveModelWithFallbacksForUnit(unitType: string): ResolvedModelConfig | undefined {
  ...
  const m = prefs.preferences.models as TACModelConfigV2;

  let phaseConfig: string | TACPhaseModelConfig | undefined;
  switch (unitType) {
...
/**
 * Resolve the default session model from TAC preferences.
 *
...
export function resolveDefaultSessionModel(
  ...
  const candidates: Array<string | TACPhaseModelConfig | undefined> = [
    m.execution,
    m.planning,
    m.research,
    m.discuss,
 ...
 * `~/.tac/agent/models.json` (Ollama, vLLM, LM Studio, OpenAI-compatible
 * proxies, etc.).
 *
 * Used by auto-mode bootstrap to decide whether the session model
 * (set via `/tac model`) should override `PREFERENCES.md`.  Custom providers
...
export function isCustomProvider(provider: string | undefined): boolean {
  ...
  const candidates = [
    join(homedir(), ".tac", "agent", "models.json"),
...
/**
 * Update the models section of the global TAC preferences file.
 ...
 */
export function updatePreferencesModels(models: TACModelConfigV2): void {
  ...
  for (const [phase, value] of Object.entries(models)) {
    if (typeof value === "string") {
      ...
      const config = value as TACPhaseModelConfig;
      lines.push(`  ${phase}:`);
 ...
 * Returns a partial TACPreferences that is used as the base layer --
 * explicit user preferences always override these defaults.
 */
export function resolveProfileDefaults(profile: TokenProfile): Partial<TACPreferences> {
  switch (profile) {
    case "budget":
      return {
        models: {
 ...
 */
export function resolveSearchProviderFromPreferences(): TACPreferences["search_provider"] | undefined {
  const prefs = loadEffectiveTACPreferences();
