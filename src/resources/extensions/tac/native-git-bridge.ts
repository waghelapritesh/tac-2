...
import { TACError, TAC_GIT_ERROR } from "./errors.js";
import { GIT_NO_PROMPT_ENV } from "./git-constants.js";
import { getErrorMessage } from "./error-utils.js";

// Issue #453: keep auto-mode bookkeeping on the stable git CLI path unless a
// caller explicitly opts into the native helper.
const NATIVE_TAC_GIT_ENABLED = process.env.TAC_ENABLE_NATIVE_TAC_GIT === "1";
...
function loadNative(): typeof nativeModule {
  ...
  loadAttempted = true;
  if (!NATIVE_TAC_GIT_ENABLED) return nativeModule;

  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const mod = require("@tac/native");
...
function gitExec(basePath: string, args: string[], allowFailure = false): string {
  try {
    ...
    throw new TACError(TAC_GIT_ERROR, `git ${args.join(" ")} failed in ${basePath}`);
  }
}
...
function isDotTacIgnored(basePath: string): boolean {
...
/**
 * Determine whether the project opts out of TAC-managed `.gitignore` via
 * `git.manage_gitignore: false` in `.tac/PREFERENCES.md`. Uses a minimal
 ...
 * file returns false (default: TAC may manage `.gitignore`).
 */
function isGitignoreManagementDisabled(basePath: string): boolean {
  const prefsPath = join(basePath, ".tac", "PREFERENCES.md");
...
/**
 * Self-heal path for the symlinked-`.tac` staging failure: append `.tac` to
 ...
 *
 * Returns true when `.gitignore` now contains an entry covering `.tac`
 * (either pre-existing or newly appended). Returns false when the opt-out
 * is set or the write fails.
 */
function trySelfHealTacGitignore(basePath: string): boolean {
  ...
  try {
    ...
    if (lines.has(".tac") || lines.has(".tac/")) return true;

    const prefix = existing.length > 0 && !existing.endsWith("\n") ? "\n" : "";
    const block = `${prefix}\n# ── TAC self-heal: .tac is a symlink to external state ──\n.tac\n`;
...
/**
 * Stage untracked files individually while skipping anything under `.tac`.
 * Used as a last-resort when `.tac` is a symlink, not gitignored, and
 * `git.manage_gitignore: false` forbids the self-heal path. Protects user
 * work by never silently dropping new real files.
 */
function stageUntrackedExcludingDotTac(basePath: string): void {
  ...
  for (const entry of status.split("\0")) {
    ...
    if (code !== "??") continue;
    // Skip TAC runtime artifacts. Under `manage_gitignore: false` the user
    // may not have these in `.gitignore`, so we filter explicitly to avoid
    // committing transient state (.tac external link, migration lock,
    // background shell scratch dir).
    if (path === ".tac" || path.startsWith(".tac/")) continue;
    if (path === ".tac-id" || path === ".tac.migrating") continue;
 ...
 * when `.tac` is a symlink. Self-heals by adding `.tac` to `.gitignore`, or
 * falls back to explicit per-file staging so user work is never dropped.
 */
function fallbackStageWithSymlinkedDotTac(basePath: string): void {
  if (isDotTacIgnored(basePath)) {
    gitFileExec(basePath, ["add", "-A"]);
    return;
  }
  if (trySelfHealTacGitignore(basePath)) {
    gitFileExec(basePath, ["add", "-A"]);
    return;
  }
  // `manage_gitignore: false` — protect work by staging files explicitly.
  stageUntrackedExcludingDotTac(basePath);
}
...
export function nativeAddAllWithExclusions(basePath: string, exclusions: readonly string[]): void {
  ...
  try {
    ...
    // either adds `.tac` to `.gitignore` and retries `git add -A`, or stages
    ...
    if (stderr.includes("beyond a symbolic link")) {
      fallbackStageWithSymlinkedDotTac(basePath);
      return;
    }
    throw new TACError(TAC_GIT_ERROR, `git add -A with exclusions failed in ${basePath}: ${getErrorMessage(err)}`);
  }
}
...
export function nativeMergeSquash(basePath: string, branch: string): GitMergeResult {
  ...
  try {
    ...
    if (
      ...
      // are dirty instead of generically blaming .tac/ (#2151).
