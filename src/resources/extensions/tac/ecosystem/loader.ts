// TAC2 — Ecosystem extension loader for ./.tac/extensions/
// Discovers and registers single-file extensions that consume TACExtensionAPI.
// Trust-gated (mirrors pi's `.pi/extensions/` model) and isolated from pi's
// own loader chain — handlers run in TAC's own dispatch step, not pi's.
...
import { getAgentDir } from "@tac/pi-coding-agent";

import { logWarning } from "../workflow-logger.js";
import {
  createTACExtensionAPI,
  type TACEcosystemBeforeAgentStartHandler,
  type TACExtensionAPI,
} from "./tac-extension-api.js";
...
/**
 * Discover and register ecosystem extensions from `./.tac/extensions/`.
...
async function _loadEcosystemExtensionsImpl(
  ...
  const extDir = path.join(cwd, ".tac", "extensions");
  ...
  if (!isProjectTrusted(cwd, getAgentDir())) {
    if (!_untrustedWarned) {
      ...
      logWarning(
        "ecosystem",
        ".tac/extensions present but project is not trusted — skipping ecosystem extensions. Run `pi trust` to opt in.",
      );
    }
    return;
  }
  ...
  const api: TACExtensionAPI = createTACExtensionAPI(pi, sharedHandlers);
