// TAC2 — Ecosystem Extension API wrapper
// Wraps pi's ExtensionAPI to expose typed TAC context (phase + active unit)
// to extensions loaded from `./.tac/extensions/`. The wrapper intercepts only
// `on("before_agent_start", ...)` so TAC can dispatch ecosystem handlers AFTER
...
import type { TACActiveUnit, TACState, Phase } from "../types.js";
import { isTACActive, getCurrentPhase } from "../../shared/tac-phase-state.js";
import { logWarning } from "../workflow-logger.js";

// ─── Public Interface ───────────────────────────────────────────────────

export interface TACExtensionAPI extends ExtensionAPI {
  /** Current TAC workflow phase, or null if no project state. */
  getPhase(): Phase | null;
  /** Currently active milestone/slice/task triple, or null if none. */
  getActiveUnit(): TACActiveUnit | null;
}

export type TACEcosystemBeforeAgentStartHandler = ExtensionHandler<
  BeforeAgentStartEvent,
...
function resolvePhase(state: TACState | null): Phase | null {
...
function resolveActiveUnit(state: TACState | null): TACActiveUnit | null {
  if (!state) return null;
...
interface Snapshot {
  phase: Phase | null;
  activeUnit: TACActiveUnit | null;
}

let _snapshot: Snapshot = { phase: null, activeUnit: null };

/** Refresh the snapshot from a freshly derived TACState (or null on failure). */
export function updateSnapshot(state: TACState | null): void {
  _snapshot = {
...
export function getSnapshotActiveUnit(): TACActiveUnit | null {
...
/**
 * Build a TACExtensionAPI by manually delegating every ExtensionAPI method
 * to the underlying pi instance, except `on("before_agent_start", ...)`
 * which is captured into `sharedHandlers` for TAC-owned dispatch.
 *
 * Uses `satisfies TACExtensionAPI` (NOT `as`) so TypeScript catches drift
 * when pi adds new ExtensionAPI methods.
 */
export function createTACExtensionAPI(
  pi: ExtensionAPI,
  sharedHandlers: TACEcosystemBeforeAgentStartHandler[],
): TACExtensionAPI {
  const wrapper = {
    // ── Event subscription (single intercept point) ────────────────────
    on(event: any, handler: any): void {
      if (event === "before_agent_start") {
        sharedHandlers.push(handler as TACEcosystemBeforeAgentStartHandler);
        return;
      }
    ...
    // ── TAC-specific additions ─────────────────────────────────────────
    getPhase: (): Phase | null => _snapshot.phase,
    getActiveUnit: (): TACActiveUnit | null => _snapshot.activeUnit,
  } satisfies TACExtensionAPI;
