// TAC Extension — Desktop Notification Helper
...
export function sendDesktopNotification(
  ...
  // When a projectName is provided and the title is the default "TAC",
  // replace it with a project-qualified title for multi-project clarity.
  if (projectName && title === "TAC") {
    title = formatNotificationTitle(projectName);
  }
  const loaded = loadEffectiveTACPreferences()?.preferences;
...
export function shouldSendDesktopNotification(
  ...
  preferences: NotificationPreferences | undefined = loadEffectiveTACPreferences()?.preferences.notifications,
): boolean {
 ...
 * Returns "TAC — projectName" when a project name is available, otherwise "TAC".
 */
export function formatNotificationTitle(projectName?: string): string {
  const trimmed = projectName?.trim();
  if (trimmed) return `TAC — ${trimmed}`;
  return "TAC";
}
...
export function buildDesktopNotificationCommand(
  ...
  if (platform === "darwin") {
    ...
    // See: https://github.com/waghelapritesh/tac-2/issues/2632
