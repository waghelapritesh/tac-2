/**
 * TAC Workflow Templates — Registry & Resolution
 *
...
const __extensionDir = resolveTacExtensionDir();
const registryPath = join(__extensionDir, "workflow-templates", "registry.json");

/** Resolve the TAC extension dir with fallback to ~/.tac/agent/extensions/tac/. */
function resolveTacExtensionDir(): string {
  ...
  if (existsSync(join(agentTacDir, "workflow-templates"))) return agentTacDir;
  return moduleDir;
}
...
export function listTemplates(): string {
  ...
  lines.push("Usage: /tac start <template> [description]");
  lines.push("       /tac templates info <name>");
...
export function getTemplateInfo(name: string): string | null {
  ...
  const lines = [
    ...
    `Requires .tac/: ${t.requires_project ? "yes" : "no"}`,
    "",
    "Phases:",
