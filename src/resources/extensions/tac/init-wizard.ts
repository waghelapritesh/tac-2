/**
 * TAC Init Wizard — Per-project onboarding.
 *
 * Guides users through project setup when entering a directory without .tac/.
 * Detects project ecosystem, offers v1 migration, configures project preferences,
 * bootstraps .tac/ structure, and transitions to the first milestone discussion.
 */
 ...
 * Called when entering a directory without .tac/ (or via /tac init).
 */
export async function showProjectInit(
  ...
  if (!signals.isGitRepo) {
    const gitChoice = await showNextAction(ctx, {
      title: "TAC — Project Setup",
      summary: ["This folder is not a git repository. TAC uses git for version control and isolation."],
      actions: [
      ...
      notYetMessage: "Run /tac init when ready.",
    });
  ...
  if (signals.verificationCommands.length > 0) {
    ...
    const verifyChoice = await showNextAction(ctx, {
      title: "TAC — Verification Commands",
      summary: [
        "Auto-detected verification commands:",
        ...verifyLines,
        "",
        "TAC runs these after each code change to verify nothing is broken.",
      ],
      actions: [
  ...
  const instructionChoice = await showNextAction(ctx, {
    title: "TAC — Project Instructions",
    summary: [
      "Any rules TAC should follow for this project?",
      "",
      "Examples:",
      ...
      "",
      "You can always add more later via /tac prefs project.",
    ],
    actions: [
  ...
  const reviewChoice = await showNextAction(ctx, {
    title: "TAC — Review All Preferences (Optional)",
    summary: [
      ...
      "",
      "Skip if you just want sensible defaults; you can always run /tac prefs project later.",
    ],
    actions: [
  ...
  if (reviewChoice === "not_yet") {
    // User deferred — don't create .tac/ or persist preferences. Pre-step state
    // (e.g. git init from Step 2) remains as-is, matching prior step semantics.
    return { completed: false, bootstrapped: false };
  }

  // ── Step 10: Bootstrap .tac/ + write preferences ───────────────────────────
  bootstrapTacDirectoryStructure(basePath, signals);
  ...
  const projectPrefsPath = join(tacRoot(basePath), "PREFERENCES.md");
  ...
  // Initialize SQLite database so TAC starts in full-capability mode (#3880).
  // Without this, isDbAvailable() returns false and TAC enters degraded
  ...
  // Write initial STATE.md so it exists before the first /tac invocation.
  // The explicit /tac init path (ops.ts) returns without entering showSmartEntry(),
  ...
  try {
    ...
    const { resolveTacRootFile } = await import("./paths.js");
    const state = await deriveState(basePath);
    await saveFile(resolveTacRootFile(basePath, "STATE"), buildStateMarkdown(state));
  } catch {
    // Non-fatal — STATE.md will be regenerated on next /tac invocation
  }
  ...
  ctx.ui.notify("TAC initialized. Starting your first milestone...", "info");
...
export async function offerMigration(
  ...
  const summary = [
    "Found .planning/ directory (TAC v1 format)",
  ];
  if (v1.phaseCount > 0) {
  ...
  const choice = await showNextAction(ctx, {
    title: "TAC — Legacy Project Detected",
    summary,
    actions: [
      {
        id: "migrate",
        label: "Migrate to TAC v2",
        description: "Convert .planning/ to .tac/ format",
        recommended: true,
      },
      {
        id: "fresh",
        label: "Start fresh",
        description: "Ignore .planning/ and create new .tac/",
      },
    ],
    notYetMessage: "Run /tac init when ready.",
  });
...
/**
 * Handle /tac init when .tac/ already exists.
...
export async function handleReinit(
  ...
  const summary = ["TAC is already initialized in this project."];
  if (detection.v2) {
  ...
  if (choice === "prefs") {
    ctx.ui.notify("Use /tac prefs project to update project preferences.", "info");
  }
}
...
async function customizeAdvancedPrefs(
  ...
  const profileChoice = await showNextAction(ctx, {
    ...
    summary: [
      "Controls how much context TAC uses per task.",
  ...
  const researchChoice = await showNextAction(ctx, {
    ...
    summary: [
      "TAC can research the codebase before planning each milestone.",
...
/**
 * Create .tac/ directory structure and seed CONTEXT.md.
 *
 ...
 */
function bootstrapTacDirectoryStructure(basePath: string, signals: ProjectSignals): void {
  // Final safety check before writing any files
  assertSafeDirectory(basePath);

  const tac = tacRoot(basePath);
  mkdirSync(join(tac, "milestones"), { recursive: true });
  mkdirSync(join(tac, "runtime"), { recursive: true });

  // Seed CONTEXT.md with detected project signals
  const contextContent = buildContextSeed(signals);
  if (contextContent) {
    writeFileSync(join(tac, "CONTEXT.md"), contextContent, "utf-8");
  }
}
...
function buildInitPreferencesBody(): string {
  return [
    "",
    "# TAC Project Preferences",
    "",
    "Generated by `/tac init`. Edit directly or use `/tac prefs project` to modify.",
    "",
    "See `~/.tac/agent/extensions/tac/docs/preferences-reference.md` for full field documentation.",
    "",
  ].join("\n");
}
...
function buildContextSeed(signals: ProjectSignals): string | null {
  ...
  lines.push("");
  lines.push("Auto-detected by TAC init wizard. Edit or expand as needed.");
  lines.push("");
