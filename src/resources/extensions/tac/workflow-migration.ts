// TAC Extension — Legacy Markdown to Engine Migration
...
import { _getAdapter, bulkInsertLegacyHierarchy } from "./tac-db.js";
...
/**
 * Returns true when engine tables are empty AND a .tac/milestones/ directory
...
export function needsAutoMigration(basePath: string): boolean {
  ...
  // Check if .tac/milestones/ directory exists
  const milestonesDir = join(basePath, ".tac", "milestones");
...
/**
 * Migrate legacy markdown-only .tac/ projects to engine DB state.
 * Reads .tac/milestones/<ID>/ directories and parses ROADMAP.md, *-PLAN.md
...
export function migrateFromMarkdown(basePath: string): void {
  ...
  if (!existsSync(milestonesDir)) {
    process.stderr.write("workflow-migration: no .tac/milestones/ directory found, nothing to migrate\n");
    return;
  }
  ...
  if (milestoneDirs.length === 0) {
    process.stderr.write("workflow-migration: no milestone directories found in .tac/milestones/\n");
    return;
  }
