// TAC Extension — Dynamic Model Router
...
import { getProviderCapabilities, type ProviderCapabilities } from "@tac/pi-ai";
import { getToolCompatibility, getAllToolCompatibility } from "@tac/pi-coding-agent";
import type { ToolCompatibility } from "@tac/pi-coding-agent";
 ...
 * Used to detect Groq at the TAC routing layer where only the provider string
...
export function filterToolsForProvider(
  ...
  if (provider && GROQ_PROVIDER_IDS.has(provider) && compatible.length > GROQ_MAX_TOOLS) {
    ...
    console.warn(
      `[tac] Groq tool limit: ${compatible.length + trimmed.length} tools active but Groq allows at most ${GROQ_MAX_TOOLS}. ` +
