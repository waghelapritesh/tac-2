 ...
 * (`/.tac/worktrees/`) and symlink-resolved (`/.tac/projects/<hash>/worktrees/`)
 * layouts.  When `.tac` is a symlink to `~/.tac/projects/<hash>`, resolved
 * paths contain the intermediate `projects/<hash>/` segment that the old
 * single-marker check missed.
 */
function findWorktreeSegment(normalizedPath: string): { tacIdx: number; afterWorktrees: number } | null {
  // Direct layout: /.tac/worktrees/<name>
  const directMarker = "/.tac/worktrees/";
  const idx = normalizedPath.indexOf(directMarker);
  if (idx !== -1) {
    return { tacIdx: idx, afterWorktrees: idx + directMarker.length };
  }
  // Symlink-resolved layout: /.tac/projects/<hash>/worktrees/<name>
  const symlinkRe = /\/\.tac\/projects\/[a-f0-9]+\/worktrees\//;
  const match = normalizedPath.match(symlinkRe);
  if (match && match.index !== undefined) {
    return { tacIdx: match.index, afterWorktrees: match.index + match[0].length };
  }
  return null;
}
 ...
 * Returns null if not inside a TAC worktree (.tac/worktrees/<name>/).
 */
 ...
 * `/.tac/`. Otherwise returns the input unchanged.
 *
 * When the worker was spawned with TAC_PROJECT_ROOT set, use that directly —
 * the coordinator already knows the real project root unambiguously.
 *
 * When `/.tac/` in the resolved path is actually the user-level `~/.tac/`
 * (common when `.tac` is a symlink into `~/.tac/projects/<hash>`), the
...
export function resolveProjectRoot(basePath: string): string {
  ...
  if (process.env.TAC_PROJECT_ROOT) {
    return process.env.TAC_PROJECT_ROOT;
  }
  ...
  const tacMarker = `${sepChar}.tac${sepChar}`;
  const tacIdx = basePath.indexOf(tacMarker);
  const candidate = tacIdx !== -1
    ? basePath.slice(0, tacIdx)
    : basePath.slice(0, seg.tacIdx);

  // Layer 2: Guard against resolving to the user's home directory.
  // When .tac is a symlink into ~/.tac/projects/<hash>, the resolved path
  // contains /.tac/ at the user-level boundary. Slicing there yields ~ — wrong.
  const tacHome = normalizePathForCompare(process.env.TAC_HOME || join(homedir(), ".tac"));
  const candidateTacPath = normalizePathForCompare(join(candidate, ".tac"));

  if (candidateTacPath === tacHome || candidateTacPath.startsWith(tacHome + "/")) {
    // The candidate is the home directory (or within it in a way that .tac
    // maps to the user-level TAC dir). Try to recover the real project root
 ...
 *
 * In the main tree:     tac/<milestoneId>/<sliceId>
 * In a worktree:        tac/<worktreeName>/<milestoneId>/<sliceId>
 *
...
export function getSliceBranchName(milestoneId: string, sliceId: string, worktreeName?: string | null): string {
  if (worktreeName) {
    return `tac/${worktreeName}/${milestoneId}/${sliceId}`;
  }
  return `tac/${milestoneId}/${sliceId}`;
}
 ...
 * Handles both `tac/M001/S01` and `tac/myworktree/M001/S01`.
 */
...
/**
 * Get the "main" branch for TAC slice operations.
 *
