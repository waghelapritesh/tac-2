...
const NATIVE_TAC_PARSER_ENABLED = process.env.TAC_ENABLE_NATIVE_TAC_PARSER === "1";
...
let nativeModule: {
  ...
  batchParseTacFiles: (directory: string) => { files: Array<{ path: string; metadata: string; body: string; sections: string; rawContent: string }>; count: number };
  ...
  scanTacTree: (directory: string) => Array<{ path: string; name: string; isDir: boolean }>;
...
function loadNative(): typeof nativeModule {
  ...
  loadAttempted = true;
  if (!NATIVE_TAC_PARSER_ENABLED) return nativeModule;
  ...
  try {
    ...
    if (mod.parseFrontmatter && mod.extractSection && mod.batchParseTacFiles) {
      nativeModule = mod;
    }
  } catch {
...
/**
 * Batch-parse all .md files in a .tac/ directory tree using the native parser.
 * Returns null if native module unavailable.
 */
export function nativeBatchParseTacFiles(directory: string): BatchParsedFile[] | null {
  const native = loadNative();
  if (!native) return null;

  const result = native.batchParseTacFiles(directory);
...
/**
 * Native-backed directory tree scan of a .tac/ directory.
 * Returns a flat list of all entries, or null if native module unavailable.
 */
export function nativeScanTacTree(directory: string): TacTreeEntry[] | null {
  const native = loadNative();
  if (!native) return null;
  return native.scanTacTree(directory);
}
