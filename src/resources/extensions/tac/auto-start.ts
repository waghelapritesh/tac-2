...
import { ensureTacSymlink, isInheritedRepo, validateProjectId } from "./repo-identity.js";
import { migrateToExternalState, recoverFailedMigration } from "./migrate-external.js";
import { collectSecretsFromManifest } from "../get-secrets-from-user.js";
import { tacRoot, resolveMilestoneFile } from "./paths.js";
...
import { isDbAvailable, getMilestone, openDatabase, getDbStatus } from "./tac-db.js";
...
export async function openProjectDbIfPresent(basePath: string): Promise<void> {
  const tacDbPath = resolveProjectRootDbPath(basePath);
  if (!existsSync(tacDbPath) || isDbAvailable()) return;

  try {
    openDatabase(tacDbPath);
  } catch (err) {
    logWarning("engine", `tac-db: failed to open existing database: ${err instanceof Error ? err.message : String(err)}`);
  }
}
  ...
  for (const branch of milestoneBranches) {
    ...
    if (!isClosedStatus(milestone.status)) {
      ...
      const wtSuffix = wtDirExists
        ? ` Worktree directory at .tac/worktrees/${milestoneId}/ holds the live work.`
        : "";
      warnings.push(
        `Branch ${branch} has ${commitsAhead} commit(s) ahead of ${mainBranch} for in-progress milestone ${milestoneId}.` +
        wtSuffix +
        ` Run \`/tac auto\` to resume, or merge manually if abandoning.`,
      );
    ...
    if (isMerged) {
      ...
      if (existsSync(wtDir)) {
        ...
        // filesystem removal — but only inside .tac/worktrees/ for safety (#2365).
        if (existsSync(wtDir)) {
          if (isInsideWorktreesDir(basePath, wtDir)) {
            ...
            warnings.push(`Orphaned worktree directory for ${milestoneId} is outside .tac/worktrees/ — skipping removal for safety.`);
          }
        } else {
          recovered.push(`Removed orphaned worktree directory for ${milestoneId}.`);
        }
      ...
      warnings.push(
        ...
        `This may contain unmerged work. Merge manually or run \`/tac health --fix\` to resolve.`,
      );
...
export async function bootstrapAutoSession(
  ...
  //
  // Precedence:
  // 1) Explicit session override via /tac model (this session)
  // 2) Current session model from settings/session restore (if provider ready)
  // 3) TAC model preferences from PREFERENCES.md (validated against live auth)
  //
  // This preserves #3517 defaults while honoring explicit runtime model
  // selection for subsequent /tac runs in the same session.
  //
  // Exception (#4122): when the session provider is a custom provider declared
  // in ~/.tac/agent/models.json (Ollama, vLLM, OpenAI-compatible proxy, etc.),
  ...
  try {
    // Validate TAC_PROJECT_ID early so the user gets immediate feedback
    const customProjectId = process.env.TAC_PROJECT_ID;
    if (customProjectId && !validateProjectId(customProjectId)) {
      ctx.ui.notify(
        `TAC_PROJECT_ID must contain only alphanumeric characters, hyphens, and underscores. Got: "${customProjectId}"`,
        "error",
      );
    ...
    // where isInheritedRepo() returns a false negative, e.g. stale .tac at
    ...
    if (!hasLocalGit || isInheritedRepo(base)) {
      const mainBranch =
        loadEffectiveTACPreferences(base)?.preferences?.git?.main_branch || "main";
      nativeInit(base, mainBranch);
    }

    // Migrate legacy in-project .tac/ to external state directory.
    // Migration MUST run before ensureGitignore to avoid adding ".tac" to
    // .gitignore when .tac/ is git-tracked (data-loss bug #1364).
    ...
    // ensureGitignore checks for git-tracked .tac/ files and skips the
    // ".tac" pattern if the project intentionally tracks .tac/ in git.
    const gitPrefs = loadEffectiveTACPreferences(base)?.preferences?.git;
    const manageGitignore = gitPrefs?.manage_gitignore;
    ensureGitignore(base, { manageGitignore });
    if (manageGitignore !== false) untrackRuntimeFiles(base);

    // Bootstrap milestones/ if it doesn't exist.
    // Check milestones/ directly — ensureTacSymlink above already created .tac/,
    // so checking .tac/ existence would be dead code (#2942).
    const tacDir = join(base, ".tac");
    const milestonesPath = join(tacDir, "milestones");
    if (!existsSync(milestonesPath)) {
      mkdirSync(milestonesPath, { recursive: true });
      try {
        nativeAddAll(base);
        nativeCommit(base, "chore: init tac");
      } catch (err) {
        /* nothing to commit */
    ...
    s.gitService = new GitServiceImpl(
      s.basePath,
      loadEffectiveTACPreferences(base)?.preferences?.git ?? {},
    );

    // ── Debug mode ──
    if (!isDebugEnabled() && process.env.TAC_DEBUG === "1") {
      enableDebug(base);
    }
    if (isDebugEnabled()) {
    ...
    if (
      ...
      !detectWorktreeName(base) &&
      !base.includes(`${pathSep}.tac${pathSep}worktrees${pathSep}`)
    ) {
    ...
    if (decideSurvivorAction(hasSurvivorBranch, state.phase) === "discuss") {
      ...
      if (
        ...
        ctx.ui.notify(
          "Discussion completed but milestone draft was not promoted. Run /tac to try again.",
          "warning",
        );
        return releaseLockAndReturn();
      }
      ...
      if (!state.activeMilestone || state.phase === "complete") {
        ...
        if (s.consecutiveCompleteBootstraps > MAX_CONSECUTIVE_COMPLETE_BOOTSTRAPS) {
          ...
          ctx.ui.notify(
            ...
            "Run /tac to start a new milestone manually.",
            "warning",
          );
          return releaseLockAndReturn();
        }
    ...
    const { activateTAC: activateTACPhaseState } = await import("../shared/tac-phase-state.js");
    activateTACPhaseState();
    s.active = true;
    ...
    const isUnderTacWorktrees = (p: string): boolean => {
      // Direct layout: /.tac/worktrees/
      const marker = `${pathSep}.tac${pathSep}worktrees${pathSep}`;
      if (p.includes(marker)) return true;
      const worktreesSuffix = `${pathSep}.tac${pathSep}worktrees`;
    ...
    // ── DB lifecycle ──
    const tacDbPath = resolveProjectRootDbPath(s.basePath);
    const tacDirPath = join(s.basePath, ".tac");
    if (existsSync(tacDirPath) && !existsSync(tacDbPath)) {
      const hasDecisions = existsSync(join(tacDirPath, "DECISIONS.md"));
      const hasRequirements = existsSync(join(tacDirPath, "REQUIREMENTS.md"));
      const hasMilestones = existsSync(join(tacDirPath, "milestones"));
      try {
        const { openDatabase: openDb } = await import("./tac-db.js");
        openDb(tacDbPath);
    ...
    if (existsSync(tacDbPath) && !isDbAvailable()) {
      try {
        const { openDatabase: openDb } = await import("./tac-db.js");
        openDb(tacDbPath);
      } catch (err) {
    ...
    // auto-mode starts but every tac_task_complete / tac_slice_complete
    ...
    if (existsSync(tacDbPath) && !isDbAvailable()) {
      ...
      ctx.ui.notify(
        `SQLite database exists but failed to open: ${tacDbPath}. ${phaseHint}${errorDetail}.${providerHint}`,
        "error",
      );
      return releaseLockAndReturn();
    }
    ...
    // TAC_WORKER_MODEL is injected by the coordinator when parallel.worker_model
    ...
    const workerModelOverride = process.env.TAC_WORKER_MODEL;
    if (workerModelOverride && process.env.TAC_PARALLEL_WORKER === "1") {
    ...
    const bannerPrefs = loadEffectiveTACPreferences(base)?.preferences;
    ...
    try {
      const msDir = join(base, ".tac", "milestones");
      if (existsSync(msDir)) {
