/**
 * TAC Config — Tool API key management.
 *
...
export async function handleConfig(ctx: ExtensionCommandContext): Promise<void> {
  if (!deprecationWarned) {
    ctx.ui.notify(
      "/tac config is deprecated and will be removed. Use /tac keys (manages both LLM and tool API keys).",
      "warning",
    );
  ...
  // Show current status
  const statusLines = ["TAC Tool Configuration\n"];
  for (const tool of TOOL_KEYS) {
