/**
 * TAC Inspect — SQLite DB diagnostics.
 *
...
export function formatInspectOutput(data: InspectData): string {
  const lines: string[] = [];
  lines.push("=== TAC Database Inspect ===");
...
export async function handleInspect(ctx: ExtensionCommandContext): Promise<void> {
  try {
    const { isDbAvailable, _getAdapter, openDatabase } = await import("./tac-db.js");

    if (!isDbAvailable()) {
      const tacDir = tacRoot(process.cwd());
      const dbPath = join(tacDir, "tac.db");
      if (!existsSync(tacDir) || !existsSync(dbPath) || !openDatabase(dbPath)) {
        ctx.ui.notify("No TAC database available. Run /tac auto to create one.", "info");
        return;
      }
    }
    ...
    logWarning("command", `/tac inspect failed: ${getErrorMessage(err)}`);
    ctx.ui.notify("Failed to inspect TAC database. Check stderr for details.", "error");
  }
}
