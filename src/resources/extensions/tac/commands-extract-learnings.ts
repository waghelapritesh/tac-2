/**
 * TAC Command — /tac extract-learnings
 *
 ...
 * the durable subset into the TAC memory store via capture_thought:
 *
 ...
 * appends and tac_save_decision call-outs were removed from this flow; the
...
import { tacRoot, resolveMilestonePath } from "./paths.js";
 ...
 * manual `/tac extract-learnings` dispatch prompt. Covers the milestone
...
export interface ExtractLearningsPromptContext {
  ...
  /** Display name of the enclosing project (from `.tac/PROJECT.md` or dir basename). */
  projectName: string;
}
...
/**
 * Parses the argument string passed to `/tac extract-learnings`.
 *
 ...
 *   1. /tac extract-learnings manual path (buildExtractLearningsPrompt).
...
\`/tac extract-learnings\` and the auto-mode milestone-completion turn.
...
/**
 * Build the full dispatch prompt for the manual `/tac extract-learnings` path.
 *
 ...
 * truth for how learnings flow into the TAC memory store via capture_thought.
 *
...
TAC memory store via \`capture_thought\` (categories: pattern, gotcha or
...
/**
 * Extracts the project display name from `.tac/PROJECT.md` frontmatter.
 *
...
export function extractProjectName(basePath: string): string {
  const projectMdPath = join(tacRoot(basePath), "PROJECT.md");
...
/**
 * Handles the `/tac extract-learnings <MID>` slash command.
 *
 ...
 * artefacts are missing, matching the behaviour of sibling `/tac` commands.
 */
export async function handleExtractLearnings(
  ...
  if (!milestoneId) {
    ctx.ui.notify("Usage: /tac extract-learnings <milestoneId>  (e.g. M001)", "warning");
    return;
  }

  // projectRoot() throws TACNoProjectError if no project found — intentional, handled by dispatcher
  ...
  pi.sendMessage(
    { customType: "tac-extract-learnings", content: prompt, display: false },
    { triggerTurn: true },
  );
}
