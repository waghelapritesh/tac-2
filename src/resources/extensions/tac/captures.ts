/**
 * TAC Captures — Fire-and-forget thought capture with triage classification
 *
 * Append-only capture file at `.tac/CAPTURES.md`. Each capture is an H3 section
 ...
 * `.tac/CAPTURES.md`, not the worktree's local `.tac/`.
 */
 ...
 *
 * In worktree-isolated mode, basePath is `.tac/worktrees/<MID>/`.
 * Captures must resolve to the *original* project root's `.tac/CAPTURES.md`,
 * not the worktree-local `.tac/`. This ensures all captures go to one file
 * regardless of which worktree the agent is running in.
 *
 * Detection: if basePath contains `/.tac/worktrees/`, walk up to the
 * directory that contains `.tac/worktrees/` — that's the project root.
 */
export function resolveCapturesPath(basePath: string): string {
  const resolved = resolve(basePath);
  // Direct layout: /.tac/worktrees/
  const worktreeMarker = `${sep}.tac${sep}worktrees${sep}`;
  let idx = resolved.indexOf(worktreeMarker);
  if (idx === -1) {
    // Symlink-resolved layout: /.tac/projects/<hash>/worktrees/
    const symlinkRe = new RegExp(
      `\\${sep}\\.tac\\${sep}projects\\${sep}[a-f0-9]+\\${sep}worktrees\\${sep}`,
    );
  ...
  if (idx !== -1) {
    ...
    return join(projectRoot, ".tac", CAPTURES_FILENAME);
  }
  return join(tacRoot(basePath), CAPTURES_FILENAME);
}
 ...
 * Creates `.tac/` and the file if they don't exist.
