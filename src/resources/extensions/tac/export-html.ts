/**
 * TAC HTML Report Generator
 *
 * Produces a single self-contained HTML file with:
 *   - Branding header (project name, path, TAC version, generated timestamp)
...
<title>TAC Report — ${esc(opts.projectName)}${opts.milestoneId ? ` — ${esc(opts.milestoneId)}` : ''}</title>
<style>${CSS}</style>
</head>
<body>
<header>
  <div class="header-inner">
    <div class="branding">
      ...
      <span class="version">v${esc(opts.tacVersion)}</span>
    </div>
...
<footer>
  <div class="footer-inner">
    <span>TAC v${esc(opts.tacVersion)}</span>
    <span class="sep">/</span>
...
(function(){
  var saved=JSON.parse(localStorage.getItem('tac-collapsed')||'{}');
  document.querySelectorAll('section[id]').forEach(function(sec){
    ...
    btn.addEventListener('click',function(e){
      ...
      localStorage.setItem('tac-collapsed',JSON.stringify(saved));
    });
  });
...
(function(){
  ...
  btn.textContent=localStorage.getItem('tac-theme')==='light'?'Dark':'Light';
  if(localStorage.getItem('tac-theme')==='light')document.documentElement.classList.add('light-theme');
