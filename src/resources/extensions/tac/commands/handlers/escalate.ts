// TAC Extension — /tac escalate Command Handler (ADR-011 Phase 2)
...
function helpMessage(): string {
  return [
    "/tac escalate — manage mid-execution escalations (ADR-011 Phase 2)",
    "",
    "Subcommands:",
    ...
    "`/tac escalate resolve`, then disable.",
  ].join("\n");
}
...
export async function handleEscalateCommand(
  ...
  if (trimmed.startsWith("resolve ")) {
    ...
    if (!ref || !choice) {
      ctx.ui.notify("Usage: /tac escalate resolve <taskId|Sxx/Tyy> <choice> [rationale...]", "warning");
      return;
    }
    ...
    if (result.status === "resolved") {
      try {
        ...
        ctx.ui.notify(
          `${result.message}\nDecision recorded as ${decisionId}. Run /tac auto to continue.`,
          "success",
        );
      } catch (decErr) {
        ctx.ui.notify(
    ...
    ctx.ui.notify(`${result.message} Run /tac auto to trigger the replan.`, "success");
    return;
  }
