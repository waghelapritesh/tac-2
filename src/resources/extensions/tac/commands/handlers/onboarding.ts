// TAC — /tac onboarding command handler (re-entry hub)
//
...
// commands (/login, /tac keys, /tac remote) that are already ctx.ui-safe.
...
async function runStep(ctx: ExtensionCommandContext, stepId: OnboardingStepId): Promise<void> {
  switch (stepId) {
    case "llm":
      ctx.ui.notify(
        "LLM provider setup: run /login to sign in via OAuth, or /tac keys add to paste an API key.",
        "info",
      )
      return
    case "search":
      ctx.ui.notify(
        "Web search setup: run /tac keys add and pick a search provider (brave, tavily, etc.).",
        "info",
      )
      return
    case "remote":
      ctx.ui.notify(
        "Remote questions setup: run /tac remote to configure Discord / Slack / Telegram notifications.",
        "info",
      )
      return
    case "tool-keys":
      ctx.ui.notify(
        "Tool keys setup: run /tac keys add to save API keys for Context7, Jina, Groq voice, etc.",
        "info",
      )
      return
    case "model": {
    ...
    case "prefs": {
      ...
      const { getGlobalTACPreferencesPath } = await import("../../preferences.js")
    ...
    case "doctor": {
      ...
      ctx.ui.notify("Run /tac doctor to validate your setup.", "info")
      return
    }
    case "skills":
      ctx.ui.notify("Skill install runs during /tac init. Use /tac init or /skill manage.", "info")
      return
    case "project": {
      const { handleCoreCommand } = await import("./core.js")
...
async function renderSetupHub(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const choice = await ctx.ui.select("TAC Setup — pick a step to configure", labels)
...
function renderStatus(): string {
  ...
  const lines: string[] = ["TAC Onboarding\n"]
  if (r.completedAt) {
...
export async function handleOnboarding(rawArgs: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (args.reset) {
    resetOnboarding()
    ctx.ui.notify(
      "Onboarding state cleared. API keys/credentials are unchanged — manage them with /tac keys. Restart TAC to re-run the first-run wizard, or pick a step below.",
      "info",
    )
