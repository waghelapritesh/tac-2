// TAC Extension — /tac notifications Command Handler
...
export async function handleNotificationsCommand(
  ...
  if (args === "tail" || args.startsWith("tail ")) {
    ...
    const suffix = all.length > entries.length
      ? `\n... and ${all.length - entries.length} more (open /tac notifications to browse all)`
      : "";
    ctx.ui.notify(`Last ${entries.length} notification(s):\n${lines.join("\n")}${suffix}`, "info");
    return true;
  }

  // /tac notifications filter <severity>
  if (args.startsWith("filter ")) {
    ...
    if (!["error", "warning", "info", "success"].includes(severity)) {
      ctx.ui.notify("Usage: /tac notifications filter <error|warning|info|success>", "warning");
      return true;
    }
    ...
    const suffix = entries.length > 20
      ? `\n... and ${entries.length - 20} more (open /tac notifications to browse all)`
      : "";
    ctx.ui.notify(`${severity} notifications (${entries.length}):\n${lines.join("\n")}${suffix}`, "info");
    return true;
  }

  // /tac notifications (no args) — open overlay in TUI, or print summary
  if (args === "" || args === "status") {
  ...
  // Unknown subcommand
  ctx.ui.notify(
    "Usage: /tac notifications [clear|tail [N]|filter <severity>]",
    "warning",
  );
  return true;
}
