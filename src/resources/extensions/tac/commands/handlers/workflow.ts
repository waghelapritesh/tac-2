...
/**
 * Refuses interactive commands that mutate durable .tac/ planning state while
...
function requireNotAutoActive(commandName: string, ctx: ExtensionCommandContext): boolean {
  ...
  ctx.ui.notify(
    ...
    `Stop auto-mode first with /tac stop, then run ${commandName}.`,
    "error",
  );
  return true;
}
...
const WORKFLOW_USAGE = [
  "Usage: /tac workflow [<name> | <subcommand>]",
  "",
 ...
 * is already known (e.g., direct `/tac workflow <name> ...` dispatch) so the
  ...
  switch (plugin.meta.mode) {
    ...
    case "auto-milestone": {
      ctx.ui.notify(
        ...
        `Use /tac auto or /tac start ${plugin.name}.`,
        "info",
      );
      return;
    }
...
async function handleCustomWorkflow(
  ...
  // Bare `/tac workflow` — list plugins
  if (!sub) {
    const base = projectRoot();
  ...
  if (head === "run") {
    if (!rest) {
      ctx.ui.notify("Usage: /tac workflow run <name> [param=value ...]", "warning");
      return true;
    }
  ...
  if (head === "info") {
    if (!rest) {
      ctx.ui.notify("Usage: /tac workflow info <name>", "warning");
      return true;
    }
    const base = projectRoot();
    const plugin = resolvePlugin(base, rest);
    if (!plugin) {
      ctx.ui.notify(`Plugin not found: ${rest}\nRun /tac workflow to list plugins.`, "warning");
      return true;
    }
  ...
  if (head === "install") {
    if (!rest) {
      ctx.ui.notify(
        "Usage: /tac workflow install <source> [--project] [--name <n>]\n\n" +
        "Sources:\n" +
    ...
    try {
      ...
      const summary = [
        ...
        "  " + preview.split("\n").join("\n  "),
        "",
        `Proceeding with install. Run /tac workflow uninstall ${name} to revert.`,
      ].join("\n");
  ...
  if (head === "uninstall") {
    if (!rest) {
      ctx.ui.notify("Usage: /tac workflow uninstall <name>", "warning");
      return true;
    }
    const base = projectRoot();
  ...
  if (head === "validate") {
    if (!rest) {
      ctx.ui.notify("Usage: /tac workflow validate <name>", "warning");
      return true;
    }
    const base = projectRoot();
    const plugin = resolvePlugin(base, rest);
    ...
    if (plugin && plugin.format === "yaml") {
      ...
      const defPath = join(base, ".tac", "workflow-defs", `${rest}.yaml`);
  ...
  if (head === "pause" && !rest) {
    ...
    if (engineId === "dev" || engineId === null) {
      ctx.ui.notify("No custom workflow is running. Use /tac pause for dev workflow.", "warning");
      return true;
    }
    if (!isAutoActive()) {
  ...
  if (head === "resume" && !rest) {
    const engineId = getActiveEngineId();
    if (engineId === "dev" || engineId === null) {
      ctx.ui.notify("No custom workflow to resume. Use /tac auto for dev workflow.", "warning");
      return true;
    }
  ...
  // ── Direct dispatch: /tac workflow <name> [args] ──
...
export async function handleWorkflowCommand(trimmed: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<boolean> {
  // ── /tac do — natural language routing (must be early to route to other commands) ──
  if (trimmed === "do" || trimmed.startsWith("do ")) {
    if (requireNotAutoActive("/tac do", ctx)) return true;
  ...
  if (trimmed === "queue") {
    if (requireNotAutoActive("/tac queue", ctx)) return true;
    await showQueue(ctx, pi, projectRoot());
    return true;
  }
  if (trimmed === "discuss") {
    if (requireNotAutoActive("/tac discuss", ctx)) return true;
    await showDiscuss(ctx, pi, projectRoot());
    return true;
  }
  if (trimmed === "quick" || trimmed.startsWith("quick ")) {
    if (requireNotAutoActive("/tac quick", ctx)) return true;
    await handleQuick(trimmed.replace(/^quick\s*/, "").trim(), ctx, pi);
    return true;
  }
  if (trimmed === "new-milestone") {
    if (requireNotAutoActive("/tac new-milestone", ctx)) return true;
    const basePath = projectRoot();
    const headlessContextPath = join(tacRoot(basePath), "runtime", "headless-context.md");
  ...
  if (trimmed === "park" || trimmed.startsWith("park ")) {
    if (requireNotAutoActive("/tac park", ctx)) return true;
    const basePath = projectRoot();
    ...
    if (isParked(basePath, targetId)) {
      ctx.ui.notify(`${targetId} is already parked. Use /tac unpark ${targetId} to reactivate.`, "info");
      return true;
    }
    const reasonParts = arg.replace(targetId, "").trim().replace(/^["']|["']$/g, "");
    const reason = reasonParts || "Parked via /tac park";
    const success = parkMilestone(basePath, targetId, reason);
    ctx.ui.notify(
      success ? `Parked ${targetId}. Run /tac unpark ${targetId} to reactivate.` : `Could not park ${targetId} — milestone not found.`,
      success ? "info" : "warning",
    );
    return true;
  }
  if (trimmed === "unpark" || trimmed.startsWith("unpark ")) {
    if (requireNotAutoActive("/tac unpark", ctx)) return true;
    const basePath = projectRoot();
    ...
    if (!targetId) {
      ...
      if (parkedEntries.length === 1) {
        ...
        ctx.ui.notify(`Parked milestones: ${parkedEntries.map((entry) => entry.id).join(", ")}. Specify which to unpark: /tac unpark <id>`, "info");
        return true;
      }
    }
...
export function getNextMilestoneId(basePath: string): string {
  ...
  const uniqueIds = !!loadEffectiveTACPreferences()?.preferences?.unique_milestone_ids;
