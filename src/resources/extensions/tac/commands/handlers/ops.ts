...
export async function handleOpsCommand(trimmed: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<boolean> {
  if (trimmed === "init") {
    ...
    if (detection.state === "v2-tac" || detection.state === "v2-tac-empty") {
  ...
  if (trimmed === "skip") {
    ctx.ui.notify("Usage: /tac skip <unit-id>  Example: /tac skip M001/S01/T03", "warning");
    return true;
  }
...
Examples:
  ...
  if (trimmed === "steer") {
    ctx.ui.notify("Usage: /tac steer <description of change>. Example: /tac steer Use Postgres instead of SQLite", "warning");
    return true;
  }
  ...
  if (trimmed === "knowledge") {
    ctx.ui.notify("Usage: /tac knowledge <rule|pattern|lesson> <description>. Example: /tac knowledge rule Use real DB for integration tests", "warning");
    return true;
  }
  ...
  if (trimmed === "dispatch" || trimmed.startsWith("dispatch ")) {
    ...
    if (!phase) {
      ctx.ui.notify("Usage: /tac dispatch <phase>  (research|plan|execute|complete|reassess|uat|replan)", "warning");
      return true;
    }
  ...
  if (trimmed === "scan" || trimmed.startsWith("scan ")) {
    ...
    // \s* (not \s+) is intentional: handles both /tac scan (no args) and /tac scan --focus X
