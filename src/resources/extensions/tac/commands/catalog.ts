...
export const TOP_LEVEL_SUBCOMMANDS: readonly TacCommandDefinition[] = [
  ...
  { cmd: "config", desc: "(deprecated) Set tool API keys — use /tac keys instead" },
  ...
  { cmd: "debug", desc: "Create and inspect persistent /tac debug sessions" },
  { cmd: "forensics", desc: "Examine execution logs" },
  { cmd: "init", desc: "Project init wizard — detect, configure, bootstrap .tac/" },
  ...
  { cmd: "migrate", desc: "Migrate a v1 .planning directory to .tac format" },
  ...
  { cmd: "update", desc: "Update TAC to the latest version" },
  ...
  { cmd: "codebase", desc: "Generate, refresh, and inspect the codebase map cache (.tac/CODEBASE.md)" },
  { cmd: "ship", desc: "Create PR from milestone artifacts and open for review" },
  { cmd: "do", desc: "Route freeform text to the right TAC command" },
  ...
  { cmd: "pr-branch", desc: "Create clean PR branch filtering .tac/ commits" },
  ...
  { cmd: "language", desc: "Set or clear the global response language (e.g. /tac language Chinese)" },
];
...
const NESTED_COMPLETIONS: CompletionMap = {
  ...
  setup: [
    ...
    { cmd: "keys", desc: "Manage API keys (alias for /tac keys)" },
    { cmd: "prefs", desc: "Global preferences wizard (alias for /tac prefs)" },
    { cmd: "onboarding", desc: "Run the full onboarding wizard (alias for /tac onboarding)" },
  ],
  onboarding: [
  ...
  cleanup: [
    ...
    { cmd: "projects", desc: "Audit orphaned ~/.tac/projects/ state directories" },
  ...
  start: [
    ...
    { cmd: "full-project", desc: "Complete TAC workflow with full ceremony" },
  ...
  mcp: [
    ...
    { cmd: "init", desc: "Write .mcp.json for the local TAC workflow MCP server" },
  ],
  doctor: [
  ...
  "session-report": [
    ...
    { cmd: "--save", desc: "Save report to .tac/reports/" },
  ],
  backlog: [
...
function filterOptions(
  partial: string,
  options: readonly TacCommandDefinition[],
  prefix = "",
) {
...
function getExtensionCompletions(prefix: string, action: string) {
  try {
    const extDir = join(tacHome, "agent", "extensions");
...
export function getTacArgumentCompletions(prefix: string) {
  ...
  if (command === "workflow" && (subcommand === "run" || subcommand === "validate") && parts.length <= 3) {
    try {
      const defsDir = join(resolveProjectRoot(process.cwd()), ".tac", "workflow-defs");
  ...
  // Completion for `/tac workflow info <name>` — list all discoverable plugins (project + global).
  if (command === "workflow" && subcommand === "info" && parts.length <= 3) {
    const results: TacCommandDefinition[] = [];
    ...
    try {
      ...
      scanDir(join(base, ".tac", "workflows"), "project");
      scanDir(join(base, ".tac", "workflow-defs"), "project-legacy");
      scanDir(join(tacHome, "workflows"), "global");
    } catch { /* ignore */ }
