/**
 * TAC Doctor — Environment Health Checks (#1221)
 *
...
const WORKTREE_PATH_SEGMENT = `${join(".tac", "worktrees")}/`;

/**
 * Resolve the project root when running inside a `.tac/worktrees/<name>/`
 * auto-worktree. Returns `null` if not in a worktree.
 *
 * Detection order:
 *   1. `TAC_WORKTREE` env var (set by the worktree launcher)
 *   2. `.tac/worktrees/` segment in basePath
 */
function resolveWorktreeProjectRoot(basePath: string): string | null {
  const envRoot = process.env.TAC_WORKTREE;
  ...
  // Everything before `.tac/worktrees/` is the project root
 ...
 * Use this for explicit /tac doctor invocations, not pre-dispatch gates.
 */
 ...
 * These are never run on the pre-dispatch gate — only on explicit /tac doctor --build/--test.
 */
