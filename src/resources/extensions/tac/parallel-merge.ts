/**
 * TAC Parallel Merge — Worktree reconciliation for parallel milestones.
 *
 ...
 * Returns true when milestones.status = 'complete' in the worktree's tac.db.
 */
 ...
 * scanning .tac/worktrees/<MID>/.tac/tac.db for each worktree directory.
 */
 ...
 * See: https://github.com/waghelapritesh/tac-2/issues/2812
 */
  ...
  for (const mid of mergeableIds) {
    ...
    if (w) {
      ...
      allMergeable.push({
        ...
        pid: 0,
        process: null,
        worktreePath: basePath ? join(basePath, ".tac", "worktrees", mid) : "",
        startedAt: 0,
        state: "stopped",
        cost: 0,
      });
...
export function formatMergeResults(results: MergeResult[]): string {
  ...
  for (const r of results) {
    if (r.success) {
      ...
      lines.push(`  Resolve conflicts manually and run \`/tac parallel merge ${r.milestoneId}\` to retry.`);
    } else {
