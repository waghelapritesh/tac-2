...
import type { ExtensionContext } from "@tac/pi-coding-agent";

import { logWarning } from "../workflow-logger.js";
import { debugTime } from "../debug-logger.js";
import { loadPrompt, getTemplatesDir } from "../prompt-loader.js";
import { readForensicsMarker } from "../forensics.js";
import { resolveAllSkillReferences, renderPreferencesForSystemPrompt, loadEffectiveTACPreferences } from "../preferences.js";
import { resolveModelWithFallbacksForUnit } from "../preferences-models.js";
import { resolveSkillReference } from "../preferences-skills.js";
import { resolveTacRootFile, resolveSliceFile, resolveSlicePath, resolveTaskFile, resolveTaskFiles, resolveTasksDir, relSliceFile, relSlicePath, relTaskFile } from "../paths.js";
...
export const BUNDLED_SKILL_TRIGGERS: Array<{ trigger: string; skill: string }> = [
  ...
  { trigger: "Post-mortem a failed TAC auto-mode run using .tac/activity, .tac/journal, and .tac/metrics.json", skill: "forensics" },
...
function warnDeprecatedAgentInstructions(): void {
  const paths = [
    join(tacHome, "agent-instructions.md"),
    join(process.cwd(), ".tac", "agent-instructions.md"),
  ];
  for (const path of paths) {
    if (existsSync(path)) {
      console.warn(
        `[TAC] DEPRECATED: ${path} is no longer loaded. ` +
        `Migrate your instructions to AGENTS.md (or CLAUDE.md) in the same directory. ` +
        `See https://github.com/waghelapritesh/tac-2/issues/1492`,
      );
    }
  }
}
...
export async function buildBeforeAgentStartResult(
  ...
  if (!existsSync(join(process.cwd(), ".tac"))) return undefined;
  ...
  let loadedPreferences = loadEffectiveTACPreferences();
  try {
    const { markCmuxPromptShown, shouldPromptToEnableCmux } = await import("../../cmux/index.js");
    if (shouldPromptToEnableCmux(loadedPreferences?.preferences)) {
      markCmuxPromptShown();
      if (autoEnableCmuxPreferences()) {
        loadedPreferences = loadEffectiveTACPreferences();
        ctx.ui.notify(
          "cmux detected — auto-enabled. Run /tac cmux off to disable.",
          "info",
        );
      }
    }
  ...
  if (loadedPreferences) {
    ...
    if (report.warnings.length > 0) {
      ctx.ui.notify(
        `TAC skill preferences: ${report.warnings.length} unresolved skill${report.warnings.length === 1 ? "" : "s"}: ${report.warnings.join(", ")}`,
        "warning",
      );
    }
  }

  const { block: knowledgeBlock, globalSizeKb } = loadKnowledgeBlock(tacHome, process.cwd());
  if (globalSizeKb > 4) {
    ctx.ui.notify(
      `TAC: ~/.tac/agent/KNOWLEDGE.md is ${globalSizeKb.toFixed(1)}KB — consider trimming to keep system prompt lean.`,
      "warning",
    );
  }
  ...
  try {
    ...
    if (written > 0) {
      ctx.ui.notify(`TAC: backfilled ${written} decision${written === 1 ? "" : "s"} into the memory store (ADR-013).`, "info");
    }
  } catch (e) {
  ...
  const codebasePath = resolveTacRootFile(process.cwd(), "CODEBASE");
  ...
  if (existsSync(codebasePath) && rawCodebase) {
    try {
      ...
      if (rawContent) {
        ...
        // Full map is always available at .tac/CODEBASE.md.
        ...
        const content = rawContent.length > MAX_CODEBASE_CHARS
          ? rawContent.slice(0, MAX_CODEBASE_CHARS) + "\n\n*(truncated — see .tac/CODEBASE.md for full map)*"
          : rawContent;
        codebaseBlock = `\n\n[PROJECT CODEBASE — File structure and descriptions (generated ${generatedAt}, auto-refreshed when TAC detects tracked file changes; use /tac codebase stats for status)]\n\n${content}`;
      }
    } catch (e) {
  ...
  const contextMessage = injection
    ? { customType: "tac-guided-context", content: injection, display: false as const }
    : forensicsInjection
      ? { customType: "tac-forensics", content: forensicsInjection, display: false as const }
      : null;
 ...
 *    that future TAC turns generally want without asking. After ADR-013
...
export async function loadMemoryBlock(userPrompt: string): Promise<string> {
  try {
    ...
    return `\n\n[MEMORY — Critical and prompt-relevant memories from the TAC memory store]\n\n${formatted}`;
  } catch (e) {
...
export function loadKnowledgeBlock(tacHomeDir: string, cwd: string): { block: string; globalSizeKb: number } {
  // 1. Global knowledge (~/.tac/agent/KNOWLEDGE.md) — cross-project, user-maintained
  let globalKnowledge = "";
  let globalSizeKb = 0;
  const globalKnowledgePath = join(tacHomeDir, "agent", "KNOWLEDGE.md");
  ...
  // 2. Project knowledge (.tac/KNOWLEDGE.md) — project-specific
  let projectKnowledge = "";
  const knowledgePath = resolveTacRootFile(cwd, "KNOWLEDGE");
...
function buildWorktreeContextBlock(): string {
  ...
  if (worktreeName && worktreeMainCwd) {
    return [
      ...
      "",
      `You are working inside a TAC worktree.`,
      ...
      "",
      "All file operations, bash commands, and TAC state resolve against the worktree path above.",
  ...
  if (autoWorktree) {
    return [
      ...
      `The actual current working directory is: ${toPosixPath(process.cwd())}`,
      "",
      "You are working inside a TAC auto-worktree.",
      ...
      "",
      "All file operations, bash commands, and TAC state resolve against the worktree path above.",
      "Write every .tac artifact in the worktree path above, never in the main project tree.",
    ].join("\n");
  }
...
export function clearForensicsMarker(basePath: string): void {
  const markerPath = join(basePath, ".tac", "runtime", "active-forensics.json");
