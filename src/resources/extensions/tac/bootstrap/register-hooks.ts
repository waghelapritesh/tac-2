...
import { isToolCallEventType } from "@tac/pi-coding-agent";

import type { TACEcosystemBeforeAgentStartHandler } from "../ecosystem/tac-extension-api.js";
import { updateSnapshot } from "../ecosystem/tac-extension-api.js";
...
async function syncServiceTierStatus(ctx: ExtensionContext): Promise<void> {
  ...
  ctx.ui.setStatus("tac-fast", formatServiceTierFooterStatus(getEffectiveServiceTier(), ctx.model?.id));
}

export function registerHooks(
  pi: ExtensionAPI,
  ecosystemHandlers: TACEcosystemBeforeAgentStartHandler[],
): void {
  pi.on("session_start", async (_event, ctx) => {
    ...
    try {
      ...
      process.env.TAC_SHOW_TOKEN_COST = prefs?.preferences.show_token_cost ? "1" : "";
    } catch { /* non-fatal */ }
    if (isFirstSession) {
      isFirstSession = false;
    } else {
      try {
        const tacBinPath = process.env.TAC_BIN_PATH;
        if (tacBinPath) {
          const { dirname } = await import("node:path");
          const { printWelcomeScreen } = await import(
            join(dirname(tacBinPath), "welcome-screen.js")
          ...
          printWelcomeScreen({ version: process.env.TAC_VERSION || "0.0.0", remoteChannel });
        }
      } catch { /* non-fatal */ }
    }
    loadToolApiKeys();
  ...
  pi.on("before_agent_start", async (event, ctx: ExtensionContext) => {
    ...
    // TAC's own context injection (existing behavior — unchanged).
    const tacResult = await buildBeforeAgentStartResult(event, ctx);
    ...
    let currentSystemPrompt = tacResult?.systemPrompt ?? event.systemPrompt;
    // `any` because pi's BeforeAgentStartEventResult.message uses an internal
    // CustomMessage type that's not re-exported (see ecosystem/tac-extension-api.ts).
    let lastMessage: any = tacResult?.message;
  ...
  pi.on("session_before_compact", async () => {
    ...
    await saveFile(continuePath, formatContinue({
      ...
      context: "Session was auto-compacted by Pi. Resume with /tac.",
      nextAction: `Resume task ${state.activeTask.id}: ${state.activeTask.title}.`,
    }));
  });

  // Context-mode snapshot: write .tac/last-snapshot.md before compaction so
  // agents can call tac_resume (or Read the file) to re-orient. Opt-in via
  ...
  pi.on("tool_call", async (event) => {
    ...
    // When /tac queue is active, the agent should only create milestones,
    // not execute work. Block write/edit to non-.tac/ paths and bash commands
  ...
  pi.on("tool_result", async (event) => {
    ...
    if (isAutoActive() && event.isError && event.toolName.startsWith("tac_")) {
  ...
  pi.on("tool_execution_end", async (event) => {
    ...
    if (event.isError && event.toolName.startsWith("tac_")) {
