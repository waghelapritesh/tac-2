// TAC2 — Memory tool registration
//
// Exposes the memory-layer tools (capture_thought, memory_query, tac_graph)
// to the LLM over MCP. All three degrade gracefully when the TAC database
// is unavailable.
...
export function registerMemoryTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description:
      ...
      "preference, or environment detail) into the TAC memory store. Use sparingly — one memory " +
      "per genuinely reusable insight, not per task.",
    promptSnippet:
      "Capture a durable project insight into the TAC memory store (categories: architecture, convention, gotcha, pattern, preference, environment)",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!ok) {
        return {
          content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot capture memory." }],
  ...
  pi.registerTool({
    ...
    label: "Query Memory",
    description:
      "Search the TAC memory store for relevant memories. Phase 1 uses keyword matching ranked " +
      "by confidence and reinforcement; future phases add semantic (embedding) retrieval.",
    promptSnippet:
      "Search the TAC memory store by keyword; returns ranked memories with id, category, and content",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!ok) {
        return {
          content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot query memory." }],
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!ok) {
        return {
          content: [{ type: "text" as const, text: "Error: TAC database is not available." }],
          details: { operation: "tac_graph", error: "db_unavailable" },
          isError: true,
        };
      }
      return executeTacGraph(params as Parameters<typeof executeTacGraph>[0]);
    },
  });
}
