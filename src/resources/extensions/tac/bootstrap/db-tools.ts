...
import { StringEnum } from "@tac/pi-ai";
...
export function registerDbTools(pi: ExtensionAPI): void {
  // ─── tac_decision_save (formerly tac_save_decision) ─────────────────────
  ...
  const decisionSaveExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    if (!dbAvailable) {
      return {
        content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot save decision." }],
    ...
    try {
      ...
      logError("tool", `tac_decision_save tool failed: ${msg}`, { tool: "tac_decision_save", error: String(err) });
      return {
  ...
  const decisionSaveTool = {
    ...
    promptSnippet: "Record a project decision to the TAC database (auto-assigns ID, regenerates DECISIONS.md)",
    promptGuidelines: [
      "Use tac_decision_save when recording an architectural, pattern, library, or observability decision.",
      ...
      "The tool writes to the DB and regenerates .tac/DECISIONS.md automatically.",
  ...
  registerAlias(pi, decisionSaveTool, "tac_save_decision", "tac_decision_save");

  // ─── tac_requirement_update (formerly tac_update_requirement) ───────────
  ...
  const requirementUpdateExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    if (!dbAvailable) {
      return {
        content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot update requirement." }],
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `tac_requirement_update tool failed: ${msg}`, { tool: "tac_requirement_update", error: String(err) });
      return {
  ...
  const requirementUpdateTool = {
    ...
    promptSnippet: "Update an existing TAC requirement by ID (regenerates REQUIREMENTS.md)",
    promptGuidelines: [
      "Use tac_requirement_update to change status, validation, notes, or other fields on an existing requirement.",
  ...
  registerAlias(pi, requirementUpdateTool, "tac_update_requirement", "tac_requirement_update");
  ...
  const requirementSaveExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    if (!dbAvailable) {
      return {
        content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot save requirement." }],
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `tac_requirement_save tool failed: ${msg}`, { tool: "tac_requirement_save", error: String(err) });
      return {
  ...
  const requirementSaveTool = {
    ...
    promptSnippet: "Record a new TAC requirement to the database (auto-assigns ID, regenerates REQUIREMENTS.md)",
    promptGuidelines: [
      "Use tac_requirement_save when recording a new functional, non-functional, or operational requirement.",
      ...
      "The tool writes to the DB and regenerates .tac/REQUIREMENTS.md automatically.",
    ],
    parameters: Type.Object({
  ...
  registerAlias(pi, requirementSaveTool, "tac_save_requirement", "tac_requirement_save");

  // ─── tac_summary_save (formerly tac_save_summary) ──────────────────────
  ...
  const summarySaveTool = {
    name: "tac_summary_save",
    label: "Save Summary",
    description:
      "Save a summary, research, context, or assessment artifact to the TAC database and write it to disk. " +
      "Computes the file path from milestone/slice/task IDs automatically.",
    promptSnippet: "Save a TAC artifact (summary/research/context/assessment) to DB and disk",
    promptGuidelines: [
      "Use tac_summary_save to persist structured artifacts (SUMMARY, RESEARCH, CONTEXT, ASSESSMENT, CONTEXT-DRAFT).",
  ...
  registerAlias(pi, summarySaveTool, "tac_save_summary", "tac_summary_save");

  // ─── tac_milestone_generate_id (formerly tac_generate_milestone_id) ────
  ...
  const milestoneGenerateIdExecute = async (_toolCallId: string, _params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    try {
      ...
      const uniqueEnabled = !!loadEffectiveTACPreferences()?.preferences?.unique_milestone_ids;
   ...
   * machine. Uses INSERT OR IGNORE — safe to call even if tac_plan_milestone
  ...
  async function ensureMilestoneDbRow(milestoneId: string): Promise<void> {
    ...
    try {
      const { insertMilestone } = await import("../tac-db.js");
  ...
  const milestoneGenerateIdTool = {
    name: "tac_milestone_generate_id",
    ...
    promptGuidelines: [
      "ALWAYS call tac_milestone_generate_id before creating a new milestone directory or writing milestone files.",
  ...
  registerAlias(pi, milestoneGenerateIdTool, "tac_generate_milestone_id", "tac_milestone_generate_id");

  // ─── tac_plan_milestone (tac_milestone_plan alias) ─────────────────────
  ...
  const planMilestoneTool = {
    name: "tac_plan_milestone",
    label: "Plan Milestone",
    description:
      "Write milestone planning state to the TAC database, render ROADMAP.md from DB, and clear caches after a successful render.",
    promptSnippet: "Plan a milestone via DB write + roadmap render + cache invalidation",
    promptGuidelines: [
      "Use tac_plan_milestone for milestone planning instead of writing ROADMAP.md directly.",
      ...
      "Use the canonical name tac_plan_milestone; tac_milestone_plan is only an alias.",
    ],
    parameters: Type.Object({
  ...
  registerAlias(pi, planMilestoneTool, "tac_milestone_plan", "tac_plan_milestone");

  // ─── tac_plan_slice (tac_slice_plan alias) ─────────────────────────────
  ...
  const planSliceTool = {
    ...
    label: "Plan Slice",
    description:
      "Write slice planning state to the TAC database, render S##-PLAN.md plus task PLAN artifacts from DB, and clear caches after a successful render.",
    promptSnippet: "Plan a slice via DB write + PLAN render + cache invalidation",
    promptGuidelines: [
      "Use tac_plan_slice for slice planning instead of writing S##-PLAN.md or task PLAN files directly.",
      ...
      "Use the canonical name tac_plan_slice; tac_slice_plan is only an alias.",
    ],
    parameters: Type.Object({
      // ── Core identification + content (required) ──────────────────────
      milestoneId: Type.String({ description: "Milestone ID (e.g. M001)" }),
  ...
  registerAlias(pi, planSliceTool, "tac_slice_plan", "tac_plan_slice");

  // ─── tac_plan_task (tac_task_plan alias) ───────────────────────────────

  const planTaskExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    const dbAvailable = await ensureDbOpen();
    if (!dbAvailable) {
      return {
        content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot plan task." }],
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `plan_task tool failed: ${msg}`, { tool: "tac_plan_task", error: String(err) });
      return {
  ...
  const planTaskTool = {
    ...
    label: "Plan Task",
    description:
      "Write task planning state to the TAC database, render tasks/T##-PLAN.md from DB, and clear caches after a successful render.",
    promptSnippet: "Plan a task via DB write + task PLAN render + cache invalidation",
    promptGuidelines: [
      "Use tac_plan_task for task planning instead of writing tasks/T##-PLAN.md directly.",
      ...
      "Use the canonical name tac_plan_task; tac_task_plan is only an alias.",
    ],
    parameters: Type.Object({
      milestoneId: Type.String({ description: "Milestone ID (e.g. M001)" }),
      sliceId: Type.String({ description: "Slice ID (e.g. S01)" }),
  ...
  registerAlias(pi, planTaskTool, "tac_task_plan", "tac_plan_task");

  // ─── tac_task_complete (tac_complete_task alias) ────────────────────────
  ...
  const taskCompleteTool = {
    name: "tac_task_complete",
    label: "Complete Task",
    description:
      "Record a completed task to the TAC database, render a SUMMARY.md to disk, and toggle the plan checkbox — all in one atomic operation. " +
      "Writes the task row inside a transaction, then performs filesystem writes outside the transaction.",
    promptSnippet: "Complete a TAC task (DB write + summary render + checkbox toggle)",
    promptGuidelines: [
      "Use tac_task_complete (or tac_complete_task) when a task is finished and needs to be recorded.",
    ...
    parameters: Type.Object({
      ...
      escalation: Type.Optional(Type.Object({
        ...
        options: Type.Array(Type.Object({
          id: Type.String({ description: "Short id (e.g. 'A', 'B') used by /tac escalate resolve." }),
        ...
        continueWithDefault: Type.Boolean({
          description: "When true, loop continues (artifact logged for later review). When false, auto-mode pauses until the user resolves via /tac escalate resolve.",
        }),
  ...
  registerAlias(pi, taskCompleteTool, "tac_complete_task", "tac_task_complete");

  // ─── tac_slice_complete (tac_complete_slice alias) ─────────────────────
  ...
  const sliceCompleteTool = {
    name: "tac_slice_complete",
    label: "Complete Slice",
    description:
      "Record a completed slice to the TAC database, render SUMMARY.md + UAT.md to disk, and toggle the roadmap checkbox — all in one atomic operation. " +
    ...
    promptSnippet: "Complete a TAC slice (DB write + summary/UAT render + roadmap checkbox toggle)",
    promptGuidelines: [
      "Use tac_slice_complete (or tac_complete_slice) when all tasks in a slice are finished and the slice needs to be recorded.",
  ...
  registerAlias(pi, sliceCompleteTool, "tac_complete_slice", "tac_slice_complete");

  // ─── tac_skip_slice (#3477 / #3487) ───────────────────────────────────

  const skipSliceExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    const dbAvailable = await ensureDbOpen();
    if (!dbAvailable) {
      return {
        content: [{ type: "text" as const, text: "Error: TAC database is not available. Cannot skip slice." }],
    ...
    try {
      ...
      // Without this, /tac auto reads stale STATE.md and resumes the skipped slice.
      try {
        const basePath = process.cwd();
        const { rebuildState } = await import("../doctor.js");
        await rebuildState(basePath);
      } catch (err) {
        logError("tool", `skip_slice rebuildState failed: ${(err as Error).message}`, { tool: "tac_skip_slice" });
      }
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `skip_slice tool failed: ${msg}`, { tool: "tac_skip_slice", error: String(err) });
      return {
  ...
  pi.registerTool({
    ...
    promptSnippet: "Skip a TAC slice (mark as skipped, auto-mode will advance past it)",
    promptGuidelines: [
      "Use tac_skip_slice when a slice should be bypassed — descoped, superseded, or no longer relevant.",
  ...
  const milestoneCompleteTool = {
    name: "tac_complete_milestone",
    label: "Complete Milestone",
    description:
      "Record a completed milestone to the TAC database, render MILESTONE-SUMMARY.md to disk — all in one atomic operation. " +
      "Validates all slices are complete before proceeding.",
    promptSnippet: "Complete a TAC milestone (DB write + summary render)",
    promptGuidelines: [
      "Use tac_complete_milestone when all slices in a milestone are finished and the milestone needs to be recorded.",
  ...
  registerAlias(pi, milestoneCompleteTool, "tac_milestone_complete", "tac_complete_milestone");

  // ─── tac_validate_milestone (tac_milestone_validate alias) ─────────────
  ...
  const milestoneValidateTool = {
    ...
    promptSnippet: "Validate a TAC milestone (DB write + VALIDATION.md render)",
    promptGuidelines: [
      "Use tac_validate_milestone when all slices are done and the milestone needs validation before completion.",
      ...
      "If verdict is 'needs-remediation', also provide remediationPlan and use tac_reassess_roadmap to add remediation slices to the roadmap.",
  ...
  registerAlias(pi, milestoneValidateTool, "tac_milestone_validate", "tac_validate_milestone");

  // ─── tac_replan_slice (tac_slice_replan alias) ─────────────────────────
  ...
  const replanSliceTool = {
    ...
    promptSnippet: "Replan a TAC slice with structural enforcement of completed tasks",
    promptGuidelines: [
      "Use tac_replan_slice (canonical) or tac_slice_replan (alias) when a blocker is discovered and the slice plan needs rewriting.",
  ...
  registerAlias(pi, replanSliceTool, "tac_slice_replan", "tac_replan_slice");

  // ─── tac_reassess_roadmap (tac_roadmap_reassess alias) ─────────────────
  ...
  const reassessRoadmapTool = {
    ...
    promptSnippet: "Reassess a TAC roadmap with structural enforcement of completed slices",
    promptGuidelines: [
      "Use tac_reassess_roadmap (canonical) or tac_roadmap_reassess (alias) after a slice completes to reassess the roadmap.",
  ...
  registerAlias(pi, reassessRoadmapTool, "tac_roadmap_reassess", "tac_reassess_roadmap");
  ...
  const saveGateResultTool = {
    name: "tac_save_gate_result",
    label: "Save Gate Result",
    description:
      "Save the result of a quality gate evaluation (Q3-Q8 or MV01-MV04) to the TAC database. " +
    ...
    promptGuidelines: [
      "Use tac_save_gate_result after evaluating a quality gate question.",
