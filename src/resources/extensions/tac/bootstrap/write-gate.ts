...
/**
 * Path segment that identifies .tac/ planning artifacts.
 * Writes to these paths are allowed during queue mode.
 */
const TAC_DIR_RE = /(^|[/\\])\.tac([/\\]|$)/;
...
function shouldPersistWriteGateSnapshot(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.TAC_PERSIST_WRITE_GATE_STATE === "1";
}

function writeGateSnapshotPath(basePath: string = process.cwd()): string {
  return join(basePath, ".tac", "runtime", "write-gate-state.json");
}
...
/**
 * Check whether a tac_summary_save CONTEXT artifact should be blocked.
...
export function shouldBlockQueueExecutionInSnapshot(
  ...
  // write/edit — allow if targeting .tac/ planning artifacts
  if (toolName === "write" || toolName === "edit") {
    if (TAC_DIR_RE.test(input)) return { block: false };
    return {
      block: true,
      reason: `Blocked: /tac queue is a planning tool — it creates milestones, not executes work. ` +
  ...
  return {
    block: true,
    reason: `Blocked: /tac queue is a planning tool — it creates milestones, not executes work. Unknown tools are not permitted during queue mode.`,
  };
}
