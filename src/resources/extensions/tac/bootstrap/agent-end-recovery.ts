...
export async function handleAgentEnd(
  ...
  if (lastMsg && "stopReason" in lastMsg && lastMsg.stopReason === "error") {
    ...
    //        same dead model isn't reselected on the next /tac auto restart,
    ...
    if (cls.kind === "unsupported-model") {
          ...
          while (true) {
            ...
            if (candidate && !isModelBlocked(dash.basePath, candidate.provider, candidate.id)) {
              ...
              if (ok) {
                ...
                pi.sendMessage(
                  { customType: "tac-auto-timeout-recovery", content: "Continue execution.", display: false },
                  { triggerTurn: true },
                );
                return;
              }
      ...
      const pauseDetail = `Model ${blockedLabel} blocked for this account${errorDetail}. Configure a different model and restart /tac auto.`;
    ...
    if (cls.kind === "network") {
      ...
      if (retryState.networkRetryCount < MAX_NETWORK_RETRIES) {
        ...
        setTimeout(() => {
          pi.sendMessage(
            { customType: "tac-auto-timeout-recovery", content: "Continue execution — retrying after transient network error.", display: false },
            { triggerTurn: true },
          );
        }, delayMs);
        return;
          ...
          if (nextModelId) {
            ...
            if (modelToSet) {
              ...
              if (ok) {
                ...
                pi.sendMessage({ customType: "tac-auto-timeout-recovery", content: "Continue execution.", display: false }, { triggerTurn: true });
                return;
              }
            }
          }
