 ...
 *
 * Called by both `/tac start <template>` (existing markdown path) and
 * `/tac workflow <name>` (new direct dispatch). Keeps the prompt-build
...
export function dispatchOneshot(
  ...
  pi.sendMessage(
    { customType: "tac-workflow-oneshot", content: prompt, display: false },
    { triggerTurn: true },
  );
}
