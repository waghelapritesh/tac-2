/**
 * TAC Preferences Wizard — TUI wizard for configuring TAC preferences.
 *
...
import {
  ...
  getLegacyGlobalTACPreferencesPath,
...
export async function handlePrefs(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (trimmed === "project" || trimmed === "wizard project" || trimmed === "setup project") {
    await ensurePreferencesFile(getProjectTACPreferencesPath(), ctx, "project");
  ...
  if (trimmed === "status") {
    ...
    const canonicalGlobal = getGlobalTACPreferencesPath();
    const legacyGlobal = getLegacyGlobalTACPreferencesPath();
    ...
    const projectStatus = projectPrefs ? `present: ${projectPrefs.path}` : `missing: ${getProjectTACPreferencesPath()}`;

    const lines = [`TAC skill prefs — global ${globalStatus}; project ${projectStatus}`];

    const effective = loadEffectiveTACPreferences();
  ...
  ctx.ui.notify("Usage: /tac prefs [global|project|status|wizard|setup|import-claude [global|project]]", "info");
}
...
export async function handlePrefsMode(ctx: ExtensionCommandContext, scope: "global" | "project"): Promise<void> {
  const path = scope === "project" ? getProjectTACPreferencesPath() : getGlobalTACPreferencesPath();
  const existing = scope === "project" ? loadProjectTACPreferences() : loadGlobalTACPreferences();
...
export async function handlePrefsWizard(
  ...
  // pathOverride lets callers like /tac init pass a basePath-derived target
  // path so the wizard doesn't fall back to cwd-based getProjectTACPreferencesPath
  // when the init target diverges from the current working directory.
  const path = opts?.pathOverride
    ?? (scope === "project" ? getProjectTACPreferencesPath() : getGlobalTACPreferencesPath());
  const existing = scope === "project" ? loadProjectTACPreferences() : loadGlobalTACPreferences();
  // Order: existing-on-disk values, overlaid with prefill (caller's seeded answers).
  // Callers like /tac init pass freshly-collected init answers as prefill so the
  ...
  ctx.ui.notify(`TAC preferences (${scope}) — pick a category to configure.`, "info");
  ...
  while (true) {
    ...
    const raw = await ctx.ui.select("TAC Preferences", options);
 ...
 *
 * Both `/tac init` and the prefs wizard route through this helper so we can't
...
export async function ensurePreferencesFile(
  ...
  if (!existsSync(path)) {
    ...
    if (!template) {
      ctx.ui.notify("Could not load TAC preferences template.", "error");
      return;
    }
    await saveFile(path, template);
    ctx.ui.notify(`Created ${scope} TAC skill preferences at ${path}`, "info");
  } else {
    ctx.ui.notify(`Using existing ${scope} TAC skill preferences at ${path}`, "info");
  }
}

/**
 * Handle `/tac language [code]` — set or clear the global language preference.
 * Without an argument, shows the current setting.
 * Project-level override can be set by editing `.tac/PREFERENCES.md` directly
...
export async function handleLanguage(args: string, ctx: ExtensionCommandContext): Promise<void> {
  const path = getGlobalTACPreferencesPath();
  const lang = args.trim();

  // Show current setting when called without argument
  if (!lang) {
    const loaded = loadGlobalTACPreferences();
    const current = loaded?.preferences.language;
    if (current) {
      ctx.ui.notify(`Current language preference: ${current}\nUse /tac language <name> to change, or /tac language off to clear.`, "info");
    } else {
      ctx.ui.notify("No language preference set. Use /tac language <name> to set one (e.g. /tac language Chinese).", "info");
    }
    return;
  }
  ...
  if (lang === "off" || lang === "none" || lang === "clear") {
    ...
    ctx.ui.notify("Language preference cleared. TAC will use the default language.", "info");
  } else {
    // Validate before writing — reject values that would fail on next load
    if (lang.length > 50 || /[\r\n]/.test(lang)) {
      ctx.ui.notify(
        "Language value must be 50 characters or fewer with no newlines (e.g. /tac language Chinese).",
        "warning",
      );
      return;
    }
    prefs.language = lang;
    ctx.ui.notify(`Language preference set to: ${lang}\nTAC will now respond in ${lang} across all sessions.`, "info");
  }
