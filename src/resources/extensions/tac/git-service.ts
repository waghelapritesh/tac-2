 ...
 * TAC Git Service
 *
 * Core git operations for TAC: types, constants, and pure helpers.
...
import { TACError, TAC_MERGE_CONFLICT, TAC_GIT_ERROR } from "./errors.js";
...
export interface GitPreferences {
  ...
  remote?: string;
  snapshots?: boolean;
  /** Deprecated. .tac/ is managed externally; retained for compatibility. */
  commit_docs?: boolean;
  ...
  /** When false, TAC will not modify .gitignore at all — no baseline patterns
   *  are added and no self-healing occurs. Use this if you manage your own
   *  .gitignore and don't want TAC touching it.
   *  Default: true (TAC ensures baseline patterns are present).
   */
  ...
  /** Whether to squash `tac snapshot:` commits into the next real autoCommit.
 ...
 * Format: `{type}: {description}` (clean conventional commit — no TAC IDs in subject).
 *
 * TAC metadata is placed in a `TAC-Task:` git trailer at the end of the body,
...
export function buildTaskCommitMessage(ctx: TaskCommitContext): string {
  ...
  // Trailers: TAC-Task first, then Resolves
  bodyParts.push(`TAC-Task: ${ctx.taskId}`);
...
export class MergeConflictError extends TACError {
  ...
  constructor(
    ...
    super(
      ...
      `failed with conflicts in ${conflictedFiles.length} non-.tac file(s): ${conflictedFiles.join(", ")}`,
    );
...
/**
 * TAC runtime paths that should be excluded from smart staging.
 ...
 * Format: .tac/milestones/<MID>/<MID>-META.json
 */
function milestoneMetaPath(basePath: string, milestoneId: string): string {
  return join(tacRoot(basePath), "milestones", milestoneId, `${milestoneId}-META.json`);
}
...
export function writeIntegrationBranch(
  ...
  // tac/<templateId>/<slug> branches that are ephemeral.
  ...
  const metaFile = milestoneMetaPath(basePath, milestoneId);
  mkdirSync(join(tacRoot(basePath), "milestones", milestoneId), { recursive: true });
  ...
  // .tac/ is managed externally (symlinked) — metadata is not committed to git.
}
...
export function runGit(basePath: string, args: string[], options: { allowFailure?: boolean; input?: string } = {}): string {
  try {
    ...
    throw new TACError(TAC_GIT_ERROR, `git ${args.join(" ")} failed in ${basePath}: ${filterGitSvnNoise(message)}`);
  }
}
...
export class GitServiceImpl {
  ...
  /**
   * Smart staging: `git add -A` excluding TAC runtime paths via pathspec.
  ...
  private smartStage(extraExclusions: readonly string[] = []): void {
    ...
    // auto.lock, etc.) — NOT all of .tac/. If .tac/milestones/ files were
    ...
    // discontinuity where the first half of a milestone has .tac/ artifacts
    ...
    if (!this._runtimeFilesCleanedUp) {
      ...
      if (cleaned) {
        nativeCommit(this.basePath, "chore: untrack .tac/ runtime files from git index", { allowEmpty: false });
      }
    ...
    //
    // Exclude only RUNTIME paths from staging — not the entire .tac/ directory.
    // When .tac/milestones/ files are already tracked in the index (projects
    // where .tac/ is not gitignored, or Windows junctions that git sees as
    // real directories), they should continue to be committed. Excluding the
    // entire .tac/ directory mid-milestone causes silent commit failure where
    // the second half of a milestone's artifacts are never committed (#1326).
    //
    // If .tac/ IS in .gitignore (the default for external state projects),
    // git add -A already skips it and the exclusions are harmless no-ops.
    const allExclusions = [...RUNTIME_EXCLUSION_PATHS, ...extraExclusions];

    // ── Parallel worker milestone scope (#1991) ──
    // When TAC_MILESTONE_LOCK is set, this process is a parallel worker that
    ...
    const milestoneLock = process.env.TAC_MILESTONE_LOCK;
    if (milestoneLock) {
      const msDir = join(tacRoot(this.basePath), "milestones");
      if (existsSync(msDir)) {
        try {
          const entries = readdirSync(msDir, { withFileTypes: true });
          for (const entry of entries) {
            if (entry.isDirectory() && entry.name !== milestoneLock) {
              allExclusions.push(`.tac/milestones/${entry.name}/`);
            }
          }
        } catch {
   ...
   * @param extraExclusions Additional paths to exclude from staging (e.g. [".tac/"] for pre-switch commits).
   */
  autoCommit(
    unitType: string,
    unitId: string,
    ...
    // Absorb any preceding tac snapshot commits into this real commit.
  ...
  /**
   * Squash consecutive `tac snapshot:` commits that sit immediately below
  ...
  private absorbSnapshotCommits(headMessage: string): void {
    try {
      ...
      const TAC_SNAPSHOT_PREFIX = "tac snapshot:";
      let count = 0;
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (!subject.startsWith(TAC_SNAPSHOT_PREFIX)) break;
        count = i;
      }
      ...
      // ALL tracked modifications including .tac/ state files. Without
      // re-staging, those .tac/ changes leak into the absorbed commit.
      this.smartStage();
   ...
   * starts TAC on a feature branch like `f-123-new-thing`, that branch
   ...
   * Ref path: refs/tac/snapshots/<label>/<timestamp>
  ...
  createSnapshot(label: string): void {
    ...
    const refPath = `refs/tac/snapshots/${label}/${ts}`;
