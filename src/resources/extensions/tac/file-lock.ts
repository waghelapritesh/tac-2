...
// back to the installed tac-2 package if we are running from a consumer
...
function _require(name: string): any {
  try {
    ...
    try {
      const tacPiRequire = createRequire(
        join(process.cwd(), "node_modules", "tac-2", "index.js"),
      );
      return tacPiRequire(name);
    } catch {
      return null;
    }
  }
