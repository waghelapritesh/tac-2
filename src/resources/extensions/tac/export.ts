// TAC Extension — Session/Milestone Export
...
export function writeExportFile(
  ...
  const exportDir = tacRoot(basePath);
  ...
  if (format === "json") {
    ...
    const md = [
      `# TAC Session Report — ${projectName}`,
      ``,
  ...
  if (args.includes("--html")) {
    ...
    try {
      ...
      if (generateAll) {
        ...
        const indexPath = join(tacRoot(basePath), "reports", "index.html");
        ctx.ui.notify(
        ...
        ctx.ui.notify(
          `HTML report saved: .tac/reports/${bn(outPath)}\nOpening in browser...`,
          "success",
        );
