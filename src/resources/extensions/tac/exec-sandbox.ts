// TAC Exec Sandbox — tool-output sandboxing for sub-sessions.
//
// Runs a script in a subprocess and persists stdout/stderr to
// `.tac/exec/<id>.{stdout,stderr,meta.json}`. Only a short digest is
...
export interface ExecSandboxOptions {
  /** Project root. stdout/stderr persist under `<baseDir>/.tac/exec/`. */
  baseDir: string;
 ...
 * `.tac/exec/<id>.{stdout,stderr,meta.json}`, and return an `ExecSandboxResult`
...
export function runExecSandbox(
  ...
  return new Promise((resolveP) => {
    ...
    const execDir = resolve(opts.baseDir, ".tac", "exec");
