...
import { isDbAvailable, getTask, getSlice, getSliceTasks, getPendingGates, updateTaskStatus, updateSliceStatus, insertSlice, getMilestone } from "./tac-db.js";
...
/**
 * Check whether a milestone produced implementation artifacts (non-`.tac/` files)
 * in the git history. Uses `git log --name-only` to inspect all commits on the
 * current branch that touch files outside `.tac/`.
 *
 * Returns "present" if implementation files found, "absent" if only .tac/ files,
...
export function hasImplementationArtifacts(basePath: string): "present" | "absent" | "unknown" {
  try {
    ...
    // Filter out .tac/ files — only implementation files count.
    // If every changed file is under .tac/, the milestone produced no
    // implementation code (#1703).
    const implFiles = changedFiles.filter(f => !f.startsWith(".tac/") && !f.startsWith(".tac\\"));
...
export function verifyExpectedArtifact(
  ...
  if (unitType === "rewrite-docs") {
    const overridesPath = resolveTacRootFile(base, "OVERRIDES");
  ...
  if (unitType === "execute-task") {
    ...
    if (mid && sid && tid) {
      ...
      if (dbTask) {
        ...
        // does not prove tac_complete_task ran. Summary file on disk alone
  ...
  // A milestone with only .tac/ plan files and zero implementation code is
 ...
 * If resolved: finalize the commit. If only .tac conflicts remain: auto-resolve.
...
export function reconcileMergeState(
  ...
  if (conflictedFiles.length === 0) {
    ...
    try {
      const commitSha = nativeCommit(basePath, "chore(tac): reconcile merge state");
      if (commitSha) {
    ...
    // Still conflicted — try auto-resolving .tac/ state file conflicts (#530)
    const tacConflicts = conflictedFiles.filter((f) => f.startsWith(".tac/"));
    const codeConflicts = conflictedFiles.filter((f) => !f.startsWith(".tac/"));

    if (tacConflicts.length > 0 && codeConflicts.length === 0) {
      // All conflicts are in .tac/ state files — auto-resolve by accepting theirs
      let resolved = true;
      try {
        nativeCheckoutTheirs(basePath, tacConflicts);
        nativeAddPaths(basePath, tacConflicts);
      } catch (e) {
        logError("recovery", `auto-resolve .tac/ conflicts failed: ${(e as Error).message}`);
        resolved = false;
      }
      if (resolved) {
        try {
          nativeCommit(
            basePath,
            "chore: auto-resolve .tac/ state file conflicts",
          );
          ctx.ui.notify(
            `Auto-resolved ${tacConflicts.length} .tac/ state file conflict(s) from prior merge.`,
            "info",
          );
        } catch (e) {
          logError("recovery", `auto-commit .tac/ conflict resolution failed: ${(e as Error).message}`);
          resolved = false;
        }
      }
      if (!resolved) {
...
export function buildLoopRemediationSteps(
  ...
  switch (unitType) {
    case "execute-task": {
      ...
      return [
        `   1. Run \`tac undo-task ${tid}\` to reset the task state`,
        `   2. Resume auto-mode — it will re-execute the task`,
        `   3. If the task keeps failing, run \`tac recover\` to rebuild DB state from disk`,
      ].join("\n");
    }
    case "plan-slice":
    case "research-slice": {
      ...
      return [
        ...
        `   2. Run \`tac recover\` to rebuild DB state from disk`,
        `   3. Resume auto-mode`,
      ].join("\n");
    }
    case "complete-slice": {
      if (!mid || !sid) break;
      return [
        `   1. Run \`tac reset-slice ${sid}\` to reset the slice and all its tasks`,
        `   2. Resume auto-mode — it will re-execute incomplete tasks and re-complete the slice`,
        `   3. If the slice keeps failing, run \`tac recover\` to rebuild DB state from disk`,
      ].join("\n");
    }
