 ...
 * (e.g. `.tac/doctor-history.jsonl`). These dirs are created by
...
function isDoctorArtifactOnly(dirPath: string): boolean {
  try {
    ...
    // Only a .tac subdirectory
    if (entries.length === 1 && entries[0] === ".tac") {
      const tacEntries = readdirSync(join(dirPath, ".tac"));
      return tacEntries.length <= 1 && tacEntries.every(e => e === "doctor-history.jsonl");
    }
    return false;
  } catch {
    return false;
...
export async function checkGitHealth(
  ...
  try {
    const branchList = nativeBranchList(basePath, "tac/*/*")
      .filter((branch) => !branch.startsWith("tac/quick/"));
  ...
  try {
    ...
    if (existsSync(wtDir)) {
      // Resolve symlinks and normalize separators so that symlinked .tac
      // paths (e.g. ~/.tac/projects/<hash>/worktrees/…) match the paths
      ...
      for (const entry of readdirSync(wtDir)) {
        ...
        if (!registeredPaths.has(normalizedFullPath)) {
          // Skip directories that only contain doctor artifacts (.tac/doctor-history.jsonl).
        ...
        if (minutesSinceCommit >= thresholdMinutes) {
          ...
          if (shouldFix("stale_uncommitted_changes")) {
            try {
              ...
              const commitMsg = `tac snapshot: uncommitted changes after ${mins}m inactivity`;
              const result = nativeCommit(basePath, commitMsg);
              if (result) {
                fixesApplied.push(`created tac snapshot after ${mins}m of uncommitted changes`);
              } else {
                fixesApplied.push("tac snapshot skipped — nothing to commit after staging tracked files");
              }
            } catch {
              fixesApplied.push("failed to create tac snapshot commit");
            }
          }
        }
      }
  ...
  // Check TAC-managed worktrees for: merged branches, stale work, dirty
  // state, and unpushed commits. Only worktrees under .tac/worktrees/.
  try {
