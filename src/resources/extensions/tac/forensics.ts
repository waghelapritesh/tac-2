/**
 * TAC Forensics — Post-mortem investigation of auto-mode failures
 *
...
import { runTACDoctor, formatDoctorIssuesForPrompt, type DoctorIssue } from "./doctor.js";
import { verifyExpectedArtifact } from "./auto-recovery.js";
import { deriveState } from "./state.js";
import { isAutoActive } from "./auto.js";
import { loadPrompt } from "./prompt-loader.js";
import { tacRoot } from "./paths.js";
import { isDbAvailable, getAllMilestones, getMilestoneSlices, getSliceTasks } from "./tac-db.js";
import { isClosedStatus } from "./status-guards.js";
import { formatDuration } from "../shared/format-utils.js";
import { getAutoWorktreePath } from "./auto-worktree.js";
import { loadEffectiveTACPreferences, loadGlobalTACPreferences, getGlobalTACPreferencesPath } from "./preferences.js";
...
/** Summary of .tac/activity/ directory metadata. */
...
/**
 * Summary of .tac/journal/ data for forensic investigation.
 *
...
Before reading TAC source code or performing deep analysis, you MUST search for existing issues and PRs that may already address this bug. This avoids wasting tokens on already-fixed bugs.
...
1. **Search closed issues** for similar keywords:
   \`\`\`
   gh issue list --repo waghelapritesh/tac-2 --state closed --search "<keywords from root cause>" --limit 20
   \`\`\`

2. **Search open PRs** that might contain the fix:
   \`\`\`
   gh pr list --repo waghelapritesh/tac-2 --state open --search "<keywords>" --limit 10
   \`\`\`

3. **Search merged PRs** that may have already fixed this:
   \`\`\`
   gh pr list --repo waghelapritesh/tac-2 --state merged --search "<keywords>" --limit 10
   \`\`\`
...
async function writeForensicsDedupPref(ctx: ExtensionCommandContext, enabled: boolean): Promise<void> {
  const prefsPath = getGlobalTACPreferencesPath();
  await ensurePreferencesFile(prefsPath, ctx, "global");
  const existing = loadGlobalTACPreferences();
...
export async function handleForensics(
  ...
  if (!existsSync(root)) {
    ctx.ui.notify("No TAC state found. Run /tac auto first.", "warning");
    return;
  }
  ...
  const effectivePrefs = loadEffectiveTACPreferences()?.preferences;
  ...
  // Derive TAC source dir for prompt — fall back to ~/.tac/agent/extensions/tac/
  // when import.meta.url resolves to the npm-global install path (Windows).
  let tacSourceDir = dirname(fileURLToPath(import.meta.url));
  if (!existsSync(join(tacSourceDir, "prompts"))) {
    const tacHome = process.env.TAC_HOME || join(homedir(), ".tac");
    const fallback = join(tacHome, "agent", "extensions", "tac");
    if (existsSync(join(fallback, "prompts"))) tacSourceDir = fallback;
  }
  ...
  pi.sendMessage(
    { customType: "tac-forensics", content, display: false },
    { triggerTurn: true },
  );
...
export async function buildForensicReport(basePath: string): Promise<ForensicReport> {
  ...
  try {
    const report = await runTACDoctor(basePath, { scope: undefined });
  ...
  // 8. TAC version — use TAC_VERSION env var set by the loader at startup.
  // Extensions run from ~/.tac/agent/extensions/tac/ at runtime, so path-traversal
  // from import.meta.url would resolve to ~/package.json (wrong on every system).
  const tacVersion = process.env.TAC_VERSION || "unknown";
...
function resolveActivityDirs(basePath: string, activeMilestone?: string | null): string[] {
  ...
  if (activeMilestone) {
    ...
    if (wtPath) {
      const wtActivityDir = join(tacRoot(wtPath), "activity");
  ...
  const rootActivityDir = join(tacRoot(basePath), "activity");
...
function scanJournalForForensics(basePath: string): JournalSummary | null {
  try {
    const journalDir = join(tacRoot(basePath), "journal");
...
function loadCompletedKeys(basePath: string): string[] {
  const file = join(tacRoot(basePath), "completed-units.json");
  try {
    if (existsSync(file)) {
...
function detectTimeouts(traces: UnitTrace[], anomalies: ForensicAnomaly[]): void {
  for (const ut of traces) {
    ...
    const hasTimeout = ut.trace.toolCalls.some(tc =>
      tc.name === "sendmessage" &&
      JSON.stringify(tc.input).includes("tac-auto-timeout-recovery"),
    );
...
function detectWorktreeOrphans(
    ...
    anomalies.push({
      ...
      details:
        reason === "in-progress-unmerged"
          ? "Auto-mode exited without completing a milestone; live work sits on an unmerged milestone branch. Run `/tac auto` to resume, or merge manually."
          : reason === "complete-unmerged"
            ? "A completed milestone's branch was never merged back to main. Run `/tac health --fix` to resolve."
            : `Reason: ${reason}.`,
    });
  }
  ...
  if (summary.exitsWithUnmergedWork > 0) {
    ...
    anomalies.push({
      ...
      details: `Exit reasons: ${reasonBreakdown || "(none)"} · Producer-side signal for #4761-class orphans. Inspect .tac/journal/*.jsonl with eventType:"auto-exit" for per-exit detail.`,
    });
  }
}
...
function saveForensicReport(basePath: string, report: ForensicReport, problemDescription: string): string {
  const dir = join(tacRoot(basePath), "forensics");
...
export function writeForensicsMarker(basePath: string, reportPath: string, promptContent: string): void {
  const dir = join(tacRoot(basePath), "runtime");
  mkdirSync(dir, { recursive: true });
...
export function readForensicsMarker(basePath: string): ForensicsMarker | null {
  const markerPath = join(tacRoot(basePath), "runtime", "active-forensics.json");
...
function formatReportForPrompt(report: ForensicReport): string {
  ...
  sections.push(`### TAC Version: ${report.tacVersion}`);
