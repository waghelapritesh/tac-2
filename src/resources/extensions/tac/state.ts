// TAC Extension — State Derivation
...
import { nativeBatchParseTacFiles, type BatchParsedFile } from './native-parser-bridge.js';
...
export function isGhostMilestone(basePath: string, mid: string): boolean {
  ...
  // tac_milestone_generate_id that was never planned (#3645).
  if (isDbAvailable()) {
...
// of re-reading the entire .tac/ tree from disk.
...
export async function getActiveMilestoneId(basePath: string): Promise<string | null> {
  ...
  if (isDbAvailable()) {
    ...
    if (allMilestones.length > 0) {
      // Respect queue-order.json so /tac queue reordering is honored (#2556).
...
/**
 * Reconstruct TAC state from DB (primary) or filesystem (fallback).
 ...
 */
export async function deriveState(basePath: string): Promise<TACState> {
...
/**
 * Derive TAC state from the milestones/slices/tasks DB tables.
 ...
 *
 * Must produce field-identical TACState to _deriveStateImpl() for the same project.
 */
...
function handleNoActiveMilestone(
  ...
  if (parkedEntries.length > 0) {
    ...
    return {
      ...
      nextAction: `All remaining milestones are parked (${parkedIds}). Run /tac unpark <id> or create a new milestone.`,
      registry, requirements,
      progress: { milestones: milestoneProgress },
    };
  }
  ...
  if (registry.length === 0) {
    return {
      ...
      phase: 'pre-planning',
      recentDecisions: [], blockers: [],
      nextAction: 'No milestones found. Run /tac to create one.',
      registry: [], requirements,
    ...
    return {
      ...
      blockers: [
        `Milestone ${activeMilestone.id} validation verdict is needs-remediation but all slices are complete. ` +
          `Add remediation slices via tac_reassess_roadmap or override the verdict manually.`,
      ],
...
function resolveSliceDependencies(activeMilestoneSlices: SliceRow[]): { activeSlice: ActiveRef | null, activeSliceRow: SliceRow | null } {
  ...
  const sliceLock = process.env.TAC_SLICE_LOCK;
  if (sliceLock) {
    ...
    if (lockedSlice) {
      ...
      logWarning("state", `TAC_SLICE_LOCK=${sliceLock} not found in active slices — worker has no assigned work`);
...
export async function deriveStateFromDb(basePath: string): Promise<TACState> {
  const requirements = parseRequirementCounts(await loadFile(resolveTacRootFile(basePath, "REQUIREMENTS")));
  ...
  if (!activeSliceContext.activeSlice) {
    ...
    if (process.env.TAC_SLICE_LOCK) {
      return {
        activeMilestone, activeSlice: null, activeTask: null,
        phase: 'blocked', recentDecisions: [], blockers: [`TAC_SLICE_LOCK=${process.env.TAC_SLICE_LOCK} not found in active milestone slices`],
  ...
  if (!planFile) {
    ...
    const progressive = loadEffectiveTACPreferences()?.preferences?.phases?.progressive_planning === true;
  ...
  if (escalatingTaskId) {
    return {
      ...
      nextAction: `Run /tac escalate show ${escalatingTaskId} to review, then /tac escalate resolve ${escalatingTaskId} <choice> to proceed.`,
      activeWorkspace: undefined,
      registry, requirements,
      progress: { milestones: milestoneProgress, slices: sliceProgress, tasks: taskProgress },
    };
...
export async function _deriveStateImpl(basePath: string): Promise<TACState> {
  const diskIds = findMilestoneIds(basePath);
  const customOrder = loadQueueOrder(basePath);
  const milestoneIds = sortByQueueOrder(diskIds, customOrder);

  // ── Parallel worker isolation ──────────────────────────────────────────
  // When TAC_MILESTONE_LOCK is set, this process is a parallel worker
  ...
  // When the native Rust parser is available, read every .md file under .tac/
  ...
  const batchFiles = nativeBatchParseTacFiles(tacDir);
  if (batchFiles) {
    for (const f of batchFiles) {
      const absPath = resolve(tacDir, f.path);
  ...
  const requirements = parseRequirementCounts(await cachedLoadFile(resolveTacRootFile(basePath, "REQUIREMENTS")));
  ...
  // When TAC_SLICE_LOCK is set, override activeSlice to only the locked slice.
  const sliceLockLegacy = process.env.TAC_SLICE_LOCK;
  if (sliceLockLegacy) {
    const lockedSlice = activeRoadmap.slices.find(s => s.id === sliceLockLegacy);
    if (lockedSlice) {
      activeSlice = { id: lockedSlice.id, title: lockedSlice.title };
    } else {
      logWarning("state", `TAC_SLICE_LOCK=${sliceLockLegacy} not found in active slices — worker has no assigned work`);
      return {
        activeMilestone,
        activeSlice: null,
        activeTask: null,
        phase: 'blocked',
        recentDecisions: [],
        blockers: [`TAC_SLICE_LOCK=${sliceLockLegacy} not found in active milestone slices`],
        nextAction: 'Slice lock references a non-existent slice — check orchestrator dispatch.',
        registry,
        requirements,
        progress: {
  ...
  // Manual `/tac triage` writes REPLAN-TRIGGER.md when a capture is classified
