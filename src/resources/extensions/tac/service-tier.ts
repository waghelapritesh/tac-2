 ...
 * the /tac fast command handler.
 *
 * Service tiers (priority/flex) are an OpenAI feature that currently only
 * applies to gpt-5.4 variants in TAC. This module centralizes the model-gating logic
 ...
 *
 * See: https://github.com/waghelapritesh/tac-2/issues/2546
 */
...
export function formatServiceTierStatus(tier: ServiceTierSetting): string {
  if (!tier) {
    return [
      ...
      "",
      "Usage:",
      "  /tac fast on     Set to priority (2x cost, faster)",
      "  /tac fast flex   Set to flex (0.5x cost, slower)",
      "  /tac fast off    Disable service tier",
      "",
...
/**
 * Handle `/tac fast [on|off|flex|status]`.
 */
export async function handleFast(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (trimmed === "on") {
    ...
    ctx.ui.setStatus("tac-fast", formatServiceTierFooterStatus("priority", ctx.model?.id));
  ...
  if (trimmed === "off") {
    ...
    ctx.ui.setStatus("tac-fast", undefined);
  ...
  if (trimmed === "flex") {
    ...
    ctx.ui.setStatus("tac-fast", formatServiceTierFooterStatus("flex", ctx.model?.id));
