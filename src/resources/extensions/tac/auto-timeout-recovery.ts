...
export async function recoverTimedOutUnit(
  ...
  if (unitType === "execute-task") {
    ...
    if (recoveryAttempts < maxRecoveryAttempts) {
      ...
      pi.sendMessage(
        {
          customType: "tac-auto-timeout-recovery",
          display: verbose,
  ...
  if (unitType === "complete-milestone") {
    ...
    ctx.ui.notify(
      `Milestone ${unitId} ${reason}-recovery exhausted ${maxRecoveryAttempts} attempt(s) — worktree branch preserved. Re-run /tac auto once blockers are resolved.`,
      "error",
    );
    return "paused";
  }
