...
/**
 * Check for orphaned project state directories in ~/.tac/projects/.
 *
...
export async function checkGlobalHealth(
  ...
  try {
    ...
    issues.push({
      ...
      scope: "project",
      unitId: "global",
      message: `${orphaned.length} orphaned TAC project state director${orphaned.length === 1 ? "y" : "ies"} in ${projectsDir} whose git root no longer exists: ${labels}${overflow}${unknownNote}. Run /tac cleanup projects to audit or /tac cleanup projects --fix to reclaim disk space.`,
      file: projectsDir,
      fixable: true,
    });
