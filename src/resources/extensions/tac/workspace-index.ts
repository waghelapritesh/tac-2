...
export type TACWorkspaceIndex = WorkspaceIndex;
...
export async function indexWorkspace(basePath: string, opts: IndexWorkspaceOptions = {}): Promise<TACWorkspaceIndex> {
...
export async function getSuggestedNextCommands(basePath: string): Promise<string[]> {
  ...
  if (index.active.phase === "planning") commands.add("/tac");
  if (index.active.phase === "executing" || index.active.phase === "summarizing") commands.add("/tac auto");
  if (scope) commands.add(`/tac doctor ${scope}`);
  if (scope) commands.add(`/tac doctor fix ${scope}`);
  commands.add("/tac status");
  return [...commands];
}
