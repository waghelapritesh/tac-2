...
import {
  ...
  resolveTacRootFile, relTacRootFile, resolveRuntimeFile,
} from "./paths.js";
import { resolveSkillDiscoveryMode, resolveInlineLevel, loadEffectiveTACPreferences, resolveAllSkillReferences } from "./preferences.js";
import { parseRoadmap } from "./parsers-legacy.js";
import type { TACState, InlineLevel } from "./types.js";
import type { TACPreferences } from "./preferences.js";
import { getLoadedSkills, type Skill } from "@tac/pi-coding-agent";
...
import { getPendingGates, getPendingGatesForTurn } from "./tac-db.js";
import {
  GATE_REGISTRY,
  assertGateCoverage,
  getGatesForTurn,
 ...
 * instruction when no TAC artifacts are found.
 *
...
export function buildSourceFilePaths(
  ...
  const projectPath = resolveTacRootFile(base, "PROJECT");
  if (existsSync(projectPath)) {
    paths.push(`- **Project**: \`${relTacRootFile("PROJECT")}\``);
  }

  const requirementsPath = resolveTacRootFile(base, "REQUIREMENTS");
  if (existsSync(requirementsPath)) {
    paths.push(`- **Requirements**: \`${relTacRootFile("REQUIREMENTS")}\``);
  }

  const decisionsPath = resolveTacRootFile(base, "DECISIONS");
  if (existsSync(decisionsPath)) {
    paths.push(`- **Decisions**: \`${relTacRootFile("DECISIONS")}\``);
  }

  const queuePath = resolveTacRootFile(base, "QUEUE");
  if (existsSync(queuePath)) {
    paths.push(`- **Queue**: \`${relTacRootFile("QUEUE")}\``);
  }
...
export async function inlineDependencySummaries(
  ...
  try {
    const { isDbAvailable, getSlice } = await import("./tac-db.js");
    if (isDbAvailable()) {
...
/**
 * Load a well-known .tac/ root file for optional inlining.
 * Handles the existsSync check internally.
 */
export async function inlineTacRootFile(
  ...
  const absPath = resolveTacRootFile(base, key);
  if (!existsSync(absPath)) return null;
  return inlineFileOptional(absPath, relTacRootFile(key), label);
}
 ...
 * Falls back to filesystem via inlineTacRootFile only when DB is unavailable.
 *
 * Cascade logic (R005):
...
export async function inlineDecisionsFromDb(
  ...
  try {
    const { isDbAvailable } = await import("./tac-db.js");
    if (isDbAvailable()) {
  ...
  return inlineTacRootFile(base, "decisions.md", "Decisions");
}

/**
 * Inline requirements with optional milestone and slice scoping from the DB.
 * Falls back to filesystem via inlineTacRootFile when DB unavailable or empty.
 */
...
export async function inlineKnowledgeScoped(
  ...
  const knowledgePath = resolveTacRootFile(base, "KNOWLEDGE");
  ...
  return `### Project Knowledge (scoped)\nSource: \`${relTacRootFile("KNOWLEDGE")}\`\n\n${scoped.trim()}`;
}
...
function resolvePreferenceSkillNames(refs: string[], base: string): string[] {
  ...
  const prefs: TACPreferences = { always_use_skills: refs };
...
function resolveSkillRuleMatches(
  prefs: TACPreferences | undefined,
...
export function buildSkillActivationBlock(params: {
  ...
  const prefs = params.preferences ?? loadEffectiveTACPreferences(params.base)?.preferences;
...
export async function checkNeedsRunUat(
  base: string, mid: string, state: TACState, prefs: TACPreferences | undefined,
  ...
  try {
    ...
    if (isDbAvailable()) {
      ...
      if (slices.length > 0) {
        ...
        // there (via tac_summary_save artifact_type:"ASSESSMENT"), not into the
...
export async function buildDiscussMilestonePrompt(
  ...
  const basePrompt = loadPrompt("guided-discuss-milestone", {
    ...
    commitInstruction: "Do not commit planning artifacts — .tac/ is managed externally.",
    fastPathInstruction: "",
  });
...
async function renderSlicePrompt(options: {
  ...
  const commitInstruction = "Do not commit — .tac/ planning docs are managed externally and not tracked in git.";
...
export async function buildExecuteTaskPrompt(
  ...
  const knowledgeAbsPath = resolveTacRootFile(base, "KNOWLEDGE");
  const knowledgeInlineET = existsSync(knowledgeAbsPath)
    ? await inlineFileSmart(
        knowledgeAbsPath,
        relTacRootFile("KNOWLEDGE"),
        "Project Knowledge",
...
export async function buildCompleteSlicePrompt(
  ...
  // after the assistant calls tac_complete_slice.
...
export async function buildCompleteMilestonePrompt(
  ...
  // Inline root TAC files (skip for minimal — completion can read these if needed)
  if (inlineLevel !== "minimal") {
    const requirementsInline = await inlineRequirementsFromDb(base, mid, undefined, inlineLevel);
    if (requirementsInline) inlined.push(requirementsInline);
    const decisionsInline = await inlineDecisionsFromDb(base, mid, undefined, inlineLevel);
  ...
  // Inline milestone context file (milestone-level, not TAC root)
  const contextPath = resolveMilestoneFile(base, mid, "CONTEXT");
  const contextRel = relMilestoneFile(base, mid, "CONTEXT");
  const contextInline = await inlineFileOptional(contextPath, contextRel, "Milestone Context");
  if (contextInline) inlined.push(contextInline);
...
export async function buildValidateMilestonePrompt(
  ...
  try {
    const { isDbAvailable, getMilestone } = await import("./tac-db.js");
    if (isDbAvailable()) {
...
export async function buildReassessRoadmapPrompt(
  ...
  const reassessCommitInstruction = "Do not commit — .tac/ planning docs are managed externally and not tracked in git.";
...
export async function buildParallelResearchSlicesPrompt(
  ...
  for (const slice of slices) {
    ...
    subagentSections.push([
      ...
      "",
      `Use this as the prompt for a \`subagent\` call${modelSuffix} (agent: \`tac-executor\` or the default agent):`,
      "",
      "```",
      slicePrompt,
      "```",
...
export async function buildGateEvaluatePrompt(
  ...
  for (const def of gateDefs) {
    ...
    const subPrompt = [
      ...
      `Call the \`tac_save_gate_result\` tool with:`,
      `- \`milestoneId\`: "${mid}"`,
      `- \`sliceId\`: "${sid}"`,
      `- \`gateId\`: "${def.id}"`,
...
export async function buildRewriteDocsPrompt(
  ...
  if (sid) {
    ...
    if (slicePlanPath) {
      ...
      if (tDir) {
        ...
        try {
          const { isDbAvailable, getSliceTasks } = await import("./tac-db.js");
          if (isDbAvailable()) {
  ...
  const decisionsPath = resolveTacRootFile(base, "DECISIONS");
  if (existsSync(decisionsPath)) docList.push(`- Decisions: \`${relTacRootFile("DECISIONS")}\``);
  const requirementsPath = resolveTacRootFile(base, "REQUIREMENTS");
  if (existsSync(requirementsPath)) docList.push(`- Requirements: \`${relTacRootFile("REQUIREMENTS")}\``);
  const projectPath = resolveTacRootFile(base, "PROJECT");
  if (existsSync(projectPath)) docList.push(`- Project: \`${relTacRootFile("PROJECT")}\``);
  const contextPath = resolveMilestoneFile(base, mid, "CONTEXT");
  const contextRel = relMilestoneFile(base, mid, "CONTEXT");
  ...
  return loadPrompt("rewrite-docs", {
    ...
    overrideContent,
    documentList,
    overridesPath: relTacRootFile("OVERRIDES"),
  });
}
