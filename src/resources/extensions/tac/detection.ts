/**
 * TAC Detection — Project state and ecosystem detection.
 *
...
export interface ProjectDetection {
  /** What kind of TAC state exists in this directory */
  state: "none" | "v1-planning" | "v2-tac" | "v2-tac-empty";

  /** Is this the first time TAC has been used on this machine? */
  isFirstEverLaunch: boolean;

  /** Does ~/.tac/ exist with preferences? */
  hasGlobalSetup: boolean;

  /** v1 details (only when state === 'v1-planning') */
  v1?: V1Detection;

  /** v2 details (only when state === 'v2-tac' or 'v2-tac-empty') */
  v2?: V2Detection;
...
/**
 * Detect a v1 .planning/ directory with TAC v1 markers.
...
function detectV2Tac(basePath: string): V2Detection | null {
  const tacPath = tacRoot(basePath);

  if (!existsSync(tacPath)) return null;

  const hasPreferences =
    existsSync(join(tacPath, "PREFERENCES.md")) ||
    existsSync(join(tacPath, "preferences.md"));

  const hasContext = existsSync(join(tacPath, "CONTEXT.md"));

  let milestoneCount = 0;
  const milestonesPath = join(tacPath, "milestones");
...
/**
 * Check if global TAC setup exists (has ~/.tac/ with preferences).
 */
export function hasGlobalSetup(): boolean {
  return (
    existsSync(join(tacHome, "PREFERENCES.md")) ||
    existsSync(join(tacHome, "preferences.md"))
  );
}

/**
 * Check if this is the very first time TAC has been used on this machine.
 * Returns true if ~/.tac/ doesn't exist or has no preferences or auth.
 */
export function isFirstEverLaunch(): boolean {
  if (!existsSync(tacHome)) return true;
  ...
  if (existsSync(join(tacHome, "agent", "auth.json"))) return false;

  // Check legacy path too
  const legacyPath = join(homedir(), ".pi", "agent", "tac-preferences.md");
...
/**
 * Check whether a project's `.tac/` directory contains the bootstrap artifacts
 * (`PREFERENCES.md` or `milestones/`) that indicate a completed init run.
 *
 * A zombie `.tac/` state — symlink exists but neither artifact is present —
 * must be treated as "needs init wizard". The previous guard checked only
 * `existsSync(tacRoot(basePath))`, which accepted zombie states and skipped
 * the wizard (#2942).
 *
 * `existsFn` is injectable so tests can run deterministically; defaults to
 * `fs.existsSync`.
 */
export function hasTacBootstrapArtifacts(
  tacPath: string,
  existsFn: (p: string) => boolean = existsSync,
): boolean {
  return (
    existsFn(tacPath) &&
    (existsFn(join(tacPath, "PREFERENCES.md")) ||
      existsFn(join(tacPath, "preferences.md")) ||
      existsFn(join(tacPath, "milestones")))
  );
}
