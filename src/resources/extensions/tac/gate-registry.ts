/**
 * TAC Gate Registry — single source of truth for quality-gate ownership.
 *
 ...
 *   - `assertGateCoverage(pending, turn)` throws a TACError if the pending
...
export function getOwnerTurn(id: GateId): OwnerTurn {
  ...
  if (!def) {
    throw new TACError(TAC_PARSE_ERROR, `gate-registry: unknown gate id "${id}"`);
  }
