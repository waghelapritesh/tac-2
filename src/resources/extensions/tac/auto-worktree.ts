/**
 * TAC Auto-Worktree -- lifecycle management for auto-mode worktrees.
 *
...
import { TACError, TAC_IO_ERROR, TAC_GIT_ERROR } from "./errors.js";
import {
  reconcileWorktreeDb,
  isDbAvailable,
  getMilestone,
...
/**
 * Root-level .tac/ state files synced between worktree and project root.
 * Single source of truth — used by syncTacStateToWorktree, syncWorktreeStateBack,
...
const ROOT_STATE_FILES = [
  ...
  // Forward-sync (main → worktree) is handled explicitly in syncTacStateToWorktree().
...
function clearProjectRootStateFiles(basePath: string, milestoneId: string): void {
  const tacDir = tacRoot(basePath);
  const transientFiles = [
    join(tacDir, "STATE.md"),
    join(tacDir, "auto.lock"),
    join(tacDir, "milestones", milestoneId, `${milestoneId}-META.json`),
  ];
  ...
  const syncedDirs = [
    join(tacDir, "milestones", milestoneId),
    join(tacDir, "runtime", "units"),
  ];
 ...
 * Covers `.tac/` state files and common build artifacts. */
export const isSafeToAutoResolve = (filePath: string): boolean =>
  filePath.startsWith(".tac/") ||
 ...
 * tac.db in the worktree so it rebuilds from fresh disk state (#853).
...
export function syncProjectRootToWorktree(
  ...
  const prTac = join(projectRoot, ".tac");
  const wtTac = join(worktreePath_, ".tac");

  // When .tac is a symlink to the same external directory in both locations,
  ...
  if (isSamePath(prTac, wtTac)) return;
  ...
  safeCopyRecursive(
    join(prTac, "milestones", milestoneId),
    join(wtTac, "milestones", milestoneId),
    { force: false },
  );
  ...
  safeCopy(
    join(prTac, "completed-units.json"),
    join(wtTac, "completed-units.json"),
    { force: true },
  );

  // Delete worktree tac.db ONLY if it is empty (0 bytes).
  // An empty DB is stale/corrupt and should be rebuilt (#853).
  // A non-empty DB was populated by tac-migrate on respawn and must be
  // preserved — deleting it truncates the file to 0 bytes when
  // openDatabase re-creates it, causing "no such table" failures (#2815).
  try {
    const wtDb = join(wtTac, "tac.db");
...
/**
 * Sync dispatch-critical .tac/ state files from worktree to project root.
...
export function syncStateToProjectRoot(
  ...
  // cpSync rejects the copy because source === destination (ERR_FS_CP_EINVAL).
  // Compare realpaths and skip when they resolve to the same physical path (#2184).
  if (isSamePath(wtTac, prTac)) return;

  // 1. STATE.md — the quick-glance status used by initial deriveState()
  safeCopy(join(wtTac, "STATE.md"), join(prTac, "STATE.md"), { force: true });

  // 2. Milestone directory — ROADMAP, slice PLANs, task summaries
  // Copy the entire milestone .tac subtree so deriveState reads current checkboxes
  safeCopyRecursive(
    join(wtTac, "milestones", milestoneId),
    join(prTac, "milestones", milestoneId),
    { force: true },
  ...
  safeCopy(join(wtTac, "metrics.json"), join(prTac, "metrics.json"), { force: true });
  ...
  safeCopyRecursive(
    join(wtTac, "runtime", "units"),
    join(prTac, "runtime", "units"),
    { force: true },
  );
}
 ...
 * Uses tacVersion instead of syncedAt so that launching a second session
 * doesn't falsely trigger staleness (#804).
 */
export function readResourceVersion(): string | null {
  const agentDir =
    process.env.TAC_CODING_AGENT_DIR || join(tacHome, "agent");
  const manifestPath = join(agentDir, "managed-resources.json");
  try {
    const manifest = JSON.parse(readFileSync(manifestPath, "utf-8"));
    return typeof manifest?.tacVersion === "string"
      ? manifest.tacVersion
      : null;
  } catch (e) {
...
export function checkResourcesStale(
  ...
  if (current !== versionOnStart) {
    return "TAC resources were updated since this session started. Restart tac to load the new code.";
  }
  return null;
}
 ...
 * the process cwd may still point inside `.tac/worktrees/<MID>/`.
...
export function escapeStaleWorktree(base: string): string {
  // Direct layout: /.tac/worktrees/
  const directMarker = `${pathSep}.tac${pathSep}worktrees${pathSep}`;
  let idx = base.indexOf(directMarker);
  if (idx === -1) {
    // Symlink-resolved layout: /.tac/projects/<hash>/worktrees/
    const symlinkRe = new RegExp(
      `\\${pathSep}\\.tac\\${pathSep}projects\\${pathSep}[a-f0-9]+\\${pathSep}worktrees\\${pathSep}`,
    );
  ...
  // base is inside .tac/worktrees/<something> — extract the project root
  const projectRoot = base.slice(0, idx);

  // Guard: If the candidate project root's .tac IS the user-level ~/.tac,
  // the string-slice heuristic matched the wrong /.tac/ boundary. This happens
  // when .tac is a symlink into ~/.tac/projects/<hash> and process.cwd()
  // resolved through the symlink. Returning ~ would be catastrophic (#1676).
  const candidateTac = join(projectRoot, ".tac").replaceAll("\\", "/");
  const tacHomePath = tacHome.replaceAll("\\", "/");
  if (candidateTac === tacHomePath || candidateTac.startsWith(tacHomePath + "/")) {
...
export function cleanStaleRuntimeUnits(
  ...
  const runtimeUnitsDir = join(tacRootPath, "runtime", "units");
...
/**
 * Sync .tac/ state from the main repo into the worktree.
 *
 * When .tac/ is a symlink to the external state directory, both the main
 * repo and worktree share the same directory — no sync needed.
 *
 * When .tac/ is a real directory (e.g., git-tracked or manage_gitignore:false),
 ...
 * PROJECT files from the main repo's .tac/ into the worktree's .tac/.
 *
 ...
 */
export function syncTacStateToWorktree(
  mainBasePath: string,
  worktreePath_: string,
): { synced: string[] } {
  const mainTac = tacRoot(mainBasePath);
  const wtTac = tacRoot(worktreePath_);
  const synced: string[] = [];

  // If both resolve to the same directory (symlink), no sync needed
  if (isSamePath(mainTac, wtTac)) return { synced };

  if (!existsSync(mainTac) || !existsSync(wtTac)) return { synced };

  // Sync root-level .tac/ files (DECISIONS, REQUIREMENTS, PROJECT, KNOWLEDGE, etc.)
  for (const f of ROOT_STATE_FILES) {
    const src = join(mainTac, f);
    const dst = join(wtTac, f);
  ...
  {
    const worktreeHasPreferences = existsSync(join(wtTac, PROJECT_PREFERENCES_FILE))
      || existsSync(join(wtTac, LEGACY_PROJECT_PREFERENCES_FILE));
    if (!worktreeHasPreferences) {
      for (const file of [PROJECT_PREFERENCES_FILE, LEGACY_PROJECT_PREFERENCES_FILE] as const) {
        const src = join(mainTac, file);
        const dst = join(wtTac, file);
        if (existsSync(src)) {
          try {
            cpSync(src, dst);
            synced.push(file);
  ...
  const mainMilestonesDir = join(mainTac, "milestones");
  const wtMilestonesDir = join(wtTac, "milestones");
 ...
 *
 * Syncs:
 *   1. Root-level .tac/ files (REQUIREMENTS, PROJECT, DECISIONS, KNOWLEDGE,
 ...
 * .tac/ files are often untracked (gitignored or never committed), so the
...
export function syncWorktreeStateBack(
  ...
  const mainTac = tacRoot(mainBasePath);
  const wtTac = tacRoot(worktreePath);
  const synced: string[] = [];

  // If both resolve to the same directory (symlink), no sync needed
  if (isSamePath(mainTac, wtTac)) return { synced };

  if (!existsSync(wtTac) || !existsSync(mainTac)) return { synced };

  // ── 0. Pre-upgrade worktree DB reconciliation ────────────────────────
  // If the worktree has its own tac.db (copied before the WAL transition),
  ...
  const wtLocalDb = join(wtTac, "tac.db");
  const mainDb = join(mainTac, "tac.db");
  if (existsSync(wtLocalDb) && existsSync(mainDb)) {
    try {
      reconcileWorktreeDb(mainDb, wtLocalDb);
      synced.push("tac.db (pre-upgrade reconcile)");
    } catch (err) {
  ...
  // ── 1. Sync root-level .tac/ files back ──────────────────────────────
  ...
  // (#1787, #2313).
  for (const f of ROOT_STATE_FILES) {
    const src = join(wtTac, f);
    const dst = join(mainTac, f);
    if (existsSync(src)) {
      try {
  ...
  try {
    ...
    for (const mid of wtMilestones) {
      ...
      syncMilestoneDir(wtTac, mainTac, mid, synced);
    }
  } catch (err) {
    /* non-fatal */
...
function syncMilestoneDir(
  ...
  const wtMilestoneDir = join(wtTac, "milestones", mid);
  const mainMilestoneDir = join(mainTac, "milestones", mid);
...
export function runWorktreePostCreateHook(
  ...
  if (hookPath === undefined) {
    const prefs = loadEffectiveTACPreferences()?.preferences?.git;
...
export function enterBranchModeForMilestone(
  ...
  if (!branchExists) {
    ...
    const gitPrefs = loadEffectiveTACPreferences()?.preferences?.git;
...
function reconcilePlanCheckboxes(
  ...
  const srcMilestone = join(projectRoot, ".tac", "milestones", milestoneId);
  const dstMilestone = join(wtPath, ".tac", "milestones", milestoneId);
...
export function createAutoWorktree(
  ...
  // Copy .tac/ planning artifacts from the source repo into the new worktree.
  ...
  // blanket .tac/ rule (pre-v2.14.0). Without this copy, auto-mode loops
...
/**
 * Copy .tac/ planning artifacts from source repo to a new worktree.
...
function copyPlanningArtifacts(srcBase: string, wtPath: string): void {
  const srcTac = join(srcBase, ".tac");
  const dstTac = join(wtPath, ".tac");
  if (!existsSync(srcTac)) return;
  if (isSamePath(srcTac, dstTac)) return;

  // Copy milestones/ directory (planning files, roadmaps, plans, research)
  safeCopyRecursive(join(srcTac, "milestones"), join(dstTac, "milestones"), {
    force: true,
  ...
  for (const file of [
    ...
    safeCopy(join(srcTac, file), join(dstTac, file), { force: true });
  }

  // Seed canonical PREFERENCES.md when available; fall back to legacy lowercase.
  if (existsSync(join(srcTac, PROJECT_PREFERENCES_FILE))) {
    safeCopy(
      join(srcTac, PROJECT_PREFERENCES_FILE),
      join(dstTac, PROJECT_PREFERENCES_FILE),
      { force: true },
    );
  } else if (existsSync(join(srcTac, LEGACY_PROJECT_PREFERENCES_FILE))) {
    safeCopy(
      join(srcTac, LEGACY_PROJECT_PREFERENCES_FILE),
      join(dstTac, LEGACY_PROJECT_PREFERENCES_FILE),
      { force: true },
    );
  }

  // Shared WAL (R012): worktrees use the project root's DB directly.
  // No longer copy tac.db into the worktree — the DB path resolver in
  ...
  // worktrees that already have a local tac.db from before this change.
}
...
export function teardownAutoWorktree(
  ...
  if (existsSync(wtDir)) {
    ...
    // path is safely inside .tac/worktrees/ to prevent #2365 data loss.
    if (isInsideWorktreesDir(originalBasePath, wtDir)) {
      ...
      console.error(
        `[TAC] REFUSING fallback rmSync — path is outside .tac/worktrees/: ${wtDir}`,
      );
    }
  }
}
...
export function isInAutoWorktree(basePath: string): boolean {
  ...
  const wtDir = join(resolvedBase, ".tac", "worktrees");
...
export function mergeMilestoneToMain(
  ...
  if (isDbAvailable()) {
    try {
      const worktreeDbPath = join(worktreeCwd, ".tac", "tac.db");
      const mainDbPath = join(originalBasePath_, ".tac", "tac.db");
  ...
  if (worktreeCwd !== originalBasePath_) {
    try {
      ...
      // Re-throw TACError (divergence); swallow rev-parse failures
      // (e.g. worktree dir already removed by external cleanup)
      if (err instanceof TACError) throw err;
      debugLog("mergeMilestoneToMain", {
  ...
  const milestonesDir = join(tacRoot(originalBasePath_), "milestones");
  const shelterDir = join(tacRoot(originalBasePath_), ".milestone-shelter");
  ...
  //     entries (e.g. a gitignored `.tac` symlink under ADR-002) (#4573).
  //     Queued CONTEXT files under `.tac/milestones/*` are already sheltered
  //     in step 7 above, so they won't be swept into the stash.
  // On Windows, SQLite holds mandatory file locks on the tac.db WAL/SHM
  ...
  try {
    ...
    if (status) {
      execFileSync(
        "git",
        ["stash", "push", "--include-untracked", "-m", `tac: pre-merge stash for ${milestoneId}`],
  ...
  // 8. Squash merge — auto-resolve .tac/ state file conflicts (#530)
  const mergeResult = nativeMergeSquash(originalBasePath_, milestoneBranch);

  if (!mergeResult.success) {
    // Dirty working tree — the merge was rejected before it started (e.g.
    // untracked .tac/ files left by syncStateToProjectRoot).  Preserve the
    ...
    if (conflictedFiles.length > 0) {
      // Separate auto-resolvable conflicts (TAC state files + build artifacts)
      // from real code conflicts. TAC state files diverge between branches
  ...
  if (stashed) {
    try {
      ...
      // Stash pop after squash merge can conflict on .tac/ state files that
      ...
      // .tac/ conflicts during the merge itself: accept HEAD (the just-committed
      // version) and drop the now-applied stash.
      const uu = nativeConflictFiles(originalBasePath_);
      const tacUU = uu.filter((f) => f.startsWith(".tac/"));
      const nonTacUU = uu.filter((f) => !f.startsWith(".tac/"));

      if (tacUU.length > 0) {
        for (const f of tacUU) {
          try {
      ...
      if (nonTacUU.length === 0) {
        // All conflicts were .tac/ files — safe to drop the stash
        try {
        ...
        // Non-.tac conflicts remain — leave stash for manual resolution
        logWarning("reconcile", "Stash pop conflict on non-.tac files after merge", {
          files: nonTacUU.join(", "),
        });
      }
    }
  ...
  // Compare only non-.tac/ paths — .tac/ state files diverge normally and
  ...
  if (nothingToCommit) {
    ...
    const codeChanges = numstat.filter(
      (entry) => !entry.path.startsWith(".tac/"),
    );
  ...
  // 9c. Detect whether any non-.tac/ code files were actually merged (#1906).
  // When a milestone only produced .tac/ metadata (summaries, roadmaps) but no
  ...
  if (!nothingToCommit) {
    try {
      ...
      codeFilesChanged = mergedFiles.some((f) => !f.startsWith(".tac/"));
    } catch (e) {
      ...
      try {
        ...
        codeFilesChanged = fallbackFiles.some((f) => !f.startsWith(".tac/"));
      } catch {
  ...
  if (prefs.auto_pr === true && !nothingToCommit) {
    ...
    try {
      ...
      execFileSync("gh", [
        ...
        "--body", "Auto-created by TAC on milestone completion.",
      ], {
        cwd: originalBasePath_,
        stdio: ["ignore", "pipe", "pipe"],
        encoding: "utf-8",
