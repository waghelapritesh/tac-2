/**
 * TAC Session Lock — OS-level exclusive locking for auto-mode sessions.
 *
 * Prevents multiple TAC processes from running auto-mode concurrently on
 ...
 *
 * The lock file (.tac/auto.lock) contains JSON metadata (PID, start time,
...
/** Registry of all tacDir paths where locks were created during this session.
 *  The exit handler cleans ALL of these, not just the current tacRoot(). (#1578) */
const _lockDirRegistry: Set<string> = new Set();

/** Snapshotted lock file path — captured at acquireSessionLock time to avoid
 *  tacRoot() resolving differently in worktree vs project root contexts (#1363). */
 ...
 * In parallel worker mode (TAC_PARALLEL_WORKER + TAC_MILESTONE_LOCK),
 * each worker uses a per-milestone lock file (`auto-<milestoneId>.lock`)
 * to avoid contending on the shared `.tac/auto.lock` (#2184).
 */
export function effectiveLockFile(): string {
  const mid = process.env.TAC_PARALLEL_WORKER ? process.env.TAC_MILESTONE_LOCK : null;
  return mid ? `auto-${mid}.lock` : LOCK_FILE;
}

/**
 * Derive the OS-level lock target directory for the current process.
 * In parallel worker mode, uses `.tac/parallel/<milestoneId>/` instead of
 * `.tac/` so workers don't contend on the same proper-lockfile directory (#2184).
 */
export function effectiveLockTarget(tacDir: string): string {
  const mid = process.env.TAC_PARALLEL_WORKER ? process.env.TAC_MILESTONE_LOCK : null;
  return mid ? join(tacDir, "parallel", mid) : tacDir;
}
...
function lockPath(basePath: string): string {
  ...
  return join(tacRoot(basePath), effectiveLockFile());
}
 ...
 *
 * Also removes stray proper-lockfile directories beyond the canonical `.tac.lock/`.
 */
export function cleanupStrayLockFiles(basePath: string): void {
  const tacDir = tacRoot(basePath);

  // Clean numbered auto lock files inside .tac/
  try {
    if (existsSync(tacDir)) {
      for (const entry of readdirSync(tacDir)) {
        // Match "auto <N>.lock" or "auto (<N>).lock" variants but NOT the canonical "auto.lock"
        if (entry !== LOCK_FILE && /^auto\s.+\.lock$/i.test(entry)) {
          try { unlinkSync(join(tacDir, entry)); } catch { /* best-effort */ }
        }
      }
    }
  } catch { /* non-fatal: directory read failure */ }

  // Clean stray proper-lockfile directories (e.g. ".tac 2.lock/")
  // The canonical one is ".tac.lock/" — anything else is stray.
  try {
    const parentDir = dirname(tacDir);
    const tacDirName = tacDir.split("/").pop() || ".tac";
    if (existsSync(parentDir)) {
      for (const entry of readdirSync(parentDir)) {
        // Match ".tac <N>.lock" or ".tac (<N>).lock" directories but NOT ".tac.lock"
        if (entry !== `${tacDirName}.lock` && entry.startsWith(tacDirName) && entry.endsWith(".lock")) {
 ...
 */
function ensureExitHandler(_tacDir: string): void {
  // Register the tacDir so exit cleanup covers it
  _lockDirRegistry.add(_tacDir);
  ...
  process.once("exit", () => {
    ...
    // Lock files accumulate across main project .tac/, worktree .tac/,
  ...
  return () => {
    ...
    if (elapsed < 1_800_000) {
      process.stderr.write(
        `[tac] Lock heartbeat caught up after ${Math.round(elapsed / 1000)}s — long LLM call, no action needed.\n`,
      );
      return;
    }
    ...
    if (existing && existing.pid === process.pid) {
      process.stderr.write(
        `[tac] Lock heartbeat mismatch after ${Math.round(elapsed / 1000)}s — lock file still owned by PID ${process.pid}, treating as false positive.\n`,
      );
      return;
    }
...
export function acquireSessionLock(basePath: string): SessionLockResult {
  ...
  const tacDir = tacRoot(basePath);
  const lockTarget = effectiveLockTarget(tacDir);
  ...
  // window from blocking /tac after crashes, SIGKILL, or laptop sleep.
  ...
  try {
    ...
    // In parallel worker mode, lockTarget is .tac/parallel/<MID>/ (#2184).
    ...
    // Lock is held by another process — or the .tac.lock/ directory is stranded.
  ...
  if (_lockCompromised) {
    ...
    if (existing && existing.pid === process.pid) {
      ...
      try {
        ...
        if (result.acquired) {
          process.stderr.write(
            `[tac] Lock recovered after onCompromised — lock file PID matched, re-acquired.\n`,
          );
...
export function releaseSessionLock(basePath: string): void {
  ...
  // In parallel worker mode, this is .tac/parallel/<MID>.lock/ (#2184).
  const tacDir = tacRoot(basePath);
  const lockTarget = effectiveLockTarget(tacDir);
  try {
    const lockDir = join(lockTarget + ".lock");
  ...
  // main project .tac/, worktree .tac/, and projects registry paths.
  for (const dir of _lockDirRegistry) {
    try {
      const lockFile = join(dir, LOCK_FILE);
      if (existsSync(lockFile)) unlinkSync(lockFile);
