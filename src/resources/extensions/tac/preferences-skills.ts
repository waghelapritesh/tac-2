...
export type { TACSkillRule, SkillDiscoveryMode, SkillResolution, SkillResolutionReport } from "./preferences-types.js";
 ...
 * Legacy ~/.tac/agent/skills/ is included as a fallback for pre-migration installs.
 */
export function getSkillSearchDirs(cwd: string): Array<{ dir: string; method: SkillResolution["method"] }> {
  ...
  // Legacy fallback — read skills from old TAC directory only if migration hasn't completed
  const legacyDir = join(homedir(), ".tac", "agent", "skills");
 ...
 */
export function resolveAllSkillReferences(preferences: TACPreferences, cwd: string): SkillResolutionReport {
...
// preferences.ts to break circular dependency (they need loadEffectiveTACPreferences).
