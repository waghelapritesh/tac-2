/**
 * TAC Workflow Template Commands — /tac start, /tac templates
 *
 * Handles the `/tac start [template] [description]` and `/tac templates` commands.
...
function findInProgressWorkflows(basePath: string): WorkflowState[] {
  const workflowsRoot = join(tacRoot(basePath), "workflows");
...
export async function handleStart(
  ...
  // /tac start --list → same as /tac templates
  ...
  if (isAutoActive()) {
    ctx.ui.notify(
      ...
      "Run /tac pause first, then /tac start.",
      "warning",
    );
    return;
  }
  ...
  if (isAutoPaused()) {
    ctx.ui.notify(
      ...
      "The paused auto-mode session can be resumed later with /tac auto.",
      "info",
    );
  }

  // ─── Resume detection ───────────────────────────────────────────────────
  // /tac start --resume or /tac start resume → resume in-progress workflow
  if (trimmed === "--resume" || trimmed === "resume") {
    ...
    pi.sendMessage(
      { customType: "tac-workflow-template", content: prompt, display: false },
      { triggerTurn: true },
    );
    return;
  }

  // Show in-progress workflows when /tac start is called with no args
  if (!trimmed) {
    const basePath = process.cwd();
    const inProgress = findInProgressWorkflows(basePath);
    if (inProgress.length > 0) {
      ...
      ctx.ui.notify(
        ...
        `Run /tac start resume to continue it.\n`,
        "info",
      );
    }
  }

  // /tac start --dry-run <template> → preview without executing
  ...
  if (!match && cleanedArgs) {
    ...
    if (detected.length === 1 || (detected.length > 0 && detected[0].confidence === "high")) {
      ...
      const choices = detected.slice(0, 4).map(
        (m) => `  /tac start ${m.id} ${cleanedArgs}`
      );
      ctx.ui.notify(
        `Multiple templates could match. Pick one:\n\n${choices.join("\n")}\n\nOr specify explicitly: /tac start <template> <description>`,
        "info",
      );
      return;
    }
  ...
  if (!match) {
    if (!trimmed) {
      ctx.ui.notify(
        "Usage: /tac start <template> [description]\n\n" +
        "Templates:\n" +
        ...
        "  full-project    Complete TAC with full ceremony\n\n" +
        "Examples:\n" +
        "  /tac start bugfix fix login button not responding\n" +
        "  /tac start spike evaluate auth libraries\n" +
        "  /tac start hotfix critical: API returns 500\n\n" +
        "Flags:\n" +
        "  --dry-run       Preview what would happen without executing\n" +
        "  --issue <ref>   Link to a GitHub issue\n\n" +
        "Run /tac templates for detailed template info.",
        "info",
      );
    } else {
      ctx.ui.notify(
        `No template matched "${firstWord}". Run /tac start to see available templates.`,
        "warning",
      );
    }
    return;
  ...
  if (dryRun) {
    ...
    lines.push(`Branch:       tac/${templateId}/${slug}`);
  ...
  // ─── Route full-project to standard TAC workflow ────────────────────────

  if (templateId === "full-project") {
    const root = tacRoot(basePath);
    if (!existsSync(root)) {
      ctx.ui.notify(
        "Routing to /tac init for full project setup...",
        "info",
      );
      // Trigger /tac init by dispatching to the handler
      pi.sendMessage(
        {
          customType: "tac-workflow-template",
          content: "The user wants to start a full TAC project. Run `/tac init` to bootstrap the project, then `/tac auto` to begin execution.",
          display: false,
        },
        { triggerTurn: true },
      );
      ...
      ctx.ui.notify(
        "Project already initialized. Use `/tac auto` to continue or `/tac discuss` to start a new milestone.",
        "info",
      );
    }
    return;
  ...
  const slug = slugify(description || templateId);
  const branchName = `tac/${templateId}/${slug}`;
...
export async function handleTemplates(
  ...
  if (trimmed.startsWith("info ")) {
    ...
    if (info) {
      ...
      ctx.ui.notify(
        `Unknown template "${name}". Run /tac templates to see available templates.`,
        "warning",
      );
    }
    return;
  ...
  // /tac templates — list all
  ctx.ui.notify(listTemplates(), "info");
}

/**
 * Return template IDs for autocomplete in /tac templates info <name>.
 */
...
// ─── Shared markdown-phase dispatcher (used by /tac workflow <name>) ────────
