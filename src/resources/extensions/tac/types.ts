// TAC Extension — Core Type Definitions
...
export interface Roadmap {
  title: string; // e.g. "M001: TAC Extension — Hierarchical Planning with Auto Mode"
  vision: string;
...
// ─── TAC Ecosystem Extension API Types ────────────────────────────────────
// Pure data type — no runtime deps. The TACExtensionAPI interface itself
// lives in ecosystem/tac-extension-api.ts (it imports from pi).

export interface TACActiveUnit {
  milestoneId: string;
...
export interface EscalationOption {
  /** Short identifier, e.g. "A", "B". Used as the `choice` value in `/tac escalate resolve <taskId> <id>`. */
  id: string;
...
export interface EscalationArtifact {
   ...
   * user resolves via `/tac escalate resolve`.
   */
  continueWithDefault: boolean;
  createdAt: string;
  /** Populated by `/tac escalate resolve`. */
  respondedAt?: string;
...
// ─── Complete Task Params (tac_complete_task tool input) ─────────────────
...
// ─── Complete Slice Params (tac_complete_slice tool input) ───────────────
