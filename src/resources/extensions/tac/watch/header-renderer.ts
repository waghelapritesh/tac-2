// TAC Watch — Header renderer: ASCII logo, session info, MCP status, remote questions
...
import { visibleWidth, truncateToWidth } from "@tac/pi-tui";
import { loadEffectiveTACPreferences } from "../preferences.js";

// ─── Constants ────────────────────────────────────────────────────────────────

/**
 * TAC ASCII logo — inlined here because the canonical src/logo.ts is outside
 * the resources rootDir and cannot be imported directly.
 */
const TAC_LOGO: readonly string[] = [
  '   ██████╗ ███████╗██████╗ ',
  '  ██╔════╝ ██╔════╝██╔══██╗',
  '  ██║  ███╗███████╗██║  ██║',
  '  ██║   ██║╚════██║██║  ██║',
...
/**
 * Read the configured execution model from TAC preferences.
 ...
 *
 * Layout: TAC ASCII logo on the left, info panel on the right separated by │.
...
export function renderHeaderLines(data: HeaderData, width: number): string[] {
  ...
  const infoLines: string[] = [
    `\x1b[1mThink. Architect. Code.\x1b[0m`,
    "",
...
function renderStackedHeader(data: HeaderData, width: number): string[] {
  ...
  // Title
  lines.push(`\x1b[1mThink. Architect. Code.\x1b[0m`);
  lines.push("");
