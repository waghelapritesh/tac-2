/**
 * TAC Preferences -- loading, merging, and rendering.
 *
...
// These lived in preferences-skills.ts but imported loadEffectiveTACPreferences
// back from this file, creating a circular dependency. Moved here since they
// are trivial wrappers over loadEffectiveTACPreferences.
export function resolveSkillDiscoveryMode(basePath?: string): SkillDiscoveryMode {
  const prefs = loadEffectiveTACPreferences(basePath);
...
function tacHome(): string {
  return process.env.TAC_HOME || join(homedir(), ".tac");
}

function globalPreferencesPath(): string {
  return join(tacHome(), "PREFERENCES.md");
}

function legacyGlobalPreferencesPath(): string {
  return join(homedir(), ".pi", "agent", "tac-preferences.md");
}

function projectPreferencesPath(basePath: string = process.cwd()): string {
  return join(tacRoot(basePath), "PREFERENCES.md");
}
...
export function getGlobalTACPreferencesPath(): string {
  return globalPreferencesPath();
}

export function getLegacyGlobalTACPreferencesPath(): string {
  return legacyGlobalPreferencesPath();
}

export function getProjectTACPreferencesPath(basePath?: string): string {
  return projectPreferencesPath(basePath);
}

// ─── Loading ────────────────────────────────────────────────────────────────

export function loadGlobalTACPreferences(): LoadedTACPreferences | null {
...
export function loadProjectTACPreferences(basePath?: string): LoadedTACPreferences | null {
...
export function loadEffectiveTACPreferences(basePath?: string): LoadedTACPreferences | null {
  const globalPreferences = loadGlobalTACPreferences();
  const projectPreferences = loadProjectTACPreferences(basePath);

  if (!globalPreferences && !projectPreferences) return null;

  let result: LoadedTACPreferences;
  ...
  if (profile) {
    ...
    result = {
      ...result,
      preferences: mergePreferences(profileDefaults as TACPreferences, result.preferences),
    };
  }
...
function loadPreferencesFile(path: string, scope: "global" | "project"): LoadedTACPreferences | null {
...
export function parsePreferencesMarkdown(content: string): TACPreferences | null {
  ...
  // TAC agents may write preferences files without frontmatter delimiters.
  ...
  if (content.trim().length > 0 && !_warnedUnrecognizedFormat) {
    ...
    console.warn(
      "[TAC] Warning: preferences file has unrecognized format — content does not use YAML frontmatter delimiters (---). " +
      "Wrap your preferences in --- fences. See https://github.com/waghelapritesh/tac-2/issues/2036",
    );
  }
  return null;
}
...
function parseFrontmatterBlock(frontmatter: string): TACPreferences {
  try {
    const parsed = parseYaml(frontmatter);
    if (typeof parsed !== 'object' || parsed === null) {
      return {} as TACPreferences;
    }
    return parsed as TACPreferences;
  } catch (e) {
...
/**
 * Parse heading+list format into a nested object, then cast to TACPreferences.
 ...
 *   - planner: sonnet
 */
function parseHeadingListFormat(content: string): TACPreferences {
  ...
  return typed as TACPreferences;
}
 ...
 */
export function applyModeDefaults(mode: WorkflowMode, prefs: TACPreferences): TACPreferences {
  const defaults = MODE_DEFAULTS[mode];
  if (!defaults) return prefs;
  return mergePreferences(defaults, prefs);
}

function mergePreferences(base: TACPreferences, override: TACPreferences): TACPreferences {
  return {
...
export function renderPreferencesForSystemPrompt(preferences: TACPreferences, resolutions?: Map<string, SkillResolution>): string {
  const validated = validatePreferences(preferences);
  const lines: string[] = ["## TAC Skill Preferences"];
  ...
  lines.push(
    "- Treat these as explicit skill-selection policy for TAC work.",
 ...
 *
 * Default is "none" so TAC works out of the box without preferences.md.
...
export function resolveParallelConfig(prefs: TACPreferences | undefined): import("./types.js").ParallelConfig {
  return {
