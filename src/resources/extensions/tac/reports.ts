/**
 * TAC Reports Registry
 *
 * Manages .tac/reports/ — the persistent progression log of HTML snapshots.
 *
 * Layout:
 *   .tac/reports/
...
export function reportsDir(basePath: string): string {
  return join(tacRoot(basePath), 'reports');
}
...
/**
 * Write a report snapshot to .tac/reports/, update reports.json, regenerate index.html.
...
export function writeReportSnapshot(args: WriteReportSnapshotArgs): string {
  ...
  index.tacVersion = args.tacVersion;
...
function buildIndexHtml(index: ReportsIndex): string {
  const { projectName, projectPath, tacVersion, entries } = index;
...
<title>TAC Reports — ${esc(projectName)}</title>
...
<header>
  <div class="hdr-inner">
    <div class="branding">
      <span class="logo">TAC</span>
      <span class="ver">v${esc(tacVersion)}</span>
    </div>
    <div class="hdr-meta">
  ...
  <main>
    ...
    <section class="idx-cards">
      ...
      ${sorted.length > 0
        ...
        : '<p class="empty">No reports generated yet. Run <code>/tac export --html</code> or enable <code>auto_report: true</code>.</p>'}
    </section>
  </main>
</div>
...
<footer>
  <div class="ftr-inner">
    <span class="ftr-brand">TAC v${esc(tacVersion)}</span>
