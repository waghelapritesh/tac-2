// TAC Memory Tools — Phase 1 executors for capture_thought, memory_query, tac_graph
//
...
//   - tac_graph       → returns a memory and its supersedes edges only (Phase 4 adds memory_relations)

import { _getAdapter, isDbAvailable } from "../tac-db.js";
import {
  createMemory,
...
function dbUnavailable(operation: string): ToolExecutionResult {
  return {
    content: [
      {
        type: "text",
        text: "Error: TAC database is not available. Memory tools require an initialized .tac/ project.",
      },
    ],
...
export interface MemoryCaptureParams {
   ...
   * tac_save_decision-style fields (scope, decision, choice, rationale,
...
export interface TacGraphParams {
  mode: "build" | "query";
  memoryId?: string;
  depth?: number;
  rel?: string;
...
export function executeTacGraph(params: TacGraphParams): ToolExecutionResult {
  if (!isDbAvailable()) return dbUnavailable("tac_graph");

  if (params.mode === "build") {
    // The extractor emits LINK actions incrementally (Phase 4). There is no
    // batch rebuild step to run today — ingest artifacts via `/tac memory
      ...
      content: [
        {
          type: "text",
          text:
            "tac_graph build acknowledged. Graph edges are populated incrementally by memory " +
            "extraction (including LINK actions). Use `/tac memory extract <SRC-...>` to trigger " +
            "extraction against a specific ingested source.",
        },
      ],
      details: { operation: "tac_graph", mode: "build", built: 0 },
    };
  }
  ...
  if (params.mode !== "query") {
    return {
      ...
      details: { operation: "tac_graph", error: "invalid_mode" },
      isError: true,
    };
  }
  ...
  if (!memoryId) {
    return {
      ...
      details: { operation: "tac_graph", error: "missing_memory_id" },
      isError: true,
    };
  }
  ...
  try {
    ...
    if (nodes.length === 0) {
      return {
        ...
        details: { operation: "tac_graph", mode: "query", memoryId, nodes: [], edges: [] },
      };
    }
    ...
    return {
      ...
      details: { operation: "tac_graph", error: (err as Error).message },
      isError: true,
    };
  }
}
