// TAC Resume Tool — returns the contents of .tac/last-snapshot.md so
      ...
      content: [
        {
          type: "text",
          text:
            "No snapshot found at .tac/last-snapshot.md. The snapshot is written automatically " +
            "on session_before_compact (enabled by default; set context_mode.enabled=false to opt out).",
        },
      ],
      details: { operation: "tac_resume", found: false },
    };
  }
  return {
    content: [{ type: "text", text: snapshot }],
    details: { operation: "tac_resume", found: true, bytes: Buffer.byteLength(snapshot, "utf-8") },
  };
}
