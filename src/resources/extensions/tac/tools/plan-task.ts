...
import { transaction, getSlice, getTask, insertTask, upsertTaskPlanning } from "../tac-db.js";
  ...
  try {
    transaction(() => {
      ...
      if (existingTask && isClosedStatus(existingTask.status)) {
        guardError = `cannot re-plan task ${params.taskId}: it is already complete — use tac_task_reopen first`;
        return;
      }
