// TAC Exec Search Tool — lists and filters prior tac_exec runs.
//
// Scans .tac/exec/*.meta.json and returns a ranked summary so agents can
...
export function executeExecSearch(
  ...
  if (hits.length === 0) {
    return {
      content: [{ type: "text", text: "No prior tac_exec runs match those filters." }],
      details: { operation: "tac_exec_search", matches: 0 },
    };
  }
