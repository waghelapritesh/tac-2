/**
 * complete-task handler — the core operation behind tac_complete_task.
 *
...
import type { TaskRow } from "../tac-db.js";
...
export async function handleCompleteTask(
  ...
  if (params.escalation) {
    escalationWriteEnabled = loadEffectiveTACPreferences()?.preferences?.phases?.mid_execution_escalation === true;
  ...
  transaction(() => {
    ...
    if (existingTask && isClosedStatus(existingTask.status)) {
      ...
      // invokes tac_complete_task. Returning an error would produce noisy
      ...
      guardError = `task ${params.taskId} is already complete — use tac_task_reopen first if you need to redo it`;
      return;
    }
  ...
  if (tasksDir) {
    ...
    const manualTasksDir = join(tacDir, "milestones", params.milestoneId, "slices", params.sliceId, "tasks");
  ...
  try {
    ...
    if (planPath) {
      ...
      process.stderr.write(
        `tac-db: complete_task — could not find plan file for ${params.sliceId}/${params.milestoneId}, skipping checkbox toggle\n`,
      );
    }
  } catch (renderErr) {
