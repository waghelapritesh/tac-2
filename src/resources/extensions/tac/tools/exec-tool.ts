// TAC Exec Tool — executor for the tac_exec MCP tool.
//
    ...
    content: [
      {
        type: "text",
        text:
          "tac_exec is disabled by `context_mode.enabled: false` in preferences. Remove that " +
          "override (or set it to true) to re-enable sandboxed tool-output execution.",
      },
    ],
    details: { operation: "tac_exec", error: "context_mode_disabled" },
    isError: true,
  };
}
...
function paramError(message: string): ToolExecutionResult {
  return {
    ...
    details: { operation: "tac_exec", error: "invalid_params", detail: message },
    isError: true,
  };
}

export async function executeTacExec(
  ...
  try {
    ...
    return {
      content: [{ type: "text", text: `Error: tac_exec failed — ${message}` }],
      details: { operation: "tac_exec", error: message },
      isError: true,
    };
  }
}
...
function formatResult(result: ExecSandboxResult): ToolExecutionResult {
  const headerLines = [
    `tac_exec[${result.id}] runtime=${result.runtime} exit=${formatExit(result)} duration=${result.duration_ms}ms`,
