...
export async function executeSummarySave(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot save artifact." }],
  ...
  try {
    ...
    logError("tool", `tac_summary_save tool failed: ${msg}`, { tool: "tac_summary_save", error: String(err) });
    return {
...
export async function executeTaskComplete(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot complete task." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_task tool failed: ${msg}`, { tool: "tac_task_complete", error: String(err) });
    return {
...
export async function executeSliceComplete(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot complete slice." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_slice tool failed: ${msg}`, { tool: "tac_slice_complete", error: String(err) });
    return {
...
export async function executeCompleteMilestone(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot complete milestone." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_milestone tool failed: ${msg}`, { tool: "tac_complete_milestone", error: String(err) });
    return {
...
export async function executeValidateMilestone(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot validate milestone." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `validate_milestone tool failed: ${msg}`, { tool: "tac_validate_milestone", error: String(err) });
    return {
...
export async function executeReassessRoadmap(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot reassess roadmap." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `reassess_roadmap tool failed: ${msg}`, { tool: "tac_reassess_roadmap", error: String(err) });
    return {
...
export async function executeSaveGateResult(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `tac_save_gate_result failed: ${msg}`, { tool: "tac_save_gate_result", error: String(err) });
    return {
...
export async function executePlanMilestone(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot plan milestone." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `plan_milestone tool failed: ${msg}`, { tool: "tac_plan_milestone", error: String(err) });
    return {
...
export async function executePlanSlice(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot plan slice." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `plan_slice tool failed: ${msg}`, { tool: "tac_plan_slice", error: String(err) });
    return {
...
export async function executeReplanSlice(
  ...
  if (!dbAvailable) {
    return {
      content: [{ type: "text", text: "Error: TAC database is not available. Cannot replan slice." }],
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `replan_slice tool failed: ${msg}`, { tool: "tac_replan_slice", error: String(err) });
    return {
...
export async function executeMilestoneStatus(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logWarning("tool", `tac_milestone_status tool failed: ${msg}`);
    return {
