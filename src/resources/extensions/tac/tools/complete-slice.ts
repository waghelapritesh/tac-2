/**
 * complete-slice handler — the core operation behind tac_slice_complete.
 *
...
export async function handleCompleteSlice(
  ...
  transaction(() => {
    ...
    if (slice && isClosedStatus(slice.status)) {
      ...
      guardError = `slice ${params.sliceId} is already complete — use tac_slice_reopen first if you need to redo it`;
      return;
    }
  ...
  if (sliceDir) {
    ...
    const manualSliceDir = join(tacDir, "milestones", params.milestoneId, "slices", params.sliceId);
  ...
  (async () => {
    try {
      const graphMod = await import("@waghelapritesh/mcp-server") as unknown as Partial<{
        buildGraph: (dir: string) => Promise<{ nodes: unknown[]; edges: unknown[]; builtAt: string }>;
        writeGraph: (tacRoot: string, graph: unknown) => Promise<void>;
        resolveTacRoot: (basePath: string) => string;
      }>;
      if (
        typeof graphMod.buildGraph !== "function"
        || typeof graphMod.writeGraph !== "function"
        || typeof graphMod.resolveTacRoot !== "function"
      ) {
        throw new Error("graph helpers unavailable from @waghelapritesh/mcp-server");
      }
      const g = await graphMod.buildGraph(basePath);
      await graphMod.writeGraph(graphMod.resolveTacRoot(basePath), g);
    } catch (graphErr) {
