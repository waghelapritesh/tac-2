    ...
    transaction(() => {
      ...
      if (completedSlices.length > 0) {
        ...
        if (droppedCompleted.length > 0) {
          guardError = `cannot re-plan milestone ${params.milestoneId}: ${droppedCompleted.length} completed slice(s) would be dropped (${droppedCompleted.map(s => s.id).join(", ")}). Use tac_reassess_roadmap to modify the roadmap.`;
          return;
        }
      }
