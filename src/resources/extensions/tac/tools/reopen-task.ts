/**
 * reopen-task handler — the core operation behind tac_task_reopen.
 *
...
export async function handleReopenTask(
  ...
  transaction(() => {
    ...
    if (isClosedStatus(slice.status)) {
      guardError = `cannot reopen task in a closed slice: ${params.sliceId} (status: ${slice.status}) — use tac_slice_reopen first`;
      return;
    }
