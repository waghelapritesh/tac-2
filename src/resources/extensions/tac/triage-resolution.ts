/**
 * TAC Triage Resolution — Execute triage classifications
 *
...
export function executeInject(
  ...
  try {
    ...
    const planPath = join(tacRoot(basePath), "milestones", mid, "slices", sid, `${sid}-PLAN.md`);
...
export function executeReplan(
  ...
  try {
    const triggerPath = join(
      basePath, ".tac", "milestones", mid, "slices", sid, `${sid}-REPLAN-TRIGGER.md`,
    );
    ...
    try {
      ...
      const { isDbAvailable, setSliceReplanTriggeredAt } = req("./tac-db.js");
      if (isDbAvailable()) {
 ...
 *
 * Writes a BACKTRACK-TRIGGER.md marker at `.tac/BACKTRACK-TRIGGER.md` with
...
export function executeBacktrack(
  ...
  try {
    ...
    const ts = new Date().toISOString();
    const triggerPath = join(tacRoot(basePath), "BACKTRACK-TRIGGER.md");
    const content = [
      `# Backtrack Trigger`,
      ``,
      `**Source:** Capture ${capture.id}`,
...
export function buildQuickTaskPrompt(capture: CaptureEntry): string {
  return [
    `You are executing a quick one-off task captured during a TAC auto-mode session.`,
    ``,
    `## Quick Task`,
    ``,
    ...
    `3. Do NOT modify any \`.tac/\` plan files — this is a one-off, not a planned task.`,
