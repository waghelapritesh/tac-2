 ...
 */
const WORKTREE_MARKER = "/.tac/worktrees/";
 ...
 * directory (contains `/.tac/worktrees/`), strip that suffix to recover the
...
export class WorktreeResolver {
  ...
  private rebuildGitService(): void {
    const gitConfig =
      this.deps.loadEffectiveTACPreferences()?.preferences?.git ?? {};
  ...
  mergeAndExit(milestoneId: string, ctx: NotifyCtx): void {
    ...
    try {
      ...
      if (startSha) {
        const prefs = loadEffectiveTACPreferences(this.s.originalBasePath || this.s.basePath)?.preferences;
  ...
  private _mergeWorktreeMode(milestoneId: string, ctx: NotifyCtx): boolean {
    ...
    try {
      ...
      // silently or .tac/ is not symlinked. Without the fallback, a missing
      ...
      if (roadmapPath) {
        ...
        if (mergeResult.codeFilesChanged) {
          ...
          // (#1906) Milestone produced only .tac/ metadata — no actual code was
          ...
          ctx.notify(
            `WARNING: Milestone ${milestoneId} merged to main but contained NO code changes — only .tac/ metadata files. ` +
      ...
      // /tac dispatch complete-milestone or merge manually once the underlying
      // issue is fixed (e.g. checkout to wrong branch, unresolved conflicts).
      // (#1668, #1891)
      ctx.notify(
        `Milestone merge failed: ${msg}. Your worktree and milestone branch are preserved — retry with \`/tac dispatch complete-milestone\` or merge manually.`,
        "warning",
      );
  ...
  private _mergeBranchMode(milestoneId: string, ctx: NotifyCtx): boolean {
    try {
      ...
      if (mergeResult.codeFilesChanged) {
        ...
        ctx.notify(
          `WARNING: Milestone ${milestoneId} merged (branch mode) but contained NO code changes — only .tac/ metadata. ` +
            `Review the milestone output and re-run if code is missing.`,
          "warning",
        );
      }
