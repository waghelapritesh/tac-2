/**
 * TAC Extensions Command — /tac extensions
 *
 ...
 * via jiti at runtime from ~/.tac/agent/, not compiled by tsc).
 */

import type { ExtensionCommandContext } from "@tac/pi-coding-agent";
...
function getRegistryPath(): string {
  return join(tacHome, "extensions", "registry.json");
}

function getAgentExtensionsDir(): string {
  return join(tacHome, "agent", "extensions");
}
 ...
 * Prevents two concurrent `tac extensions install/uninstall/update` invocations
...
export function validateExtensionPackage(packageDir: string): ValidationResult {
  ...
  // (a) tac.extension: true marker (D-12a)
  const tacField = pkg.tac as Record<string, unknown> | undefined;
  if (tacField?.extension !== true) {
    errors.push('package.json missing "tac": { "extension": true }');
  }
  ...
  // (c) @tac/* packages must be in peerDependencies, not dependencies/devDependencies (D-12c)
...
function getInstalledExtDir(): string {
  return join(tacHome, "extensions");
}
...
function validateExtensionManifest(pkg: unknown, opts: { extensionId?: string; allowTacNamespace?: boolean } = {}): ManifestValidationResult {
  const errors: ManifestValidationError[] = [];

  // Check tac.extension === true (strict)
  if (typeof pkg !== "object" || pkg === null) {
    errors.push({ code: "MISSING_TAC_MARKER", message: 'package.json must declare "tac": { "extension": true } to be recognized as a TAC extension.', field: "tac.extension" });
  } else {
    const obj = pkg as Record<string, unknown>;
    const tac = obj.tac;
    if (typeof tac !== "object" || tac === null || (tac as Record<string, unknown>).extension !== true) {
      errors.push({ code: "MISSING_TAC_MARKER", message: 'package.json must declare "tac": { "extension": true } to be recognized as a TAC extension.', field: "tac.extension" });
    }
  }

  // Check namespace reservation
  if (opts.extensionId && opts.extensionId.startsWith("tac.") && opts.allowTacNamespace !== true) {
    errors.push({ code: "RESERVED_NAMESPACE", message: `Extension ID "${opts.extensionId}" is reserved for TAC core extensions. Use a different namespace for community extensions.`, field: "extensionId" });
  }
...
function handleUninstall(id: string | undefined, ctx: ExtensionCommandContext): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac extensions uninstall <id>", "warning");
    return;
  }
  ...
  if (!result.ok) {
    if (result.reason === "not-found") {
      ctx.ui.notify(
        `Extension "${id}" not found in registry. Run /tac extensions list to see installed extensions.`,
        "warning",
      );
  ...
  ctx.ui.notify(`Uninstalled "${id}". Restart TAC to deactivate.`, "info");
}
...
async function updateSingleExtension(
  ...
  if (entry.installType !== "npm") {
    ...
    const hint = entry.installedFrom ? `tac extensions install ${entry.installedFrom}` : `tac extensions install <specifier>`;
    ctx.ui.notify(
  ...
  if (pin) {
    ctx.ui.notify(
      `"${id}" was installed with a pinned version (${pin}). To update, run: tac extensions install ${packageName}@<new-version>`,
      "info",
    );
    return;
  }
...
async function updateAllExtensions(
  ...
  if (userEntries.length === 0) {
    ctx.ui.notify("No user-installed extensions found. Use: tac extensions install <package> to add one.", "warning");
    return;
  }
...
async function handleInstall(specifier: string | undefined, ctx: ExtensionCommandContext): Promise<void> {
  if (!specifier) {
    ctx.ui.notify("Usage: /tac extensions install <npm-package|git-url|local-path>", "warning");
    return;
  }
...
function installFromNpm(specifier: string, installedExtDir: string, ctx: ExtensionCommandContext): void {
  ...
  // filesystem (avoids EXDEV when tmpdir() and ~/.tac live on different mounts).
  const packDir = mkdtempSync(join(tmpdir(), "tac-install-"));
  ...
  try {
    ...
    ctx.ui.notify(`Installed "${validated.id}" v${validated.manifest.version ?? "unknown"}. Restart TAC to activate.`, "info");
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
...
export async function handleExtensions(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  ctx.ui.notify(
    `Unknown: /tac extensions ${subCmd}. Usage: /tac extensions [list|enable|disable|info|install|uninstall|update|validate]`,
    "warning",
  );
}
...
function handleList(ctx: ExtensionCommandContext): void {
  ...
  for (const m of sorted) {
    ...
    if (!enabled) {
      lines.push(`  ↳ tac extensions enable ${m.id}`);
    }
  }
...
function handleEnable(id: string | undefined, ctx: ExtensionCommandContext): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac extensions enable <id>", "warning");
    return;
  }

  const manifests = discoverManifests();
  if (!manifests.has(id)) {
    ctx.ui.notify(`Extension "${id}" not found. Run /tac extensions list to see available extensions.`, "warning");
    return;
  }
  ...
  ctx.ui.notify(`Enabled "${id}". Restart TAC to activate.`, "info");
}

function handleDisable(id: string | undefined, reason: string, ctx: ExtensionCommandContext): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac extensions disable <id>", "warning");
    return;
  }
  ...
  ctx.ui.notify(`Disabled "${id}". Restart TAC to deactivate.`, "info");
}

function handleInfo(id: string | undefined, ctx: ExtensionCommandContext): void {
  if (!id) {
    ctx.ui.notify("Usage: /tac extensions info <id>", "warning");
    return;
  }
...
function handleValidate(path: string | undefined, ctx: ExtensionCommandContext): void {
  if (!path) {
    ctx.ui.notify("Usage: /tac extensions validate <path>", "warning");
    return;
  }
