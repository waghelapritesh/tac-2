/**
 * TAC Slice Parallel Orchestrator — Engine for parallel slice execution
 ...
 * child_process, each running in its own git worktree with TAC_SLICE_LOCK
 * + TAC_MILESTONE_LOCK env vars set.
 *
 * Key differences from milestone-level parallelism:
 * - Scope: slices within one milestone, not milestones within a project
 * - Lock env: TAC_SLICE_LOCK (in addition to TAC_MILESTONE_LOCK)
 ...
 *
 * For each eligible slice: create a worktree, spawn `tac --mode json --print "/tac auto"`
 * with env TAC_SLICE_LOCK=<SID> + TAC_MILESTONE_LOCK=<MID> + TAC_PARALLEL_WORKER=1.
 */
 ...
 * Same logic as parallel-orchestrator.ts resolveTacBin().
 */
 ...
 * The worker runs `tac --mode json --print "/tac auto"` in the slice's worktree
 * with TAC_SLICE_LOCK, TAC_MILESTONE_LOCK, and TAC_PARALLEL_WORKER set.
 */
function spawnSliceWorker(
  ...
  try {
    child = spawn(process.execPath, [binPath, "--mode", "json", "--print", "/tac auto"], {
      cwd: worker.worktreePath,
      env: {
        ...process.env,
        TAC_SLICE_LOCK: sliceId,
...
function sliceLogDir(basePath: string): string {
  return join(tacRoot(basePath), "parallel", "slice-logs");
}
