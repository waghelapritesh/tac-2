/**
 * TAC Command — /tac backlog
 *
 * Structured backlog management with 999.x numbering.
 * Items stored in .tac/BACKLOG.md as markdown checklist.
...
function backlogPath(basePath: string): string {
  return join(tacRoot(basePath), "BACKLOG.md");
}
...
async function listBacklog(basePath: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (items.length === 0) {
    ctx.ui.notify("Backlog is empty. Add items with /tac backlog add <title>", "info");
    return;
  }
...
async function addBacklogItem(basePath: string, title: string, ctx: ExtensionCommandContext): Promise<void> {
  if (!title) {
    ctx.ui.notify("Usage: /tac backlog add <title>", "warning");
    return;
  }
...
async function promoteBacklogItem(
  ...
  if (!itemId) {
    ctx.ui.notify("Usage: /tac backlog promote <id>\nExample: /tac backlog promote 999.1", "warning");
    return;
  }
...
async function removeBacklogItem(basePath: string, itemId: string, ctx: ExtensionCommandContext): Promise<void> {
  if (!itemId) {
    ctx.ui.notify("Usage: /tac backlog remove <id>", "warning");
    return;
  }
