...
export function readPausedSessionMetadata(
  ...
  const pausedPath = join(tacRoot(basePath), "runtime", "paused-session.json");
...
export function hasResumableDerivedState(state: TACState | null): boolean {
...
export async function assessInterruptedSession(
  ...
  if (lock && !isBootstrapCrash && !artifactSatisfied) {
    recovery = synthesizeCrashRecovery(
      ...
      lock.unitId,
      lock.sessionFile,
      join(tacRoot(assessmentBasePath), "activity"),
    );
  }
