/**
 * TAC Command Handlers — fire-and-forget handlers that delegate to other modules.
 *
...
const UPDATE_REGISTRY_URL = "https://registry.npmjs.org/tac-2/latest";
const UPDATE_FETCH_TIMEOUT_MS = 5000;

// Detects a bun-installed tac via `process.argv[1]`. Mirrors isBunInstall in
...
export function dispatchDoctorHeal(pi: ExtensionAPI, scope: string | undefined, reportText: string, structuredIssues: string): void {
  ...
  const content = `Read the following TAC workflow protocol and execute exactly.\n\n${workflow}\n\n## Your Task\n\n${prompt}`;

  pi.sendMessage(
    { customType: "tac-doctor-heal", content, display: false },
    { triggerTurn: true },
  );
}
...
export async function handleDoctor(args: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
  ...
  const report = await runTACDoctor(projectRoot(), {
  ...
  const reportText = formatDoctorReport(report, {
    ...
    title: mode === "audit" ? "TAC doctor audit." : mode === "heal" ? "TAC doctor heal prep." : undefined,
  });
...
export async function handleSkillHealth(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  // /tac skill-health <skill-name> — detail view
...
export async function handleCapture(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  let text = args.trim();
  if (!text) {
    ctx.ui.notify('Usage: /tac capture "your thought here"', "warning");
    return;
  }
  ...
  // Ensure .tac/ exists — capture should work even without a milestone
...
export async function handleSteer(change: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
  ...
  const overrideLoc = wtPath ? "worktree `.tac/OVERRIDES.md`" : "`.tac/OVERRIDES.md`";

  if (isAutoActive()) {
    pi.sendMessage({
      customType: "tac-hard-steer",
      content: [
...
export async function handleKnowledge(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (!typeArg || !["rule", "pattern", "lesson"].includes(typeArg)) {
    ctx.ui.notify(
      "Usage: /tac knowledge <rule|pattern|lesson> <description>\nExample: /tac knowledge rule Use real DB for integration tests",
      "warning",
    );
    return;
  }
  ...
  if (!entryText) {
    ctx.ui.notify(`Usage: /tac knowledge ${typeArg} <description>`, "warning");
    return;
  }
...
export async function handleRunHook(args: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
  ...
  if (parts.length < 3) {
    ctx.ui.notify(`Usage: /tac run-hook <hook-name> <unit-type> <unit-id>
...
Examples:
  /tac run-hook code-review execute-task M001/S01/T01
  /tac run-hook lint-check plan-slice M001/S01`, "warning");
    return;
  }
...
export async function handleUpdate(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const NPM_PACKAGE = "tac-2";
  const current = process.env.TAC_VERSION || "0.0.0";
  ...
  try {
    ...
    ctx.ui.notify(
      `Updated to v${latest}. Restart your TAC session to use the new version.`,
      "info",
    );
  } catch {
    ctx.ui.notify(
