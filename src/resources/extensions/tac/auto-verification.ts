...
import { isDbAvailable, getTask, getSliceTasks, getMilestoneSlices, type TaskRow } from "./tac-db.js";
 ...
 * expected to also call tac_reassess_roadmap in the same turn to add
...
async function runValidateMilestonePostCheck(
  ...
  process.stderr.write(
    `validate-milestone: pausing — verdict=needs-remediation with no incomplete slices for ${mid}. ` +
      `The agent must call tac_reassess_roadmap to add remediation slices before re-validation.\n`,
  );
  await persistMilestoneValidationGate(
    "manual-attention",
    "manual-attention",
...
export async function runPostUnitVerification(
  ...
  try {
    const effectivePrefs = loadEffectiveTACPreferences();
      ...
      if (enhancedEnabled && postEnabled && isDbAvailable()) {
        try {
          ...
          if (taskRow && taskRow.key_files && taskRow.key_files.length > 0) {
            ...
            // Log summary to stderr with tac-post-exec: prefix
            const emoji =
              postExecResult.status === "pass"
                ...
                : postExecResult.status === "warn"
                  ...
                  : "❌";
            process.stderr.write(
              `tac-post-exec: ${emoji} Post-execution checks ${postExecResult.status} for ${mid}/${sid}/${tid} (${postExecResult.durationMs}ms)\n`
            );
            ...
            for (const check of postExecResult.checks) {
              const checkEmoji = check.passed
                ...
                : check.blocking
                  ...
                  : "⚠";
              process.stderr.write(
                `tac-post-exec:   ${checkEmoji} [${check.category}] ${check.target}: ${check.message}\n`
              );
            }
          ...
          logWarning("engine", `tac-post-exec: error — ${(postExecErr as Error).message}`);
        }
      }
    }
