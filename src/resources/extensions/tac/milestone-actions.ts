/**
 * TAC Milestone Actions — Park, Unpark, and Discard operations.
 *
...
import { deleteMilestone, getMilestone, isDbAvailable, updateMilestoneStatus } from "./tac-db.js";
...
function assertNotAutoActive(action: string): void {
  if (isAutoActive()) {
    throw new Error(
      `${action} cannot run while auto-mode is active. Stop auto-mode first with /tac stop.`,
    );
  }
}
...
export function unparkMilestone(basePath: string, milestoneId: string): boolean {
  ...
  // PARKED marker was lost on disk, and /tac unpark should repair that state.
