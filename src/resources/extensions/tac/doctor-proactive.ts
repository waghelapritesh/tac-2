/**
 * TAC Doctor — Proactive Healing Layer
 *
...
import { tacRoot, resolveTacRootFile } from "./paths.js";
...
export async function preDispatchHealthGate(basePath: string): Promise<PreDispatchHealthResult> {
    ...
    if (existsSync(gitDir)) {
      ...
      if (blockers.length > 0) {
        ...
        try {
          ...
          issues.push(`Corrupt git state: ${blockers.join(", ")}. Run /tac doctor fix.`);
        }
      }
    }
  } catch {
  ...
  try {
    ...
    const milestonesDir = join(tacRoot(basePath), "milestones");
    ...
    if (nativeIsRepo(basePath)) {
      ...
      if (state.activeMilestone) {
        ...
        if (resolution.status === "fallback" && resolution.effectiveBranch) {
          ...
          issues.push(
            `${resolution.reason} Restore the branch or update the integration branch before dispatching. Run /tac doctor for details.`,
          );
        }
      }
    }
      ...
      if (snapshotsEnabled && thresholdMinutes > 0 && nativeHasChanges(basePath)) {
        ...
        if (minutesSinceCommit >= thresholdMinutes) {
          ...
          try {
            ...
            const commitMsg = `tac snapshot: pre-dispatch, uncommitted changes after ${mins}m inactivity`;
            const result = nativeCommit(basePath, commitMsg);
            if (result) {
              fixesApplied.push(`pre-dispatch: created tac snapshot after ${mins}m of uncommitted changes`);
            }
          } catch {
            // Non-blocking — snapshot failed but dispatch can continue
            fixesApplied.push("pre-dispatch: tac snapshot failed");
          }
        }
      }
    }
