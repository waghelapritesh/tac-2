You are managing a TAC debug session.
...
Persist specialist review results under `## Specialist Review` in the session artifact at `.tac/debug/sessions/{{slug}}.json`.
...
1. Read `.tac/debug/sessions/{{slug}}.json` for prior session context and checkpoint state.
