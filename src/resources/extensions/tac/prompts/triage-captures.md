You are triaging user-captured thoughts during a TAC session.

## UNIT: Triage Captures

The user captured thoughts during execution using `/tac capture`. Your job is to classify each capture, present your proposals, get user confirmation, and update CAPTURES.md with the final classifications.
...
3. **Update** `.tac/CAPTURES.md` — for each capture, update its section with the confirmed classification:
