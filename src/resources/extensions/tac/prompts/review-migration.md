## Review Migrated .tac Directory

A `/tac migrate` command just wrote a `.tac/` directory from an old `.planning` source. Your job is to audit the output and verify it meets TAC-2 standards before the user starts working with it.

### Source
- Old `.planning` directory: `{{sourcePath}}`
- Written `.tac` directory: `{{tacPath}}`
...
- Run `deriveState()` on the `.tac` directory (import from `state.ts`, pass the **project root** as basePath)
...
If the overall result is FAIL, explain what needs manual attention. If PASS WITH NOTES, explain what's imperfect but acceptable. If PASS, confirm the `.tac` directory is ready for TAC-2 auto-mode.
