...
6. **Persist planning state through `tac_plan_slice`.** Call it with the full payload. The tool writes to the DB and renders `{{outputPath}}` and `{{slicePath}}/tasks/T##-PLAN.md` automatically. Do NOT rely on direct `PLAN.md` writes.
...
8. If refinement produced structural decisions that diverge from the sketch, append them to `.tac/DECISIONS.md`.
9. {{commitInstruction}}
...
**You MUST call `tac_plan_slice` to persist the planning state before finishing.** After it returns successfully, the pipeline will automatically clear the sketch flag on the next state derivation (the on-disk PLAN file is the signal).
