You are executing TAC auto-mode.
...
**On-demand Read ordering:** Complete all slice SUMMARY Reads you need for cross-slice synthesis, the Decision Re-evaluation table, and LEARNINGS **before** calling `tac_complete_milestone` (step 10). Once that tool runs, the milestone is marked complete in the DB — running out of tool budget between step 10 and the LEARNINGS write (step 12) leaves the milestone committed without its LEARNINGS artifact.
...
2. {{skillActivation}}
3. **Verify code changes exist.** Run `git diff --stat HEAD $(git merge-base HEAD main) -- ':!.tac/'` (or the equivalent for the integration branch). If no non-`.tac/` files appear in the diff, the milestone produced only planning artifacts and no actual code. Record this as a **verification failure**.
...
7. Fill the **Decision Re-evaluation** table in the milestone summary. For each key decision from `.tac/DECISIONS.md` made during this milestone, evaluate whether it is still valid given what was actually built. Flag decisions that should be revisited next milestone.
...
**DB access safety:** Do NOT query `.tac/tac.db` directly via `sqlite3` or `node -e require('better-sqlite3')` — the engine owns the WAL connection. Use `tac_milestone_status` to read milestone and slice state. All data you need is already inlined in the context above or accessible via the `tac_*` tools — never via direct SQL.
...
- Do NOT call `tac_complete_milestone` — the milestone must not be marked as complete.
- Do NOT update `.tac/PROJECT.md` to reflect completion.
- Do NOT update `.tac/REQUIREMENTS.md` to mark requirements as validated.
...
9. For each requirement whose status changed in step 8, call `tac_requirement_update` with the requirement ID and updated `status` and `validation` fields — the tool regenerates `.tac/REQUIREMENTS.md` automatically. Do this BEFORE completing the milestone so requirement updates are persisted.
10. **Persist completion through `tac_complete_milestone`.** Call it with the parameters below. The tool updates the milestone status in the DB, renders `{{milestoneSummaryPath}}`, and validates all slices are complete before proceeding.
...
11. Update `.tac/PROJECT.md`: use the `write` tool with `path: ".tac/PROJECT.md"` and `content` containing the full updated document reflecting milestone completion and current project state. Do NOT use the `edit` tool for this — PROJECT.md is a full-document refresh.
12. Extract structured learnings from this milestone and persist them to the TAC memory store. Follow the procedure block immediately below — it writes `{{milestoneId}}-LEARNINGS.md` as the audit trail and persists Patterns, Lessons, and Decisions via `capture_thought` (categories: pattern, gotcha/convention, architecture). The memory store is the single source of truth for cross-session durable knowledge (ADR-013).
