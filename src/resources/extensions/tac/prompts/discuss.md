...
**Incremental persistence:** After every 2 question rounds (across any layer), silently save a `{{milestoneId}}-CONTEXT-DRAFT.md` using `tac_summary_save` with `artifact_type: "CONTEXT-DRAFT"` and `milestone_id: "{{milestoneId}}"`. This protects confirmed work against session crashes. Do NOT mention this save to the user.
...
For a new project or any project that does not yet have `.tac/REQUIREMENTS.md`, do a focused research pass before roadmap creation.
...
Before writing a roadmap, produce or update `.tac/REQUIREMENTS.md`.
...
- Milestone dir: `.tac/milestones/{{milestoneId}}/`
...
1. `mkdir -p .tac/milestones/{{milestoneId}}/slices`
2. Write or update `.tac/PROJECT.md` — use the **Project** output template below. Describe what the project is, its current state, and list the milestone sequence.
3. Write or update `.tac/REQUIREMENTS.md` — use the **Requirements** output template below. Confirm requirement states, ownership, and traceability before roadmap creation.
...
- [ ] Written `.tac/PROJECT.md` (step 2)
- [ ] Written `.tac/REQUIREMENTS.md` (step 3)
- [ ] Written `{{contextPath}}` (step 4)
- [ ] Called `tac_plan_milestone` (step 5)
...
1. For each milestone, call `tac_milestone_generate_id` to get its ID — never invent milestone IDs manually. Then `mkdir -p .tac/milestones/<ID>/slices`.
2. Write `.tac/PROJECT.md` — use the **Project** output template below.
3. Write `.tac/REQUIREMENTS.md` — use the **Requirements** output template below. Capture Active, Deferred, Out of Scope, and any already Validated requirements. Later milestones may have provisional ownership where slice plans do not exist yet.
4. For any architectural or pattern decisions made during discussion, call `tac_decision_save` — the tool auto-assigns IDs and regenerates `.tac/DECISIONS.md` automatically.

#### Phase 2: Primary milestone

5. Write a full `CONTEXT.md` for the primary milestone (the one discussed in depth).
6. Call `tac_plan_milestone` for **only the primary milestone** — detail-planning later milestones now is waste because the codebase will change. Include requirement coverage and a milestone definition of done.
...
After EVERY Phase 3 gate decision, immediately write or update `.tac/DISCUSSION-MANIFEST.json` with the cumulative state. This file is mechanically validated by the system before auto-mode starts — if gates are incomplete, auto-mode will NOT start.
...
- [ ] Written `.tac/PROJECT.md` (Phase 1)
- [ ] Written `.tac/REQUIREMENTS.md` (Phase 1)
- [ ] Written primary-milestone `CONTEXT.md` (Phase 2)
- [ ] Called `tac_plan_milestone` for the primary milestone (Phase 2)
- [ ] Written `.tac/DISCUSSION-MANIFEST.json` with `gates_completed === total` (Phase 3)
