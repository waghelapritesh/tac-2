...
You are creating a TAC milestone from a provided specification document. This is a **headless** (non-interactive) flow — do NOT ask the user any questions. Wherever the interactive flow would ask the user, make your best-judgment call and document it as an assumption.
...
Before writing a roadmap, produce `.tac/REQUIREMENTS.md`.
...
6. For each architectural or pattern decision, call `tac_decision_save` — the tool auto-assigns IDs and regenerates `.tac/DECISIONS.md` automatically.
7. {{commitInstruction}}
...
1. For each milestone, call `tac_milestone_generate_id` to get its ID — never invent milestone IDs manually. Then `mkdir -p .tac/milestones/<ID>/slices` for each.
...
4. For any architectural or pattern decisions, call `tac_decision_save` — the tool auto-assigns IDs and regenerates `.tac/DECISIONS.md` automatically.
...
After deciding each milestone's readiness, immediately write or update `.tac/DISCUSSION-MANIFEST.json`:
...
- [ ] Written `.tac/PROJECT.md`
- [ ] Written `.tac/REQUIREMENTS.md`
- [ ] Written the primary milestone `CONTEXT.md`
- [ ] Called `tac_plan_milestone` for the primary milestone
- [ ] Written `.tac/DISCUSSION-MANIFEST.json` with `gates_completed === total`
...
- **Use proper tools** — `tac_plan_milestone` for roadmaps, `tac_decision_save` for decisions, `tac_milestone_generate_id` for IDs
...
- **Naming convention** — always use `tac_milestone_generate_id` for IDs. Directories use bare IDs, files use ID-SUFFIX format.
