/**
 * TAC Guided Flow — Smart Entry Wizard
 *
 * One function: showSmartEntry(). Reads state from disk, shows a contextual
 * wizard via showNextAction(), and dispatches through TAC-WORKFLOW.md.
 * No execution state, no hooks, no tools — the LLM does the rest.
 */

import type { ExtensionAPI, ExtensionContext, ExtensionCommandContext } from "@tac/pi-coding-agent";
import type { TACState } from "./types.js";
import { showNextAction } from "../shared/tui.js";
import { loadFile, saveFile } from "./files.js";
import { isDbAvailable, getMilestoneSlices } from "./tac-db.js";
...
import {
  tacRoot, milestonesDir, resolveMilestoneFile, resolveMilestonePath,
  resolveSliceFile, resolveSlicePath, resolveTacRootFile, relTacRootFile,
...
import { detectProjectState, hasTacBootstrapArtifacts } from "./detection.js";
 ...
 * Ensures any preview ID shown in the UI matches what `tac_milestone_generate_id`
 * will later return.
 */
...
function needsPlanV2Gate(state: TACState): boolean {
...
function runPlanV2Gate(
  ...
  const prefs = loadEffectiveTACPreferences()?.preferences;
...
/** Build commit instruction for planning prompts. .tac/ is managed externally and always gitignored. */
function buildDocsCommitInstruction(_message: string): string {
  return "Do not commit planning artifacts — .tac/ is managed externally.";
}
...
// phrase before giving up and asking the user to re-run /tac.
...
export function checkAutoStartAfterDiscuss(): boolean {
  ...
  // processed yet.
  const stateFile = resolveTacRootFile(basePath, "STATE");
  ...
  const projectFile = resolveTacRootFile(basePath, "PROJECT");
    ...
    try {
      ...
      if (projectIds.length > 1) {
        const missing = projectIds.filter(id => {
          ...
          const hasDir = existsSync(join(tacRoot(basePath), "milestones", id));
  ...
  const manifestPath = join(tacRoot(basePath), "DISCUSSION-MANIFEST.json");
 ...
 * next /tac invocation loops into the "All milestones complete" warning.
 *
...
export function maybeHandleReadyPhraseWithoutFiles(event: { messages: any[] }): boolean {
  ...
  if (entry.readyRejectCount > MAX_READY_REJECTS) {
    // Give up: clear state and tell the user to re-run /tac. Avoids an
    ...
    ctx.ui.notify(
      ...
      `Stopping auto-nudge. Run /tac to try again.`,
      "error",
    );
    return true;
  }
  ...
  const nudge =
    ...
    `.tac/milestones/${milestoneId}/${milestoneId}-CONTEXT.md nor ` +
    `.tac/milestones/${milestoneId}/${milestoneId}-ROADMAP.md exists on disk. ` +
    ...
    `CONTEXT.md, (2) call tac_plan_milestone, then (3) emit the ready phrase. ` +
  ...
  try {
    pi.sendMessage(
      { customType: "tac-ready-no-files", content: nudge, display: false },
      { triggerTurn: true },
    );
  } catch (e) {
...
export function maybeHandleEmptyIntentTurn(
  ...
  // /tac discuss (user-driven) produces legitimate text-only turns.
  ...
  try {
    pi.sendMessage(
      { customType: "tac-empty-turn-recovery", content: nudge, display: false },
      { triggerTurn: true },
    );
  } catch (e) {
...
/**
 * Read TAC-WORKFLOW.md and dispatch it to the LLM with a contextual note.
...
async function dispatchWorkflow(
  ...
  // Discuss flows only need a small subset of TAC tools — strip the heavy
  ...
  if (unitType?.startsWith("discuss-")) {
    ...
    // Keep all non-TAC tools (builtins, other extensions) and only the
    // TAC tools on the discuss allowlist.
    const scopedTools = currentTools.filter(
      (t) => !t.startsWith("tac_") || DISCUSS_TOOLS_ALLOWLIST.includes(t),
    );
  ...
  const workflowPath = process.env.TAC_WORKFLOW_PATH ?? join(process.env.HOME ?? "~", ".tac", "agent", "TAC-WORKFLOW.md");
  const workflow = readFileSync(workflowPath, "utf-8");

  pi.sendMessage(
    {
      customType,
      content: `Read the following TAC workflow protocol and execute exactly.\n\n${workflow}\n\n## Your Task\n\n${note}`,
      display: false,
    },
    { triggerTurn: true },
  );
...
function buildDiscussPrompt(nextId: string, preamble: string, _basePath: string, pi: ExtensionAPI, ctx: ExtensionCommandContext, preparationContext?: string): string {
  const milestoneRel = `.tac/milestones/${nextId}`;
...
/**
 * Bootstrap a .tac/ project from scratch for headless use.
 * Ensures git repo, .tac/ structure, gitignore, and preferences all exist.
 */
function bootstrapTacProject(basePath: string): void {
  if (!nativeIsRepo(basePath) || isInheritedRepo(basePath)) {
    const mainBranch = loadEffectiveTACPreferences()?.preferences?.git?.main_branch || "main";
...
export async function showHeadlessMilestoneCreation(
  ...
  // Ensure .tac/ is bootstrapped
  bootstrapTacProject(basePath);

  // Generate next milestone ID
  const existingIds = findMilestoneIds(basePath);
  const prefs = loadEffectiveTACPreferences();
  const nextId = nextMilestoneIdReserved(existingIds, prefs?.preferences?.unique_milestone_ids ?? false);

  // Create milestone directory
  const milestoneDir = join(tacRoot(basePath), "milestones", nextId, "slices");
  ...
  // and CONTEXT.md, then calls tac_plan_milestone — this is semantically the
  ...
  await dispatchWorkflow(pi, prompt, "tac-run", ctx, "discuss-milestone");
}
...
async function buildDiscussSlicePrompt(
  ...
  const sliceDirPath = `.tac/milestones/${mid}/slices/${sid}`;
...
/**
 * /tac discuss — show a picker of non-done slices and run a slice interview.
...
export async function showDiscuss(
  ...
  // Guard: no .tac/ project
  if (!existsSync(tacRoot(basePath))) {
    ctx.ui.notify("No TAC project found. Run /tac to start one first.", "warning");
    return;
  }
  ...
  try {
    ...
    await saveFile(resolveTacRootFile(basePath, "STATE"), buildStateMarkdown(state));
  } catch (err) {
  ...
  if (!state.activeMilestone?.id) {
    ...
    if (pendingMilestones.length === 0) {
      ctx.ui.notify("No active milestone. Run /tac to create one first.", "warning");
      return;
    }
  ...
  if (state.phase === "needs-discussion") {
    ...
    const choice = await showNextAction(ctx, {
      ...
      notYetMessage: "Run /tac discuss when ready to discuss this milestone.",
    });
    ...
    if (choice === "discuss_draft") {
      ...
      await dispatchWorkflow(pi, seed, "tac-discuss", ctx, "discuss-milestone");
      ...
      await dispatchWorkflow(pi, loadPrompt("guided-discuss-milestone", {
        ...
        commitInstruction: buildDocsCommitInstruction(`docs(${mid}): milestone context from discuss`),
        fastPathInstruction: "",
      }), "tac-discuss", ctx, "discuss-milestone");
    } else if (choice === "skip_milestone") {
      const milestoneIds = findMilestoneIds(basePath);
      const uniqueMilestoneIds = !!loadEffectiveTACPreferences()?.preferences?.unique_milestone_ids;
      ...
      await dispatchWorkflow(pi, await prepareAndBuildDiscussPrompt(ctx, pi, nextId, `New milestone ${nextId}.`, basePath), "tac-run", ctx, "discuss-milestone");
    }
    return;
  }
  ...
  if (!roadmapContent && !isDbAvailable()) {
    ctx.ui.notify("No roadmap yet for this milestone. Run /tac to plan first.", "warning");
    return;
  }
  ...
  while (true) {
    ...
    if (allDiscussed) {
      ...
      const nextStep = remoteAutoRunning
        ? "Auto-mode is already running — use /tac status to check progress."
        : "Run /tac to start planning.";
      ctx.ui.notify(
    ...
    const choice = await showNextAction(ctx, {
      title: "TAC — Discuss a slice",
      summary: [
        `${mid}: ${milestoneTitle}`,
        "Pick a slice to interview. Context file will be written when done.",
      ],
      actions,
      notYetMessage: "Run /tac discuss when ready.",
    });
    ...
    await dispatchWorkflow(pi, prompt, "tac-discuss", ctx, "discuss-slice");
...
async function showDiscussQueuedMilestone(
  ...
  const choice = await showNextAction(ctx, {
    title: "TAC — Discuss a queued milestone",
    summary: [
...
async function dispatchDiscussForMilestone(
  ...
  const prompt = draftContent
    ? `${basePrompt}\n\n## Prior Discussion (Draft Seed)\n\n${draftContent}`
    : basePrompt;
  await dispatchWorkflow(pi, prompt, "tac-discuss", ctx, "discuss-milestone");
}
 ...
 * but guided-flow (manual /tac mode) never called it — meaning stale records
 * persisted until the next /tac auto run.  This ensures the wizard always
...
async function handleMilestoneActions(
  ...
  if (choice === "park") {
    ...
    if (success) {
      ctx.ui.notify(`Parked ${milestoneId}. Run /tac unpark ${milestoneId} to reactivate.`, "info");
    } else {
...
export async function showSmartEntry(
  ...
  // Reservations only need to survive within a single /tac interaction.
  ...
  // Check bootstrap completeness, not just .tac/ directory existence.
  // A zombie .tac/ state (symlink exists but missing PREFERENCES.md and
  // milestones/) must trigger the init wizard, not skip it (#2942).
  const tacPath = tacRoot(basePath);
  const hasBootstrapArtifacts = hasTacBootstrapArtifacts(tacPath);
  ...
  if (!hasBootstrapArtifacts) {
    ...
    // No .tac/ or zombie .tac/ — run the project init wizard
    const result = await showProjectInit(ctx, pi, basePath, detection);
    if (!result.completed) return; // User cancelled

    // Init wizard bootstrapped .tac/ — fall through to the normal flow below
    // which will detect "no milestones" and start the discuss prompt
  }

  // ── Ensure git repo exists — TAC needs it for worktree isolation ──────
  // Also handle inherited repos: if basePath is a subdirectory of another
  // git repo that has no .tac, create a fresh repo to prevent cross-project
  // state leaks (#1639).
  if (!nativeIsRepo(basePath) || isInheritedRepo(basePath)) {
    const mainBranch = loadEffectiveTACPreferences()?.preferences?.git?.main_branch || "main";
    nativeInit(basePath, mainBranch);
  ...
  if (interrupted.classification === "stale") {
    ...
    if (interrupted.pausedSession) {
      try {
        unlinkSync(join(tacRoot(basePath), "runtime", "paused-session.json"));
      } catch (e) {
    ...
    const resumeLabel = interrupted.pausedSession?.stepMode
      ? "Resume with /tac next"
      : "Resume with /tac auto";
    const resume = await showNextAction(ctx, {
      title: "TAC — Interrupted Session Detected",
  ...
  if (!state.activeMilestone?.id) {
    ...
    // Both /tac and /tac auto reach this branch when no milestone exists yet.
    // Without this guard, every subsequent /tac call overwrites the pending auto-start
    ...
    if (pendingAutoStartMap.has(basePath)) {
      ...
      const manifestExists = existsSync(join(tacRoot(basePath), "DISCUSSION-MANIFEST.json"));
      const milestoneHasContext = existsSync(
        join(tacRoot(basePath), "milestones", entry.milestoneId, `${entry.milestoneId}-CONTEXT.md`),
      );
        ...
        try {
          ...
          if (entries.length > 0) {
            ctx.ui.notify(
              ...
              `This may indicate a corrupted state or wrong working directory. Run \`/tac doctor\` to diagnose.`,
              "warning",
            );
            return;
          }
    ...
    if (isFirst) {
      ...
      pendingAutoStartMap.set(basePath, { ctx, pi, basePath, milestoneId: nextId, step: stepMode, createdAt: Date.now() });
      await dispatchWorkflow(pi, await prepareAndBuildDiscussPrompt(ctx, pi, nextId,
        `New project, milestone ${nextId}. Do NOT read or explore .tac/ — it's empty scaffolding.`,
        basePath
      ), "tac-run", ctx, "discuss-milestone");
    } else {
      const choice = await showNextAction(ctx, {
        title: "TAC — Think. Architect. Code.",
  ...
  if (state.phase === "complete") {
    const choice = await showNextAction(ctx, {
      title: `TAC — ${milestoneId}: ${milestoneTitle}`,
  ...
  if (state.phase === "needs-discussion") {
    ...
    const choice = await showNextAction(ctx, {
      ...
      notYetMessage: "Run /tac when ready to discuss this milestone.",
    });
  ...
  if (!state.activeSlice) {
    ...
    if (!hasRoadmap || !roadmapHasSlices) {
      ...
      const choice = await showNextAction(ctx, {
        ...
        actions,
        notYetMessage: "Run /tac status for details.",
      });
  ...
  if (state.phase === "planning") {
    ...
    const choice = await showNextAction(ctx, {
      title: `TAC — ${milestoneId} / ${sliceId}: ${sliceTitle}`,
      summary: [summaryLine],
      actions,
      notYetMessage: "Run /tac when ready.",
    });
    ...
    if (choice === "plan") {
      ...
      }), "tac-run", ctx, "plan-slice");
    } else if (choice === "discuss") {
      const sqAvail = getStructuredQuestionsAvailability(pi, ctx);
      await dispatchWorkflow(pi, await buildDiscussSlicePrompt(milestoneId, sliceId, sliceTitle, basePath, { rediscuss: hasContext, structuredQuestionsAvailable: sqAvail }), "tac-run", ctx, "discuss-slice");
