...
import { tacRoot, resolveSliceFile } from "./paths.js";
...
function findLastCompletedSlice(basePath: string, milestoneId: string): string | null {
  ...
  const slicesDir = join(tacRoot(basePath), "milestones", milestoneId, "slices");
...
export async function handleAddTests(
  ...
  if (!targetId) {
    ctx.ui.notify(
      "No completed slices found. Specify a slice ID: /tac add-tests S03",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    pi.sendMessage(
      { customType: "tac-add-tests", content: prompt, display: false },
      { triggerTurn: true },
    );
  } catch (err) {
