...
// Claims are stored in SQLite (.tac/unit-claims.db) for atomic
...
// ─── SQLite Provider (mirrors tac-db.ts pattern) ─────────────────────────
...
function claimsDbPath(basePath: string): string {
  return join(basePath, ".tac", "unit-claims.db");
}
 ...
 * Creates .tac/ directory and unit-claims.db with the unit_claims table.
