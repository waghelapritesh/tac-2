...
export function autoEnableCmuxPreferences(): boolean {
  ...
  const existing = loadProjectTACPreferences();
...
function resolveProjectPreferencesWritePath(): string {
  return loadProjectTACPreferences()?.path ?? getProjectTACPreferencesPath();
}
...
async function ensureCmuxAvailableForEnable(ctx: ExtensionCommandContext): Promise<boolean> {
  ...
  ctx.ui.notify(
    "cmux not detected. Install it from https://cmux.com and run tac inside a cmux terminal.",
    "warning",
  );
  return false;
}
...
export async function handleCmux(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (trimmed === "off") {
    const effective = loadEffectiveTACPreferences()?.preferences;
    await writeProjectCmuxPreferences(ctx, (prefs) => {
  ...
  if (parts.length === 2 && ["notifications", "sidebar", "splits", "browser"].includes(parts[0]) && ["on", "off"].includes(parts[1])) {
    ...
    if (!enabled && feature === "sidebar") {
      const { clearCmuxSidebar } = await import("../cmux/index.js");
      clearCmuxSidebar(loadEffectiveTACPreferences()?.preferences);
    }
  ...
  ctx.ui.notify(
    "Usage: /tac cmux <status|on|off|notifications on|notifications off|sidebar on|sidebar off|splits on|splits off|browser on|browser off>",
    "info",
  );
}
