 ...
 * Throws TACError on dirty main / detection failures.
 */
export function mergeSliceToMain(
  ...
  try {
    ...
    const commitSha = nativeCommit(
      projectRoot,
      `tac: merge ${sliceId} of ${milestoneId} (slice-cadence)`,
    );
...
export function resquashMilestoneOnMain(
  ...
  try {
    ...
    const newSha = nativeCommit(
      projectRoot,
      `tac: complete milestone ${milestoneId} (${sliceCount} slices re-squashed)`,
      { allowEmpty: true },
    );
 ...
 * a raw preferences object (the shape loadEffectiveTACPreferences returns).
 */
