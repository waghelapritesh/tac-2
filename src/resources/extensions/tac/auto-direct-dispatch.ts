/**
 * Direct phase dispatch — handles manual /tac dispatch commands.
...
export async function dispatchDirectPhase(
  ...
  switch (normalized) {
    ...
    case "research-slice": {
      ...
      if (isSlice) {
        ...
        const requireDiscussion = loadEffectiveTACPreferences()?.preferences?.phases?.require_slice_discussion;
        if (requireDiscussion && !sliceContextFile) {
          ctx.ui.notify(
            `Slice ${sid} requires discussion before planning. Run /tac discuss to discuss this slice, then /tac auto to resume.`,
            "info",
          );
  ...
  pi.sendMessage(
    { customType: "tac-dispatch", content: prompt, display: false },
    { triggerTurn: true },
  );
}
