// TAC Extension — Persistent Notification Store
// Captures all ctx.ui.notify() calls and workflow-logger warnings to
// .tac/notifications.jsonl so they survive context resets and session restarts.
...
function _readEntriesFromDisk(basePath: string): NotificationEntry[] {
  const filePath = join(basePath, ".tac", FILENAME);
...
function _withLock<T>(basePath: string, fn: () => T): T {
  const lockPath = join(basePath, ".tac", LOCKFILE);
