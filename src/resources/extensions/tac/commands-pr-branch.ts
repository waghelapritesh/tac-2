/**
 * TAC Command — /tac pr-branch
 *
 * Creates a clean PR branch by cherry-picking commits while stripping
 * any changes to .tac/, .planning/, and PLAN.md paths. Useful for
...
const EXCLUDED_PATHS = [".tac", ".planning", "PLAN.md"] as const;
...
export async function handlePrBranch(
  ...
  if (commits.length === 0) {
    ctx.ui.notify("No code-only commits found (all commits only touch .tac/ files).", "info");
    return;
  }

  if (dryRun) {
    const lines = [`Would create PR branch with ${commits.length} commits (filtering .tac/ paths):\n`];
  ...
  try {
    ...
    ctx.ui.notify(
      `Created ${prBranch} with ${picked} commits${skippedMsg} (no .tac/ artifacts).\nSwitch back: git checkout ${currentBranch}`,
      "success",
    );
  } catch (err) {
