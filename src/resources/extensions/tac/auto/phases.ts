...
import { importExtensionModule, type ExtensionAPI, type ExtensionContext } from "@tac/pi-coding-agent";
...
import { setCurrentPhase, clearCurrentPhase } from "../../shared/tac-phase-state.js";
...
export function _resolveReportBasePath(s: Pick<AutoSession, "originalBasePath" | "basePath">): string {
  // Strip /.tac/worktrees/ suffix when basePath is itself a worktree path and
  // originalBasePath is falsy — prevents reports landing in the worktree (#3729).
  const resolved = s.originalBasePath || s.basePath;
  const markerIdx = resolved.indexOf("/.tac/worktrees/");
...
async function generateMilestoneReport(
  ...
  const tacVersion = process.env.TAC_VERSION ?? "0.0.0";
  ...
  ctx.ui.notify(
    `Report saved: .tac/reports/${basename(outPath)} — open index.html to browse progression.`,
    "info",
  );
}
...
export async function runPreDispatch(
  ...
  try {
    ...
    if (!healthGate.proceed) {
      ...
      ctx.ui.notify(
        healthGate.reason || "Pre-dispatch health check failed — run /tac doctor for details.",
        "error",
      );
      await deps.pauseAuto(ctx, pi);
  ...
  if (prefs?.uok?.plan_v2?.enabled && shouldRunPlanV2Gate(state.phase)) {
    ...
    if (!compiled.ok) {
      ...
      if (isMissingFinalizedContextResult(compiled)) {
        ...
        ctx.ui.notify(`Plan gate failed-closed: ${reason}\n\nIf this keeps happening, try: /tac doctor heal`, "error");
        await deps.pauseAuto(ctx, pi);
  ...
  if (
    ...
    mid &&
    !process.env.TAC_PARALLEL_WORKER &&
    isDbAvailable()
  ) {
    try {
  ...
  if (mid && s.currentMilestoneId && mid !== s.currentMilestoneId) {
    ...
    if (vizPrefs?.auto_visualize) {
      ctx.ui.notify("Run /tac visualize to see progress overview.", "info");
    }
    ...
    try {
      ...
      if (mergeErr instanceof MergeConflictError) {
        ...
        ctx.ui.notify(
          `Merge conflict: ${mergeErr.conflictedFiles.join(", ")}. Resolve conflicts manually and run /tac auto to resume.`,
          "error",
        );
      ...
      ctx.ui.notify(
        `Merge failed: ${mergeErr instanceof Error ? mergeErr.message : String(mergeErr)}. Resolve and run /tac auto to resume.`,
        "error",
      );
    ...
    try {
      const completedKeysPath = join(tacRoot(s.basePath), "completed-units.json");
  ...
  if (!mid) {
    ...
    if (incomplete.length === 0 && state.registry.length > 0) {
      ...
      // Pause instead of hard-stop so the session is resumable with `/tac auto`.
      ...
      await deps.pauseAuto(ctx, pi);
      ctx.ui.notify(`${blockerMsg}. Fix and run /tac auto to resume.`, "warning");
      deps.sendDesktopNotification("TAC", blockerMsg, "warning", "attention", basename(s.originalBasePath || s.basePath));
...
export async function runDispatch(
  ...
  if (dispatchResult.action === "stop") {
    ...
    // `/tac auto`. Error/info-level stops remain hard stops for infrastructure
    // failures and terminal conditions respectively.
    // See: https://github.com/waghelapritesh/tac-2/issues/2474
...
export async function runGuards(
  ...
  try {
    ...
    if (stopCaptures.length > 0) {
      ...
      deps.sendDesktopNotification(
        "TAC", label, "warning", "stop-directive",
        basename(s.originalBasePath || s.basePath),
      );
  ...
  if (budgetCeiling !== undefined && budgetCeiling > 0) {
    ...
    if (process.env.TAC_PARALLEL_WORKER && s.autoStartTime && Array.isArray(costUnits)) {
    ...
    if (threshold) {
      ...
      if (threshold.pct === 100 && effectiveAction !== "none") {
        ...
        if (effectiveAction === "halt") {
          deps.sendDesktopNotification("TAC", msg, "error", "budget", basename(s.originalBasePath || s.basePath));
        ...
        if (effectiveAction === "pause") {
          ctx.ui.notify(
            `${msg} Pausing auto-mode — /tac auto to override and continue.`,
            "warning",
          );
          deps.sendDesktopNotification("TAC", msg, "warning", "budget", basename(s.originalBasePath || s.basePath));
  ...
  if (contextThreshold > 0 && s.cmdCtx) {
    ...
    if (
      ...
      ctx.ui.notify(
        `${msg} Run /tac auto to continue (will start fresh session).`,
        "warning",
      );
      deps.sendDesktopNotification(
        "TAC",
...
export async function runUnitPhase(
  ...
  ctx.ui.setStatus("tac-auto", "auto");
  if (mid)
  ...
  if (deps.isDbAvailable()) {
    try {
      const { inlineTacRootFile } = await importExtensionModule<typeof import("../auto-prompts.js")>(import.meta.url, "../auto-prompts.js");
      const [decisionsContent, requirementsContent, projectContent] =
        await Promise.all([
          inlineTacRootFile(s.basePath, "decisions.md", "Decisions"),
          inlineTacRootFile(s.basePath, "requirements.md", "Requirements"),
          inlineTacRootFile(s.basePath, "project.md", "Project"),
        ]);
