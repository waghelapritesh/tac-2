...
function stuckStatePath(basePath: string): string {
  return join(tacRoot(basePath), "runtime", "stuck-state.json");
}
...
function customVerifyRetryStateDir(s: Pick<AutoSession, "activeRunDir" | "basePath">): string {
  return s.activeRunDir ? join(s.activeRunDir, "runtime") : join(tacRoot(s.basePath), "runtime");
}
...
export async function autoLoop(
  ...
  while (s.active) {
    ...
    if (iteration % MEMORY_CHECK_INTERVAL === 0) {
      ...
      if (mem.pressured) {
        ...
        await deps.stopAuto(
          ...
          `Resume with /tac auto to continue from where you left off.`,
        );
    ...
    try {
      ...
      const prefs = deps.loadEffectiveTACPreferences()?.preferences;
      ...
      //
      // TAC_ENGINE_BYPASS=1 skips the engine layer entirely — falls through
      // to the dev path below.
      if (s.activeEngineId != null && s.activeEngineId !== "dev" && !sidecarItem && process.env.TAC_ENGINE_BYPASS !== "1") {
        ...
        const tacState = await deps.deriveState(s.basePath);
