/**
 * TAC Worktree Command — /worktree
 *
 * Create, list, merge, and remove git worktrees under .tac/worktrees/.
 *
 * Usage:
...
import { tacRoot, milestonesDir } from "./paths.js";
import {
  createWorktree,
  listWorktrees,
  removeWorktree,
...
export function getActiveWorktreeName(): string | null {
  ...
  const wtDir = join(originalCwd, ".tac", "worktrees");
...
/**
 * Check if the worktree has existing TAC milestones that would
...
/**
 * Clear TAC planning artifacts so auto-mode starts fresh with the discuss flow.
 * Keeps the .tac/ directory structure intact but removes milestones and root planning files.
 */
function clearTACPlans(wtPath: string): void {
  const mDir = milestonesDir(wtPath);
  if (existsSync(mDir)) {
  ...
  try {
    ...
    if (hasExistingMilestones(info.path)) {
      ...
      const keepExisting = await showConfirm(ctx, {
        ...
        message: [
          `This worktree inherited existing TAC milestones from the main branch.`,
          ``,
          `  Continue — keep milestones and pick up where main left off`,
          `  Start fresh — clear milestones so /tac auto starts a new project`,
        ].join("\n"),
        confirmLabel: "Continue",
        declineLabel: "Start fresh",
      });
      if (!keepExisting) {
        clearTACPlans(info.path);
        clearedPlans = true;
      }
    }
    ...
    const freshNote = clearedPlans
      ? `  ${CLR.ok("✓")} Cleared milestones — ${CLR.hint("/tac auto")} will start fresh.`
      : "";
    ctx.ui.notify(
      [
...
async function handleList(
  ...
  try {
    ...
    if (worktrees.length === 0) {
      ctx.ui.notify("No TAC worktrees found. Create one with /worktree <name>.", "info");
      return;
    }
    ...
    const cwd = process.cwd();
    const lines = [CLR.header("TAC Worktrees"), ""];
...
async function handleMerge(
  ...
  try {
    ...
    // Gather merge context — full repo diff, not just .tac/
    const diffSummary = diffWorktreeAll(basePath, name);
    const numstat = diffWorktreeNumstat(basePath, name);
    const tacDiff = getWorktreeTACDiff(basePath, name);
    ...
    // Split files into code vs TAC for the preview
    const isTAC = (f: string) => f.startsWith(".tac/");
    const codeChanges = diffSummary.added.filter(f => !isTAC(f)).length
      + diffSummary.modified.filter(f => !isTAC(f)).length
      + diffSummary.removed.filter(f => !isTAC(f)).length;
    const tacChanges = diffSummary.added.filter(isTAC).length
      + diffSummary.modified.filter(isTAC).length
      + diffSummary.removed.filter(isTAC).length;
    ...
    const previewLines = [
      ...
      "",
      `  ${totalChanges} file${totalChanges === 1 ? "" : "s"} changed, ${CLR.ok(`+${totalAdded}`)} ${RED}-${totalRemoved}${RESET} lines ${CLR.muted(`(${codeChanges} code, ${tacChanges} TAC)`)}`,
    ];
    ...
    const wtDbPath = join(worktreePath(basePath, name), ".tac", "tac.db");
    const mainDbPath = join(basePath, ".tac", "tac.db");
    if (existsSync(wtDbPath) && existsSync(mainDbPath)) {
      try {
        const { reconcileWorktreeDb } = await import("./tac-db.js");
    ...
    const prompt = loadPrompt("worktree-merge", {
      ...
      tacDiff: tacDiff || "(no TAC artifact changes)",
    ...
    ctx.ui.notify(
      `${CLR.ok("✓")} Merge helper started for ${CLR.name(name)} ${CLR.muted(`(${codeChanges} code + ${tacChanges} TAC artifact change${totalChanges === 1 ? "" : "s"})`)}`,
      "info",
    );
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
