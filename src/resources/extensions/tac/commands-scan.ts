/**
 * TAC Command — /tac scan
 *
 * Rapid codebase assessment — lightweight alternative to /tac map-codebase.
 * Spawns one focused AI analysis pass and writes structured documents to
 * .tac/codebase/ for use by planning and execution phases.
 *
 * Usage:
 *   /tac scan                   — tech+arch focus (default)
 *   /tac scan --focus tech      — technology stack + integrations only
 *   /tac scan --focus arch      — architecture + structure only
 *   /tac scan --focus quality   — conventions + testing patterns only
 *   /tac scan --focus concerns  — technical debt + concerns only
 *   /tac scan --focus tech+arch — explicit default (same as no flag)
 */
...
export async function handleScan(
  ...
  const outputDir = join(basePath, ".tac", "codebase");
  ...
  try {
    ...
    pi.sendMessage(
      { customType: "tac-scan", content: prompt, display: false },
      { triggerTurn: true },
    );
  } catch (err) {
