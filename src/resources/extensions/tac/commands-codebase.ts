/**
 * TAC Command — /tac codebase
 *
 * Generate and manage the codebase map (.tac/CODEBASE.md).
...
const USAGE =
  "Usage: /tac codebase [generate|update|stats]\n\n" +
  ...
  "TAC also refreshes CODEBASE.md automatically before prompt injection and after completed units when tracked files change.\n\n" +
...
export async function handleCodebase(
  ...
  switch (sub) {
    ...
    case "update": {
      ...
      if (!existing) {
        ctx.ui.notify(
          "No codebase map found. Run /tac codebase generate to create one.",
          "warning",
        );
        return;
      }
...
function showStats(basePath: string, ctx: ExtensionCommandContext): void {
  ...
  if (!stats.exists) {
    ctx.ui.notify("No codebase map found. Run /tac codebase generate to create one.", "info");
    return;
  }
  ...
  ctx.ui.notify(
    ...
    (stats.undescribedCount > 0
      ? `Tip: Auto-refresh keeps the cache current, but /tac codebase update forces an immediate refresh.`
...
function resolveCodebaseOptions(args: string, ctx: ExtensionCommandContext): CodebaseMapOptions | false {
  ...
  const prefs = loadEffectiveTACPreferences()?.preferences?.codebase;
