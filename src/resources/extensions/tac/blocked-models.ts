// TAC — Persistent per-project blocklist of provider/model pairs that the
// provider has rejected at request time for account entitlement reasons.
//
// Lives at `.tac/runtime/blocked-models.json` so the block survives /tac auto
...
function blockedModelsPath(basePath: string): string {
  return join(tacRoot(basePath), "runtime", "blocked-models.json");
}
