// TAC Memory Extractor — Background LLM extraction from activity logs
//
...
import type { Api, AssistantMessage, Model } from '@tac/pi-ai';
import {
  getActiveMemories,
  isUnitProcessed,
  markUnitProcessed,
...
export function buildMemoryLLMCall(ctx: ExtensionContext): LLMCallFn | null {
  try {
    ...
    // See: https://github.com/waghelapritesh/tac-2/issues/2959
    ...
    const llmCall = async (system: string, user: string): Promise<string> => {
      const { completeSimple } = await import('@tac/pi-ai');
