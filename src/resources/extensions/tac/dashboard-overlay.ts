...
export class TACDashboardOverlay {
  ...
  private buildContentLines(width: number): string[] {
    ...
    const title = th.fg("accent", th.bold("TAC Dashboard"));
    ...
    if (this.dashData.currentUnit) {
      ...
      lines.push(row(th.fg("dim", "/tac auto to resume")));
      lines.push(blank());
    } else if (isRemote) {
      ...
      lines.push(row(th.fg("dim", "No unit running · /tac auto to start")));
      lines.push(blank());
    }
    ...
    if (ledger && ledger.units.length > 0) {
      ...
      if (this.milestoneData) {
        ...
        const overlayPrefs = loadEffectiveTACPreferences()?.preferences;
