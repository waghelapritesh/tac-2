/**
 * Type definitions, constants, and configuration shapes for TAC preferences.
 *
...
/**
 * Opt-in tool-output sandboxing for sub-sessions. When enabled, the tac_exec
...
/**
 * Resolve whether context-mode features (tac_exec sandbox + compaction
...
export const MODE_DEFAULTS: Record<WorkflowMode, Partial<TACPreferences>> = {
  solo: {
    git: {
      auto_push: true,
      push_branches: false,
...
/** All recognized top-level keys in TACPreferences. Used to detect typos / stale config. */
 ...
 */
export interface TACPhaseModelConfig {
 ...
 * Kept for backward compatibility; will be migrated to TACModelConfigV2 on load.
 */
export interface TACModelConfig {
  research?: string;
  planning?: string;
  discuss?: string;
  execution?: string;
 ...
 */
export interface TACModelConfigV2 {
  research?: string | TACPhaseModelConfig;
  planning?: string | TACPhaseModelConfig;
  discuss?: string | TACPhaseModelConfig;
  execution?: string | TACPhaseModelConfig;
  execution_simple?: string | TACPhaseModelConfig;
  completion?: string | TACPhaseModelConfig;
  validation?: string | TACPhaseModelConfig;
  subagent?: string | TACPhaseModelConfig;
}
...
/** Configuration for the codebase map generator (/tac codebase). */
...
export interface TACPreferences {
  version?: number;
  mode?: WorkflowMode;
  always_use_skills?: string[];
  prefer_skills?: string[];
  avoid_skills?: string[];
  skill_rules?: TACSkillRule[];
  custom_instructions?: string[];
  models?: TACModelConfig | TACModelConfigV2;
  ...
  /**
   * Tool-output sandboxing via tac_exec. Keeps sub-session context windows
  ...
  /** GitHub sync configuration. Opt-in: syncs TAC events to GitHub Issues, Milestones, and PRs. */
   ...
   * auto-commit a safety snapshot tagged with `[tac safety]`. Default: 30.
   * Set to 0 to disable.
   */
  ...
  /**
   * Language preference for TAC responses. Accepts any language name or code
   * (e.g. "Chinese", "zh", "German", "de", "日本語"). Persists across /clear.
   */
  language?: string;
}

export interface LoadedTACPreferences {
  path: string;
