/**
 * TAC Codebase Map Generator
 *
 * Produces .tac/CODEBASE.md — a structural table of contents for the project.
...
const CODEBASE_METADATA_PREFIX = "<!-- tac:codebase-meta ";
 ...
 * Also scans <!-- tac:collapsed-descriptions --> comment blocks to preserve
...
export function parseCodebaseMap(content: string): Map<string, string> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (line.trimStart().startsWith("<!-- tac:collapsed-descriptions")) {
  ...
  for (const group of groups) {
    ...
    if (group.collapsed) {
      ...
      if (descLines.length > 0) {
        lines.push("<!-- tac:collapsed-descriptions");
...
export function updateCodebaseMap(
  ...
  const codebasePath = join(tacRoot(basePath), "CODEBASE.md");
  const resolved = resolveGeneratorOptions(options);
...
/**
 * Write CODEBASE.md to .tac/ directory.
 */
