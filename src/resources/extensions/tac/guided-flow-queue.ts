/**
 * TAC Queue Management — showQueue, reorder, add, and context builder.
 *
...
import {
  tacRoot, resolveMilestoneFile, resolveSliceFile,
  resolveTacRootFile, relTacRootFile, relSliceFile,
} from "./paths.js";
...
export async function showQueue(
  ...
  if (milestoneIds.length === 0) {
    ctx.ui.notify("No milestones exist yet. Run /tac to create the first one.", "warning");
    return;
  }
  ...
  if (pendingMilestones.length > 1) {
    ...
    const choice = await showNextAction(ctx, {
      ...
      notYetMessage: "Run /tac queue when ready.",
    });
...
export async function handleQueueReorder(
  ...
  // Commit the change
  const filesToAdd = [".tac/QUEUE-ORDER.json", ".tac/PROJECT.md"];
  for (const r of result.depsToRemove) {
    filesToAdd.push(`.tac/milestones/${r.milestone}/${r.milestone}-CONTEXT.md`);
  }
  try {
...
export async function showQueueAdd(
  ...
  // Note: the LLM will use the tac_milestone_generate_id tool to get IDs
  ...
  const preamble = [
    `Queuing new work onto an existing TAC project.`,
    activePart,
...
function syncProjectMdSequence(
  ...
  // another worktree's working tree during a concurrent /tac auto merge.
