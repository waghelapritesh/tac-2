...
import { truncateToWidth, visibleWidth } from "@tac/pi-tui";
...
function peekNext(unitType: string, state: TACState): string {
 ...
 */
export function describeNextUnit(state: TACState): { label: string; description: string } {
  const sid = state.activeSlice?.id;
...
function resolveWidgetModePreferencePath(
  projectPath = getProjectTACPreferencesPath(),
  globalPath = getGlobalTACPreferencesPath(),
): string {
  ...
  return getGlobalTACPreferencesPath();
}
...
function ensureWidgetModeLoaded(projectPath?: string, globalPath?: string): void {
  ...
  try {
    const resolvedProjectPath = projectPath ?? getProjectTACPreferencesPath();
    const resolvedGlobalPath = globalPath ?? getGlobalTACPreferencesPath();
    ...
    widgetModePreferencePath = getGlobalTACPreferencesPath();
  }
}
...
export function updateProgressWidget(
  ...
  ctx.ui.setWidget("tac-progress", (tui, theme) => {
    ...
    return {
      render(width: number): string[] {
        ...
        const headerLeft = `${pad}${dot} ${theme.fg("accent", theme.bold("TAC"))}  ${theme.fg("success", modeTag)}${healthStr}`;
