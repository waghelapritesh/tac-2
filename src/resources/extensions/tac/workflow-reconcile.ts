  ...
  if (tasks.length > 0) {
    ...
    if (incompleteTasks.length > 0) {
      process.stderr.write(
        `[tac] reconcile: skipping complete_slice replay for ${sliceId} — ` +
 ...
 * tac-db function.  This replaces the old engine.replayAll() pattern with
 * direct DB calls.
 */
function replayEvents(events: WorkflowEvent[]): void {
  ...
  for (const event of events) {
    ...
    switch (cmd) {
      ...
      case "plan_milestone": {
        // Replay milestone creation — uses INSERT OR IGNORE (tac-db's insertMilestone is safe)
...
/**
 * Write a human-readable CONFLICTS.md to basePath/.tac/CONFLICTS.md.
...
export function writeConflictsFile(
  ...
  const lines: string[] = [
    ...
    `Run \`tac resolve-conflict\` to resolve each conflict.`,
    "",
  ];
  ...
  conflicts.forEach((conflict, idx) => {
    ...
    lines.push("");
    lines.push(`**Resolve with:** \`tac resolve-conflict --entity ${conflict.entityType}:${conflict.entityId} --pick [main|worktree]\``);
    lines.push("");
  });
...
function _reconcileWorktreeLogsInner(
  ...
  const mainLogPath = join(mainBasePath, ".tac", "event-log.jsonl");
  const wtLogPath = join(worktreeBasePath, ".tac", "event-log.jsonl");
  ...
  if (conflicts.length > 0) {
    ...
    logError("reconcile", `${conflicts.length} conflict(s) detected on ${conflictSummary}${truncated}. Details: .tac/CONFLICTS.md`, { count: String(conflicts.length), path: join(mainBasePath, ".tac", "CONFLICTS.md") });
    return { autoMerged: 0, conflicts };
  }
  ...
  mkdirSync(join(mainBasePath, ".tac"), { recursive: true });
  atomicWriteSync(join(mainBasePath, ".tac", "event-log.jsonl"), logContent);

  // Step 8: Replay into DB (wrapped in a transaction by replayEvents)
  openDatabase(join(mainBasePath, ".tac", "tac.db"));
  replayEvents(merged);
...
export function listConflicts(basePath: string): ConflictEntry[] {
  const conflictsPath = join(basePath, ".tac", "CONFLICTS.md");
...
export function resolveConflict(
  ...
  const mainLogPath = join(basePath, ".tac", "event-log.jsonl");
  const wtLogPath = join(worktreeBasePath, ".tac", "event-log.jsonl");
  const mainEvents = readEvents(mainLogPath);
  const wtEvents = readEvents(wtLogPath);
  const forkPoint = findForkPoint(mainEvents, wtEvents);
  ...
  openDatabase(join(basePath, ".tac", "tac.db"));
