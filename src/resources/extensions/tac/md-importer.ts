// TAC Markdown Importer
// Parses DECISIONS.md, REQUIREMENTS.md, and hierarchy artifacts from a .tac/ tree,
 ...
 */
function importDecisions(tacDir: string): number {
  const filePath = resolveTacRootFile(tacDir, 'DECISIONS');
 ...
 */
function importRequirements(tacDir: string): number {
  const filePath = resolveTacRootFile(tacDir, 'REQUIREMENTS');
  if (!existsSync(filePath)) return 0;
...
/**
 * Import hierarchy artifacts (roadmaps, plans, summaries, etc.) from the .tac/ tree.
 * Walks milestones → slices → tasks directories.
 */
function importHierarchyArtifacts(tacDir: string): number {
  let count = 0;
  const tacPath = tacRoot(tacDir);

  // Root-level artifacts: PROJECT.md, QUEUE.md
  const rootFiles = ['PROJECT.md', 'QUEUE.md', 'SECRETS-MANIFEST.md'];
  for (const fileName of rootFiles) {
    const filePath = join(tacPath, fileName);
  ...
  // Walk milestones
  const milestoneIds = findMilestoneIds(tacDir);
  const msDir = milestonesDir(tacDir);
...
/**
 * Walk .tac/milestones/ dirs, parse roadmaps and plans, and populate
          ...
          if (tDir) {
            ...
            if (!existsSync(summaryFile)) {
              ...
              process.stderr.write(
                `tac-migrate: ${milestoneId}/${sliceEntry.id}/${taskEntry.id} marked done but missing summary — importing as pending\n`,
              );
            }
          }
        }
      ...
      if (!sliceEntry.done) {
        ...
        if (allTasksDone && hasSliceSummary) {
          if (_getAdapter()) {
            ...
            process.stderr.write(
              `tac-migrate: ${milestoneId}/${sliceEntry.id} all tasks + slice summary complete — upgrading slice to complete\n`,
            );
          }
        }
      }
...
/**
 * Import all markdown artifacts from a .tac/ directory into the database.
 ...
 */
export function migrateFromMarkdown(tacDir: string): {
  decisions: number;
  requirements: number;
  artifacts: number;
  hierarchy: { milestones: number; slices: number; tasks: number };
} {
  const dbPath = join(tacRoot(tacDir), 'tac.db');
  ...
  process.stderr.write(
    `tac-migrate: imported ${decisions} decisions, ${requirements} requirements, ${artifacts} artifacts, ${hierarchy.milestones}M/${hierarchy.slices}S/${hierarchy.tasks}T hierarchy\n`,
  );
