 ...
 * that controls how `/tac workflow <name>` dispatches it:
 *
 ...
 *   auto-milestone  — hooks into /tac auto pipeline (full-project only)
 *
...
function resolveBundledDir(): string {
  ...
  const agentTacDir = join(tacHome, "agent", "extensions", "tac", "workflow-templates");
  if (existsSync(agentTacDir)) return agentTacDir;
  return local;
}

function globalPluginsDir(): string {
  const tacHome = process.env.TAC_HOME || join(homedir(), ".tac");
  return join(tacHome, "workflows");
}

function projectPluginsDir(basePath: string): string {
  return join(basePath, ".tac", "workflows");
}

function legacyDefsDir(basePath: string): string {
  return join(basePath, ".tac", "workflow-defs");
}
 ...
 *
 * The legacy `.tac/workflow-defs/*.yaml` directory is also scanned as a
...
export function listPluginsFormatted(basePath: string): string {
  const plugins = discoverPlugins(basePath);
  if (plugins.size === 0) {
    return "No workflow plugins found.\n\nRun /tac workflow new to author one.";
  }
  ...
  lines.push("Usage:");
  lines.push("  /tac workflow <name>          Run a plugin directly");
  lines.push("  /tac workflow info <name>     Show plugin details");
  lines.push("  /tac workflow install <src>   Install a plugin from a URL");

  return lines.join("\n");
}

/**
 * Format a single plugin's metadata for `/tac workflow info <name>`.
 */
