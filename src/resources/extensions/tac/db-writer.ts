// TAC DB Writer — Markdown generators + DB-first write helpers
//
...
import { resolveTacRootFile } from './paths.js';
import { saveFile } from './files.js';
import { TACError, TAC_STALE_STATE, TAC_IO_ERROR } from './errors.js';
...
function nextDecisionIdSync(adapter: ReturnType<typeof import('./tac-db.js')._getAdapter>): string {
  if (!adapter) return 'D001';
  const row = adapter
    .prepare('SELECT MAX(CAST(SUBSTR(id, 2) AS INTEGER)) as max_num FROM decisions')
    .get();
...
export async function saveRequirementToDb(
  ...
  try {
    ...
    const id = db.transaction(() => {
      const adapter = db._getAdapter();
      if (!adapter) throw new TACError(TAC_STALE_STATE, "tac-db: No database open");
    ...
    const filePath = resolveTacRootFile(basePath, 'REQUIREMENTS');
    try {
...
export async function saveDecisionToDb(
  ...
  try {
    ...
    const filePath = resolveTacRootFile(basePath, 'DECISIONS');
    ...
    // persisted via the legacy tac_save_decision path. Without this, prompts
    // that still call tac_save_decision (discuss.md, plan-milestone.md,
...
export async function updateRequirementInDb(
  ...
  try {
    ...
    // imported into the database — making tac_requirement_update always fail
    // with "not_found" at milestone completion.
    if (!existing) {
      const reqFilePath = resolveTacRootFile(basePath, 'REQUIREMENTS');
      try {
 ...
 * The path is relative to .tac/ (e.g. "milestones/M001/slices/S06/tasks/T01-SUMMARY.md").
 * The full file path is computed as basePath + '.tac/' + path.
 */
export async function saveArtifactToDb(
  ...
  try {
    ...
    const tacDir = resolve(basePath, '.tac');
    const fullPath = resolve(basePath, '.tac', opts.path);
    if (!fullPath.startsWith(tacDir)) {
      throw new TACError(TAC_IO_ERROR, `saveArtifactToDb: path escapes .tac/ directory: ${opts.path}`);
    }
