/**
 * Validation logic for TAC preferences.
 *
 * Pure validation -- no filesystem access, no loading, no merging.
 * Accepts a raw TACPreferences object and returns a sanitized copy
...
export function validatePreferences(preferences: TACPreferences): {
  ...
  const validated: TACPreferences = {};

  // ─── Unknown Key Detection ──────────────────────────────────────────
  // Common key migration hints for pi-level settings that don't map to TAC prefs
  ...
  if (preferences.skill_rules) {
    ...
    for (const rule of preferences.skill_rules) {
      ...
      const validatedRule: TACSkillRule = { when };
      for (const action of SKILL_ACTIONS) {
        const values = normalizeStringArray((rule as unknown as Record<string, unknown>)[action]);
        if (values.length > 0) {
          validatedRule[action as keyof TACSkillRule] = values as never;
        }
      }
  ...
  if (preferences.uok !== undefined) {
    if (typeof preferences.uok === "object" && preferences.uok !== null) {
      ...
      const valid: NonNullable<TACPreferences["uok"]> = {};
      ...
      if (raw.gitops !== undefined) {
        if (typeof raw.gitops !== "object" || raw.gitops === null) {
          ...
          const parsed: NonNullable<NonNullable<TACPreferences["uok"]>["gitops"]> = {};
  ...
  if (preferences.search_provider !== undefined) {
    ...
    if (typeof preferences.search_provider === "string" && validSearchProviders.has(preferences.search_provider)) {
      validated.search_provider = preferences.search_provider as TACPreferences["search_provider"];
    } else {
  ...
  if (preferences.cmux !== undefined) {
    if (preferences.cmux && typeof preferences.cmux === "object") {
      ...
      const validatedCmux: NonNullable<TACPreferences["cmux"]> = {};
  ...
  if (preferences.slice_parallel !== undefined) {
    if (typeof preferences.slice_parallel === "object" && preferences.slice_parallel !== null) {
      ...
      const validSp: NonNullable<TACPreferences["slice_parallel"]> = {};
  ...
  if (preferences.git && typeof preferences.git === "object") {
    ...
    if (g.commit_docs !== undefined) {
      warnings.push("git.commit_docs is deprecated — .tac/ is managed externally and always gitignored. Remove this setting.");
    }
  ...
  if (preferences.context_selection !== undefined) {
    ...
    if (typeof preferences.context_selection === "string" && validModes.has(preferences.context_selection)) {
      validated.context_selection = preferences.context_selection as TACPreferences["context_selection"];
    } else {
  ...
  if (preferences.discuss_depth !== undefined) {
    ...
    if (typeof preferences.discuss_depth === "string" && validDepths.has(preferences.discuss_depth)) {
      validated.discuss_depth = preferences.discuss_depth as TACPreferences["discuss_depth"];
    } else {
