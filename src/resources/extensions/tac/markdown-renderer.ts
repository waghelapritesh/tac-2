// TAC Markdown Renderer — DB → Markdown file generation
//
...
import type { MilestoneRow, SliceRow, TaskRow, ArtifactRow } from "./tac-db.js";
...
/**
 * Convert an absolute file path to a .tac-relative artifact path.
 * E.g. "/project/.tac/milestones/M001/M001-ROADMAP.md" → "milestones/M001/M001-ROADMAP.md"
 */
...
export async function renderPlanFromDb(
  ...
  const slicePath = resolveSlicePath(basePath, milestoneId, sliceId)
    ?? join(tacRoot(basePath), "milestones", milestoneId, "slices", sliceId);
...
export async function renderTaskPlanFromDb(
  ...
  const tasksDir = resolveTasksDir(basePath, milestoneId, sliceId)
    ?? join(tacRoot(basePath), "milestones", milestoneId, "slices", sliceId, "tasks");
...
export async function renderRoadmapFromDb(
  ...
  const absPath = resolveMilestoneFile(basePath, milestoneId, "ROADMAP") ??
    join(tacRoot(basePath), "milestones", milestoneId, `${milestoneId}-ROADMAP.md`);
  const artifactPath = toArtifactPath(absPath, basePath);
