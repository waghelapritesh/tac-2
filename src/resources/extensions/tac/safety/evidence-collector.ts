 ...
 *
 * Evidence is persisted to .tac/safety/evidence-<mid>-<sid>-<tid>.json so it
 ...
 * Lives under .tac/safety/ which is gitignored and session-scoped.
 */
function evidencePath(basePath: string, milestoneId: string, sliceId: string, taskId: string): string {
  return join(basePath, ".tac", "safety", `evidence-${milestoneId}-${sliceId}-${taskId}.json`);
}
