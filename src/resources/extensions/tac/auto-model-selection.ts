...
import type { Api, Model } from "@tac/pi-ai";
import { getProviderCapabilities } from "@tac/pi-ai";
...
// at auto start AND auto stop so that a second `/tac auto` run on the same
...
export async function selectAndApplyModel(
  ...
  isAutoMode = true,
  /** Explicit /tac model pin captured at bootstrap for long-running auto loops. */
  ...
  if (modelConfig) {
    ...
    for (const modelId of modelsToTry) {
      ...
      // (issue #4513).  The block is persisted in .tac/runtime/blocked-models.json
      // so it survives /tac auto restarts — without this, the same dead model
