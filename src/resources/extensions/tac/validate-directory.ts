/**
 * TAC Directory Validation — Safeguards against running in dangerous directories.
 *
 * Prevents TAC from creating .tac/ structures in system paths, home directories,
...
export interface DirectoryValidationResult {
  /** Whether the directory is safe for TAC operations */
  safe: boolean;
...
/** Paths where TAC must never create .tac/ — no override possible. */
...
/**
 * Validate whether a directory is safe for TAC to operate in.
 *
 * Checks in order:
...
export function validateDirectory(dirPath: string): DirectoryValidationResult {
  ...
  if (platform() === "win32" ? isBlockedWindowsPath(normalized) : blockedPaths.has(normalized)) {
    return {
      safe: false,
      severity: "blocked",
      reason: `Refusing to run in system directory: ${normalized}. TAC must be run inside a project directory.`,
    };
  }
  ...
  if (normalized === resolvedHome) {
    return {
      safe: false,
      severity: "blocked",
      reason: `Refusing to run in your home directory (${normalized}). TAC must be run inside a project directory, not $HOME.`,
    };
  }
  ...
  try {
    ...
    if (entries.length > 200) {
      return {
        safe: false,
        severity: "warning",
        reason: `This directory has ${entries.length} entries, which suggests it may not be a project directory. Are you sure you want to initialize TAC here?`,
      };
    }
  } catch {
...
/**
 * Assert that a directory is safe for TAC operations.
