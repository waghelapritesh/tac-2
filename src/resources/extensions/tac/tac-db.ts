// TAC Database Abstraction Layer
...
// the engine database at `.tac/tac.db`. All other modules must call the
...
//
// The separate `.tac/unit-claims.db` managed by `unit-ownership.ts` is an
...
import { TACError, TAC_STALE_STATE } from "./errors.js";
...
function loadProvider(): void {
  ...
  const versionHint = nodeMajor < 22
    ? ` TAC requires Node >= 22.0.0 (current: v${process.versions.node}). Upgrade Node to fix this.`
    : "";
  process.stderr.write(
    `tac-db: No SQLite provider available (tried node:sqlite, better-sqlite3).${versionHint}\n`,
  );
}
...
function migrateSchema(db: DbAdapter): void {
  ...
  try {
    ...
    if (currentVersion < 21) {
      // ADR-013 Step 2: preserve structured fields (tac_save_decision's
  ...
  try {
    ...
    if (fileBacked && err instanceof Error && err.message?.includes("malformed")) {
      try {
        adapter.exec("VACUUM");
        initSchema(adapter, fileBacked);
        process.stderr.write("tac-db: recovered corrupt database via VACUUM\n");
      } catch (retryErr) {
...
/** Flush WAL into tac.db so `git add .tac/tac.db` stages current state — safe while DB is open. */
...
export function transaction<T>(fn: () => T): T {
  if (!currentDb) throw new TACError(TAC_STALE_STATE, "tac-db: No database open");
 ...
 *   1. Crash between `tac_plan_slice` write and the sketch flag flip.
 ...
 * `tac_plan_slice` tool (which also inserts tasks), so a "stale PLAN.md with
...
/** List tasks with active escalation artifacts across a milestone (for /tac escalate list). */
 ...
 * See: https://github.com/waghelapritesh/tac-2/issues/2694
 */
 ...
 * transaction. Used by `tac recover` to rebuild engine state from markdown.
 */
...
// All memory writes go through tac-db.ts so the single-writer invariant
...
export function insertMemoryRow(args: {
   ...
   * `content` field. Used to retain tac_save_decision-style fields (scope,
