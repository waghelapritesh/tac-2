 ...
 *
 * Each rule maps a TAC state to the unit type, unit ID, and prompt builder
...
import { isDbAvailable, getMilestoneSlices, getPendingGates, markAllGatesOmitted, getMilestone, updateMilestoneStatus } from "./tac-db.js";
...
function missingSliceStop(mid: string, phase: string): DispatchAction {
  return {
    action: "stop",
    reason: `${mid}: phase "${phase}" has no active slice — run /tac doctor.`,
    level: "error",
  };
}
...
// breaker to never trip — see https://github.com/waghelapritesh/tac-2/issues/2203
function rewriteCountPath(basePath: string): string {
  return join(tacRoot(basePath), "runtime", "rewrite-count.json");
}
...
export function setRewriteCount(basePath: string, count: number): void {
  ...
  mkdirSync(join(tacRoot(basePath), "runtime"), { recursive: true });
...
function uatCountPath(basePath: string, mid: string, sid: string): string {
  return join(tacRoot(basePath), "runtime", `uat-count-${mid}-${sid}.json`);
}
 ...
 *
 * @see https://github.com/waghelapritesh/tac-2/issues/2931
 */
...
export const DISPATCH_RULES: DispatchRule[] = [
  {
    ...
    match: async ({ state, mid }) => {
      ...
      return {
        ...
        reason:
          state.nextAction ||
          `${mid}: task escalation awaits user resolution. Run /tac escalate list to see pending items.`,
        level: "info",
      };
    },
  },
  ...
  },
  {
    ...
    // milestone is stuck until `/tac doctor heal` repairs it (and heal
    ...
    match: async ({ state, mid, midTitle, basePath, structuredQuestionsAvailable }) => {
      ...
      // as missing, and an auto worktree may fall back to TAC_PROJECT_ROOT.
  ...
  },
  {
    ...
    match: async ({ state, mid, basePath, prefs }) => {
      ...
      return {
        ...
        pauseAfterDispatch: !process.env.TAC_HEADLESS && uatType !== "artifact-driven" && uatType !== "browser-executable" && uatType !== "runtime-executable",
      };
    },
  },
  {
    ...
    match: async ({ mid, basePath, prefs }) => {
      ...
      for (const sliceId of completedSliceIds) {
        ...
        if (verdict && !isAcceptableUatVerdict(verdict, uatType)) {
          return {
            action: "stop" as const,
            reason: `UAT verdict for ${sliceId} is "${verdict}" — blocking progression until resolved.\nReview the UAT result and update the verdict to PASS, or re-run /tac auto after fixing.`,
            level: "warning" as const,
          };
        }
      }
  ...
  },
  {
    ...
    match: async ({ state, mid, basePath, prefs }) => {
      ...
      return {
        action: "stop" as const,
        reason: `Slice ${state.activeSlice.id} requires discussion before planning (require_slice_discussion is enabled). Run /tac discuss to discuss this slice, then /tac auto to resume.`,
        level: "info" as const,
      };
    },
  },
    ...
    match: async ({ state, mid, midTitle, basePath, prefs, sessionContextWindow, modelRegistry }) => {
      ...
      try {
        ...
        process.stderr.write(
          `tac-reactive: ${mid}/${sid} graph — tasks:${metrics.taskCount} edges:${metrics.edgeCount} ` +
  ...
  },
  {
    ...
    match: async ({ state, mid, midTitle, basePath }) => {
      ...
      if (missingSlices.length > 0) {
        return {
          action: "stop",
          reason: `Cannot complete milestone ${mid}: slices ${missingSlices.join(", ")} are missing SUMMARY files. Run /tac doctor to diagnose.`,
          level: "error",
        };
      }

      // Safety guard (#1703): verify the milestone produced implementation
      // artifacts (non-.tac/ files). A milestone with only plan files and
      ...
      if (artifactCheck === "absent") {
        return {
          action: "stop",
          reason: `Cannot complete milestone ${mid}: no implementation files found outside .tac/. The milestone has only plan files — actual code changes are required.`,
          level: "error",
        };
      }
...
export async function resolveDispatch(
  ...
  return {
    action: "stop",
    reason: `Unhandled phase "${ctx.state.phase}" — run /tac doctor to diagnose.`,
    level: "warning",
