...
import { isDbAvailable, insertAuditEvent } from "../tac-db.js";
import type { AuditEventEnvelope } from "./contracts.js";

function auditLogPath(basePath: string): string {
  return join(tacRoot(basePath), "audit", "events.jsonl");
}

function ensureAuditDir(basePath: string): void {
  mkdirSync(join(tacRoot(basePath), "audit"), { recursive: true });
}
