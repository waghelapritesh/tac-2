...
import type { TACState, Phase } from "../types.js";
import { tacRoot, resolveMilestoneFile, resolveSliceFile } from "../paths.js";
import { isDbAvailable, getMilestoneSlices, getSliceTasks, type SliceRow } from "../tac-db.js";
...
function graphOutputPath(basePath: string): string {
  return join(tacRoot(basePath), "runtime", "uok-plan-v2-graph.json");
}
...
function getArtifactLookupBases(basePath: string): string[] {
  ...
  const projectRoot = process.env.TAC_PROJECT_ROOT;
...
export function compileUnitGraphFromState(basePath: string, state: TACState): PlanV2CompileResult {
...
export function ensurePlanV2Graph(basePath: string, state: TACState): PlanV2CompileResult {
