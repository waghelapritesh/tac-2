 ...
 *
 * Subcommands:
 *   /tac logs              — List recent activity + debug logs
 *   /tac logs <N>          — Show summary of activity log #N
 *   /tac logs debug        — List debug log files
 *   /tac logs debug <N>    — Show debug log summary #N
 *   /tac logs tail [N]     — Show last N activity log entries (default 5)
 *   /tac logs clear        — Remove old activity and debug logs
 */
...
function activityDir(basePath: string): string {
  return join(tacRoot(basePath), "activity");
}

function debugDir(basePath: string): string {
  return join(tacRoot(basePath), "debug");
}
...
export async function handleLogs(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  // /tac logs <N> — show specific activity log
...
async function handleLogsList(basePath: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (activities.length > 0) {
    lines.push("Activity Logs (.tac/activity/):");
    ...
    lines.push("");
    lines.push("  View details: /tac logs <#>");
  }

  if (debugLogs.length > 0) {
    lines.push("");
    lines.push("Debug Logs (.tac/debug/):");
    ...
    lines.push("");
    lines.push("  View details: /tac logs debug <#>");
  }

  // Metrics summary
  const metricsPath = join(tacRoot(basePath), "metrics.json");
  ...
  lines.push("");
  lines.push("Tip: Enable debug logging with TAC_DEBUG=1 before /tac auto");
...
async function handleLogsShow(basePath: string, ctx: ExtensionCommandContext, seq: number): Promise<void> {
  ...
  if (!entry) {
    ctx.ui.notify(`Activity log #${seq} not found. Run /tac logs to see available logs.`, "warning");
    return;
  }
...
async function handleLogsDebug(basePath: string, ctx: ExtensionCommandContext, idx?: number): Promise<void> {
  ...
  if (debugLogs.length === 0) {
    ctx.ui.notify(
      "No debug logs found.\n\nEnable debug logging: TAC_DEBUG=1 tac auto",
      "info",
    );
    return;
  }
  ...
  if (idx === undefined) {
    // List debug logs
    const lines: string[] = ["Debug Logs (.tac/debug/):", ""];
    for (let i = 0; i < debugLogs.length; i++) {
      const d = debugLogs[i];
