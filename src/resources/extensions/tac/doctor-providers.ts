/**
 * TAC Doctor — Provider & Integration Health Checks
 *
...
function modelsJsonPaths(): string[] {
  ...
  return [
    join(home, ".tac", "agent", "models.json"),
...
function resolveKey(providerId: string): KeyLookup {
  ...
  // claude-code never stores credentials in auth.json — TAC delegates entirely to
    ...
    if (!lookup.found) {
      ...
      results.push({
        ...
        detail: providerId === "anthropic-vertex"
          ...
          : info?.hasOAuth
          ? `Run /tac keys to authenticate`
          : `Set ${envVar} or run /tac keys`,
        required: true,
      });
      ...
      results.push({
        ...
        detail: `TAC will retry automatically`,
        required: true,
      });
    } else {
      results.push({
  ...
  try {
    ...
    if (!lookup.found) {
      return {
        ...
        detail: info?.envVar ? `Set ${info.envVar} or run /tac keys` : `Run /tac keys to configure`,
        required: true,
      };
    }
