/**
 * TAC Quick Mode — /tac quick <task>
 * Copyright (c) 2026 Jeremy McSpadden <jeremy@fluxlabs.net>
 *
 * Lightweight task execution with TAC guarantees (atomic commits, state
 * tracking) but without the full milestone/slice ceremony.
 *
 * Quick tasks live in `.tac/quick/` and are tracked in STATE.md's
...
function ensureQuickDir(basePath: string, taskNum: number, slug: string): string {
  const quickDir = join(tacRoot(basePath), "quick");
...
function quickReturnStatePath(basePath: string): string {
  return join(tacRoot(basePath), "runtime", "quick-return.json");
}

function persistPendingReturn(state: QuickReturnState): void {
  pendingQuickReturn = state;
  mkdirSync(join(tacRoot(state.basePath), "runtime"), { recursive: true });
...
export async function handleQuick(
  ...
  // Validate: .tac/ must exist
  if (!existsSync(root)) {
    ctx.ui.notify(
      "No .tac/ directory found. Run /tac to initialize a project first.",
      "error",
    );
    return;
  }
  ...
  if (!description) {
    ctx.ui.notify(
      "Usage: /tac quick <task description>\n\nExample: /tac quick fix login button not responding on mobile",
      "info",
    );
    return;
  }
