...
import { SettingsManager, getAgentDir } from "@tac/pi-coding-agent";
...
export async function runClaudeImportFlow(
  ...
  const assetChoice = await ctx.ui.select("Import Claude assets into TAC/Pi config", [
    "Skills + plugins",
    "Skills only",
    "Plugins only",
    "Cancel",
  ...
  if (importSkills) {
    ...
    if (selectedSkills.length > 0) {
      const prefMode = await ctx.ui.select("How should TAC treat the imported skills?", [
  ...
  if (importedSkillsCount > 0) {
    ...
    lines.push(`- Skill refs added to TAC preferences (${scope}) when selected`);
  }
