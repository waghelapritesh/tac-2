/**
 * TAC Maintenance — cleanup, skip, dry-run, and recover handlers.
 *
...
export async function handleCleanupBranches(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  let branches: string[];
  try {
    branches = nativeBranchList(basePath, "tac/*");
  } catch (e) {
    logWarning("command", `branch list failed: ${(e as Error).message}`);
    ctx.ui.notify("No TAC branches to clean up.", "info");
    return;
  }

  const quickBranches = branches.filter((b) => b.startsWith("tac/quick/"));

  const mainBranch = nativeDetectMainBranch(basePath);
  let merged: string[];
  try {
    merged = nativeBranchListMerged(basePath, mainBranch, "tac/*");
  } catch (e) {
    logWarning("command", `merged branch list failed: ${(e as Error).message}`);
    merged = [];
  }

  const mergedNonQuick = merged.filter((b) => !b.startsWith("tac/quick/"));
  let deletedMerged = 0;
  ...
  if (quickBranches.length > 0) {
    summary.push(`Skipped ${quickBranches.length} quick branch${quickBranches.length === 1 ? "" : "es"} (tac/quick/*).`);
  }

  if (summary.length === 0) {
    const nonQuickCount = branches.filter((b) => !b.startsWith("tac/quick/")).length;
    ctx.ui.notify(
      nonQuickCount > 0
        ? `${nonQuickCount} TAC branch${nonQuickCount === 1 ? "" : "es"} found, none merged into ${mainBranch} yet.`
        : "No non-quick TAC branches to clean up.",
      "info",
    );
    return;
  }
...
export async function handleCleanupSnapshots(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  let refs: string[];
  try {
    refs = nativeForEachRef(basePath, "refs/tac/snapshots/");
  } catch (e) {
...
export async function handleCleanupWorktrees(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  ...
  if (statuses.length === 0) {
    ctx.ui.notify("No TAC worktrees found.", "info");
    return;
  }
...
export async function handleSkip(unitArg: string, ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  if (!unitArg) {
    ctx.ui.notify("Usage: /tac skip <unit-id>  (e.g., /tac skip execute-task/M001/S01/T03 or /tac skip T03)", "info");
    return;
  }
  ...
  const completedKeysFile = pathJoin(basePath, ".tac", "completed-units.json");
  let keys: string[] = [];
  try {
  ...
  keys.push(skipKey);
  mkDir(pathJoin(basePath, ".tac"), { recursive: true });
...
export async function handleDryRun(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  ...
  const { loadEffectiveTACPreferences: loadPrefs } = await import("./preferences.js");
...
export async function handleCleanupProjects(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (unknown.length > 0) {
    ...
    for (const h of unknown) {
      lines.push(`  ? ${h}  (open that project in TAC once to register metadata)`);
    }
    lines.push("");
  }
  ...
  if (!fix && orphaned.length > 0) {
    lines.push(`Run /tac cleanup projects --fix to permanently delete ${pl(orphaned.length, "orphaned director")}${orphaned.length === 1 ? "y" : "ies"}.`);
    ctx.ui.notify(lines.join("\n"), "warning");
    return;
  }
...
/**
 * `tac recover` — Reconstruct DB hierarchy state from rendered markdown on disk.
 *
...
export async function handleRecover(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  const { isDbAvailable: dbAvailable, clearEngineHierarchy, transaction: dbTransaction } = await import("./tac-db.js");
  const { migrateHierarchyToDb } = await import("./md-importer.js");
  const { invalidateStateCache } = await import("./state.js");

  if (!dbAvailable()) {
    ctx.ui.notify("tac recover: No database open. Run a TAC command first to initialize the DB.", "error");
    return;
  }
  ...
  try {
    ...
    // 5. Report
    const lines = [
      `tac recover: reconstructed hierarchy from markdown`,
    ...
    process.stderr.write(
      `tac-recover: recovered ${counts.milestones}M/${counts.slices}S/${counts.tasks}T hierarchy\n`,
    );
    ctx.ui.notify(lines.join("\n"), "success");
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logWarning("command", `recover failed: ${msg}`);
    ctx.ui.notify(`tac recover failed: ${msg}`, "error");
  }
}
