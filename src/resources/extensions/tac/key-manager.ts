...
import { getEnvApiKey } from "@tac/pi-ai";
...
export const PROVIDER_REGISTRY: ProviderInfo[] = [
  ...
  // See: https://github.com/waghelapritesh/tac-2/issues/4541
...
export function getAuthPath(): string {
  return join(process.env.HOME ?? "~", ".tac", "agent", "auth.json");
}
...
export function formatKeyDashboard(statuses: KeyStatus[]): string {
  ...
  const lines: string[] = ["TAC API Key Manager\n"];
...
export async function handleAddKey(
  ...
  if (providerArg) {
    ...
    if (!provider) {
      ctx.ui.notify(`Unknown provider: "${providerArg}". Use /tac keys list to see available providers.`, "error");
      return false;
    }
  } else {
...
export function runKeyDoctor(auth: AuthStorage): DoctorFinding[] {
    ...
    for (const cred of creds) {
      if (cred.type === "api_key" && !(cred as ApiKeyCredential).key) {
        findings.push({
          ...
          message: `${provider.label}: empty key stored (from skipped setup) — run /tac keys add ${provider.id}`,
        });
      }
    }
  }
  ...
  if (!hasAnyLlm) {
    findings.push({
      severity: "error",
      message: "No LLM provider configured — run /tac keys add or /login",
    });
  }
...
/**
 * Main entry point for /tac keys [subcommand].
 */
export async function handleKeys(
  ...
  switch (subcommand) {
    ...
    default:
      ctx.ui.notify(
        "Usage: /tac keys [list|add|remove|test|rotate|doctor]\n\n" +
        "  /tac keys              Show key status dashboard\n" +
        "  /tac keys list         List all configured keys\n" +
        "  /tac keys add [id]     Add a key for a provider\n" +
        "  /tac keys remove [id]  Remove a key\n" +
        "  /tac keys test [id]    Validate key(s) with API call\n" +
        "  /tac keys rotate [id]  Replace an existing key\n" +
        "  /tac keys doctor       Health check all keys",
        "info",
      );
      return;
  }
