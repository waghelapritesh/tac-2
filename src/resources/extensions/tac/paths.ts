/**
 * TAC Paths — ID-based path resolution
 *
...
import { nativeScanTacTree, type TacTreeEntry } from "./native-parser-bridge.js";
...
// ─── Native Tree Cache ────────────────────────────────────────────────────────
// When the native module is available, scan the entire .tac/ tree in one call
// and serve directory listings from memory instead of individual readdirSync calls.

let nativeTreeCache: Map<string, TacTreeEntry[]> | null = null;
let nativeTreeBase: string | null = null;

function getNativeTree(tacDir: string): Map<string, TacTreeEntry[]> | null {
  if (nativeTreeCache && nativeTreeBase === tacDir) return nativeTreeCache;

  const entries = nativeScanTacTree(tacDir);
  if (!entries) return null;

  // Build a map of parent directory -> entries
  const tree = new Map<string, TacTreeEntry[]>();
  ...
  nativeTreeBase = tacDir;
  return tree;
}

/**
 * Convert a native tree lookup into a relative key for the tree map.
 * Returns the relative path from the tacDir, or null if the path isn't under tacDir.
 */
function nativeTreeKey(dirPath: string, tacDir: string): string | null {
  if (!dirPath.startsWith(tacDir)) return null;
  const rel = dirPath.slice(tacDir.length).replace(/^\//, '');
  return rel || '.';
}
...
function cachedReaddirWithTypes(dirPath: string): Dirent[] {
  ...
  // Try native tree cache for paths under .tac/
  if (nativeTreeBase) {
...
export type TACRootFileKey = keyof typeof TAC_ROOT_FILES;

const LEGACY_TAC_ROOT_FILES: Record<TACRootFileKey, string> = {
  PROJECT: "project.md",
  DECISIONS: "decisions.md",
  QUEUE: "queue.md",
  STATE: "state.md",
...
const tacRootCache = new Map<string, string>();

/** Exported for tests only — do not call in production code. */
export function _clearTacRootCache(): void {
  tacRootCache.clear();
}

/**
 * Resolve the `.tac` directory for a given project base path.
 *
 * Probe order:
 *   1. basePath/.tac         — fast path (common case)
 *   2. git rev-parse root    — handles cwd-is-a-subdirectory
 *   3. Walk up from basePath — handles moved .tac in an ancestor (bounded by git root)
 *   4. basePath/.tac         — creation fallback (init scenario)
 *
 * Result is cached per basePath for the process lifetime.
 */
export function tacRoot(basePath: string): string {
  const cached = tacRootCache.get(basePath);
  if (cached) return cached;

  const result = probeTacRoot(basePath);
  tacRootCache.set(basePath, result);
  return result;
}

/**
 * Detect if a path is inside a .tac/worktrees/<name>/ structure.
 *
 * TAC auto-worktrees live at <project>/.tac/worktrees/<milestoneId>/.
 * When tacRoot() is called with such a path, we must NOT walk up to the
 * project root's .tac — each worktree manages its own .tac state (#2594).
 *
 * Matches both forward-slash and platform-native separators to handle
 * Windows paths (path.sep = '\\') and normalized Unix paths.
 */
function isInsideTacWorktree(p: string): boolean {
  // Match /.tac/worktrees/<name> where <name> is the final segment or
  // followed by a separator. The <name> segment must be non-empty.
  const sepFwd = "/";
  const sepNative = "\\";
  const markers = [
    `${sepFwd}.tac${sepFwd}worktrees${sepFwd}`,
    `${sepNative}.tac${sepNative}worktrees${sepNative}`,
  ];
...
function probeTacRoot(rawBasePath: string): string {
  // 1. Fast path — check the input path directly
  const local = join(rawBasePath, ".tac");
  if (existsSync(local)) return local;

  // 1b. Worktree guard (#2594) — if basePath is inside a .tac/worktrees/<name>/
  //     structure, return the worktree-local .tac path immediately. Without this,
  //     the git-root probe (step 2) or walk-up (step 3) escapes to the project
  //     root's .tac, causing ensurePreconditions() and deriveState() to read/write
  //     state in the wrong location.
  if (isInsideTacWorktree(rawBasePath)) return local;
  ...
  if (basePath !== rawBasePath && isInsideTacWorktree(basePath)) return local;
  ...
  if (gitRoot) {
    const candidate = join(gitRoot, ".tac");
...
export function milestonesDir(basePath: string): string {
  return join(tacRoot(basePath), "milestones");
}

export function resolveRuntimeFile(basePath: string): string {
  return join(tacRoot(basePath), "RUNTIME.md");
}

export function resolveTacRootFile(basePath: string, key: TACRootFileKey): string {
  const root = tacRoot(basePath);
  const canonical = join(root, TAC_ROOT_FILES[key]);
  if (existsSync(canonical)) return canonical;
  const legacy = join(root, LEGACY_TAC_ROOT_FILES[key]);
  if (existsSync(legacy)) return legacy;
  return canonical;
}

export function relTacRootFile(key: TACRootFileKey): string {
  return `.tac/${TAC_ROOT_FILES[key]}`;
}
...
// ─── Relative Path Builders (for prompts — .tac/milestones/...) ────────────

/**
 * Build relative .tac/ path to a milestone directory.
...
export function relMilestonePath(basePath: string, milestoneId: string): string {
  const dir = resolveDir(milestonesDir(basePath), milestoneId);
  if (dir) return `.tac/milestones/${dir}`;
  return `.tac/milestones/${milestoneId}`;
}

/**
 * Build relative .tac/ path to a milestone file.
 */
...
/**
 * Build relative .tac/ path to a slice directory.
 */
...
/**
 * Build relative .tac/ path to a slice file.
 */
...
/**
 * Build relative .tac/ path to a task file.
 */
