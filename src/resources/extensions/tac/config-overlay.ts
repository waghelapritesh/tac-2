/**
 * TAC Configuration Overlay
 *
 * Read-only TUI overlay showing the effective TAC configuration:
 * token profile, model assignments, dynamic routing, git settings,
 * budget, workflow toggles, and preference file sources.
 * Opened via `/tac show-config` or `/tac config`.
 */

import type { Theme } from "@tac/pi-coding-agent";
import { matchesKey, Key, truncateToWidth } from "@tac/pi-tui";
...
function collectConfigSections(): ConfigSection[] {
  ...
  sections.push({
    title: "Sources",
    rows: [
      { label: "Global", value: globalPrefs ? globalPrefs.path : `(none) ${getGlobalTACPreferencesPath()}` },
      { label: "Project", value: projectPrefs ? projectPrefs.path : `(none) ${getProjectTACPreferencesPath()}` },
    ],
  });
...
export function formatConfigText(): string {
  ...
  const lines: string[] = ["TAC Configuration\n"];
...
export class TACConfigOverlay {
  ...
  render(width: number): string[] {
    ...
    // Header
    allLines.push(t.bold(t.fg("accent", " TAC Configuration ")));
    ...
    allLines.push(t.fg("muted", "  esc/q close  \u2502  \u2191\u2193/jk scroll  \u2502  /tac prefs to edit"));
