/**
 * TAC Auto Mode — Fresh Session Per Unit
 *
 * State machine driven by .tac/ files on disk. Each "unit" of work
...
import type { TACPreferences } from "./preferences.js";
import {
...
import { runTACDoctor, rebuildState } from "./doctor.js";
import {
...
import { deactivateTAC } from "../shared/tac-phase-state.js";
import {
  initMetrics,
  resetMetrics,
  getLedger,
...
function captureProjectRootEnv(projectRoot: string): void {
  if (!s.projectRootEnvCaptured) {
    s.hadProjectRootEnv = Object.prototype.hasOwnProperty.call(process.env, "TAC_PROJECT_ROOT");
    s.previousProjectRootEnv = process.env.TAC_PROJECT_ROOT ?? null;
    s.projectRootEnvCaptured = true;
  }
  process.env.TAC_PROJECT_ROOT = projectRoot;
}
...
function restoreProjectRootEnv(): void {
  ...
  if (s.hadProjectRootEnv && s.previousProjectRootEnv !== null) {
    process.env.TAC_PROJECT_ROOT = s.previousProjectRootEnv;
  } else {
    delete process.env.TAC_PROJECT_ROOT;
  }
...
function captureMilestoneLockEnv(milestoneId: string | null): void {
  if (!s.milestoneLockEnvCaptured) {
    s.hadMilestoneLockEnv = Object.prototype.hasOwnProperty.call(process.env, "TAC_MILESTONE_LOCK");
    s.previousMilestoneLockEnv = process.env.TAC_MILESTONE_LOCK ?? null;
    s.milestoneLockEnvCaptured = true;
  }

  if (milestoneId) {
    process.env.TAC_MILESTONE_LOCK = milestoneId;
  } else {
...
function restoreMilestoneLockEnv(): void {
  ...
  if (s.hadMilestoneLockEnv && s.previousMilestoneLockEnv !== null) {
    process.env.TAC_MILESTONE_LOCK = s.previousMilestoneLockEnv;
  } else {
    delete process.env.TAC_MILESTONE_LOCK;
  }
...
export function shouldUseWorktreeIsolation(basePath?: string): boolean {
  const prefs = loadEffectiveTACPreferences(basePath)?.preferences?.git;
 ...
 * concurrent TAC instances sharing the same global settings.json (#650).
...
export function getAutoDashboardData(): AutoDashboardData {
  ...
  const rtkSavings = sessionId && s.basePath
    ...
    : null;
  const rtkEnabled = loadEffectiveTACPreferences(s.basePath || undefined)?.preferences.experimental?.rtk === true;
 ...
 * Called from tool_execution_end when a TAC tool fails with isError.
 ...
 * Used by the guard in commands.ts to prevent bare /tac, /tac next, and
 * /tac auto from stealing the session lock.
 */
...
function buildSnapshotOpts(
  ...
  const prefs = loadEffectiveTACPreferences(s.basePath || undefined)?.preferences;
...
function handleLostSessionLock(
  ...
  const base = lockBase();
  const lockFilePath = base ? join(tacRoot(base), "auto.lock") : "unknown";
  const recoverySuggestion = "\nTo recover, run: tac doctor --fix";
  const message =
    lockStatus?.failureReason === "pid-mismatch"
      ? lockStatus.existingPid
        ? `Session lock (${lockFilePath}) moved to PID ${lockStatus.existingPid} — another TAC process appears to have taken over. Stopping gracefully.${recoverySuggestion}`
        : `Session lock (${lockFilePath}) moved to a different process — another TAC process appears to have taken over. Stopping gracefully.${recoverySuggestion}`
  ...
  ctx?.ui.setStatus("tac-auto", undefined);
  ctx?.ui.setWidget("tac-progress", undefined);
...
function cleanupAfterLoopExit(ctx: ExtensionContext): void {
  ...
  // Clear crash lock and release session lock so the next `/tac next` does
...
export async function stopAuto(
  ...
  const loadedPreferences = loadEffectiveTACPreferences(s.basePath || undefined)?.preferences;
  ...
  try {
    ...
    // Late async notifications (async_job_result, tac-auto-wrapup) can trigger
    ...
    try {
      if (s.currentMilestoneId && !s.milestoneMergedInPhases) {
        ...
        // signal — only a successful tac_complete_milestone call flips it to
    ...
    if (isDbAvailable()) {
      try {
        const { closeDatabase } = await import("./tac-db.js");
        closeDatabase();
      } catch (e) {
    ...
    try {
      const pausedPath = join(tacRoot(s.originalBasePath || s.basePath), "runtime", "paused-session.json");
    ...
    // Drop the active-tool baseline so a subsequent /tac auto run on the
 ...
 * The user can interact with the agent, then `/tac auto` resumes
...
export async function pauseAuto(
  ...
  try {
    ...
    const runtimeDir = join(tacRoot(s.originalBasePath || s.basePath), "runtime");
  ...
  ctx?.ui.setStatus("tac-auto", "paused");
  ctx?.ui.setWidget("tac-progress", undefined);
  if (ctx) initHealthWidget(ctx);
  const resumeCmd = s.stepMode ? "/tac next" : "/tac auto";
  ctx?.ui.notify(
...
function buildResolverDeps(): WorktreeResolverDeps {
  return {
    ...
    loadEffectiveTACPreferences:
      loadEffectiveTACPreferences as unknown as WorktreeResolverDeps["loadEffectiveTACPreferences"],
    invalidateAllCaches,
    captureIntegrationBranch,
  };
}
...
function buildLoopDeps(pi: ExtensionAPI): LoopDeps {
  ...
  return {
    ...
    handleLostSessionLock: (ctx: ExtensionContext | undefined, lockStatus: SessionLockStatus | undefined) => {
      cmux.clearCmuxSidebar(loadEffectiveTACPreferences(s.basePath || undefined)?.preferences);
...
export async function startAuto(
  ...
  // Heal .tac.migrating before any branching — covers both fresh-start and
  ...
  if (recoverFailedMigration(base)) {
    ctx.ui.notify("Recovered unfinished migration (.tac.migrating → .tac).", "info");
  }
  ...
  if (!s.paused) {
    try {
      ...
      const pausedPath = join(tacRoot(base), "runtime", "paused-session.json");
  ...
  if (s.paused) {
    ...
    ctx.ui.setStatus("tac-auto", s.stepMode ? "next" : "auto");
    ctx.ui.setWidget("tac-health", undefined);
    ctx.ui.notify(
    ...
    // TAC_PKG_ROOT is set by loader.ts and points to the tac-2 package root.
    // The relative import ("../../../resource-loader.js") only works from the source
    // tree; deployed extensions live at ~/.tac/agent/extensions/tac/ where the
    // relative path resolves to ~/.tac/agent/resource-loader.js which doesn't exist.
    // Using TAC_PKG_ROOT constructs a correct absolute path in both contexts (#3949).
    const agentDir = process.env.TAC_CODING_AGENT_DIR || join(process.env.TAC_HOME || homedir(), ".tac", "agent");
    const pkgRoot = process.env.TAC_PKG_ROOT;
    ...
    try {
      await rebuildState(s.basePath);
      pi.events.emit(CMUX_CHANNELS.SIDEBAR, { action: "sync" as const, preferences: loadEffectiveTACPreferences(s.basePath || undefined)?.preferences, state: await deriveState(s.basePath) });
    } catch (e) {
    ...
    try {
      const report = await runTACDoctor(s.basePath, { fix: true });
    ...
    if (s.pausedSessionFile) {
      const activityDir = join(tacRoot(s.basePath), "activity");
    ...
    pi.events.emit(CMUX_CHANNELS.LOG, { preferences: loadEffectiveTACPreferences(s.basePath || undefined)?.preferences, message: s.stepMode ? "Step-mode resumed." : "Auto-mode resumed.", level: "progress" });
  ...
  pi.events.emit(CMUX_CHANNELS.LOG, { preferences: loadEffectiveTACPreferences(s.basePath || undefined)?.preferences, message: requestedStepMode ? "Step-mode started." : "Auto-mode started.", level: "progress" });
...
export async function dispatchHookUnit(
  ...
  pi.sendMessage(
    { customType: "tac-auto", content: hookPrompt, display: true },
    { triggerTurn: true },
  );
