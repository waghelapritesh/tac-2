// TAC Extension — Debug Logger
// Structured JSONL debug logging for diagnosing stuck/slow TAC sessions.
 ...
 * Can be activated via `--debug` flag or `TAC_DEBUG=1` env var.
 */
export function enableDebug(basePath: string): void {
  const debugDir = join(tacRoot(basePath), 'debug');
...
export function debugLog(event: string, data?: Record<string, unknown>): void {
  ...
  try {
    ...
    // Silently ignore write failures — debug logging must never break TAC
  }
}
