// TAC Compaction Snapshot — writes a ≤2 KB markdown digest of durable
// project state before the session context is compacted. On resume, an
// agent can `tac_resume` (or Read .tac/last-snapshot.md) to re-orient
...
export function buildSnapshot(sources: SnapshotSources, opts: BuildSnapshotOptions = {}): string {
  ...
  const lines: string[] = [];
  lines.push(`# TAC context snapshot (${sources.generatedAt.toISOString()})`);
  lines.push("");
  ...
  if (exec.length > 0) {
    lines.push("## Recent tac_exec runs");
...
export function writeCompactionSnapshot(
  ...
  const tacDir = resolve(baseDir, ".tac");
  if (!existsSync(tacDir)) mkdirSync(tacDir, { recursive: true });
  const path = resolve(tacDir, SNAPSHOT_FILENAME);
...
export function readCompactionSnapshot(baseDir: string): string | null {
  const path = resolve(baseDir, ".tac", SNAPSHOT_FILENAME);
