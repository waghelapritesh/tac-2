/**
 * TAC Session Forensics — Deep analysis of pi session JSONL files
 *
 ...
 * - Stuck-retry diagnostics (reading TAC activity log copies)
 *
 ...
 * Works with both pi session JSONL and TAC activity log JSONL.
 */
 ...
 *
 * Reads the surviving pi session file (or falls back to the last TAC activity
...
export function synthesizeCrashRecovery(
  ...
  try {
    ...
    // Fallback: last TAC activity log
...
export function getDeepDiagnostic(basePath: string, worktreePath?: string): string | null {
  ...
  try {
    if (worktreePath) {
      const wtActivityDir = join(tacRoot(worktreePath), "activity");
...
export function readActiveMilestoneId(basePath: string): string | null {
  try {
    const statePath = join(tacRoot(basePath), "STATE.md");
...
function formatTraceSummary(trace: ExecutionTrace): string {
  ...
  // See: https://github.com/waghelapritesh/tac-2/issues/2195
  return parts.join("\n");
}
