...
// Error-severity entries are persisted to .tac/audit-log.jsonl (sanitized) for
...
export type LogComponent =
  ...
  | "ecosystem"     // TAC ecosystem extension loader and dispatch
...
export function readAuditLog(basePath?: string): LogEntry[] {
  ...
  if (!bp) return [];
  const auditPath = join(bp, ".tac", "audit-log.jsonl");
...
function _push(
  ...
  _writeStderr(`[tac:${component}] ${prefix}: ${message}${ctxStr}\n`);
  ...
  try {
    ...
    _writeStderr(`[tac:workflow-logger] notification-store append failed: ${(notifErr as Error).message}\n`);
  }
  ...
  if (_auditBasePath && isUnifiedAuditEnabled()) {
    try {
      ...
      _writeStderr(`[tac:workflow-logger] unified-audit emit failed: ${(auditEmitErr as Error).message}\n`);
    }
  }

  // Persist errors to .tac/audit-log.jsonl so they survive context resets.
  ...
  if (_auditBasePath && severity === "error") {
    try {
      const auditDir = join(_auditBasePath, ".tac");
      ...
      _writeStderr(`[tac:audit] failed to persist log entry: ${(auditErr as Error).message}\n`);
    }
  }
}
