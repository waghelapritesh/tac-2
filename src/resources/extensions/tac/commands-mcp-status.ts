/**
 * MCP Status — `/tac mcp` command handler.
 *
 * Shows configured MCP servers, their connection status, and available tools.
 *
 * Subcommands:
 *   /tac mcp             — Overview of all servers (alias: /tac mcp status)
 *   /tac mcp status      — Same as bare /tac mcp
 *   /tac mcp check <srv> — Detailed status for a specific server
 *   /tac mcp init [dir]  — Write project-local TAC workflow MCP config
 */
...
export function formatMcpInitResult(
  ...
  return [
    ...
    `Config:   ${configPath}`,
    "",
    "Claude Code can now load the TAC workflow MCP server from this folder.",
  ].join("\n");
}
...
export function formatMcpStatusReport(servers: McpServerStatus[]): string {
  if (servers.length === 0) {
    return [
      ...
      "",
      "Add servers to .mcp.json or .tac/mcp.json to enable MCP integrations.",
      "Tip: run /tac mcp init . to write the local TAC workflow MCP config.",
  ...
  lines.push("");
  lines.push("Use /tac mcp check <server> for details on a specific server.");
...
/**
 * Handle `/tac mcp [status|check <server>]`.
 */
export async function handleMcpStatus(
  ...
  // Unknown subcommand
  ctx.ui.notify(
    "Usage: /tac mcp [status|check <server>|init [dir]]\n\n" +
    "  status           Show all MCP server statuses (default)\n" +
    "  check <server>   Detailed status for a specific server\n" +
    "  init [dir]       Write .mcp.json for the local TAC workflow MCP server",
    "warning",
  );
}
