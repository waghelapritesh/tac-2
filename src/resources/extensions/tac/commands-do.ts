 ...
 * TAC Command — /tac do
 *
 * Routes freeform natural language to the correct /tac subcommand
 * using keyword matching. Falls back to /tac quick for task-like input.
 */
...
export async function handleDo(
  ...
  if (!args.trim()) {
    ctx.ui.notify(
      "Usage: /tac do <what you want to do>\n\n" +
      "Examples:\n" +
      "  /tac do show me progress\n" +
      "  /tac do run autonomously\n" +
      "  /tac do clean up old branches\n" +
      "  /tac do fix the login bug",
      "warning",
    );
    return;
  }
  ...
  if (match) {
    ...
    ctx.ui.notify(`→ /tac ${fullCommand}`, "info");

    // Re-dispatch through the main dispatcher
    const { handleTACCommand } = await import("./commands/dispatcher.js");
    await handleTACCommand(fullCommand, ctx, pi);
    return;
  }

  // No keyword match → treat as quick task
  ctx.ui.notify(`→ /tac quick ${args}`, "info");
