// Migration transformer — converts parsed PlanningProject into TACProject.
...
function buildTaskSummary(summary: PlanningSummary): TACTaskSummaryData {
  return {
...
function mapTask(plan: PlanningPlan, index: number, summaries: Record<string, PlanningSummary>): TACTask {
...
function buildSliceSummary(phase: PlanningPhase): TACSliceSummaryData | null {
...
function mapRequirements(reqs: PlanningRequirement[]): TACRequirement[] {
  let autoId = 0;
  return reqs.map((req) => {
    autoId++;
    return {
...
export function transformToTAC(parsed: PlanningProject): TACProject {
  const milestones: TACMilestone[] = [];
  ...
  if (isMultiMilestone) {
    // Multi-milestone mode: each roadmap milestone section → one TACMilestone
