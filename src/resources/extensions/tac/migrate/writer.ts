// TAC Directory Writer — Format Functions & Directory Orchestrator
// Format functions: pure string-returning functions that serialize TAC types into the exact markdown
// format that TAC-2's parsers expect (parseRoadmap, parsePlan, parseSummary, parseRequirementCounts).
// writeTACDirectory: orchestrator that writes a complete .tac directory tree from a TACProject.
...
/** Result of writeTACDirectory — lists all files that were written. */
...
/** Pre-write statistics computed from a TACProject without I/O. */
 ...
 */
export function formatRoadmap(milestone: TACMilestone): string {
  const lines: string[] = [];
 ...
 */
export function formatPlan(slice: TACSlice): string {
  const lines: string[] = [];
 ...
 */
export function formatSliceSummary(slice: TACSlice, milestoneId: string): string {
 ...
 * Output must parse correctly through parseSummary().
 */
export function formatTaskSummary(task: TACTask, sliceId: string, milestoneId: string): string {
 ...
 */
export function formatTaskPlan(task: TACTask, sliceId: string, milestoneId: string): string {
  const lines: string[] = [];
 ...
 */
export function formatRequirements(requirements: TACRequirement[]): string {
  const lines: string[] = [];
  lines.push('# Requirements');
  lines.push('');

  const groups: Record<string, TACRequirement[]> = {
    active: [],
    validated: [],
    deferred: [],
    'out-of-scope': [],
 ...
 * Write a minimal stub that will be overwritten on first /tac status.
 */
export function formatState(milestones: TACMilestone[]): string {
  const lines: string[] = [];
...
/**
 * Write a complete .tac directory tree from a TACProject.
 ...
 */
export async function writeTACDirectory(
  project: TACProject,
  targetPath: string,
): Promise<WrittenFiles> {
  const tacDir = tacRoot(targetPath);
  const milestonesBase = join(tacDir, 'milestones');
  const paths: string[] = [];
