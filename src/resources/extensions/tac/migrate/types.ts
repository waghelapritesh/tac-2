...
// ═══════════════════════════════════════════════════════════════════════════
// TAC Output Types — produced by transformer, consumed by writer (S03)
// Mirror TAC-2 runtime shapes so deriveState() works on migrated output.
// ═══════════════════════════════════════════════════════════════════════════

export interface TACProject {
  milestones: TACMilestone[];
  /** Raw PROJECT.md text (pass through from old format) */
  projectContent: string;
  requirements: TACRequirement[];
  /** Empty or pass-through from old project key decisions */
  decisionsContent: string;
}

export interface TACMilestone {
  /** e.g. "M001", "M002" */
  id: string;
  ...
  boundaryMap: TACBoundaryEntry[];
}

export interface TACSlice {
  /** e.g. "S01", "S02" */
  id: string;
  ...
  summary: TACSliceSummaryData | null;
}

export interface TACTask {
  /** e.g. "T01", "T02" */
  id: string;
  ...
  mustHaves: string[];
  /** Only populated if done */
  summary: TACTaskSummaryData | null;
}

export interface TACRequirement {
  /** e.g. "R001" */
  id: string;
  title: string;
...
export interface TACSliceSummaryData {
...
export interface TACTaskSummaryData {
  completedAt: string;
  provides: string[];
  keyFiles: string[];
  duration: string;
...
export interface TACBoundaryEntry {
  fromSlice: string;
  toSlice: string;
  produces: string;
  consumes: string;
