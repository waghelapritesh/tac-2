// TAC Extension — Undo Last Unit + Targeted State Reset
...
import type { ExtensionCommandContext, ExtensionAPI } from "@tac/pi-coding-agent";
...
import { tacRoot, resolveTasksDir, resolveSlicePath, resolveTaskFile, buildTaskFileName, buildSliceFileName } from "./paths.js";
import { sendDesktopNotification } from "./notifications.js";
import { getTask, getSlice, getSliceTasks, updateTaskStatus, updateSliceStatus } from "./tac-db.js";
...
export async function handleUndo(args: string, ctx: ExtensionCommandContext, _pi: ExtensionAPI, basePath: string): Promise<void> {
  ...
  // Find the last TAC-related commit from git activity logs
  const activityDir = join(tacRoot(basePath), "activity");
  ...
  sendDesktopNotification("TAC", `Undone: ${unitType} (${unitId})`, "info", "complete", basename(basePath));
}
...
export async function handleUndoTask(
  ...
  if (!rawId) {
    ctx.ui.notify(
      "Usage: /tac undo-task <taskId> [--force]\n\n" +
  ...
  if (!force) {
    ctx.ui.notify(
      ...
      `  - Re-render plan checkboxes\n\n` +
      `Run /tac undo-task ${rawId} --force to confirm.`,
      "warning",
    );
    return;
  }
...
export async function handleResetSlice(
  ...
  if (!rawId) {
    ctx.ui.notify(
      "Usage: /tac reset-slice <sliceId> [--force]\n\n" +
  ...
  if (!force) {
    ctx.ui.notify(
      ...
      `  - Delete task summary files, slice summary, and UAT files\n` +
      `  - Re-render plan + roadmap checkboxes\n\n` +
      `Run /tac reset-slice ${rawId} --force to confirm.`,
      "warning",
    );
    return;
  }
