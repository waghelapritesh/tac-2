...
import { getMilestoneSlices, isDbAvailable } from "./tac-db.js";
import { buildExistingMilestonesContext } from "./guided-flow-queue.js";
import { loadPrompt } from "./prompt-loader.js";
import { isTacGitignored } from "./gitignore.js";
...
export async function handleRethink(
  ...
  if (!existsSync(root)) {
    ctx.ui.notify("No TAC project found. Run /tac init first.", "warning");
    return;
  }
  ...
  const commitInstruction = isTacGitignored(basePath)
    ? "Do not commit planning artifacts — .tac/ is gitignored in this project."
    : 'After changes, run `git add .tac/ && git commit -m "docs(tac): rethink milestone plan"` to persist (rethink runs interactively outside auto-mode, so no system auto-commit)';
  ...
  pi.sendMessage(
    { customType: "tac-rethink", content, display: false },
    { triggerTurn: true },
  );
}
