/**
 * TAC Changelog — Fetch and display categorized release notes from GitHub
 *
 * Fetches releases from the waghelapritesh/tac-2 GitHub repository,
...
const RELEASES_URL = "https://api.github.com/repos/waghelapritesh/tac-2/releases?per_page=100";
...
export async function handleChangelog(
  ...
  try {
    const response = await fetch(RELEASES_URL, {
      headers: { "User-Agent": "tac-changelog" },
    });
  ...
  const currentVersion = process.env.TAC_VERSION || "";
  ...
  if (showCurrentOnly) {
    if (!currentVersion) {
      ctx.ui.notify(
        "TAC_VERSION is not set — cannot determine current release. Provide a version instead.",
        "warning",
      );
      return;
    }
  ...
  const prompt = [
    `Here are the raw TAC changelog entries ${versionRange}.`,
  ...
  pi.sendMessage(
    { customType: "tac-changelog", content: prompt, display: true },
    { triggerTurn: true },
  );
}
