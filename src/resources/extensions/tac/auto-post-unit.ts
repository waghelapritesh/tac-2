...
import type { ExtensionContext, ExtensionAPI } from "@tac/pi-coding-agent";
...
import { isDbAvailable, getTask, getSlice, getMilestone, updateTaskStatus, updateSliceStatus, _getAdapter } from "./tac-db.js";
...
import { getSliceTasks } from "./tac-db.js";
...
/** Unit types that only touch `.tac/` internal state files (no code changes).
...
export const STEP_COMPLETE_FALLBACK_MESSAGE =
  "Step complete. Run /clear, then /tac to continue (or /tac auto to run continuously).";

export function buildStepCompleteMessage(nextState: import("./types.js").TACState): string {
  if (nextState.phase === "complete") {
    return "Step complete — milestone finished. Run /tac status to review, or start the next milestone.";
  }
  const next = describeNextUnit(nextState);
  return `Step complete. Next: ${next.label}\n`
    + `Run /clear, then /tac to continue (or /tac auto to run continuously).`;
}
...
export interface PostUnitContext {
  ...
  updateProgressWidget: (ctx: ExtensionContext, unitType: string, unitId: string, state: import("./types.js").TACState) => void;
}
  ...
  if (s.currentUnit) {
    ...
    if (s.currentUnit.type === "complete-slice") {
      ...
      await runSafely("postUnit", "slice-cadence-merge", async () => {
        const prefsResult = loadEffectiveTACPreferences(s.basePath);
        ...
        try {
          ...
          if (err instanceof MergeConflictError) {
            ctx.ui.notify(
              ...
              `Resolve manually on main and run \`/tac auto\` to resume.`,
              "error",
            );
    ...
    try {
      const { loadEffectiveTACPreferences } = await import("./preferences.js");
      const prefs = loadEffectiveTACPreferences()?.preferences;
        ...
        if (hasExpectedArtifact) {
          ...
          if (attempt > MAX_VERIFICATION_RETRIES) {
            ...
            if (s.currentUnit.type === "complete-milestone") {
              ...
              ctx.ui.notify(
                `Milestone ${s.currentUnit.id} verification failed after ${MAX_VERIFICATION_RETRIES} retries — worktree branch preserved. Re-run /tac auto once blockers are resolved.`,
                "error",
              );
              await pauseAuto(ctx, pi);
              return "dispatched";
...
export async function postUnitPostVerification(pctx: PostUnitContext): Promise<"continue" | "step-wizard" | "stopped"> {
  ...
  if (s.currentUnit) {
    try {
      const codebasePrefs = loadEffectiveTACPreferences()?.preferences?.codebase;
        ...
        try {
          ...
          if (mid && sid && tid) {
            try {
              ...
              // Use 'tac recover' to rebuild DB state from disk if needed.
              logError("engine", `retry state-reset failed (DB unavailable): ${(dbErr as Error).message}. Run 'tac recover' to reconcile.`);
            }
          }
  ...
  // Both emit the same PLAN.md + task artifacts via tac_plan_slice, so the
    ...
    await runSafely("postUnitPostVerification", "pre-execution-checks", async () => {
      ...
      try {
        ...
        process.stderr.write(
          `tac-pre-exec: ${emoji} Pre-execution checks ${result.status} for ${mid}/${sid} (${result.durationMs}ms)\n`,
        );
        ...
        for (const check of result.checks) {
          ...
          process.stderr.write(
            `tac-pre-exec:   ${checkEmoji} [${check.category}] ${check.target}: ${check.message}\n`,
          );
        }
        ...
        logError("engine", `tac-pre-exec: Pre-execution checks threw an error: ${errorMessage}`);
        ctx.ui.notify(
  ...
  // Without this notify(), /tac in step mode finishes a unit and silently
  // exits the loop, leaving the user with no hint to /clear and /tac again.
  if (s.stepMode) {
    try {
