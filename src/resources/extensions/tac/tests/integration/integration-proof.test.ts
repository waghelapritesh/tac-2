...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), "tac-integration-proof-"));
}
...
/**
 * Create a temp directory with a realistic .tac/ structure:
...
function createRealisticFixture(): string {
  ...
  const mDir = join(tacDir, "milestones", "M001");
  const sliceDir = join(mDir, "slices", "S01");
  const tasksDir = join(sliceDir, "tasks");

  mkdirSync(tasksDir, { recursive: true });
  mkdirSync(join(tacDir, "activity"), { recursive: true });
...
test("full lifecycle: migration through completion through doctor", async (t) => {
  ...
  // ── (a) Open file-backed DB ──────────────────────────────────────
    ...
    const doctorReport = await runTACDoctor(base, {
      fix: false,
      isolationMode: "none",
    });
    ...
    const rogueDir = join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks");
...
test("recovery: DB loss → migrateFromMarkdown restores state, stale render detection", async (t) => {
  ...
  // Set up a completed state first
    ...
    // Clear path caches so tacRoot re-probes after DB deletion
