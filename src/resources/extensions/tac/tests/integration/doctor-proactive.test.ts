...
describe('doctor-proactive', async () => {
  ...
  try {
    ...
    test('health gate: missing STATE.md does NOT block dispatch (#889)', async () => {
      ...
      writeFileSync(join(dir, ".tac", "milestones", "M001", "M001-ROADMAP.md"), "# Roadmap\n");
    ...
    test('health gate: stale crash lock auto-cleared', async () => {
      ...
      // Write a stale lock
      writeFileSync(join(dir, ".tac", "auto.lock"), JSON.stringify({
      ...
      assert.ok(!existsSync(join(dir, ".tac", "auto.lock")), "lock file removed");
    });
    ...
    test('health gate: STATE.md missing — auto-healed', async () => {
      const dir = realpathSync(mkdtempSync(join(tmpdir(), "doc-proactive-")));
      cleanups.push(dir);
      // Minimal .tac structure: milestones dir exists but no STATE.md
      mkdirSync(join(dir, ".tac", "milestones"), { recursive: true });

      const stateFile = join(dir, ".tac", "STATE.md");
    ...
    test('health gate: git.snapshots:false suppresses stale-commit snapshot (#4420)', async () => {
      ...
      try {
        ...
        assert.ok(
          !result.fixesApplied.some(f => f.includes("tac snapshot")),
