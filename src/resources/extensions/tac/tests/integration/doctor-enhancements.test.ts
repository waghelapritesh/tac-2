...
function makeBase(): { base: string; tac: string; mDir: string } {
  const base = mkdtempSync(join(tmpdir(), "tac-doctor-enh-"));
...
describe('doctor-enhancements', async () => {
  ...
  test('metrics ledger corrupt', async () => {
    const { base, tac, mDir } = makeBase();
    writeRoadmap(mDir, `# M001: Metrics Test\n\n## Slices\n- [ ] **S01: Slice** \`risk:low\` \`depends:[]\`\n  > After this: done\n`);
    writeSlice(mDir, "S01", "# S01: Slice\n\n**Goal:** G\n**Demo:** D\n\n## Tasks\n- [ ] **T01: Task** `est:10m`\n  Pending.\n");
    // Write invalid metrics.json
    writeFileSync(join(tac, "metrics.json"), '{"version":2,"data":[]}');
  ...
  test('dry-run mode', async () => {
    ...
    const result = await runTACDoctor(base, { fix: true, dryRun: true });
  ...
  test('per-check timing', async () => {
    ...
    assert.ok(typeof result.timing?.tacState === "number", "timing.tacState is a number");
  ...
  test('doctor history', async () => {
    ...
    await runTACDoctor(base, { fix: false });

    const historyPath = join(tac, "doctor-history.jsonl");
