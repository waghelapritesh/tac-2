...
import { runTACDoctor } from "../../doctor.js";
import { parsePlan } from "../../parsers-legacy.js";

// ── Helpers ─────────────────────────────────────────────────────────────────

function makeBase(): { base: string; tac: string; mDir: string } {
  const base = mkdtempSync(join(tmpdir(), "tac-doctor-fp-"));
  const tac = join(base, ".tac");
  const mDir = join(tac, "milestones", "M001");
  mkdirSync(join(mDir, "slices"), { recursive: true });
  return { base, tac, mDir };
}
...
describe('doctor false-positives (#3105)', async () => {
  ...
  test('Bug 1: orphaned worktree check ignores dirs containing only .tac/doctor-history.jsonl', async () => {
    // Simulate: a worktree dir that only contains .tac/doctor-history.jsonl
    // (created by appendDoctorHistory writing to the worktree-scoped path).
    // The orphan check should NOT warn about this directory.
    const { base, tac } = makeBase();
    writeRoadmap(join(tac, "milestones", "M001"), `# M001: Test\n\n## Slices\n- [ ] **S01: Slice** \`risk:low\` \`depends:[]\`\n  > After this: done\n`);
    writeSlice(join(tac, "milestones", "M001"), "S01", "# S01: Slice\n\n**Goal:** G\n**Demo:** D\n\n## Tasks\n- [ ] **T01: Task** `est:10m`\n  Pending.\n");

    // Create a worktree directory that only has .tac/doctor-history.jsonl
    const wtDir = join(tac, "worktrees", "M042");
    const wtTacDir = join(wtDir, ".tac");
    mkdirSync(wtTacDir, { recursive: true });
    writeFileSync(join(wtTacDir, "doctor-history.jsonl"), '{"ts":"2026-01-01","ok":true}\n');

    const result = await runTACDoctor(base, { fix: false });
    ...
    assert.equal(orphanIssues.length, 0,
      "should not warn about worktree dir that only contains .tac/doctor-history.jsonl");
