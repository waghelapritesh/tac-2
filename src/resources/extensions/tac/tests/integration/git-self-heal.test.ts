...
function makeTempRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-self-heal-"));
...

{
  const cases: Array<{ input: string; shouldContain: string; label: string }> = [
    { input: "CONFLICT (content): Merge conflict in file.ts", shouldContain: "/tac doctor", label: "merge conflict" },
    { input: "error: pathspec 'foo' did not match any file(s)", shouldContain: "/tac doctor", label: "checkout failure" },
    { input: "HEAD detached at abc123", shouldContain: "/tac doctor", label: "detached HEAD" },
    { input: "Unable to create '/path/.git/index.lock': File exists", shouldContain: "/tac doctor", label: "lock file" },
    { input: "fatal: not a git repository", shouldContain: "/tac doctor", label: "not a repo" },
    { input: "some unknown error", shouldContain: "/tac doctor", label: "unknown error" },
  ];

  for (const { input, shouldContain, label } of cases) {
    const result = formatGitError(input);
    assert.ok(result.includes(shouldContain), `${label}: should suggest /tac doctor`);
    console.log(`  ✓ ${label} → suggests /tac doctor`);
  }

  // Test with Error object
  const result = formatGitError(new Error("CONFLICT in merge"));
  assert.ok(result.includes("/tac doctor"), "should handle Error objects");
