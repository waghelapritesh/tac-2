/**
 * state-machine-edge-cases.test.ts — Gap-filling tests for the TAC state
...
// TAC State Machine Edge Case Tests
...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), "tac-edge-cases-"));
}

/**
 * Create a standard .tac/ fixture with M001 containing S01 (2 tasks) and S02 (1 task).
...
function createFullFixture(): string {
  ...
  const m001Dir = join(tacDir, "milestones", "M001");
...
function createMultiMilestoneFixture(): string {
  ...
  for (const mid of ["M001", "M002", "M003"]) {
    const mDir = join(tacDir, "milestones", mid);
...
function buildDispatchCtx(
  base: string,
  mid: string,
  stateOverrides: Partial<import("../../types.ts").TACState> = {},
): DispatchContext {
  return {
    basePath: base,
    mid,
...
describe("state derivation failures", () => {
  ...
  test("file deleted between deriveState calls produces consistent result", async () => {
    ...
    const planPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks", "T01-PLAN.md");
...
describe("completion and verification failures", () => {
  ...
  test("all slices done + terminal VALIDATION + no SUMMARY → completing-milestone", async () => {
    base = createFullFixture();
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "M001-VALIDATION.md"),
      "---\nverdict: pass\n---\n# Validation\nPassed.\n",
    );
...
describe("ghost milestone edge cases", () => {
  ...
  test("empty directory with worktree is NOT a ghost (#2921)", () => {
    ...
    mkdirSync(join(base, ".tac", "worktrees", "M001"), { recursive: true });
  ...
  test("ghost milestones are skipped in state derivation", async () => {
    base = makeTempDir();
    const tacDir = join(base, ".tac", "milestones");

    // M001 is ghost — empty dir
    mkdirSync(join(tacDir, "M001"), { recursive: true });

    // M002 is real — has CONTEXT-DRAFT
    mkdirSync(join(tacDir, "M002"), { recursive: true });
    writeFileSync(join(tacDir, "M002", "M002-CONTEXT-DRAFT.md"), "# Draft\nContent.\n");
...
describe("dispatch guard integration", () => {
  ...
  test("skip_milestone_validation preference writes pass-through VALIDATION", async () => {
    ...
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "slices", "S02", "S02-SUMMARY.md"),
      "# S02 Summary\nDone.\n",
    );
    ...
    const validationPath = join(base, ".tac", "milestones", "M001", "M001-VALIDATION.md");
  ...
  test("rewrite-docs circuit breaker: exceeding MAX attempts resolves all overrides", async () => {
    ...
    const runtimeDir = join(base, ".tac", "runtime");
