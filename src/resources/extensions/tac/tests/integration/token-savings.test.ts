...
import { openDatabase, closeDatabase } from '../../tac-db.ts';
...
{
  const base = mkdtempSync(join(tmpdir(), 'tac-token-savings-'));
  mkdirSync(join(base, '.tac'), { recursive: true });
  writeFileSync(join(base, '.tac', 'DECISIONS.md'), decisionsMarkdown);
  writeFileSync(join(base, '.tac', 'REQUIREMENTS.md'), requirementsMarkdown);
  writeFileSync(join(base, '.tac', 'PROJECT.md'), PROJECT_CONTENT);
  ...
  // ── Full-markdown equivalents (what inlineTacRootFile would return) ──
  const fullDecisionsContent = readFileSync(join(base, '.tac', 'DECISIONS.md'), 'utf-8');
  const fullRequirementsContent = readFileSync(join(base, '.tac', 'REQUIREMENTS.md'), 'utf-8');
