...
describe('worktree-e2e', async () => {
  ...
  try {
    ...
    console.log("\n=== Doctor: orphaned worktree detection ===");
    {
      ...
      const msDir = join(repo, ".tac", "milestones", "M001");
...
`);
      ...
      mkdirSync(join(repo, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b milestone/M001 .tac/worktrees/M001", repo);

      // Detect
      const detect = await runTACDoctor(repo, { isolationMode: "worktree" });
      ...
      // Fix
      const fixed = await runTACDoctor(repo, { fix: true, isolationMode: "worktree" });
      assert.ok(
