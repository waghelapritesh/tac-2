...
describe("auto-worktree lifecycle", () => {
  ...
  test("create → detect → teardown", () => {
    ...
    // Create .tac/milestones/M003 with a dummy file (simulates planning artifacts)
    const msDir = join(tempDir, ".tac", "milestones", "M003");
    ...
    assert.ok(
      existsSync(join(wtPath, ".tac", "milestones", "M003", "CONTEXT.md")),
  ...
  test("#1526: getMainBranch returns milestone/<MID> in auto-worktree", async () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".tac", "milestones", "M005");
    mkdirSync(msDir, { recursive: true });
  ...
  test("#1713: stale worktree directory without .git file", async () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".tac", "milestones", "M010");
    mkdirSync(msDir, { recursive: true });
  ...
  test("#778: reconcile plan checkboxes on re-attach", async () => {
    ...
    const planRelPath = join(".tac", "milestones", "M004", "slices", "S01", "S01-PLAN.md");
    const planDir = join(tempDir, ".tac", "milestones", "M004", "slices", "S01");
  ...
  test("#2791: mcp.json copied into worktree via copyPlanningArtifacts", () => {
    ...
    // Create mcp.json in .tac/ AFTER the commit (untracked, like real usage).
    // copyPlanningArtifacts should copy it into the worktree's .tac/.
    writeFileSync(
      join(tempDir, ".tac", "mcp.json"),
    ...
    try {
      assert.ok(
        existsSync(join(wtPath, ".tac", "mcp.json")),
        "mcp.json should be copied into worktree .tac/ on creation",
      );
    } finally {
      teardownAutoWorktree(tempDir, "M003");
    }
  ...
  test("#2791: mcp.json synced via syncTacStateToWorktree (ROOT_STATE_FILES)", () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".tac", "milestones", "M003");
    mkdirSync(msDir, { recursive: true });
    writeFileSync(join(msDir, "CONTEXT.md"), "# M003 Context\n");
    ...
    try {
      // Now add mcp.json to the main .tac/ after worktree was created
      writeFileSync(
        join(tempDir, ".tac", "mcp.json"),
        JSON.stringify({ servers: { test: { command: "echo" } } }),
      );
      ...
      const { synced } = syncTacStateToWorktree(tempDir, wtPath);
