 ...
 *
 * Creates real temp git repos with deliberate broken state, runs runTACDoctor,
...
import { closeDatabase, insertMilestone, insertSlice, openDatabase } from "../../tac-db.ts";
...
function createRepoWithCompletedMilestone(): string {
  ...
  // Create .tac structure with milestone M001 — all slices done → complete
...
/** Write a .tac/PREFERENCES.md with the given git isolation mode. */
function writePreferencesFile(dir: string, isolation: "none" | "worktree" | "branch"): void {
  const tacDir = join(dir, ".tac");
  mkdirSync(tacDir, { recursive: true });
  writeFileSync(join(tacDir, "PREFERENCES.md"), `---\ngit:\n  isolation: "${isolation}"\n---\n`);
}
...
describe('doctor-git', async () => {
  ...
  try {
    ...
    test('orphaned_auto_worktree', async () => {
      ...
      // Create worktree with milestone/M001 branch under .tac/worktrees/
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b milestone/M001 .tac/worktrees/M001", dir);

      const detect = await runTACDoctor(dir, { isolationMode: "worktree" });
      ...
      const fixed = await runTACDoctor(dir, { fix: true, isolationMode: "worktree" });
    ...
    console.log("\n=== orphaned_auto_worktree (cwd inside worktree) ===");
    {
      ...
      const wtPath = realpathSync(join(dir, ".tac", "worktrees", "M001"));
    ...
    test('tracked_runtime_files', async () => {
      ...
      run("git add -f .tac/activity/test.log", dir);
      ...
      const tracked = run("git ls-files .tac/activity/", dir);
    ...
    test('non-git directory', async () => {
      ...
      // Create minimal .tac structure (no git)
      mkdirSync(join(dir, ".tac"), { recursive: true });

      const result = await runTACDoctor(dir);
    ...
    test('active worktree safety', async () => {
      ...
      // Create worktree for in-progress milestone under .tac/worktrees/
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b milestone/M001 .tac/worktrees/M001", dir);
    ...
    test('completing-milestone worktree safety (DB-backed, no summary)', async () => {
      ...
      const dbPath = join(dir, ".tac", "tac.db");
      assert.equal(openDatabase(dbPath), true, "opens tac.db");
      try {
    ...
    // NOTE: loadEffectiveTACPreferences() resolves PROJECT_PREFERENCES_PATH
    // at module load time from process.cwd(). We write the prefs file to
    // the test runner's cwd .tac/PREFERENCES.md and clean up afterwards.
    if (process.platform !== "win32") {
    test('none-mode skips orphaned worktree', async () => {
      ...
      const result = await runTACDoctor(dir, { isolationMode: "none" });
    ...
    test('integration_branch_missing', async () => {
      ...
      const metaPath = join(dir, ".tac", "milestones", "M001", "M001-META.json");
    ...
    test('integration_branch_missing: stale metadata with configured fallback', async () => {
      ...
      writeFileSync(join(dir, ".tac", "PREFERENCES.md"), `---\ngit:\n  isolation: "worktree"\n  main_branch: "trunk"\n---\n`);
    ...
    test('worktree_directory_orphaned', async () => {
      ...
      const orphanDir = join(dir, ".tac", "worktrees", "orphan-feature");
    ...
    test('worktree_directory_orphaned (registered worktree not flagged)', async () => {
      ...
      // Create a real registered worktree under .tac/worktrees/
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/feature-1 .tac/worktrees/feature-1", dir);
    ...
    // ─── Test: Symlinked .tac does not cause false orphan detection ────
    if (process.platform !== "win32") {
    test('worktree_directory_orphaned (symlinked .tac not false-positive)', async () => {
      const dir = createRepoWithActiveMilestone();
      cleanups.push(dir);

      // Move .tac to an external location and replace with a symlink.
      // This simulates the ~/.tac/projects/<hash> layout where .tac is a symlink.
      const externalTac = join(realpathSync(mkdtempSync(join(tmpdir(), "doc-git-symlink-"))), "tac-data");
      cleanups.push(externalTac);
      renameSync(join(dir, ".tac"), externalTac);
      symlinkSync(externalTac, join(dir, ".tac"));

      // Create a real registered worktree under the (now symlinked) .tac/worktrees/
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/symlink-test .tac/worktrees/symlink-test", dir);

      const detect = await runTACDoctor(dir);
      const orphanDirIssues = detect.issues.filter(i => i.code === "worktree_directory_orphaned");
      assert.deepStrictEqual(orphanDirIssues.length, 0, "registered worktree via symlinked .tac NOT flagged as orphaned");
    });
    } else {
    }
    ...
    test('worktree_branch_merged', async () => {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/merged-feature .tac/worktrees/merged-feature", dir);
      const wtPath = join(dir, ".tac", "worktrees", "merged-feature");
    ...
    test('worktree_branch_merged (milestone branch cleanup)', async () => {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b milestone/M001 .tac/worktrees/M001", dir);
      const wtPath = join(dir, ".tac", "worktrees", "M001");
      writeFileSync(join(wtPath, "feature.txt"), "feature\n");
      run("git add -A", wtPath);
      run("git -c user.email=test@test.com -c user.name=Test commit -m \"feature work\"", wtPath);
    ...
    test('worktree_branch_merged (no false positive)', async () => {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/active-feature .tac/worktrees/active-feature", dir);
      const wtPath = join(dir, ".tac", "worktrees", "active-feature");
    ...
    test('legacy_slice_branches (fixable)', async () => {
      ...
      // Create legacy tac/M001/S01 branches
      run("git branch tac/M001/S01", dir);
      run("git branch tac/M001/S02", dir);
      // Active quick branches share tac/*/* shape and must NOT be deleted.
      run("git branch tac/quick/1-fix-typo", dir);
      ...
      const remaining = run("git branch --list tac/*/*", dir);
      assert.deepStrictEqual(remaining, "tac/quick/1-fix-typo", "quick branch preserved; legacy branches removed");
    });
    } else {
    }
    ...
    test('stale_uncommitted_changes (detected and auto-committed)', async () => {
      ...
      // Fix should create a tac snapshot commit
      const fixed = await runTACDoctor(dir, { fix: true });
      assert.ok(
        fixed.fixesApplied.some(f => f.includes("tac snapshot")),
        "fix creates a tac snapshot commit",
      );

      // Verify the snapshot commit was created with the tac snapshot tag
      const log = run("git log -1 --oneline", dir);
      assert.ok(log.includes("tac snapshot"), "commit is tagged with tac snapshot");
    });
