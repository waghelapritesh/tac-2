...
//
// Crosses ≥4 module boundaries: tac-db, md-importer, context-store, db-writer.
...
import { openDatabase, closeDatabase, isDbAvailable, _getAdapter } from '../../tac-db.ts';
...
test('integration-lifecycle: full pipeline', async () => {
    // ── Step 1: Set up temp dir with realistic .tac/ structure ──────────
    const base = mkdtempSync(join(tmpdir(), 'tac-int-lifecycle-'));
    const tacDir = join(base, '.tac');
    mkdirSync(tacDir, { recursive: true });
    mkdirSync(join(tacDir, 'milestones', 'M001'), { recursive: true });
    mkdirSync(join(tacDir, 'milestones', 'M002'), { recursive: true });
    ...
    writeFileSync(join(tacDir, 'DECISIONS.md'), decisionsMarkdown);
    writeFileSync(join(tacDir, 'REQUIREMENTS.md'), requirementsMarkdown);
    writeFileSync(join(tacDir, 'milestones', 'M001', 'M001-ROADMAP.md'), ROADMAP_CONTENT);

    const dbPath = join(tacDir, 'test-lifecycle.db');
    ...
    try {
      ...
      const fullDecisionsContent = readFileSync(join(tacDir, 'DECISIONS.md'), 'utf-8');
      const fullRequirementsContent = readFileSync(join(tacDir, 'REQUIREMENTS.md'), 'utf-8');
      ...
      appendFileSync(join(tacDir, 'DECISIONS.md'), '\n' + newDecisionRow + '\n');
      ...
      const regeneratedMd = readFileSync(join(tacDir, 'DECISIONS.md'), 'utf-8');
