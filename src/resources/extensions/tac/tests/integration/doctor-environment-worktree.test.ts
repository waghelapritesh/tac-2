 ...
 *
 * Fix: when the basePath contains `.tac/worktrees/`, resolve the project root
...
function createDir(files: Record<string, string> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-wt-env-"));
...
describe('doctor-environment-worktree', async () => {
  ...
  try {
    ...
    test('worktree: missing node_modules should NOT error when project root has them', () => {
      ...
      // Simulate a worktree inside .tac/worktrees/<name>/
      const worktreeDir = join(projectRoot, ".tac", "worktrees", "slice-abc");
    ...
    test('worktree: missing node_modules everywhere should still error', () => {
      ...
      const worktreeDir = join(projectRoot, ".tac", "worktrees", "slice-xyz");
      mkdirSync(worktreeDir, { recursive: true });
      writeFileSync(
        join(worktreeDir, "package.json"),
        JSON.stringify({ name: "test-project" }),
    ...
    test('worktree: checkEnvironmentHealth should not add env_dependencies for valid worktree', async () => {
      ...
      const worktreeDir = join(projectRoot, ".tac", "worktrees", "slice-pr");
      mkdirSync(worktreeDir, { recursive: true });
      writeFileSync(
        join(worktreeDir, "package.json"),
        JSON.stringify({ name: "test-project" }),
    ...
    // ── TAC_WORKTREE env var detection ─────────────────────────────────
    test('TAC_WORKTREE env: should resolve project root node_modules', () => {
      const projectRoot = createDir({
        "package.json": JSON.stringify({ name: "test-project" }),
      });
      mkdirSync(join(projectRoot, "node_modules"), { recursive: true });
      ...
      // Create a directory that doesn't have .tac/worktrees in path but
      // has TAC_WORKTREE env pointing to project root
      const someDir = createDir({
        "package.json": JSON.stringify({ name: "test-project" }),
      });
      cleanups.push(someDir);

      const origEnv = process.env.TAC_WORKTREE;
      try {
        process.env.TAC_WORKTREE = projectRoot;
        const results = runEnvironmentChecks(someDir);
        const depsCheck = results.find(r => r.name === "dependencies");
        assert.ok(
          depsCheck === undefined || depsCheck.status !== "error",
          "TAC_WORKTREE env allows fallback to project root node_modules",
        );
      } finally {
        if (origEnv === undefined) {
          delete process.env.TAC_WORKTREE;
        } else {
          process.env.TAC_WORKTREE = origEnv;
        }
      }
    });
