...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-test-${randomUUID()}`);
  // Create .tac/milestones/M001/slices/S01/tasks/ structure
...
test("resolveExpectedArtifactPath for run-uat returns ASSESSMENT path, not UAT (#2873)", (t) => {
  // The run-uat prompt instructs the agent to call tac_summary_save with
...
test("verifyExpectedArtifact passes for run-uat when ASSESSMENT file exists (#2873)", (t) => {
  // Regression test: run-uat writes S##-ASSESSMENT.md via tac_summary_save,
  // but verification looked for S##-UAT.md, causing false stuck retries.
  const base = makeTmpBase();
  t.after(() => cleanup(base));

  // Write the ASSESSMENT file (what tac_summary_save actually produces)
  const assessPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-ASSESSMENT.md");
...
test("buildLoopRemediationSteps returns steps for execute-task", (t) => {
  ...
  assert.ok(steps!.includes("tac undo-task"));
});
...
test("buildLoopRemediationSteps returns steps for plan-slice", (t) => {
  ...
  assert.ok(steps!.includes("tac recover"));
});
...
test("buildLoopRemediationSteps returns steps for complete-slice", (t) => {
  ...
  assert.ok(steps!.includes("tac reset-slice"));
});
...
test("verifyExpectedArtifact detects roadmap [x] change despite parse cache", (t) => {
  ...
  const roadmapPath = join(base, ".tac", "milestones", "M001", "M001-ROADMAP.md");
  writeFileSync(roadmapPath, roadmapAfter);
  const summaryPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md");
  writeFileSync(summaryPath, "# Summary\nDone.");
  const uatPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-UAT.md");
...
test("verifyExpectedArtifact rejects plan-slice with empty scaffold", (t) => {
  ...
  const sliceDir = join(base, ".tac", "milestones", "M001", "slices", "S01");
...
test("verifyExpectedArtifact plan-slice passes when all task plan files exist", (t) => {
  ...
  const tasksDir = join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks");
  const planPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md");
  const planContent = [
    "# S01: Test Slice",
    "",
    "## Tasks",
...
test("verifyExpectedArtifact execute-task rejects heading-style plan without checked checkbox (#3607)", (t) => {
  ...
  // checkboxes prove tac_complete_task ran (#3607).
  assert.strictEqual(
  ...
  try {
    ...
    insertTask({
      ...
      planning: {
        ...
        estimate: "30m",
        files: ["src/resources/extensions/tac/markdown-renderer.ts"],
        verify: "node --test markdown-renderer.test.ts",
        inputs: ["src/resources/extensions/tac/tac-db.ts"],
        expectedOutput: ["src/resources/extensions/tac/tests/markdown-renderer.test.ts"],
    ...
    insertTask({
      ...
      planning: {
        ...
        estimate: "20m",
        files: ["src/resources/extensions/tac/auto-recovery.ts"],
        verify: "node --test auto-recovery.test.ts",
        inputs: ["src/resources/extensions/tac/auto-recovery.ts"],
        expectedOutput: ["src/resources/extensions/tac/tests/auto-recovery.test.ts"],
...
test("#793: invalidateAllCaches clears all caches so deriveState sees fresh disk state", async (t) => {
  ...
  const mid = "M001";
  const sid = "S01";
  const planDir = join(base, ".tac", "milestones", mid, "slices", sid);
  const tasksDir = join(planDir, "tasks");
  mkdirSync(tasksDir, { recursive: true });
  mkdirSync(join(base, ".tac", "milestones", mid), { recursive: true });

  writeFileSync(
    join(base, ".tac", "milestones", mid, `${mid}-ROADMAP.md`),
...
function makeGitBase(): string {
  const base = join(tmpdir(), `tac-test-git-${randomUUID()}`);
...
test("hasImplementationArtifacts returns 'absent' when only .tac/ files committed (#1703)", (t) => {
  const base = makeGitBase();
  t.after(() => cleanup(base));

  // Create a feature branch and commit only .tac/ files
  execFileSync("git", ["checkout", "-b", "feat/test-milestone"], { cwd: base, stdio: "ignore" });
  mkdirSync(join(base, ".tac", "milestones", "M001"), { recursive: true });
  writeFileSync(join(base, ".tac", "milestones", "M001", "M001-ROADMAP.md"), "# Roadmap");
  writeFileSync(join(base, ".tac", "milestones", "M001", "M001-SUMMARY.md"), "# Summary");
  execFileSync("git", ["add", "."], { cwd: base, stdio: "ignore" });
  execFileSync("git", ["commit", "-m", "chore: add plan files"], { cwd: base, stdio: "ignore" });

  const result = hasImplementationArtifacts(base);
  assert.equal(result, "absent", "should return 'absent' when only .tac/ files were committed");
});
...
test("hasImplementationArtifacts returns 'present' when implementation files committed (#1703)", (t) => {
  ...
  // Create a feature branch with both .tac/ and implementation files
...
test("hasImplementationArtifacts returns 'unknown' on non-git directory (fail-open)", (t) => {
  const base = join(tmpdir(), `tac-test-nogit-${randomUUID()}`);
  mkdirSync(base, { recursive: true });
  t.after(() => cleanup(base));
...
test("verifyExpectedArtifact complete-milestone fails with only .tac/ files (#1703)", (t) => {
  const base = makeGitBase();
  t.after(() => cleanup(base));

  // Create feature branch with only .tac/ files
  execFileSync("git", ["checkout", "-b", "feat/ms-only-tac"], { cwd: base, stdio: "ignore" });
  mkdirSync(join(base, ".tac", "milestones", "M001"), { recursive: true });
  writeFileSync(join(base, ".tac", "milestones", "M001", "M001-SUMMARY.md"), "# Milestone Summary\nDone.");
  execFileSync("git", ["add", "."], { cwd: base, stdio: "ignore" });
  ...
  assert.equal(result, false, "complete-milestone should fail verification when only .tac/ files present");
});
