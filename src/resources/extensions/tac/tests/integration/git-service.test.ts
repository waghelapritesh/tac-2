...
describe('git-service', async () => {
  ...
  test('buildTaskCommitMessage', () => {
    ...
    assert.ok(!msg.includes("(S01/T02)"), "no TAC ID in subject line");
    ...
    assert.ok(msg.includes("TAC-Task: S01/T02"), "TAC-Task trailer in body");
  });

  {
    ...
    assert.ok(msg.includes("TAC-Task: S02/T01"), "TAC-Task trailer present");
  }

  {
    ...
    assert.ok(msg.includes("TAC-Task: S01/T03"), "TAC-Task trailer present");
  }
  ...
  assert.ok(
    RUNTIME_EXCLUSION_PATHS.includes(".tac/activity/"),
    "includes .tac/activity/"
  );
  assert.ok(
    RUNTIME_EXCLUSION_PATHS.includes(".tac/STATE.md"),
    "includes .tac/STATE.md"
  );

  // ─── runGit ────────────────────────────────────────────────────────────


  const tempDir = mkdtempSync(join(tmpdir(), "tac-git-service-test-"));
  ...
  function initTempRepo(): string {
    const dir = mkdtempSync(join(tmpdir(), "tac-git-t02-"));
  ...
  test('GitServiceImpl: smart staging', () => {
    ...
    createFile(repo, ".tac/activity/log.jsonl", "log data");
    createFile(repo, ".tac/runtime/state.json", '{"state":true}');
    createFile(repo, ".tac/STATE.md", "# State");
    createFile(repo, ".tac/auto.lock", "lock");
    createFile(repo, ".tac/metrics.json", "{}");
    createFile(repo, ".tac/worktrees/wt/file.txt", "wt data");
    ...
    assert.ok(!showStat.includes(".tac/activity"), ".tac/activity/ excluded from commit");
    assert.ok(!showStat.includes(".tac/runtime"), ".tac/runtime/ excluded from commit");
    assert.ok(!showStat.includes("STATE.md"), ".tac/STATE.md excluded from commit");
    assert.ok(!showStat.includes("auto.lock"), ".tac/auto.lock excluded from commit");
    assert.ok(!showStat.includes("metrics.json"), ".tac/metrics.json excluded from commit");
    assert.ok(!showStat.includes(".tac/worktrees"), ".tac/worktrees/ excluded from commit");

    // Verify runtime files are still untracked
    // git status --short may collapse to "?? .tac/" or show individual files
    // Use --untracked-files=all to force individual listing
    const statusOut = run("git status --short --untracked-files=all", repo);
    assert.ok(statusOut.includes(".tac/activity/"), "activity still untracked after commit");
    assert.ok(statusOut.includes(".tac/runtime/"), "runtime still untracked after commit");
    assert.ok(statusOut.includes(".tac/STATE.md"), "STATE.md still untracked after commit");
  ...
  test('GitServiceImpl: smart staging excludes tracked runtime files', () => {
    // Reproduces the real bug: .tac/ runtime files that are already tracked
    // (in the git index) must be excluded from staging even when .tac/ is
    ...
    // 3. Pre-checkout discard: git checkout -- .tac/ clears dirty runtime files

    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);

    // Simulate a repo where .tac/ files were previously force-added
    createFile(repo, ".tac/metrics.json", '{"version":1}');
    createFile(repo, ".tac/completed-units.json", '["unit1"]');
    createFile(repo, ".tac/activity/log.jsonl", '{"ts":1}');
    createFile(repo, "src/real.ts", "real code");
    // Force-add .tac/ files to simulate historical tracking
    runGit(repo, ["add", "-f", ".tac/metrics.json", ".tac/completed-units.json", ".tac/activity/log.jsonl", "src/real.ts"]);
    runGit(repo, ["commit", "-F", "-"], { input: "init with tracked runtime files" });

    // Add .gitignore with .tac/ (matches real-world setup from ensureGitignore)
    createFile(repo, ".gitignore", ".tac/\n");
    runGit(repo, ["add", ".gitignore"]);
    runGit(repo, ["commit", "-F", "-"], { input: "add gitignore" });

    // Verify runtime files are tracked (precondition)
    const tracked = run("git ls-files .tac/", repo);
    ...
    createFile(repo, ".tac/metrics.json", '{"version":2}');
    createFile(repo, ".tac/completed-units.json", '["unit1","unit2"]');
    createFile(repo, ".tac/activity/log.jsonl", '{"ts":2}');
    ...
    const trackedAfter = run("git ls-files .tac/", repo);
    assert.deepStrictEqual(trackedAfter, "", "no .tac/ runtime files remain in the index");

    // Verify a second autoCommit with changed runtime files does NOT stage them
    createFile(repo, ".tac/metrics.json", '{"version":3}');
    createFile(repo, ".tac/completed-units.json", '["unit1","unit2","unit3"]');
  ...
  test('GitServiceImpl: autoCommit on dirty repo', () => {
    ...
    assert.deepStrictEqual(msg, "chore: auto-commit after task\n\nTAC-Unit: T01", "autoCommit returns generic format with trailer");
    ...
    assert.ok(msg2!.includes("TAC-Task: S01/T02"), "meaningful commit has TAC-Task trailer");
  ...
  test('GitServiceImpl: empty-after-staging guard', () => {
    ...
    createFile(repo, ".tac/activity/x.jsonl", "data");
  ...
  test('GitServiceImpl: autoCommit with extraExclusions', () => {
    ...
    // Create both a .tac/ planning file and a regular source file
    createFile(repo, ".tac/milestones/M001/M001-ROADMAP.md", "- [x] S01");
    createFile(repo, "src/feature.ts", "export const y = 2;");

    // Auto-commit with .tac/ excluded (simulates pre-switch)
    const msg = svc.autoCommit("pre-switch", "main", [".tac/"]);
    assert.deepStrictEqual(msg, "chore: auto-commit after pre-switch\n\nTAC-Unit: main", "pre-switch autoCommit with .tac/ exclusion commits");

    // Verify .tac/ file was NOT committed
    const show = run("git show --stat HEAD", repo);
    assert.ok(!show.includes("ROADMAP"), ".tac/ files excluded from pre-switch auto-commit");
    assert.ok(show.includes("feature.ts"), "non-.tac/ files included in pre-switch auto-commit");

    rmSync(repo, { recursive: true, force: true });
  });

  // ─── GitServiceImpl: autoCommit extraExclusions — only .tac/ dirty ────

  test('GitServiceImpl: autoCommit extraExclusions — only .tac/ dirty', () => {
    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);

    // Create only .tac/ planning files
    createFile(repo, ".tac/milestones/M001/M001-ROADMAP.md", "- [x] S01");
    createFile(repo, ".tac/STATE.md", "state content");

    // Auto-commit with .tac/ excluded — nothing else to commit
    const result = svc.autoCommit("pre-switch", "main", [".tac/"]);
    assert.deepStrictEqual(result, null, "autoCommit returns null when only .tac/ files are dirty and excluded");
  ...
  function initBranchTestRepo(): string {
    const dir = mkdtempSync(join(tmpdir(), "tac-git-t03-"));
    runGit(dir, ["init", "-b", "main"]);
    runGit(dir, ["config", "user.name", "Pi Test"]);
    runGit(dir, ["config", "user.email", "pi@example.com"]);
    createFile(dir, ".gitkeep", "");
  ...
  test('Branch queries', () => {
    ...
    run("git checkout -b tac/M001/S01", repo);
    assert.deepStrictEqual(svc.getCurrentBranch(), "tac/M001/S01", "getCurrentBranch returns slice branch name");
  ...
  {
    // master-only repo
    const repo = mkdtempSync(join(tmpdir(), "tac-git-t03-master-"));
  ...
  test('createSnapshot: enabled by default when prefs omitted', () => {
    ...
    svc.createSnapshot("tac/M001/S01");

    // Verify ref exists under refs/tac/snapshots/
    const refs = run("git for-each-ref refs/tac/snapshots/", repo);
    assert.ok(refs.includes("refs/tac/snapshots/tac/M001/S01/"), "snapshot ref created under refs/tac/snapshots/");
  ...
  test('Integration branch: rejects slice branches', () => {
    ...
    writeIntegrationBranch(repo, "M001", "tac/M001/S01");
  ...
  test('Integration branch: records non-ephemeral tac branches', () => {
    const repo = initBranchTestRepo();
  ...
  test('untrackRuntimeFiles', async () => {
    ...
    const repo = mkdtempSync(join(tmpdir(), "tac-untrack-"));
    runGit(repo, ["init", "-b", "main"]);
    runGit(repo, ["config", "user.email", "test@test.com"]);
    runGit(repo, ["config", "user.name", "Test"]);

    // Create and track runtime files (simulates pre-.gitignore state)
    mkdirSync(join(repo, ".tac", "activity"), { recursive: true });
    mkdirSync(join(repo, ".tac", "runtime"), { recursive: true });
    writeFileSync(join(repo, ".tac", "completed-units.json"), '["u1"]');
    writeFileSync(join(repo, ".tac", "metrics.json"), '{}');
    writeFileSync(join(repo, ".tac", "STATE.md"), "# State");
    writeFileSync(join(repo, ".tac", "activity", "log.jsonl"), "{}");
    writeFileSync(join(repo, ".tac", "runtime", "data.json"), "{}");
    writeFileSync(join(repo, "src.ts"), "code");
    runGit(repo, ["add", "-A"]);
    runGit(repo, ["commit", "-m", "init"]);

    // Precondition: runtime files are tracked
    const trackedBefore = run("git ls-files .tac/", repo);
    ...
    assert.ok(existsSync(join(repo, ".tac", "completed-units.json")),
      "untrack: completed-units.json still on disk");
    assert.ok(existsSync(join(repo, ".tac", "metrics.json")),
  ...
  test('smartStage excludes runtime files, allows milestone artifacts', () => {
    const repo = mkdtempSync(join(tmpdir(), "tac-smart-stage-excludes-"));
    runGit(repo, ["init", "-b", "main"]);
    runGit(repo, ["config", "user.email", "test@test.com"]);
    runGit(repo, ["config", "user.name", "Test"]);
    ...
    // Create .tac/ runtime files + milestone artifacts + a normal source file
    mkdirSync(join(repo, ".tac", "milestones", "M001"), { recursive: true });
    mkdirSync(join(repo, ".tac", "runtime"), { recursive: true });
    mkdirSync(join(repo, ".tac", "activity"), { recursive: true });
    writeFileSync(join(repo, ".tac", "milestones", "M001", "ROADMAP.md"), "# Roadmap");
    writeFileSync(join(repo, ".tac", "PREFERENCES.md"), "---\nversion: 1\n---");
    writeFileSync(join(repo, ".tac", "STATE.md"), "# State");
    writeFileSync(join(repo, ".tac", "runtime", "units.json"), "{}");
    writeFileSync(join(repo, ".tac", "activity", "log.jsonl"), "{}");
    writeFileSync(join(repo, "src.ts"), "const x = 1;");

    // smartStage excludes only runtime paths, not all of .tac/ (#1326)
    const svc = new GitServiceImpl(repo);
    ...
    assert.ok(!committed.includes(".tac/STATE.md"), "smartStage: STATE.md excluded (runtime)");
    assert.ok(!committed.includes(".tac/runtime/"), "smartStage: runtime/ excluded");
    assert.ok(!committed.includes(".tac/activity/"), "smartStage: activity/ excluded");
    // Milestone artifacts SHOULD be committed when not gitignored (#1326)
    assert.ok(committed.includes(".tac/milestones/"), "smartStage: milestone artifacts ARE committed");
  ...
  test('writeIntegrationBranch: no commit', () => {
    ...
    // No commit — .tac/ is managed externally
  ...
  // ─── ensureGitignore: always adds .tac to gitignore ──────────────────

  test('ensureGitignore: adds .tac entry', async () => {
    const { ensureGitignore } = await import("../../gitignore.ts");
    const repo = mkdtempSync(join(tmpdir(), "tac-gitignore-external-state-"));

    // Should add .tac to gitignore (external state dir is a symlink)
    ...
    assert.ok(lines.includes(".tac"), "ensureGitignore: .gitignore contains .tac");
  ...
  // ─── nativeAddAllWithExclusions: symlinked .tac fallback ───────────────

  test('nativeAddAllWithExclusions: symlinked .tac fallback', () => {
    // When .tac is a symlink, git rejects `:!.tac/...` pathspecs with
    // "fatal: pathspec '...' is beyond a symbolic link". When `.tac` is
    // already gitignored, the fallback should still stage untracked real files.
    const repo = initTempRepo();

    // Create the real .tac directory outside the repo, then symlink it
    const externalTac = mkdtempSync(join(tmpdir(), "tac-external-"));
    mkdirSync(join(externalTac, "activity"), { recursive: true });
    writeFileSync(join(externalTac, "activity", "log.jsonl"), "log data");
    writeFileSync(join(externalTac, "STATE.md"), "# State");

    // Symlink .tac -> external directory
    symlinkSync(externalTac, join(repo, ".tac"));

    // Add .gitignore so .tac/ is ignored
    writeFileSync(join(repo, ".gitignore"), ".tac\n");
    ...
    // because `.tac` itself is already protected by .gitignore.
    createFile(repo, "src/new-feature.ts", "export const fresh = true;");

    // nativeAddAllWithExclusions should NOT throw despite .tac being a symlink
    let threw = false;
    try {
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions does not throw with symlinked .tac");

    // Verify the tracked modified file was staged
    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/app.ts"), "modified tracked file staged despite symlinked .tac");

    assert.ok(staged.includes("src/new-feature.ts"),
      "symlink fallback should still stage new real files when .tac is gitignored");
    assert.ok(!staged.includes(".tac"), ".tac content not staged");

    rmSync(repo, { recursive: true, force: true });
    rmSync(externalTac, { recursive: true, force: true });
  });

  test('nativeAddAllWithExclusions: self-heals symlinked .tac when .gitignore lacks it (#4423)', () => {
    // When `.tac` is a symlink AND not listed in `.gitignore`, the staging
    // fallback must self-heal by appending `.tac` to `.gitignore` and retrying
    // `git add -A`. Without this, new user files are silently dropped.
    const repo = initTempRepo();

    const externalTac = mkdtempSync(join(tmpdir(), "tac-external-unignored-"));
    mkdirSync(join(externalTac, "activity"), { recursive: true });
    writeFileSync(join(externalTac, "activity", "log.jsonl"), "log data");
    writeFileSync(join(externalTac, "STATE.md"), "# State");
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions does not throw with symlinked .tac when .tac is not gitignored");

    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/app.ts"), "tracked modifications stage");
    assert.ok(
      staged.includes("src/new-feature.ts"),
      "self-heal adds .tac to .gitignore so new user files are staged",
    );
    assert.ok(!staged.includes(".tac"), ".tac contents stay unstaged after self-heal");

    // Verify the self-heal actually wrote to .gitignore
    const gitignore = readFileSync(join(repo, ".gitignore"), "utf-8");
    assert.ok(/^\.tac\/?$/m.test(gitignore), ".gitignore contains .tac entry after self-heal");
  ...
  test('nativeAddAllWithExclusions: explicit staging protects work when manage_gitignore:false (#4423)', () => {
    ...
    // by explicitly staging untracked real files while skipping `.tac`.
    const repo = initTempRepo();

    const externalTac = mkdtempSync(join(tmpdir(), "tac-external-optout-"));
    mkdirSync(join(externalTac, "activity"), { recursive: true });
    writeFileSync(join(externalTac, "activity", "log.jsonl"), "log data");
    writeFileSync(join(externalTac, "STATE.md"), "# State");
    ...
    // Create PREFERENCES.md inside the symlink target (the linked .tac dir)
    // with the opt-out flag. The regex matches a top-level occurrence.
    writeFileSync(
      join(repo, ".tac", "PREFERENCES.md"),
    ...
    assert.ok(!staged.includes(".tac"), ".tac contents stay unstaged");
  ...
  // ─── nativeAddAllWithExclusions: non-symlinked .tac still works ───────

  test('nativeAddAllWithExclusions: non-symlinked .tac still works', () => {
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions works with normal .tac directory");

    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/code.ts"), "real file staged with normal .tac");
  ...
  test('MergeConflictError: constructor fields', () => {
    ...
    assert.deepStrictEqual(err.branch, "tac/M001/S01", "MergeConflictError.branch set");
  ...
  // ─── Integration branch: rejects tac/quick/* branches ────────────────────

  test('Integration branch: rejects tac/quick/* branches', () => {
    const repo = initBranchTestRepo();

    writeIntegrationBranch(repo, "M001", "tac/quick/1234-some-task");
    assert.deepStrictEqual(readIntegrationBranch(repo, "M001"), null, "tac/quick/* branches are not recorded as integration branch");
  ...
  test('buildTaskCommitMessage: issueNumber appends Resolves trailer', () => {
    ...
    assert.ok(msg.includes("TAC-Task: S01/T03"), "TAC-Task trailer present");
    // TAC-Task should come before Resolves
    const taskIdx = msg.indexOf("TAC-Task: S01/T03");
    const resolvesIdx = msg.indexOf("Resolves #42");
    assert.ok(taskIdx < resolvesIdx, "TAC-Task trailer before Resolves trailer");
  });

  {
    ...
    assert.ok(msg.includes("TAC-Task: S01/T04"), "TAC-Task trailer still present");
  }
  ...
  // ─── autoCommit: symlinked .tac does NOT stage milestone artifacts (#2247) ──

  test('autoCommit: symlinked .tac does NOT stage milestone artifacts (#2247)', () => {
    // When .tac is a symlink (external state project), .tac/ files live outside
    // the repo by design. smartStage() must NOT force-stage them into git — the
    // .gitignore exclusion is correct and intentional.
    const repo = initTempRepo();

    // Create an external .tac directory and symlink it into the repo
    const externalTac = mkdtempSync(join(tmpdir(), "tac-external-symlink-"));
    mkdirSync(join(externalTac, "milestones", "M009"), { recursive: true });
    mkdirSync(join(externalTac, "activity"), { recursive: true });
    mkdirSync(join(externalTac, "runtime"), { recursive: true });

    symlinkSync(externalTac, join(repo, ".tac"));

    // .gitignore blocks .tac (as ensureGitignore would do for symlink projects)
    writeFileSync(join(repo, ".gitignore"), ".tac\n");
    ...
    writeFileSync(join(externalTac, "milestones", "M009", "M009-SUMMARY.md"), "# M009 Summary");
    writeFileSync(join(externalTac, "milestones", "M009", "S01-SUMMARY.md"), "# S01 Summary");
    writeFileSync(join(externalTac, "milestones", "M009", "T01-VERIFY.json"), '{"passed":true}');
    ...
    assert.ok(!committed.includes(".tac/milestones/"),
      "symlink autoCommit: .tac/milestones/ files are NOT staged (external state stays external)");

    try { rmSync(repo, { recursive: true, force: true }); } catch {}
    try { rmSync(externalTac, { recursive: true, force: true }); } catch {}
  });

  // ─── autoCommit: absorbs preceding tac snapshot commits ─────────────────

  test('autoCommit: absorbs preceding tac snapshot commits', () => {
    const repo = initTempRepo();

    // Simulate 2 tac snapshot commits
    createFile(repo, "file1.ts", "v1");
    run("git add -A", repo);
    run('git commit -m "tac snapshot: uncommitted changes after 35m inactivity"', repo);

    createFile(repo, "file2.ts", "v2");
    run("git add -A", repo);
    run('git commit -m "tac snapshot: pre-dispatch, uncommitted changes after 40m inactivity"', repo);
    ...
    // No tac snapshot commits in log
    const log = run("git log --oneline", repo);
    assert.ok(!log.includes("tac snapshot"), "no tac snapshot commits remain in history");
