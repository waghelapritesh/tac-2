...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-e2e-parallel-'));
  mkdirSync(join(base, '.tac', 'milestones'), { recursive: true });
  return base;
}

function writeMetricsLedger(base: string, ledger: MetricsLedger): void {
  writeFileSync(join(base, '.tac', 'metrics.json'), JSON.stringify(ledger, null, 2));
}

function readMetricsLedger(base: string): MetricsLedger {
  return JSON.parse(readFileSync(join(base, '.tac', 'metrics.json'), 'utf-8'));
}
