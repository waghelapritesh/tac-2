...
function createTempRepoWithExternalTac(): { repo: string; externalState: string } {
  ...
  symlinkSync(externalState, join(repo, ".tac"));
...
describe("auto-worktree-milestone-merge", { timeout: 300_000 }, () => {
  ...
  function freshRepoWithExternalTac(): { repo: string; externalState: string } {
    const { repo, externalState } = createTempRepoWithExternalTac();
  ...
  test("basic squash merge — one commit on main", () => {
    ...
    const worktreeDir = join(repo, ".tac", "worktrees", "M010");
  ...
  test("rich commit message format", () => {
    ...
    assert.ok(result.commitMessage.includes("TAC-Milestone: M020"), "body has TAC-Milestone trailer");
    ...
    assert.ok(gitMsg.includes("TAC-Milestone: M020"), "git commit has TAC-Milestone trailer");
  ...
  test("auto-resolve .tac/ state file conflicts", () => {
    const repo = freshRepo();
    ...
    writeFileSync(join(wtPath, ".tac", "STATE.md"), "# State\n\n## Updated on milestone branch\n");
    run("git add .", wtPath);
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M050"), "merge commit created despite .tac conflict");
    } catch (err) {
      threw = true;
    }
    assert.ok(!threw, "auto-resolves .tac/ state file conflicts without throwing");
  ...
  test("skip checkout when main already current (#757)", () => {
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M060"), "merge commit created");
    } catch (err) {
      threw = true;
    }
  ...
  test("master-branch repo — no META.json, no prefs (#1668)", () => {
    ...
    const metaFile = join(dir, ".tac", "milestones", "M070", "M070-META.json");
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M070"), "merge commit created on master");
    } catch (err) {
      threw = true;
  ...
  test("#1738 bug 3: synced .tac/ dirs cleaned before merge", () => {
    const repo = freshRepo();
    ...
    const msDir = join(repo, ".tac", "milestones", "M090", "slices", "S01");
    mkdirSync(msDir, { recursive: true });
    writeFileSync(join(msDir, "S01-PLAN.md"), "# synced plan\n");
    writeFileSync(
      join(repo, ".tac", "milestones", "M090", "M090-ROADMAP.md"),
      "# synced roadmap\n",
    );

    const runtimeDir = join(repo, ".tac", "runtime", "units");
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M090"), "#1738 merge succeeds after cleaning synced dirs");
    } catch (err: unknown) {
      threw = true;
    }
    assert.ok(!threw, "#1738 merge does not fail on synced .tac/ files");
  ...
  test("#1738 e2e: dirty tree is stashed before merge (#2151)", () => {
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M100"), "#2151: merge succeeds after stashing dirty files");
    } catch {
      threw = true;
    }
  ...
  test("stale branch ref — fast-forward before squash merge (#1846)", () => {
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M140"), "merge commit created");
    } catch (err) {
      threw = true;
      errMsg = err instanceof Error ? err.message : String(err);
    }
  ...
  test("#1853 bug 1: SQUASH_MSG cleaned up after successful squash-merge", () => {
    ...
    assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M160"), "merge commit created");
  ...
  test("#1853 bug 2: uncommitted worktree changes committed before teardown", () => {
    ...
    assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("TAC-Milestone: M170"), "merge commit created");
  ...
  test("#1906: codeFilesChanged=false when only .tac/ metadata merged", () => {
    const repo = freshRepo();
    const wtPath = createAutoWorktree(repo, "M180");

    mkdirSync(join(wtPath, ".tac", "milestones", "M180"), { recursive: true });
    writeFileSync(
      join(wtPath, ".tac", "milestones", "M180", "SUMMARY.md"),
    ...
    assert.strictEqual(result.codeFilesChanged, false,
      "#1906: codeFilesChanged must be false when only .tac/ files were merged");
  });

  test("#2156: mergeMilestoneToMain removes external-state worktrees using the milestone branch name", () => {
    const { repo, externalState } = freshRepoWithExternalTac();
    ...
    assert.ok(
      ...
      `worktree should be registered under external .tac state, got ${realWtPath}`,
    );

    // Recreate the exact divergence from #1852: local .tac/ is replaced with a
    // stale real directory, so worktreePath() no longer matches git's record.
    unlinkSync(join(repo, ".tac"));
    mkdirSync(join(repo, ".tac", "worktrees", "M215"), { recursive: true });
    writeFileSync(join(repo, ".tac", "STATE.md"), "# Local stale state\n");
    writeFileSync(join(repo, ".tac", "worktrees", "M215", "stale.txt"), "stale local artifact\n");
  ...
  test("#2505: shelter retained when restore fails; cleaned up on success", () => {
    ...
    // Seed a queued (non-target) milestone in .tac/milestones/ that will be
    // sheltered during the merge and restored afterwards.
    const queuedDir = join(repo, ".tac", "milestones", "M201");
    ...
    assert.ok(!existsSync(join(repo, ".tac", ".milestone-shelter")), "shelter removed on successful restore");
