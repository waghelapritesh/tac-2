...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-integration-mixed-'));
...
function createGitRepo(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-integration-git-'));
  mkdirSync(join(base, '.tac', 'milestones'), { recursive: true });
...
`);
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M002-abc123'), { recursive: true });
...
test('Group 6: Branch name helpers with new-format IDs', () => {
    ...
    assert.deepStrictEqual(
      ...
      'tac/M001-abc123/S01',
      'G6: getSliceBranchName returns tac/M001-abc123/S01',
    );

    // Test parseSliceBranch with new-format branch name
    const parsed = parseSliceBranch('tac/M001-abc123/S01');
