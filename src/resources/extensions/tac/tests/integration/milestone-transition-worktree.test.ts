 ...
 * requires the @tac/pi-coding-agent runtime.
 */
...
function createTempRepo(): string {
  const dir = realpathSync(mkdtempSync(join(tmpdir(), "tac-mt-wt-test-")));
  run("git init", dir);
...
test("worktree swap on milestone transition: merge old, create new", () => {
  ...
  try {
    ...
    const roadmapPath = join(tempDir, ".tac", "milestones", "M001", "M001-ROADMAP.md");
