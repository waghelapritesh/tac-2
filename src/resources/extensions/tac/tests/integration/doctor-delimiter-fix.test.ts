 ...
 *
 * Verifies that `runTACDoctor({ fix: true })` sanitizes em/en dashes
...
test("doctor fix=true sanitizes em-dash in milestone title", async (t) => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-doctor-delim-"));
...
test("doctor fix=false still reports delimiter_in_title as warning", async (t) => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-doctor-delim-nf-"));
  const tac = join(tmpBase, ".tac");
  const mDir = join(tac, "milestones", "M001");
  const sDir = join(mDir, "slices", "S01");
  const tDir = join(sDir, "tasks");
