...
### R002 — Output Format
- Status: validated
- Description: Output matches TAC format.
`;
...
function createCompleteFixture(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-cmd-test-'));
...
test('Validation gating: non-existent path returns invalid', async () => {
    const fakePath = join(tmpdir(), 'tac-cmd-nonexistent-' + Date.now(), '.planning');
...
test('Full pipeline: parse → transform → preview → write → deriveState', async () => {
    const base = createCompleteFixture();
    const writeTarget = mkdtempSync(join(tmpdir(), 'tac-cmd-write-'));
    try {
      const planningPath = join(base, '.planning');
      ...
      // (e) Write
      const result = await writeTACDirectory(project, writeTarget);
      assert.ok(result.paths.length > 0, 'pipeline: files written');

      // Key files exist
      const tac = join(writeTarget, '.tac');
      assert.ok(existsSync(join(tac, 'PROJECT.md')), 'pipeline: PROJECT.md written');
      assert.ok(existsSync(join(tac, 'STATE.md')), 'pipeline: STATE.md written');
      assert.ok(existsSync(join(tac, 'REQUIREMENTS.md')), 'pipeline: REQUIREMENTS.md written');

      const m001 = join(tac, 'milestones', 'M001');
  ...
  // ─── Test 6: .tac/ exists detection ────────────────────────────────────

test('.tac/ exists detection', () => {
    const base = mkdtempSync(join(tmpdir(), 'tac-cmd-exists-'));
    try {
      // No .tac/ yet
      assert.ok(!existsSync(join(base, '.tac')), 'exists-detection: .tac absent initially');

      // Create .tac/
      mkdirSync(join(base, '.tac'), { recursive: true });
      assert.ok(existsSync(join(base, '.tac')), 'exists-detection: .tac detected after creation');
    } finally {
      rmSync(base, { recursive: true, force: true });
    }
});
