...
function createTempRepo(): string {
  ...
  // Mirror production: .tac/worktrees/ is gitignored so autoCommitDirtyState
...
/** Set up a milestone roadmap file in .tac/milestones/<MID>/ */
function setupRoadmap(repo: string, mid: string, title: string, slices: string[]): void {
  const dir = join(repo, ".tac", "milestones", mid);
...
test("mergeCompletedMilestone — clean merge, session status cleaned up", async () => {
  ...
  try {
    ...
    writeSessionStatus(repo, {
      ...
      worktreePath: join(repo, ".tac", "worktrees", "M010"),
    });
    ...
    // Verify commit on main (M010 is now in the body as a TAC-Milestone trailer)
    const log = run("git log -1 --format=%B main", repo);
    assert.ok(log.includes("TAC-Milestone: M010"), "commit message should reference M010 in trailer");
...
function setupWorktreeDb(basePath: string, mid: string): void {
  const wtTacDir = join(basePath, ".tac", "worktrees", mid, ".tac");
  mkdirSync(wtTacDir, { recursive: true });
  const dbPath = join(wtTacDir, "tac.db");
  openDatabase(dbPath);
