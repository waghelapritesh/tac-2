...
/**
 * Build a minimal .tac structure: milestone with one slice, one task
...
test("fixLevel:task — no reconciliation issue codes are reported", async (t) => {
  ...
  const report = await runTACDoctor(tmp, { fix: true, fixLevel: "task" });
...
test("fixLevel:all — no reconciliation issue codes are reported", async (t) => {
  ...
  const report = await runTACDoctor(tmp, { fix: true });
  ...
  const sliceSummaryPath = join(tmp, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md");
  ...
  const roadmapContent = readFileSync(join(tmp, ".tac", "milestones", "M001", "M001-ROADMAP.md"), "utf8");
...
`);
  ...
  const report = await runTACDoctor(tmp, { scope: "M001" });
...
test("doctor source treats skipped DB slices as closed and directory-optional", () => {
  const doctorSource = readFileSync(join(process.cwd(), "src/resources/extensions/tac/doctor.ts"), "utf8");
  assert.match(
    doctorSource,
