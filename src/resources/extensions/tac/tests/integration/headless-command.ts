/**
 * Integration test for `tac headless` CLI subcommand
 *
 * Validates that the headless CLI entry point works end-to-end:
 *   1. Creates a temp dir with a complete .tac/ project fixture
 ...
 *
 * Auth: Uses OAuth credentials from ~/.tac/agent/auth.json (Claude Code Max).
 * Falls back to ANTHROPIC_API_KEY env var if OAuth is not configured (D013).
 *
 * Usage:
 *   npx tsx src/resources/extensions/tac/tests/integration/headless-command.ts
...
// ── Fixture Data ─────────────────────────────────────────────────────────────
// A complete .tac/ project state that deriveState() can parse.
...
Headless proof test project. A minimal fixture used to validate TAC auto-mode via RPC.
...
Empty project with TAC milestone planned.
...
const FIXTURE_STATE_MD = `# TAC State
...
A minimal test project for validating TAC auto-mode in headless/RPC mode.
...
- Run TAC in headless mode and have it complete a trivial task
...
**Vision:** Prove TAC auto-mode works headlessly.

## Success Criteria

- Agent creates hello.txt with content "Hello from headless TAC"
...
- hello.txt created with content "Hello from headless TAC"

## Verification

- File hello.txt exists in project root with content "Hello from headless TAC"
...
- [ ] **T01: Create hello.txt** \`est:5m\`
  ...
  - Files: \`hello.txt\`
  - Do: Create a file called hello.txt in the project root with the content "Hello from headless TAC"
  - Verify: File exists with correct content
  - Done when: hello.txt exists with content "Hello from headless TAC"
...
Create a file called hello.txt in the project root with the content "Hello from headless TAC".

## Steps

1. Create the file hello.txt with the content "Hello from headless TAC"
...
- \`hello.txt\` — file containing "Hello from headless TAC"
`;

// ── Fixture Creation ─────────────────────────────────────────────────────────

function createFixture(): string {
  const tmpDir = mkdtempSync(join(tmpdir(), "tac-headless-cmd-"));

  // Initialize git repo (TAC requires it for branch-per-slice)
  execSync("git init -b main", { cwd: tmpDir, stdio: "pipe" });
  execSync('git config user.email "test@test.com"', { cwd: tmpDir, stdio: "pipe" });
  execSync('git config user.name "Test"', { cwd: tmpDir, stdio: "pipe" });

  // Create .tac/ structure
  const tacDir = join(tmpDir, ".tac");
  ...
  // Write fixture files
  writeFileSync(join(tacDir, "PROJECT.md"), FIXTURE_PROJECT_MD);
  writeFileSync(join(tacDir, "STATE.md"), FIXTURE_STATE_MD);
  ...
  // Initial commit so TAC has a clean git state
...
async function main(): Promise<void> {
  ...
  // Resolve tac-2 repo root (6 levels up from tests/integration/)
  const repoRoot = join(__dirname, "..", "..", "..", "..", "..", "..");

  console.log("=== TAC Headless Command Integration Test ===\n");
  ...
  const requiredFiles = [
    ".tac/PROJECT.md",
    ".tac/STATE.md",
    ".tac/milestones/M001/M001-CONTEXT.md",
    ".tac/milestones/M001/M001-ROADMAP.md",
    ".tac/milestones/M001/slices/S01/S01-PLAN.md",
  ...
  // Auth: prefer OAuth credentials from ~/.tac/agent/auth.json (D013).
  // Fall back to ANTHROPIC_API_KEY env var if present.
  const authJsonPath = join(homedir(), ".tac", "agent", "auth.json");
  let hasOAuth = false;
  ...
  if (hasOAuth) {
    console.log("  OK OAuth credentials found in ~/.tac/agent/auth.json (Claude Code Max)");
  } else if (process.env.ANTHROPIC_API_KEY) {
    console.log("  OK ANTHROPIC_API_KEY present (env var fallback)");
  } else {
    console.error("  FAIL: No auth available. Need either:");
    console.error("    - OAuth credentials in ~/.tac/agent/auth.json (Claude Code Max)");
  ...
  if (artifactExists) {
    ...
    const contentMatch = content === "Hello from headless TAC";
