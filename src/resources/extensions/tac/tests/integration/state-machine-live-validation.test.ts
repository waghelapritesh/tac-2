 ...
 * TAC state machine with real handlers, real DB, and real filesystem.
 *
...
// TAC State Machine Live Validation (#3161)
...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), "tac-live-validation-"));
}

/**
 * Create a realistic .tac/ fixture with:
...
describe("state-machine-live-validation", () => {
  ...
  describe("happy path: full lifecycle M001 → complete", () => {
    ...
    test("step 4: complete T01 in S01 — handler succeeds, DB reflects completion", async () => {
      ...
      const events = readEvents(join(base, ".tac", "event-log.jsonl"));
    ...
    test("step 8: complete milestone M001 — full lifecycle done", async () => {
      ...
      // SUMMARY.md on disk
      const summaryPath = join(base, ".tac", "milestones", "M001", "M001-SUMMARY.md");
