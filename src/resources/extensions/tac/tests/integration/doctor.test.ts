...
import { formatDoctorReport, runTACDoctor, summarizeDoctorIssues, filterDoctorIssues, selectDoctorScope, validateTitle } from "../../doctor.js";
const tmpBase = mkdtempSync(join(tmpdir(), "tac-doctor-test-"));
const tac = join(tmpBase, ".tac");
const mDir = join(tac, "milestones", "M001");
...
describe('doctor', async () => {
  test('doctor diagnose', async () => {
    const report = await runTACDoctor(tmpBase, { fix: false });
  ...
  test('doctor fix', async () => {
    const report = await runTACDoctor(tmpBase, { fix: true });
  ...
  test('doctor detects missing milestone summary', async () => {
    const msBase = mkdtempSync(join(tmpdir(), "tac-doctor-ms-test-"));
    const msTac = join(msBase, ".tac");
    const msMDir = join(msTac, "milestones", "M001");
...
`);
    ...
    const report = await runTACDoctor(msBase, { fix: false });
    assert.ok(
  ...
  test('doctor does NOT flag milestone with summary', async () => {
    const msBase = mkdtempSync(join(tmpdir(), "tac-doctor-ms-ok-test-"));
    const msTac = join(msBase, ".tac");
    const msMDir = join(msTac, "milestones", "M001");
    const msSDir = join(msMDir, "slices", "S01");
    const msTDir = join(msSDir, "tasks");
...
`);
  ...
  test('doctor detects blocker_discovered_no_replan', async () => {
    const bBase = mkdtempSync(join(tmpdir(), "tac-doctor-blocker-test-"));
    const bTac = join(bBase, ".tac");
    const bMDir = join(bTac, "milestones", "M001");
...
`);
    ...
    const report = await runTACDoctor(bBase, { fix: false });
  ...
  test('doctor does NOT flag blocker when REPLAN.md exists', async () => {
    const bBase = mkdtempSync(join(tmpdir(), "tac-doctor-blocker-ok-test-"));
    const bTac = join(bBase, ".tac");
    const bMDir = join(bTac, "milestones", "M001");
    const bSDir = join(bMDir, "slices", "S01");
    const bTDir = join(bSDir, "tasks");
...
`);
  ...
  test('doctor: done task with must-haves all addressed → no issue', async () => {
    const mhBase = mkdtempSync(join(tmpdir(), "tac-doctor-mh-ok-"));
    const mhTac = join(mhBase, ".tac");
    const mhMDir = join(mhTac, "milestones", "M001");
    ...
    const report = await runTACDoctor(mhBase, { fix: false });
    assert.ok(
  ...
  test('doctor: done task with must-haves NOT addressed → warning', async () => {
    const mhBase = mkdtempSync(join(tmpdir(), "tac-doctor-mh-fail-"));
    const mhTac = join(mhBase, ".tac");
    const mhMDir = join(mhTac, "milestones", "M001");
    const mhSDir = join(mhMDir, "slices", "S01");
    const mhTDir = join(mhSDir, "tasks");
  ...
  test('doctor: done task with no task plan file → no issue', async () => {
    const mhBase = mkdtempSync(join(tmpdir(), "tac-doctor-mh-noplan-"));
    const mhTac = join(mhBase, ".tac");
    const mhMDir = join(mhTac, "milestones", "M001");
    const mhSDir = join(mhMDir, "slices", "S01");
    const mhTDir = join(mhSDir, "tasks");
  ...
  test('doctor: done task with plan but no Must-Haves section → no issue', async () => {
    const mhBase = mkdtempSync(join(tmpdir(), "tac-doctor-mh-nosect-"));
    const mhTac = join(mhBase, ".tac");
    const mhMDir = join(mhTac, "milestones", "M001");
    const mhSDir = join(mhMDir, "slices", "S01");
    const mhTDir = join(mhSDir, "tasks");
  ...
  test('doctor detects em dash in milestone title', async () => {
    const dtBase = mkdtempSync(join(tmpdir(), "tac-doctor-dt-test-"));
    const dtTac = join(dtBase, ".tac");
    const dtMDir = join(dtTac, "milestones", "M001");
    ...
    const report = await runTACDoctor(dtBase, { fix: false });
  ...
  test('doctor detects em dash in slice title', async () => {
    const dtBase = mkdtempSync(join(tmpdir(), "tac-doctor-dt-slice-"));
    const dtTac = join(dtBase, ".tac");
    const dtMDir = join(dtTac, "milestones", "M001");
    const dtSDir = join(dtMDir, "slices", "S01");
    const dtTDir = join(dtSDir, "tasks");
  ...
  test('doctor does NOT flag milestone with clean title', async () => {
    const dtBase = mkdtempSync(join(tmpdir(), "tac-doctor-dt-clean-"));
    const dtTac = join(dtBase, ".tac");
    const dtMDir = join(dtTac, "milestones", "M001");
    const dtSDir = join(dtMDir, "slices", "S01");
    const dtTDir = join(dtSDir, "tasks");
  ...
  test('doctor: unresolvable_dependency warns for leftover range ID', async () => {
    ...
    const base = mkdtempSync(join(tmpdir(), "tac-doctor-udep-"));
    const mDir2 = join(base, ".tac", "milestones", "M001");
    ...
    const r = await runTACDoctor(base, { fix: false });
  ...
  test('doctor: no unresolvable_dependency for valid deps', async () => {
    const base = mkdtempSync(join(tmpdir(), "tac-doctor-udep-ok-"));
    const mDir2 = join(base, ".tac", "milestones", "M001");
    const sDir2 = join(mDir2, "slices", "S01");
    const tDir2 = join(sDir2, "tasks");
    mkdirSync(tDir2, { recursive: true });
