...
function createFeatureBranchRepo(featureBranch: string): string {
  const dir = realpathSync(mkdtempSync(join(tmpdir(), "tac-fb-lifecycle-")));
  run("git init", dir);
  ...
  // Mirror production: TAC runtime dirs are gitignored so autoCommitDirtyState
  // doesn't pick up the worktrees directory as dirty state (#1127 fix).
  writeFileSync(join(dir, ".gitignore"), ".tac/worktrees/\n");
...
describe('feature-branch-lifecycle-integration', async () => {
  ...
  try {
    ...
    test('Feature-branch lifecycle with unique milestone IDs', () => {
      ...
      // startAuto bootstraps .tac/ which commits .tac/ files. It also calls
      ...
      // TAC bootstraps .tac/ and captureIntegrationBranch commits metadata.
      ...
      mkdirSync(join(repo, ".tac", "milestones", milestoneId), { recursive: true });
      ...
      const worktreeDir = join(repo, ".tac", "worktrees", milestoneId);
    ...
    // ================================================================
    // Test 2: Uncommitted .tac/ planning files are available in worktree
    //
    // When auto-mode starts, .tac/ files may be untracked/uncommitted.
    ...
    test('Untracked planning files copied to worktree', () => {
      ...
      mkdirSync(join(repo, ".tac", "milestones", milestoneId, "slices", "S01", "tasks"), { recursive: true });
      writeFileSync(
        join(repo, ".tac", "milestones", milestoneId, `${milestoneId}-ROADMAP.md`),
        makeRoadmap(milestoneId, "Planning test", [{ id: "S01", title: "First" }]),
      );
      writeFileSync(
        join(repo, ".tac", "milestones", milestoneId, "slices", "S01", "S01-PLAN.md"),
        "# S01: First\n\n**Goal:** Test\n**Demo:** Test\n\n## Tasks\n- [ ] **T01: Do it** `est:10m`\n",
      );
      writeFileSync(join(repo, ".tac", "PROJECT.md"), "# Planning Test Project\n");
      writeFileSync(join(repo, ".tac", "DECISIONS.md"), "# Decisions\n\n## D001\nTest decision.\n");
      ...
      // With external state, worktree .tac is a symlink to shared state.
      // Verify symlink was created (planning files are shared, not copied).
      const wtTac = join(wtPath, ".tac");
      assert.ok(existsSync(wtTac), "worktree .tac exists (symlink or dir)");
    ...
    test('Multiple unique milestones on same feature branch', () => {
      ...
      mkdirSync(join(repo, ".tac", "milestones", mid1), { recursive: true });
      ...
      mkdirSync(join(repo, ".tac", "milestones", mid2), { recursive: true });
