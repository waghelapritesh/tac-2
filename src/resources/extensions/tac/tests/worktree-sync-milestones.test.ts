 ...
 * from the main repo's .tac/ into the worktree's .tac/ for the
 * specified milestone, and deletes tac.db so it rebuilds from fresh state.
 *
 ...
 *   - tac.db deleted in worktree after sync
 ...
 *   - syncWorktreeStateBack syncs root-level .tac/ files (REQUIREMENTS, PROJECT, etc.)
 ...
 *   - syncTacStateToWorktree syncs non-standard milestone dir names (#1547)
...
import { syncTacStateToWorktree, syncWorktreeStateBack } from '../auto-worktree.ts';
import { describe, test } from 'node:test';
import assert from 'node:assert/strict';


function createBase(name: string): string {
  const base = mkdtempSync(join(tmpdir(), `tac-wt-sync-${name}-`));
...
describe('worktree-sync-milestones', async () => {
  ...
  console.log('\n=== 1. milestone directory synced from main to worktree ===');
  {
    ...
    try {
      ...
      assert.ok(!existsSync(join(wtBase, '.tac', 'milestones', 'M001')), 'M001 missing before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001')), '#1311: M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 2. missing slices within milestone are synced ===');
  {
    ...
    try {
      ...
      const wtM001Dir = join(wtBase, '.tac', 'milestones', 'M001');
      mkdirSync(join(wtM001Dir, 'slices', 'S01'), { recursive: true });
      writeFileSync(join(wtM001Dir, 'slices', 'S01', 'S01-PLAN.md'), '# S01 Plan');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'slices', 'S02')), '#1311: S02 synced');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md')), 'S02 PLAN synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  // ─── 3. empty tac.db deleted in worktree after sync ────────────────────
  console.log('\n=== 3. empty tac.db deleted in worktree after sync ===');
  {
    const mainBase = createBase('main');
    const wtBase = createBase('wt');
    ...
    try {
      ...
      // Worktree has an empty (0-byte) tac.db — stale/corrupt
      writeFileSync(join(wtBase, '.tac', 'tac.db'), '');
      assert.ok(existsSync(join(wtBase, '.tac', 'tac.db')), 'tac.db exists before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(!existsSync(join(wtBase, '.tac', 'tac.db')), '#853: empty tac.db deleted after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  // ─── 3b. non-empty tac.db preserved in worktree after sync (#2815) ───
  console.log('\n=== 3b. non-empty tac.db preserved in worktree after sync (#2815) ===');
  {
    const mainBase = createBase('main');
    const wtBase = createBase('wt');
    ...
    try {
      ...
      // Worktree has a populated tac.db (e.g. from tac-migrate on respawn)
      writeFileSync(join(wtBase, '.tac', 'tac.db'), 'migrated-db-content');
      assert.ok(existsSync(join(wtBase, '.tac', 'tac.db')), 'tac.db exists before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.tac', 'tac.db')), '#2815: non-empty tac.db preserved after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  {
    const mainBase = createBase('main');
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-sync-wt-'));

    try {
      // Worktree has .tac/ but NO milestones/ subdirectory
      ...
      assert.ok(!existsSync(join(wtBase, '.tac', 'milestones')), 'milestones/ missing before sync');

      const result = syncTacStateToWorktree(mainBase, wtBase);

      assert.ok(existsSync(join(wtBase, '.tac', 'milestones')), 'milestones/ created in worktree');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001')), 'M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
  ...
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-wt-'));
    ...
    try {
      ...
      const wtSliceDir = join(wtBase, '.tac', 'milestones', 'M002', 'slices', 'S01');
      ...
      mkdirSync(join(mainBase, '.tac', 'milestones', 'M002'), { recursive: true });
      ...
      const mainSliceDir = join(mainBase, '.tac', 'milestones', 'M002', 'slices', 'S01');
  ...
  // ─── 9. syncWorktreeStateBack syncs root-level .tac/ files ──────────
  console.log('\n=== 9. syncWorktreeStateBack syncs root-level files (REQUIREMENTS, PROJECT) ===');
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-root-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-root-wt-'));

    try {
      mkdirSync(join(mainBase, '.tac', 'milestones', 'M001'), { recursive: true });
      mkdirSync(join(wtBase, '.tac', 'milestones', 'M001'), { recursive: true });

      // Main has original REQUIREMENTS and PROJECT
      writeFileSync(join(mainBase, '.tac', 'REQUIREMENTS.md'), '# Requirements\n## R001');
      writeFileSync(join(mainBase, '.tac', 'PROJECT.md'), '# Project\n## Milestone: M001');

      // Worktree has updated versions (complete-milestone added M002 refs)
      writeFileSync(join(wtBase, '.tac', 'REQUIREMENTS.md'), '# Requirements\n## R001\n## R002 — New req');
      writeFileSync(join(wtBase, '.tac', 'PROJECT.md'), '# Project\n## Milestone: M001\n## Milestone: M002');
      writeFileSync(join(wtBase, '.tac', 'KNOWLEDGE.md'), '# Knowledge\nLearned something.');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Root-level files should be overwritten with worktree versions
      const reqContent = readFileSync(join(mainBase, '.tac', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        reqContent.includes('R002'),
        'REQUIREMENTS.md updated with worktree content',
      );

      const projContent = readFileSync(join(mainBase, '.tac', 'PROJECT.md'), 'utf-8');
      assert.ok(
      ...
      assert.ok(
        existsSync(join(mainBase, '.tac', 'KNOWLEDGE.md')),
  ...
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-all-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-all-wt-'));

    try {
      mkdirSync(join(mainBase, '.tac', 'milestones'), { recursive: true });
      mkdirSync(join(wtBase, '.tac', 'milestones'), { recursive: true });
      ...
      const wtM002Dir = join(wtBase, '.tac', 'milestones', 'M002-abc123');
      ...
      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'milestones', 'M001')),
        'M001 missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'milestones', 'M002-abc123')),
      ...
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'milestones', 'M001', 'M001-SUMMARY.md')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.tac', 'milestones', 'M002-abc123', 'M002-abc123-CONTEXT.md')),
        'M002 CONTEXT synced to main (next-milestone fix)',
      );
      assert.ok(
        existsSync(join(mainBase, '.tac', 'milestones', 'M002-abc123', 'M002-abc123-ROADMAP.md')),
  ...
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-transition-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-transition-wt-'));
    ...
    try {
      ...
      const mainM006 = join(mainBase, '.tac', 'milestones', 'M006-589wvh');
      ...
      const wtM006 = join(wtBase, '.tac', 'milestones', 'M006-589wvh');
      ...
      const wtM007 = join(wtBase, '.tac', 'milestones', 'M007-wortc8');
      ...
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'milestones', 'M006-589wvh', 'M006-589wvh-SUMMARY.md')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.tac', 'milestones', 'M007-wortc8', 'M007-wortc8-CONTEXT.md')),
        'M007 CONTEXT synced to main (next-milestone)',
      );
      assert.ok(
        existsSync(join(mainBase, '.tac', 'milestones', 'M007-wortc8', 'M007-wortc8-ROADMAP.md')),
  ...
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-noroot-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-noroot-wt-'));
    ...
    try {
      ...
      writeFileSync(join(mainBase, '.tac', 'REQUIREMENTS.md'), '# Original');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Main's REQUIREMENTS should be untouched (worktree had nothing to sync)
      const content = readFileSync(join(mainBase, '.tac', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        content === '# Original',
  ...
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-queue-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-queue-wt-'));
    ...
    try {
      ...
      writeFileSync(join(wtBase, '.tac', 'QUEUE.md'), '# Queue\n- M002 next');
      writeFileSync(
        join(wtBase, '.tac', 'completed-units.json'),
        JSON.stringify({ units: [{ id: 'M001-S01-T01', completed: true }] }),
      );

      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'QUEUE.md')),
        'QUEUE.md missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.tac', 'completed-units.json')),
      ...
      const queueContent = readFileSync(join(mainBase, '.tac', 'QUEUE.md'), 'utf-8');
      assert.ok(
      ...
      const cuContent = readFileSync(join(mainBase, '.tac', 'completed-units.json'), 'utf-8');
      assert.ok(
  ...
  // ─── 14. syncTacStateToWorktree syncs non-standard milestone dir names (#1547) ──
  console.log('\n=== 14. syncTacStateToWorktree syncs non-standard milestone dir names (#1547) ===');
  {
    const mainBase = createBase('main');
    const wtBase = createBase('wt');
    ...
    try {
      ...
      const customDir = join(mainBase, '.tac', 'milestones', 'sprint-alpha');
      mkdirSync(customDir, { recursive: true });
      writeFileSync(join(customDir, 'CONTEXT.md'), '# Sprint Alpha Context');

      const suffixDir = join(mainBase, '.tac', 'milestones', 'M001-abc123');
      mkdirSync(suffixDir, { recursive: true });
      writeFileSync(join(suffixDir, 'M001-abc123-CONTEXT.md'), '# M001 Context');

      assert.ok(!existsSync(join(wtBase, '.tac', 'milestones', 'sprint-alpha')), 'sprint-alpha missing before sync');
      assert.ok(!existsSync(join(wtBase, '.tac', 'milestones', 'M001-abc123')), 'M001-abc123 missing before sync');

      const result = syncTacStateToWorktree(mainBase, wtBase);

      assert.ok(
        existsSync(join(wtBase, '.tac', 'milestones', 'sprint-alpha', 'CONTEXT.md')),
        '#1547: non-standard milestone dir "sprint-alpha" synced to worktree',
      );
      assert.ok(
        existsSync(join(wtBase, '.tac', 'milestones', 'M001-abc123', 'M001-abc123-CONTEXT.md')),
  ...
  console.log('\n=== 15. syncWorktreeStateBack syncs non-standard milestone dir names (#1547) ===');
  {
    const mainBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-custom-main-'));
    const wtBase = mkdtempSync(join(tmpdir(), 'tac-wt-back-custom-wt-'));
    ...
    try {
      ...
      const wtCustomDir = join(wtBase, '.tac', 'milestones', 'sprint-beta');
      mkdirSync(wtCustomDir, { recursive: true });
      writeFileSync(join(wtCustomDir, 'SUMMARY.md'), '# Sprint Beta Summary');

      assert.ok(
        !existsSync(join(mainBase, '.tac', 'milestones', 'sprint-beta')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.tac', 'milestones', 'sprint-beta', 'SUMMARY.md')),
