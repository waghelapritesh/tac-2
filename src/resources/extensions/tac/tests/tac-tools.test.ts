...
// tac-tools — Structured LLM tool tests
//
// Tests the three registered tools: tac_decision_save, tac_requirement_update, tac_summary_save.
...
function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-tools-'));
...
describe('tac-tools', () => {
  test('tac_decision_save', async () => {
    ...
    try {
      ...
      const mdPath = path.join(tmpDir, '.tac', 'DECISIONS.md');
  ...
  test('tac_requirement_update', async () => {
    const tmpDir = makeTmpDir();
    try {
      const dbPath = path.join(tmpDir, '.tac', 'tac.db');
      openDatabase(dbPath);
      ...
      const mdPath = path.join(tmpDir, '.tac', 'REQUIREMENTS.md');
  ...
  test('tac_summary_save', async () => {
    ...
    try {
      ...
      const filePath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-SUMMARY.md');
      ...
      const mFilePath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'M001-CONTEXT.md');
      ...
      const tFilePath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'tasks', 'T01-SUMMARY.md');
  ...
  test('tac_summary_save supports CONTEXT-DRAFT persistence', async () => {
    const tmpDir = makeTmpDir();
    try {
      const dbPath = path.join(tmpDir, '.tac', 'tac.db');
      openDatabase(dbPath);
      ...
      const draftPath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'M001-CONTEXT-DRAFT.md');
  ...
  test('tac_requirement_save creates new requirement', async () => {
    const tmpDir = makeTmpDir();
    try {
      const dbPath = path.join(tmpDir, '.tac', 'tac.db');
      openDatabase(dbPath);
  ...
  test('tac_requirement_update upserts when requirement not in DB', async () => {
    const tmpDir = makeTmpDir();
    try {
      const dbPath = path.join(tmpDir, '.tac', 'tac.db');
      openDatabase(dbPath);
