// TAC State Machine — Wave 5 Consistency Regression Tests
...
import { openDatabase, closeDatabase, upsertDecision, _getAdapter, insertMilestone, insertSlice, insertTask, getTask } from "../tac-db.js";
...
describe("upsertDecision preserves seq column", () => {
  test("seq is preserved when decision is re-upserted", () => {
    const tmp = mkdtempSync(join(tmpdir(), "tac-upsert-test-"));
    const dbPath = join(tmp, "tac.db");
    try {
      openDatabase(dbPath);
...
describe("WorkflowEvent v field", () => {
  test("appendEvent includes v:2 in output", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "tac-event-v-test-"));
    try {
      ...
      const logPath = join(tmp, ".tac", "event-log.jsonl");
