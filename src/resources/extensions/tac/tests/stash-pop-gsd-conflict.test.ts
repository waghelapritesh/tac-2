/**
 * stash-pop-tac-conflict.test.ts — Regression test for #2766.
 *
 * When a squash merge stash-pops and hits conflicts on .tac/ state files,
 * the UU entries block every subsequent merge. This test verifies that
 * mergeMilestoneToMain auto-resolves .tac/ conflicts by accepting HEAD
...
test("#2766: stash pop conflict on .tac/ files is auto-resolved", () => {
  ...
  try {
    ...
    // Modify .tac/STATE.md on the milestone branch (diverges from main)
    writeFileSync(join(wtPath, ".tac", "STATE.md"), "version: 2-milestone\n");
    run("git add .", wtPath);
    ...
    // Dirty .tac/STATE.md in the main repo (stash will conflict on pop)
    writeFileSync(join(repo, ".tac", "STATE.md"), "version: 2-main-dirty\n");

    const roadmap = makeRoadmap("M300", "Stash pop conflict test", [
      { id: "S01", title: "Feature" },
    ]);

    // mergeMilestoneToMain should succeed — .tac/ conflict auto-resolved
    const result = mergeMilestoneToMain(repo, "M300", roadmap);
    assert.ok(
      result.commitMessage.includes("TAC-Milestone: M300"),
      "merge succeeds despite stash pop conflict on .tac/ file",
    );
    ...
    assert.strictEqual(stashList, "", "stash is empty after .tac/ conflict auto-resolution");
  } finally {
    try { rmSync(repo, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }); } catch { /* cleanup best-effort */ }
  }
});

test("#2766: stash pop conflict on non-.tac files preserves stash for manual resolution", () => {
  const repo = createTempRepo();
  try {
    ...
    // and is NOT a .tac/ file, so it should be left for manual resolution
    writeFileSync(join(repo, "README.md"), "# locally modified\n");

    const roadmap = makeRoadmap("M301", "Non-tac stash conflict", [
    ...
    assert.ok(
      result.commitMessage.includes("TAC-Milestone: M301"),
      "merge succeeds even with non-.tac stash pop conflict",
    );
  } finally {
    try { rmSync(repo, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }); } catch { /* cleanup best-effort */ }
  }
