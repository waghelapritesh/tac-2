...
function makeDeps(
  ...
  const deps: WorktreeResolverDeps & { calls: CallLog[] } = {
    ...
    createAutoWorktree: (basePath: string, milestoneId: string) => {
      ...
      return `/project/.tac/worktrees/${milestoneId}`;
    },
    ...
    resolveMilestoneFile: (
      ...
      return `/project/.tac/milestones/${milestoneId}/${milestoneId}-ROADMAP.md`;
    },
    ...
    loadEffectiveTACPreferences: () => {
      calls.push({ fn: "loadEffectiveTACPreferences", args: [] });
...
test("workPath returns s.basePath", () => {
  const s = makeSession({ basePath: "/project/.tac/worktrees/M001" });
  const resolver = new WorktreeResolver(s, makeDeps());
  assert.equal(resolver.workPath, "/project/.tac/worktrees/M001");
});

test("projectRoot returns originalBasePath when set", () => {
  const s = makeSession({
    basePath: "/project/.tac/worktrees/M001",
...
test("enterMilestone creates new worktree when none exists", () => {
  ...
  assert.equal(s.basePath, "/project/.tac/worktrees/M001");
...
test("enterMilestone enters existing worktree instead of creating", () => {
  const s = makeSession();
  const deps = makeDeps({
    getAutoWorktreePath: () => "/project/.tac/worktrees/M001",
  });
  const ctx = makeNotifyCtx();
  const resolver = new WorktreeResolver(s, deps);
...
test("enterMilestone uses originalBasePath as base for worktree ops", () => {
  ...
  const deps = makeDeps({
    ...
    createAutoWorktree: (basePath: string, _mid: string) => {
      ...
      return "/project/.tac/worktrees/M002";
    },
  });
  const ctx = makeNotifyCtx();
  const resolver = new WorktreeResolver(s, deps);
...
test("enterMilestone does not create double-nested worktree when originalBasePath is empty and basePath is a worktree path", () => {
  ...
  // like /project/.tac/worktrees/M001/.tac/worktrees/M002.
  const wtPath = "/project/.tac/worktrees/M001";
  const s = makeSession({
    basePath: wtPath,
  ...
  // /project/.tac/worktrees/M001/.tac/worktrees/M002 (double-nesting).
  assert.ok(
    !createdFromPath.includes("/.tac/worktrees/"),
...
test("mergeAndExit resolves roadmap from worktree when missing at project root (#1573)", () => {
  ...
  const deps = makeDeps({
    ...
    resolveMilestoneFile: (basePath: string) => {
      ...
      if (basePath === "/project/.tac/worktrees/M001") {
        return "/project/.tac/worktrees/M001/.tac/milestones/M001/M001-ROADMAP.md";
      }
      return null;
    },
  });
...
test("mergeAndExit failure message references /tac dispatch complete-milestone, not /complete-milestone (#1891)", () => {
  ...
  // recovery command is "/tac dispatch complete-milestone".
  const s = makeSession({
    basePath: "/project/.tac/worktrees/M001",
    originalBasePath: "/project",
  });
  ...
  assert.ok(
    warning!.msg.includes("/tac dispatch complete-milestone"),
    "warning references /tac dispatch complete-milestone, not bare /complete-milestone",
  );
...
test("mergeAndExit warns when merge contains no code changes (#1906)", () => {
  ...
  assert.ok(
    ...
    "must emit warning when only .tac/ metadata was merged",
  );
  assert.ok(
...
test("mergeAndExit branch mode warns when merge contains no code changes (#1906)", () => {
  ...
  assert.ok(
    ctx.messages.some((m) => m.msg.includes("NO code changes") && m.level === "warning"),
    "branch mode must emit warning when only .tac/ metadata was merged",
  );
});
...
test("mergeAndEnterNext calls mergeAndExit then enterMilestone", () => {
  ...
  assert.equal(s.basePath, "/project/.tac/worktrees/M002");
});
...
test("GitService is rebuilt with the NEW basePath after enterMilestone", () => {
  ...
  assert.equal(gitServiceBasePath, "/project/.tac/worktrees/M001"); // new path, not old
});
