...
import { openDatabase, closeDatabase, insertMilestone, insertSlice, insertTask } from "../tac-db.ts";
import { clearPathCache } from "../paths.ts";
import { clearParseCache } from "../files.ts";

// loadPrompt reads from ~/.tac/agent/extensions/tac/prompts/ (main checkout).
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-complete-ms-test-"));
  mkdirSync(join(base, ".tac", "milestones"), { recursive: true });
  return base;
}

function writeRoadmap(base: string, mid: string, content: string): void {
  const dir = join(base, ".tac", "milestones", mid);
...
describe("complete-milestone", () => {
  ...
  test("prompt variable substitution", () => {
    ...
    assert.ok(prompt.includes(".tac/milestones/M001/M001-ROADMAP.md"), "prompt contains roadmapPath");
  ...
  test("prompt content integrity", () => {
    const prompt = loadPromptFromWorktree("complete-milestone", {
      ...
      roadmapPath: ".tac/milestones/M002/M002-ROADMAP.md",
  ...
  test("prompt contains verification gate that blocks completion on failure", () => {
    ...
    // Failure path must block tac_complete_milestone
    assert.ok(
      prompt.includes("Do NOT call `tac_complete_milestone`"),
  ...
  test("step 11 specifies write tool for PROJECT.md update (#2946)", () => {
    ...
    assert.ok(
      prompt.includes("`.tac/PROJECT.md`") || prompt.includes('".tac/PROJECT.md"'),
  ...
  test("milestoneCompleteExecute uses sanitized params", async () => {
    ...
    try {
      ...
      // Calling handleCompleteMilestone may throw TAC_STALE_STATE (no DB in test env)
      // but it should NOT throw TypeError from type mismatches — that's the bug fix.
      try {
        await handleCompleteMilestone(sanitized, base);
      } catch (err: any) {
        // TAC_STALE_STATE or "No database open" is acceptable — it means we got past
        // the type-sensitive code and failed on DB access, which is expected in tests.
        assert.ok(
          err.code === "TAC_STALE_STATE" || err.message?.includes("database"),
  ...
  test("handleCompleteMilestone does not overwrite existing SUMMARY.md on re-dispatch (#4598)", async () => {
    ...
    try {
      ...
      const milestoneDir = join(base, ".tac", "milestones", mid);
