 ...
 *
 * See: https://github.com/waghelapritesh/tac-2/issues/2771
 */
...
// ─── tac_slice_complete: enrichment arrays must be optional ──────────────────

test("tac_slice_complete — enrichment arrays are optional", () => {
  const tool = getTool("tac_slice_complete");
  assert.ok(tool, "tac_slice_complete must be registered");
...
test("tac_slice_complete — validates with only core params", () => {
  const tool = getTool("tac_slice_complete");
  assert.ok(tool, "tac_slice_complete must be registered");
...
// ─── tac_plan_milestone: enrichment arrays must be optional ──────────────────

test("tac_plan_milestone — enrichment arrays are optional", () => {
  const tool = getTool("tac_plan_milestone");
  assert.ok(tool, "tac_plan_milestone must be registered");
...
test("tac_plan_milestone — validates with only core params", () => {
  const tool = getTool("tac_plan_milestone");
  assert.ok(tool, "tac_plan_milestone must be registered");
...
// ─── tac_task_complete: enrichment arrays must be optional ───────────────────

test("tac_task_complete — enrichment arrays are optional", () => {
  const tool = getTool("tac_task_complete");
  assert.ok(tool, "tac_task_complete must be registered");
...
test("tac_task_complete — validates with only core params", () => {
  const tool = getTool("tac_task_complete");
  assert.ok(tool, "tac_task_complete must be registered");
...
// ─── tac_complete_milestone: enrichment arrays must be optional ──────────────

test("tac_complete_milestone — enrichment arrays are optional", () => {
  const tool = getTool("tac_complete_milestone");
  assert.ok(tool, "tac_complete_milestone must be registered");
...
test("tac_complete_milestone — validates with only core params", () => {
  const tool = getTool("tac_complete_milestone");
  assert.ok(tool, "tac_complete_milestone must be registered");
...
// ─── tac_plan_slice: enrichment fields must be optional ──────────────────────

test("tac_plan_slice — enrichment fields are optional", () => {
  const tool = getTool("tac_plan_slice");
  assert.ok(tool, "tac_plan_slice must be registered");
...
test("tac_plan_slice — validates with only core params", () => {
  const tool = getTool("tac_plan_slice");
  assert.ok(tool, "tac_plan_slice must be registered");
