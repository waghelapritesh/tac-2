...
import type { TACState, Phase } from "../types.ts";
import {
  ensurePlanV2Graph,
...
function createBasePath(): string {
  const basePath = mkdtempSync(join(tmpdir(), "tac-uok-planv2-"));
  mkdirSync(join(basePath, ".tac", "milestones", MILESTONE_ID), { recursive: true });
  tempDirs.add(basePath);
  return basePath;
}

function writeMilestoneFile(basePath: string, suffix: string, content: string): void {
  const milestoneDir = join(basePath, ".tac", "milestones", MILESTONE_ID);
...
function writeSliceFile(basePath: string, suffix: string, content: string): void {
  const sliceDir = join(basePath, ".tac", "milestones", MILESTONE_ID, "slices", SLICE_ID);
...
function buildState(phase: Phase): TACState {
  return {
    phase,
...
test("guided flow keeps plan-v2 fail-closed handling for non-recoverable failures", () => {
  const source = readFileSync(join(tacDir, "guided-flow.ts"), "utf-8");
  assert.ok(
...
test("plan-v2 gate accepts finalized context from project-root fallback", () => {
  ...
  const prevProjectRoot = process.env.TAC_PROJECT_ROOT;
  ...
  try {
    ...
    if (prevProjectRoot === undefined) {
      ...
      process.env.TAC_PROJECT_ROOT = prevProjectRoot;
    }
  }
});
