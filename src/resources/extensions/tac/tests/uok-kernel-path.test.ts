...
function makeBasePath(): string {
  return mkdtempSync(join(tmpdir(), "tac-uok-kernel-"));
}

function makeArgs(
  basePath: string,
  preferences: TACPreferences | undefined,
): {
...
function readParityEvents(basePath: string): Array<Record<string, unknown>> {
  const file = join(tacRoot(basePath), "runtime", "uok-parity.jsonl");
...
test("runAutoLoopWithUok uses kernel path by default and records uok-kernel parity", async () => {
  ...
  try {
    ...
    assert.equal(process.env.TAC_UOK_AUDIT_UNIFIED, "0");
...
test("runAutoLoopWithUok respects TAC_UOK_FORCE_LEGACY emergency switch", async () => {
  const basePath = makeBasePath();
  const previous = process.env.TAC_UOK_FORCE_LEGACY;
  process.env.TAC_UOK_FORCE_LEGACY = "1";
  try {
    const args = makeArgs(basePath, {
      uok: {
        enabled: true,
    ...
    if (previous === undefined) delete process.env.TAC_UOK_FORCE_LEGACY;
    else process.env.TAC_UOK_FORCE_LEGACY = previous;
    rmSync(basePath, { recursive: true, force: true });
  }
});
