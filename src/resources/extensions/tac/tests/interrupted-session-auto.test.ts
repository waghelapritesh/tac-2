...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-auto-interrupted-${randomUUID()}`);
...
test("direct /tac auto stale complete repo yields stale classification with no recovery payload", async () => {
...
test("direct /tac auto paused-session metadata remains recoverable when work is unfinished", async () => {
  const base = makeTmpBase();
  try {
...
test("direct /tac auto stale paused-session metadata is treated as stale when no resumable work remains", async () => {
  const base = makeTmpBase();
  try {
    writeRoadmap(base, true);
    writeCompleteArtifacts(base);
...
test("direct /tac auto source only resumes paused-session metadata for recoverable state with real recovery signals", async () => {
