// TAC Extension — String coercion regression tests for complete-slice/task tools
...
describe("handleCompleteSlice with coerced string arrays (#3565)", () => {
  ...
  beforeEach(() => {
    dbPath = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "tac-coerce-")),
      "test.db",
    );
    openDatabase(dbPath);

    basePath = fs.mkdtempSync(path.join(os.tmpdir(), "tac-coerce-handler-"));
    const sliceDir = path.join(basePath, ".tac", "milestones", "M001", "slices", "S01", "tasks");
