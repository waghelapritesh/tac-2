 ...
 * TAC preferences override of settings.json defaults (#3517), and custom
 * provider precedence over PREFERENCES.md when set via `/tac model` (#4122).
 */
...
function makeTmpDir(suffix: string): string {
  const dir = join(tmpdir(), `tac-test-650-${suffix}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("custom provider session model overrides PREFERENCES.md (#4122)", () => {
  ...
  it("custom provider from /tac model wins over PREFERENCES.md built-in default", () => {
    // User runs `/tac model ollama/llama3.1:8b`, then `/tac auto`.
