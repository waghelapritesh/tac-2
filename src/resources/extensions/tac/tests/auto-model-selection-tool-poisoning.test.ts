...
function makeTempProject(): { dir: string; cleanup: () => void; restoreEnv: () => void } {
  ...
  const dir = mkdtempSync(join(tmpdir(), "tac-policy-poison-"));
  const home = mkdtempSync(join(tmpdir(), "tac-policy-home-"));
...
test("vacuous-truth (a): unit type with empty workflow-required tools → dispatch succeeds", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(env.dir, ".tac", "PREFERENCES.md"),
...
test("vacuous-truth (b): non-empty workflow tool requirement that the model carries → dispatch succeeds", async () => {
  const env = makeTempProject();
  try {
    // gate-evaluate has tool requirement ["tac_save_gate_result"]; if the
    ...
    const pi = makeRecordingPi(["tac_save_gate_result"]);
    clearToolBaseline(pi as unknown as object);
...
test("cross-unit poisoning: prior unit narrowing must not deny next unit's eligible model", async () => {
  ...
  try {
    ...
    // does not support.
    const pi = makeRecordingPi(["tac_save_gate_result", "thinking_partner"]);
    clearToolBaseline(pi as unknown as object);
    ...
    // 4959 path: a second unit whose workflow requires "tac_save_gate_result"
...
// Registers `tac_plan_slice` as `producesImages: true`, then offers only an
...
test("genuinely-impossible (a): workflow tool incompatible with candidate API → typed error names tool + api", async () => {
  ...
  try {
    ...
    registerToolCompatibility("tac_plan_slice", { producesImages: true });
    ...
    const pi = makeRecordingPi(["tac_plan_slice"]);
    clearToolBaseline(pi as unknown as object);
    ...
    assert.match(err.message, /tac_plan_slice/, "throw must name the incompatible tool");
...
test("genuinely-impossible (b): cross-provider routing disabled + provider mismatch → typed error", async () => {
  const env = makeTempProject();
  try {
    // Use plan-slice (workflow-required: ["tac_plan_slice"]) but pretend no
    ...
    // fall back to a model whose api is recognized to deny `tac_plan_slice`.
    //
...
test("restore baseline: setActiveTools(BASELINE) called between units before next dispatch", async () => {
  ...
  try {
    ...
    const baselineTools = ["tac_save_gate_result", "tool_a", "tool_b"];
    ...
    pi.setActiveTools(["tac_save_gate_result"]);
...
// 1:1 with `pi` instances — a single `pi` can host multiple `/tac auto` runs
...
test("lifecycle: clearToolBaseline forces recapture; subsequent runs respect intervening tool edits", async () => {
  ...
  try {
    ...
    // ── Simulate `/tac auto` stop + intervening user tool edit ──
