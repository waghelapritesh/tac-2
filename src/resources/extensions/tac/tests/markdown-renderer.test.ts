...
import { clearPathCache, _clearTacRootCache } from '../paths.ts';
...
function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-renderer-'));
  fs.mkdirSync(path.join(dir, '.tac'), { recursive: true });
  return dir;
}
...
function scaffoldDirs(tmpDir: string, mid: string, sliceIds: string[]): void {
  const msDir = path.join(tmpDir, '.tac', 'milestones', mid);
...
test('── markdown-renderer: renderRoadmapCheckboxes round-trip ──', async () => {
  ...
  const dbPath = path.join(tmpDir, '.tac', 'tac.db');
  openDatabase(dbPath);
  clearAllCaches();
  ...
  try {
    ...
    const roadmapPath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md');
...
test('── markdown-renderer: renderPlanCheckboxes round-trip ──', async () => {
  ...
  try {
    ...
    const planPath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md');
  ...
  try {
    ...
    insertTask({
      ...
      planning: {
        ...
        inputs: ['src/resources/extensions/tac/markdown-renderer.ts'],
    ...
    insertTask({
      ...
      planning: {
        ...
        estimate: '30m',
        files: ['src/resources/extensions/tac/files.ts'],
        verify: 'node --test auto-recovery.test.ts',
        inputs: ['src/resources/extensions/tac/files.ts'],
    ...
    assert.strictEqual(parsedPlan.tasks[0].files?.[0], 'src/resources/extensions/tac/markdown-renderer.ts', 'files list preserved in slice plan');
    ...
    const taskPlanPath = path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S02', 'tasks', 'T01-PLAN.md');
...
test('── markdown-renderer: renderTaskSummary round-trip ──', async () => {
  ...
  try {
    ...
    const summaryPath = path.join(
      tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'tasks', 'T01-SUMMARY.md',
    );
...
test('── markdown-renderer: renderSliceSummary round-trip ──', async () => {
  ...
  try {
    ...
    const db = await import('../tac-db.ts');
    ...
    // Verify SUMMARY file
    const summaryPath = path.join(
      tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-SUMMARY.md',
    );
    ...
    const uatPath = path.join(
      tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-UAT.md',
    );
...
test('── markdown-renderer: renderAllFromDb produces all files ──', async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md'),
      roadmap1,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M002', 'M002-ROADMAP.md'),
      roadmap2,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md'),
      plan1,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md'),
      plan2,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M002', 'slices', 'S01', 'S01-PLAN.md'),
      plan3,
    );
    ...
    const m1Roadmap = fs.readFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md'), 'utf-8',
    );
    clearAllCaches();
    ...
    const m1s1Plan = fs.readFileSync(
      path.join(tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md'), 'utf-8',
    );
    clearAllCaches();
