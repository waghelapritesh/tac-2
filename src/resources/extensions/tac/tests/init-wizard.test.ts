/**
 * Unit tests for TAC Init Wizard — project onboarding flow.
 *
...
function makeTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `tac-init-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
test("init-wizard: existing .tac/ with milestones skips init", (t) => {
  const dir = makeTempDir("existing");
  try {
    mkdirSync(join(dir, ".tac", "milestones", "M001"), { recursive: true });
    mkdirSync(join(dir, ".tac", "milestones", "M002"), { recursive: true });

    const detection = detectProjectState(dir);
    assert.equal(detection.state, "v2-tac");
...
test("init-wizard: empty .tac/ (no milestones) returns v2-tac-empty", (t) => {
  const dir = makeTempDir("empty-tac");
  try {
    mkdirSync(join(dir, ".tac", "milestones"), { recursive: true });

    const detection = detectProjectState(dir);
    assert.equal(detection.state, "v2-tac-empty");
    assert.ok(detection.v2);
...
test("init-wizard: v2 .tac/ preferences detected", (t) => {
...
test("init-wizard: CONTEXT.md detected in v2", (t) => {
  ...
  try {
    mkdirSync(join(dir, ".tac", "milestones"), { recursive: true });
    writeFileSync(join(dir, ".tac", "CONTEXT.md"), "# Project Context\n", "utf-8");
...
test("init-wizard: v1 with both .planning/ and .tac/ prioritizes v2", (t) => {
