...
import { resolveTacRootFile } from "../paths.ts";
import {
  openDatabase,
  closeDatabase,
  insertMilestone,
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-guided-state-"));
...
describe("guided-flow STATE.md rebuild (#3475)", () => {
  ...
  test("rebuildState writes STATE.md matching derived state, not stale cache", async () => {
    ...
    const statePath = resolveTacRootFile(base, "STATE");
  ...
  test("buildStateMarkdown produces correct active milestone from TACState", async () => {
    base = createFixtureBase();
    openDatabase(":memory:");
