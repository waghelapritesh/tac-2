/**
 * Verify that catch blocks across TAC source files use the centralized
 ...
 * 2. All TAC files must not use raw stderr/console in catch blocks.
 *
...
function getAutoModeFiles(): string[] {
  ...
  for (const f of readdirSync(tacDir)) {
    if (f.startsWith("auto") && f.endsWith(".ts") && !f.endsWith(".test.ts")) {
      files.push(join(tacDir, f));
    }
  }

  // auto/ subdirectory
  const autoSubDir = join(tacDir, "auto");
...
function getTacSourceFiles(): string[] {
  const files: string[] = [];
...
describe("workflow-logger coverage (#3348)", () => {
  test("no empty catch blocks remain in migrated files", () => {
    ...
    const allFiles = getTacSourceFiles();
    const migratedPaths = new Set(autoFiles);
    for (const file of allFiles) {
      const rel = relative(tacDir, file);
    ...
    for (const file of migratedPaths) {
      ...
      // tac-db.ts has intentionally silent provider probes
      if (basename === "tac-db.ts" || basename === "session-lock.ts") continue;
  ...
  test("catch blocks use workflow-logger instead of raw stderr/console", () => {
    const files = getTacSourceFiles();
    assert.ok(files.length > 0, "should find TAC source files");
