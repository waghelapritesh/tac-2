 ...
 * prefs to the runner's cwd .tac/PREFERENCES.md and clean up in finally.
 */
...
const RUNNER_PREFS_PATH = join(process.cwd(), ".tac", "PREFERENCES.md");

function writeRunnerPreferences(isolation: "none" | "worktree" | "branch"): void {
  mkdirSync(join(process.cwd(), ".tac"), { recursive: true });
...
// Worktree isolation requires explicit opt-in — default is "none" so TAC
// works out of the box without PREFERENCES.md (#2480).
// Skip if global prefs exist — they override the default and this test
// cannot control ~/.tac/PREFERENCES.md.

test('shouldUseWorktreeIsolation returns false for no prefs (default: none)', () => {
  const globalPrefsExist = existsSync(join(homedir(), ".tac", "PREFERENCES.md"))
    || existsSync(join(homedir(), ".tac", "PREFERENCES.md"));
  if (!globalPrefsExist) {
    try {
