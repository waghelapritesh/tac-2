...
test('write-gate: blocks CONTEXT.md write during discussion without depth verification (absolute path)', () => {
  const result = shouldBlockContextWrite(
    'write',
    '/Users/dev/project/.tac/milestones/M001/M001-CONTEXT.md',
    'M001',
    false,
  );
...
test('write-gate: blocks CONTEXT.md write during discussion without depth verification (relative path)', () => {
  const result = shouldBlockContextWrite(
    'write',
    '.tac/milestones/M005/M005-CONTEXT.md',
    'M005',
    false,
  );
  assert.strictEqual(result.block, true, 'should block the write');
...
test('write-gate: allows non-CONTEXT.md writes during discussion', () => {
  ...
  const r1 = shouldBlockContextWrite(
    'write',
    '.tac/milestones/M001/M001-DISCUSSION.md',
    'M001',
  );
...
test('write-gate: regex does not match slice context files (S01-CONTEXT.md)', () => {
  const result = shouldBlockContextWrite(
    'write',
    '.tac/milestones/M001/slices/S01/S01-CONTEXT.md',
    'M001',
  );
...
test('write-gate: blocked reason contains depth_verification keyword and anti-bypass language', () => {
  const result = shouldBlockContextWrite(
    'write',
    '.tac/milestones/M999/M999-CONTEXT.md',
    'M999',
  );
...
test('write-gate: markDepthVerified unlocks only the matching milestone', () => {
  ...
  const blockedOther = shouldBlockContextWrite(
    'write',
    '.tac/milestones/M002/M002-CONTEXT.md',
    null,
  );
...
// ─── Scenario 11: tac_summary_save CONTEXT contract is milestone-scoped ──

test('write-gate: tac_summary_save only blocks final milestone CONTEXT writes', () => {
  clearDiscussionFlowState();
...
test('write-gate: shouldBlockPendingGate blocks write/edit during pending gate', () => {
  ...
  // tac tools should be blocked
  const tacResult = shouldBlockPendingGate('tac_plan_milestone', 'M001', false);
  assert.strictEqual(tacResult.block, true, 'tac tools should be blocked');
...
test('write-gate: loadWriteGateSnapshot returns empty default when persist file is deleted (#4343)', () => {
  const base = join(tmpdir(), `tac-write-gate-4343-${randomUUID()}`);
  mkdirSync(join(base, '.tac', 'runtime'), { recursive: true });
  const stateFilePath = join(base, '.tac', 'runtime', 'write-gate-state.json');
  const originalEnv = process.env.TAC_PERSIST_WRITE_GATE_STATE;

  try {
    process.env.TAC_PERSIST_WRITE_GATE_STATE = '1';
    ...
    if (originalEnv === undefined) {
      delete process.env.TAC_PERSIST_WRITE_GATE_STATE;
    } else {
      process.env.TAC_PERSIST_WRITE_GATE_STATE = originalEnv;
    }
    clearDiscussionFlowState();
    try {
