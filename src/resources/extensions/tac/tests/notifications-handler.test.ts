...
function makeTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `tac-notifications-handler-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
test("notifications tail caps inline output and hints to open overlay", async (t) => {
  ...
  assert.match(notices[0].message, /\.\.\. and \d+ more \(open \/tac notifications to browse all\)/);
});
