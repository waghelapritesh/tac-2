...
function makeTmpRepo(): string {
  const base = join(tmpdir(), `tac-codebase-test-${randomUUID()}`);
...
test("generateCodebaseMap: excludes .tac/ files", () => {
  const base = makeTmpRepo();
  try {
    addFile(base, "src/main.ts");
    addFile(base, ".tac/PROJECT.md");
...
test("generateCodebaseMap: collapsed directories preserve descriptions in hidden comment", () => {
  ...
  try {
    ...
    assert.ok(result.content.includes("<!-- tac:collapsed-descriptions"));
...
test("writeCodebaseMap: creates .tac/ directory if missing", () => {
  const base = join(tmpdir(), `tac-codebase-test-${randomUUID()}`);
  mkdirSync(base, { recursive: true });
  // Intentionally do NOT pre-create .tac/
  try {
