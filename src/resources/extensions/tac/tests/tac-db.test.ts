...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-db-test-'));
...
describe('tac-db', () => {
  test('tac-db: provider detection', () => {
  ...
  test('tac-db: fresh DB schema init (memory)', () => {
  ...
  test('tac-db: double-init idempotency', () => {
  ...
  test('tac-db: insert + get decision', () => {
    openDatabase(':memory:');
    insertDecision({
      id: 'D042',
  ...
  test('tac-db: insert + get requirement', () => {
    openDatabase(':memory:');
    insertRequirement({
      id: 'R007',
      class: 'functional',
  ...
  test('tac-db: active_decisions view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('tac-db: active_requirements view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('tac-db: WAL mode on file-backed DB', () => {
    const dbPath = tempDbPath();
    openDatabase(dbPath);
  ...
  test('tac-db: mmap stays disabled on darwin file-backed DBs', () => {
  ...
  test('tac-db: transaction rollback on error', () => {
    openDatabase(':memory:');
  ...
  test('tac-db: recreates missing verification evidence dedup index after removing duplicate rows', () => {
    const dbPath = tempDbPath();
    openDatabase(dbPath);
  ...
  test('tac-db: legacy DB missing memories.scope opens and bootstraps index columns', () => {
    const dbPath = tempDbPath();
  ...
  test('tac-db: pre-v18 DB with memory_sources missing scope opens without crash (issue #4607)', () => {
  ...
  test('tac-db: rowToTask tolerates legacy comma-separated task arrays', () => {
    openDatabase(':memory:');
  ...
  test('tac-db: query wrappers return null/empty when DB unavailable', () => {
    // Ensure DB is closed
    closeDatabase();
  ...
  test('tac-db: closeDatabase resets wasDbOpenAttempted after an intentional close', () => {
    openDatabase(':memory:');
  ...
  test('tac-db: rowToTask tolerates corrupt comma-separated task arrays', () => {
    openDatabase(':memory:');
    ...
    adapter.prepare(
      ...
      'src/resources/extensions/tac/tac-db.ts, src/resources/extensions/tac/state.ts',
      '"decision-1"',
      'M001',
      'S01',
      'T01',
  ...
  describe('getDbStatus', () => {
    ...
    test('getDbStatus: resets lastError/lastPhase after closeDatabase', () => {
      ...
      const corruptPath = path.join(os.tmpdir(), `tac-corrupt-${Date.now()}.db`);
