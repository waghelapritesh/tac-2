...
function tempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "tac-wt-test-"));
}
...
test("copyWorktreeDb copies DB file and data is queryable", (t) => {
  ...
  const srcDb = path.join(srcDir, "tac.db");
  const destDb = path.join(destDir, "nested", "tac.db");
...
test("copyWorktreeDb skips -wal and -shm files", (t) => {
  ...
  const srcDb = path.join(srcDir, "tac.db");
  const destDb = path.join(destDir, "tac.db");
...
test("copyWorktreeDb returns false when source doesn't exist", (t) => {
  ...
  const missingSrc = path.join(destDir, "missing", "tac.db");
  const result = copyWorktreeDb(missingSrc, path.join(destDir, "tac.db"));
...
test("copyWorktreeDb creates deeply nested dest directories", (t) => {
  ...
  const srcDb = path.join(srcDir, "tac.db");
  const deepDest = path.join(destDir, "a", "b", "c", "tac.db");
...
test("reconcileWorktreeDb merges new decisions from worktree into main", (t) => {
  ...
  const mainDb = path.join(mainDir, "tac.db");
  const wtDb = path.join(wtDir, "tac.db");
