/**
 * TAC-2 / guided-flow — regression tests for #4573
 *
...
function mkBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-4573-"));
...
describe("#4573 maybeHandleReadyPhraseWithoutFiles", () => {
  ...
  test("pending entry, ready phrase, no files → notify + sendMessage", () => {
    ...
    try {
      ...
      assert.equal(cap.messages[0].payload.customType, "tac-ready-no-files");
  ...
  test("files present → no nudge (happy path already fired)", () => {
    const base = mkBase();
    try {
      writeFileSync(join(base, ".tac", "milestones", "M001", "M001-CONTEXT.md"), "# ctx");
      const cap = mkCapture();
      setPendingAutoStart(base, {
        basePath: base,
        milestoneId: "M001",
  ...
  test("fresh entry after give-up resets counter", () => {
    ...
    try {
      ...
      // New /tac run — re-seeds entry; counter must be 0 again
...
describe("#4573 maybeHandleEmptyIntentTurn", () => {
  ...
  test("commit-intent phrase WITHOUT tool call → nudge fires", () => {
    ...
    try {
      ...
      assert.equal(handled, true);
      assert.equal(cap.messages.length, 1);
      assert.equal(cap.messages[0].payload.customType, "tac-empty-turn-recovery");
    } finally {
      clearPendingAutoStart();
    }
  });
