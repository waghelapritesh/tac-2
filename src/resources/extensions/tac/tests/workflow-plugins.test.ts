...
let savedTacHome: string | undefined;
...
beforeEach(() => {
  savedTacHome = process.env.TAC_HOME;
  const fakeHome = makeTmpBase();
  process.env.TAC_HOME = fakeHome;
});

afterEach(() => {
  if (savedTacHome === undefined) {
    delete process.env.TAC_HOME;
  } else {
    process.env.TAC_HOME = savedTacHome;
  }
...
version: 1
mode: markdown-phase
artifact_dir: .tac/workflows/samples/
</template_meta>
...
describe("discoverPlugins: precedence", () => {
  it("project overrides bundled", () => {
    ...
    assert.ok(bugfix.path.includes(".tac/workflows/bugfix.md"));
  });
...
describe("discoverPlugins: error handling", () => {
  ...
  it("ignores subdirectories under .tac/workflows/ (artifact dirs)", () => {
    const base = makeTmpBase();
    // Create a subdir — simulates `/tac start bugfix` artifact dir
    mkdirSync(join(base, ".tac", "workflows", "bugfixes", "250101-1-slug"), { recursive: true });
    writeFileSync(join(base, ".tac", "workflows", "bugfixes", "250101-1-slug", "STATE.json"), "{}", "utf-8");
    const plugins = discoverPlugins(base);
...
describe("discoverPlugins: legacy .tac/workflow-defs/", () => {
