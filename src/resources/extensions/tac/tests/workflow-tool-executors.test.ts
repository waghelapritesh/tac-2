...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-workflow-executors-${randomUUID()}`);
...
function writeRoadmap(base: string, milestoneId: string, sliceIds: string[]): void {
  const milestoneDir = join(base, ".tac", "milestones", milestoneId);
...
test("executeSummarySave persists artifact and returns computed path", async () => {
  ...
  try {
    ...
    const filePath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md");
...
test("executeTaskComplete coerces string verificationEvidence entries", async () => {
  ...
  try {
    openTestDb(base);
    const planDir = join(base, ".tac", "milestones", "M001", "slices", "S01");
