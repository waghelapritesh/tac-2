 ...
 * directories outside .tac/worktrees/.  The bug: removeWorktree overrides
 * the computed worktree path with whatever `git worktree list` reports.
 * When .tac/ was (or is) a symlink, git resolves the symlink at creation
 ...
 * on paths that are actually under .tac/worktrees/.
 */
...
describe("worktree-teardown-safety", () => {
  ...
  it("removeWorktree does not delete sibling data directories", () => {
    ...
    // Create a project data directory that lives alongside .tac/
  ...
  it("path validation rejects paths outside .tac/worktrees/", () => {
    const tempDir = createTempRepo();
    dirs.push(tempDir);
  ...
  it("worktreePath always returns paths under .tac/worktrees/", () => {
    const tempDir = createTempRepo();
    dirs.push(tempDir);

    const wtPathResult = worktreePath(tempDir, "anything");
    assertTrue(
      wtPathResult.startsWith(join(tempDir, ".tac", "worktrees")),
      "worktreePath returns path under .tac/worktrees/",
    );
  });
  ...
  it("isInsideWorktreesDir rejects path traversal attempts", () => {
    ...
    assertTrue(
      isInsideWorktreesDir(tempDir, join(tempDir, ".tac", "worktrees", "my-wt")),
      "path inside .tac/worktrees/ is accepted",
    );

    assertTrue(
      !isInsideWorktreesDir(tempDir, join(tempDir, "project-data")),
      "path outside .tac/worktrees/ is rejected",
    );

    assertTrue(
      !isInsideWorktreesDir(tempDir, join(tempDir, ".tac", "worktrees", "..", "..", "project-data")),
