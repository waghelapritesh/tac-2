...
function makeMockDeps(
  ...
  const baseDeps: LoopDeps = {
    ...
    loadEffectiveTACPreferences: () => ({ preferences: {} }),
...
test("runDispatch checks prior-slice completion against the project root in worktree mode", async () => {
  ...
  const ic = makeIC(deps, {
    s: {
      ...makeSession(),
      basePath: "/tmp/project/.tac/worktrees/M029-xoklo9",
...
test("runDispatch pauses when complete-milestone summary exists on disk but the unit is still stuck (#4289)", async (t) => {
  ...
  let pauseCalls = 0;
  let stopCalls = 0;
  const base = join(tmpdir(), `tac-stuck-complete-${randomUUID()}`);
  t.after(() => {
  ...
  execFileSync("git", ["add", ".tac/milestones/M001/M001-SUMMARY.md", "src/app.ts"], { cwd: base, stdio: "ignore" });
...
test("#4671: plan-v2 missing CONTEXT.md reaches dispatch recovery instead of pausing", async () => {
  const basePath = mkdtempSync(join(tmpdir(), "tac-4671-predispatch-"));
  mkdirSync(join(basePath, ".tac", "milestones", "M001", "slices", "S01", "tasks"), { recursive: true });
...
test("runFinalize pauses and emits unit-end when pre-verification times out", async () => {
  const capture = createEventCapture();
  let pauseCalls = 0;
  const basePath = mkdtempSync(join(tmpdir(), "tac-finalize-timeout-"));
