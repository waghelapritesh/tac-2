/**
 * Regression test for #1852: removeWorktree targets wrong path when .tac/ is a symlink.
 *
 * When .tac/ is a symlink to an external state directory, git registers
...
// Set up a test repo with .tac/ as a symlink to an external directory,
// mimicking the external state directory layout (~/.tac/projects/<hash>/).
// Resolve tmpdir to handle macOS /tmp -> /private/var/... symlink.
const realTmp = realpathSync(tmpdir());
const base = mkdtempSync(join(realTmp, "tac-wt-symlink-test-"));
const externalState = mkdtempSync(join(realTmp, "tac-wt-symlink-ext-"));
...
// Create .tac as a symlink to the external state directory
symlinkSync(externalState, join(base, ".tac"));

// Verify the symlink is in place
assert.ok(existsSync(join(base, ".tac")), ".tac symlink exists");
assert.ok(
  realpathSync(join(base, ".tac")) === externalState,
  ".tac resolves to external state dir",
);
...
describe('worktree-symlink-removal', async () => {
  console.log("\n=== #1852: removeWorktree with symlinked .tac/ ===");
  ...
  // Simulate what syncStateToProjectRoot does: replace the .tac symlink with
  ...
  unlinkSync(join(base, ".tac"));  // remove the symlink
  mkdirSync(join(base, ".tac", "worktrees", "M002"), { recursive: true });
  // Write a dummy file so the stale directory is non-empty
  writeFileSync(join(base, ".tac", "worktrees", "M002", "stale.txt"), "stale sync artifact", "utf-8");
