...
const autoSource = readFileSync(join(process.cwd(), "src", "resources", "extensions", "tac", "auto.ts"), "utf-8");
const dashboardSource = readFileSync(join(process.cwd(), "src", "resources", "extensions", "tac", "auto-dashboard.ts"), "utf-8");

function makeTempDir(prefix: string): string {
  return join(
    tmpdir(),
    `tac-auto-dashboard-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
}
...
test("last commit refresh still returns commit info for a valid git repo", (t) => {
  ...
  execFileSync("git", ["config", "user.name", "TAC Test"], { cwd: dir, stdio: "pipe" });
  execFileSync("git", ["config", "user.email", "tac@example.com"], { cwd: dir, stdio: "pipe" });
...
test("widget mode respects project preference precedence and persists there", (t) => {
  ...
  const globalPrefsPath = join(homeDir, ".tac", "preferences.md");
  const projectPrefsPath = join(projectDir, ".tac", "preferences.md");

  mkdirSync(join(homeDir, ".tac"), { recursive: true });
  mkdirSync(join(projectDir, ".tac"), { recursive: true });
