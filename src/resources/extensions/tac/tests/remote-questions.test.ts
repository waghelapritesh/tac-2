...
test("formatForTelegram single-question produces inline keyboard", () => {
  ...
  assert.ok(msg.text.includes("<b>TAC needs your input</b>"));
...
test("resolveRemoteConfig returns null when preferences are absent (no env side-effects)", () => {
  ...
  try {
    ...
    process.env.HOME = "/tmp/tac-no-such-home-for-test";
    process.env.USERPROFILE = "/tmp/tac-no-such-home-for-test";
