...
// Proves both paths produce field-identical TACState across 7 fixture scenarios,
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-crossval-'));
...
/**
 * Compare every TACState field between DB and filesystem derivation.
 * prefix identifies the scenario in assertion messages.
 */
function assertStatesEqual(dbState: TACState, fileState: TACState, prefix: string): void {
  // Phase
