...
test("workspace index: indexes active milestone/slice/task and suggests commands", async () => {
  const base = mkdtempSync(join(tmpdir(), "tac-workspace-index-test-"));
  ...
  try {
    ...
    assert.ok(commands.includes("/tac auto"));
    assert.ok(commands.includes("/tac doctor M001/S01"));
    assert.ok(commands.includes("/tac status"));
  } finally {
