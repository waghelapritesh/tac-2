// TAC2 — Tests for auditOrphanedMilestoneBranches bootstrap audit
...
import { openDatabase, closeDatabase, insertMilestone, updateMilestoneStatus } from "../tac-db.ts";

function run(cmd: string, cwd: string): string {
  return execSync(cmd, { cwd, stdio: ["ignore", "pipe", "pipe"], encoding: "utf-8" }).trim();
}

/** Create a temp git repo with .tac structure and DB. */
function createRepo(): string {
  ...
  // Create .tac structure on disk (not tracked in git)
...
describe("auditOrphanedMilestoneBranches", () => {
  ...
  test("#4762 — also surfaces worktree directory for in-progress orphan when present", () => {
    ...
    const wtDir = join(dir, ".tac", "worktrees", "M001");
    ...
    assert.ok(
      result.warnings.some(w => w.includes(".tac/worktrees/M001") || w.includes("worktree")),
