...
import { openDatabase, closeDatabase, insertMilestone, insertSlice, insertGateRow } from "../tac-db.ts";
...
function makeTmpProject(): string {
  ...
  openDatabase(join(dir, ".tac", "tac.db"));
...
test("verifyExpectedArtifact treats complete-slice as satisfied when summary, UAT, and roadmap checkbox exist", () => {
  const base = makeTmpBase();
  try {
    const milestoneDir = join(base, ".tac", "milestones", "M001");
...
test("hasImplementationArtifacts returns false when only .tac/ files committed (#1703)", () => {
...
test("verifyExpectedArtifact complete-milestone fails with only .tac/ files (#1703)", () => {
  const base = makeGitBase();
  try {
...
test("verifyExpectedArtifact complete-milestone fails when DB milestone is not complete (#4658)", () => {
  ...
  try {
    ...
    openDatabase(join(base, ".tac", "tac.db"));
...
test("verifyExpectedArtifact complete-milestone tolerates transient DB lag when SUMMARY is canonical success (#4658)", () => {
  ...
  try {
    ...
    mkdirSync(join(base, ".tac", "milestones", "M001"), { recursive: true });
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "M001-SUMMARY.md"),
      [
        "---",
        "id: M001",
        "status: complete",
...
test("#4068: verifyExpectedArtifact parallel-research treats PARALLEL-BLOCKER as terminal completion", () => {
  ...
  try {
    ...
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "M001-ROADMAP.md"),
      [
        "# M001: Timeout Test",
        "",
        "## Slices",
...
test("#4414: verifyExpectedArtifact parallel-research succeeds when all research-ready slices have RESEARCH", () => {
  const base = makeTmpBase();
  try {
    mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S02", "tasks"), { recursive: true });
    mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S03", "tasks"), { recursive: true });
    ...
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-RESEARCH.md"),
      "# research",
      "utf-8",
    );
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "slices", "S02", "S02-RESEARCH.md"),
      "# research",
      "utf-8",
    );
    ...
    writeFileSync(
      join(base, ".tac", "milestones", "M001", "slices", "S03", "S03-RESEARCH.md"),
      "# research",
      "utf-8",
    );
    clearParseCache();
