// TAC — Tests for persistent blocked-models store (issue #4513)
...
function mkBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-blocked-models-"));
...
test("blocked-models: corrupted JSON recovers to empty", () => {
  const base = mkBase();
  try {
    const path = join(base, ".tac", "runtime", "blocked-models.json");
...
test("blocked-models: file created under .tac/runtime/", () => {
  const base = mkBase();
  try {
    blockModel(base, "openai-codex", "gpt-5", "reason");
    assert.ok(existsSync(join(base, ".tac", "runtime", "blocked-models.json")));
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
});
