...
describe("complete-slice closes complete-slice-owned gates", () => {
  ...
  beforeEach(() => {
    dbPath = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "tac-slice-gate-")),
      "test.db",
    );
    openDatabase(dbPath);

    basePath = fs.mkdtempSync(path.join(os.tmpdir(), "tac-slice-gate-handler-"));
    const sliceDir = path.join(
      basePath, ".tac", "milestones", "M001", "slices", "S01", "tasks",
    );
    fs.mkdirSync(sliceDir, { recursive: true });
    fs.writeFileSync(
      path.join(basePath, ".tac", "milestones", "M001", "M001-ROADMAP.md"),
      [
        "# M001: Test Milestone",
        "",
        "## Slices",
