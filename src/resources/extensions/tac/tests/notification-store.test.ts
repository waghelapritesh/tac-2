// TAC Extension — Notification Store Tests
...
describe("notification-store", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "tac-notif-test-"));
  ...
  test("appendNotification creates file and writes entry", () => {
    ...
    const filePath = join(tmp, ".tac", "notifications.jsonl");
  ...
  test("reinit switches to new project path", () => {
    const tmp2 = mkdtempSync(join(tmpdir(), "tac-notif-test2-"));
    mkdirSync(join(tmp2, ".tac"), { recursive: true });
  ...
  test("markAllRead does not delete a foreign lock file", () => {
    ...
    const lockPath = join(tmp, ".tac", "notifications.lock");
