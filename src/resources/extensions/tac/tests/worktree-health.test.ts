 ...
 *
 * Creates real temp git repos with TAC worktrees in various states and verifies
...
describe('worktree-health', async () => {
  ...
  try {
    ...
    console.log("\n=== worktree health: merged worktree ===");
    {
      ...
      run("git worktree add -b worktree/done-feature .tac/worktrees/done-feature", dir);
      const wtPath = join(dir, ".tac", "worktrees", "done-feature");
    ...
    console.log("\n=== worktree health: dirty unmerged worktree ===");
    {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/dirty-wip .tac/worktrees/dirty-wip", dir);
      const wtPath = join(dir, ".tac", "worktrees", "dirty-wip");
    ...
    console.log("\n=== worktree health: unpushed commits ===");
    {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/unpushed .tac/worktrees/unpushed", dir);
      const wtPath = join(dir, ".tac", "worktrees", "unpushed");
    ...
    console.log("\n=== worktree health: stale detection ===");
    {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/stale-test .tac/worktrees/stale-test", dir);
      // Diverge from main so the branch is not "merged"
      const wtPath = join(dir, ".tac", "worktrees", "stale-test");
    ...
    console.log("\n=== worktree health: format clean active worktree ===");
    {
      ...
      mkdirSync(join(dir, ".tac", "worktrees"), { recursive: true });
      run("git worktree add -b worktree/clean-active .tac/worktrees/clean-active", dir);
      // Diverge from main so it's not "merged"
      const wtPath = join(dir, ".tac", "worktrees", "clean-active");
