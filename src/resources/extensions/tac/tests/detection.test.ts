/**
 * Unit tests for TAC Detection — project state and ecosystem detection.
 *
...
function makeTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `tac-detection-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
test("detectProjectState: directory with .tac/milestones/M001 returns v2-tac", (t) => {
  const dir = makeTempDir("v2-tac");
  t.after(() => cleanup(dir));

  mkdirSync(join(dir, ".tac", "milestones", "M001"), { recursive: true });
  const result = detectProjectState(dir);
  assert.equal(result.state, "v2-tac");
  assert.ok(result.v2);
  assert.equal(result.v2!.milestoneCount, 1);
});

test("detectProjectState: directory with empty .tac/milestones returns v2-tac-empty", (t) => {
  const dir = makeTempDir("v2-empty");
  t.after(() => cleanup(dir));

  mkdirSync(join(dir, ".tac", "milestones"), { recursive: true });
  const result = detectProjectState(dir);
  assert.equal(result.state, "v2-tac-empty");
  assert.ok(result.v2);
...
test("detectProjectState: detects preferences in .tac/", (t) => {
...
test("scanProjectFiles: excludes .claude, .tac, .planning, .plans, .cursor, .vscode directories", () => {
  ...
  try {
    ...
    const excludedDirs = [".claude", ".tac", ".planning", ".plans", ".cursor", ".vscode"];
