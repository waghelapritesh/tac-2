...
test("clearActivityLogState resets dedup state so identical saves write again", () => {
  ...
  // Pre-resolve baseDir so tacRoot() returns a stable key across calls.
  ...
  const baseDir = realpathSync(mkdtempSync(join(tmpdir(), "tac-memleak-test-")));
  try {
    ...
    const actDir = join(baseDir, ".tac", "activity");
...
test("saveActivityLog writes valid JSONL via streaming", () => {
  clearActivityLogState();
  const baseDir = realpathSync(mkdtempSync(join(tmpdir(), "tac-memleak-jsonl-")));
  try {
    const entries = [
