...
function freshBase(): string {
  return mkdtempSync(join(tmpdir(), 'tac-exec-history-'));
}
...
test('listExecHistory: returns empty list when .tac/exec missing', () => {
...
test('executeExecSearch: returns helpful empty-state message when no matches', () => {
  ...
  try {
    ...
    assert.match(result.content[0].text, /No prior tac_exec runs/);
  } finally {
    cleanup(base);
  }
});
