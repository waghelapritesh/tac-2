// TAC-2 — Tests for google-search deprecation stub (STUB-01, STUB-02)
...
test("google-search stub: session_start warning contains package name", async (_t) => {
  // STUB-01: warning includes @tac-extensions/google-search
  const mod = await import("../../google-search/index.ts");
  const stubFn = mod.default;
  ...
  assert.ok(
    capturedMessage?.includes("@tac-extensions/google-search"),
    `Expected message to include "@tac-extensions/google-search", got: "${capturedMessage}"`,
  );
});

test("google-search stub: session_start warning explains package is not yet published", async (_t) => {
  // STUB-01: stub must NOT advise `tac extensions install` — the replacement
  ...
  assert.ok(
    !capturedMessage?.includes("tac extensions install"),
