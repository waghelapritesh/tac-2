...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-journal-test-${randomUUID()}`);
...
describe("emitJournalEvent", () => {
  ...
  test("creates journal directory and JSONL file", () => {
    ...
    const filePath = join(base, ".tac", "journal", "2025-03-21.jsonl");
  ...
  test("silently catches read-only directory errors", () => {
    const journalDir = join(base, ".tac", "journal");
...
describe("emitJournalEvent — auto-creates parent directory", () => {
  let base: string;
  beforeEach(() => {
    base = join(tmpdir(), `tac-journal-test-${randomUUID()}`);
    // Don't create .tac/ — emitJournalEvent should handle it via mkdirSync recursive
  });
  afterEach(() => { cleanup(base); });
