 ...
 * target source code (non-.tac/ paths) during queue mode.
 *
 ...
 *   (d) write/edit to .tac/ path → pass (planning artifacts)
 *   (e) write/edit to source path → block
 *   (f) bash command → block (could execute work)
 *   (g) registered TAC tools (tac_milestone_generate_id, tac_summary_save) → pass
...
test('queue-guard: allows discussion and planning tools during queue mode', () => {
  ...
  const r2 = shouldBlockQueueExecution('tac_milestone_generate_id', '', true);
  assert.strictEqual(r2.block, false, 'tac_milestone_generate_id should pass');

  const r3 = shouldBlockQueueExecution('tac_summary_save', '', true);
  assert.strictEqual(r3.block, false, 'tac_summary_save should pass');
});

// ─── Scenario 4: Write to .tac/ paths passes (planning artifacts) ──

test('queue-guard: allows writes to .tac/ paths during queue mode', () => {
  const r1 = shouldBlockQueueExecution('write', '.tac/milestones/M001/M001-CONTEXT.md', true);
  assert.strictEqual(r1.block, false, 'write to .tac/ should pass');

  const r2 = shouldBlockQueueExecution('write', '/project/.tac/PROJECT.md', true);
  assert.strictEqual(r2.block, false, 'write to .tac/PROJECT.md should pass');

  const r3 = shouldBlockQueueExecution('edit', '.tac/QUEUE.md', true);
  assert.strictEqual(r3.block, false, 'edit to .tac/QUEUE.md should pass');

  const r4 = shouldBlockQueueExecution('write', '.tac/REQUIREMENTS.md', true);
  assert.strictEqual(r4.block, false, 'write to .tac/REQUIREMENTS.md should pass');

  const r5 = shouldBlockQueueExecution('write', '.tac/DECISIONS.md', true);
  assert.strictEqual(r5.block, false, 'write to .tac/DECISIONS.md should pass');
});
...
// ─── Scenario 8: mkdir for .tac/ milestone directories passes ──

test('queue-guard: allows mkdir for .tac/ milestone directories', () => {
  const r1 = shouldBlockQueueExecution('bash', 'mkdir -p .tac/milestones/M010/slices', true);
  assert.strictEqual(r1.block, false, 'mkdir -p .tac/ should pass');
});
