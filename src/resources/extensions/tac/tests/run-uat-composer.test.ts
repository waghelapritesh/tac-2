// TAC-2 — #4782 phase 3: run-uat migrated to compose context via manifest.
...
function makeBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-runuat-composer-"));
...
test("#4782 phase 3: buildRunUatPrompt inlines slice UAT, slice summary, project via composer", async (t) => {
  ...
  const uatRel = ".tac/milestones/M001/slices/S01/S01-UAT.md";
