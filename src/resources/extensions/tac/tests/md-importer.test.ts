...
| D002 | M001 | arch | DB location | .tac/tac.db | Derived state | No |
...
function createFixtureTree(baseDir: string): void {
  const tac = path.join(baseDir, '.tac');
  fs.mkdirSync(tac, { recursive: true });
  fs.writeFileSync(path.join(tac, 'DECISIONS.md'), DECISIONS_MD);
  fs.writeFileSync(path.join(tac, 'REQUIREMENTS.md'), REQUIREMENTS_MD);
  fs.writeFileSync(path.join(tac, 'PROJECT.md'), '# Test Project\nA test project.');

  // Create milestone hierarchy
  const m001 = path.join(tac, 'milestones', 'M001');
...
| D002 | M001 | arch | DB location | .tac/tac.db | Derived state | No | agent |
...
test('md-importer: migrateFromMarkdown orchestrator', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-import-test-'));
...
test('md-importer: idempotent re-import', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-idemp-test-'));
  createFixtureTree(tmpDir);
...
test('md-importer: missing file handling', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-empty-test-'));
  // Create empty .tac/ with no files
  fs.mkdirSync(path.join(tmpDir, '.tac'), { recursive: true });
...
test('md-importer: round-trip fidelity', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-roundtrip-test-'));
  createFixtureTree(tmpDir);
  ...
  try {
    ...
    assert.deepStrictEqual(d002?.choice, '.tac/tac.db', 'D002 choice round-trip');
