...
describe('auto-remediate stale slice status (#3673)', () => {
  test('updateSliceStatus is imported from tac-db', () => {
    assert.match(source, /import\s*\{[^}]*updateSliceStatus[^}]*\}\s*from\s*["']\.\/tac-db/,
      'updateSliceStatus should be imported from tac-db');
  });
