...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-park-test-'));
...
function createMilestone(base: string, mid: string, opts?: { withRoadmap?: boolean; withSummary?: boolean; dependsOn?: string[] }): void {
  const mDir = join(base, '.tac', 'milestones', mid);
...
function initGitRepo(base: string): void {
  ...
  writeFileSync(join(base, ".tac", "STATE.md"), "# State\n", "utf-8");
  run("git init", base);
...
test('discardMilestone updates queue order', () => {
    ...
    try {
      ...
      const queuePath = join(base, '.tac', 'QUEUE-ORDER.json');
...
test('discardMilestone removes DB rows, worktree, and milestone branch', () => {
    ...
    try {
      ...
      assert.ok(openDatabase(join(base, '.tac', 'tac.db')), 'database opens');
