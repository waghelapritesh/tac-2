...
function makeTempBase(): string {
  const tmp = mkdtempSync(join(tmpdir(), "tac-anchor-test-"));
  mkdirSync(join(tmp, ".tac", "milestones", "M001", "anchors"), { recursive: true });
  return tmp;
}
...
test("writePhaseAnchor creates anchor file in correct location", () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(base, ".tac", "milestones", "M001", "anchors", "discuss.json")));
  } finally {
