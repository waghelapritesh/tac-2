...

{
  ...
  // basePath is undefined, which causes join(undefined, ".tac") to throw.
...

{
  ...
  const base = mkdtempSync(join(tmpdir(), "tac-stalled-tool-test-"));
  mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks"), { recursive: true });
  mkdirSync(join(base, ".tac", "runtime", "units"), { recursive: true });
