...
describe('unique-milestone-ids', async () => {
  ...
  // (h) Preferences round-trip via renderPreferencesForSystemPrompt
  {
    ...
    const prefsTrue: TACPreferences = { unique_milestone_ids: true };
    ...
    const prefsUndefined: TACPreferences = {};
    ...
    const prefsFalse: TACPreferences = { unique_milestone_ids: false };
    ...
    const prefsCoerced: TACPreferences = { unique_milestone_ids: 1 as unknown as boolean };
    ...
    // TACPreferences interface accepts the field (compile-time check — if this compiles, it works)
    const prefs: TACPreferences = { unique_milestone_ids: true, version: 1 };
    assert.ok(prefs.unique_milestone_ids === true, 'TACPreferences interface accepts unique_milestone_ids');
  }
});
