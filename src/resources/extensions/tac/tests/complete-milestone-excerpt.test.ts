// TAC-2 — #4780: slice-summary excerpts replace full inlining in
...
function createBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-cm-excerpt-"));
...
function writeRoadmap(base: string, content: string): void {
  writeFileSync(join(base, ".tac", "milestones", "M001", "M001-ROADMAP.md"), content);
}

function writeSummary(base: string, sid: string, content: string): void {
  writeFileSync(
    join(base, ".tac", "milestones", "M001", "slices", sid, `${sid}-SUMMARY.md`),
    content,
  );
}
...
test("#4780 excerpt: emits compact block with frontmatter fields + section heads", async (t) => {
  ...
  const absPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md");
  const relPath = ".tac/milestones/M001/slices/S01/S01-SUMMARY.md";
  ...
  assert.match(out, /Source: `\.tac\/milestones\/M001\/slices\/S01\/S01-SUMMARY\.md`/);
...
test("#4780 excerpt: fall back to full inline when frontmatter is unrecognizable", async (t) => {
  ...
  const absPath = join(base, ".tac", "milestones", "M001", "slices", "S01", "S99-SUMMARY.md");
