...
function makeBasePath(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `tac-4671-${prefix}-`));
  mkdirSync(join(dir, ".tac", "milestones", "M001"), { recursive: true });
  return dir;
}

function buildCtx(basePath: string, state: TACState): DispatchContext {
  return {
    basePath,
    mid: "M001",
...
describe("#4671 execution-entry phase missing-context recovery", () => {
  ...
  test("phase=executing with CONTEXT.md present → falls through", async () => {
    ...
    try {
      writeFileSync(
        join(basePath, ".tac", "milestones", "M001", "M001-CONTEXT.md"),
        "# M001 Context\n\nSome real context.\n",
      );
  ...
  test("phase=executing accepts finalized CONTEXT.md from TAC_PROJECT_ROOT fallback", async () => {
    ...
    try {
      writeFileSync(
        join(projectRoot, ".tac", "milestones", "M001", "M001-CONTEXT.md"),
