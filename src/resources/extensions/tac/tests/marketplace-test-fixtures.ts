...
const CLONE_FIXTURES_ENABLED = process.env.TAC_TEST_CLONE_MARKETPLACES === '1';
...
export function getMarketplaceFixtures(testFileDir: string): { available: boolean; skipReason?: string; fixtures?: MarketplaceFixtureSet } {
  const tac2Root = resolve(testFileDir, '../../../../..');
  const localClaudeSkillsPath = resolve(tac2Root, '../claude_skills');
  const localClaudePluginsOfficialPath = resolve(tac2Root, '../claude-plugins-official');
  ...
  if (!CLONE_FIXTURES_ENABLED) {
    return {
      available: false,
      skipReason: 'Marketplace repos absent and clone-based fixtures are disabled (set TAC_TEST_CLONE_MARKETPLACES=1 to enable)',
    };
  }
  ...
  try {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'tac-marketplace-fixtures-'));
