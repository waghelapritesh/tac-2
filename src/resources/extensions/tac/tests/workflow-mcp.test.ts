...
test("guided execute-task requires canonical task completion tool", () => {
  assert.deepEqual(getRequiredWorkflowToolsForGuidedUnit("execute-task"), ["tac_task_complete"]);
});

test("auto execute-task requires legacy completion alias until prompt contract is aligned", () => {
  assert.deepEqual(getRequiredWorkflowToolsForAutoUnit("execute-task"), ["tac_complete_task"]);
});

test("detectWorkflowMcpLaunchConfig prefers explicit env override", () => {
  const launch = detectWorkflowMcpLaunchConfig("/tmp/project", {
    TAC_WORKFLOW_MCP_NAME: "workflow-tools",
    TAC_WORKFLOW_MCP_COMMAND: "node",
    TAC_WORKFLOW_MCP_ARGS: JSON.stringify(["dist/cli.js"]),
    TAC_WORKFLOW_MCP_ENV: JSON.stringify({ FOO: "bar" }),
    TAC_WORKFLOW_MCP_CWD: "/tmp/project",
    TAC_CLI_PATH: "/tmp/tac",
  });
  ...
  assert.equal(launch?.env?.TAC_CLI_PATH, "/tmp/tac");
  assert.equal(launch?.env?.TAC_PERSIST_WRITE_GATE_STATE, "1");
  assert.equal(launch?.env?.TAC_WORKFLOW_PROJECT_ROOT, "/tmp/project");
  assert.match(launch?.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match(launch?.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
});
...
test("buildWorkflowMcpServers mirrors explicit launch config", () => {
  ...
  assert.deepEqual(servers, {
    "tac-workflow": {
      command: "node",
      args: ["dist/cli.js"],
      env: servers?.["tac-workflow"]?.env,
    },
  });
  assert.equal((servers?.["tac-workflow"]?.env as Record<string, string> | undefined)?.TAC_PERSIST_WRITE_GATE_STATE, "1");
  assert.equal((servers?.["tac-workflow"]?.env as Record<string, string> | undefined)?.TAC_WORKFLOW_PROJECT_ROOT, "/tmp/project");
  assert.match((servers?.["tac-workflow"]?.env as Record<string, string> | undefined)?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match((servers?.["tac-workflow"]?.env as Record<string, string> | undefined)?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
});

test("detectWorkflowMcpLaunchConfig resolves the bundled server from TAC_PROJECT_ROOT", () => {
  const repoRoot = mkdtempSync(join(tmpdir(), "tac-workflow-root-"));
  const worktreeRoot = mkdtempSync(join(tmpdir(), "tac-workflow-worktree-"));
  ...
  assert.equal(launch?.env?.TAC_PERSIST_WRITE_GATE_STATE, "1");
  assert.equal(launch?.env?.TAC_WORKFLOW_PROJECT_ROOT, repoRoot);
  assert.match(launch?.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match(launch?.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
});

test("detectWorkflowMcpLaunchConfig resolves the bundled server from TAC_BIN_PATH ancestry", () => {
  const repoRoot = mkdtempSync(join(tmpdir(), "tac-workflow-root-"));
  const worktreeRoot = mkdtempSync(join(tmpdir(), "tac-workflow-worktree-"));
  const cliPath = join(repoRoot, "packages", "mcp-server", "dist", "cli.js");
  ...
  assert.equal(launch?.env?.TAC_CLI_PATH, devCliPath);
  assert.equal(launch?.env?.TAC_PERSIST_WRITE_GATE_STATE, "1");
  assert.equal(launch?.env?.TAC_WORKFLOW_PROJECT_ROOT, worktreeRoot);
  assert.match(launch?.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match(launch?.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
});

test("detectWorkflowMcpLaunchConfig resolves the bundled server relative to the installed TAC package", () => {
  const launch = detectWorkflowMcpLaunchConfig("/tmp/project", {
    TAC_BIN_PATH: "/tmp/tac-loader.js",
  });

  assert.equal(launch?.command, process.execPath);
  assert.equal(launch?.cwd, "/tmp/project");
  assert.equal(launch?.env?.TAC_CLI_PATH, "/tmp/tac-loader.js");
  assert.equal(launch?.env?.TAC_WORKFLOW_PROJECT_ROOT, "/tmp/project");
  assert.match(launch?.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match(launch?.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
...
test("detectWorkflowMcpLaunchConfig resolves the bundled server relative to the package without env hints", () => {
  ...
  assert.equal(launch?.command, process.execPath);
  assert.equal(launch?.cwd, "/tmp/project");
  assert.equal(launch?.env?.TAC_CLI_PATH, undefined);
  assert.equal(launch?.env?.TAC_WORKFLOW_PROJECT_ROOT, "/tmp/project");
  assert.match(launch?.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
  assert.match(launch?.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
  assert.equal(typeof launch?.args?.[0], "string");
...
test("workflow MCP launch config reaches mutation tools over stdio", async () => {
  const projectRoot = mkdtempSync(join(tmpdir(), "tac-workflow-transport-"));
  mkdirSync(join(projectRoot, ".tac"), { recursive: true });
  // Isolate the spawned MCP server from the developer's real ~/.tac so it
  ...
  const isolatedTacHome = mkdtempSync(join(tmpdir(), "tac-workflow-home-"));

  const launch = detectWorkflowMcpLaunchConfig(projectRoot, {});
  assert.ok(launch, "expected a workflow MCP launch config");
  assert.match(
    launch.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "",
    /(dist[\/\\]resources[\/\\]extensions[\/\\]tac[\/\\]tools[\/\\]workflow-tool-executors\.js|src[\/\\]resources[\/\\]extensions[\/\\]tac[\/\\]tools[\/\\]workflow-tool-executors\.(js|ts))$/,
  );
  assert.match(
    launch.env?.TAC_WORKFLOW_WRITE_GATE_MODULE ?? "",
    /(dist[\/\\]resources[\/\\]extensions[\/\\]tac[\/\\]bootstrap[\/\\]write-gate\.js|src[\/\\]resources[\/\\]extensions[\/\\]tac[\/\\]bootstrap[\/\\]write-gate\.(js|ts))$/,
  );
  if ((launch.env?.TAC_WORKFLOW_EXECUTORS_MODULE ?? "").endsWith(".ts")) {
    assert.match(launch.env?.NODE_OPTIONS ?? "", /--experimental-strip-types/);
    assert.match(launch.env?.NODE_OPTIONS ?? "", /resolve-ts\.mjs/);
  }
  ...
  try {
    ...
    assert.ok(
      (tools.tools ?? []).some((tool) => tool.name === "tac_plan_slice"),
      "expected workflow MCP surface to expose tac_plan_slice",
    );
    assert.ok(
          ...
          slices: [
            {
              ...
              successCriteria: "tac_plan_slice can write plan artifacts.",
          ...
          tasks: [
            {
              ...
              estimate: "10m",
              files: ["src/resources/extensions/tac/workflow-mcp.ts"],
              verify: "node --test",
    ...
    assert.ok(
      existsSync(join(projectRoot, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md")),
      "expected slice plan artifact to be written through stdio MCP",
    );
    assert.ok(
      existsSync(
        join(projectRoot, ".tac", "milestones", "M001", "slices", "S01", "tasks", "T01-PLAN.md"),
      ),
    ...
    rmSync(isolatedTacHome, { recursive: true, force: true });
  }
});

test("workflow MCP ask_user_questions uses stdio elicitation round-trip", async () => {
  const projectRoot = mkdtempSync(join(tmpdir(), "tac-workflow-elicit-"));
  mkdirSync(join(projectRoot, ".tac"), { recursive: true });
  const isolatedTacHome = mkdtempSync(join(tmpdir(), "tac-workflow-home-"));
...
test("transport compatibility passes when required tools fit current MCP surface", () => {
  const error = getWorkflowTransportSupportError(
    ...
    ["tac_task_complete"],
    {
      ...
      env: { TAC_WORKFLOW_MCP_COMMAND: "node" },
      surface: "guided flow",
...
test("transport compatibility now allows reassess-roadmap over workflow MCP surface", () => {
  const error = getWorkflowTransportSupportError(
    "claude-code",
    ["tac_milestone_status", "tac_reassess_roadmap"],
    {
      projectRoot: "/tmp/project",
      env: { TAC_WORKFLOW_MCP_COMMAND: "node" },
      surface: "auto-mode",
...
test("transport compatibility now allows validate-milestone over workflow MCP surface", () => {
  const error = getWorkflowTransportSupportError(
    "claude-code",
    ["tac_milestone_status", "tac_validate_milestone"],
    {
      projectRoot: "/tmp/project",
      env: { TAC_WORKFLOW_MCP_COMMAND: "node" },
      surface: "auto-mode",
...
test("transport compatibility now allows complete-milestone over workflow MCP surface", () => {
  const error = getWorkflowTransportSupportError(
    "claude-code",
    ["tac_milestone_status", "tac_complete_milestone"],
    {
      projectRoot: "/tmp/project",
      env: { TAC_WORKFLOW_MCP_COMMAND: "node" },
      surface: "auto-mode",
...
test("workflow transport error guidance includes /tac mcp init hint", () => {
  const src = readSrc("workflow-mcp.ts");
  assert.match(src, /Please run \/tac mcp init \./);
});
