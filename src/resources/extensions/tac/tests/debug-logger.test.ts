...
function createTempTacDir(): string {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-debug-test-'));
...
test('enableDebug creates log file and sets enabled', () => {
  ...
  assert.ok(normalized.includes('.tac/debug/debug-'), 'log path should be in .tac/debug/');
...
test('auto-prunes old debug logs', () => {
  const tmp = createTempTacDir();
  const debugDir = join(tmp, '.tac', 'debug');
