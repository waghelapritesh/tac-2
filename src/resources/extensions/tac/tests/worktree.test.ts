...
const base = mkdtempSync(join(tmpdir(), "tac-branch-test-"));
run("git init -b main", base);
run('git config user.name "Pi Test"', base);
run('git config user.email "pi@example.com"', base);
mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks"), { recursive: true });
writeFileSync(join(base, "README.md"), "hello\n", "utf-8");
writeFileSync(join(base, ".tac", "milestones", "M001", "M001-ROADMAP.md"), `# M001: Demo\n\n## Slices\n- [ ] **S01: Slice One** \`risk:low\` \`depends:[]\`\n  > After this: demo works\n`, "utf-8");
writeFileSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md"), `# S01: Slice One\n\n**Goal:** Demo\n**Demo:** Demo\n\n## Must-Haves\n- done\n\n## Tasks\n- [ ] **T01: Implement** \`est:10m\`\n  do it\n`, "utf-8");
run("git add .", base);
...
describe('worktree', async () => {
  ...
  console.log("\n=== getSliceBranchName ===");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01"), "tac/M001/S01", "branch name format correct");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", null), "tac/M001/S01", "null worktree = plain branch");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", "my-wt"), "tac/my-wt/M001/S01", "worktree-namespaced branch");

  console.log("\n=== parseSliceBranch ===");
  const plain = parseSliceBranch("tac/M001/S01");
  ...
  const namespaced = parseSliceBranch("tac/feature-auth/M001/S01");
  ...
  console.log("\n=== SLICE_BRANCH_RE ===");
  assert.ok(SLICE_BRANCH_RE.test("tac/M001/S01"), "regex matches plain branch");
  assert.ok(SLICE_BRANCH_RE.test("tac/my-wt/M001/S01"), "regex matches worktree branch");
  assert.ok(!SLICE_BRANCH_RE.test("main"), "regex rejects main");
  assert.ok(!SLICE_BRANCH_RE.test("tac/"), "regex rejects bare tac/");
  ...
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.tac/worktrees/feature-auth"), "feature-auth", "detects worktree name");
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.tac/worktrees/my-wt/subdir"), "my-wt", "detects worktree with subdir");
  ...
  {
    const repo = mkdtempSync(join(tmpdir(), "tac-integ-facade-"));
  ...
  {
    const repo = mkdtempSync(join(tmpdir(), "tac-integ-skip-"));
    run("git init -b main", repo);
    run("git config user.name 'Pi Test'", repo);
    run("git config user.email 'pi@example.com'", repo);
    writeFileSync(join(repo, "README.md"), "init\n");
  ...
  {
    const repo = mkdtempSync(join(tmpdir(), "tac-integ-main-"));
    run("git init -b main", repo);
    run("git config user.name 'Pi Test'", repo);
    run("git config user.email 'pi@example.com'", repo);
    writeFileSync(join(repo, "README.md"), "init\n");
    ...
    const fakeHome = mkdtempSync(join(tmpdir(), "tac-fake-home-"));
  ...
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.tac/projects/89e1c9ad49bf/worktrees/M001"),
    "M001",
    "detects milestone in symlink-resolved path",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.tac/projects/abc123/worktrees/M002/subdir"),
    "M002",
    "detects milestone with trailing subdir in symlink-resolved path",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.tac/projects/abc123"),
    null,
    "returns null for project root without worktrees segment",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/foo/.tac/worktrees/M001"),
    "M001",
  ...
  // BUG FIX: symlink-resolved paths that land inside ~/.tac should NOT
  ...
  // With TAC_PROJECT_ROOT env var set (layer 1 — coordinator passes it)
  process.env.TAC_PROJECT_ROOT = "/real/project";
  assert.deepStrictEqual(
    resolveProjectRoot("/Users/fran/.tac/projects/89e1c9ad49bf/worktrees/M001"),
    "/real/project",
    "uses TAC_PROJECT_ROOT when set",
  );
  delete process.env.TAC_PROJECT_ROOT;

  // Without TAC_PROJECT_ROOT, direct layout still works (no ~/.tac collision)
  assert.deepStrictEqual(
    resolveProjectRoot("/some/repo"),
    "/some/repo",
    "ignores TAC_PROJECT_ROOT override for non-worktree paths",
  );
  delete process.env.TAC_PROJECT_ROOT;

  // Without TAC_PROJECT_ROOT, direct layout still works (no ~/.tac collision)
  assert.deepStrictEqual(
    resolveProjectRoot("/foo/.tac/worktrees/M001"),
    "/foo",
  ...
  // Without TAC_PROJECT_ROOT, direct layout with nested subdirs
  assert.deepStrictEqual(
    resolveProjectRoot("/data/.tac/worktrees/M003/nested"),
    "/data",
  ...
  {
    const fakeHome = mkdtempSync(join(tmpdir(), "tac-home-"));
    const project = realpathSync(mkdtempSync(join(tmpdir(), "tac-proj-")));
    const storage = join(fakeHome, ".tac", "projects", "abc123def456");
    mkdirSync(storage, { recursive: true });
    symlinkSync(storage, join(project, ".tac"));
    ...
    run("git worktree add .tac/worktrees/M001 -b worktree/M001", project);

    const deep = join(project, ".tac", "worktrees", "M001", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k");
