...
assertTrue(
  dbLifecycleRegion.includes("const tacDbPath = resolveProjectRootDbPath(s.basePath);"),
  "DB lifecycle resolves the project-root DB path after worktree entry (#3822)",
);

assertTrue(
  !dbLifecycleRegion.includes('join(s.basePath, ".tac", "tac.db")'),
  "DB lifecycle no longer derives tac.db directly from the worktree path (#3822)",
);
