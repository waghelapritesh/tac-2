...
Add the \`completing-milestone\` phase to the TAC state machine.
...
- \`agent/extensions/tac/types.ts\` — Phase type definition

## Expected Output

- \`agent/extensions/tac/types.ts\` — Phase union includes \`'completing-milestone'\`
`;
  const result = parseTaskPlanMustHaves(content);
