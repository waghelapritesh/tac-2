...
test("buildSourceFilePaths fallback does not reference rg or ripgrep", (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-prompts-fallback-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));

  // No TAC files exist in tmp — forces the fallback branch.
...
test("buildSourceFilePaths with sid also produces rg-free fallback", (t) => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-prompts-fallback-sid-"));
  t.after(() => rmSync(tmp, { recursive: true, force: true }));
