...
import { buildExecOptions, executeTacExec } from '../tools/exec-tool.ts';
import { isContextModeEnabled } from '../preferences-types.ts';

function freshBase(): string {
  return mkdtempSync(join(tmpdir(), 'tac-exec-test-'));
}
...
test('runExecSandbox: captures stdout, persists artifacts, returns digest', async () => {
  ...
  try {
    ...
    assert.ok(result.stdout_path.startsWith(join(base, '.tac', 'exec')), 'stdout path under .tac/exec');
...
test('runExecSandbox: forwards only allowlisted env vars', async () => {
  ...
  try {
    const result = await runExecSandbox(
      { runtime: 'bash', script: 'echo PATH=$PATH SECRET=$TAC_TEST_SECRET' },
      baseOpts(base, {
        env_allowlist: [],
        env: { PATH: '/usr/bin:/bin', HOME: '/tmp', TAC_TEST_SECRET: 'should-be-blocked' },
      }),
    );
    const stdout = readFileSync(result.stdout_path, 'utf-8');
...
test('executeTacExec: runs by default when context_mode is unset', async () => {
  const base = freshBase();
  try {
    const result = await executeTacExec(
    ...
    assert.equal(result.details.operation, 'tac_exec');
...
test('executeTacExec: runs when preferences is null (fresh project)', async () => {
  const base = freshBase();
  try {
    const result = await executeTacExec(
...
test('executeTacExec: blocked only when context_mode.enabled=false', async () => {
  const base = freshBase();
  try {
    const result = await executeTacExec(
...
test('executeTacExec: runs when enabled explicitly set to true', async () => {
  const base = freshBase();
  try {
    const result = await executeTacExec(
...
test('executeTacExec: rejects empty script', async () => {
  const base = freshBase();
  try {
    const result = await executeTacExec(
      { runtime: 'bash', script: '   ' },
