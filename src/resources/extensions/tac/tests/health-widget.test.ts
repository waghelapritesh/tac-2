...
function makeTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `tac-health-widget-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
test("detectHealthWidgetProjectState: no .tac returns none", (t) => {
...
test("detectHealthWidgetProjectState: bootstrapped .tac without milestones returns initialized", (t) => {
...
test("buildHealthLines: none state shows onboarding copy", (t) => {
  assert.deepEqual(buildHealthLines(activeData({ projectState: "none" })), [
    "  TAC  No project loaded — run /tac to start",
  ]);
});
...
test("buildHealthLines: initialized state shows continue setup copy", (t) => {
  assert.deepEqual(buildHealthLines(activeData({ projectState: "initialized" })), [
    "  TAC  Project initialized — run /tac to continue setup",
  ]);
});
...
test("session_start bootstraps the health widget alongside notifications", async (t) => {
  ...
  assert.ok(widgets.includes("tac-health"), "health widget is bootstrapped");
  assert.ok(
