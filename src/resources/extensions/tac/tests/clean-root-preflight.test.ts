...
function createTempRepo(): string {
  const dir = realpathSync(mkdtempSync(join(tmpdir(), "tac-preflight-test-")));
  run("git init", dir);
...
test("preflightCleanRoot — dirty tree warns user and auto-stashes", () => {
  ...
  try {
    ...
    const stashList = run("git stash list", repo);
    assert.ok(stashList.includes("tac-preflight-stash"), "stash entry must be named tac-preflight-stash");
  } finally {
    try { rmSync(repo, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }); } catch { /* ignore */ }
  }
});
