...
test("#2865: no dispatchWorkflow with tac-discuss customType uses plan-milestone", () => {
  // Match dispatchWorkflow calls where "tac-discuss" appears before "plan-milestone"
  ...
  for (const block of blocks) {
    ...
    if (call.includes('"tac-discuss"') && call.includes('"plan-milestone"')) {
      assert.fail(`Discuss dispatch should not use plan-milestone: ...dispatchWorkflow(${call.slice(0, 120).trim()}...`);
    }
  }
});

test("#2865: no dispatchWorkflow with tac-discuss customType uses plan-slice", () => {
  const blocks = guidedFlowSrc.split(/dispatchWorkflow\(/);
  for (const block of blocks) {
    const callEnd = block.indexOf(");");
    if (callEnd === -1) continue;
    const call = block.slice(0, callEnd);
    if (call.includes('"tac-discuss"') && call.includes('"plan-slice"')) {
