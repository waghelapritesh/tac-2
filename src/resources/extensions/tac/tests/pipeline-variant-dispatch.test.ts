// TAC-2 — #4781 phase 2: dispatch-rule gates read pipeline variant from DB.
...
function makeBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-2peline-variant-"));
...
test("#4781 phase 2: validate-milestone rule writes pass-through VALIDATION for trivial variant", async (t) => {
  ...
  const { writeFileSync } = await import("node:fs");
  writeFileSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md"), "# S01\n");
  writeFileSync(join(base, ".tac", "milestones", "M001", "slices", "S02", "S02-SUMMARY.md"), "# S02\n");
