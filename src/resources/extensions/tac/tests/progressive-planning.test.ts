// TAC Extension — ADR-011 Progressive Planning tests
...
function makeFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-"));
  mkdirSync(join(base, ".tac"), { recursive: true });
  mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S02"), { recursive: true });
...
function writePreferences(base: string, phasesBlock: string): void {
  const prefsPath = join(base, ".tac", "PREFERENCES.md");
  const body = [
    "---",
    "version: 1",
    phasesBlock,
...
function writeS01Artifacts(base: string): void {
  writeFileSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-PLAN.md"), "# S01 Plan\n");
  writeFileSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md"), "# S01 Summary\n");
}
...
test("ADR-011: sketch slice + progressive_planning OFF → phase='planning' (backwards compat)", async (t) => {
  ...
  seedMilestoneWithSketchedS02(base);
  writeS01Artifacts(base);
  // Write a PREFERENCES.md without the flag so loadEffectiveTACPreferences finds
...
test("ADR-011: autoHealSketchFlags flips is_sketch=0 when PLAN file exists", async (t) => {
  ...
  autoHealSketchFlags("M001", (sid) => {
    const planPath = join(base, ".tac", "milestones", "M001", "slices", sid, `${sid}-PLAN.md`);
...
test("ADR-011: schema v16 is idempotent — re-opening DB preserves is_sketch and sketch_scope columns", async (t) => {
  const originalCwd = process.cwd();
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-schema-"));
  t.after(() => {
    try { closeDatabase(); } catch { /* noop */ }
...
test("ADR-011 ON CONFLICT: omitted isSketch preserves existing is_sketch=1", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-conflict-"));
  t.after(() => {
    try { closeDatabase(); } catch { /* noop */ }
    rmSync(base, { recursive: true, force: true });
  });
  openDatabase(join(base, "tac.db"));
  insertMilestone({ id: "M001", title: "Test", status: "active" });
...
test("ADR-011 ON CONFLICT: explicit isSketch=false clears existing sketch flag", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-conflict-false-"));
  t.after(() => {
    try { closeDatabase(); } catch { /* noop */ }
    rmSync(base, { recursive: true, force: true });
  });
...
test("ADR-011 ON CONFLICT: isSketch=true upgrades existing non-sketch to sketch", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-conflict-true-"));
  t.after(() => {
    try { closeDatabase(); } catch { /* noop */ }
    rmSync(base, { recursive: true, force: true });
  });
...
test("ADR-011 ON CONFLICT: empty-string sketchScope clears existing scope (not preserves it)", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-conflict-empty-"));
  t.after(() => {
    try { closeDatabase(); } catch { /* noop */ }
    rmSync(base, { recursive: true, force: true });
  });
...
test("ADR-011 P3 #19: refine-slice prompt incorporates prior slice findings + sketch scope as hard constraint", async (t) => {
  ...
  writeFileSync(
    join(base, ".tac", "milestones", "M001", "ROADMAP.md"),
    [
  ...
  writeFileSync(
    join(base, ".tac", "milestones", "M001", "slices", "S01", "S01-SUMMARY.md"),
    s01Findings,
  );
