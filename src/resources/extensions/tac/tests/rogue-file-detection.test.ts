...
import { openDatabase, closeDatabase, isDbAvailable, insertMilestone, insertSlice, insertTask, updateSliceStatus, upsertMilestonePlanning } from "../tac-db.ts";

// ── Helpers ──────────────────────────────────────────────────────────────────

function createTmpBase(): string {
  return realpathSync(mkdtempSync(join(tmpdir(), "tac-rogue-test-")));
}

/**
 * Create a minimal .tac/ directory structure with a task summary file.
 */
function createTaskSummaryOnDisk(basePath: string, mid: string, sid: string, tid: string): string {
  const tasksDir = join(basePath, ".tac", "milestones", mid, "slices", sid, "tasks");
...
/**
 * Create a minimal .tac/ directory structure with a slice summary file.
 */
function createSliceSummaryOnDisk(basePath: string, mid: string, sid: string): string {
  const sliceDir = join(basePath, ".tac", "milestones", mid, "slices", sid);
...
function createRoadmapOnDisk(basePath: string, mid: string): string {
  const milestoneDir = join(basePath, ".tac", "milestones", mid);
...
test("rogue detection: task summary on disk, no DB row → detected as rogue", () => {
  ...
  const dbPath = join(basePath, ".tac", "tac.db");
  mkdirSync(join(basePath, ".tac"), { recursive: true });
