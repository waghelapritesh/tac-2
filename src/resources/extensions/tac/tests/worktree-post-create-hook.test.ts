...
function makeTmpDir(): string {
  return mkdtempSync(join(tmpdir(), "tac-wt-hook-test-"));
}
...
test("returns error when hook script does not exist", () => {
  ...
  const wt = makeTmpDir();
  try {
    const result = runWorktreePostCreateHook(src, wt, ".tac/hooks/nonexistent");
...
test("executes hook script with correct SOURCE_DIR and WORKTREE_DIR env vars", () => {
  ...
  const wt = makeTmpDir();
  try {
    const hooksDir = join(src, ".tac", "hooks");
    ...
    const result = runWorktreePostCreateHook(src, wt, hookPath(".tac/hooks/post-create"));
...
test("returns error message when hook script fails", () => {
  ...
  try {
    ...
    const result = runWorktreePostCreateHook(src, wt, hookPath(".tac/hooks/failing-hook"));
    assert.ok(result !== null, "should return error string");
