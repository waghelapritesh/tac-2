// TAC Extension — Tests for extracted deriveStateFromDb helper functions
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-helpers-'));
...
describe('derive-state-helpers', () => {
  ...
  // ─── resolveSliceDependencies: TAC_SLICE_LOCK with missing slice ────
  test('resolveSliceDependencies: TAC_SLICE_LOCK pointing to non-existent slice returns blocked', async () => {
    const base = createFixtureBase();
    const origLock = process.env.TAC_SLICE_LOCK;
    try {
      ...
      process.env.TAC_SLICE_LOCK = 'S99';

      invalidateStateCache();
      const state = await deriveStateFromDb(base);

      assert.equal(state.phase, 'blocked', 'slice-lock-miss: phase is blocked');
      assert.ok(state.blockers.some(b => b.includes('TAC_SLICE_LOCK=S99')), 'slice-lock-miss: blocker mentions lock');
    } finally {
      if (origLock !== undefined) process.env.TAC_SLICE_LOCK = origLock;
      else delete process.env.TAC_SLICE_LOCK;
      closeDatabase();
      cleanup(base);
    }
  });

  // ─── resolveSliceDependencies: TAC_SLICE_LOCK with valid slice ──────
  test('resolveSliceDependencies: TAC_SLICE_LOCK targeting valid slice bypasses deps', async () => {
    const base = createFixtureBase();
    const origLock = process.env.TAC_SLICE_LOCK;
    try {
      writeFile(base, 'milestones/M001/M001-ROADMAP.md', ROADMAP_CONTENT);
      ...
      process.env.TAC_SLICE_LOCK = 'S02';
  ...
  test('deriveStateFromDb respects custom queue order from QUEUE-ORDER.json', async () => {
    ...
    try {
      ...
      writeFileSync(join(base, '.tac', 'QUEUE-ORDER.json'), queueOrder);
      writeFile(base, 'milestones/M001/M001-CONTEXT.md', '# M001\n\nContext.');
      writeFile(base, 'milestones/M002/M002-CONTEXT.md', '# M002\n\nContext.');
