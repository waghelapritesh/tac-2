 ...
 * untracked `.tac/milestones/M<queued>/` directories created by `/tac queue`
 ...
 * only needs to handle tracked dirty files. Untracked `.tac/` files are
...
test.before(() => {
  ...
  fakeHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-fake-home-")));
...
function createTempRepo(): string {
  ...
  writeFileSync(join(dir, ".tac", "STATE.md"), "version: 1\n");
  // In projects with tracked .tac/ files (hasGitTrackedTacFiles=true),
  // .tac is NOT added to .gitignore. This means untracked files under
  // .tac/ are visible to --include-untracked and get swept into the
  // stash, destroying queued milestone CONTEXT files (#2505).
  run("git add -f .tac/STATE.md", dir);
  run("git add .", dir);
...
function createTempRepoWithSymlinkedTac(): { repo: string; stateDir: string } {
  ...
  symlinkSync(stateDir, join(repo, ".tac"));
...
test("#2505: git stash --include-untracked sweeps queued CONTEXT files (demonstrates the bug)", () => {
  ...
  try {
    ...
    const m013Dir = join(dir, ".tac", "milestones", "M013");
    ...
    assert.ok(status.includes("?? .tac/milestones/"), "precondition: M013 dir is untracked");
...
test("#2505: mergeMilestoneToMain preserves queued CONTEXT files (not swept into stash)", () => {
  ...
  try {
    ...
    // Simulate `/tac queue` creating queued milestone CONTEXT files at the
    // project root. These are untracked, and in repos with tracked .tac/
    // files they are NOT gitignored.
    const m013Dir = join(repo, ".tac", "milestones", "M013");
    const m014Dir = join(repo, ".tac", "milestones", "M014");
    mkdirSync(m013Dir, { recursive: true });
    ...
    assert.ok(
      statusBefore.includes("?? .tac/milestones/"),
    ...
    assert.ok(
      result.commitMessage.includes("TAC-Milestone: M015"),
...
test("#2505: pre-merge stash handles symlinked .tac without traversing it", () => {
  const { repo, stateDir } = createTempRepoWithSymlinkedTac();
  try {
    ...
    assert.ok(result.commitMessage.includes("TAC-Milestone: M016"), "merge should succeed");
    assert.ok(existsSync(join(repo, "app.ts")), "milestone code merged to main");
    assert.equal(lstatSync(join(repo, ".tac")).isSymbolicLink(), true, ".tac symlink remains in place");
...
// #4573: When `.tac` is a gitignored symlink (ADR-002 layout) and the project
// `.gitignore` contains `.tac`, `git stash push --include-untracked -- <pathspec>`
// fatals with "The following paths are ignored by one of your .gitignore files".
// The prior tests used a symlinked `.tac` but no `.gitignore`, so this failure
// mode was invisible to CI. Fixture must include BOTH the symlink AND the
// ignore rule to reproduce the bug on pre-fix code.
test("#4573: gitignored .tac symlink does not break pre-merge stash", () => {
  ...
  try {
    ...
    assert.ok(
      result.commitMessage.includes("TAC-Milestone: M001"),
      "merge must succeed despite gitignored .tac symlink",
    );
    assert.ok(existsSync(join(repo, "app.ts")), "milestone code merged to main");
    assert.equal(
      lstatSync(join(repo, ".tac")).isSymbolicLink(),
      true,
      ".tac symlink remains in place",
    );
  } finally {
    rmSync(repo, { recursive: true, force: true });
    rmSync(stateDir, { recursive: true, force: true });
