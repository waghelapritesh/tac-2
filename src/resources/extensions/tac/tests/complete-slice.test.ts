...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-complete-slice-'));
...
/**
 * Create a temp project directory with .tac structure and roadmap for handler tests.
 */
function createTempProject(): { basePath: string; roadmapPath: string } {
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-slice-handler-'));
  const sliceDir = path.join(basePath, '.tac', 'milestones', 'M001', 'slices', 'S01');
  const tasksDir = path.join(sliceDir, 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });

  const roadmapPath = path.join(basePath, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md');
...
function makeValidSliceParams(): CompleteSliceParams {
  return {
    ...
    provides: ['complete_slice handler', 'tac_slice_complete tool'],
...
console.log('\n=== complete-slice: handler with missing roadmap ===');
{
  ...
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-no-roadmap-'));
  const sliceDir = path.join(basePath, '.tac', 'milestones', 'M001', 'slices', 'S01');
