...
test("system prompt references CODEBASE.md and /tac codebase", () => {
  const prompt = readPrompt("system");
  assert.match(prompt, /CODEBASE\.md/);
  assert.match(prompt, /\/tac codebase \[generate\|update\|stats\]/);
...
// ─── Prompt migration: execute-task → tac_complete_task ───────────────

test("execute-task prompt references tac_complete_task tool", () => {
  const prompt = readPrompt("execute-task");
  assert.match(prompt, /tac_complete_task/);
});

test("execute-task prompt uses tac_complete_task as canonical summary write path", () => {
  const prompt = readPrompt("execute-task");
...
test("guided-execute-task prompt references tac_task_complete tool", () => {
  const prompt = readPrompt("guided-execute-task");
  assert.match(prompt, /tac_task_complete/);
});
...
// ─── Prompt migration: complete-slice → tac_complete_slice ────────────

test("complete-slice prompt references tac_complete_slice tool", () => {
  const prompt = readPrompt("complete-slice");
  assert.match(prompt, /tac_complete_slice/);
});
...
test("guided-complete-slice prompt references tac_slice_complete tool", () => {
  const prompt = readPrompt("guided-complete-slice");
  assert.match(prompt, /tac_slice_complete/);
});
...
test("validate-milestone prompt uses tac_validate_milestone as canonical validation write path", () => {
  const prompt = readPrompt("validate-milestone");
  assert.match(prompt, /tac_validate_milestone/);
...
test("plan-milestone prompt references DB-backed planning tool and explicitly forbids manual roadmap writes", () => {
  ...
  assert.match(prompt, /tac_plan_milestone/);
...
test("plan-slice prompt explicitly names tac_plan_slice as DB-backed planning tool", () => {
  const prompt = readPrompt("plan-slice");
  assert.match(prompt, /tac_plan_slice/);
  assert.match(prompt, /tac_plan_task/);
...
test("plan-slice prompt clarifies tac_plan_slice handles task persistence", () => {
  const prompt = readPrompt("plan-slice");
  // tac_plan_slice persists tasks in its transaction — no separate tac_plan_task calls needed
  assert.match(prompt, /tac_plan_task/);
  assert.match(prompt, /tac_plan_slice` handles task persistence/i);
});

test("replan-slice prompt uses tac_replan_slice as canonical DB-backed tool", () => {
  const prompt = readPrompt("replan-slice");
  assert.match(prompt, /tac_replan_slice/);
...
test("refine-slice prompt names tac_plan_slice as the DB-backed write path", () => {
  const prompt = readPrompt("refine-slice");
  assert.match(prompt, /tac_plan_slice/, "refine-slice must call tac_plan_slice to persist");
});
...
test("reassess-roadmap prompt references tac_reassess_roadmap tool", () => {
  const prompt = readPrompt("reassess-roadmap");
  assert.match(prompt, /tac_reassess_roadmap/);
});
...
// ─── Prompt migration: replan-slice → tac_replan_slice ────────────────

test("replan-slice prompt names tac_replan_slice as the tool to use", () => {
  const prompt = readPrompt("replan-slice");
  assert.match(prompt, /tac_replan_slice/);
});

// ─── Prompt migration: reassess-roadmap → tac_reassess_roadmap ───────

test("reassess-roadmap prompt names tac_reassess_roadmap as the tool to use", () => {
  const prompt = readPrompt("reassess-roadmap");
  assert.match(prompt, /tac_reassess_roadmap/);
});
...
test("execute-task prompt uses camelCase parameter names matching TypeBox schema", () => {
  const prompt = readPrompt("execute-task");
  // The tac_complete_task tool schema uses camelCase: milestoneId, sliceId, taskId
  // Prompts must NOT tell the LLM to use snake_case (milestone_id, slice_id, task_id)
  const toolCallLine = prompt.split("\n").find((l) => /tac_complete_task/.test(l) || /tac_task_complete/.test(l));
  assert.ok(toolCallLine, "prompt must contain a tac_complete_task or tac_task_complete tool call line");
...
test("complete-slice prompt uses camelCase parameter names matching TypeBox schema", () => {
  const prompt = readPrompt("complete-slice");
  // The tac_complete_slice tool schema uses camelCase: milestoneId, sliceId
  const toolCallLine = prompt.split("\n").find((l) => /tac_complete_slice/.test(l) || /tac_slice_complete/.test(l));
  assert.ok(toolCallLine, "prompt must contain a tac_complete_slice or tac_slice_complete tool call line");
  assert.doesNotMatch(toolCallLine!, /milestone_id/, "must use milestoneId, not milestone_id");
  assert.doesNotMatch(toolCallLine!, /slice_id/, "must use sliceId, not slice_id");
  // Positive: must mention the camelCase names
  assert.match(toolCallLine!, /milestoneId/);
