/**
 * Tests that /tac quick is blocked when auto-mode is active.
 *
 * Relates to #2417: /tac quick freezes terminal when auto-mode is active.
...
describe("/tac quick auto-mode guard (#2417)", () => {
  ...
  it("guard shows error message mentioning /tac stop", () => {
    ...
    // so assert on the message fragment and the /tac stop instruction.
    assert.ok(
