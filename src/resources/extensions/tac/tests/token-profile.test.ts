 ...
 * @tac/pi-coding-agent import resolution issues in dev environments.
 */
...
// ═══════════════════════════════════════════════════════════════════════════
// TACPreferences Interface
// ═══════════════════════════════════════════════════════════════════════════

test("preferences: TACPreferences includes token_profile field", () => {
  assert.ok(
    preferencesSrc.includes("token_profile?: TokenProfile"),
    "TACPreferences should have token_profile field",
  );
});

test("preferences: TACPreferences includes phases field", () => {
  assert.ok(
    preferencesSrc.includes("phases?: PhaseSkipPreferences"),
    "TACPreferences should have phases field",
  );
});

test("preferences: TACModelConfig includes subagent field", () => {
  // Check both v1 and v2 configs
  const v1Match = preferencesSrc.match(/interface TACModelConfig\s*\{[^}]*subagent/);
  assert.ok(v1Match, "TACModelConfig should have subagent field");
  const v2Match = preferencesSrc.match(/interface TACModelConfigV2\s*\{[^}]*subagent/);
  assert.ok(v2Match, "TACModelConfigV2 should have subagent field");
});
