 ...
 * that tac_complete_task ran. A heading alone does not prove completion —
...
import { closeDatabase, isDbAvailable } from "../tac-db.ts";

/** Scaffold .tac/milestones/M001/slices/S01/ with tasks/ and a T01-SUMMARY.md. */
function scaffoldProject(t: { after: (fn: () => void) => void }): {
  base: string;
  planPath: string;
} {
  const base = mkdtempSync(join(tmpdir(), "tac-verify-artifact-"));
  t.after(() => {
    closeDatabase();
