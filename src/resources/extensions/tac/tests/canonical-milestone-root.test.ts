 ...
 * live git worktree exists for that milestone at .tac/worktrees/<MID>/;
...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-canon-test-${randomUUID()}`);
...
/**
 * Create a worktree directory shape that looks live: the .tac/worktrees/<MID>/
...
function makeLiveWorktree(base: string, mid: string): string {
  const wtPath = join(base, ".tac", "worktrees", mid);
