// TAC-2 — Regression test for #3616: discuss tool scoping must not leak into subsequent sessions
 ...
 * was unchanged. This caused tac_plan_slice and other DB tools to be
 ...
 *   2. The narrowed set explicitly excludes tac_plan_slice (a HEAVY_TOOL)
...
describe("#3616 — discuss tool scoping must not leak across sessions", () => {
	test("tac_plan_slice is NOT in DISCUSS_TOOLS_ALLOWLIST", () => {
		assert.ok(
			!DISCUSS_TOOLS_ALLOWLIST.includes("tac_plan_slice"),
			"tac_plan_slice should be excluded from discuss scope (it's a heavy planning tool)",
		);
	});
