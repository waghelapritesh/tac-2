...
describe("parallel-worker-monitoring", () => {
  ...
  it("refreshWorkerStatuses restores persisted workers from disk", () => {
    const base = mkdtempSync(join(tmpdir(), "tac-parallel-monitoring-"));
    try {
      mkdirSync(join(base, ".tac"), { recursive: true });
      writeFileSync(join(base, ".tac", "orchestrator.json"), JSON.stringify({
        active: true,
        workers: [
          {
            milestoneId: "M001",
  ...
  it("refreshWorkerStatuses restores persisted workers from live session status files", () => {
    const base = mkdtempSync(join(tmpdir(), "tac-parallel-stderr-"));
    try {
      mkdirSync(join(base, ".tac", "parallel"), { recursive: true });
      writeFileSync(join(base, ".tac", "parallel", "M009.status.json"), JSON.stringify({
        milestoneId: "M009",
        pid: process.pid,
        state: "running",
        currentUnit: null,
