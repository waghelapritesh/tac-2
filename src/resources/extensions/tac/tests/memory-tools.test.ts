...
test('memory-tools: tac_graph build acknowledges request', () => {
  openDatabase(':memory:');
  const result = executeTacGraph({ mode: 'build' });
  assert.ok(!result.isError);
  assert.equal(result.details.operation, 'tac_graph');
  assert.equal(result.details.mode, 'build');
  closeDatabase();
});

test('memory-tools: tac_graph query rejects missing memoryId', () => {
  openDatabase(':memory:');
  const result = executeTacGraph({ mode: 'query' });
  assert.ok(result.isError);
  assert.equal(result.details.error, 'missing_memory_id');
  closeDatabase();
});

test('memory-tools: tac_graph query returns node + supersedes edge', () => {
  openDatabase(':memory:');
  createMemory({ category: 'convention', content: 'first' });
  createMemory({ category: 'convention', content: 'second' });
  supersedeMemory('MEM001', 'MEM002');

  const result = executeTacGraph({ mode: 'query', memoryId: 'MEM001', depth: 1 });
  assert.ok(!result.isError);
...
test('memory-tools: tac_graph query returns empty when memoryId does not exist', () => {
  openDatabase(':memory:');
  const result = executeTacGraph({ mode: 'query', memoryId: 'MEM999' });
  assert.ok(!result.isError, 'missing memory is not an error, just empty');
  assert.equal((result.details.nodes as unknown[]).length, 0);
  closeDatabase();
});

test('memory-tools: tac_graph errors when DB is closed', () => {
  closeDatabase();
  const result = executeTacGraph({ mode: 'query', memoryId: 'MEM001' });
  assert.ok(result.isError);
  assert.equal(result.details.error, 'db_unavailable');
});
