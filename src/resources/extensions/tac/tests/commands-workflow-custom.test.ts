/**
 * commands-workflow-custom.test.ts — Tests for `/tac workflow` subcommands
...
import { getTacArgumentCompletions, TOP_LEVEL_SUBCOMMANDS } from "../commands/catalog.ts";
...
function writeDefinition(basePath: string, name: string, content: string): void {
  const defsDir = join(basePath, ".tac", "workflow-defs");
...
describe("workflow catalog registration", () => {
  ...
  it("getTacArgumentCompletions('m') includes model", () => {
    const completions = getTacArgumentCompletions("m");
  ...
  it("getTacArgumentCompletions('workflow ') returns the full subcommand set", () => {
    const completions = getTacArgumentCompletions("workflow ");
    const labels = completions.map((c: any) => c.label);
    for (const sub of [
  ...
  it("getTacArgumentCompletions('workflow r') filters to run and resume", () => {
    const completions = getTacArgumentCompletions("workflow r");
    const labels = completions.map((c: any) => c.label);
  ...
  it("getTacArgumentCompletions('workflow run ') returns definition names", () => {
    const base = makeTmpBase();
    writeDefinition(base, "deploy-pipeline", SIMPLE_DEF);
    writeDefinition(base, "test-suite", SIMPLE_DEF);

    // Change cwd so the completion scanner can find `.tac/workflow-defs/`
    process.chdir(base);

    const completions = getTacArgumentCompletions("workflow run ");
    const labels = completions.map((c: any) => c.label);
  ...
  it("getTacArgumentCompletions('workflow validate ') returns definition names", () => {
    const base = makeTmpBase();
    writeDefinition(base, "my-workflow", SIMPLE_DEF);

    process.chdir(base);

    const completions = getTacArgumentCompletions("workflow validate ");
    const labels = completions.map((c: any) => c.label);
    assert.ok(labels.includes("my-workflow"), "should include my-workflow");
  });

  it("getTacArgumentCompletions('workflow run d') filters by prefix", () => {
    const base = makeTmpBase();
    writeDefinition(base, "deploy-pipeline", SIMPLE_DEF);
    writeDefinition(base, "test-suite", SIMPLE_DEF);
    ...
    const completions = getTacArgumentCompletions("workflow run d");
    const labels = completions.map((c: any) => c.label);
    assert.ok(labels.includes("deploy-pipeline"), "should include deploy-pipeline");
...
describe("workflow command handler", () => {
  ...
  it("bare '/tac workflow' lists plugins grouped by mode", async () => {
  ...
  it("'/tac workflow new' shows skill invocation message", async () => {
  ...
  it("'/tac workflow run' without name shows usage warning", async () => {
  ...
  it("'/tac workflow run nonexistent' shows error for missing definition", async () => {
  ...
  it("'/tac workflow validate' without name shows usage warning", async () => {
  ...
  it("'/tac workflow validate nonexistent' shows definition not found", async () => {
  ...
  it("'/tac workflow pause' without custom engine shows warning", async () => {
  ...
  it("'/tac workflow resume' without custom engine shows warning", async () => {
  ...
  it("'/tac workflow unknown-sub' shows unknown subcommand", async () => {
  ...
  it("'/tac workflow list' with no runs shows empty message", async () => {
