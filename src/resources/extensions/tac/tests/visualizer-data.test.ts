// Tests for TAC visualizer data loader.
...
assert.ok(
  dataSrc.includes("loadEffectiveTACPreferences"),
  "uses loadEffectiveTACPreferences",
);

assert.ok(
  dataSrc.includes("resolveTacRootFile"),
  "uses resolveTacRootFile for KNOWLEDGE path",
);
...
assert.ok(
  overlaySrc.includes("export class TACVisualizerOverlay"),
  "exports TACVisualizerOverlay class",
);
...
assert.ok(
  coreHandlerSrc.includes("TACVisualizerOverlay"),
  "core.ts imports TACVisualizerOverlay",
);
