...
describe("checkFilePathConsistency additional edge cases", () => {
  ...
  test("backticked path cited mid-sentence resolves to the path", () => {
    ...
    writeFileSync(join(tempDir, ".tac/REQUIREMENTS.md"), "# Requirements");

    try {
      const tasks = [
        createTask({
          id: "T01",
          inputs: ["R014 verbatim text from `.tac/REQUIREMENTS.md` (the owned requirement statement)"],
        }),
      ];
...
describe("normalizeFilePath tilde expansion (#4446)", () => {
  ...
  test("expands ~/ prefixed paths to homedir", () => {
    assert.equal(
      normalizeFilePath("~/.tac/agent/extensions/tac/native-git-bridge.js"),
      join(homedir(), ".tac/agent/extensions/tac/native-git-bridge.js"),
    );
  });
});
...
describe("checkFilePathConsistency directory inputs (#4446)", () => {
  ...
  test("tilde-prefixed input is matched against $HOME, not the project basePath", (t) => {
    ...
    mkdirSync(join(fakeHome, ".tac"), { recursive: true });
    writeFileSync(join(fakeHome, ".tac/tool.js"), "// present");
