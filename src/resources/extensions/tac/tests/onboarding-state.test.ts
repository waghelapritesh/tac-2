// TAC — Onboarding state record tests.
...
// Tests must isolate per-process to avoid clobbering the user's real ~/.tac/agent/onboarding.json.
// We point TAC_HOME at a fresh tmp dir before importing the modules under test.
const tmpHome = mkdtempSync(join(tmpdir(), "tac-onboarding-state-test-"));
