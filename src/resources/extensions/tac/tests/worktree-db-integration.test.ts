 ...
 *
 * Test cases:
 *   1. Copy: createAutoWorktree seeds .tac/tac.db into the worktree when main has one
 *   2. Copy-skip: createAutoWorktree silently skips when main has no tac.db
...
describe('worktree-db-integration', async () => {
  ...
  try {
    ...
    console.log("\n=== Test 1: copy on worktree creation ===");
    {
      ...
      // Seed a tac.db in the main repo
      const tacDir = join(tempDir, ".tac");
      mkdirSync(tacDir, { recursive: true });
      const mainDbPath = join(tacDir, "tac.db");
      openDatabase(mainDbPath);
      closeDatabase();

      // Commit so createAutoWorktree can copy planning artifacts
      run("git add .", tempDir);
      run('git commit -m "add tac dir"', tempDir);

      // createAutoWorktree should copy the DB into the worktree
      const wtPath = createAutoWorktree(tempDir, "M004");

      const worktreeDbPath = join(worktreePath(tempDir, "M004"), ".tac", "tac.db");
      assert.ok(
        existsSync(worktreeDbPath),
        "tac.db exists in worktree .tac after createAutoWorktree",
      );
    ...
    console.log("\n=== Test 2: copy skip when no source DB ===");
    {
      ...
      assert.ok(
        !existsSync(worktreeDbPath),
        "tac.db is absent in worktree when source had none",
      );
    ...
    console.log("\n=== Test 4: reconcile non-fatal on nonexistent paths ===");
    {
      let threw = false;
      try {
        reconcileWorktreeDb("/nonexistent/path/tac.db", "/also/nonexistent/tac.db");
      } catch {
        threw = true;
      }
