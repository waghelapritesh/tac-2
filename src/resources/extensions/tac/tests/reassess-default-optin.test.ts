// TAC-2 — ADR-003 §4 behavior contract: reassess-roadmap is opt-in.
...
import type { TACPreferences } from "../preferences.ts";

const REASSESS_RULE_NAME = "reassess-roadmap (post-completion)";

function makeIsolatedBase(): string {
  const base = join(tmpdir(), `tac-reassess-default-${randomUUID()}`);
  mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S01", "tasks"), { recursive: true });
  return base;
}

function makeCtx(prefs: TACPreferences | undefined, basePath: string): DispatchContext {
...
const guardCases: Array<{ name: string; prefs: TACPreferences | undefined; message?: string }> = [
  {
  ...
  },
  {
    ...
    prefs: {} as TACPreferences,
  },
  {
    name: "phases.reassess_after_slice is explicitly false",
    prefs: { phases: { reassess_after_slice: false } } as unknown as TACPreferences,
  },
  {
    name: "phases.skip_reassess is true (short-circuit guard preserved)",
    prefs: { phases: { skip_reassess: true, reassess_after_slice: true } } as unknown as TACPreferences,
...
test("ADR-003 §4: reassess-roadmap opt-in path dispatches after reassessment detection", async (t) => {
  ...
  const prefs = { phases: { reassess_after_slice: true } } as unknown as TACPreferences;
...
test("ADR-003 §4: plan-slice prompt and MCP tool agree on reassess sliceChanges shape", () => {
  ...
  assert.match(planSlicePrompt, /tac_reassess_roadmap/);
  ...
  assert.match(dbToolsSource, /name:\s*"tac_reassess_roadmap"/);
