...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-queue-order-'));
...
test('test block at line 76', () => {
  ...
  const raw = JSON.parse(readFileSync(join(base, '.tac', 'QUEUE-ORDER.json'), 'utf-8'));
...
test('test block at line 90', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.tac', 'QUEUE-ORDER.json'), 'not json');
...
test('test block at line 98', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.tac', 'QUEUE-ORDER.json'), '{"order": "invalid"}');
...
test('test block at line 121', () => {
  ...
  assert.ok(!existsSync(join(base, '.tac', 'QUEUE-ORDER.json')), 'prune does not create file');
  cleanup(base);
});
