/**
 * Tests for /tac scan command — pure functions + handler integration
 */
...
function makeTmpDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-scan-test-"));
  tmpDirs.push(dir);
  return dir;
}
...
describe("buildScanOutputPaths", () => {
  test("returns paths under .tac/codebase/ relative to basePath", () => {
    const base = "/some/project";
    const paths = buildScanOutputPaths("tech", base);
    assert.ok(paths.every((p) => p.startsWith(join(base, ".tac", "codebase"))));
  });
...
describe("handleScan — handler integration", () => {
  test("creates .tac/codebase/ directory", async (t) => {
    const tmpDir = makeTmpDir();
    ...
    assert.ok(existsSync(join(tmpDir, ".tac", "codebase")));
  });
  ...
  test("warns when documents already exist", async (t) => {
    ...
    const codesbaseDir = join(tmpDir, ".tac", "codebase");
