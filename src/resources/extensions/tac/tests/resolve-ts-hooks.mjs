...
export function resolve(specifier, context, nextResolve) {
  ...
  if (specifier.includes('@tac/')) {
    tsSpecifier = specifier.replace('@tac/', PACKAGES_ROOT).replace('/dist/', '/src/');
    if (tsSpecifier.includes('/packages/pi-ai') && !tsSpecifier.endsWith('.ts')) {
        ...
        // Fallback for other tac packages like pi-coding-agent, pi-tui, pi-agent-core
