...
function makeTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `tac-register-shortcuts-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
test("dashboard shortcut resolves the project root instead of the current worktree path", async (t) => {
  ...
  const worktreeRoot = join(projectRoot, ".tac", "worktrees", "M001");
  ...
  assert.equal(notices.length, 0, "shortcut does not fall back to the missing-.tac warning");
  assert.equal(shortcuts.length, 5, "all TAC shortcuts are still registered");
...
test("parallel shortcut passes resolved project root into overlay", async (t) => {
  ...
  const worktreeRoot = join(base, ".tac", "worktrees", "M001");
  mkdirSync(join(base, ".tac", "parallel"), { recursive: true });
  mkdirSync(worktreeRoot, { recursive: true });
