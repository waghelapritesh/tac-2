...
test("formatNotificationTitle returns 'TAC' when no project name is given", () => {
  assert.equal(formatNotificationTitle(), "TAC");
  assert.equal(formatNotificationTitle(undefined), "TAC");
  assert.equal(formatNotificationTitle(""), "TAC");
});

test("formatNotificationTitle includes project name when provided", () => {
  assert.equal(formatNotificationTitle("my-app"), "TAC — my-app");
});

test("formatNotificationTitle trims whitespace from project name", () => {
  assert.equal(formatNotificationTitle("  spaced  "), "TAC — spaced");
});
...
test("buildDesktopNotificationCommand includes project name in title on linux", () => {
  ...
  assert.ok(command);
  assert.equal(command.args[2], "TAC — my-project");
...
test("buildDesktopNotificationCommand includes project name in title on macOS", () => {
  ...
  if (command.file.includes("terminal-notifier")) {
    ...
    assert.equal(command.args[titleIdx + 1], "TAC — my-project");
  } else {
    assert.match(command.args[1], /TAC — my-project/);
  }
});
