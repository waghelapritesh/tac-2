...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-derive-db-'));
...
describe('derive-state-db', async () => {
  ...
  // ─── Test 1: DB-backed deriveState produces identical TACState ─────────
...
`;
    ...
    try {
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M001'), { recursive: true });
      mkdirSync(join(base, '.tac', 'milestones', 'M002'), { recursive: true });
...
`;
  ...
  test('derive-state-db: replanning-slice via DB', async () => {
    ...
    try {
      ...
      const { _getAdapter } = await import('../tac-db.ts');
...
`;
  ...
  test('derive-state-db: ghost milestone skipped when no DB row and no worktree', async () => {
    ...
    try {
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M001'), { recursive: true });
      writeFileSync(join(base, '.tac', 'milestones', 'M001', 'META.json'), '{}');
      // Real milestone
  ...
  test('derive-state-db: disk-only milestone auto-synced into DB (#2416)', async () => {
    ...
    try {
      ...
      openDatabase(':memory:');
      // Only insert M001 — simulates the state after migration guard ran then /tac queue added M002
  ...
  test('derive-state-db: queued milestone row survives tac_plan_milestone INSERT OR IGNORE', async () => {
    try {
      openDatabase(':memory:');

      // Simulates tac_milestone_generate_id inserting a minimal queued row
      ...
      // Simulates tac_plan_milestone calling insertMilestone (INSERT OR IGNORE)
  ...
  test('derive-state-db: queued milestone with worktree not flagged as ghost (#2921)', async () => {
    ...
    try {
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M002', 'slices'), { recursive: true });
