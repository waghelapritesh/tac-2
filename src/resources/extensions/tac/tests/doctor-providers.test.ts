...
test("formatProviderReport shows error icon and detail for error status", () => {
  const results: ProviderCheckResult[] = [{
    ...
    detail: "Set ANTHROPIC_API_KEY or run /tac keys",
    required: true,
  }];
  const out = formatProviderReport(results);
...
test("runProviderChecks detects Anthropic key from ANTHROPIC_API_KEY env var", () => {
  // Isolate from real HOME so loadEffectiveTACPreferences returns null (default → anthropic)
  // and auth.json lookups hit an empty directory.
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-env-test-")));
...
test("runProviderChecks returns error for Anthropic when no key present", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-test-")));
  withEnv({
...
test("runProviderChecks optional providers show unconfigured when no key", () => {
  withEnv(
    ...
    () => {
      ...
      process.env.HOME = mkdtempSync(join(tmpdir(), "tac-providers-test-"));
      try {
        const results = runProviderChecks();
...
test("runProviderChecks detects key from auth.json", () => {
  withEnv({ ANTHROPIC_API_KEY: undefined }, () => {
    const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-test-")));
    const agentDir = join(tmpHome, ".tac", "agent");
...
test("runProviderChecks detects custom provider keys from models.json", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-custom-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-custom-repo-")));
  const agentDir = join(tmpHome, ".tac", "agent");
  mkdirSync(agentDir, { recursive: true });
  mkdirSync(join(repo, ".tac"), { recursive: true });
...
test("runProviderChecks reports missing custom provider key without models.json apiKey", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-custom-missing-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-custom-missing-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
...
test("runProviderChecks reports ok for Anthropic when GitHub Copilot env var is set", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-copilot-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
...
test("runProviderChecks reports ok for Anthropic via GITHUB_TOKEN cross-provider routing", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-ghtoken-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
    COPILOT_GITHUB_TOKEN: undefined,
...
test("runProviderChecks detects ANTHROPIC_OAUTH_TOKEN as valid Anthropic auth", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-oauth-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
...
test("runProviderChecks reports ok via Copilot auth.json for Anthropic", () => {
  withEnv({
    ...
    const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-copilot-auth-test-")));
    const agentDir = join(tmpHome, ".tac", "agent");
    mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks uses provider-qualified anthropic-vertex model IDs", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-vertex-prefix-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-vertex-prefix-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
  writeFileSync(
    join(repo, ".tac", "PREFERENCES.md"),
    [
...
test("runProviderChecks uses object provider field for anthropic-vertex models", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-vertex-provider-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-vertex-provider-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
  writeFileSync(
    join(repo, ".tac", "PREFERENCES.md"),
    [
...
test("runProviderChecks reports ok for Google via google-gemini-cli auth.json (#2922)", () => {
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-gemini-cli-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
  writeFileSync(
    join(repo, ".tac", "PREFERENCES.md"),
    [
  ...
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-gemini-cli-home-")));
  const agentDir = join(tmpHome, ".tac", "agent");
  mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks reports ok for OpenAI via openai-codex auth.json (#2922)", () => {
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-codex-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
  writeFileSync(
    join(repo, ".tac", "PREFERENCES.md"),
    [
  ...
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-codex-home-")));
  const agentDir = join(tmpHome, ".tac", "agent");
  mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks reports ok for claude-code without any API key", () => {
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-cc-repo-")));
  mkdirSync(join(repo, ".tac"), { recursive: true });
  writeFileSync(
    join(repo, ".tac", "PREFERENCES.md"),
    [
  ...
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-cc-home-")));
...
test("runProviderChecks reports ok for Anthropic via claude-code binary in PATH", () => {
  ...
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-cc-route-home-")));
...
test("runProviderChecks detects claude.cmd in PATH on Windows (#4503)", { skip: process.platform !== "win32" }, () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-cc-win-route-home-")));
  const binDir = join(tmpHome, "bin");
  mkdirSync(binDir, { recursive: true });
...
test("runProviderChecks detects claude.exe in PATH on Windows (#4548)", { skip: process.platform !== "win32" }, () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-providers-cc-exe-home-")));
  const binDir = join(tmpHome, "bin");
  mkdirSync(binDir, { recursive: true });
