...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-resume-wt-${randomUUID()}`);
...
function writePausedSession(
  ...
  writeFileSync(
    join(base, ".tac", "runtime", "paused-session.json"),
    JSON.stringify({
      milestoneId,
...
function makeWorktreePath(base: string, milestoneId: string): string {
  return join(base, ".tac", "worktrees", milestoneId);
}
...
test("readPausedSessionMetadata round-trips worktreePath from paused-session.json", () => {
  ...
  try {
    ...
    const pausedPath = join(base, ".tac", "runtime", "paused-session.json");
