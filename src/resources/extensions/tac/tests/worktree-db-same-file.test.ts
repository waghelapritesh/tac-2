...
describe("#2823: reconcileWorktreeDb same-file guard", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "tac-2823-"));
  });
  ...
  test("returns zero result when both paths resolve to the same file", () => {
    const mainTac = join(tmpDir, "main", ".tac");
    mkdirSync(mainTac, { recursive: true });
    const mainDbPath = join(mainTac, "tac.db");
    ...
    const wtTac = join(tmpDir, "worktree", ".tac");
    mkdirSync(join(tmpDir, "worktree"), { recursive: true });
    symlinkSync(mainTac, wtTac, "junction");
    const worktreeDbPath = join(wtTac, "tac.db");
  ...
  test("returns zero result when both paths are identical strings", () => {
    const mainTac = join(tmpDir, "project", ".tac");
    mkdirSync(mainTac, { recursive: true });
    const dbPath = join(mainTac, "tac.db");
  ...
  test("still reconciles when paths are genuinely different files", () => {
    ...
    const wtTac = join(tmpDir, "worktree", ".tac");
    mkdirSync(wtTac, { recursive: true });
    const worktreeDbPath = join(wtTac, "tac.db");
