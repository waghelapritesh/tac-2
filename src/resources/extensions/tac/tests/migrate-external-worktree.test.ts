...
describe("migrate-external worktree guard (#2970)", () => {
  ...
  before(() => {
    base = realpathSync(mkdtempSync(join(tmpdir(), "tac-migrate-wt-")));
    ...
    // Populate worktree with a .tac directory (simulating syncTacStateToWorktree)
    const worktreeTac = join(worktreePath, ".tac");
    mkdirSync(worktreeTac, { recursive: true });
    writeFileSync(join(worktreeTac, "PREFERENCES.md"), "# prefs\n", "utf-8");
  });
  ...
  test("migrateToExternalState skips when basePath is a git worktree", () => {
    // The worktree has a real .tac directory — migration would normally run.
    ...
    // .tac should still exist as a real directory (not renamed/removed)
    assert.ok(
      existsSync(join(worktreePath, ".tac")),
      ".tac directory should still exist after skipped migration"
    );

    // .tac.migrating should NOT exist
    assert.ok(
      !existsSync(join(worktreePath, ".tac.migrating")),
      ".tac.migrating should not be created in a worktree"
    );
  });

  test("migrateToExternalState still works on main repo", () => {
    // Create a fresh temp repo to test main repo migration path
    const mainBase = realpathSync(mkdtempSync(join(tmpdir(), "tac-migrate-main-")));
    try {
      ...
      // Create a .tac directory with content
      mkdirSync(join(mainBase, ".tac"), { recursive: true });
      writeFileSync(join(mainBase, ".tac", "PREFERENCES.md"), "# prefs\n", "utf-8");
