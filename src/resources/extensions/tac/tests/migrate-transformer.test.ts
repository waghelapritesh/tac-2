...
// Tests for transforming parsed PlanningProject into TACProject structures.
// Uses synthetic in-memory fixtures — no filesystem needed.
// Transformer is pure: PlanningProject → TACProject.

import { transformToTAC } from '../migrate/transformer.ts';
import type {
  PlanningProject,
  PlanningPhase,
  PlanningPlan,
...
test('Scenario 1: Flat single-milestone', () => {
  ...
  const result = transformToTAC(project);
...
test('Scenario 12: Vision derivation', () => {
  ...
  const result1 = transformToTAC(project1);
  ...
  const result2 = transformToTAC(project2);
