...
test("initMetrics creates ledger, snapshotUnitMetrics persists across resets", () => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-test-"));
  ...
  try {
    ...
    // Verify file content
    const raw = readFileSync(join(tmpBase, ".tac", "metrics.json"), "utf-8");
...
test("snapshotUnitMetrics deduplicates entries with same type+id+startedAt", () => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-dedup-"));
  mkdirSync(join(tmpBase, ".tac"), { recursive: true });
  try {
    initMetrics(tmpBase);
...
test("snapshotUnitMetrics handles simulated idle-watchdog duplicate pattern", () => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-watchdog-"));
  mkdirSync(join(tmpBase, ".tac"), { recursive: true });
  try {
    initMetrics(tmpBase);
...
test("snapshotUnitMetrics counts toolCall blocks correctly (#1713)", () => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-toolcall-"));
  mkdirSync(join(tmpBase, ".tac"), { recursive: true });
...
test("#1943 initMetrics deduplicates entries loaded from a corrupted disk ledger", () => {
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-dedup-load-"));
  mkdirSync(join(tmpBase, ".tac"), { recursive: true });
  ...
  try {
    ...
    const diskRaw = readFileSync(join(tmpBase, ".tac", "metrics.json"), "utf-8");
...
test("#1943 getProjectTotals reports correct cost after dedup (no 35% inflation)", () => {
  ...
  const tmpBase = mkdtempSync(join(tmpdir(), "tac-metrics-cost-inflation-"));
  mkdirSync(join(tmpBase, ".tac"), { recursive: true });
  try {
    resetMetrics();
    writeFileSync(
