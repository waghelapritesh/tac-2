...
const tacDir = resolve(import.meta.dirname, "..");

function readTacFile(relativePath: string): string {
  return readFileSync(resolve(tacDir, relativePath), "utf-8");
}

test("command entrypoints use startAutoDetached instead of awaiting startAuto (#3733)", () => {
  const autoHandlerSrc = readTacFile("commands/handlers/auto.ts");
  const workflowHandlerSrc = readTacFile("commands/handlers/workflow.ts");
  const guidedFlowSrc = readTacFile("guided-flow.ts");
...
test("startAutoDetached reports failures asynchronously (#3733)", () => {
  const autoSrc = readTacFile("auto.ts");
...
test("detached auto-start preserves milestone lock across pause/stop cleanup (#3733)", () => {
  const autoSrc = readTacFile("auto.ts");
  const sessionSrc = readTacFile("auto/session.ts");
