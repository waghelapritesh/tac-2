...
describe("DevWorkflowEngine", () => {
  ...
  test("deriveState returns EngineState with expected fields", async (t) => {
    ...
    // Create a minimal temp .tac structure for deriveState
    const tempDir = mkdtempSync(join(tmpdir(), "tac-engine-test-"));
    mkdirSync(join(tempDir, ".tac", "milestones"), { recursive: true });
  ...
  test("getDisplayMetadata returns expected fields", async () => {
    ...
    assert.equal(meta.engineLabel, "TAC Dev");
  });
});
...
describe("Kill switch (TAC_ENGINE_BYPASS)", () => {
  const originalBypass = process.env.TAC_ENGINE_BYPASS;

  after(() => {
    // Restore original env var state
    if (originalBypass === undefined) {
      delete process.env.TAC_ENGINE_BYPASS;
    } else {
      process.env.TAC_ENGINE_BYPASS = originalBypass;
    }
  });

  test("TAC_ENGINE_BYPASS=1 does not affect resolveEngine (bypass checked in autoLoop)", async (t) => {
    const { resolveEngine } = await import("../engine-resolver.ts");
    process.env.TAC_ENGINE_BYPASS = "1";
    t.after(() => delete process.env.TAC_ENGINE_BYPASS);
