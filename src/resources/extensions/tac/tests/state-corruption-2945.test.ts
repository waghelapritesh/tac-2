...
import type { MilestoneRow, SliceRow } from "../tac-db.ts";
import type { AutoSession } from "../auto/session.ts";

// ─── Fixture helpers ────────────────────────────────────────────────────────

function tempDbPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-2945-"));
...
function createTempProject(): { basePath: string } {
  const basePath = mkdtempSync(join(tmpdir(), "tac-2945-project-"));
  mkdirSync(join(basePath, ".tac", "milestones", "M001"), { recursive: true });
  return { basePath };
}
...
describe("#2945 Bug 3: mergeAndExit must teardown worktree after successful merge", () => {
  ...
  test("_mergeWorktreeMode calls teardownAutoWorktree after successful merge", async () => {
    ...
    const mockDeps = {
      ...
      loadEffectiveTACPreferences: () => undefined,
      invalidateAllCaches: () => {},
...
describe("#2945 Bug 4: validate-milestone must persist quality_gates records", () => {
  ...
  test("handleValidateMilestone persists quality_gates records in DB", async () => {
    ...
    const adapter = (await import("../tac-db.ts"))._getAdapter()!;
