...
import { executeTacGraph } from '../tools/memory-tools.ts';
...
test('memory-tools: tac_graph returns LINK edges and filters by rel', () => {
  openDatabase(':memory:');
  ...
  const all = executeTacGraph({ mode: 'query', memoryId: 'MEM001', depth: 1 });
  ...
  const filtered = executeTacGraph({ mode: 'query', memoryId: 'MEM001', depth: 1, rel: 'related_to' });
