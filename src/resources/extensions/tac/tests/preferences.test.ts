...
import {
  ...
  applyModeDefaults,
  getIsolationMode,
  getGlobalTACPreferencesPath,
  getProjectTACPreferencesPath,
  loadEffectiveTACPreferences,
  loadGlobalTACPreferences,
  loadProjectTACPreferences,
...
import type { TACPreferences, TACModelConfigV2, TACPhaseModelConfig } from "../preferences.ts";
...
test("getIsolationMode defaults to none when preferences have no isolation setting", () => {
  ...
  // Default changed from "worktree" to "none" so TAC works out of the box
...
test("parses OpenRouter model config with org/model IDs and fallbacks", () => {
  ...
  const models = prefs!.models as TACModelConfigV2;
  const research = models.research as TACPhaseModelConfig;
  assert.equal(research.model, "moonshotai/kimi-k2.5");
  assert.deepEqual(research.fallbacks, ["qwen/qwen3.5-397b-a17b"]);
  const execution = models.execution as TACPhaseModelConfig;
...
test("parsePreferencesMarkdown parses heading+list format without frontmatter (#2036)", () => {
  // A TAC agent recovery session wrote preferences in markdown heading+list
...
test("experimental.rtk: non-boolean produces error", () => {
  const result = validatePreferences({ experimental: { rtk: "yes" } } as unknown as TACPreferences);
...
test("experimental: non-object produces error", () => {
  const result = validatePreferences({ experimental: true } as unknown as TACPreferences);
  assert.ok(result.errors.some(e => e.includes("experimental must be an object")));
});

test("experimental: unknown key produces warning", () => {
  const result = validatePreferences({ experimental: { rtk: true, future_flag: true } } as unknown as TACPreferences);
...
test("loadEffectiveTACPreferences preserves experimental prefs across global+project merge", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-prefs-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-prefs-home-"));

  try {
    mkdirSync(join(tempProject, ".tac"), { recursive: true });

    writeFileSync(
      join(tempTacHome, "preferences.md"),
      [
        "---",
        "version: 1",
        "experimental:",
    ...
    writeFileSync(
      join(tempProject, ".tac", "PREFERENCES.md"),
      [
        "---",
        "version: 1",
        "git:",
    ...
    process.env.TAC_HOME = tempTacHome;
    ...
    if (originalTacHome === undefined) delete process.env.TAC_HOME;
    else process.env.TAC_HOME = originalTacHome;
    rmSync(tempProject, { recursive: true, force: true });
    rmSync(tempTacHome, { recursive: true, force: true });
  }
});

test("loadEffectiveTACPreferences exposes slice_parallel prefs to runtime callers", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-slice-parallel-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-slice-parallel-home-"));
...
test("preferences paths use canonical uppercase filenames", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-prefs-canonical-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-prefs-canonical-home-"));
  ...
  try {
    ...
    assert.equal(basename(getGlobalTACPreferencesPath()), "PREFERENCES.md");
    assert.ok(
      getProjectTACPreferencesPath().endsWith("/.tac/PREFERENCES.md")
        || getProjectTACPreferencesPath().endsWith("\\.tac\\PREFERENCES.md"),
      "project preferences path should use .tac/PREFERENCES.md",
    );
  } finally {
    process.chdir(originalCwd);
    if (originalTacHome === undefined) delete process.env.TAC_HOME;
...
test("explicit base path preference loading survives a deleted cwd (#4498)", (t) => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-prefs-base-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-prefs-base-home-"));
  const deletedCwd = mkdtempSync(join(tmpdir(), "tac-prefs-deleted-cwd-"));
  ...
  const loaded = loadEffectiveTACPreferences(tempProject);
  assert.notEqual(loaded, null);
...
test("uppercase PREFERENCES.md wins over legacy lowercase preferences.md", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-prefs-priority-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-prefs-priority-home-"));

  try {
    mkdirSync(join(tempProject, ".tac"), { recursive: true });

    writeFileSync(join(tempTacHome, "preferences.md"), "---\nversion: 1\nmode: solo\n---\n", "utf-8");
    writeFileSync(join(tempTacHome, "PREFERENCES.md"), "---\nversion: 1\nmode: team\n---\n", "utf-8");
    writeFileSync(join(tempProject, ".tac", "preferences.md"), "---\nversion: 1\nlanguage: German\n---\n", "utf-8");
    writeFileSync(join(tempProject, ".tac", "PREFERENCES.md"), "---\nversion: 1\nlanguage: Japanese\n---\n", "utf-8");

    process.env.TAC_HOME = tempTacHome;
    process.chdir(tempProject);

    const globalPrefs = loadGlobalTACPreferences();
    const projectPrefs = loadProjectTACPreferences();
    ...
    assert.ok(
      projectPrefs!.path.endsWith("/.tac/PREFERENCES.md")
        || projectPrefs!.path.endsWith("\\.tac\\PREFERENCES.md"),
      "project loader should prefer .tac/PREFERENCES.md",
    );
  } finally {
    process.chdir(originalCwd);
    if (originalTacHome === undefined) delete process.env.TAC_HOME;
...
test("language: project setting overrides global via loadEffectiveTACPreferences", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-lang-project-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-lang-home-"));
...
test("language: global setting used when project has none", () => {
  const originalCwd = process.cwd();
  const originalTacHome = process.env.TAC_HOME;
  const tempProject = mkdtempSync(join(tmpdir(), "tac-lang-noproj-"));
  const tempTacHome = mkdtempSync(join(tmpdir(), "tac-lang-nhome-"));
