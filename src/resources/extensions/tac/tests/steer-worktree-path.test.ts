// TAC Extension - Steer Worktree Path Resolution Test
// Regression test for #3476: /tac steer must write overrides to the worktree .tac/,
// not the project root .tac/, when a worktree is active.
...
describe("steer worktree path resolution (#3476)", () => {
  ...
  beforeEach(() => {
    projectRoot = mkdtempSync(join(tmpdir(), "tac-steer-wt-"));
    mkdirSync(join(projectRoot, ".tac"), { recursive: true });

    // Simulate a worktree with its own .tac directory
    worktreePath = join(projectRoot, ".tac", "worktrees", "M001");
    mkdirSync(join(worktreePath, ".tac"), { recursive: true });
  });
  ...
  test("appendOverride writes to worktree .tac/ when worktree path is used", async () => {
    await appendOverride(worktreePath, "Use Postgres instead of SQLite", "M001/S01/T01");

    // Override should be in the worktree .tac/
    const wtOverrides = join(worktreePath, ".tac", "OVERRIDES.md");
    assert.ok(existsSync(wtOverrides), "override file exists in worktree .tac/");
    ...
    // Override should NOT be in the project root .tac/
    const rootOverrides = join(projectRoot, ".tac", "OVERRIDES.md");
    assert.ok(!existsSync(rootOverrides), "no override file in project root .tac/");
  });

  test("loadActiveOverrides reads from worktree .tac/ when worktree path is used", async () => {
  ...
  test("appendOverride falls back to project root when no worktree exists", async () => {
    ...
    const rootOverrides = join(projectRoot, ".tac", "OVERRIDES.md");
    assert.ok(existsSync(rootOverrides), "override file exists in project root .tac/");
