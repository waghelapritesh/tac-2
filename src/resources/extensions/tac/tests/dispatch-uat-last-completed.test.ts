// Regression test for #1693 — /tac dispatch uat targets the last completed
...
function createFixture(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-dispatch-uat-"));
...
test("dispatch uat warns when no completed slices exist", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-dispatch-uat-none-"));
  invalidateStateCache();
