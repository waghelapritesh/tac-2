 ...
 *
 * Testing strategy:
 *   @waghelapritesh/mcp-server is dynamically imported inside inlineGraphSubgraph().
...
/**
 * Creates a temp project directory with a .tac/graphs/graph.json file.
...
function makeProjectDir(fixture: GraphFixture): string {
  ...
  const tacDir = join(projectDir, ".tac");
  const graphsDir = join(tacDir, "graphs");
...
describe("inlineGraphSubgraph — null returns", () => {
  ...
  it("returns null (no throw) when graph.json is missing", async () => {
    // A project dir with no .tac directory at all — graphQuery returns zero nodes
