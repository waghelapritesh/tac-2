 ...
 *
 * /tac discuss was previously gated on state.activeMilestone, which prevented
...
function createBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-discuss-queued-"));
...
function writeContext(base: string, mid: string, content: string): void {
  ...
  writeFileSync(join(base, ".tac", "milestones", mid, `${mid}-CONTEXT.md`), content);
}

function writeContextDraft(base: string, mid: string, content: string): void {
  writeMilestoneDir(base, mid);
  writeFileSync(join(base, ".tac", "milestones", mid, `${mid}-CONTEXT-DRAFT.md`), content);
}

function writeRoadmap(base: string, mid: string, content: string): void {
  writeMilestoneDir(base, mid);
  writeFileSync(join(base, ".tac", "milestones", mid, `${mid}-ROADMAP.md`), content);
}
