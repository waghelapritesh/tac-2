...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-budget-test-${randomUUID()}`);
...
function makePrefs(ceiling?: number): TACPreferences {
  return {
    parallel: {
      enabled: true,
      max_workers: 2,
...
function writeWorkerCost(
  ...
  writeSessionStatus(base, {
    ...
    worktreePath: join(base, ".tac", "worktrees", milestoneId.toLowerCase()),
  });
}
