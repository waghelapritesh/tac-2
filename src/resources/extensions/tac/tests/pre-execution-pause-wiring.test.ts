...
import { openDatabase, closeDatabase, insertMilestone, insertSlice, insertTask, _getAdapter } from "../tac-db.ts";
...
/**
 * Set up a temp directory with TAC structure and DB.
...
function setupTestEnvironment(): void {
  ...
  const milestonesDir = join(tacDir, "milestones", "M001", "slices", "S01", "tasks");
  mkdirSync(milestonesDir, { recursive: true });
  
  // Change cwd so loadEffectiveTACPreferences finds our PREFERENCES.md
  process.chdir(tempDir);
  
  // Clear tacRoot cache so it finds the new .tac directory
  _clearTacRootCache();
  
  // Initialize DB
  dbPath = join(tacDir, "tac.db");
  openDatabase(dbPath);
}
...
# TAC Preferences
`;
  writeFileSync(join(tempDir, ".tac", "PREFERENCES.md"), prefsContent);
