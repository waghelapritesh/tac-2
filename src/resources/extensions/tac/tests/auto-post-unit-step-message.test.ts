// TAC-2 — Tests for step-mode completion messages in auto-post-unit
...
test("buildStepCompleteMessage: mid-flight step includes next unit label and /clear hint", () => {
  ...
  assert.match(msg, /\/tac to continue/);
});
...
test("buildStepCompleteMessage: unknown phase falls back to generic continue label", () => {
  ...
  const state = makeState({ phase: "totally-unknown" as unknown as TACState["phase"] });
  const msg = buildStepCompleteMessage(state);
  assert.match(msg, /Next: Continue/);
  assert.match(msg, /\/clear/);
});

test("STEP_COMPLETE_FALLBACK_MESSAGE: used when deriveState throws, still points users at /clear + /tac", () => {
  assert.match(STEP_COMPLETE_FALLBACK_MESSAGE, /\/clear/);
  assert.match(STEP_COMPLETE_FALLBACK_MESSAGE, /\/tac/);
});
