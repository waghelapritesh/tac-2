// tac-2 — Regression tests for the validate-milestone stuck-loop guard (#4094)
...
function setupTestEnvironment(): void {
  ...
  const milestoneDir = join(tempDir, ".tac", "milestones", "M001");
  mkdirSync(milestoneDir, { recursive: true });

  process.chdir(tempDir);
  _clearTacRootCache();

  dbPath = join(tempDir, ".tac", "tac.db");
  openDatabase(dbPath);
  invalidateAllCaches();
}
...
function writeValidationFile(verdict: string): void {
  const path = join(tempDir, ".tac", "milestones", "M001", "M001-VALIDATION.md");
  const content = `---
verdict: ${verdict}
remediation_round: 1
---
