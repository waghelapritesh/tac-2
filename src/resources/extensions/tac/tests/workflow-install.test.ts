// tac-2 — Regression tests for workflow-install path containment and ext fallback.
...
beforeEach(() => {
  ...
  process.env.TAC_HOME = makeTmpBase();
});

afterEach(() => {
  if (savedTacHome === undefined) delete process.env.TAC_HOME;
  else process.env.TAC_HOME = savedTacHome;
