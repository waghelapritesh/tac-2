...
function setupTacDir(tmp: string): void {
...
test("revertExecutorResolvedCaptures reverts captures resolved without classification", () => {
  ...
  const capPath = join(tmp, ".tac", "CAPTURES.md");
...
test("executeBacktrack writes trigger and regression markers", () => {
  ...
  mkdirSync(join(tmp, ".tac", "milestones", "M003"), { recursive: true });
  ...
  const triggerPath = join(tmp, ".tac", "BACKTRACK-TRIGGER.md");
  ...
  const regressionPath = join(tmp, ".tac", "milestones", "M003", "M003-REGRESSION.md");
