// TAC2 — Tests for tac_milestone_status read-only query tool
...
function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-query-tool-test-${randomUUID()}`);
...
test("registerQueryTools registers tac_milestone_status tool", () => {
  const pi = makeMockPi();
  registerQueryTools(pi);
  const names = pi.tools.map((t: { name: string }) => t.name);
  assert.ok(names.includes("tac_milestone_status"), "Should register tac_milestone_status");
  assert.ok(names.includes("tac_checkpoint_db"), "Should register tac_checkpoint_db");
});

test("tac_milestone_status has promptGuidelines mentioning prohibited alternatives", () => {
  const pi = makeMockPi();
  registerQueryTools(pi);
...
test("tac_milestone_status returns milestone metadata and slice statuses", async () => {
...
test("tac_milestone_status returns empty slices array for milestone with no slices", async () => {
  const base = makeTmpBase();
  try {
    openTestDb(base);
...
test("tac_milestone_status returns not-found for missing milestone", async () => {
  const base = makeTmpBase();
  try {
    openTestDb(base);
...
test("tac_milestone_status handles missing DB gracefully", async () => {
  // Create a directory without .tac/ to ensure ensureDbOpen has nothing to open
  const base = join(tmpdir(), `tac-no-db-${randomUUID()}`);
  ...
  try {
    ...
    const result = await executeToolInDir(tool, { milestoneId: "M001" }, base);
    assert.match(result.content[0].text, /TAC database is not available/);
