...
import { loadSkills } from "@tac/pi-coding-agent";
import {
...
function makeTempBase(): string {
  return mkdtempSync(join(tmpdir(), "tac-skill-activation-"));
}
...
function writeProjectPreferences(base: string, preferences: string): void {
  ...
  writeFileSync(join(base, ".tac", "PREFERENCES.md"), `---\n${preferences}---\n`);
}
...
test("skill manifest strict warnings require TAC_SKILL_MANIFEST_STRICT=1", (t) => {
  const previousStrict = process.env.TAC_SKILL_MANIFEST_STRICT;
  const previousStderr = setStderrLoggingEnabled(false);
  t.after(() => {
    if (previousStrict === undefined) {
      delete process.env.TAC_SKILL_MANIFEST_STRICT;
    } else {
      process.env.TAC_SKILL_MANIFEST_STRICT = previousStrict;
    }
    setStderrLoggingEnabled(previousStderr);
    _resetLogs();
  });

  process.env.TAC_SKILL_MANIFEST_STRICT = "0";
  _resetLogs();
  ...
  process.env.TAC_SKILL_MANIFEST_STRICT = "1";
  _resetLogs();
  warnIfManifestHasMissingSkills("research-milestone", new Set());
