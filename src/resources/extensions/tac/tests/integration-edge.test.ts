...
import { openDatabase, closeDatabase, isDbAvailable, _resetProvider } from '../tac-db.ts';
...
test('integration-edge: empty project', () => {
  const base = mkdtempSync(join(tmpdir(), 'tac-int-edge-empty-'));
  const tacDir = join(base, '.tac');
  mkdirSync(tacDir, { recursive: true });

  const dbPath = join(tacDir, 'test-edge-empty.db');
...
test('integration-edge: partial migration', () => {
  const base = mkdtempSync(join(tmpdir(), 'tac-int-edge-partial-'));
  const tacDir = join(base, '.tac');
  mkdirSync(tacDir, { recursive: true });
  ...
  const dbPath = join(tacDir, 'test-edge-partial.db');
...
test('integration-edge: fallback mode', () => {
  const base = mkdtempSync(join(tmpdir(), 'tac-int-edge-fallback-'));
  const tacDir = join(base, '.tac');
  mkdirSync(tacDir, { recursive: true });
  ...
  const dbPath = join(tacDir, 'test-edge-fallback.db');
