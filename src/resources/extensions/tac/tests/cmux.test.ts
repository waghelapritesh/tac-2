...
describe("autoEnableCmuxPreferences", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(tmp, ".tac"), { recursive: true });
    process.chdir(tmp);
  });
  ...
  test("writes cmux.enabled true when preferences file exists with no cmux config", () => {
    const prefsPath = path.join(tmp, ".tac", "preferences.md");
...
describe("createGridLayout", () => {
  ...
  function makeMockClient() {
    ...
    const client = {
      ...
      async createGridLayout(count: number) {
        ...
        const rightCol = await createSplitFrom("tac-surface", "right");
        ...
        const bottomLeft = await createSplitFrom("tac-surface", "down");
  ...
  test("1 agent creates single right split", async () => {
    ...
    assert.deepEqual(mock.calls, [
      { source: "tac-surface", direction: "right" },
    ]);
  });
  ...
  test("3 agents creates 2x2 grid (tac + 3 agent surfaces)", async () => {
    const mock = makeMockClient();
    ...
    assert.deepEqual(mock.calls, [
      { source: "tac-surface", direction: "right" },
      { source: "surface-1", direction: "down" },
      { source: "tac-surface", direction: "down" },
    ]);
  });
