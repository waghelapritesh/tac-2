...
function createTmpDir(suffix: string): string {
  return mkdtempSync(join(tmpdir(), `tac-wal-${suffix}-`));
}
...
describe('shared-wal', async () => {
  ...
  console.log('\n=== shared-wal: resolve worktree path to project root DB ===');
  {
    ...
    const worktreePath = join(projectRoot, '.tac', 'worktrees', 'M001');
    const result = resolveProjectRootDbPath(worktreePath);
    assert.deepStrictEqual(result, join(projectRoot, '.tac', 'tac.db'),
  ...
  {
    const projectRoot = '/home/user/myproject';
    const nestedPath = join(projectRoot, '.tac', 'worktrees', 'M002', 'src', 'lib');
  ...
  {
    const result = resolveProjectRootDbPath('/proj/.tac/worktrees/M001');
    assert.deepStrictEqual(result, join('/proj', '.tac', 'tac.db'),
  ...
  console.log('\n=== shared-wal: resolve external-state worktree path (#2952) ===');
  {
    ...
    // Should resolve to:     ~/.tac/projects/<hash>/tac.db
    const stateRoot = '/home/user/.tac/projects/a1b2c3d4';
    const worktreePath = join(stateRoot, 'worktrees', 'M002');
    const result = resolveProjectRootDbPath(worktreePath);
    assert.deepStrictEqual(result, join(stateRoot, 'tac.db'),
  ...
  {
    const stateRoot = '/home/user/.tac/projects/deadbeef42';
  ...
  {
    const result = resolveProjectRootDbPath('/Users/dev/.tac/projects/cafe0123/worktrees/M001');
    assert.deepStrictEqual(result, join('/Users/dev/.tac/projects/cafe0123', 'tac.db'),
