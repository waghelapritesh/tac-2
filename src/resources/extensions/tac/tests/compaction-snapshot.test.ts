...
function freshBase(): string {
  return mkdtempSync(join(tmpdir(), 'tac-snap-'));
}
...
test('buildSnapshot: renders memories, exec history, and active context', () => {
  const snap = buildSnapshot({
    ...
    activeContext: 'M001 / S01 / T01 — wire tac_exec',
    memories: [
...
test('writeCompactionSnapshot + readCompactionSnapshot + executeResume: end-to-end', () => {
  ...
  try {
    openDatabase(':memory:');
    createMemory({ category: 'architecture', content: 'Single-writer DB through tac-db.ts', confidence: 0.95 });
    ...
    assert.ok(contents);
    assert.match(contents!, /Single-writer DB through tac-db\.ts/);
    ...
    assert.match(tool.content[0].text, /Single-writer DB through tac-db\.ts/);
