...
import { closeDatabase } from "../tac-db.ts";
...
test("checkEngineHealth reports db_unavailable when tac.db exists but the DB is closed", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "tac-doctor-db-unavailable-"));
  t.after(() => rmSync(base, { recursive: true, force: true }));

  const tacDir = join(base, ".tac");
  mkdirSync(tacDir, { recursive: true });
  writeFileSync(join(tacDir, "tac.db"), "");
  ...
  assert.equal(dbIssue.file, ".tac/tac.db");
});
