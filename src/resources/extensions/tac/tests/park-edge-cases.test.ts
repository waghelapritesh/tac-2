...
function createFixture(): string {
  const b = mkdtempSync(join(tmpdir(), 'tac-edge-'));
  mkdirSync(join(b, '.tac', 'milestones'), { recursive: true });
  return b;
}

function createM(b: string, mid: string, opts?: { roadmap?: boolean; summary?: boolean; dependsOn?: string[] }): void {
  const d = join(b, '.tac', 'milestones', mid);
...
test('EDGE 6: Queue order after discard', async () => {
    ...
    try {
      ...
      const order = JSON.parse(readFileSync(join(b, '.tac', 'QUEUE-ORDER.json'), 'utf-8'));
