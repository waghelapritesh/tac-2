...
// Writes a complete .tac tree to a temp dir, verifies file existence,
...
import { writeTACDirectory } from '../migrate/writer.ts';
...
function makeTask(id: string, title: string, done: boolean, hasSummary: boolean): TACTask {
  return {
    id,
    title,
...
function buildIncompleteProject(): TACProject {
  ...
  const requirements: TACRequirement[] = [
...
function buildCompleteProject(): TACProject {
...
test('Scenario 1: Incomplete project — write, parse, deriveState', async () => {
    const base = mkdtempSync(join(tmpdir(), 'tac-writer-int-'));
    try {
      const project = buildIncompleteProject();
      const result = await writeTACDirectory(project, base);

      // (a) Key files exist
      console.log('  --- file existence ---');
      const tac = join(base, '.tac');
      const m = join(tac, 'milestones', 'M001');
      ...
      assert.ok(existsSync(join(tac, 'REQUIREMENTS.md')), 'incomplete: REQUIREMENTS.md exists');
      assert.ok(existsSync(join(tac, 'PROJECT.md')), 'incomplete: PROJECT.md exists');
      assert.ok(existsSync(join(tac, 'DECISIONS.md')), 'incomplete: DECISIONS.md exists');
      assert.ok(existsSync(join(tac, 'STATE.md')), 'incomplete: STATE.md exists');
...
test('Scenario 2: Fully complete project — deriveState phase', async () => {
    const base = mkdtempSync(join(tmpdir(), 'tac-writer-int-complete-'));
    try {
      const project = buildCompleteProject();
      await writeTACDirectory(project, base);

      // Null research should NOT produce a file
      const m = join(base, '.tac', 'milestones', 'M001');
      ...
      assert.ok(!existsSync(join(base, '.tac', 'REQUIREMENTS.md')), 'complete: REQUIREMENTS.md NOT written (empty)');
