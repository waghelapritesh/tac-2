 ...
 * non-project directories (node_modules, .tac, target, etc.) are skipped.
 */
...
function makeRoot(t: { after: (fn: () => void) => void }): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-nested-git-"));
...
test("#2616: findNestedGitDirs skips excluded directories (node_modules, .tac, target)", (t) => {
  const root = makeRoot(t);

  // All three of these contain a .git *directory*, but the scan must skip them.
  for (const excluded of ["node_modules", ".tac", "target"]) {
