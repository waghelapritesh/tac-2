...
// when .tac/ exists with Markdown content but no tac.db file.
//
// This covers the bug where interactive (non-auto) sessions got
// "TAC database is not available" because ensureDbOpen only opened
...
import { closeDatabase, isDbAvailable, getDecisionById, _getAdapter } from '../tac-db.ts';

const _require = createRequire(import.meta.url);

function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-ensure-db-'));
  return dir;
}
...
// ═══════════════════════════════════════════════════════════════════════════
// ensureDbOpen creates DB + migrates when .tac/ has Markdown
// ═══════════════════════════════════════════════════════════════════════════

describe('ensure-db-open', () => {
  test('ensureDbOpen: creates DB from Markdown', async () => {
    const tmpDir = makeTmpDir();
    const tacDir = path.join(tmpDir, '.tac');
  ...
  `;
    fs.writeFileSync(path.join(tacDir, 'DECISIONS.md'), decisionsContent);
    ...
    try {
      ...
      assert.ok(result === true, 'ensureDbOpen should return true when .tac/ has Markdown');
  ...
  test('ensureDbOpen: explicit basePath opens target project without cwd override', async () => {
    ...
    const tacDir = path.join(tmpDir, '.tac');
    fs.mkdirSync(tacDir, { recursive: true });
    fs.writeFileSync(path.join(tacDir, 'DECISIONS.md'), `# Decisions
...
`);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ensureDbOpen returns false when no .tac/ exists
  // ═══════════════════════════════════════════════════════════════════════════

  test('ensureDbOpen: no .tac/ returns false', async () => {
    const tmpDir = makeTmpDir();
    // No .tac/ directory at all
    ...
    try {
      const { ensureDbOpen } = await import('../bootstrap/dynamic-tools.ts');
      const result = await ensureDbOpen();
      assert.ok(result === false, 'ensureDbOpen should return false when no .tac/ exists');
  ...
  test('ensureDbOpen: opens existing DB', async () => {
    ...
    const dbPath = path.join(tacDir, 'tac.db');
    const { openDatabase } = await import('../tac-db.ts');
    openDatabase(dbPath);
    closeDatabase();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ensureDbOpen returns false for empty .tac/ (no Markdown, no DB)
  // ═══════════════════════════════════════════════════════════════════════════

  test('ensureDbOpen: empty .tac/ creates empty DB (#2510)', async () => {
    const tmpDir = makeTmpDir();
    const tacDir = path.join(tmpDir, '.tac');
    fs.mkdirSync(tacDir, { recursive: true });
    // .tac/ exists but no DECISIONS.md, REQUIREMENTS.md, or milestones/
    ...
    try {
      const { ensureDbOpen } = await import('../bootstrap/dynamic-tools.ts');
      const result = await ensureDbOpen();
      assert.ok(result === true, 'ensureDbOpen should create empty DB for fresh .tac/');
      assert.ok(fs.existsSync(path.join(tacDir, 'tac.db')), 'DB file should be created');
      assert.ok(isDbAvailable(), 'DB should be available');
    } finally {
      process.cwd = origCwd;
      closeDatabase();
  ...
  test('ensureDbOpen: switches open database when basePath changes', async () => {
    ...
    fs.mkdirSync(path.join(firstDir, '.tac'), { recursive: true });
    fs.mkdirSync(path.join(secondDir, '.tac'), { recursive: true });
    fs.writeFileSync(path.join(firstDir, '.tac', 'DECISIONS.md'), `# Decisions

| # | When | Scope | Decision | Choice | Rationale | Revisable |
|---|------|-------|----------|--------|-----------|-----------|
| D101 | M001 | architecture | First DB | First | First rationale | Yes |
`);
    fs.writeFileSync(path.join(secondDir, '.tac', 'DECISIONS.md'), `# Decisions
