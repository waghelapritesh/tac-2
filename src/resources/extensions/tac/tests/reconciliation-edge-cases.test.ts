// TAC State Machine Regression Tests — Event Replay & Reconciliation (#3161)
...
function tempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tac-recon-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("reconciliation-edge-cases", () => {
  ...
  test("appendEvent creates event log if directory does not exist", () => {
    const base = tempDir();
    // Remove the .tac directory if it somehow exists — appendEvent should create it.
    const tacDir = path.join(base, ".tac");
    if (fs.existsSync(tacDir)) fs.rmSync(tacDir, { recursive: true, force: true });
