// TAC Extension — Workflow Logger Audit Persistence Tests
...
describe("workflow-logger audit persistence", () => {
  ...
  test("persisted errors have context filtered to safe allowlist", () => {
    logError("tool", "tool failed", {
      fn: "saveDecisionToDb",
      tool: "tac_decision_save",
      error: "SQLITE_BUSY: database is locked",
      file: "/home/user/project/tac.db",
    });
    const lines = readAuditLines(tmp);
    assert.equal(lines.length, 1);
    ...
    assert.equal(ctx.tool, "tac_decision_save");
