...
test("completed-units.json is cleared on milestone transition (functional)", () => {
  const tempDir = realpathSync(mkdtempSync(join(tmpdir(), "tac-cu-reset-")));
  try {
    // Create .tac directory with a populated completed-units.json
    const tacDir = join(tempDir, ".tac");
    mkdirSync(tacDir, { recursive: true });

    const completedKeysPath = join(tacDir, "completed-units.json");
    const staleEntries = [
      "context-gather/M001",
      "roadmap-plan/M001",
      "plan-slice/S01",
