 ...
 * circuit breaker.
 *
 * The fix persists the counter to `.tac/runtime/rewrite-count.json`.
 */
...
describe("rewrite-docs circuit breaker persistence (#2203)", () => {
  ...
  beforeEach(() => {
    tempBase = mkdtempSync(join(tmpdir(), "tac-rewrite-test-"));
    // Create .tac/ directory so tacRoot resolves to it
    mkdirSync(join(tempBase, ".tac", "runtime"), { recursive: true });
  });
  ...
  test("getRewriteCount handles corrupt JSON gracefully", () => {
    const filePath = join(tempBase, ".tac", "runtime", "rewrite-count.json");
  ...
  test("rewrite-count.json is written to .tac/runtime/", () => {
    setRewriteCount(tempBase, 1);
    const filePath = join(tempBase, ".tac", "runtime", "rewrite-count.json");
