...
function makeBasePath(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `tac-req-slice-${prefix}-`));
...
describe("require_slice_discussion dispatch rule (#3454)", () => {
  ...
  test("returns stop action when preference enabled and CONTEXT missing", async () => {
    ...
    try {
      const prefs = { phases: { require_slice_discussion: true } } as unknown as TACPreferences;
      ...
      if (action!.action === "stop") {
        ...
        assert.match(action!.reason, /\/tac discuss/);
  ...
  test("falls through (null) when preference is disabled", async () => {
    ...
    try {
      const prefs = { phases: { require_slice_discussion: false } } as unknown as TACPreferences;
      const action = await findRule().match(buildCtx(basePath, prefs));
  ...
  test("falls through (null) when CONTEXT file already exists on disk", async () => {
    ...
    try {
      // Seed the CONTEXT file that /tac discuss would have written.
      const sliceDir = join(basePath, ".tac", "milestones", "M001", "slices", "S01");
