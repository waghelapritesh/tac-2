 ...
 *
 * Covers: .tac/ state files, build artifacts (.tsbuildinfo, .pyc, __pycache__,
...
describe("isSafeToAutoResolve", () => {
  // ─── .tac/ state files ───────────────────────────────────────────────────
  test("returns true for .tac/ prefixed paths", () => {
    assert.ok(isSafeToAutoResolve(".tac/STATE.md"));
    assert.ok(isSafeToAutoResolve(".tac/milestones/M001/CONTEXT.md"));
    assert.ok(isSafeToAutoResolve(".tac/tac.db"));
  });
