...
describe("flat-rate provider routing guard (#3453)", () => {
  ...
  test("resolvePreferredModelConfig returns undefined for copilot start model", () => {
    ...
    const tempProject = mkdtempSync(join(tmpdir(), "tac-flat-rate-project-"));
    const tempTacHome = mkdtempSync(join(tmpdir(), "tac-flat-rate-home-"));
...
describe("flat-rate routing opt-in (#4386)", () => {
  function withPrefs(prefsYaml: string, fn: () => void): void {
    const originalCwd = process.cwd();
    const originalTacHome = process.env.TAC_HOME;
    const tempProject = mkdtempSync(join(tmpdir(), "tac-4386-project-"));
    const tempTacHome = mkdtempSync(join(tmpdir(), "tac-4386-home-"));
    try {
      mkdirSync(join(tempProject, ".tac"), { recursive: true });
      writeFileSync(
        join(tempProject, ".tac", "PREFERENCES.md"),
