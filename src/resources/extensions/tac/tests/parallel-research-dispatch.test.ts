...
// Point TAC_HOME at a throwaway directory *before* the prompt-loader
...
// ~/.tac/ copy (which may be a stale cached version from a prior
...
process.env.TAC_HOME = process.env.TAC_HOME_TEST_OVERRIDE
  ?? join(tmpdir(), `tac-test-home-${process.pid}-${Date.now()}`);

after(() => {
  if (previousTacHome === undefined) {
    delete process.env.TAC_HOME;
  } else {
    process.env.TAC_HOME = previousTacHome;
  }
});
...
describe("parallel-research-slices dispatch rule", () => {
  ...
  test("excludes slices that already have a RESEARCH file (falls back to <2)", async () => {
    ...
    const s01Dir = join(base, ".tac", "milestones", "M001", "slices", "S01");
