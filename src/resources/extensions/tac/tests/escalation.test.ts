// TAC Extension — ADR-011 Phase 2 Mid-Execution Escalation tests
...
function makeBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-adr011-p2-"));
...
function writePrefs(base: string, enabled: boolean): void {
  const path = join(base, ".tac", "PREFERENCES.md");
  writeFileSync(path, [
    "---",
    "version: 1",
    "phases:",
...
test("ADR-011 P3 #22: ADR-009 audit envelopes emitted across the escalation lifecycle", (t) => {
  ...
  // .tac/audit/events.jsonl. ADR-009 control-plane consumers depend on this
  ...
  const logPath = join(base, ".tac", "audit", "events.jsonl");
  assert.ok(existsSync(logPath), "audit log must exist at .tac/audit/events.jsonl");
...
test("ADR-011 P3 #25: artifact write failure surfaces, leaves DB flags clean, and retries successfully once recovered", async (t) => {
  ...
  mkdirSync(join(base, ".tac", "milestones", "M001", "slices", "S09", "tasks"), { recursive: true });
