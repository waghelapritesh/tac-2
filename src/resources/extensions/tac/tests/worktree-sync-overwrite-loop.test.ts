...
function createBase(name: string): string {
  const base = mkdtempSync(join(tmpdir(), `tac-wt-1886-${name}-`));
...
test("#1886: worktree VALIDATION.md is not overwritten by project root sync", (t) => {
  ...
  const prM004 = join(mainBase, ".tac", "milestones", "M004");
  ...
  const wtM004 = join(wtBase, ".tac", "milestones", "M004");
...
test("#1886: missing worktree files are still copied from project root", (t) => {
  ...
  assert.ok(
    existsSync(join(wtBase, ".tac", "milestones", "M004", "M004-CONTEXT.md")),
    "missing CONTEXT.md copied from project root",
  );
  assert.ok(
    existsSync(join(wtBase, ".tac", "milestones", "M004", "M004-ROADMAP.md")),
...
test("#1886: completed-units.json is forward-synced to worktree", (t) => {
  ...
  writeFileSync(
    join(mainBase, ".tac", "completed-units.json"),
  ...
  const wtCompleted = JSON.parse(
    readFileSync(join(wtBase, ".tac", "completed-units.json"), "utf-8"),
  );
  assert.deepEqual(
    wtCompleted,
