...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-draft-test-'));
...
`);
      ...
      writeFileSync(join(base, '.tac', 'milestones', 'M003', 'M003-CONTEXT.md'), '# M003\n\nPending milestone.');
...
`);
  ...
  console.log('\n=== CONTEXT-DRAFT on non-active milestone → pending ===');
  {
    ...
    try {
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M001'), { recursive: true });
      writeFileSync(join(base, '.tac', 'milestones', 'M001', 'M001-CONTEXT.md'), '# M001\n\nFirst milestone.');
