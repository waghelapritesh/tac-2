...
test("handleExport --html --all generates reports for milestones missing from the index", async () => {
  ...
  const tmp = join(tmpdir(), `tac-export-all-test-${Date.now()}`);
  const tacDir = join(tmp, ".tac");
  const reportsDir = join(tacDir, "reports");
...
test("export completions include --html and --html --all", async () => {
  const { registerTACCommand } = await import("../commands.js");
  ...
  const tac = commands.get("tac");
  assert.ok(tac, "should register /tac command");

  const completions = tac.getArgumentCompletions("export --");
