 ...
 *   Bug 1 — Invalid quality_gates migration bricks tac.db
...
function tmpDb(): { dir: string; dbPath: string } {
  const dir = mkdtempSync(join(tmpdir(), "tac-4540-"));
  return { dir, dbPath: join(dir, "tac.db") };
}
...
describe("Bug 2 — artifact-verification-retry journal event (#4540)", () => {
  test("emitJournalEvent accepts artifact-verification-retry event type", () => {
    const basePath = mkdtempSync(join(tmpdir(), "tac-journal-4540-"));
    try {
  ...
  test("artifact-verification-retry event carries attempt count", () => {
    const basePath = mkdtempSync(join(tmpdir(), "tac-journal-4540b-"));
    try {
      mkdirSync(join(basePath, ".tac"), { recursive: true });
      emitJournalEvent(basePath, {
        ts: new Date().toISOString(),
