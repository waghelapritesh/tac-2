// TAC-2 — Regression tests for #3512: tac-auto-wrapup mid-turn interruption
...
describe("#3512: tac-auto-wrapup must not interrupt in-flight tool calls", () => {
...
describe("#4276: pending/skipped tools stay visible to auto-mode hooks", () => {
  test("tool_call handler marks TAC tools in-flight before execution_start", () => {
  ...
  test("tool_result handler clears pending tools and records queued-skip errors", () => {
    ...
    assert.ok(
      ...
      "tool_result handler must surface queued-skip errors for TAC tools",
    );
  });
});
