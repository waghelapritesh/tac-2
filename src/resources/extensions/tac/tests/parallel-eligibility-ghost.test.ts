...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-parallel-elig-"));
...
function writeMilestoneFile(
  ...
  const filePath = join(base, ".tac", "milestones", milestoneId, filename);
  mkdirSync(join(filePath, ".."), { recursive: true });
  writeFileSync(filePath, content);
}

function makeMilestoneDir(base: string, milestoneId: string): void {
  mkdirSync(join(base, ".tac", "milestones", milestoneId), { recursive: true });
}
...
describe("parallel-eligibility: ghost milestone ineligibility (#2501)", () => {
  ...
  test("ghost milestone (directory only, no planning files) is ineligible", async () => {
    ...
    mkdirSync(join(base, ".tac", "milestones", "M017", "slices"), { recursive: true });
