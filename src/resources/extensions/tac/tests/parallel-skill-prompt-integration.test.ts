...
function setupProjectWithSkill(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-worker-skill-int-"));
  tmpDirs.push(base);
  ...
  // loadEffectiveTACPreferences(), which reads from `${cwd}/.tac/PREFERENCES.md`.
  writeFileSync(
    join(base, ".tac", "PREFERENCES.md"),
