...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-state-test-'));
...
function writePlan(base: string, mid: string, sid: string, content: string): void {
  const dir = join(base, '.tac', 'milestones', mid, 'slices', sid);
...
function writeRequirements(base: string, content: string): void {
  writeFileSync(join(base, '.tac', 'REQUIREMENTS.md'), content);
}
...
describe('derive-state', async () => {
  ...
  test('milestone dir exists but no roadmap → pre-planning', async () => {
    ...
    try {
      ...
      writeFileSync(join(base, '.tac', 'milestones', 'M001', 'M001-CONTEXT.md'), '# First Milestone\n\nContext for M001.');
...
`);
      ...
      mkdirSync(join(base, '.tac', 'milestones', 'M003'), { recursive: true });
      writeFileSync(join(base, '.tac', 'milestones', 'M003', 'M003-CONTEXT.md'), '# Third Milestone\n\nContext for M003.');
...
`);
  ...
  // ═══ Milestone with summary but no roadmap → complete ═══════════════════
  {
    ...
    try {
      ...
      const m1dir = join(base, '.tac', 'milestones', 'M001');
      mkdirSync(m1dir, { recursive: true });
      writeFileSync(join(m1dir, 'M001-SUMMARY.md'), '---\nid: M001\n---\n# Bootstrap\nDone.');

      const m2dir = join(base, '.tac', 'milestones', 'M002');
...
`);
  ...
  test('unchecked roadmap + summary satisfies dependency', async () => {
    ...
    try {
      ...
      const contextDir = join(base, '.tac', 'milestones', 'M002');
  ...
  test('ghost milestone (only META.json) is skipped', async () => {
    ...
    try {
      ...
      const ghostDir = join(base, '.tac', 'milestones', 'M001');
  ...
  test('ghost milestone skipped alongside real milestones', async () => {
    ...
    try {
      ...
      const realDir = join(base, '.tac', 'milestones', 'M002');
