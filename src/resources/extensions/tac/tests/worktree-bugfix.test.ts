...
describe("worktree-bugfix", () => {
  ...
  it("resolveGitDir returns .git directory in normal repo", () => {
    const repo = mkdtempSync(join(tmpdir(), "tac-wt-fix-"));
    dirs.push(repo);
    initRepo(repo);
  ...
  it("resolveGitDir follows gitdir: pointer in worktree", () => {
    ...
    const wtDir = mkdtempSync(join(tmpdir(), "tac-wt-fix-wt-"));
    dirs.push(wtDir);
  ...
  it("resolveGitDir returns default when .git doesn't exist", () => {
    const noGit = mkdtempSync(join(tmpdir(), "tac-wt-fix-"));
    dirs.push(noGit);
  ...
  it("detectWorktreeName returns name for worktree path", () => {
    assert.deepStrictEqual(
      detectWorktreeName("/project/.tac/worktrees/M005"),
      "M005",
  ...
  it("captureIntegrationBranch is a no-op when in a worktree", () => {
    ...
    const wtPath = join(repo, ".tac", "worktrees", "M005");
    mkdirSync(wtPath, { recursive: true });
    mkdirSync(join(wtPath, ".tac", "milestones", "M005"), { recursive: true });
    // Initialize git in the worktree so getService doesn't fail
    initRepo(wtPath);

    // captureIntegrationBranch should be a no-op — no META.json written
    const metaPath = join(wtPath, ".tac", "milestones", "M005", "M005-META.json");
  ...
  it("detectWorktreeName prevents pull in worktree context", () => {
    ...
    const inWorktree = detectWorktreeName("/project/.tac/worktrees/M006");
