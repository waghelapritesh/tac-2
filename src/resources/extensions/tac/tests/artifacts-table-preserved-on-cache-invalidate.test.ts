...
describe("cache.ts must not re-import clearArtifacts into invalidateAllCaches", () => {
  const src = readFileSync(
    resolve(process.cwd(), "src", "resources", "extensions", "tac", "cache.ts"),
    "utf-8",
  );

  test("clearArtifacts is not imported from tac-db", () => {
    assert.ok(
      !/import\s*\{[^}]*clearArtifacts[^}]*\}\s*from\s*['"]\.\/tac-db/.test(src),
