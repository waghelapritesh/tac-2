...
// /tac prefs wizard.  Without this guard, a new preference can be added to the
...
const EXPOSED_OUTSIDE_WIZARD = new Set<string>([
  ...
  "context_mode",     // advanced sandbox config (tac_exec + compaction) — enabled by default; edit PREFERENCES.md directly to tune timeouts/caps. Wizard coverage tracked separately.
]);
...
test("every KNOWN_PREFERENCE_KEYS entry is reachable from the wizard source", () => {
  ...
  assert.deepEqual(
    missing,
    [],
    `These preference keys are in KNOWN_PREFERENCE_KEYS but are not referenced anywhere in the /tac prefs wizard — they cannot be configured through the UI. Either add wizard coverage or add them to EXPOSED_OUTSIDE_WIZARD with an explanatory comment:\n${missing.join("\n")}`,
  );
});
