...
import { closeDatabase, insertMilestone, insertSlice, openDatabase, getMilestone } from "../tac-db.ts";

// ─── Helpers ──────────────────────────────────────────────────────────────

function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-val-test-${randomUUID()}`);
...
test("buildLoopRemediationSteps returns steps for validate-milestone", () => {
  ...
  try {
    ...
    assert.ok(result!.includes("tac recover"));
  } finally {
    cleanup(base);
  }
});
