 ...
 *
 * Tests:
 * - KNOWLEDGE is registered in TAC_ROOT_FILES
 * - resolveTacRootFile resolves KNOWLEDGE paths correctly
 * - inlineTacRootFile works with the KNOWLEDGE key
...
import { TAC_ROOT_FILES, resolveTacRootFile } from '../paths.ts';
import { inlineTacRootFile, inlineKnowledgeBudgeted } from '../auto-prompts.ts';
import { appendKnowledge } from '../files.ts';
import { loadKnowledgeBlock } from '../bootstrap/system-context.ts';

// ─── KNOWLEDGE is registered in TAC_ROOT_FILES ─────────────────────────────

test('knowledge: KNOWLEDGE key exists in TAC_ROOT_FILES', () => {
  assert.ok('KNOWLEDGE' in TAC_ROOT_FILES, 'TAC_ROOT_FILES should have KNOWLEDGE key');
  assert.strictEqual(TAC_ROOT_FILES.KNOWLEDGE, 'KNOWLEDGE.md');
});

// ─── resolveTacRootFile resolves KNOWLEDGE.md ───────────────────────────────

test('knowledge: resolveTacRootFile returns canonical path when KNOWLEDGE.md exists', () => {
  const tmp = realpathSync(mkdtempSync(join(tmpdir(), 'tac-knowledge-')));
  const tacDir = join(tmp, '.tac');
  mkdirSync(tacDir, { recursive: true });
  writeFileSync(join(tacDir, 'KNOWLEDGE.md'), '# Project Knowledge\n');

  const resolved = resolveTacRootFile(tmp, 'KNOWLEDGE');
  assert.strictEqual(resolved, join(tacDir, 'KNOWLEDGE.md'));

  rmSync(tmp, { recursive: true, force: true });
});

test('knowledge: resolveTacRootFile resolves when legacy knowledge.md exists', () => {
  const tmp = realpathSync(mkdtempSync(join(tmpdir(), 'tac-knowledge-')));
  const tacDir = join(tmp, '.tac');
  mkdirSync(tacDir, { recursive: true });
  writeFileSync(join(tacDir, 'knowledge.md'), '# Project Knowledge\n');
  ...
  const canonical = join(tacDir, 'KNOWLEDGE.md');
  const legacy = join(tacDir, 'knowledge.md');
  assert.ok(
...
test('knowledge: resolveTacRootFile returns canonical path when file does not exist', () => {
  const tmp = realpathSync(mkdtempSync(join(tmpdir(), 'tac-knowledge-')));
  const tacDir = join(tmp, '.tac');
  mkdirSync(tacDir, { recursive: true });
...
// ─── inlineTacRootFile works with knowledge.md ─────────────────────────────

test('knowledge: inlineTacRootFile returns content when KNOWLEDGE.md exists', async () => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-knowledge-'));
  const tacDir = join(tmp, '.tac');
  mkdirSync(tacDir, { recursive: true });
  writeFileSync(join(tacDir, 'KNOWLEDGE.md'), '# Project Knowledge\n\n## Rules\n\nK001: Use real DB');

  const result = await inlineTacRootFile(tmp, 'knowledge.md', 'Project Knowledge');
...
test('knowledge: inlineTacRootFile returns null when KNOWLEDGE.md does not exist', async () => {
  const tmp = mkdtempSync(join(tmpdir(), 'tac-knowledge-'));
  const tacDir = join(tmp, '.tac');
  mkdirSync(tacDir, { recursive: true });
...
test('knowledge: appendKnowledge creates KNOWLEDGE.md with rule when file does not exist', async () => {
  ...
  const content = readFileSync(join(tacDir, 'KNOWLEDGE.md'), 'utf-8');
...
test('loadKnowledgeBlock: returns empty block when neither file exists', () => {
  const tmp = realpathSync(mkdtempSync(join(tmpdir(), 'tac-kb-')));
  const tacHome = join(tmp, 'home');
  const cwd = join(tmp, 'project');
  mkdirSync(join(cwd, '.tac'), { recursive: true });
  mkdirSync(join(tacHome, 'agent'), { recursive: true });

  const result = loadKnowledgeBlock(tacHome, cwd);
...
test('loadKnowledgeBlock: uses project knowledge alone when no global file', () => {
  ...
  mkdirSync(join(cwd, '.tac'), { recursive: true });
  mkdirSync(join(tacHome, 'agent'), { recursive: true });
  writeFileSync(join(cwd, '.tac', 'KNOWLEDGE.md'), 'K001: Use real DB');
...
test('loadKnowledgeBlock: uses global knowledge alone when no project file', () => {
  ...
  mkdirSync(join(cwd, '.tac'), { recursive: true });
  mkdirSync(join(tacHome, 'agent'), { recursive: true });
  writeFileSync(join(tacHome, 'agent', 'KNOWLEDGE.md'), 'G001: Respond in English');
...
test('loadKnowledgeBlock: merges global before project when both exist', () => {
  ...
  mkdirSync(join(cwd, '.tac'), { recursive: true });
  mkdirSync(join(tacHome, 'agent'), { recursive: true });
  writeFileSync(join(tacHome, 'agent', 'KNOWLEDGE.md'), 'G001: Global rule');
  writeFileSync(join(cwd, '.tac', 'KNOWLEDGE.md'), 'K001: Project rule');
...
test('loadKnowledgeBlock: reports globalSizeKb above 4KB threshold', () => {
  ...
  writeFileSync(join(tacHome, 'agent', 'KNOWLEDGE.md'), 'x'.repeat(5000));
...
`;
  writeFileSync(join(tacDir, 'KNOWLEDGE.md'), content);
