 ...
 *
 * 1. register-extension.ts: writeCrashLog writes to ~/.tac/crash/ directory
 * 2. register-extension.ts: _tacRejectionGuard registered for unhandledRejection
 * 3. register-extension.ts: _tacEpipeGuard exits with code 1 for unrecoverable errors (no log-and-continue)
...
describe('register-extension crash handler secondary fixes (#3348)', () => {
  test('writeCrashLog is exported and writes a file to the crash directory', async () => {
    // Dynamic import so TAC_HOME can be pointed at a temp dir without polluting ~/.tac
    const tmpHome = join(tmpdir(), `tac-crash-test-${randomUUID()}`);
    const origHome = process.env.TAC_HOME;
    process.env.TAC_HOME = tmpHome;
    try {
      ...
      process.env.TAC_HOME = origHome;
      rmSync(tmpHome, { recursive: true, force: true });
    }
  });

  test('_tacRejectionGuard is registered for unhandledRejection', () => {
    assert.match(
      registerExtSrc,
      /_tacRejectionGuard/,
      '_tacRejectionGuard handler should be defined',
    );
    assert.match(
      registerExtSrc,
      /unhandledRejection/,
  ...
  test('_tacEpipeGuard calls process.exit(1) for unrecoverable errors, not log-and-continue', () => {
    ...
    assert.ok(
      !registerExtSrc.includes('process.stderr.write(`[tac] uncaught extension error (non-fatal)'),
      '_tacEpipeGuard should NOT log errors as non-fatal and continue',
    );
    assert.match(
      registerExtSrc,
      /process\.exit\(1\)/,
      '_tacEpipeGuard should call process.exit(1) for unrecoverable errors',
    );
  });
  ...
  test('writeCrashLog never throws even when directory is unwritable', async () => {
    ...
    const tmpFile = join(tmpdir(), `tac-not-a-dir-${randomUUID()}`);
    // Don't create it — mkdirSync with bad path should be caught internally
    process.env.TAC_HOME = join(tmpFile, 'nested', 'deeply');
    try {
      // Should not throw
      assert.doesNotThrow(() => {
