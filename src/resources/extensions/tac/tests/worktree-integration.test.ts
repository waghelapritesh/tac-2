 ...
 *
 * Tests the full lifecycle of TAC operations inside a worktree:
 * - Branch namespacing (tac/<wt>/<M>/<S> instead of tac/<M>/<S>)
...
const base = mkdtempSync(join(tmpdir(), "tac-wt-integration-"));
...
writeFileSync(
  join(base, ".tac", "milestones", "M001", "slices", "S02", "S02-PLAN.md"),
  "# S02: Second\n\n**Goal:** Demo\n**Demo:** Demo\n\n## Must-Haves\n- done\n\n## Tasks\n- [ ] **T01: Implement** `est:10m`\n  do it\n",
  "utf-8",
);
run("git add .", base);
...
describe('worktree-integration', async () => {
  ...
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", "alpha"), "tac/alpha/M001/S01", "explicit worktree param");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01"), "tac/M001/S01", "no worktree param = plain branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "tac/alpha/M001/S01", "worktree-namespaced slice branch");
  ...
  assert.ok(!branches.includes("tac/alpha/M001/S01"), "slice branch cleaned up");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "tac/alpha/M001/S02", "on S02 namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt2.path), "tac/beta/M001/S01", "beta has its own namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "tac/alpha/M001/S01", "alpha re-created S01");

  // Both exist simultaneously
  const allBranches = run("git branch", base);
  assert.ok(allBranches.includes("tac/alpha/M001/S01"), "alpha S01 branch exists");
  assert.ok(allBranches.includes("tac/beta/M001/S01"), "beta S01 branch exists");
