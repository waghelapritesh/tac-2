...
test("auto-mode captures TAC_PROJECT_ROOT before entering the dispatch loop", () => {
  ...
  assert.ok(resumeCallIdx > -1, "auto.ts should capture TAC_PROJECT_ROOT before resume autoLoop");
  ...
  assert.ok(
    ...
    "auto.ts must set TAC_PROJECT_ROOT before the first loop call",
  );
});

test("auto-mode restores TAC_PROJECT_ROOT when execution stops or pauses", () => {
