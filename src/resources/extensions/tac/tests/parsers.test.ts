...
test('parseRoadmap: full roadmap', () => {
  const content = `# M001: TAC Extension — Hierarchical Planning
...
`;
  ...
  assert.deepStrictEqual(r.title, 'M001: TAC Extension — Hierarchical Planning', 'roadmap title');
...
Working in the tac-s01 worktree. All imports use .ts extensions.
...
`;
  ...
  assert.ok(c.context.includes('tac-s01 worktree'), 'context content');
