...
import { openDatabase, closeDatabase, _getAdapter, insertMilestone, insertSlice } from "../tac-db.js";
import { clearPathCache } from "../paths.js";
import { clearParseCache } from "../files.js";

function makeTmpBase(): string {
  const base = join(tmpdir(), `tac-val-handler-${randomUUID()}`);
...
describe("handleValidateMilestone write ordering (#2725)", () => {
  ...
  it("writes DB row and disk file on success", async () => {
    ...
    // Disk file exists
    const filePath = join(base, ".tac", "milestones", "M001", "M001-VALIDATION.md");
