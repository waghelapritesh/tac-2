...
function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-dbwriter-'));
...
describe('db-writer', () => {
  ...
  test('updateRequirementInDb — seeds from REQUIREMENTS.md when DB empty (#3346)', async () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(tmpDir, '.tac', 'REQUIREMENTS.md'), reqContent);
  ...
  test('saveArtifactToDb', async () => {
    ...
    try {
      ...
      const filePath = path.join(
        tmpDir, '.tac', 'milestones', 'M001', 'slices', 'S06', 'tasks', 'T01-SUMMARY.md',
      );
  ...
  test('saveArtifactToDb — shrinkage guard preserves larger existing file', async () => {
    ...
    try {
      ...
      const filePath = path.join(tmpDir, '.tac', relPath);
