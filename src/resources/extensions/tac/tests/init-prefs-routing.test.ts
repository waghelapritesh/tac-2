// TAC — /tac init → unified preferences-write routing tests.
//
...
test("writePreferencesFile — writes valid frontmatter from prefill", async () => {
  const tmp = mkdtempSync(join(tmpdir(), "tac-init-prefs-routing-"));
...
test("init — preferences path is basePath-derived, not cwd-derived (#4457 review)", async () => {
  ...
  // join(tacRoot(basePath), "PREFERENCES.md"), not the cwd-derived path.
  // Static check: the post-#4457-review code constructs the path from tacRoot(basePath).
  const src = readFileSync(
    new URL("../init-wizard.ts", import.meta.url),
    "utf-8",
  );
  assert.match(
    src,
    /projectPrefsPath\s*=\s*join\(tacRoot\(basePath\),\s*"PREFERENCES\.md"\)/,
    "init must derive the project preferences path from basePath",
  );
  // And neither write site should call getProjectTACPreferencesPath() (which
  // resolves from process.cwd()).
  assert.doesNotMatch(
    src,
    /getProjectTACPreferencesPath\s*\(/,
    "init must not use the cwd-derived getProjectTACPreferencesPath()",
  );
});

test("handlePrefsWizard — accepts pathOverride to target a non-cwd location", async () => {
  // The wizard's signature must support pathOverride so /tac init can route
  ...
  assert.match(
    src,
    /opts\?\.pathOverride[\s\S]*?\?\?[\s\S]*?(getProjectTACPreferencesPath|getGlobalTACPreferencesPath)/,
