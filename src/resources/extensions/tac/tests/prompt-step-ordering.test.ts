 ...
 *
 * 1. complete-milestone.md: tac_requirement_update (step 9) before
 *    tac_complete_milestone (step 10)
 * 2. complete-slice.md: uses tac_requirement_update
 * 3. register-extension.ts: _tacEpipeGuard logs instead of re-throwing
...
describe('prompt step ordering (#3696)', () => {
  test('tac_requirement_update step appears before tac_complete_milestone step', () => {
    // Search for the numbered step definitions, not early "Do NOT call" warnings
    const reqUpdateMatch = completeMilestoneMd.match(/^\d+\.\s.*tac_requirement_update/m);
    const completeMilestoneMatch = completeMilestoneMd.match(/^\d+\.\s.*tac_complete_milestone/m);
    assert.ok(reqUpdateMatch, 'tac_requirement_update should appear in a numbered step');
    assert.ok(completeMilestoneMatch, 'tac_complete_milestone should appear in a numbered step');
    ...
    assert.ok(
      ...
      'tac_requirement_update step must come before tac_complete_milestone step',
    );
  });

  test('complete-slice.md uses tac_requirement_update', () => {
    assert.match(completeSliceMd, /tac_requirement_update/,
      'complete-slice.md should reference tac_requirement_update');
  });
});

describe('register-extension _tacEpipeGuard (#3696)', () => {
  test('_tacEpipeGuard exists and does not re-throw', () => {
    assert.match(registerExtSrc, /_tacEpipeGuard/,
      '_tacEpipeGuard should be defined in register-extension.ts');
    // After the fix, the handler logs instead of throwing
    assert.ok(
      !registerExtSrc.includes('throw err'),
      '_tacEpipeGuard should NOT contain "throw err"',
    );
  });
});
