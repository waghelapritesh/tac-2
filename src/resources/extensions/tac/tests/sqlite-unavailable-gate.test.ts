 ...
 * refuse to start auto-mode. Otherwise tac_task_complete returns
...
const gateMatch = afterDbLifecycle.match(
  /if\s*\(existsSync\(tacDbPath\)\s*&&\s*!isDbAvailable\(\)\)\s*\{[\s\S]*?releaseLockAndReturn\(\);[\s\S]*?\}/,
);

assertTrue(
  !!gateMatch,
  "auto-start.ts has a hard abort gate when tac.db exists but SQLite is still unavailable (#2419)",
);
