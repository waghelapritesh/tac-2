// TAC-2 — Tests for validateExtensionPackage (EXTR-02, PKG-05)
...
test("validateExtensionPackage: valid package returns { valid: true }", (t) => {
  // EXTR-02: tac.extension: true, peerDependencies, pi.extensions
  const dir = makeTempDir();
  t.after(() => rmSync(dir, { recursive: true, force: true }));

  writeIndexTs(dir);
  writePackageJson(dir, {
    name: "@tac-extensions/test",
    version: "1.0.0",
    tac: { extension: true },
    pi: { extensions: ["./index.ts"] },
    peerDependencies: { "@tac/pi-coding-agent": "*" },
...
test("validateExtensionPackage: missing tac.extension marker returns error", (t) => {
  const dir = makeTempDir();
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  ...
  assert.ok(result.errors.some(e => e.includes("tac")), `Expected error about tac, got: ${JSON.stringify(result.errors)}`);
});
...
test("validateExtensionPackage: @tac/* in dependencies (not peerDependencies) returns error", (t) => {
  const dir = makeTempDir();
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  ...
  const result = validateExtensionPackage(dir);
  assert.equal(result.valid, false);
  assert.ok(result.errors.some(e => e.includes("@tac/pi-coding-agent")), `Expected error about @tac/ dep, got: ${JSON.stringify(result.errors)}`);
});

test("validateExtensionPackage: @tac/* in devDependencies returns error", (t) => {
  const dir = makeTempDir();
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  ...
  writePackageJson(dir, {
    ...
    pi: { extensions: ["./index.ts"] },
    peerDependencies: { "@tac/pi-coding-agent": "*" },
    devDependencies: { "@tac/pi-tui": "^2.0.0" },
  });

  const result = validateExtensionPackage(dir);
  assert.equal(result.valid, false);
  assert.ok(
    result.errors.some(e => e.includes("@tac/pi-tui") && e.includes("devDependencies")),
    `Expected error about @tac/ in devDependencies, got: ${JSON.stringify(result.errors)}`,
  );
});
