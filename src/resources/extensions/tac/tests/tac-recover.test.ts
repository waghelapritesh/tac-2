...
// tac-recover.test.ts — Tests for the `tac recover` recovery logic.
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-recover-'));
...
// ─── Recovery helpers (mirrors tac recover handler logic) ─────────────────
