...
test('prompt-db: re-import updates DB when source markdown changes', () => {
  // Create a temp dir simulating a project with .tac/DECISIONS.md
...
`;
  writeFileSync(join(tacDir, 'DECISIONS.md'), initialDecisions);
...
`;
  writeFileSync(join(tacDir, 'DECISIONS.md'), updatedDecisions);
