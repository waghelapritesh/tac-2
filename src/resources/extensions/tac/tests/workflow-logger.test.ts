// TAC Extension — Workflow Logger Tests
...
describe("workflow-logger", () => {
  ...
  describe("audit log persistence", () => {
    ...
    test("writes entry to .tac/audit-log.jsonl after setLogBasePath", () => {
      setLogBasePath(dir);
      logError("engine", "audit test entry");

      const auditPath = join(dir, ".tac", "audit-log.jsonl");
  ...
  describe("stderr output", () => {
    test("writes WARN prefix to stderr for warnings", (t) => {
      ...
      assert.ok(written[0].includes("[tac:engine] WARN: test warn"));
    });
    ...
    test("writes ERROR prefix to stderr for errors", (t) => {
      ...
      logError("intercept", "blocked");
      assert.ok(written[0].includes("[tac:intercept] ERROR: blocked"));
    });
