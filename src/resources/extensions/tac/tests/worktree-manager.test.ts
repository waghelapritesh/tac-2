...
function makeBaseRepo(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-wt-test-"));
...
function makeRepoWithChanges(worktreeName: string): { base: string; wtPath: string } {
  ...
  mkdirSync(join(wtPath, ".tac", "milestones", "M002"), { recursive: true });
  writeFileSync(
    join(wtPath, ".tac", "milestones", "M002", "M002-ROADMAP.md"),
    "# M002: New Feature\n\n## Slices\n- [ ] **S01: Setup** `risk:low` `depends:[]`\n  > After this: new feature ready\n",
    "utf-8",
  );
  writeFileSync(
    join(wtPath, ".tac", "milestones", "M001", "M001-ROADMAP.md"),
...
describe("createWorktree", () => {
  ...
  test("creates worktree with correct metadata", () => {
    ...
    assert.ok(
      existsSync(join(info.path, ".tac", "milestones", "M001", "M001-ROADMAP.md")),
      ".tac files should be in worktree",
    );
...
describe("diffWorktreeTAC and getWorktreeTACDiff", () => {
  let base: string;
  beforeEach(() => {
  ...
  test("detects added and modified TAC files", () => {
    const diff = diffWorktreeTAC(base, "feature-x");
  ...
  test("returns patch content", () => {
    const fullDiff = getWorktreeTACDiff(base, "feature-x");
