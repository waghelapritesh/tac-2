...
test("SLICE_BRANCH_RE captures milestone + slice without worktree prefix", () => {
  for (const { input, expectMid } of [
    { input: "tac/M001/S01", expectMid: "M001" },
    { input: "tac/M001-abc123/S01", expectMid: "M001-abc123" },
  ]) {
...
test("SLICE_BRANCH_RE captures worktree prefix when present", () => {
  for (const { input, expectMid } of [
    { input: "tac/worktree/M001/S01", expectMid: "M001" },
    { input: "tac/worktree/M001-abc123/S01", expectMid: "M001-abc123" },
  ]) {
    const m = input.match(SLICE_BRANCH_RE);
    assert.ok(m, `should match ${input}`);
...
test("SLICE_BRANCH_RE rejects malformed inputs", () => {
  assert.ok(!SLICE_BRANCH_RE.test("tac/S01"), "no milestone");
  assert.ok(!SLICE_BRANCH_RE.test("main"), "non-tac branch");
  assert.ok(!SLICE_BRANCH_RE.test("tac/M001"), "no slice");
...
test("MILESTONE_CONTEXT_RE matches legacy and unique CONTEXT.md names", () => {
  ...
  assert.ok(
    MILESTONE_CONTEXT_RE.test(".tac/milestones/M001/M001-CONTEXT.md"),
    "full path legacy format",
  );
  assert.ok(
    MILESTONE_CONTEXT_RE.test(".tac/milestones/M001-abc123/M001-abc123-CONTEXT.md"),
