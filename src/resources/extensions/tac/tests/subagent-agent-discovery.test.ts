...
function makeProjectRoot(t: test.TestContext): string {
	const root = mkdtempSync(join(tmpdir(), "tac-subagent-agents-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
	return root;
}

function writeAgent(root: string, configDirName: ".tac" | ".pi", name = "ping"): string {
...
test("discoverAgents finds project agents in .tac/agents", (t) => {
	const root = makeProjectRoot(t);
	const agentsDir = writeAgent(root, ".tac");
...
test("discoverAgents accepts tools frontmatter as a YAML list", (t) => {
	const root = makeProjectRoot(t);
	const agentsDir = join(root, ".tac", "agents");
	mkdirSync(agentsDir, { recursive: true });
	writeFileSync(
