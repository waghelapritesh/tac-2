/**
 * TAC visualizer overlay — behaviour tests.
 *
...
import { TACVisualizerOverlay } from "../visualizer-overlay.ts";
...
function makeOverlay(t: { after: (fn: () => void) => void }, onClose: () => void = () => {}) {
  ...
  const overlay = new TACVisualizerOverlay(tui as any, mockTheme, onClose);
...
test("overlay dispose is idempotent and flips the disposed flag", (t) => {
  const { tui } = makeTui();
  const overlay = new TACVisualizerOverlay(tui as any, mockTheme, () => {});
