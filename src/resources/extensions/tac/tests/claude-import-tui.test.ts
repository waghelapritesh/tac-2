 ...
 *
 * Tests R015: validates the TUI command flow for /tac prefs import-claude.
...
describe(
	...
	() => {
		...
		before(() => {
			tempDir = mkdtempSync(join(tmpdir(), 'tac-tui-test-'));
		...
		describe('runClaudeImportFlow()', () => {
			...
			it('should not persist marketplace agent directories into package sources', async (t) => {
				const isolatedAgentDir = join(tempDir, '.tac', 'agent');
				const settingsPath = join(isolatedAgentDir, 'settings.json');
				rmSync(isolatedAgentDir, { recursive: true, force: true });
				process.env.TAC_CODING_AGENT_DIR = isolatedAgentDir;

				t.after(() => {
					delete process.env.TAC_CODING_AGENT_DIR;
					rmSync(isolatedAgentDir, { recursive: true, force: true });
				});
