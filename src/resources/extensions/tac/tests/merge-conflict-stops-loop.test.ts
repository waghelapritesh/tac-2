 ...
 *
 * When a squash merge has real code conflicts (not just .tac/ files),
...
describe("WorktreeResolver.mergeAndExit re-throws MergeConflictError (#2330)", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(baseDir, ".tac", "milestones", "M001"), { recursive: true });
  });
  ...
  test("propagates MergeConflictError with conflicted file list", () => {
    ...
    const roadmapPath = join(baseDir, ".tac", "milestones", "M001", "M001-ROADMAP.md");
    const deps = makeDeps({
