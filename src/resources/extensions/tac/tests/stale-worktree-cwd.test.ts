 ...
 *
 * Verifies that when process.cwd() is inside a stale .tac/worktrees/ path,
...
test("detects stale worktree path and extracts project root", () => {
  // Simulate the path pattern: /project/.tac/worktrees/M004/...
  const projectRoot = "/Users/test/myproject";
  const stalePath = `${projectRoot}${sep}.tac${sep}worktrees${sep}M004`;
...
test("mergeMilestoneToMain restores cwd to project root", () => {
  ...
  const fakeHome = realpathSync(mkdtempSync(join(tmpdir(), "tac-fake-home-")));
  ...
  try {
    ...
    const msDir = join(tempDir, ".tac", "milestones", "M050");
...
test("process.cwd() inside removed worktree is recoverable", () => {
  ...
  try {
    ...
    // Create a .tac/worktrees/M099 directory to simulate stale state
    const staleWtDir = join(tempDir, ".tac", "worktrees", "M099");
    mkdirSync(staleWtDir, { recursive: true });

    // Enter the stale directory
    process.chdir(staleWtDir);
    const cwdBefore = process.cwd();
    assert.ok(cwdBefore.includes(`${sep}.tac${sep}worktrees${sep}`), "cwd is inside worktree dir");
