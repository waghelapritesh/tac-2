...
function createTestDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-logs-test-"));
  mkdirSync(join(dir, ".tac", "activity"), { recursive: true });
  mkdirSync(join(dir, ".tac", "debug"), { recursive: true });
  return dir;
}
...
function writeActivityLog(dir: string, seq: number, unitType: string, unitId: string, entries: Record<string, unknown>[]): void {
  ...
  writeFileSync(join(dir, ".tac", "activity", filename), content);
}

function writeDebugLog(dir: string, name: string, entries: Record<string, unknown>[]): void {
  const content = entries.map(e => JSON.stringify(e)).join("\n") + "\n";
  writeFileSync(join(dir, ".tac", "debug", name), content);
}
...
test("logs lists activity logs", async (t) => {
  ...
  assert.ok(msg.includes("/tac logs <#>"), "should show usage hint");
});
...
test("logs clear removes old logs", async (t) => {
  ...
  const oldFile = join(dir, ".tac", "activity", "001-execute-task-M001-S01-T01.jsonl");
  ...
  assert.ok(
    existsSync(join(dir, ".tac", "activity", "007-execute-task-M001-S01-T07.jsonl")),
