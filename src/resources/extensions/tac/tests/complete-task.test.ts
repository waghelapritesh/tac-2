...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-complete-task-'));
...
/**
 * Create a temp project directory with .tac structure for handler tests.
 */
function createTempProject(): { basePath: string; planPath: string } {
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-handler-'));
  const tasksDir = path.join(basePath, '.tac', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });

  const planPath = path.join(basePath, '.tac', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md');
...
console.log('\n=== complete-task: accessor stale-state error ===');
{
  // No DB open — accessors should throw TAC_STALE_STATE
  closeDatabase();
  let threw = false;
  try {
    ...
    threw = true;
    assertTrue(err.code === 'TAC_STALE_STATE' || err.message.includes('No database open'),
      'should throw TAC_STALE_STATE when no DB open');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'TAC_STALE_STATE' || err.message.includes('No database open'),
      'insertSlice should throw TAC_STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'TAC_STALE_STATE' || err.message.includes('No database open'),
      'insertTask should throw TAC_STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'TAC_STALE_STATE' || err.message.includes('No database open'),
      'insertVerificationEvidence should throw TAC_STALE_STATE');
  }
...
console.log('\n=== complete-task: handler with missing plan file ===');
{
  ...
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'tac-no-plan-'));
  const tasksDir = path.join(basePath, '.tac', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });
