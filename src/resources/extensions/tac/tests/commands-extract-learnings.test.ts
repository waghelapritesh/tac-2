...
describe("buildLearningsOutputPath", () => {
  it("builds the correct output path", () => {
    const result = buildLearningsOutputPath("/base/.tac/milestones/M001", "M001");
    assert.equal(result, "/base/.tac/milestones/M001/M001-LEARNINGS.md");
  });

  it("builds path for different milestone ID", () => {
    const result = buildLearningsOutputPath("/project/.tac/milestones/M005", "M005");
    assert.equal(result, "/project/.tac/milestones/M005/M005-LEARNINGS.md");
  });
});
...
describe("resolvePhaseArtifacts", () => {
  ...
  beforeEach(() => {
    tmpBase = join(tmpdir(), `tac-learnings-test-${randomUUID()}`);
...
describe("buildExtractLearningsPrompt", () => {
  it("includes milestoneId and outputPath", () => {
    const result = buildExtractLearningsPrompt({
      ...
      outputPath: "/project/.tac/milestones/M001/M001-LEARNINGS.md",
      relativeOutputPath: ".tac/milestones/M001/M001-LEARNINGS.md",
    ...
    assert.ok(result.includes("/project/.tac/milestones/M001/M001-LEARNINGS.md"));
  });
  ...
  it("does NOT reference phantom tac_graph tool (regression for #4429)", () => {
    const result = buildExtractLearningsPrompt({
      milestoneId: "M001",
      milestoneName: "Test Milestone",
      outputPath: "/out/M001-LEARNINGS.md",
    ...
    assert.ok(
      !result.includes("tac_graph"),
      "prompt must not advertise the non-existent tac_graph tool",
    );
  });
...
describe("extractProjectName", () => {
  ...
  beforeEach(() => {
    tmpBase = join(tmpdir(), `tac-projname-test-${randomUUID()}`);
    mkdirSync(join(tmpBase, ".tac"), { recursive: true });
  });
...
// into KNOWLEDGE.md and the DECISIONS DB. Both the manual /tac extract-learnings
...
describe("buildExtractionStepsBlock", () => {
  const ctx = {
    milestoneId: "M042",
    outputPath: "/project/.tac/milestones/M042/M042-LEARNINGS.md",
    relativeOutputPath: ".tac/milestones/M042/M042-LEARNINGS.md",
  };
  ...
  // template) and the tac_save_decision call-out (parameter list, "never edit
  ...
  // longer reference KNOWLEDGE.md tables or tac_save_decision at all, so each
  // assertion is now structurally false.
  //
  // The replacement assertions ("removes the legacy KNOWLEDGE.md table append
  // step", "removes the tac_save_decision call", "requires structuredFields
  ...
  it("removes the legacy KNOWLEDGE.md table append step (ADR-013 step 6 cutover)", () => {
    ...
    assert.ok(!block.includes(".tac/KNOWLEDGE.md"), "extraction flow must not reference KNOWLEDGE.md as a write target");
  });

  it("removes the tac_save_decision call (ADR-013 step 6 cutover)", () => {
    const block = buildExtractionStepsBlock(ctx);
    ...
    // tac_save_decision schema. The legacy MCP tool is no longer called from
    // the extraction flow.
    assert.ok(!block.includes("tac_save_decision"), "tac_save_decision must no longer appear in extraction steps");
  });
  ...
  it("requires structuredFields payload on architecture-category memories (ADR-013 lossless projection)", () => {
    ...
    // structuredFields with the original tac_save_decision schema fields.
  ...
  it("does NOT reference the non-existent tac_graph tool (#4429 regression)", () => {
    const block = buildExtractionStepsBlock(ctx);
    assert.ok(!block.includes("tac_graph"));
  });

  it("substitutes the milestone ID into every placeholder callout", () => {
    const block = buildExtractionStepsBlock({
      milestoneId: "M999",
      outputPath: "/p/.tac/milestones/M999/M999-LEARNINGS.md",
      relativeOutputPath: ".tac/milestones/M999/M999-LEARNINGS.md",
    });
...
describe("buildExtractLearningsPrompt composes the steps block", () => {
  it("embeds the exact buildExtractionStepsBlock output for the same context", () => {
    const shared = {
      milestoneId: "M007",
      outputPath: "/p/.tac/milestones/M007/M007-LEARNINGS.md",
      relativeOutputPath: ".tac/milestones/M007/M007-LEARNINGS.md",
    };
...
describe("complete-milestone loadPrompt round-trip (#4429)", () => {
  it("substitutes {{extractLearningsSteps}} end-to-end via prompt-loader", async () => {
    ...
    const stepsBlock = buildExtractionStepsBlock({
      milestoneId: "M123",
      outputPath: "/p/.tac/milestones/M123/M123-LEARNINGS.md",
      relativeOutputPath: ".tac/milestones/M123/M123-LEARNINGS.md",
    });
    ...
    const rendered = loadPrompt("complete-milestone", {
      ...
      roadmapPath: ".tac/milestones/M123/M123-ROADMAP.md",
      inlinedContext: "(inlined context stub)",
      milestoneSummaryPath: "/p/.tac/milestones/M123/M123-SUMMARY.md",
    ...
    // ADR-013 cutover: tac_save_decision is no longer in the rendered block;
