...
function createBase(): string {
  const base = mkdtempSync(join(tmpdir(), "tac-plan-queue-"));
  mkdirSync(join(base, ".tac", "milestones", "M010"), { recursive: true });
  return base;
}
...
describe("plan-milestone queue context", () => {
  test("includes queue brief when planning milestone without roadmap context", async () => {
    ...
    try {
      ...
      assert.match(prompt, /Source: `\.tac\/QUEUE\.md`/);
