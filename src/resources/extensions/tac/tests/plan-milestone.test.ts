...
import { openDatabase, closeDatabase, getMilestone, getMilestoneSlices, getSlice, updateSliceStatus, deleteSlice, insertMilestone } from '../tac-db.ts';
import { handlePlanMilestone } from '../tools/plan-milestone.ts';
import { parseRoadmap } from '../parsers-legacy.ts';

function makeTmpBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'tac-plan-milestone-'));
...
test('handlePlanMilestone surfaces render failures and does not clear parse-visible state on failure', async () => {
  ...
  try {
    const fallbackRoadmapPath = join(base, '.tac', 'milestones', 'MISSING', 'MISSING-ROADMAP.md');
    ...
    const existingRoadmapPath = join(base, '.tac', 'milestones', 'M001', 'M001-ROADMAP.md');
...
test('handlePlanMilestone promotes pre-existing queued milestone to active (#3022)', async () => {
  ...
  try {
    ...
    // (this is what tac_milestone_generate_id does)
