...
test("#2684: preferences files are NOT in ROOT_STATE_FILES (forward-only sync)", () => {
  ...
  // in syncTacStateToWorktree() (forward-only, additive). Including it in
...
test("syncTacStateToWorktree copies canonical PREFERENCES.md", async () => {
  // Functional test: create a mock source and destination, call the sync
  const srcBase = mkdtempSync(join(tmpdir(), "tac-wt-prefs-src-"));
  const dstBase = mkdtempSync(join(tmpdir(), "tac-wt-prefs-dst-"));
  const srcTac = join(srcBase, ".tac");
  const dstTac = join(dstBase, ".tac");
  mkdirSync(srcTac, { recursive: true });
  mkdirSync(dstTac, { recursive: true });

  try {
    // Write a canonical PREFERENCES.md in source
    writeFileSync(
      join(srcTac, "PREFERENCES.md"),
      "---\nversion: 1\n---\n\npost_unit_hooks:\n  - name: notify\n    command: echo done\n",
    );

    // Import and call syncTacStateToWorktree
    const { syncTacStateToWorktree } = await import("../auto-worktree.ts");
    syncTacStateToWorktree(srcBase, dstBase);

    // Verify PREFERENCES.md was copied
    assert.ok(
      existsSync(join(dstTac, "PREFERENCES.md")),
      "PREFERENCES.md should be copied to worktree",
    );

    const content = readFileSync(join(dstTac, "PREFERENCES.md"), "utf-8");
    assert.ok(
...
test("syncTacStateToWorktree falls back to legacy lowercase preferences.md", async () => {
  const srcBase = mkdtempSync(join(tmpdir(), "tac-wt-prefs-legacy-src-"));
  const dstBase = mkdtempSync(join(tmpdir(), "tac-wt-prefs-legacy-dst-"));
  const srcTac = join(srcBase, ".tac");
  const dstTac = join(dstBase, ".tac");
  mkdirSync(srcTac, { recursive: true });
  mkdirSync(dstTac, { recursive: true });
  ...
  try {
    ...
    const { syncTacStateToWorktree } = await import("../auto-worktree.ts");
    const result = syncTacStateToWorktree(srcBase, dstBase);

    const copiedEntries = readdirSync(dstTac)
