// TAC Exec History — read-side helpers for the exec sandbox.
//
// Pure I/O: scans `.tac/exec/*.meta.json` under a base directory and
// returns lightweight records. Used by the tac_exec_search tool and
...
function listMetaFiles(baseDir: string): string[] {
  const dir = resolve(baseDir, ".tac", "exec");
  try {
