...
// ─── restore ─────────────────────────────────────────────────────────────
//
// The actual restore() implementation lives in tac-db.ts (single-writer
...
/**
 * Write current DB state to .tac/state-manifest.json via atomicWriteSync.
...
export function readManifest(basePath: string): StateManifest | null {
  const manifestPath = join(basePath, ".tac", "state-manifest.json");
