 ...
 *
 * Installed files land in `~/.tac/workflows/<name>.<ext>` by default, or
 * `.tac/workflows/<name>.<ext>` with the `--project` flag.
 *
 * A provenance file `~/.tac/workflows/.installed.json` (or project equivalent)
 * records source URL, timestamp, and sha256 so `/tac workflow uninstall` can
 * clean up and future `/tac workflow update` can refresh.
 */
...
export async function fetchWorkflowSource(url: string): Promise<FetchedContent> {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "tac-workflow-install" },
    });
