/**
 * TAC Preparation — Structured brief generation for discussion LLM sessions.
 *
...
/** Prior context brief aggregated from TAC artifacts. */
...
/**
 * Aggregate prior context from TAC artifacts.
 *
 * Reads DECISIONS.md, REQUIREMENTS.md, KNOWLEDGE.md from the .tac directory
 * and milestone summaries from each milestone's MILESTONE-SUMMARY.md file.
 *
 * @param basePath - Root directory of the project (contains .tac/)
...
export async function aggregatePriorContext(basePath: string): Promise<PriorContextBrief> {
  const tacPath = join(basePath, ".tac");

  // Load decisions
  const decisionsContent = await loadFile(join(tacPath, "DECISIONS.md"));
  const decisions = parseDecisions(decisionsContent);

  // Load requirements
  const requirementsContent = await loadFile(join(tacPath, "REQUIREMENTS.md"));
  const requirements = parseRequirements(requirementsContent);

  // Load knowledge
  const knowledgeContent = await loadFile(join(tacPath, "KNOWLEDGE.md"));
  const knowledge = truncateSection(knowledgeContent || "", MAX_SECTION_CHARS);

  // Load milestone summaries
  const summaries = await loadMilestoneSummaries(tacPath);
 ...
 */
async function loadMilestoneSummaries(tacPath: string): Promise<string> {
