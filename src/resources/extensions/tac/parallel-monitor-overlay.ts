/**
 * TAC Parallel Monitor Overlay
 *
 * Full-screen TUI overlay showing real-time parallel worker progress.
 * Opened via `/tac parallel watch`, Ctrl+Alt+P (⌃⌥P on macOS),
...
import { truncateToWidth, visibleWidth, matchesKey, Key } from "@tac/pi-tui";
...
function discoverWorkers(basePath: string): string[] {
  ...
  const worktreeDir = join(basePath, ".tac", "worktrees");
  ...
  if (existsSync(worktreeDir)) {
    try {
      for (const d of readdirSync(worktreeDir)) {
        if (d.startsWith("M") && existsSync(join(worktreeDir, d, ".tac", "auto.lock"))) {
          mids.add(d);
        }
      }
    } catch { /* skip */ }
...
function querySliceProgress(basePath: string, mid: string): SliceProgress[] {
  const dbPath = join(basePath, ".tac", "worktrees", mid, ".tac", "tac.db");
...
function extractCostFromNdjson(basePath: string, mid: string): number {
  const stdoutPath = join(basePath, ".tac", "parallel", `${mid}.stdout.log`);
...
function collectWorkerData(basePath: string): WorkerView[] {
  ...
  for (const mid of mids) {
    ...
    const lock = readJsonSafe<AutoLock>(join(basePath, ".tac", "worktrees", mid, ".tac", "auto.lock"));
...
export class ParallelMonitorOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(t.bold(t.fg("accent", " TAC Parallel Monitor ")));
    lines.push(
    ...
    if (this.workers.length === 0) {
      ...
      lines.push(t.fg("muted", "  Run /tac parallel start to begin."));
    } else {
      for (const wk of this.workers) {
        lines.push("");
