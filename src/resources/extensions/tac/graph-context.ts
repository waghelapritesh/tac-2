...
function readGraphFile(projectDir: string): GraphFileShape | null {
  try {
    const graphPath = join(projectDir, ".tac", "graphs", "graph.json");
...
async function resolveGraphApi(): Promise<GraphApi> {
  ...
  try {
    const imported = await import("@waghelapritesh/mcp-server");
    if (isGraphApi(imported)) {
      cachedGraphApi = imported;
      return cachedGraphApi;
    }
    logWarning("prompt", "@waghelapritesh/mcp-server graph exports unavailable; using local graph fallback");
  } catch {
 ...
 *
 * Returns null when:
 * - @waghelapritesh/mcp-server fails to import
