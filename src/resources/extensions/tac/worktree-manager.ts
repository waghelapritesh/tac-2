/**
 * TAC Worktree Manager
 *
 * Creates and manages git worktrees under .tac/worktrees/<name>/.
 * Each worktree gets its own branch (worktree/<name>) and a full
 * working copy of the project, enabling parallel work streams.
 *
 * The merge helper compares .tac/ artifacts between a worktree and
 * the main branch, then dispatches an LLM-guided merge flow.
 *
 * Flow:
 *   1. create()  — git worktree add .tac/worktrees/<name> -b worktree/<name>
 *   2. user works in the worktree (new plans, milestones, etc.)
 *   3. merge()   — LLM-guided reconciliation of .tac/ artifacts back to main
...
import { TACError, TAC_PARSE_ERROR, TAC_STALE_STATE, TAC_LOCK_HELD, TAC_GIT_ERROR, TAC_MERGE_CONFLICT } from "./errors.js";
...
export interface WorktreeDiffSummary {
  /** Files only in the worktree .tac/ (new artifacts) */
  added: string[];
  /** Files in both but with different content */
  modified: string[];
  /** Files only in main .tac/ (deleted in worktree) */
  removed: string[];
}
...
export function worktreesDir(basePath: string): string {
  return join(basePath, ".tac", "worktrees");
}
...
/**
 * Validate that a path is inside the .tac/worktrees/ directory.
...
export function isInsideWorktreesDir(basePath: string, targetPath: string): boolean {
  ...
  // not merely be a prefix match (e.g. ".tac/worktrees-extra" must not match).
 ...
 *
 * If a live git worktree exists for this milestone at `.tac/worktrees/<MID>/`
...
/**
 * Create a new git worktree under .tac/worktrees/<name>/ with branch worktree/<name>.
...
export function createWorktree(basePath: string, name: string, opts: { branch?: string; startPoint?: string; reuseExistingBranch?: boolean } = {}): WorktreeInfo {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
    throw new TACError(TAC_PARSE_ERROR, `Invalid worktree name "${name}". Use only letters, numbers, hyphens, and underscores.`);
  }
  ...
  if (existsSync(wtPath)) {
    ...
    if (!existsSync(gitFilePath)) {
      ...
      throw new TACError(TAC_STALE_STATE, `Worktree "${name}" already exists at ${wtPath}`);
    }
  }

  // Ensure the .tac/worktrees/ directory exists
...
/**
 * List all TAC-managed worktrees.
 * Uses native worktree list and filters to those under .tac/worktrees/.
 */
export function listWorktrees(basePath: string): WorktreeInfo[] {
  ...
  const worktreeRoots = baseVariants
    .map(baseVariant => {
      const path = join(baseVariant, ".tac", "worktrees");
      return {
  ...
  for (const entry of entries) {
    ...
    // Only include worktrees under .tac/worktrees/
...
const NESTED_GIT_SKIP_DIRS = new Set([
  ".git", ".tac", "node_modules", ".next", ".nuxt", "dist", "build",
 ...
 * Skips node_modules, .tac, and other non-project directories for performance.
 *
...
export function removeWorktree(
  ...
  // The computed path may differ when .tac/ is (or was) a symlink to an
  ...
  // If syncStateToProjectRoot later creates a real .tac/ directory that
  ...
  // inside .tac/worktrees/.  When .tac/ was a symlink, git may have resolved
  ...
  if (gitReportedPath && isInsideWorktreesDir(basePath, gitReportedPath)) {
    ...
    console.error(
      `[TAC] WARNING: git worktree list reported path outside .tac/worktrees/: ${gitReportedPath}\n` +
  ...
  // inside .tac/worktrees/ — a symlink inside the directory could point out.
  ...
  if (existsSync(gitmodulesPath)) {
    try {
      ...
      if (hasSubmoduleChanges) {
        ...
        try {
          execFileSync(
            "git", ["stash", "push", "-m", "tac: auto-stash submodule changes before worktree teardown"],
            { cwd: resolvedWtPath, stdio: ["ignore", "pipe", "pipe"], encoding: "utf-8" },
          );
  ...
  if (resolvedPathSafe) {
    ...
    // causes every subsequent `/tac auto` to re-enter the stale worktree.
    if (existsSync(resolvedWtPath)) {
      try {
    ...
    console.error(
      `[TAC] WARNING: Resolved worktree path is outside .tac/worktrees/: ${resolvedWtPath}\n` +
 ...
 *
 * NOTE: These arrays must stay synchronized with TAC_RUNTIME_PATTERNS in gitignore.ts.
...
/** File prefixes to skip (for wildcard patterns like completed-units*.json, tac.db*). */
const SKIP_PREFIXES = [
  ".tac/completed-units",
  ".tac/tac.db",
];
...
/**
 * Diff the .tac/ directory between the worktree branch and main branch.
 * Returns a summary of added, modified, and removed TAC artifacts.
 */
export function diffWorktreeTAC(basePath: string, name: string): WorktreeDiffSummary {
  const branch = worktreeBranchName(name);
  const mainBranch = nativeDetectMainBranch(basePath);

  const entries = nativeDiffNameStatus(basePath, mainBranch, branch, ".tac/", true);
...
/**
 * Get the full diff content for .tac/ between the worktree branch and main.
 * Returns the raw unified diff for LLM consumption.
 */
export function getWorktreeTACDiff(basePath: string, name: string): string {
  const branch = worktreeBranchName(name);
  const mainBranch = nativeDetectMainBranch(basePath);

  return nativeDiffContent(basePath, mainBranch, branch, ".tac/", undefined, true);
}

/**
 * Get the full diff content for non-.tac/ files between the worktree branch and main.
 * Returns the raw unified diff for LLM consumption.
 */
export function getWorktreeCodeDiff(basePath: string, name: string): string {
  ...
  return nativeDiffContent(basePath, mainBranch, branch, undefined, ".tac/", true);
}
...
export function mergeWorktreeToMain(basePath: string, name: string, commitMessage: string): string {
  ...
  if (current !== mainBranch) {
    throw new TACError(TAC_GIT_ERROR, `Must be on ${mainBranch} to merge. Currently on ${current}.`);
  }

  const result = nativeMergeSquash(basePath, branch);
  if (!result.success) {
    throw new TACError(TAC_MERGE_CONFLICT, `Merge conflicts detected in: ${result.conflicts.join(", ")}`);
  }
