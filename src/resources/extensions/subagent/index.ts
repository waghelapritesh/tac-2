...
import type { AgentToolResult } from "@tac/pi-agent-core";
import type { Message } from "@tac/pi-ai";
import { StringEnum } from "@tac/pi-ai";
import { type ExtensionAPI, getMarkdownTheme } from "@tac/pi-coding-agent";
import { Container, Markdown, Spacer, Text } from "@tac/pi-tui";
import { Type } from "@sinclair/typebox";
import { formatTokenCount } from "../shared/mod.js";
import { getCurrentPhase } from "../shared/tac-phase-state.js";
...
import { loadEffectiveTACPreferences } from "../tac/preferences.js";
...
async function runSingleAgent(
	...
	// TAC phase guard: block agents that conflict with the active TAC phase
	if (agent.conflictsWith && agent.conflictsWith.length > 0) {
		...
		if (activePhase && agent.conflictsWith.includes(activePhase)) {
			return {
				...
				exitCode: 1,
				messages: [],
				stderr: `Agent "${agentName}" is blocked: it conflicts with the active TAC phase "${activePhase}". Use the built-in TAC workflow instead.`,
				usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, contextTokens: 0, turns: 0 },
				step,
			};
		}
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			const bundledPaths = (process.env.TAC_BUNDLED_EXTENSION_PATHS ?? "").split(path.delimiter).map(s => s.trim()).filter(Boolean);
			const extensionArgs = bundledPaths.flatMap(p => ["--extension", p]);
			const proc = spawn(
				process.execPath,
				[process.env.TAC_BIN_PATH!, ...extensionArgs, ...args],
...
async function runSingleAgentInCmuxSplit(
	...
	try {
		...
		const bundledPaths = (process.env.TAC_BUNDLED_EXTENSION_PATHS ?? "").split(path.delimiter).map((s) => s.trim()).filter(Boolean);
		const extensionArgs = bundledPaths.flatMap((p) => ["--extension", p]);
		const processArgs = [process.env.TAC_BIN_PATH!, ...extensionArgs, ...buildSubagentProcessArgs(agent, task, tmpPromptPath)];
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("subagent", {
		...
		handler: async (_args, ctx) => {
			...
			if (discovery.agents.length === 0) {
				ctx.ui.notify("No agents found. Add .md files to ~/.tac/agent/agents/ or .tac/agents/", "warning");
				return;
			}
	...
	pi.registerTool({
		...
		description: [
			...
			"Agents are defined as .md files in ~/.tac/agent/agents/ (user) or .tac/agents/ (project).",
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const cmuxClient = CmuxClient.fromPreferences(loadEffectiveTACPreferences()?.preferences);
