...
const tacHome = process.env.TAC_HOME || path.join(os.homedir(), ".tac");

function getIsolationBaseDir(cwd: string, taskId: string): string {
	return path.join(tacHome, "wt", encodeCwd(cwd), taskId);
}
...
async function applyBaseline(
	...
	if (baseline.stagedDiff.trim()) {
		const patchPath = path.join(worktreeDir, ".tac-staged.patch");
	...
	if (baseline.unstagedDiff.trim()) {
		const patchPath = path.join(worktreeDir, ".tac-unstaged.patch");
	...
	await gitSilent(
		["commit", "--allow-empty", "-m", "tac: baseline snapshot"],
		worktreeDir,
	);
}
...
export function readIsolationMode(): IsolationMode {
	try {
		const { getAgentDir } = require("@tac/pi-coding-agent");
