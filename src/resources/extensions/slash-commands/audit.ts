...
export default function auditCommand(pi: ExtensionAPI) {
	pi.registerCommand("audit", {
		description: "Audit the current codebase against a specific goal and write a structured report to .tac/audits/",
		async handler(args: string, ctx: ExtensionCommandContext) {
			...
			// ── Step 2: Build output path (.tac/audits/<timestamp>-<slug>.md) ────
			...
			const outputPath = `.tac/audits/${timestamp}-${slug}.md`;

			// ── Step 3: Ensure the output directory exists ───────────────────────

			mkdirSync(".tac/audits", { recursive: true });
...
## Next Steps
<!-- Concrete, prioritised actions. These should be directly usable as input to /tac-roadmap. -->
