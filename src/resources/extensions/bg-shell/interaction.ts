...
export async function queryShellEnv(
	...
	const sentinel = `__TAC_ENV_${randomUUID().slice(0, 8)}__`;
...
export async function runOnSession(
	...
	const startMarker = `__TAC_SENTINEL_${sentinel}_START__`;
	const endMarker = `__TAC_SENTINEL_${sentinel}_END__`;
	const exitVar = `__TAC_EXIT_${sentinel}__`;
