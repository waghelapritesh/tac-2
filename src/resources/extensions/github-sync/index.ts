/**
 * GitHub Sync extension for TAC.
 *
 * Opt-in extension that syncs TAC lifecycle events to GitHub:
...
async function showStatus(ctx: import("@tac/pi-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
...
async function runBootstrap(ctx: import("@tac/pi-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
    ctx.ui.notify("GitHub sync: `gh` CLI not installed or not authenticated.", "warning");
    return;
  }
