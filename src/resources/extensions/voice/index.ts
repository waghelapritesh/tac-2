...
import type { AssistantMessage } from "@tac/pi-ai";
import { isKeyRelease, Key, matchesKey, truncateToWidth, visibleWidth } from "@tac/pi-tui";
...
function ensureLinuxReady(ctx: ExtensionContext): boolean {
	...
	if (!process.env.GROQ_API_KEY) {
		ctx.ui.notify("Voice: GROQ_API_KEY not set — run 'tac config' to configure", "error");
		return false;
	}
