...
import { resolveSearchProviderFromPreferences } from '../tac/preferences.js'

// Compute authFilePath locally instead of importing from app-paths.ts,
// because extensions are copied to ~/.tac/agent/extensions/ at runtime
// where the relative import '../../../app-paths.ts' doesn't resolve.
const tacHome = process.env.TAC_HOME || join(homedir(), '.tac')
const authFilePath = join(tacHome, 'agent', 'auth.json')
