/**
 * Remote Questions — /tac remote command
 */

import type { ExtensionAPI, ExtensionCommandContext } from "@tac/pi-coding-agent";
import { AuthStorage } from "@tac/pi-coding-agent";
import { Editor, type EditorTheme, Key, matchesKey, truncateToWidth } from "@tac/pi-tui";
import { existsSync, readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { getGlobalTACPreferencesPath, loadEffectiveTACPreferences } from "../tac/preferences.js";
...
async function handleSetupSlack(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const send = await fetchJson("https://slack.com/api/chat.postMessage", {
    ...
    body: JSON.stringify({ channel: channelId, text: "TAC remote questions connected." }),
  });
...
async function handleSetupDiscord(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const sendResponse = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages`, {
    ...
    body: JSON.stringify({ content: "TAC remote questions connected." }),
...
async function handleSetupTelegram(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const send = await fetchJson(`https://api.telegram.org/bot${token}/sendMessage`, {
    ...
    body: JSON.stringify({ chat_id: chatId, text: "TAC remote questions connected." }),
  });
...
function getAuthStorage(): AuthStorage {
  const authPath = join(process.env.HOME ?? "", ".tac", "agent", "auth.json");
