...
async function buildStatus(basePath: string): Promise<string> {
  ...
  try {
    const autoMod = await tryImportModule<AutoMod>("../tac/auto.js");
    if (autoMod) {
...
async function buildProgress(basePath: string): Promise<string> {
  ...
  if (milestones.length === 0) {
    return "No milestones found in .tac database.\n\nRun /tac to start TAC first.";
  }
  ...
  const lines: string[] = ["TAC Progress"];
...
async function buildBudget(basePath: string): Promise<string> {
  ...
  try {
    const metricsMod = await tryImportModule<MetricsMod>("../tac/metrics.js");
    if (metricsMod) {
...
async function buildPause(basePath: string): Promise<string> {
  try {
    const capturesMod = await import("../tac/captures.js");
...
// Dynamic import() is used for optional runtime modules (auto, metrics, tac-db)
 ...
 * available outside a full TAC process context.
 */
...
function tacRootPath(basePath: string): string {
  // Inline resolution: .tac lives directly under basePath.
  // Avoids importing paths.ts to keep this module's dependency surface small.
  return join(basePath, ".tac");
}

function readPausedSession(basePath: string): PausedSessionMeta | null {
  try {
    const p = join(tacRootPath(basePath), "runtime", "paused-session.json");
...
function readMetricsFromDisk(basePath: string): ProjectTotalsSnapshot | null {
  try {
    const p = join(tacRootPath(basePath), "metrics.json");
    if (!existsSync(p)) return null;
...
function resolveActivityDir(basePath: string): string {
  return join(tacRootPath(basePath), "activity");
}
