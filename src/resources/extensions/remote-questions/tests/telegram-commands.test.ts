 ...
 *   - /status returns current TAC state
...
function makeBasePath(): string {
  const dir = mkdtempSync(join(tmpdir(), "tac-cmd-test-"));
  // Create minimal .tac directory structure
  mkdirSync(join(dir, ".tac", "activity"), { recursive: true });
...
test("/status returns TAC state text (idle when no active session)", async (t) => {
  const dir = makeBasePath();
  const { sender, messages } = makeCapturingSender();
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  ...
  assert.ok(
    reply.toLowerCase().includes("state") || reply.toLowerCase().includes("tac"),
    `Expected status reply to mention state or TAC, got: ${reply}`,
  );
});
...
test("/status reads paused-session.json when present", async (t) => {
  ...
  writeFileSync(
    join(dir, ".tac", "runtime", "paused-session.json"),
...
test("/pause writes a stop capture to CAPTURES.md", async (t) => {
  ...
  const capturesPath = join(dir, ".tac", "CAPTURES.md");
