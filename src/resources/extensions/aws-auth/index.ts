 ...
 * ## Setup
 *
 * Add to ~/.tac/agent/settings.json (or project-level .tac/settings.json):
 *
 ...
 * Checks project-level first, then global (~/.tac/agent/settings.json).
 */
function getAwsAuthRefreshCommand(): string | undefined {
	const configDir = process.env.PI_CONFIG_DIR || ".tac";
	const paths = [
