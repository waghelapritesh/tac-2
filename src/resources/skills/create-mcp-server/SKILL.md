...
<context>
TAC-2 consumes MCP heavily — see `src/resources/extensions/mcp-client/`, `src/resources/extensions/tac/mcp-project-config.ts`, `src/resources/extensions/tac/workflow-mcp.ts`, and `/tac mcp` commands. Users frequently want to extend TAC with project-specific MCP servers (internal APIs, data sources, domain tools). This skill fills the authoring gap between "MCP exists" and "I have a working server."

Invocation points:
- User describes a service or API they want an LLM to reach
- `/tac mcp init` scaffolds config but there's a tool integration to build
...
## Step 6: Wire into TAC

Write the project's `.mcp.json` entry using `/tac mcp init` as a starting point. Document env vars and startup in README.md. If the server is globally useful, suggest the user file it as a durable skill via `spike-wrap-up` or publish it.
...
- [ ] Server is reachable from TAC via `.mcp.json` entry.
