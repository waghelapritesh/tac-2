...
```bash
# Test in isolation
tac -e ./path/to/extension.ts

# Check TAC startup output for errors
# Extension errors are logged but don't crash TAC
```
...
```typescript
// Temporary: log to stderr (visible in TAC output)
