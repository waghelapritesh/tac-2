...
```bash
gh <command> -R waghelapritesh/tac-2
```
...
```bash
# List open PRs
gh pr list -R waghelapritesh/tac-2

# View PR details
gh pr view <number> -R waghelapritesh/tac-2

# Check PR CI status
gh pr checks <number> -R waghelapritesh/tac-2

# Create PR
gh pr create -R waghelapritesh/tac-2 --title "title" --body "body"

# View PR comments
gh api repos/waghelapritesh/tac-2/pulls/<number>/comments
```
...
```bash
# List issues
gh issue list -R waghelapritesh/tac-2

# List by label
gh issue list -R waghelapritesh/tac-2 --label "priority:p1" --state open
...
gh issue create -R waghelapritesh/tac-2 \
  --title "feat: add feature X" \
  --label "priority:p1" \
  --milestone "v1.0"

# View issue
gh issue view <number> -R waghelapritesh/tac-2

# Close issue with comment
gh issue close <number> -R waghelapritesh/tac-2 --comment "Implemented in PR #N"

# Edit labels on issue
gh issue edit <number> -R waghelapritesh/tac-2 \
...
ISSUE_URL=$(gh issue create -R waghelapritesh/tac-2 \
  --title "..." --body "...")
...
```bash
# List all labels
gh label list -R waghelapritesh/tac-2

# Create label
gh label create "priority:p1" --color "E99695" \
  --description "High priority" -R waghelapritesh/tac-2
```
...
```bash
# List projects
gh project list --owner waghelapritesh

# Create project
gh project create --owner waghelapritesh --title "tac-2 Backlog"

# Add issue to project
gh project item-add 1 --owner waghelapritesh \
  --url https://github.com/waghelapritesh/tac-2/issues/42
```
...
```bash
# List milestones
gh api repos/waghelapritesh/tac-2/milestones
...
gh api repos/waghelapritesh/tac-2/issues/42 \
...
```bash
# List recent runs
gh run list -R waghelapritesh/tac-2 --limit 5

# View specific run
gh run view <run-id> -R waghelapritesh/tac-2

# View failed job logs
gh run view <run-id> -R waghelapritesh/tac-2 --log-failed
```
...
```bash
# List releases
gh release list -R waghelapritesh/tac-2

# View latest release
gh release view --repo waghelapritesh/tac-2
```
...
```bash
# GET request
gh api repos/waghelapritesh/tac-2

# POST with fields
gh api repos/waghelapritesh/tac-2/issues -f title="Bug" -f body="Details"

# GraphQL
gh api graphql -f query='{ viewer { login } }'

# Paginated results
gh api repos/waghelapritesh/tac-2/contributors --paginate
```
...
```bash
# Clone
gh repo clone waghelapritesh/tac-2

# View repo info
gh repo view -R waghelapritesh/tac-2
```
...
```bash
# JSON output
gh pr list -R waghelapritesh/tac-2 --json number,title,state

# JQ filtering
gh pr list -R waghelapritesh/tac-2 --json number,title --jq '.[].title'

# Template formatting
gh pr list -R waghelapritesh/tac-2 --json number,title \
