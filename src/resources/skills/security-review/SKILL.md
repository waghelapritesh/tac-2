...
<context>
TAC-2's general `review` skill covers security as one of several categories. This skill is the deeper pass — triggered deliberately when security is the primary concern. It complements v1's `/tac-secure-phase` concept, adapted to the tac-2 artifact model.
...
Offer to file as a GitHub issue — requires explicit confirmation per the outward-action rule. Sensitive findings should stay in `.tac/security-reviews/` and not be pushed to a public tracker; check the repository's security policy first.
...
- Flag the finding for `/tac start hotfix` if it's in the scope of active work.
- Append one line to `.tac/DECISIONS.md` if the remediation involves an architectural change.
