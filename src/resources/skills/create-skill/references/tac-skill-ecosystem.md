<overview>
TAC-specific skill ecosystem details: directory conventions, discovery mechanics, telemetry, and health monitoring. Read this reference when creating or auditing skills within TAC.
</overview>

<skill_directories>
TAC supports two skill directories, checked in order:

**User-scope (global):** `~/.agents/skills/`
- Available in every TAC session regardless of working directory
- Installed via [skills.sh](https://skills.sh) or manually

**Project-scope (local):** `.agents/skills/`
- Available only when TAC runs inside the project directory
...
<skill_discovery>
TAC auto-discovers skills at session start and during auto-mode:
...
Telemetry data is stored in `~/.tac/metrics.json` alongside other TAC metrics.
</skill_telemetry>
