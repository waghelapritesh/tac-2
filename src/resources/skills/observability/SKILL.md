...
<context>
TAC-2's `VISION.md` lists "agent-first observability" as a principle, and the system prompt calls it out: "A future version of you will land in this codebase with no memory… you add observability because you're the one who'll need it at 3am." TAC-2 already exemplifies this — `activity/*.jsonl`, `journal/*.jsonl`, `metrics.json`, `doctor-history.jsonl` — but new code doesn't get that treatment automatically.
...
Use the project's existing logger if one exists. In tac-2, follow the patterns in `src/resources/extensions/tac/activity-log.ts` and `src/resources/extensions/tac/journal.ts` — structured JSONL, one event per line, with `ts`, `event`, and domain-specific fields.
...
await writeAtomically(
  resolve(".tac/runtime/last-error.json"),
  JSON.stringify({
...
A fresh agent reading `.tac/runtime/` sees what happened last, what was retried, and where the process stopped. Pattern exists already in tac-2 — reuse the `atomic-write.ts` helpers and the `.tac/runtime/` and `.tac/forensics/` directories.
...
- **Digest view** — a small representation of recent work. In tac-2, this is `STATE.md` and the health widget. In a server, it's `/internal/status` with last 10 request summaries.
...
- [ ] Failure state is persisted to a known location (`.tac/runtime/`, `/var/log/`, a status file).
