...
<context>
TAC already organizes work into slices (S##) and tasks (T##). This skill operates at the task level — inside a single T##-PLAN.md, it structures the execution as a sequence of tiny red-green-refactor cycles rather than write-all-then-test or write-all-without-tests.
...
- Exploratory spikes (use `/tac start spike` — no production code ships)
...
2. Append a one-line summary of what is now pinned to `.tac/KNOWLEDGE.md` if the behavior is non-obvious or the test surfaced a trap future agents should know about.
3. Use `tac_*` tools to mark the task complete — do not edit checkboxes by hand.
