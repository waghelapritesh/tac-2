...
function readTacState(): TacState | undefined {
  try {
    const raw = readFileSync(join(process.cwd(), '.tac', 'STATE.md'), 'utf-8')
...
export function printWelcomeScreen(opts: WelcomeScreenOptions): void {
  ...
  if (termWidth < 70) {
    process.stderr.write(`\n  Think. Architect. Code. v${version}\n  ${shortCwd}\n\n`)
    return
  }
  ...
  const leftRows = ['', ...TAC_LOGO, '']

  // ── Right rows (8 total, null = divider) ────────────────────────────────────
  const titleLeft  = `  ${chalk.bold('Think. Architect. Code.')}`
  ...
  const hintRight  = chalk.dim('/tac to begin  ·  /tac help')
  ...
  // "Welcome back" context lines — TAC state if available, else hint.
  // Intentionally avoids data already shown in the footer (model, provider,
  // pwd, branch).
  const state = readTacState()
  let line1 = ''
  let line2 = ''
  if (state?.milestone) {
    ...
    line1 = `  Status     ${chalk.dim('No active TAC project')}`
    line2 = `             ${chalk.dim('/tac to begin')}`
  }
