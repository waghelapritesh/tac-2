...
/**
 * Detects whether the currently-running tac binary was installed via `bun add -g`.
 *
...
function printUpdateBanner(current: string, latest: string): void {
  const installCmd = resolveInstallCommand('tac-2')
  process.stderr.write(
    `  ${chalk.yellow('Update available:')} ${chalk.dim(`v${current}`)} → ${chalk.bold(`v${latest}`)}\n` +
    `  ${chalk.dim('Run')} ${installCmd} ${chalk.dim('or')} /tac update ${chalk.dim('to upgrade')}\n\n`,
  )
}
...
export async function checkForUpdates(options: UpdateCheckOptions = {}): Promise<void> {
  const currentVersion = options.currentVersion || process.env.TAC_VERSION || '0.0.0'
...
export async function checkAndPromptForUpdates(options: UpdateCheckOptions = {}): Promise<boolean> {
  ...
  if (choice === '1') {
    ...
    process.stderr.write(`  ${chalk.dim('Skipped. Run')} tac update ${chalk.dim('anytime to upgrade.')}\n\n`)
  }
