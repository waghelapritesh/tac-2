...
/**
 * Collects project inspection data by reading tac-db.json directly.
 * No child process needed — tac-db.json is plain JSON with no .js imports.
 */
export async function collectInspectData(projectCwdOverride?: string): Promise<InspectData> {
  ...
  const dbPath = join(tacDir, "tac-db.json")
