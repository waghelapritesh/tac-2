...
/**
 * Parse a project's `.tac/STATE.md` for active milestone, slice, phase,
...
export function readProjectProgress(projectPath: string): ProjectProgressInfo | null {
  try {
    const content = readFileSync(join(projectPath, ".tac", "STATE.md"), "utf-8");
...
export function discoverProjects(devRootPath: string, includeProgress?: boolean): ProjectMetadata[] {
  try {
    ...
    // .tac, or is a recognizable project), return it as a single entry.
