 ...
 *
 * Every web service that loads upstream TAC extension modules needs to spawn
...
export interface ModuleSpec {
  ...
  envKey: string
  /** Path relative to packageRoot (e.g. "src/resources/extensions/tac/doctor.ts"). */
  relativePath: string
}
...
export async function runSubprocess<T>(options: RunSubprocessOptions): Promise<T> {
  ...
  const tsLoaderPath =
    ...
    join(packageRoot, "src", "resources", "extensions", "tac", "tests", "resolve-ts.mjs")
