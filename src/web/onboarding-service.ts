 ...
 *
 * Mirrors the JSON written by `src/resources/extensions/tac/onboarding-state.ts`.
 ...
 * storing credentials in TAC. They are always treated as "configured" by the
 ...
 * in `src/resources/extensions/tac/doctor-providers.ts`. If a new ExternalCli
...
function resolveCredentialSource(
  ...
  // are stored in TAC. Treat them as always configured.
...
function resolveRuntimeTestIsExternalCliProvider(env: NodeJS.ProcessEnv): OnboardingServiceDeps["isExternalCliProvider"] | undefined {
  if (env.TAC_WEB_TEST_DISABLE_EXTERNAL_CLI !== "1") {
    return undefined;
  }
  return () => false;
}
...
function resolveRuntimeTestValidateApiKey(env: NodeJS.ProcessEnv): OnboardingServiceDeps["validateApiKey"] | undefined {
  if (env.TAC_WEB_TEST_FAKE_API_KEY_VALIDATION !== "1") {
    return undefined;
  }
  ...
  private async buildState(): Promise<OnboardingState> {
    ...
    try {
      const { readOnboardingRecord, isOnboardingComplete } = await import(
        "../resources/extensions/tac/onboarding-state.js"
      );
