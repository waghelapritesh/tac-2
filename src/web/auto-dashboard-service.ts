...
const TEST_AUTO_DASHBOARD_MODULE_ENV = "TAC_WEB_TEST_AUTO_DASHBOARD_MODULE";
const TEST_AUTO_DASHBOARD_FALLBACK_ENV = "TAC_WEB_TEST_USE_FALLBACK_AUTO_DASHBOARD";
const AUTO_DASHBOARD_MODULE_ENV = "TAC_AUTO_DASHBOARD_MODULE";
 ...
 *   1. .tac/auto.lock — if present and its PID is alive, auto IS running.
 *   2. .tac/runtime/paused-session.json — if present, auto IS paused.
 *
 * See #2705.
 */
function reconcileWithDiskState(
  ...
  const pausedPath = join(projectCwd, ".tac", "runtime", "paused-session.json");
  ...
  const lockPath = join(projectCwd, ".tac", "auto.lock");
...
export async function collectAuthoritativeAutoDashboardData(
  ...
  const moduleResolution = testModulePath
    ...
    : resolveSubprocessModule(packageRoot, "resources/extensions/tac/auto.ts", checkExists);
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        ...
        try {
          ...
          const projectCwd = env.TAC_WEB_PROJECT_CWD || "";
