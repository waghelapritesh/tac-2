...
function buildCommandSuggestions(
  ...
  if (phase === "planning") add("/tac", "Open TAC planning")
  if (phase === "executing" || phase === "summarizing") add("/tac auto", "Resume TAC auto mode")
  if (activeScope) add(`/tac doctor ${activeScope}`, "Inspect scoped doctor report")
  if (activeScope) add(`/tac doctor fix ${activeScope}`, "Apply scoped doctor fixes")
  if (validationCount > 0 && activeScope) add(`/tac doctor audit ${activeScope}`, "Audit validation diagnostics")
  add("/tac status", "Check current-project status")
...
async function collectRecoveryDiagnosticsChildPayload(
  ...
  const doctorResolution = resolveSubprocessModule(packageRoot, "resources/extensions/tac/doctor.ts", checkExists)
  const forensicsResolution = resolveSubprocessModule(packageRoot, "resources/extensions/tac/session-forensics.ts", checkExists)
  ...
  const script = [
    ...
    'const doctor = await import(pathToFileURL(process.env.TAC_RECOVERY_DOCTOR_MODULE).href);',
    'const forensics = await import(pathToFileURL(process.env.TAC_RECOVERY_FORENSICS_MODULE).href);',
    'const basePath = process.env.TAC_RECOVERY_BASE;',
    'const scope = process.env.TAC_RECOVERY_SCOPE || undefined;',
    'const unitType = process.env.TAC_RECOVERY_UNIT_TYPE || "execute-project";',
    'const unitId = process.env.TAC_RECOVERY_UNIT_ID || "project";',
    'const sessionFile = process.env.TAC_RECOVERY_SESSION_FILE || undefined;',
    'const activityDir = process.env.TAC_RECOVERY_ACTIVITY_DIR || undefined;',
    'const report = await doctor.runTACDoctor(basePath, { fix: false, scope, fixLevel: "task" });',
  ...
  return await new Promise<RecoveryDiagnosticsChildPayload>((resolveResult, reject) => {
    execFile(
      ...
      ],
      {
        ...
        env: {
          ...env,
          TAC_RECOVERY_BASE: basePath,
          TAC_RECOVERY_SCOPE: scope ?? "",
          TAC_RECOVERY_UNIT_TYPE: unit?.type ?? "execute-project",
          TAC_RECOVERY_UNIT_ID: unit?.id ?? "project",
          TAC_RECOVERY_SESSION_FILE: sessionFile ?? "",
          TAC_RECOVERY_ACTIVITY_DIR: join(basePath, ".tac", "activity"),
          TAC_RECOVERY_DOCTOR_MODULE: doctorModulePath,
          TAC_RECOVERY_FORENSICS_MODULE: sessionForensicsModulePath,
        },
