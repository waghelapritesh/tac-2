...
import type {
  WorkspaceIndex as TACWorkspaceIndex,
  WorkspaceMilestoneTarget as TACWorkspaceMilestoneTarget,
  WorkspaceScopeTarget as TACWorkspaceScopeTarget,
  WorkspaceSliceTarget as TACWorkspaceSliceTarget,
  WorkspaceTaskTarget as TACWorkspaceTaskTarget,
...
import { resolveTacCliEntry } from "./cli-entry.ts";
...
export type {
  TACWorkspaceTaskTarget,
  TACWorkspaceSliceTarget,
  TACWorkspaceMilestoneTarget,
  TACWorkspaceScopeTarget,
  TACWorkspaceIndex,
};
...
export type ProjectDetectionKind =
  | "active-tac"    // .tac with milestones — normal operation
  | "empty-tac"     // .tac exists but no milestones (freshly bootstrapped)
  | "v1-legacy"     // .planning/ exists, no .tac
  | "brownfield"    // existing code (git, package.json, files) but no .tac
  | "blank";        // empty/near-empty folder

export interface ProjectDetectionSignals {
  hasTacFolder: boolean;
...
export function detectProjectKind(projectCwd: string): ProjectDetection {
  ...
  const hasTacFolder = checkExists(join(projectCwd, ".tac"));
  ...
  if (hasTacFolder) {
    ...
    const milestonesDir = join(projectCwd, ".tac", "milestones");
    ...
    kind = hasMilestones ? "active-tac" : "empty-tac";
...
interface BridgeServiceDeps {
  ...
  indexWorkspace?: (basePath: string) => Promise<TACWorkspaceIndex>;
...
type WorkspaceIndexCacheEntry = {
  value: TACWorkspaceIndex | null;
  expiresAt: number;
  promise: Promise<TACWorkspaceIndex> | null;
};
...
async function loadSessionBrowserSessionsViaChildProcess(config: BridgeRuntimeConfig): Promise<SessionInfo[]> {
  ...
  const script = [
    ...
    'const mod = await import(pathToFileURL(process.env.TAC_SESSION_MANAGER_MODULE).href);',
    'const sessions = await mod.SessionManager.list(process.env.TAC_SESSION_BROWSER_CWD, process.env.TAC_SESSION_BROWSER_DIR);',
    ...
    execFile(
      ...
      ["--input-type=module", "--eval", script],
      {
        ...
        env: {
          ...
          TAC_SESSION_MANAGER_MODULE: sessionManagerModulePath,
          TAC_SESSION_BROWSER_CWD: config.projectCwd,
          TAC_SESSION_BROWSER_DIR: config.projectSessionsDir,
        },
        maxBuffer: 1024 * 1024,
        windowsHide: true,
      },
...
async function appendSessionInfoViaChildProcess(
  ...
  const script = [
    'const { pathToFileURL } = await import("node:url");',
    'const mod = await import(pathToFileURL(process.env.TAC_SESSION_MANAGER_MODULE).href);',
    'const manager = mod.SessionManager.open(process.env.TAC_TARGET_SESSION_PATH, process.env.TAC_SESSION_BROWSER_DIR);',
    'manager.appendSessionInfo(process.env.TAC_TARGET_SESSION_NAME);',
  ].join(" ");
    ...
    execFile(
      ...
      ["--input-type=module", "--eval", script],
      {
        ...
        env: {
          ...
          TAC_SESSION_MANAGER_MODULE: sessionManagerModulePath,
          TAC_SESSION_BROWSER_DIR: config.projectSessionsDir,
          TAC_TARGET_SESSION_PATH: sessionPath,
          TAC_TARGET_SESSION_NAME: name,
        },
        maxBuffer: 1024 * 1024,
        windowsHide: true,
      },
...
function cloneWorkspaceIndex(index: TACWorkspaceIndex): TACWorkspaceIndex {
...
async function loadCachedWorkspaceIndex(
  basePath: string,
  loader: () => Promise<TACWorkspaceIndex>,
): Promise<TACWorkspaceIndex> {
...
async function loadWorkspaceIndexViaChildProcess(basePath: string, packageRoot: string): Promise<TACWorkspaceIndex> {
  const deps = getBridgeDeps();
  const checkExists = deps.existsSync ?? existsSync;
  const resolveTsLoader = join(packageRoot, "src", "resources", "extensions", "tac", "tests", "resolve-ts.mjs");
  ...
  const script = [
    'const { pathToFileURL } = await import("node:url");',
    'const mod = await import(pathToFileURL(process.env.TAC_WORKSPACE_MODULE).href);',
    'const result = await mod.indexWorkspace(process.env.TAC_WORKSPACE_BASE);',
  ...
  return await new Promise<TACWorkspaceIndex>((resolveResult, reject) => {
    execFile(
      deps.execPath ?? process.execPath,
      [
        ...prefixArgs,
      ...
      ],
      {
        ...
        env: {
          ...(deps.env ?? process.env),
          TAC_WORKSPACE_MODULE: workspaceModulePath,
          TAC_WORKSPACE_BASE: basePath,
        },
        maxBuffer: 1024 * 1024,
        windowsHide: true,
      },
      (error, stdout, stderr) => {
        ...
        try {
          resolveResult(JSON.parse(stdout) as TACWorkspaceIndex);
        } catch (parseError) {
...
async function fallbackWorkspaceIndex(basePath: string): Promise<TACWorkspaceIndex> {
...
export function resolveBridgeRuntimeConfig(env: NodeJS.ProcessEnv = getBridgeDeps().env ?? process.env, projectCwdOverride?: string): BridgeRuntimeConfig {
  const projectCwd = projectCwdOverride || env.TAC_WEB_PROJECT_CWD || process.cwd();
  const projectSessionsDir = env.TAC_WEB_PROJECT_SESSIONS_DIR || getProjectSessionsDir(projectCwd);
  const packageRoot = env.TAC_WEB_PACKAGE_ROOT || getDefaultPackageRoot();
...
function resolveBridgeCliEntry(config: BridgeRuntimeConfig, deps: BridgeServiceDeps): BridgeCliEntry {
  return resolveTacCliEntry({
    packageRoot: config.packageRoot,
    cwd: config.projectCwd,
    execPath: deps.execPath ?? process.execPath,
    hostKind: (deps.env ?? process.env).TAC_WEB_HOST_KIND,
    mode: "rpc",
...
export class BridgeService {
  ...
  private async startInternal(): Promise<void> {
    ...
    delete childEnv.TAC_CODING_AGENT_DIR;
    childEnv.TAC_WEB_BRIDGE_TUI = "1";
...
export function resolveProjectCwd(request: Request): string | null {
  ...
  return (getBridgeDeps().env ?? process.env).TAC_WEB_PROJECT_CWD || null;
}
...
export async function collectBootPayload(projectCwd?: string): Promise<BridgeBootPayload> {
  ...
  if (onboarding.locked && env.TAC_WEB_HOST_KIND === "packaged-standalone") {
    return {
      project: {
        cwd: config.projectCwd,
        sessionsDir: config.projectSessionsDir,
