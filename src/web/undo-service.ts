...
const UNDO_MODULE_ENV = "TAC_UNDO_MODULE"
const PATHS_MODULE_ENV = "TAC_PATHS_MODULE"
...
export async function collectUndoInfo(projectCwdOverride?: string): Promise<UndoInfo> {
  ...
  const tacDir = join(projectCwd, ".tac")
  const completedPath = join(tacDir, "completed-units.json")
  ...
  const activityDir = join(tacDir, "activity")
  let commits: string[] = []
...
export async function executeUndo(projectCwdOverride?: string): Promise<UndoResult> {
  ...
  const undoResolution = resolveSubprocessModule(packageRoot, "resources/extensions/tac/undo.ts")
  const pathsResolution = resolveSubprocessModule(packageRoot, "resources/extensions/tac/paths.ts")
  ...
  const script = [
    ...
    'const basePath = process.env.TAC_UNDO_BASE;',
    'const tacDir = pathsMod.tacRoot(basePath);',
    'const completedPath = join(tacDir, "completed-units.json");',
