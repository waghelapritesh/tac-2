...
import { type SettingsManager, setAllowedCommandPrefixes } from '@tac/pi-coding-agent'
...
export function applySecurityOverrides(settingsManager: SettingsManager): void {
  ...
  const envPrefixes = process.env.TAC_ALLOWED_COMMAND_PREFIXES
  if (envPrefixes) {
  ...
  const envUrls = process.env.TAC_FETCH_ALLOWED_URLS
  if (envUrls) {
