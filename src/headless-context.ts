 ...
 * and bootstraps the .tac/ directory structure when needed.
 */
...
/**
 * Bootstrap .tac/ directory structure for headless new-milestone.
 * Mirrors the bootstrap logic from guided-flow.ts showSmartEntry().
 */
export function bootstrapTacProject(basePath: string): void {
  const tacDir = join(basePath, '.tac')
  mkdirSync(join(tacDir, 'milestones'), { recursive: true })
  mkdirSync(join(tacDir, 'runtime'), { recursive: true })
}
