...
/**
 * Minimal tool interface matching TAC's AgentTool shape.
 * Avoids a direct dependency on @tac/pi-agent-core from this compiled module.
 *
 ...
 *
 * This enables TAC's tools (read, write, edit, bash, grep, glob, ls, etc.)
 ...
 * maps MCP tools/list and tools/call requests to TAC tool definitions and
...
export async function startMcpServer(options: {
  ...
  // tools/list — return every registered TAC tool with its JSON Schema parameters
  ...
  process.stderr.write(`[tac] MCP server started (v${version})\n`)
}
