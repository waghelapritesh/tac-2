/**
 * TAC Worktree CLI — standalone subcommand and -w flag handling.
 *
 * Manages the full worktree lifecycle from the command line:
 *   tac -w                    Create auto-named worktree, start interactive session
 *   tac -w my-feature         Create/resume named worktree
 *   tac worktree list         List worktrees with status
 *   tac worktree merge [name] Squash-merge a worktree into main
 *   tac worktree clean        Remove all merged/empty worktrees
 *   tac worktree remove <n>   Remove a specific worktree
 *
 * On session exit (via session_shutdown event), auto-commits dirty work
 * so nothing is lost. The TAC extension reads TAC_CLI_WORKTREE to know
 ...
 * from resources/extensions/tac/ which are shipped as raw .ts (#1283).
 */
...
function logDebugFailure(scope: string, error: unknown): void {
  if (process.env.TAC_DEBUG === '1') {
    process.stderr.write(chalk.dim(`[tac] ${scope} failed: ${toErrorMessage(error)}\n`))
  }
}
...
async function loadExtensionModules(): Promise<ExtensionModules> {
  ...
  const [wtMgr, autoWt, gitBridge, gitSvc, wt] = await Promise.all([
    jiti.import(tacExtensionPath('worktree-manager.ts'), {}) as Promise<WorktreeManagerModule>,
    jiti.import(tacExtensionPath('auto-worktree.ts'), {}) as Promise<AutoWorktreeModule>,
    jiti.import(tacExtensionPath('native-git-bridge.ts'), {}) as Promise<NativeGitBridgeModule>,
    jiti.import(tacExtensionPath('git-service.ts'), {}) as Promise<GitServiceModule>,
    jiti.import(tacExtensionPath('worktree.ts'), {}) as Promise<WorktreeModule>,
  ])
  _ext = {
...
async function handleList(basePath: string): Promise<void> {
  ...
  if (worktrees.length === 0) {
    process.stderr.write(chalk.dim('No worktrees. Create one with: tac -w <name>\n'))
    return
  }
...
async function handleMerge(basePath: string, args: string[]): Promise<void> {
  ...
  if (!name) {
    ...
    process.stderr.write(chalk.red('Usage: tac worktree merge <name>\n'))
    process.stderr.write(chalk.dim('Run tac worktree list to see worktrees.\n'))
    process.exit(1)
  }
...
async function doMerge(ext: ExtensionModules, basePath: string, name: string): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(chalk.dim('  Resolve conflicts manually, then run tac worktree merge again.\n'))
    process.exit(1)
  }
}
...
async function handleRemove(basePath: string, args: string[]): Promise<void> {
  ...
  const name = args[0]
  if (!name) {
    process.stderr.write(chalk.red('Usage: tac worktree remove <name>\n'))
    process.exit(1)
  }
  ...
  if (status.filesChanged > 0 || status.uncommitted) {
    ...
    process.stderr.write(chalk.yellow('  Use --force to remove anyway, or merge first: tac worktree merge ' + name + '\n'))
...
async function handleStatusBanner(basePath: string): Promise<void> {
  ...
  process.stderr.write(
    ...
    names + '\n' +
    chalk.dim('[tac] ') +
    chalk.dim('Resume: tac -w <name>  |  Merge: tac worktree merge <name>  |  List: tac worktree list\n\n'),
  )
}
...
async function handleWorktreeFlag(worktreeFlag: boolean | string): Promise<void> {
  ...
  // tac -w (no name) — resume most recent worktree with changes, or create new
  if (worktreeFlag === true) {
    ...
    if (withChanges.length === 1) {
      ...
      process.env.TAC_CLI_WORKTREE = wt.name
      process.env.TAC_CLI_WORKTREE_BASE = basePath
    ...
    if (withChanges.length > 1) {
      ...
      process.stderr.write(chalk.dim('Specify which one: tac -w <name>\n'))
      process.exit(0)
    }
  ...
  // tac -w <name> — create or resume named worktree
  ...
  if (found) {
    ...
    process.env.TAC_CLI_WORKTREE = name
    process.env.TAC_CLI_WORKTREE_BASE = basePath
...
async function createAndEnter(ext: ExtensionModules, basePath: string, name: string): Promise<void> {
  try {
    ...
    if (hookError) {
      process.stderr.write(chalk.yellow(`[tac] ${hookError}\n`))
    }
    ...
    const msg = err instanceof Error ? err.message : String(err)
    process.stderr.write(chalk.red(`[tac] Failed to create worktree: ${msg}\n`))
    process.exit(1)
  }
}
