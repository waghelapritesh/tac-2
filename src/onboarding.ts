...
import type { AuthStorage } from '@tac/pi-coding-agent'
...
} from './resources/extensions/tac/onboarding-state.js'
import { getLlmProviderIds } from './resources/extensions/tac/setup-catalog.js'
...
async function loadClack(): Promise<ClackModule> {
  try {
    ...
    throw new Error('[tac] @clack/prompts not found — onboarding wizard requires this dependency')
  }
}
...
export async function runOnboarding(
  ...
  try {
    ...
    process.stderr.write(`[tac] Onboarding wizard unavailable: ${err instanceof Error ? err.message : String(err)}\n`)
    return
  }
  ...
  if (opts.showIntro !== false) {
    ...
    p.intro(pc.bold('Welcome to TAC — let\'s get you set up'))
  }
  ...
  const llmResult = await runStep(p, 'LLM setup failed', () => runLlmStep(p, pc, authStorage), {
    cancelMessage: 'Setup cancelled — you can run /tac onboarding --resume later.',
    errorInfo: 'You can configure your LLM provider later with /login inside TAC.',
  })
  ...
  if (llmConfigured) {
    ...
    summaryLines.push(`${pc.yellow('↷')} LLM provider: skipped — use /login inside TAC`)
  }

  if (searchConfigured) {
    summaryLines.push(`${pc.green('✓')} Web search: ${searchConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Web search: not configured — use /search-provider inside TAC`)
  }

  if (remoteConfigured) {
    summaryLines.push(`${pc.green('✓')} Remote questions: ${remoteConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Remote questions: not configured — use /tac remote inside TAC`)
  }
  ...
  summaryLines.push('')
  summaryLines.push(`${pc.dim('Tip:')} re-run anytime with ${pc.cyan('/tac onboarding')}`)

  p.note(summaryLines.join('\n'), 'Setup complete')
  p.outro(pc.dim('Launching TAC...'))
}
...
export async function runLlmStep(p: ClackModule, pc: PicoModule, authStorage: AuthStorage): Promise<boolean> {
  ...
  authOptions.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /login inside TAC later' },
  )
...
async function runApiKeyFlow(
  ...
  if (providerId === 'openrouter') {
    p.log.info(`Use ${pc.cyan('/model')} inside TAC to pick an OpenRouter model.`)
...
export async function runWebSearchStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /search-provider inside TAC later' },
  )
...
export async function runRemoteQuestionsStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /tac remote inside TAC later' },
  )

  const choice = await p.select({
    message: 'Set up remote questions? (get notified when TAC needs input)',
    options,
  })
  ...
  if (choice === 'telegram') {
    ...
    try {
      const res = await fetch(`https://api.telegram.org/bot${trimmed}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: trimmedChatId, text: 'TAC remote questions connected.' }),
        signal: AbortSignal.timeout(15_000),
      })
      const data = await res.json() as TelegramGetMeResponse
      if (!data?.ok) {
...
async function runDiscordChannelStep(p: ClackModule, pc: PicoModule, token: string): Promise<string | null> {
  ...
  try {
    ...
    p.log.warn('Could not fetch Discord servers — configure channel later with /tac remote discord')
    return null
  }

  if (guilds.length === 0) {
    p.log.warn('Bot is not in any Discord servers — configure channel later with /tac remote discord')
    return null
  }
  ...
  try {
    ...
    p.log.warn('Could not fetch channels — configure later with /tac remote discord')
    return null
  }

  if (channels.length === 0) {
    p.log.warn('No text channels found — configure later with /tac remote discord')
    return null
  }
  ...
  const channelChoice = await p.select({
    message: 'Which channel should TAC use for remote questions?',
    options: [
