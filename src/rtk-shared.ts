...
export const TAC_RTK_DISABLED_ENV = "TAC_RTK_DISABLED";
export const TAC_RTK_PATH_ENV = "TAC_RTK_PATH";
...
export function isRtkEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return !isTruthy(env[TAC_RTK_DISABLED_ENV]);
}

export function getManagedRtkDir(env: NodeJS.ProcessEnv = process.env): string {
  return join(env.TAC_HOME || join(osHomedir(), ".tac"), "agent", "bin");
}
