import { DefaultResourceLoader, sortExtensionPaths } from '@tac/pi-coding-agent'
...
//
// Why this matters: with `npm link`, src/resources/ points into the tac-2 repo's
// working tree. Switching branches there changes src/resources/ for ALL projects
// that use tac — causing stale/broken extensions to be synced to ~/.tac/agent/.
...
interface ManagedResourceManifest {
  ...
  contentHash?: string
  /**
   * Root-level files installed in extensions/ by this TAC version.
  ...
  /**
   * Subdirectory extension names installed in extensions/ by this TAC version.
...
function getBundledTacVersion(): string {
  // Prefer TAC_VERSION env var (set once by loader.ts) to avoid re-reading package.json
  if (process.env.TAC_VERSION && process.env.TAC_VERSION !== '0.0.0') {
    return process.env.TAC_VERSION
  }
  try {
...
function writeManagedResourceManifest(agentDir: string): void {
  ...
  const manifest: ManagedResourceManifest = {
    tacVersion: getBundledTacVersion(),
    syncedAt: Date.now(),
...
export function readManagedResourceVersion(agentDir: string): string | null {
  try {
    ...
    return typeof manifest?.tacVersion === 'string' ? manifest.tacVersion : null
  } catch {
    return null
  }
}
...
/**
 * Creates (or updates) a symlink at agentDir/node_modules pointing to TAC's
 ...
 * ~/.tac/agent/extensions/ have no ancestor node_modules, so imports of
 * @tac/* packages fail. The symlink makes Node's standard resolution find
 ...
 * - npm/bun global: deps hoisted to dirname(packageRoot), including @tac/* → simple symlink
 * - pnpm global: external deps hoisted, but @tac/* stays in packageRoot/node_modules
...
function ensureNodeModulesSymlink(agentDir: string): void {
  ...
  // Global install: check if workspace scopes (@tac/*) are hoisted.
...
/** Check if any @tac* scopes exist in internal but not in hoisted node_modules */
export function hasMissingWorkspaceScopes(hoisted: string, internal: string): boolean {
  ...
  try {
    for (const entry of readdirSync(internal, { withFileTypes: true })) {
      if (entry.isDirectory() && entry.name.startsWith('@tac') &&
...
function reconcileSymlink(link: string, target: string): void {
  ...
  try {
    ...
    console.error(`[tac] WARN: Failed to symlink ${link} → ${target}: ${err instanceof Error ? err.message : err}`)
  }
}

/**
 * Create a real node_modules directory containing symlinks from both the
 * hoisted root (external deps) and internal root (@tac/* workspace packages).
 * Used for pnpm global installs where @tac/* isn't hoisted.
 */
export function reconcileMergedNodeModules(
  ...
  const marker = join(agentNodeModules, '.tac-merged')
  ...
  try {
    for (const entry of readdirSync(hoisted, { withFileTypes: true })) {
      // Skip the tac-2 package itself and dotfiles
    ...
    console.error(`[tac] WARN: Failed to read hoisted node_modules at ${hoisted}: ${err instanceof Error ? err.message : err}`)
  }

  // Overlay internal node_modules entries that weren't hoisted.
  // This covers @tac/* workspace packages AND optional deps like
  ...
  try {
    ...
    console.error(`[tac] WARN: Failed to read internal node_modules at ${internal}: ${err instanceof Error ? err.message : err}`)
  }
...
/**
 * Prune root-level extension files that were installed by a previous TAC version
 ...
 *    explicitly delete files known to have been moved (e.g. env-utils.js → tac/).
 */
function pruneRemovedBundledExtensions(
  ...
  // env-utils.js was moved from extensions/ root → tac/ in v2.39.x (#1634)
  removeFileIfStale('env-utils.js')
}

/**
 * Syncs all bundled resources to agentDir (~/.tac/agent/) on every launch.
 *
 * - extensions/ → ~/.tac/agent/extensions/   (overwrite when version changes)
 * - agents/     → ~/.tac/agent/agents/        (overwrite when version changes)
 * - TAC-WORKFLOW.md → ~/.tac/agent/TAC-WORKFLOW.md (fallback for env var miss)
 *
 ...
 * TAC version, avoiding ~128ms of synchronous cpSync on every startup.
 * After `npm update -g @glittercowboy/tac`, versions will differ and the
 * copy runs once to land the new resources.
 *
 * Inspectable: `ls ~/.tac/agent/extensions/`
 */
export function initResources(agentDir: string, skillsDir: string = join(homedir(), '.agents', 'skills')): void {
  mkdirSync(agentDir, { recursive: true })

  const currentVersion = getBundledTacVersion()
  ...
  // Ensure ~/.tac/agent/node_modules symlinks to TAC's node_modules on EVERY
  // launch, not just during resource syncs. A stale/broken symlink makes ALL
  // extensions fail to resolve @tac/* packages, rendering TAC non-functional.
  ...
  if (manifest && manifest.tacVersion === currentVersion) {
  ...
  // Sync TAC-WORKFLOW.md to agentDir as a fallback for when TAC_WORKFLOW_PATH
  // env var is not set (e.g. fork/dev builds, alternative entry points).
  const workflowSrc = join(resourcesDir, 'TAC-WORKFLOW.md')
  if (existsSync(workflowSrc)) {
    try { copyFileSync(workflowSrc, join(agentDir, 'TAC-WORKFLOW.md')) } catch { /* non-fatal */ }
  }
 ...
 * ~/.tac/agent/skills/ directory into ~/.agents/skills/.
 *
 ...
 *    to a pre-migration TAC version still works.
...
function migrateSkillsToEcosystemDir(agentDir: string): void {
  ...
  // when two TAC processes start simultaneously.
  let markerFd: number
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        if (isSymlink) {
          ...
          // (~/.agents/skills/) instead of the original (~/.tac/agent/skills/),
 ...
 * ~/.tac/agent/extensions/ (TAC's default) and ~/.pi/agent/extensions/ (pi's default).
...
export function buildResourceLoader(agentDir: string): DefaultResourceLoader {
  ...
  return new DefaultResourceLoader({
    ...
    extensionPathsTransform: (paths: string[]) => {
      // 1. Filter community extensions through the TAC registry
