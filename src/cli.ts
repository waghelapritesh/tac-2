...
import { bootstrapRtk, TAC_RTK_DISABLED_ENV } from './rtk.js'
import { loadEffectiveTACPreferences } from './resources/extensions/tac/preferences.js'
...
function exitIfManagedResourcesAreNewer(currentAgentDir: string): void {
  const currentVersion = process.env.TAC_VERSION || '0.0.0'
  ...
  process.stderr.write(
    `[tac] ${chalk.yellow('Version mismatch detected')}\n` +
    `[tac] Synced resources are from ${chalk.bold(`v${managedVersion}`)}, but this \`tac\` binary is ${chalk.dim(`v${currentVersion}`)}.\n` +
    `[tac] Run ${chalk.bold('npm install -g tac-2@latest')} or ${chalk.bold('tac update')}, then try again.\n`,
  )
  process.exit(1)
}
...
function printNonTtyErrorAndExit(missing: string | undefined, includeWebHint: boolean): never {
  ...
  process.stderr.write(`[tac] Error: Interactive mode requires a terminal (TTY)${suffix}.\n`)
  process.stderr.write('[tac] Non-interactive alternatives:\n')
  process.stderr.write('[tac]   tac auto                       Auto-mode (pipeable, no TUI)\n')
  process.stderr.write('[tac]   tac --print "your message"     Single-shot prompt\n')
  if (includeWebHint) {
    process.stderr.write('[tac]   tac --web [path]               Browser-only web mode\n')
  }
  process.stderr.write('[tac]   tac --mode rpc                 JSON-RPC over stdin/stdout\n')
  process.stderr.write('[tac]   tac --mode mcp                 MCP server over stdin/stdout\n')
  process.stderr.write('[tac]   tac --mode text "message"      Text output mode\n')
  if (includeWebHint) {
    process.stderr.write('[tac]   tac headless                   Auto-mode without TUI\n')
  }
  process.exit(1)
}
...
function printExtensionErrors(errors: ReadonlyArray<{ error: string }>): void {
  for (const err of errors) {
    ...
    process.stderr.write(`[tac] ${prefix}: ${err.error}\n`)
  }
}
...
function printExtensionWarnings(warnings: ReadonlyArray<{ message: string }> | undefined): void {
  ...
  for (const w of warnings) {
    process.stderr.write(`[tac] Extension warning: ${w.message}\n`)
  }
}
...
// `tac [subcommand] --help` / `-h` — print help before any subcommand runs.
// loader.ts only catches --help/-h as the *first* arg; here we handle the
// case where it appears later (e.g. `tac update --help`, `tac --foo --help`).
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  ...
  const version = process.env.TAC_VERSION || '0.0.0'
...
async function doRtkBootstrap(): Promise<void> {
  ...
  if (!rtkStatus.available && rtkStatus.supported && rtkStatus.enabled && rtkStatus.reason) {
    process.stderr.write(`[tac] Warning: RTK unavailable — continuing without shell-command compression (${rtkStatus.reason}).\n`)
  }
}
...
// `tac update` — update to the latest version via npm
...
// ---------------------------------------------------------------------------
// Graph subcommand — `tac graph build|status|query|diff`
// ---------------------------------------------------------------------------
if (cliFlags.messages[0] === 'graph') {
  const sub = cliFlags.messages[1]
  const { buildGraph, writeGraph, graphStatus, graphQuery, graphDiff, resolveTacRoot } = await import('@waghelapritesh/mcp-server')

  const projectDir = process.cwd()
  const tacRoot = resolveTacRoot(projectDir)

  if (!sub || sub === 'build') {
    try {
      const graph = await buildGraph(projectDir)
      await writeGraph(tacRoot, graph)
      process.stdout.write(`Graph built: ${graph.nodes.length} nodes, ${graph.edges.length} edges\n`)
    } catch (err) {
      process.stderr.write(`[tac] graph build failed: ${err instanceof Error ? err.message : String(err)}\n`)
      process.exit(1)
    }
  } else if (sub === 'status') {
    try {
      ...
      if (!result.exists) {
        process.stdout.write('Graph: not built yet. Run: tac graph build\n')
      } else {
      ...
      process.stderr.write(`[tac] graph status failed: ${err instanceof Error ? err.message : String(err)}\n`)
      process.exit(1)
    }
  } else if (sub === 'query') {
    const term = cliFlags.messages[2]
    if (!term) {
      process.stderr.write('Usage: tac graph query <term>\n')
      process.exit(1)
    }
    try {
      ...
      process.stderr.write(`[tac] graph query failed: ${err instanceof Error ? err.message : String(err)}\n`)
      process.exit(1)
    }
  } else if (sub === 'diff') {
    try {
      ...
      process.stderr.write(`[tac] graph diff failed: ${err instanceof Error ? err.message : String(err)}\n`)
      process.exit(1)
    }
  } else {
...
// `tac config` — replay the setup wizard and exit
...
// `tac web stop [path|all]` — stop web server before anything else
...
// `tac --web [path]` or `tac web [start] [path]` — launch browser-only web mode
...
// `tac sessions` — list past sessions and pick one to resume
...
// `tac headless` — run auto-mode without TUI
if (cliFlags.messages[0] === 'headless') {
  ...
  // from ~/.tac/agent/extensions/ — different extension copies diverge.
...
// `tac auto [args...]` — shorthand for `tac headless auto [args...]` (#2732)
// Without this, `tac auto` falls through to the interactive TUI which hangs
...
if (!isPrintMode && process.stdout.columns && process.stdout.columns < 40) {
  process.stderr.write(
    chalk.yellow(`[tac] Terminal width is ${process.stdout.columns} columns (minimum recommended: 40). Output may be unreadable.\n`),
  )
}
...
// TAC always uses quiet startup — the tac extension renders its own branded header
...
if (isPrintMode) {
  ...
  if (mode === 'mcp') {
    ...
    // tools (tac workflow, browser-tools, mac-tools, search-the-web, …)
    ...
    await startMcpServer({
      ...
      version: process.env.TAC_VERSION || '0.0.0',
    })
...
// ---------------------------------------------------------------------------
// Worktree subcommand — `tac worktree <list|merge|clean|remove>`
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Auto-redirect: `tac auto` with piped stdout → headless mode (#2732)
// When stdout is not a TTY (e.g. `tac auto | cat`, `tac auto > file`),
...
if (cliFlags.messages[0] === 'auto' && !process.stdout.isTTY) {
  process.stderr.write('[tac] stdout is not a terminal — running auto-mode in headless mode.\n')
...
// files lived directly in ~/.tac/sessions/. Move them into the correct per-cwd
...
if (!process.env.TAC_FIRST_RUN_BANNER) {
