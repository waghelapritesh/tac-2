...
If you use cmux (terminal multiplexer), TAC can integrate with it:

| Command | Description |
|---------|-------------|
| `/tac cmux status` | Show cmux detection and capabilities |
| `/tac cmux on` / `off` | Enable/disable integration |
| `/tac cmux notifications on/off` | Toggle desktop notifications |
| `/tac cmux sidebar on/off` | Toggle sidebar metadata |
| `/tac cmux splits on/off` | Toggle visual subagent splits |
