...
2. Set it:
   ...
   ```
3. In TAC, type `/model` to select an OpenRouter model (prefixed with `openrouter/`)

To add models not in the built-in list, add them to `~/.tac/agent/models.json`. See [Custom Models](custom-models.md).
...
Local providers run on your machine. They require a `models.json` configuration file at `~/.tac/agent/models.json` because TAC needs to know the endpoint URL and available models.
...
3. Create `~/.tac/agent/models.json`:
   ```json
   {
     "providers": {
       "ollama": {
...
4. In TAC, type `/model` and select your Ollama model.
...
Any server that implements the OpenAI Chat Completions API can work with TAC — proxies (LiteLLM, Portkey, Helicone), self-hosted inference, new providers.
...
This writes `~/.tac/agent/models.json` for you. See [Custom Models](custom-models.md) for manual setup.
...
| Problem | Cause | Fix |
|---------|-------|-----|
| "Authentication failed" with valid key | Key not visible to TAC | Export in the same terminal, or save via `tac config` |
