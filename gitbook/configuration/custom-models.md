...
Define custom models and providers in `~/.tac/agent/models.json`. This lets you add models not in the default registry — self-hosted endpoints, fine-tuned models, proxies, or new provider releases.

## File Location

TAC looks for models.json at:
1. `~/.tac/agent/models.json` (primary)
...
- **An environment variable name**: `"OPENROUTER_API_KEY"` — TAC resolves it automatically
...
For providers not built into TAC, community extensions add full provider support:

| Extension | Provider | Install |
|-----------|----------|---------|
| `pi-dashscope` | Alibaba DashScope (Qwen3, GLM-5, etc.) | `tac install npm:pi-dashscope` |
