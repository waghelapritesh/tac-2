...
TAC uses git for milestone isolation and sequential commits. The strategy is fully automated — you don't need to manage branches manually.

## Isolation Modes

TAC supports three isolation modes, configured via `git.isolation` in preferences:

| Mode | Working Directory | Branch | Best For |
|------|-------------------|--------|----------|
| `worktree` (default) | `.tac/worktrees/<MID>/` | `milestone/<MID>` | Most projects — full isolation |
...
Work happens directly on your current branch. No worktree, no milestone branch. TAC still commits with conventional commit messages. Use this when file isolation breaks dev tooling (file watchers, hot-reload, etc.).
...
git:
  ...
  commit_docs: true           # commit .tac/ artifacts to git
  manage_gitignore: true      # let TAC manage .gitignore
...
When `collapse_cadence: "slice"` AND `milestone_resquash: true` (the default when cadence is slice), TAC collapses the per-slice commits on main into a single milestone commit at milestone completion. Main history looks identical to `collapse_cadence: "milestone"` — one commit per milestone — but the orphan-window and incremental-conflict benefits of slice cadence are preserved.
...
- Run `/tac forensics` after sessions to see orphan detection and merge telemetry — the **Worktree Telemetry** section reports how often work was stranded, which informs whether slice cadence would help.
...
When a milestone completes, TAC pushes the branch and creates a PR targeting your specified branch. Requires `gh` CLI installed and authenticated.
...
This adds `.tac/` to `.gitignore` entirely. You get structured planning without affecting teammates who don't use TAC.

## Commit Format

Commits use conventional commit format with TAC metadata:
...
TAC automatically recovers from common git issues:
