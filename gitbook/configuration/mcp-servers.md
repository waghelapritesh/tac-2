...
TAC can connect to external MCP (Model Context Protocol) servers for local tools, internal APIs, self-hosted services, or integrations not built in as native extensions.

## Configuration Files

TAC reads MCP config from these project-local paths:

- `.mcp.json` — repo-shared config (safe to commit)
- `.tac/mcp.json` — local-only config (not shared)
...
After adding config, verify from a TAC session:

1. `mcp_servers` — confirms TAC sees the config
...
- Use `.mcp.json` for team-shared servers; `.tac/mcp.json` for machine-local ones
- If a server depends on local paths or personal secrets, keep it in `.tac/mcp.json`
