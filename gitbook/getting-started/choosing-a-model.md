...
TAC auto-selects a default model after you log in to a provider. You can switch models at any time.

## Switch Models

Inside a TAC session, type:
...
If a model is unavailable (provider down, rate limited, credits exhausted), TAC can automatically fall back to another:
...
When enabled, TAC automatically picks cheaper models for simple tasks and reserves expensive ones for complex work:
...
TAC supports 20+ providers out of the box. See [Provider Setup](../configuration/providers.md) for setup instructions:
