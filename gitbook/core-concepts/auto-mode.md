...
TAC reads `.tac/STATE.md`, determines the next unit of work, creates a fresh AI session with all relevant context, and lets the AI execute. When it finishes, TAC reads disk state again and dispatches the next unit. This continues until the milestone is complete.
...
TAC isolates milestone work using one of three modes:
...
If a session dies, the next `/tac auto` reads the surviving session file, synthesizes a recovery briefing from every tool call that made it to disk, and resumes with full context.

In headless mode (`tac headless auto`), crashes trigger automatic restart with exponential backoff (5s → 10s → 30s, up to 3 attempts). Combined with crash recovery, this enables true overnight "fire and forget" execution.

## Provider Error Recovery

TAC handles provider errors automatically:
...
TAC uses sliding-window analysis to detect stuck loops — not just "same unit dispatched twice" but also cycles like A→B→A→B. On detection, TAC retries once with a diagnostic prompt. If it fails again, auto mode stops with details so you can intervene.
...
After a milestone completes, TAC generates a self-contained HTML report in `.tac/reports/` with project summary, progress tree, dependency graph, cost metrics, timeline, and changelog. Generate manually with:

```
/tac export --html
/tac export --html --all    # all milestones
```

## Diagnostic Tools

If auto mode has issues, TAC provides two diagnostic tools:

- **`/tac doctor`** — validates `.tac/` integrity, checks referential consistency, fixes structural issues
- **`/tac forensics`** — full post-mortem debugger with anomaly detection, unit traces, metrics analysis, worktree lifecycle telemetry, and AI-guided investigation
...
`/tac forensics` includes a **Worktree Telemetry** section that summarizes the auto-mode worktree lifecycle across recorded sessions:
...
For per-event detail (specific milestone IDs, timestamps, exit reasons) inspect `.tac/journal/*.jsonl` directly.
