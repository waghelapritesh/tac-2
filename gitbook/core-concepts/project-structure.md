...
TAC uses a three-level hierarchy to break projects into manageable pieces that an AI can execute reliably.
...
All project state lives on disk in a `.tac/` directory at your project root:
...
| `KNOWLEDGE.md` | Rules, patterns, and lessons learned across sessions — TAC reads this at the start of every task |
...
1. **Plan** — TAC scouts the codebase, researches relevant docs, and decomposes the slice into tasks with clear requirements
2. **Execute** — Each task runs in a fresh AI session with focused context
3. **Complete** — TAC writes summaries, generates a UAT script, and commits
...
TAC maintains a knowledge base that persists across sessions. Add rules, patterns, or lessons:

```
/tac knowledge rule "Always use parameterized queries for database access"
/tac knowledge pattern "Service classes go in src/services/"
/tac knowledge lesson "The OAuth flow requires the redirect URL to match exactly"
```
