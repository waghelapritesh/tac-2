...
TAC tracks routing outcomes in `.tac/routing-history.json`. If a tier's failure rate exceeds 20% for a given task type, future classifications are bumped up.

Use `/tac rate` to submit feedback:

```
/tac rate over    # too powerful — use cheaper next time
/tac rate ok      # just right
/tac rate under   # too weak — use stronger next time
```
