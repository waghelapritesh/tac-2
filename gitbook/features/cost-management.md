...
TAC tracks token usage and cost for every unit of work during auto mode. This data powers the dashboard, budget enforcement, and cost projections.

## Viewing Costs

**Dashboard:** Press `Ctrl+Alt+G` or type `/tac status` for real-time cost breakdown.

**Visualizer:** `/tac visualize` → Metrics tab for detailed charts.
...
Once at least two slices have completed, TAC projects the remaining cost:
...
When approaching the budget ceiling, TAC automatically uses cheaper models:
...
- Check `/tac status` after a few slices to see per-slice cost averages
...
- Use `/tac visualize` → Metrics tab to see where your budget is going
