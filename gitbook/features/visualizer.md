...
```
/tac export --html              # current milestone
/tac export --html --all        # all milestones
```

Generates self-contained HTML files in `.tac/reports/` with progress tree, dependency graph, cost charts, timeline, and changelog. All CSS and JS are inlined — no external dependencies. Printable to PDF from any browser.
