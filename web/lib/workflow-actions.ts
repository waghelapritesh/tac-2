...
export function deriveWorkflowAction(input: WorkflowActionInput): WorkflowActionResult {
  ...
  if (
    ...
    projectDetectionKind !== "active-tac" &&
    projectDetectionKind !== "empty-tac"
  ) {
  ...
  if (autoActive && !autoPaused) {
    primary = { label: "Stop Auto", command: "/tac stop", variant: "destructive" }
  } else if (autoPaused) {
    primary = { label: "Resume Auto", command: "/tac auto", variant: "default" }
  } else {
    // Auto is not active
    if (phase === "complete") {
      // All milestones done — surface a distinct "New Milestone" action
      primary = { label: "New Milestone", command: "/tac", variant: "default" }
      isNewMilestone = true
    } else if (phase === "planning") {
      primary = { label: "Plan", command: "/tac", variant: "default" }
    } else if (phase === "executing" || phase === "summarizing") {
      primary = { label: "Start Auto", command: "/tac auto", variant: "default" }
    } else if (phase === "pre-planning" && !hasMilestones) {
      primary = { label: "Initialize Project", command: "/tac", variant: "default" }
    } else if (phase === "blocked") {
      primary = { label: "Blocked", command: "/tac", variant: "default" }
      disabled = true
      disabledReason = "Project is blocked — check blockers"
    } else if (phase === "paused") {
      primary = { label: "Resume", command: "/tac auto", variant: "default" }
    } else if (phase === "validating-milestone") {
      primary = { label: "Validate", command: "/tac", variant: "default" }
    } else if (phase === "completing-milestone") {
      primary = { label: "Complete Milestone", command: "/tac", variant: "default" }
    } else if (phase === "needs-discussion") {
      primary = { label: "Discuss", command: "/tac", variant: "default" }
    } else if (phase === "replanning-slice") {
      primary = { label: "Replan", command: "/tac", variant: "default" }
    } else {
      primary = { label: "Continue", command: "/tac", variant: "default" }
    }

    // Add "Step" secondary when auto is not active (not for new milestone — no step concept there)
    if (primary.command !== "/tac next" && !isNewMilestone) {
      secondaries.push({ label: "Step", command: "/tac next" })
    }
  }
