...
export interface WorkspaceOnboardingCompletionRecord {
  ...
  skippedSteps: string[]
  /** Last step the wizard was on, used by /tac onboarding --resume. */
...
function bootSeedLines(boot: WorkspaceBootPayload): WorkspaceTerminalLine[] {
  const lines = [
    createTerminalLine("system", `TAC web workspace attached to ${boot.project.cwd}`),
...
function createInitialState(): WorkspaceStoreState {
  return {
    ...
    terminalLines: [createTerminalLine("system", "Preparing the live TAC workspace…")],
...
export class TACWorkspaceStore {
    ...
    switch (outcome.kind) {
      ...
      case "view-navigate": {
        ...
        window.dispatchEvent(
          new CustomEvent("tac:navigate-view", { detail: { view: outcome.view } }),
        )
        return outcome
      }
    }
...
const WorkspaceStoreContext = createContext<TACWorkspaceStore | null>(null)

export function TACWorkspaceProvider({ children, store: externalStore }: { children: ReactNode; store?: TACWorkspaceStore }) {
  const [internalStore] = useState(() => new TACWorkspaceStore())
...
function useWorkspaceStore(): TACWorkspaceStore {
  const store = useContext(WorkspaceStoreContext)
  if (!store) {
    throw new Error("useWorkspaceStore must be used within TACWorkspaceProvider")
  }
  return store
}

export function useTACWorkspaceState(): WorkspaceStoreState {
  const store = useWorkspaceStore()
  return useSyncExternalStore(store.subscribe, store.getSnapshot, store.getSnapshot)
}

export function useTACWorkspaceActions(): Pick<
  TACWorkspaceStore,
  | "sendCommand"
  | "submitInput"
  | "clearTerminalLines"
