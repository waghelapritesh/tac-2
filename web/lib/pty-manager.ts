...
import { resolveTacCliEntry } from "../../src/web/cli-entry.ts";
...
const GLOBAL_KEY = "__tac_pty_sessions__" as const;
const CLEANUP_GUARD_KEY = "__tac_pty_cleanup_installed__" as const;
...
function getProjectCwd(): string {
  return process.env.TAC_WEB_PROJECT_CWD || process.cwd();
}
...
function resolveTerminalSpawnSpec(cwd: string, command?: string, commandArgs: string[] = []): TerminalSpawnSpec {
  ...
  if (command === "tac") {
    try {
      const cliEntry = resolveTacCliEntry({
        packageRoot: process.env.TAC_WEB_PACKAGE_ROOT || process.cwd(),
        cwd,
        execPath: process.execPath,
        hostKind: process.env.TAC_WEB_HOST_KIND,
        mode: "interactive",
      ...
      console.warn(
        "[pty] Falling back to PATH-resolved tac:",
...
function getNodePtyCandidateRoots(): string[] {
  ...
  const packageRoot = process.env.TAC_WEB_PACKAGE_ROOT;
  if (packageRoot) {
...
export function getOrCreateSession(sessionId: string, projectCwd?: string, command?: string, commandArgs: string[] = []): PtySession {
  ...
  // Build a clean env — remove TAC-specific vars that would confuse a shell.
  // We preserve them if the command is "tac" because the CLI needs its configuration.
  const cleanEnv: Record<string, string> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && (command === "tac" || !key.startsWith("TAC_WEB_"))) {
      cleanEnv[key] = value;
    }
  }
