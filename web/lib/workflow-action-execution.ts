import type { WorkspaceTerminalLine } from "./tac-workspace-store"
import { getUserMode } from "./use-user-mode"

export type TACViewName = "dashboard" | "power" | "chat" | "roadmap" | "files" | "activity" | "visualize"

export function navigateToTACView(view: TACViewName): void {
  if (typeof window === "undefined") return
  window.dispatchEvent(new CustomEvent("tac:navigate-view", { detail: { view } }))
}
...
export function executeWorkflowActionInPowerMode({
  ...
  navigateToTACView(mode === "vibe-coder" ? "chat" : "power")
}
