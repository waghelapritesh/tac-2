...
function getTacRoot(projectCwd: string): string {
  return join(projectCwd, ".tac");
}

function getRootForMode(mode: RootMode, projectCwd: string): string {
  return mode === "project" ? projectCwd : getTacRoot(projectCwd);
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  const rootParam = (searchParams.get("root") ?? "tac") as RootMode;

  if (rootParam !== "tac" && rootParam !== "project") {
    return Response.json(
      { error: `Invalid root: must be "tac" or "project"` },
      { status: 400 },
    );
  }
  ...
  if (!resolvedPath) {
    const label = rootParam === "project" ? "project root" : ".tac/";
    return Response.json(
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { path: pathParam, content, root: rootParam = "tac" } = body as {
    path?: string;
    content?: unknown;
    root?: string;
  };
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  const { from, to, root: rootParam = "tac" } = body as {
    from?: string;
    to?: string;
    root?: string;
  };
...
export async function PUT(request: Request): Promise<Response> {
  ...
  const { path: pathParam, type = "file", root: rootParam = "tac" } = body as {
    path?: string;
    type?: "file" | "directory";
    root?: string;
  };
