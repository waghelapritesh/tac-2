...
import { filterInitialTacHeader } from "@/lib/initial-tac-header-filter"
...
function TerminalInstance({
  ...
  const initialHeaderSettledRef = useRef(!hideInitialTacHeader)
  ...
  useEffect(() => {
    initialHeaderSettledRef.current = !hideInitialTacHeader
    initialHeaderBufferRef.current = ""
  }, [hideInitialTacHeader, sessionId])
      ...
      es.onmessage = (event) => {
        try {
          ...
          if (msg.type === "connected") {
            ...
            if (hideInitialTacHeader && !initialHeaderSettledRef.current) {
              initialHeaderBufferRef.current += output
              const filtered = filterInitialTacHeader(initialHeaderBufferRef.current)
  ...
  }, [sessionId, command, commandArgs, commandArgsKey, fontSize, hideInitialTacHeader, isDark, projectCwd, sendInput, sendResize])
      ...
      {!hasOutput && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-terminal">
          ...
          <span className="text-xs text-muted-foreground">
            {command ? "Starting TAC…" : "Connecting…"}
          </span>
        </div>
      )}
      <div
...
function deriveProjectScopedSessionId(
  ...
  const base = sessionPrefix ?? (command ? "tac-default" : "default")
      ...
      >
        {tabs.map((tab) => (
          <TerminalInstance
            ...
            isDark={isDark}
            fontSize={fontSize}
            hideInitialTacHeader={hideInitialTacHeader}
