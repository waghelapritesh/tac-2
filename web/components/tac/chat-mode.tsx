...
import {
  useTACWorkspaceState,
  useTACWorkspaceActions,
  buildPromptCommand,
  type CompletedToolExecution,
  type ActiveToolExecution,
  type PendingUiRequest,
  type TurnSegment,
} from "@/lib/tac-workspace-store"
...
/* ─── TAC Action Definitions ─── */

/**
 * Defines every /tac subcommand available in the chat input bar.
...
const TAC_ACTIONS: TACActionDef[] = [
  // ── Top 3 (standalone buttons) ──
  { label: "Discuss",   command: "/tac discuss",   icon: MessageCircle,     description: "Start guided milestone/slice discussion",                    category: "workflow",    disabledDuringAuto: true },
  { label: "Next",      command: "/tac next",      icon: Play,              description: "Execute next task, then pause",                              category: "workflow" },
  { label: "Auto",      command: "/tac auto",      icon: Zap,               description: "Run all queued units continuously",                         category: "workflow" },
  // ── Overflow: Workflow ──
  { label: "Stop",      command: "/tac stop",      icon: Square,            description: "Stop auto-mode gracefully",                                  category: "workflow" },
  { label: "Pause",     command: "/tac pause",     icon: Pause,             description: "Pause auto-mode (preserves state)",                          category: "workflow" },
  // ── Overflow: Visibility ──
  { label: "Status",    command: "/tac status",    icon: BarChart3,         description: "Show progress dashboard",                                    category: "visibility" },
  { label: "Visualize", command: "/tac visualize", icon: LayoutGrid,        description: "Interactive TUI (progress, deps, metrics, timeline)",        category: "visibility" },
  { label: "Queue",     command: "/tac queue",     icon: ListOrdered,       description: "Show queued/dispatched units and execution order",            category: "visibility" },
  { label: "History",   command: "/tac history",   icon: History,           description: "View execution history with cost/phase/model details",        category: "visibility" },
  // ── Overflow: Course correction ──
  { label: "Steer",     command: "/tac steer",     icon: Compass,           description: "Apply user override to active work",                         category: "correction" },
  { label: "Capture",   command: "/tac capture",   icon: PenLine,           description: "Quick-capture a thought to CAPTURES.md",                     category: "correction" },
  { label: "Triage",    command: "/tac triage",    icon: Inbox,             description: "Classify and route pending captures",                        category: "correction",  disabledDuringAuto: true },
  { label: "Skip",      command: "/tac skip",      icon: SkipForward,       description: "Prevent a unit from auto-mode dispatch",                     category: "correction" },
  { label: "Undo",      command: "/tac undo",      icon: Undo2,             description: "Revert last completed unit",                                 category: "correction" },
  // ── Overflow: Knowledge ──
  { label: "Knowledge", command: "/tac knowledge", icon: BookOpen,          description: "Add rule, pattern, or lesson to KNOWLEDGE.md",               category: "knowledge" },
  // ── Overflow: Configuration ──
  { label: "Mode",      command: "/tac mode",      icon: SlidersHorizontal, description: "Set workflow mode (solo/team)",                               category: "config" },
  { label: "Prefs",     command: "/tac prefs",     icon: Settings,          description: "Manage preferences (global/project)",                        category: "config" },
  // ── Overflow: Maintenance ──
  { label: "Doctor",    command: "/tac doctor",    icon: Stethoscope,       description: "Diagnose and repair .tac/ state",                            category: "maintenance" },
  { label: "Export",    command: "/tac export",    icon: FileOutput,        description: "Export milestone/slice results (JSON or Markdown)",           category: "maintenance" },
  { label: "Cleanup",   command: "/tac cleanup",   icon: Trash2,            description: "Remove merged branches or snapshots",                        category: "maintenance" },
  { label: "Remote",    command: "/tac remote",    icon: Globe,             description: "Control remote auto-mode (Slack/Discord)",                    category: "maintenance" },
]

/** Top 3 shown as standalone buttons next to chat input */
const TOP_ACTIONS = TAC_ACTIONS.slice(0, 3)
/** Remaining actions in the overflow menu */
const OVERFLOW_ACTIONS = TAC_ACTIONS.slice(3)

const CATEGORY_LABELS: Record<TACActionDef["category"], string> = {
  workflow: "Workflow",
...
function groupByCategory(actions: TACActionDef[]): Array<{ category: TACActionDef["category"]; label: string; items: TACActionDef[] }> {
  const seen = new Map<TACActionDef["category"], TACActionDef[]>()
 ...
 *
 * All /tac commands dispatch through the main bridge session.
 ...
 *   - sessionStorage key "tac-active-view:<cwd>" equals "chat" when this view is active.
...
export function ChatMode({ className }: { className?: string }) {
  const state = useTACWorkspaceState()
  const { sendCommand } = useTACWorkspaceActions()
...
function ChatModeHeader({ onPrimaryAction, onSecondaryAction }: ChatModeHeaderProps) {
  ...
  // Derive a short TAC state badge label
  const stateBadge = (() => {
...
/**
 * Renders a TAC arrow-key select prompt as a native clickable list.
 *
...
/**
 * Renders a TAC text prompt as a native labeled input field.
 *
...
/**
 * Renders a TAC password/API-key prompt as a native masked input field.
 *
 ...
 * - Overflow menu (⋯) contains all remaining /tac subcommands grouped by category
...
function ChatInputBar({
  ...
  connected: boolean
  onOpenAction?: (action: TACActionDef) => void
}) {
  const autoActive = useTACWorkspaceState().boot?.auto?.active ?? false
  const [value, setValue] = useState("")
    ...
    <div className="flex flex-1 flex-col items-center justify-center text-center py-16">
      ...
      <div className="mt-3 space-y-1">
        ...
        {showSpinner ? (
          ...
          <p className="max-w-xs text-xs text-muted-foreground">
            Connected — waiting for TAC output…
          </p>
        )}
      </div>
    </div>
...
function InlineUiRequest({ request }: { request: PendingUiRequest }) {
  const { respondToUiRequest, dismissUiRequest } = useTACWorkspaceActions()
  const isSubmitting = useTACWorkspaceState().commandInFlight === "extension_ui_response"
...
export function ChatPane({ className, onOpenAction }: ChatPaneProps) {
  const state = useTACWorkspaceState()
  const { submitInput, sendCommand, pushChatUserMessage } = useTACWorkspaceActions()
