...
import { Sidebar, MilestoneExplorer, CollapsedMilestoneSidebar } from "@/components/tac/sidebar"
import { ShellTerminal } from "@/components/tac/shell-terminal"
import { Dashboard } from "@/components/tac/dashboard"
import { Roadmap } from "@/components/tac/roadmap"
import { FilesView } from "@/components/tac/files-view"
import { ActivityView } from "@/components/tac/activity-view"
import { VisualizerView } from "@/components/tac/visualizer-view"
import { StatusBar } from "@/components/tac/status-bar"
import { DualTerminal } from "@/components/tac/dual-terminal"
import { FocusedPanel } from "@/components/tac/focused-panel"
import { OnboardingGate } from "@/components/tac/onboarding-gate"
import { CommandSurface } from "@/components/tac/command-surface"
...
import { ChatMode } from "@/components/tac/chat-mode"
import { ScopeBadge } from "@/components/tac/scope-badge"
import { Badge } from "@/components/ui/badge"
import { ProjectsPanel, ProjectSelectionGate } from "@/components/tac/projects-view"
import { UpdateBanner } from "@/components/tac/update-banner"
...
function viewStorageKey(projectCwd: string): string {
  return `tac-active-view:${projectCwd}`
}
...
function WorkspaceChrome() {
  ...
  const { refreshBoot } = useTACWorkspaceActions()
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      try {
        const stored = localStorage.getItem("tac-sidebar-collapsed")
  ...
  useEffect(() => {
    try {
      localStorage.setItem("tac-sidebar-collapsed", String(sidebarCollapsed))
    } catch {
      // localStorage may be unavailable
    }
  }, [sidebarCollapsed])
  ...
  useEffect(() => {
    ...
    const base = projectLabel ? `TAC - ${projectLabel}` : "TAC"
  ...
  useEffect(() => {
    ...
    window.addEventListener("tac:open-file", handler)
    return () => window.removeEventListener("tac:open-file", handler)
  }, [])

  // Listen for cross-component view navigation events (e.g. /tac visualize dispatch)
  useEffect(() => {
    ...
    window.addEventListener("tac:navigate-view", handler as EventListener)
    return () => window.removeEventListener("tac:navigate-view", handler as EventListener)
  }, [handleViewChange])

  // Listen for projects panel toggle (sidebar icon, or programmatic)
  useEffect(() => {
    const handler = () => setProjectsPanelOpen(true)
    window.addEventListener("tac:open-projects", handler)
    return () => window.removeEventListener("tac:open-projects", handler)
  }, [])
  ...
  const isWelcomeState =
    ...
    detection != null &&
    detection.kind !== "active-tac" &&
    detection.kind !== "empty-tac"
      ...
      <div className="flex h-dvh flex-col items-center justify-center gap-6 bg-background p-8 text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">tac --web</code>.
          </p>
        </div>
      </div>
    )
...
function ProjectAwareWorkspace() {
  ...
  return (
    <TACWorkspaceProvider store={activeStore}>
