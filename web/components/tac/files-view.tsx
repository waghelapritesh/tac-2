...
import { useTACWorkspaceState, buildProjectUrl } from "@/lib/tac-workspace-store"
import { authFetch } from "@/lib/auth"
import { FileContentViewer } from "@/components/tac/file-content-viewer"
import { ChatPane } from "@/components/tac/chat-mode"

type RootMode = "tac" | "project"
...
if (typeof window !== "undefined") {
  window.addEventListener("tac:open-file", (e: Event) => {
...
function storageKey(projectCwd: string, root: RootMode): string {
  return `tac-files-expanded:${root}:${projectCwd}`
}
...
function tabDisplayPath(tab: OpenTab): string {
  return tab.root === "tac" ? `.tac/${tab.path}` : tab.path
}
...
export function FilesView() {
  ...
  const [activeRoot, setActiveRoot] = useState<RootMode>("tac")
  const [leftPanel, setLeftPanel] = useState<LeftPanel>("tree")
  const [tacTree, setTacTree] = useState<FileNode[] | null>(null)
  ...
  const [tacExpanded, setTacExpanded] = useState<Set<string>>(() => loadExpanded(projectCwd, "tac"))
  ...
  useEffect(() => {
    ...
    setTacExpanded(loadExpanded(projectCwd, "tac"))
    setProjectExpanded(loadExpanded(projectCwd, "project"))
  }, [projectCwd])

  const expandedPaths = activeRoot === "tac" ? tacExpanded : projectExpanded
  const setExpandedPaths = activeRoot === "tac" ? setTacExpanded : setProjectExpanded
  ...
  const tree = activeRoot === "tac" ? tacTree : projectTree
  const treeLoaded = activeRoot === "tac" ? tacTree !== null : projectTree !== null
  ...
  const openFileTab = useCallback(async (root: RootMode, path: string) => {
    ...
    const setExpanded = root === "tac" ? setTacExpanded : setProjectExpanded
    setExpanded((prev) => {
  ...
  }, [tacTree, projectTree, fetchTree, openFileTab])
  ...
  const handleNewFile = useCallback((parentDir: string) => {
    ...
    if (parentDir) {
      const setExpanded = activeRoot === "tac" ? setTacExpanded : setProjectExpanded
      setExpanded((prev) => {
        const next = new Set(prev)
  ...
  const handleCopyPath = useCallback((path: string) => {
    const displayPath = activeRoot === "tac" ? `.tac/${path}` : path
  ...
  useEffect(() => {
    ...
    if (!tacTree || openTabs.length > 0 || consumedPendingRef.current) return
    const hasStateMd = tacTree.some((n) => n.name === "STATE.md" && n.type === "file")
    if (hasStateMd) {
      autoSelectedRef.current = true
      void openFileTab("tac", "STATE.md")
    }
  }, [tacTree, openTabs.length, openFileTab])
  ...
  useEffect(() => {
    ...
    for (const tool of newTools) {
      ...
      if (relativePath.startsWith(tacPrefix)) {
        root = "tac"
        relativePath = relativePath.slice(tacPrefix.length)
      }
    ...
    <div className="flex h-full">
      ...
      <div className="flex-shrink-0 border-r border-border overflow-hidden flex flex-col" style={{ width: treeWidth }}>
        ...
        <div className="flex border-b border-border flex-shrink-0">
          <button
            onClick={() => { setLeftPanel("tree"); handleTreeRootChange("tac") }}
            className={cn(
              "flex-1 px-3 py-2 text-xs font-medium transition-colors",
              leftPanel === "tree" && activeRoot === "tac"
        ...
        >
          {loading && !treeLoaded ? (
            ...
            <div className="flex items-center justify-center py-8 text-muted-foreground text-xs">
              {activeRoot === "tac" ? "No .tac/ files found" : "No files found"}
            </div>
          ) : tree ? (
            <>
