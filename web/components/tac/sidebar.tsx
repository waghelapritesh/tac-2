...
export function NavRail({ activeView, onViewChange, isConnecting = false }: NavRailProps) {
  const { openCommandSurface } = useTACWorkspaceActions()
  ...
  return (
    <div className="flex w-12 flex-col items-center gap-1 border-r border-border bg-sidebar py-3">
      ...
      <div className="mt-auto flex flex-col gap-1">
        <button
          onClick={() => window.dispatchEvent(new CustomEvent("tac:open-projects"))}
          disabled={isConnecting}
          className={cn(
            "flex h-10 w-10 items-center justify-center rounded-md transition-colors",
            isConnecting
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Stop the TAC web server?</DialogTitle>
            <DialogDescription>
              This will shut down the server process and close this tab. Run{" "}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">tac --web</code> again to restart.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
export function MilestoneExplorer({ isConnecting = false, width, onCollapse }: { isConnecting?: boolean; width?: number; onCollapse?: () => void }) {
  ...
  const { openCommandSurface, setCommandSurfaceSection, sendCommand } = useTACWorkspaceActions()
  ...
  const openTaskFile = (absolutePath: string | undefined) => {
    ...
    const tacPrefix = `${projectCwd}/.tac/`
    if (!absolutePath.startsWith(tacPrefix)) return
    const relativePath = absolutePath.slice(tacPrefix.length)
    window.dispatchEvent(new CustomEvent("tac:open-file", { detail: { root: "tac", path: relativePath } }))
  }
