/**
 * Smoke: `tac --help` must print usage.
 *
 ...
 * is currently published on npm.  Setting `TAC_SMOKE_BINARY` overrides
...
const binary = process.env.TAC_SMOKE_BINARY || DEFAULT_BINARY;
if (!existsSync(binary)) {
  console.error(
    `Smoke binary not found: ${binary}\n` +
      `Run \`npm run build\` first, or set TAC_SMOKE_BINARY to an existing binary.`,
  );
...
if (!lower.includes("tac")) {
  console.error(`Help output does not contain "tac": "${output}"`);
  process.exit(1);
}
