 ...
 *
 * These tests run AFTER `npm publish` against the installed `tac` binary.
 * They exercise the dispatch loop state machine end-to-end by:
 *
 * 1. Creating real `.tac/` directory structures with milestone artifacts
 * 2. Calling `tac headless query` to verify state derivation
 ...
 *
 * Run from CI pipeline after `npm install -g tac-2@<version>`:
 *   node --experimental-strip-types tests/live-regression/run.ts
 *
 * Or locally:
 *   TAC_SMOKE_BINARY=dist/loader.js node --experimental-strip-types tests/live-regression/run.ts
 */
...
const binary = process.env.TAC_SMOKE_BINARY || "tac";
let passed = 0;
let failed = 0;
...
function tac(
  ...
  try {
    const stdout = execFileSync(
      binary === "tac" ? "tac" : "node",
      binary === "tac" ? args : [binary, ...args],
      {
        cwd,
        encoding: "utf-8",
        timeout: 30_000,
        stdio: ["pipe", "pipe", "pipe"],
        env: { ...process.env, ...env, TAC_NON_INTERACTIVE: "1" },
      },
    );
...
function createTempProject(name: string): string {
  const dir = mkdtempSync(join(tmpdir(), `tac-live-${name}-`));
  gitInitRepo(dir);
  return dir;
}
...
run("headless query returns valid JSON on initialized project", () => {
  ...
  try {
    ...
    mkdirSync(join(tacDir, "milestones"), { recursive: true });

    const result = tac(["headless", "query"], dir);
    assert(
      result.code === 0,
...
run("headless query: milestone with roadmap reports planning phase", () => {
  ...
  try {
    const mDir = join(dir, ".tac", "milestones", "M001");
...
runAsync("tac doctor surfaces actionable guidance about the stale lock", async () => {
  ...
  try {
    ...
    if (!ran) {
      throw new Error("tac doctor command is not available on this binary");
    }
...
run("version skew is detected and named in stderr", () => {
  ...
  try {
    const fakeHome = dir;
    mkdirSync(join(fakeHome, ".tac", "agent"), { recursive: true });
    writeFileSync(
      join(fakeHome, ".tac", "agent", "managed-resources.json"),
      JSON.stringify({ tacVersion: "999.0.0" }),
    );

    const result = tac([], dir, { HOME: fakeHome });
...
// NB: the previous "tac --help works" test was a duplicate of
